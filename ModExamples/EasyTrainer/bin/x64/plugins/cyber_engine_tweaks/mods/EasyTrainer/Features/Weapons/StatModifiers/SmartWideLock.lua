...
SmartWideLock.toggleWideLock = {
    value = false
}
SmartWideLock.handles = {}
SmartWideLock.appliedToWeaponID = nil

local stats = {gamedataStatType.SmartGunAdsLockingAnglePitch, gamedataStatType.SmartGunAdsLockingAngleYaw,
               gamedataStatType.SmartGunAdsTargetableAnglePitch, gamedataStatType.SmartGunAdsTargetableAngleYaw,
               gamedataStatType.SmartGunHipLockingAnglePitch, gamedataStatType.SmartGunHipLockingAngleYaw,
               gamedataStatType.SmartGunHipTargetableAnglePitch, gamedataStatType.SmartGunHipTargetableAngleYaw}
...
function SmartWideLock.Tick(deltaTime)
    ...
    if (Weapon.HasChanged() or #SmartWideLock.handles == 0) and Weapon.IsRangedEquipped() then
        ...
        if newWeaponID ~= SmartWideLock.appliedToWeaponID then
            SmartWideLock.SetWideLock(true)
            Logger.Log(
                string.format("SmartWideLock: Weapon changed > reapplying modifiers to %s", tostring(newWeaponID)))
