...
SmartExtraTargets.toggleExtraTargets = {
    value = false
}
SmartExtraTargets.handles = {}
SmartExtraTargets.appliedToWeaponID = nil

local modifiers = {{gamedataStatType.SmartGunMaxLockedPointsPerTarget, gameStatModifierType.Additive, 2},
...
function SmartExtraTargets.SetExtraTargets(remove)
    ...
    if remove then
        ...
        Logger.Log(string.format("SmartExtraTargets: removed modifiers from %s",
            tostring(SmartExtraTargets.appliedToWeaponID)))
        SmartExtraTargets.handles = {}
        SmartExtraTargets.appliedToWeaponID = nil
    else
...
function SmartExtraTargets.Tick(deltaTime)
    ...
    if (Weapon.HasChanged() or #SmartExtraTargets.handles == 0) and Weapon.IsRangedEquipped() then
        ...
        if newWeaponID ~= SmartExtraTargets.appliedToWeaponID then
            SmartExtraTargets.SetExtraTargets(true)
            Logger.Log(string.format("SmartExtraTargets: Weapon changed > reapplying modifiers to %s",
                tostring(newWeaponID)))
