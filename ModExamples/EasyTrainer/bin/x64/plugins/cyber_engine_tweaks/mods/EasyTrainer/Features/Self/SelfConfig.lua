...
local function CreateEnabledProxy(ref)
    return setmetatable({}, {
...
local function RegisterDynamicOption(key, ref, defaultValue, defaultEnabled)
    ConfigManager.Register("value." .. key, ref, defaultValue)
    ConfigManager.Register("toggle." .. key, CreateEnabledProxy(ref), defaultEnabled)
end
...
local function RegisterAllSelfOptions()
    ...
    -- Persist movement sliders (value + enabled state)
    RegisterDynamicOption("self.movement.speed", Self.StatModifiers.Movement.speedMultiplier, 5.0, false)
    RegisterDynamicOption("self.sandevistan.timescale", Self.StatModifiers.Movement.sandevistanTimeScaleMultiplier, 0.1, false)
    RegisterDynamicOption("self.sandevistan.duration", Self.StatModifiers.Movement.sandevistanDurationMultiplier, 2.5, false)
    RegisterDynamicOption("self.berserk.damage", Self.Berserk.effectEnhance, 1.0, false)
    ConfigManager.Register("toggle.self.berserk.visual", Self.Berserk.visualEnhance, false)
    ...
    ConfigManager.Register("toggle.self.cooldown.groundslam", Self.StatModifiers.Cooldown.toggleGroundSlam, false)
