...
local function TryLoadModules()
    if Session.IsLoaded() and not modulesLoaded then
        ...
        if not (Utils and SelfFeature and AutoTeleport and WorldWeather and WorldTime and SelfTick and Weapon and
            Vehicle and MainMenu) then
            ok = false
        end
...
Event.RegisterInit(function()
    ...
    Event.Observe("PlayerPuppet", "OnTakeControl", function(_)
        if modulesLoaded then
            SelfFeature.Berserk.InvalidateEffectCache()
        end
    end)

    Event.ObserveAfter("RipperDocGameController", "EquipCyberware", function(_, itemData)
        if not (modulesLoaded and itemData) then
            return
        end
        ...
        local tdb = TDBID.ToStringDEBUG(id.tdbid)
        -- 仅在换装 SystemReplacementCW/Berserk 相关时清缓存，可按需调整匹配条件
        if tdb and (tdb:find("SystemReplacementCW", 1, true) or tdb:find("Berserk", 1, true)) then
            if SelfFeature and SelfFeature.Berserk and SelfFeature.Berserk.InvalidateEffectCache then
                SelfFeature.Berserk.InvalidateEffectCache()
            end
        end
    end)
...
Event.RegisterUpdate(function(dt)
    ...
    if GameState.isPaused ~= lastState.isPaused then
        if GameState.isPaused then
            -- Paused
        else
            -- Unpaused
        end
        lastState.isPaused = GameState.isPaused
    end
...
Event.RegisterDraw(function()
    ...
    if ImGui.Begin("EasyTrainer",
        ImGuiWindowFlags.NoScrollbar + ImGuiWindowFlags.NoScrollWithMouse + ImGuiWindowFlags.NoTitleBar) then
