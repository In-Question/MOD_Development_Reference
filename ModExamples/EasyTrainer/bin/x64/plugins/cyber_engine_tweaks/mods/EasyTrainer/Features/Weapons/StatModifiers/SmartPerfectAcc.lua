...
SmartPerfectAcc.togglePerfectAcc = {
    value = false
}
SmartPerfectAcc.handles = {}
SmartPerfectAcc.appliedToWeaponID = nil

local modifiers = {{gamedataStatType.SmartGunHitProbability, gameStatModifierType.Additive, 1},
...
function SmartPerfectAcc.SetPerfectAcc(remove)
    ...
    if remove then
        ...
        Logger.Log(string.format("SmartPerfectAcc: removed modifiers from %s",
            tostring(SmartPerfectAcc.appliedToWeaponID)))
        SmartPerfectAcc.handles = {}
        SmartPerfectAcc.appliedToWeaponID = nil
    else
...
function SmartPerfectAcc.Tick(deltaTime)
    ...
    if (Weapon.HasChanged() or #SmartPerfectAcc.handles == 0) and Weapon.IsRangedEquipped() then
        ...
        if newWeaponID ~= SmartPerfectAcc.appliedToWeaponID then
            SmartPerfectAcc.SetPerfectAcc(true)
            Logger.Log(string.format("SmartPerfectAcc: Weapon changed > reapplying modifiers to %s",
