...
Cooldown.toggleGroundSlam = { value = false }
...
Cooldown.groundSlamHandle = nil
function Cooldown.SetGroundSlamCooldown(remove)
    if remove then
        if Cooldown.groundSlamHandle then StatModifiers.Remove(Cooldown.groundSlamHandle) end
    else
        Cooldown.groundSlamHandle = StatModifiers.Create(gamedataStatType.GroundSlamCooldownModifier, gameStatModifierType.Multiplier, 0.01)
        StatModifiers.Apply(Cooldown.groundSlamHandle)
    end
end
