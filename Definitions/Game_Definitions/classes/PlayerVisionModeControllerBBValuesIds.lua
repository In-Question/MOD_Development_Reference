---@meta
---@diagnostic disable

---@class PlayerVisionModeControllerBBValuesIds
---@field kerenzikov gamebbScriptID_Int32
---@field restrictedScene gamebbScriptID_Int32
---@field dead gamebbScriptID_Int32
---@field takedown gamebbScriptID_Int32
---@field deviceTakeover gamebbScriptID_EntityID
---@field braindanceFPP gamebbScriptID_Bool
---@field braindanceActive gamebbScriptID_Bool
---@field veryHardLanding gamebbScriptID_Int32
---@field isBriefingActive gamebbScriptID_Bool
PlayerVisionModeControllerBBValuesIds = {}

---@return PlayerVisionModeControllerBBValuesIds
function PlayerVisionModeControllerBBValuesIds.new() return end

---@param props table
---@return PlayerVisionModeControllerBBValuesIds
function PlayerVisionModeControllerBBValuesIds.new(props) return end

