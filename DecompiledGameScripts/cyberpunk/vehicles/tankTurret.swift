...
public class TankTurretComponent extends ScriptableComponent {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Attack_GameEffect")
  ...
  private final func Shoot(slotComponent: ref<SlotComponent>, slotName: CName) -> Void {
    ...
    let validSlotPosition: Bool = slotComponent.GetSlotTransform(slotName, slotTransform);
    ...
    if validSlotPosition {
      ...
      position = WorldPosition.ToVector4(WorldTransform.GetWorldPosition(this.GetEntity().GetWorldTransform()));
    };
