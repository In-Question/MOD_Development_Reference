...
public class MonoDisc extends BaseProjectile {
  ...
  private final func NoTargetLaunch(const localToWorld: script_ref<Matrix>, startVelocity: Float, distance: Float, sideOffset: Float, height: Float) -> Void {
    ...
    fakeTargetPosition = Matrix.GetTranslation(Deref(localToWorld)) + Matrix.GetAxisY(Deref(localToWorld)) * distance - Matrix.GetAxisX(Deref(localToWorld)) * sideOffset + Matrix.GetAxisZ(Deref(localToWorld)) * height;
  ...
  private final func IsPlayerInKerenzikov() -> Bool {
    ...
    if IsDefined(this.m_blackboard) {
      isPlayerInKerenzikov = this.GetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.Locomotion) == 3;
    };
