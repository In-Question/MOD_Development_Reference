...
public class SpiderbotHeavyProjectile extends BaseProjectile {
  ...
  protected func Explode(const hitInstance: script_ref<gameprojectileHitInstance>) -> Void {
    let effect: ref<EffectInstance>;
    let damageHitEvent: ref<gameprojectileHitEvent> = new gameprojectileHitEvent();
    ArrayPush(damageHitEvent.hitInstances, Deref(hitInstance));
    ...
    EffectData.SetVector(effect.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.position, Deref(hitInstance).position);
