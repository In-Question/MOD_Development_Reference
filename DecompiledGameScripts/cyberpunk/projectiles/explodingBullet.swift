  ...
  protected final func Explode(opt position: Vector4) -> Void {
    ...
    if !this.hasExploded {
      ...
      if IsDefined(this.m_attack) {
        explosionAttackRecord = IAttack.GetExplosiveHitAttack(this.m_attack_record);
        if IsDefined(explosionAttackRecord) {
          ...
          Attack_GameEffect.SpawnExplosionAttack(explosionAttackRecord, this.m_weapon, this.m_user, this, position, 0.05);
        };
      };
      GameObjectEffectHelper.BreakEffectLoopEvent(this, n"trail");
    };
