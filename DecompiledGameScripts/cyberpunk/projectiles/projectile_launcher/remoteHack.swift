...
public class NanoWireProjectile extends BaseProjectile {
  ...
  protected final func SetNanoWireProjectileLaunchMode() -> Void {
    ...
    if isPrimary {
      ...
      if !isPrimary {
        ...
        this.m_launchMode = ELaunchMode.None;
      };
    };
  }
  ...
  protected cb func OnProjectileInitialize(eventData: ref<gameprojectileSetUpEvent>) -> Bool {
    ...
    this.m_maxAttackRange = GameInstance.GetStatsSystem(this.m_user.GetGame()).GetStatValue(Cast<StatsObjectID>(this.m_user.GetEntityID()), gamedataStatType.Range);
