...
public class BaseProjectile extends ItemObject {
  ...
  protected final func ProjectileBreachDevice(const hitInstance: script_ref<gameprojectileHitInstance>, value: Float) -> Void {
  ...
  protected final func CreateProjectileDeviceBreachEvent(const hitInstance: script_ref<gameprojectileHitInstance>, value: Float) -> Void {
  ...
  protected final func SpawnLandVFXs(fx: FxResource, fxposition: Vector4) -> ref<FxInstance> {
  ...
  protected final func GetLeftHandCyberwareAction(user: wref<GameObject>) -> EActionType {
    let actionType: EActionType;
    let quickAction: Bool = ProjectileHelper.GetPSMBlackboardIntVariable(user, GetAllBlackboardDefs().PlayerStateMachine.LeftHandCyberware) == 8;
    let chargeAction: Bool = ProjectileHelper.GetPSMBlackboardIntVariable(user, GetAllBlackboardDefs().PlayerStateMachine.LeftHandCyberware) == 9;
    if quickAction {
  ...
  protected final func GetProjectileTweakDBFloatParameter(const param: script_ref<String>) -> Float {
    return TweakDBInterface.GetFloat(TDBID.Create("projectile." + this.m_tweakDBPath + "." + param), -1.00);
  }

  public final func GetInstigator() -> wref<GameObject> {
    return this.m_user;
  }
  ...
  protected final func TriggerSingleStimuli(const hitInstance: script_ref<gameprojectileHitInstance>, stimToSend: gamedataStimType) -> Void {
  ...
  protected final func TriggerActiveStimuliWithLifetime(const hitInstance: script_ref<gameprojectileHitInstance>, stimToSend: gamedataStimType, lifetime: Float, radius: Float) -> Void {
    let broadcaster: ref<StimBroadcasterComponent> = this.GetStimBroadcasterComponent();
    if IsDefined(broadcaster) {
  ...
  protected final func ProjectileHitAoE(const hitInstance: script_ref<gameprojectileHitInstance>, attackRadius: Float, opt attackRecord: ref<Attack_Record>) -> Void {
    let aoeData: ProjectileHitAoEData;
    aoeData.source = this;
    aoeData.instigator = this.m_user;
    aoeData.position = Deref(hitInstance).position;
