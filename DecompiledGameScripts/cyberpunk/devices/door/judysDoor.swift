...
public class JudysDoor extends InteractiveDevice {
    ...
    if super.UpdateDeviceState(isDelayed) {
      if this.IsInitialized() {
        if NotEquals(this.m_isVisuallyOpened, this.GetDevicePS().IsOpen()) {
          this.MoveDoor(this.GetDevicePS().IsOpen(), false, false);
        };
  ...
  protected func RestoreDeviceState() -> Void {
    this.MoveDoor(this.GetDevicePS().IsOpen(), true, true);
  ...
  protected func ApplyReplicatedState(const state: ref<DeviceReplicatedState>) -> Void {
    ...
    if NotEquals(doorState.m_isOpen, this.m_isVisuallyOpened) {
      this.MoveDoor(doorState.m_isOpen, doorState.m_wasImmediateChange, false);
    };
  }
  ...
  private final func MoveDoor(shouldBeOpened: Bool, immediate: Bool, force: Bool, opt movingSpeedMultiplier: Float) -> Bool {
    let broadcaster: ref<StimBroadcasterComponent>;
    let evt: ref<DoorTriggerDelayedEvent>;
    ...
    this.RefreshInteraction(gamedeviceRequestType.Direct, GetPlayer(this.GetGame()));
    this.NotifyParents();
    if this.m_isVisuallyOpened && Equals(this.GetDoorTriggerSide(this.GetPlayerEntity()), EDoorTriggerSide.OUTSIDE) {
      if !this.IsSomeoneAuthorizedNearby() || force {
        evt = new DoorTriggerDelayedEvent();
        evt.activator = GetPlayer(this.GetGame());
        evt.force = force;
        GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evt, this.m_automaticCloseDelay);
      };
    };
    return true;
  }
