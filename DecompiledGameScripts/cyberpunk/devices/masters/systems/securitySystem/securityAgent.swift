...
public struct Agent {
  ...
  @runtimeProperty("unsavable", "true")
  public persistent let reprimands: [ReprimandData];

  @runtimeProperty("unsavable", "true")
  public persistent let supportingAgents: [PersistentID];

  @runtimeProperty("unsavable", "true")
  public persistent let areas: [DeviceLink];
  ...
  public final static func Construct(link: DeviceLink, const areas: script_ref<[ref<SecurityAreaControllerPS>]>) -> Agent {
    let agent: Agent;
    let i: Int32;
    if !DeviceLink.IsValid(link) || ArraySize(Deref(areas)) == 0 {
      return agent;
    };
    agent.link = link;
    ...
    while i < ArraySize(Deref(areas)) {
      ArrayPush(agent.areas, DeviceLink.Construct(Deref(areas)[i]));
      i += 1;
    };
    return agent;
  }
  ...
  public final static func RemoveArea(self: script_ref<Agent>, const remainingAreas: script_ref<[ref<SecurityAreaControllerPS>]>) -> Void {
    let i: Int32;
    Agent.SetIncomingFilter(self, remainingAreas);
    ArrayClear(Deref(self).areas);
    i = 0;
    while i < ArraySize(Deref(remainingAreas)) {
      ArrayPush(Deref(self).areas, DeviceLink.Construct(Deref(remainingAreas)[i]));
      i += 1;
    };
  }

  public final static func SetIncomingFilter(self: script_ref<Agent>, const areas: script_ref<[ref<SecurityAreaControllerPS>]>) -> Void {
    Deref(self).incomingFilter = EFilterType.ALLOW_ALL;
    let i: Int32 = 0;
    while i < ArraySize(Deref(areas)) {
      Agent.SetIncomingFilter(self, Deref(areas)[i]);
      i += 1;
    };
  }
  ...
  public final static func GetAreas(const self: script_ref<Agent>, areas: script_ref<[DeviceLink]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).areas) {
      ArrayPush(Deref(areas), Deref(self).areas[i]);
      i += 1;
    };
  }

  public final static func IsEligible(const self: script_ref<Agent>, state: ESecuritySystemState, const breachedAreas: script_ref<[ref<SecurityAreaControllerPS>]>, inputsOutgoingFilter: EFilterType, out breachOrigin: EBreachOrigin) -> Bool {
    let k: Int32;
    breachOrigin = EBreachOrigin.EXTERNAL;
    let i: Int32 = 0;
    while i < ArraySize(Deref(breachedAreas)) {
      k = 0;
      while k < ArraySize(Deref(self).areas) {
        if Deref(breachedAreas)[i] == Deref(self).areas[k] {
    ...
    if Equals(inputsOutgoingFilter, EFilterType.ALLOW_ALL) {
      if Equals(Deref(self).incomingFilter, EFilterType.ALLOW_ALL) {
        return true;
      };
      if Equals(Deref(self).incomingFilter, EFilterType.ALLOW_COMBAT_ONLY) && Equals(state, ESecuritySystemState.COMBAT) {
        return true;
      };
      return false;
    };
  ...
  public final static func IsEligibleToShareData(const self: script_ref<Agent>, state: ESecuritySystemState, const breachedAreas: script_ref<[ref<SecurityAreaControllerPS>]>, inputsOutgoingFilter: EFilterType) -> Bool {
    let i: Int32;
    let k: Int32;
    if Equals(inputsOutgoingFilter, EFilterType.ALLOW_ALL) {
      if Equals(Deref(self).incomingFilter, EFilterType.ALLOW_ALL) {
  ...
  public final static func HasSupport(const self: script_ref<Agent>) -> Bool {
    return ArraySize(Deref(self).supportingAgents) > 0;
  }
  ...
  public final static func IsPerformingReprimand(const self: script_ref<Agent>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).reprimands) {
      if Deref(self).reprimands[i].isActive {
        return true;
      };
      i += 1;
    };
  ...
  public final static func IsPerformingReprimandAgainst(const self: script_ref<Agent>, target: EntityID) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).reprimands) {
      if Deref(self).reprimands[i].receiver == target && Deref(self).reprimands[i].isActive {
        return true;
      };
      i += 1;
    };
  ...
  public final static func GetReprimandReceiver(const self: script_ref<Agent>) -> EntityID {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).reprimands) {
      if Deref(self).reprimands[i].isActive {
        return Deref(self).reprimands[i].receiver;
      };
      i += 1;
    };
  ...
  public final static func GetReprimandsCount(const self: script_ref<Agent>, target: EntityID) -> Int32 {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).reprimands) {
      if Deref(self).reprimands[i].receiver == target {
        return Deref(self).reprimands[i].count;
      };
      i += 1;
    };
    return 0;
