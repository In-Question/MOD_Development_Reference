...
public class SecSystemDebugger extends ScriptableSystem {
  ...
  @runtimeProperty("unsavable", "true")
  private persistent let callstack: [CName];

  @runtimeProperty("unsavable", "true")
  private persistent let ids: [Uint32];
  ...
  private final func Update(const sys: ref<SecuritySystemControllerPS>, time: Float, instructionsAttached: Bool, inputAttached: Bool, trace: CName, opt instruction: EReprimandInstructions, opt recentInput: ref<SecuritySystemInput>) -> Void {
    ...
    ArrayPush(this.ids, this.sysState = dvs.DrawText(new Vector4(500.00, 520.00, 0.00, 0.00), "STATE: " + EnumValueToString("ESecuritySystemState", Cast<Int64>(EnumInt(sys.GetSecurityState()))), gameDebugViewETextAlignment.Left, color));
    ...
    if IsDefined(area) {
      ...
      ArrayPush(this.ids, this.mostDangerousArea = dvs.DrawText(new Vector4(500.00, 540.00, 0.00, 0.00), "AREA: " + EnumValueToString("ESecurityAreaType", Cast<Int64>(EnumInt(area.GetSecurityAreaType()))), gameDebugViewETextAlignment.Left, color));
    } else {
    ...
    if NotEquals(blackReason, BlacklistReason.UNINITIALIZED) {
      ArrayPush(this.ids, this.blacklistReason = dvs.DrawText(new Vector4(500.00, 560.00, 0.00, 0.00), "BLACKLISTED FOR: " + EnumValueToString("BlacklistReason", Cast<Int64>(EnumInt(blackReason))), gameDebugViewETextAlignment.Left, color));
    } else {
    ...
    if this.instructionSet {
      ...
      ArrayPush(this.ids, this.repInstruction = dvs.DrawText(new Vector4(650.00, 580.00, 0.00, 0.00), extraTag + "INSTRUCTION: " + EnumValueToString("EReprimandInstructions", Cast<Int64>(EnumInt(this.lastInstruction))), gameDebugViewETextAlignment.Left, SColor.Yellow()));
    };
    ...
    if this.inputSet {
      ...
      ArrayPush(this.ids, this.input = dvs.DrawText(new Vector4(500.00, 600.00, 0.00, 0.00), extraTag + "LAST INPUT: " + EnumValueToString("ESecurityNotificationType", Cast<Int64>(EnumInt(this.lastInput))), gameDebugViewETextAlignment.Left, SColor.Yellow()));
    };
...
public class PlayerSpotted extends Event {
  ...
  private let agentAreas: [ref<SecurityAreaControllerPS>];

  public final static func Construct(isReporterNPC: Bool, owner: PersistentID, doSee: Bool, const areas: script_ref<[ref<SecurityAreaControllerPS>]>) -> ref<PlayerSpotted> {
  ...
  public final const func GetAgentAreas() -> [ref<SecurityAreaControllerPS>] {
...
public struct NPCReference {
  ...
  public final static func IsValid(self: NPCReference) -> Bool {
    return GlobalNodeRef.IsDefined(ResolveNodeRef(self.communitySpawner, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot()))) && IsNameValid(self.entryName);
  }

  public final static func GetSpawnerEntityID(self: NPCReference) -> EntityID {
    return Cast<EntityID>(ResolveNodeRef(self.communitySpawner, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot())));
  }
}
...
public class SuppressSecuritySystemStateChange extends Event {
  ...
  @runtimeProperty("tooltip", "From now on security system can be changed ONLY via quest blocks. ALL gameplay events will be discarded until this flag is set to false.")
...
public class AuthorizePlayerInSecuritySystem extends Event {
  ...
  @runtimeProperty("tooltip", "Should this event authorize player, or remove him from the list of authorized users. ESL is ingored if authorize == FALSE")
  @default(AuthorizePlayerInSecuritySystem, true)
  public edit let authorize: Bool;

  @runtimeProperty("tooltip", "If this is turned to FALSE player will be able to use devices in given SecuritySystem (doors/computers etc) however if he is blacklisted he will still be attacked by turrets and NPCs. We support case where player is both blacklisted and authorized at the same time")
  @default(AuthorizePlayerInSecuritySystem, true)
  public edit let forceRemoveFromBlacklist: Bool;

  @runtimeProperty("tooltip", "how strong should the authorization be. If you have two areas with different ESL (i.e ESL_1 and ESL_2) with this property you can give player authorization in one area but not the other. ESL_4 == TOTAL AUTHORIZATION EVERYWHERE")
...
public class BlacklistPlayer extends Event {
  ...
  @runtimeProperty("tooltip", "Should this event blacklist player, or removing him from the blacklist. Reason is ingored if blacklist == FALSE")
  @default(BlacklistPlayer, true)
  public edit let blacklist: Bool;

  @runtimeProperty("tooltip", "Determine why target is blacklisted. It will influence how Security System resolves response against player in the future. Trespasser will be treated differently than someone blacklisted for starting a combat.")
  @default(BlacklistPlayer, BlacklistReason.COMBAT)
  public edit let reason: BlacklistReason;

  @runtimeProperty("tooltip", "Player's authorization will be void. Keep in mind this may remove authorization that player has granted himself via gameplay if it was possible")
...
public class SuppressNPCInSecuritySystem extends Event {
  ...
  @runtimeProperty("tooltip", "This NPC will NO LONGER receive events FROM security system")
  public edit let suppressIncomingEvents: Bool;

  @runtimeProperty("tooltip", "This NPC will NO LONGER send events TO security system")
...
public class QuestChangeSecuritySystemAttitudeGroup extends Event {
  ...
  @runtimeProperty("tooltip", "Security System and all of its DEVICES (not NPCs) will change attitude group to this one")
...
public class QuestAddTransition extends Event {
  ...
  @runtimeProperty("tooltip", "This transition will be added to  the list of already exisitng transtions. It will not be executed immediately.")
...
public class QuestRemoveTransition extends Event {
  ...
  @runtimeProperty("rangeMax", "23")
  @runtimeProperty("rangeMin", "0")
  @runtimeProperty("tooltip", "Pick hour. If any transition will be found at this hour it will be removed.")
...
public class QuestExecuteTransition extends Event {
  ...
  @runtimeProperty("tooltip", "Perform transition immediately. Transition Hour property is ignored in this event")
  public edit let transition: AreaTypeTransition;
...
public class TakeOverSecuritySystem extends ActionBool {
  ...
  public final static func IsDefaultConditionMet(device: ref<ScriptableDeviceComponentPS>, const context: script_ref<GetActionsContext>) -> Bool {
    if TakeOverSecuritySystem.IsAvailable(device) && TakeOverSecuritySystem.IsClearanceValid(Deref(context).clearance) {
      return true;
    };
    return false;
  }
  ...
  public final static func IsClearanceValid(requesterClearancer: ref<Clearance>) -> Bool {
    if Clearance.IsInRange(requesterClearancer, 10) {
      return true;
    };
    return false;
  }
...
public class FullSystemRestart extends ActionBool {
  ...
  public final static func IsDefaultConditionMet(device: ref<ScriptableDeviceComponentPS>, const context: script_ref<GetActionsContext>) -> Bool {
    if FullSystemRestart.IsAvailable(device) && FullSystemRestart.IsClearanceValid(Deref(context).clearance) {
      return true;
    };
    return false;
  }
...
public class SecuritySystemStatus extends BaseDeviceStatus {
  ...
  public final static func IsDefaultConditionMet(device: ref<ScriptableDeviceComponentPS>, const context: script_ref<GetActionsContext>) -> Bool {
    if SecuritySystemStatus.IsAvailable(device) && SecuritySystemStatus.IsClearanceValid(Deref(context).clearance) {
      return true;
    };
    return false;
  }
...
public class SecuritySystemInput extends SecurityAreaEvent {
  ...
  private let m_customRecipientsList: [EntityID];
  ...
  public final func SetProperties(lkp: Vector4, whoBreached: ref<GameObject>, reporter: wref<SharedGameplayPS>, type: ESecurityNotificationType, canDoReprimand: Bool, shouldLeadReprimand: Bool, id: Int32, const customRecipients: script_ref<[EntityID]>, isSharingRestricted: Bool) -> Void {
    this.SetProperties(lkp, whoBreached, reporter, type, canDoReprimand, shouldLeadReprimand);
    this.m_id = id;
    this.m_customRecipientsList = Deref(customRecipients);
  ...
  public final const func GetCustomRecipientsList() -> [EntityID] {
  ...
  public final func AttachCustomRecipientsList(const list: script_ref<[EntityID]>) -> Void {
    this.m_customRecipientsList = Deref(list);
  }
...
public struct SecurityAccessLevelEntry {
  ...
  @runtimeProperty("unsavable", "true")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Keycards.Keycard")
...
public struct OutputValidationDataStruct {
  ...
  public let breachedAreas: [PersistentID];

  public final static func Construct(evt: ref<SecuritySystemInput>, currentReprimender: EntityID, const breachedAreas: script_ref<[PersistentID]>) -> OutputValidationDataStruct {
    ...
    ovd.breachedAreas = Deref(breachedAreas);
    return ovd;
  }

  public final static func IsDuplicated(const self: script_ref<OutputValidationDataStruct>, evt: ref<SecuritySystemInput>, currentReprimender: EntityID, const currentlyBreachedAreas: script_ref<[PersistentID]>) -> Bool {
    let i: Int32;
    let k: Int32;
    let securityArea: SecurityAreaData;
    if evt.GetWhoBreached().GetEntityID() != Deref(self).targetID {
      return false;
    };
    if NotEquals(evt.GetNotifierHandle().GetID(), Deref(self).agentID) {
      return false;
    };
    if currentReprimender != Deref(self).reprimenderID {
      return false;
    };
    if NotEquals(evt.GetNotificationType(), Deref(self).eventType) {
      return false;
    };
    if NotEquals(securityArea.id, Deref(self).eventReportedFromArea) {
      return false;
    };
    if ArraySize(Deref(self).breachedAreas) != ArraySize(Deref(currentlyBreachedAreas)) {
      return false;
    };
    i = 0;
    while i < ArraySize(Deref(self).breachedAreas) {
      k = 0;
      while k < ArraySize(Deref(currentlyBreachedAreas)) {
        if NotEquals(Deref(self).breachedAreas[i], Deref(currentlyBreachedAreas)[i]) {
          return false;
        };
        k += 1;
      };
...
public static func OperatorSubtract(level1: ESecurityAccessLevel, value: Int32) -> ESecurityAccessLevel {
  ...
  if outcomeInt > Cast<Int32>(EnumGetMax(n"ESecurityAccessLevel")) {
    outcomeInt = Cast<Int32>(EnumGetMax(n"ESecurityAccessLevel"));
  };
  outcome = IntEnum<ESecurityAccessLevel>(outcomeInt);
  return outcome;
}
