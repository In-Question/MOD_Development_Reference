...
public class SecurityAreaEvent extends ActionBool {
  ...
  public final func SetAreaData(const areaData: script_ref<SecurityAreaData>) -> Void {
    this.m_securityAreaData = Deref(areaData);
  }
...
public class SecurityAreaController extends MasterController {
  ...
  public const func GetPS() -> ref<SecurityAreaControllerPS> {
    return this.GetBasePS() as SecurityAreaControllerPS;
  }
}
...
public struct OutputPersistentData {
  ...
  public final static func IsValid(const self: script_ref<OutputPersistentData>) -> Bool {
    if EntityID.IsDefined(Deref(self).m_whoBreached) || EntityID.IsDefined(Deref(self).m_objectOfInterest) {
      return true;
    };
    return false;
  }
...
public class SecurityAreaControllerPS extends MasterControllerPS {
  ...
  private persistent let m_usersInPerimeter: [AreaEntry];

  private persistent let m_isPlayerInside: Bool;

  @runtimeProperty("tooltip", "Utilized during Authorization only. Determines the list of passwords and keycards that are working in this area. Passwords & Keycards are specified in Security System")
  private let m_securityAccessLevel: ESecurityAccessLevel;

  @runtimeProperty("tooltip", "This determines what actions are legal inside this area as well what type of response will be initiated as a countermeasure")
  @default(SecurityAreaControllerPS, ESecurityAreaType.DANGEROUS)
  private persistent let m_securityAreaType: ESecurityAreaType;

  @runtimeProperty("tooltip", "[ OPTIONAL ] This determines what kind of events can get out of given area or get inside given area and its agents. By default all events are received and broadcasted")
  private let m_eventsFilters: EventsFilters;

  @runtimeProperty("tooltip", "[ OPTIONAL ] If you want your Security Area to change its type regularly (i.e: Store / Alley (day/night) / Arasaka Lobby) here you can control it. TIP: You can perform transitions using SecurityAreaManager Quest Block :) TIP2: UPON TRANSITION, UI PROMPT ABOUT BEING IN AREA X MAY SHOW UP! TIP3: ORDER IS NOT IMPORANT!")
  private persistent const let m_areaTransitions: [AreaTypeTransition];

  private persistent let m_runtimeTransitions: [AreaTypeTransition];
  ...
  protected func GameAttached() -> Void {
    this.CopyAreaTransitions();
    this.RegisterTimeSystemListeners(this.GetOwnerEntityWeak());
    if this.AreThereAnyValidTransitions() {
      this.ResolveSecurityAreaType();
    };
    this.UpdateMiniMapRepresentation();
  }

  private final func CopyAreaTransitions() -> Void {
    let alreadyCopied: Bool;
    let i: Int32;
    let k: Int32;
    let transitionsToCopy: array<AreaTypeTransition>;
    if ArraySize(this.m_areaTransitions) > 0 {
      i = 0;
      while i < ArraySize(this.m_areaTransitions) {
        alreadyCopied = false;
        k = 0;
        while k < ArraySize(this.m_runtimeTransitions) {
          if this.m_areaTransitions[i].transitionHour == this.m_runtimeTransitions[k].transitionHour {
            alreadyCopied = true;
            break;
          };
          k += 1;
        ...
        if alreadyCopied {
          ...
          ArrayPush(transitionsToCopy, this.m_areaTransitions[i]);
        };
        i += 1;
      };
      i = 0;
      while i < ArraySize(transitionsToCopy) {
        ArrayPush(this.m_runtimeTransitions, transitionsToCopy[i]);
        i += 1;
      };
    };
  }
  ...
  public final func RegisterTimeSystemListeners(entity: ref<Entity>) -> Void {
    ...
    i = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      transitionEvent = new Transition();
      hour = GameTime.MakeGameTime(0, this.m_runtimeTransitions[i].transitionHour, 0, 0);
      this.m_runtimeTransitions[i].listenerID = GameInstance.GetTimeSystem(this.GetGameInstance()).RegisterListener(entity, transitionEvent, hour, -1, true);
      transitionEvent.listenerID = this.m_runtimeTransitions[i].listenerID;
      i += 1;
    };
  }
  ...
  public final func UnregisterTimeSystemListeners() -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      GameInstance.GetTimeSystem(this.GetGameInstance()).UnregisterListener(this.m_runtimeTransitions[i].listenerID);
      this.m_runtimeTransitions[i].listenerID = 0u;
      i += 1;
    };
  }
  ...
  private final func ResolveSecurityAreaType() -> Void {
    ...
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      if this.m_runtimeTransitions[i].transitionHour < hourOfTheDay {
        if cachedTransitionHour > this.m_runtimeTransitions[i].transitionHour {
          cachedTransitionHour = this.m_runtimeTransitions[i].transitionHour;
          mostRelevantTransition = this.m_runtimeTransitions[i].listenerID;
        };
      };
      i += 1;
    };
  ...
  public final func ApplyTransition(listenerIndex: Uint32) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      if this.m_runtimeTransitions[i].listenerID == listenerIndex {
        return this.ApplyTransition(this.m_runtimeTransitions[i], true);
      };
      i += 1;
    };
    return false;
  ...
  private final func ApplyTransition(transition: AreaTypeTransition, isScheduled: Bool) -> Bool {
    ...
    if Equals(this.m_securityAreaType, transition.transitionTo) || transition.locked {
      return false;
    };
  ...
  private final func PostponeAreaDisabling(const turrets: script_ref<[ref<SecurityTurretControllerPS>]>) -> Void {
    let evt: ref<PendingSecuritySystemDisable>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(turrets)) {
      evt = new PendingSecuritySystemDisable();
      evt.isPending = true;
      this.QueuePSEvent(Deref(turrets)[i], evt);
      i += 1;
    };
  ...
  private final const func GetTurrets(turrets: script_ref<[ref<SecurityTurretControllerPS>]>) -> Void {
  ...
  private final const func IsDisableAllowed(turrets: script_ref<[ref<SecurityTurretControllerPS>]>) -> Bool {
    let i: Int32;
  ...
  public final const func AreThereAnyValidTransitions() -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      if !this.m_runtimeTransitions[i].locked {
        return true;
      };
      i += 1;
    };
  ...
  public final func OnQuestAddTransition(evt: ref<QuestAddTransition>) -> EntityNotificationType {
    ...
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      if this.m_runtimeTransitions[i].transitionHour == evt.transition.transitionHour {
        if !this.m_runtimeTransitions[i].locked {
          return EntityNotificationType.DoNotNotifyEntity;
        };
        this.m_runtimeTransitions[i].locked = false;
        alreadyExisted = true;
      };
      i += 1;
    };
    if !alreadyExisted {
      ArrayPush(this.m_runtimeTransitions, evt.transition);
    };
  ...
  public final func OnQuestRemoveTransition(evt: ref<QuestRemoveTransition>) -> EntityNotificationType {
    let i: Int32 = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
      if this.m_runtimeTransitions[i].transitionHour == evt.removeTransitionFrom {
        GameInstance.GetTimeSystem(this.GetGameInstance()).UnregisterListener(this.m_runtimeTransitions[i].listenerID);
        this.m_runtimeTransitions[i].locked = true;
        if !IsFinal() {
        };
      };
      i += 1;
  ...
  public final func OnQuestExecuteTransition(evt: ref<QuestExecuteTransition>) -> EntityNotificationType {
    this.ApplyTransition(evt.transition, false);
    return EntityNotificationType.DoNotNotifyEntity;
  }

  public final func OnPurgeTransitions(evt: ref<PurgeAllTransitions>) -> EntityNotificationType {
    let i: Int32;
    this.UnregisterTimeSystemListeners();
    i = 0;
    while i < ArraySize(this.m_runtimeTransitions) {
  ...
  public final func OnQuestModifyFilter(evt: ref<QuestModifyFilters>) -> EntityNotificationType {
    if NotEquals(evt.incomingFilters, EQuestFilterType.DONT_CHANGE) {
      this.m_eventsFilters.incomingEventsFilter = IntEnum<EFilterType>(EnumInt(evt.incomingFilters) - 1);
    };
    if NotEquals(evt.outgoingFilters, EQuestFilterType.DONT_CHANGE) {
      this.m_eventsFilters.outgoingEventsFilter = IntEnum<EFilterType>(EnumInt(evt.outgoingFilters) - 1);
    };
    return EntityNotificationType.DoNotNotifyEntity;
  }
  ...
  public final const func GetUsersInPerimeter() -> [AreaEntry] {
  ...
  private final func SetSecurityAreaType(newType: ESecurityAreaType, opt wasScheduled: Bool) -> Void {
    ...
    if Equals(newType, ESecurityAreaType.DISABLED) {
      ...
      PreventionSystem.PreventionSecurityAreaEnterRequest(this.GetGameInstance(), false, this.GetID());
    };
  ...
  private final const func ExtractSquadProxies() -> [ref<CommunityProxyPS>] {
  ...
  public final const func GetSecurityAreaAgents() -> [EntityID] {
  ...
  public final const func GetNPCs() -> [EntityID] {
  ...
  public final const func GetDevices() -> [EntityID] {
