...
public class SecurityAgentSpawnedEvent extends Event {
  ...
  public let securityAreas: [ref<SecurityAreaControllerPS>];

  public final static func Construct(agentLink: DeviceLink, type: gameEntitySpawnerEventType, const areas: script_ref<[ref<SecurityAreaControllerPS>]>) -> ref<SecurityAgentSpawnedEvent> {
    ...
    spawnEvent.securityAreas = Deref(areas);
    return spawnEvent;
  }
}
...
public class AgentRegistry extends IScriptable {
  ...
  @runtimeProperty("unsavable", "true")
  protected persistent let m_agents: [Agent];
  ...
  public final func RegisterAgent(agentPS: DeviceLink, const areas: script_ref<[ref<SecurityAreaControllerPS>]>) -> Void {
    let agent: Agent;
    let index: Int32;
  ...
  public final const func GetAgent(id: PersistentID, recordCopy: script_ref<Agent>) -> Bool {
    let index: Int32;
  ...
  public final const func GetAgentsIDs() -> [PersistentID] {
    let i: Int32;
  ...
  public final const func GetAgents() -> [Agent] {
  ...
  public final const func GetAgents(const filter: script_ref<[ref<SecurityAreaControllerPS>]>) -> [Agent] {
    ...
    while i < ArraySize(this.m_agents) {
      ...
      while k < ArraySize(this.m_agents[i].areas) {
        y = 0;
        while y < ArraySize(Deref(filter)) {
          if Equals(DeviceLink.GetLinkID(this.m_agents[i].areas[k]), Deref(filter)[y].GetID()) {
  ...
  public final const func GetAgentsIDs(const filter: script_ref<[ref<SecurityAreaControllerPS>]>) -> [PersistentID] {
  ...
  public final func AddArea(area: ref<SecurityAreaControllerPS>, const agents: script_ref<[ref<DeviceComponentPS>]>) -> Void {
    let i: Int32;
    let index: Int32;
    RWLock.Acquire(this.m_agentsLock);
    i = 0;
    while i < ArraySize(Deref(agents)) {
      if this.GetAgentIndex_NoLock(Deref(agents)[i].GetID(), index) {
  ...
  public final func RemoveArea(const data: script_ref<[OnDisableAreaData]>) -> Void {
    let i: Int32;
    let index: Int32;
    RWLock.Acquire(this.m_agentsLock);
    i = 0;
    while i < ArraySize(Deref(data)) {
      if this.GetAgentIndex_NoLock(Deref(data)[i].agent, index) {
        Agent.RemoveArea(this.m_agents[index], Deref(data)[i].remainingAreas);
      };
      i += 1;
    };
    RWLock.Release(this.m_agentsLock);
  ...
  public final const func GetTurrets() -> [Agent] {
  ...
  public final const func GetSensors() -> [Agent] {
    let agentClassName: CName;
    let i: Int32;
    let matchingAgents: array<Agent>;
    RWLock.AcquireShared(this.m_agentsLock);
  ...
  public final const func GetReprimandPerformer(target: EntityID, agent: script_ref<Agent>) -> Bool {
    let i: Int32;
    RWLock.AcquireShared(this.m_agentsLock);
    i = 0;
    while i < ArraySize(this.m_agents) {
  ...
  public final func ReleaseAllReprimands(agents: script_ref<[Agent]>) -> Void {
    let i: Int32;
    RWLock.Acquire(this.m_agentsLock);
    i = 0;
    while i < ArraySize(this.m_agents) {
      if Agent.IsPerformingReprimand(this.m_agents[i]) {
        ArrayPush(Deref(agents), this.m_agents[i]);
  ...
  protected final func SaveAgent_NoLock(const agent: script_ref<Agent>) -> Void {
    if !Agent.IsValid(Deref(agent)) {
      return;
    };
    ArrayPush(this.m_agents, Deref(agent));
  }
  ...
  public final const func GetValidAgents(state: ESecuritySystemState, const breachedAreas: script_ref<[ref<SecurityAreaControllerPS>]>) -> [SecuritySystemOutputData] {
    ...
    while i < ArraySize(Deref(breachedAreas)) {
      if EnumInt(Deref(breachedAreas)[i].GetOutgoingFilter()) > EnumInt(outgoingFilterType) {
        outgoingFilterType = Deref(breachedAreas)[i].GetOutgoingFilter();
      };
      i += 1;
    };
    RWLock.AcquireShared(this.m_agentsLock);
  ...
  public final const func GetAgentAreas(id: PersistentID) -> [DeviceLink] {
  ...
  public final const func GetSupportedAgents() -> [Agent] {
    let i: Int32;
  ...
  public final func ProcessOnPlayerSpotted(evt: ref<PlayerSpotted>, modifiedAgents: script_ref<[Agent]>, out hasAnySupport: Bool) -> Bool {
    let i: Int32;
    let outgoingFilterType: EFilterType;
