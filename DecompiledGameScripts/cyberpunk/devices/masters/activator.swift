...
public class Activator extends InteractiveMasterDevice {
  ...
  private const func GetController() -> ref<ActivatorController> {
    return this.m_controller as ActivatorController;
  }

  public const func GetDevicePS() -> ref<ActivatorControllerPS> {
  ...
  protected cb func OnSpiderbotActivateActivator(evt: ref<SpiderbotActivateActivator>) -> Bool {
    ...
    locationOverrideRef = this.GetDevicePS().GetSpiderbotInteractionLocationOverride();
    locationOverrideGlobalRef = ResolveNodeRefWithEntityID(locationOverrideRef, this.GetEntityID());
    if GlobalNodeRef.IsDefined(ResolveNodeRef(locationOverrideRef, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot()))) {
  ...
  protected cb func OnSpiderbotOrderCompletedEvent(evt: ref<SpiderbotOrderCompletedEvent>) -> Bool {
    ...
    this.GetDevicePS().ActivateConnectedDevices();
  ...
  public const func GetCurrentGameplayRole() -> EGameplayRole {
    ...
    if NotEquals(gameplayRole, EGameplayRole.None) {
      return gameplayRole;
    };
