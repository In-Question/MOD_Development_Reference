...
public class TerminalController extends MasterController {
  ...
  public const func GetPS() -> ref<TerminalControllerPS> {
    return this.GetBasePS() as TerminalControllerPS;
  }
}
...
public class TerminalControllerPS extends MasterControllerPS {
  ...
  protected let m_spawnedSystems: [ref<VirtualSystemPS>];
  ...
  @runtimeProperty("category", "Glitch")
  ...
  protected func GetSkillCheckContainerForSetup() -> ref<BaseSkillCheckContainer> {
    return this.m_terminalSkillChecks;
  }
  ...
  protected const func GenerateContext(requestType: gamedeviceRequestType, providedClearance: ref<Clearance>, opt providedProcessInitiator: ref<GameObject>, opt providedRequestor: EntityID) -> GetActionsContext {
    ...
    generatedContext.allowsRemoteAuthorization = this.m_terminalSetup.m_allowRemoteAuthorization;
    return generatedContext;
  }

  private final func GetMatchingVirtualSystemsData(device: ref<DeviceComponentPS>, const listToCheck: script_ref<[ref<DeviceComponentPS>]>, data: script_ref<DeviceCounter>) -> Bool {
    let i: Int32;
    ...
    if NotEquals(type, EVirtualSystem.None) {
      Deref(data).systemType = type;
      i = 0;
      while i < ArraySize(Deref(listToCheck)) {
        if device == Deref(listToCheck)[i] {
        } else {
          if Equals(Deref(listToCheck)[i].GetVirtualSystemType(), type) {
            ArrayPush(Deref(data).devices, Deref(listToCheck)[i]);
            returnValue = true;
          };
        };
        i += 1;
    ...
    if returnValue {
      ArrayPush(Deref(data).devices, device);
    };
    return returnValue;
  }

  private final func HasMatchingVirtualSystemType(device: ref<DeviceComponentPS>, const listToCheck: script_ref<[ref<DeviceComponentPS>]>, out type: EVirtualSystem) -> Bool {
    let i: Int32;
    if device == null {
      return false;
    };
  ...
  private final func SpawnSurveillanceSystemUI(const slavesToConnect: script_ref<[ref<DeviceComponentPS>]>) -> Void {
  ...
  private final func SpawnSecuritySystemUI(const slavesToConnect: script_ref<[ref<DeviceComponentPS>]>) -> Void {
  ...
  private final func SpawnDoorSystemUI(const slavesToConnect: script_ref<[ref<DeviceComponentPS>]>) -> Void {
  ...
  private final func SpawnMediaSystemUI(const slavesToConnect: script_ref<[ref<DeviceComponentPS>]>) -> Void {
  ...
  protected func GetQuickHackActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Void {
    ...
    currentAction = this.ActionGlitchScreen(t"DeviceAction.GlitchScreenSuicide", t"QuickHack.GrenadeHackBase");
