...
public class FuseController extends MasterController {
  ...
  public const func GetPS() -> ref<FuseControllerPS> {
    return this.GetBasePS() as FuseControllerPS;
  }
}

public class FuseControllerPS extends MasterControllerPS {

  @runtimeProperty("category", "City Light System")
  ...
  @runtimeProperty("rangeMax", "1.0f")
  @runtimeProperty("category", "City Light System")
  @runtimeProperty("rangeMin", "0.0f")
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Interactions.InteractionChoice")
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Interactions.InteractionChoice")
  private let m_alternativeNameForPower: TweakDBID;

  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Interactions.InteractionChoice")
  private let m_alternativeNameForUnpower: TweakDBID;
  ...
  protected func ActionTogglePower() -> ref<TogglePower> {
    ...
    action.SetUp(this);
    action.SetProperties(this.m_deviceState, this.m_alternativeNameForPower, this.m_alternativeNameForUnpower);
    action.AddDeviceName(this.m_deviceName);
    action.CreateActionWidgetPackage();
    return action;
  }
  ...
  private final func RefreshSlaves(devices: [ref<DeviceComponentPS>], opt force: Bool) -> Void {
  ...
  private final func RefreshPowerOnSlaves(const devices: script_ref<[ref<DeviceComponentPS>]>) -> Void {
    ...
    if !this.IsON() {
      this.CutPowerOnSlaveDevices(Deref(devices));
    } else {
      this.RestorePowerOnSlaveDevices(Deref(devices));
    };
  }

  private final func RestorePowerOnSlaveDevices(devices: [ref<DeviceComponentPS>]) -> Void {
    let action: ref<ScriptableDeviceAction>;
    let device: ref<ScriptableDeviceComponentPS>;
    devices = this.GetImmediateSlaves();
    let i: Int32 = 0;
  ...
  private final func CutPowerOnSlaveDevices(devices: [ref<DeviceComponentPS>]) -> Void {
    let action: ref<ScriptableDeviceAction>;
    let device: ref<ScriptableDeviceComponentPS>;
    devices = this.GetImmediateSlaves();
    let i: Int32 = 0;
  ...
  private final func RefreshCLSoNslaves(state: EDeviceStatus, restorePower: Bool, const devices: script_ref<[ref<DeviceComponentPS>]>) -> Void {
    let delay: Float;
    ...
    while i < ArraySize(Deref(devices)) {
      if Deref(devices)[i] == null || !Deref(devices)[i].IsAttachedToGame() {
      } else {
        if totalEventsSent >= this.m_maxLightsSwitchedAtOnce {
          ArrayPush(excessDevices, Deref(devices)[i]);
        } else {
          id = PersistentID.ExtractEntityID(Deref(devices)[i].GetID());
  ...
  private final func SendCLSRefreshByEvent(const devices: script_ref<[ref<DeviceComponentPS>]>, state: EDeviceStatus, restorePower: Bool, delay: Float) -> Void {
    let evt: ref<RefreshCLSOnSlavesEvent> = new RefreshCLSOnSlavesEvent();
    evt.state = state;
    evt.slaves = Deref(devices);
