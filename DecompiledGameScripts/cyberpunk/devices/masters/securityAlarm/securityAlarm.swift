...
public class SecurityAlarm extends InteractiveMasterDevice {
  ...
  private const func GetController() -> ref<SecurityAlarmController> {
    return this.m_controller as SecurityAlarmController;
  }

  public const func GetDevicePS() -> ref<SecurityAlarmControllerPS> {
  ...
  protected func StartGlitching(glitchState: EGlitchState, opt intensity: Float) -> Void {
    if NotEquals(this.GetDevicePS().GetAlarmState(), ESecuritySystemState.COMBAT) {
  ...
  protected final func DetermineState() -> Void {
    if !this.m_isGlitching {
      ...
      if Equals(this.GetDevicePS().GetAlarmState(), ESecuritySystemState.COMBAT) {
        this.SetCombatState();
      };
    };
  }
  ...
  protected final func StopSound() -> Void {
    if this.GetDevicePS().UsesSound() {
      GameObject.StopSound(this, this.GetDevicePS().AlarmSound());
    };
  }

  protected final func SendStim() -> Void {
    let stimType: gamedataStimType = this.GetDevicePS().UsesSound() ? gamedataStimType.Alarm : gamedataStimType.SilentAlarm;
