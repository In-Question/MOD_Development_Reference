...
public class ActionsSequencerController extends ScriptableDeviceComponent {

  public const func GetPS() -> ref<ActionsSequencerControllerPS> {
    return this.GetBasePS() as ActionsSequencerControllerPS;
  }
}

public class ActionsSequencerControllerPS extends MasterControllerPS {

  @runtimeProperty("tooltip", "IMPORTANT: DON'T MAKE THIS VALUE TOO HIGH AS ALL QUICK HACKS ARE BLOCKED UNTIL THIS TIMES PASSES. Time when the last device connected to ActionSequencer will receive the forwarded action. If 0.0 then sequenceMode is forced to AT_THE_SAME_TIME")
  @default(ActionsSequencerControllerPS, 2f)
  private let m_sequenceDuration: Float;

  @runtimeProperty("tooltip", "Immediate - all devices receive actins immediately, REGULAR - Max Sequence Duration will be divided by the number of devices. Each consecutive device receives event after the same amount of time, ACCELERATING - start slow, end fast DECELARTING - start fast end slow, RANDOM - random")
  private let m_sequencerMode: EActionsSequencerMode;

  @runtimeProperty("tooltip", "Filter actions by source of action. There can be more than 1 srespected source")
  ...
  private final func CommenceSequence(actionToForward: ref<ScriptableDeviceAction>, const eligibleSlaves: script_ref<[ref<DeviceComponentPS>]>, const delays: script_ref<[Float]>) -> Void {
    let i: Int32;
    ...
    this.m_ongoingSequence.maxActionsInSequence = ArraySize(Deref(eligibleSlaves));
    ...
    if Equals(this.m_sequencerMode, EActionsSequencerMode.AT_THE_SAME_TIME_TODO) {
      ...
      if ArraySize(Deref(eligibleSlaves)) != ArraySize(Deref(delays)) {
        if !IsFinal() {
        };
        return;
      };
      i = 0;
      while i < ArraySize(Deref(delays)) {
        sequenceCallback = new SequenceCallback();
        sequenceCallback.persistentID = Deref(eligibleSlaves)[i].GetID();
        sequenceCallback.className = Deref(eligibleSlaves)[i].GetClassName();
        sequenceCallback.actionToForward = actionToForward;
        ArrayPush(this.m_ongoingSequence.delayIDs, delaySystem.DelayPSEvent(this.GetID(), this.GetClassName(), sequenceCallback, Deref(delays)[i]));
        i += 1;
      };
    };
  }
  ...
  private final const func ForceLockOnAllSlaves() -> Void {
    ...
    let eligibleSlaves: array<ref<DeviceComponentPS>> = this.GetEligibleSlaves(Cast<PersistentID>(this.m_ongoingSequence.sequenceInitiator));
    let i: Int32 = 0;
  ...
  private final const func GetEligibleSlaves(sequenceInitiator: PersistentID) -> [ref<DeviceComponentPS>] {
  ...
  private final const func GetDelayTimeStamps(intervals: Int32) -> [Float] {
  ...
  private final const func GetRegularDelays(amountOfIntervals: Int32, delays: script_ref<[Float]>) -> Void {
    let interval: Float = this.m_sequenceDuration / Cast<Float>(amountOfIntervals);
    let i: Int32 = 1;
    while i < amountOfIntervals + 1 {
      ArrayPush(Deref(delays), interval * Cast<Float>(i));
      i += 1;
    };
  }

  private final const func GetAcceleratingDelays(amountOfIntervals: Int32, delays: script_ref<[Float]>) -> Void;

  private final const func GetDecceleratingDelays(amountOfIntervals: Int32, delays: script_ref<[Float]>) -> Void;

  private final const func GetRandomDelays(amountOfIntervals: Int32, delays: script_ref<[Float]>) -> Void;
