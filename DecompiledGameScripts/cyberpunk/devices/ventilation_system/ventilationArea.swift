...
public class VentilationArea extends InteractiveMasterDevice {
  ...
  protected cb func OnGameAttached() -> Bool {
    ...
    if this.GetDevicePS().IsAreaActive() && this.m_RestartGameEffectOnAttach {
      this.PlayGameEffect();
    };
  }
  ...
  private const func GetController() -> ref<VentilationAreaController> {
    return this.m_controller as VentilationAreaController;
  }

  public const func GetDevicePS() -> ref<VentilationAreaControllerPS> {
  ...
  private final func PlayGameEffect() -> Void {
    ...
    switch this.GetDevicePS().GetAreaEffect() {
  ...
  protected final func ApplyStatusEffect(const effectTDBID: script_ref<String>) -> Void {
    let statusEffectID: TweakDBID = TDBID.Create(Deref(effectTDBID));
    let entities: array<ref<Entity>> = this.GetEntitiesInArea();
    let i: Int32 = 0;
    while i < ArraySize(entities) {
