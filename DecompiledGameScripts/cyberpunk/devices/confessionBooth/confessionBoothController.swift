...
public class Confess extends Pay {
  ...
  public final static func IsDefaultConditionMet(device: ref<ScriptableDeviceComponentPS>, const context: script_ref<GetActionsContext>) -> Bool {
    if Confess.IsAvailable(device) && Confess.IsClearanceValid(Deref(context).clearance) {
      return true;
    };
    return false;
  }
...
public class ConfessionBoothController extends BasicDistractionDeviceController {
  ...
  public const func GetPS() -> ref<ConfessionBoothControllerPS> {
    return this.GetBasePS() as ConfessionBoothControllerPS;
  }
}
