...
public class ConfessionBooth extends BasicDistractionDevice {
  ...
  public const func GetBlackboardDef() -> ref<ConfessionalBlackboardDef> {
    return this.GetDevicePS().GetBlackboardDef();
  }

  private const func GetController() -> ref<ConfessionBoothController> {
    return this.m_controller as ConfessionBoothController;
  }

  public const func GetDevicePS() -> ref<ConfessionBoothControllerPS> {
  ...
  private final func StartConfessing() -> Void {
    this.GetBlackboard().SetBool(this.GetBlackboardDef().IsConfessing, true);
    this.GetBlackboard().FireCallbacks();
    this.StartBlinking();
  }

  private final func StopConfessing() -> Void {
    this.GetBlackboard().SetBool(this.GetBlackboardDef().IsConfessing, false);
    this.GetBlackboard().FireCallbacks();
    this.StopBlinking();
  }
}
