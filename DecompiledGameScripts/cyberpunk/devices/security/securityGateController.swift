...
public class SecurityGateController extends MasterController {
  ...
  public const func GetPS() -> ref<SecurityGateControllerPS> {
    return this.GetBasePS() as SecurityGateControllerPS;
  }
}
...
public class SecurityGateControllerPS extends MasterControllerPS {
  ...
  private let m_trespassersDataList: [TrespasserEntry];
  ...
  protected func GetQuickHackActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Void {
    ...
    currentAction.SetInactiveWithReason(!this.IsUserAuthorized(Deref(context).processInitiatorObject.GetEntityID()), "LocKey#7016");
    ArrayPush(outActions, currentAction);
  ...
  private final func UpdateTrespasserEntry(index: Int32, isEntering: Bool, areaName: CName) -> Void {
    ...
    if this.IsTrespasserOutside(index) {
      this.ProtectEntityFromSecuritySystem(false, this.m_trespassersDataList[index].trespasser.GetEntityID(), false, this.DetermineIfEntityIsWithdrawing(index, areaName));
      this.RemoveTrespasserEntry(index);
    };
    if isEntering {
      ArrayPush(this.m_trespassersDataList[index].areaStack, areaName);
    };
  }
  ...
  private final func InitiateScan(entryIndex: Int32) -> Void {
    ...
    GameInstance.GetActivityLogSystem(this.GetGameInstance()).AddLog(GetLocalizedText("LocKey#93347"));
  ...
  private final func TriggerScanResponse(isSuccessful: Bool) -> Void {
    ...
    if isSuccessful {
      GameInstance.GetActivityLogSystem(this.GetGameInstance()).AddLog(GetLocalizedText("LocKey#93860"));
      if this.IsPartOfSystem(ESystems.SecuritySystem) {
        if EnumInt(this.m_securityGateResponseProperties.securityLevelAccessGranted) > 0 {
          ...
          GameInstance.GetActivityLogSystem(this.GetGameInstance()).AddLog(GetLocalizedText("LocKey#93861") + " " + EnumValueToString("ESecurityAccessLevel", Cast<Int64>(EnumInt(this.m_securityGateResponseProperties.securityLevelAccessGranted))));
        };
      };
      this.m_securityGateStatus = ESecurityGateStatus.READY;
    } else {
      GameInstance.GetActivityLogSystem(this.GetGameInstance()).AddLog(GetLocalizedText("LocKey#93862"));
  ...
  protected func WakeUpDevice() -> Bool {
    let emptyID: EntityID;
    let value: Bool = super.WakeUpDevice();
