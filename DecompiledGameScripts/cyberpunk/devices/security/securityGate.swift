...
public class SecurityGate extends InteractiveMasterDevice {
  ...
  public const func GetDevicePS() -> ref<SecurityGateControllerPS> {
    return this.GetController().GetPS();
  }

  private const func GetController() -> ref<SecurityGateController> {
    return this.m_controller as SecurityGateController;
  }

  protected cb func OnAreaEnter(evt: ref<AreaEnteredEvent>) -> Bool {
    this.GetDevicePS().UpdateTrespassersList(evt, true);
  }

  protected cb func OnAreaExit(evt: ref<AreaExitedEvent>) -> Bool {
    this.GetDevicePS().UpdateTrespassersList(evt, false);
  }
