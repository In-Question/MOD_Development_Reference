...
public class ComputerMenuWidgetController extends inkLogicController {
  ...
  private let m_fileWidgetsData: [SDocumentWidgetPackage];

  private let m_fileThumbnailWidgetsData: [SDocumentThumbnailWidgetPackage];

  public func InitializeFiles(gameController: ref<ComputerInkGameController>, const widgetsData: script_ref<[SDocumentWidgetPackage]>) -> Void {
    let i: Int32;
    let widget: ref<inkWidget>;
    this.HideFileWidgets();
    i = 0;
    while i < ArraySize(Deref(widgetsData)) {
      widget = this.GetFileWidget(Deref(widgetsData)[i], gameController);
      if widget == null {
        if Equals(Deref(widgetsData)[i].placement, EWidgetPlacementType.FLOATING) {
          this.CreateDocumentWidgetAsync(gameController, gameController.GetMainLayoutController().GetMenuContainer(), Deref(widgetsData)[i]);
        } else {
          this.CreateDocumentWidgetAsync(gameController, inkWidgetRef.Get(this.m_contentWidget), Deref(widgetsData)[i]);
        };
      } else {
        this.InitializeDocumentWidget(gameController, widget, Deref(widgetsData)[i]);
      };
      i += 1;
    };
  ...
  public func InitializeFilesThumbnails(gameController: ref<ComputerInkGameController>, const widgetsData: script_ref<[SDocumentThumbnailWidgetPackage]>) -> Void {
    let i: Int32;
    let widget: ref<inkWidget>;
    this.HideFileThumbnailWidgets();
    i = 0;
    while i < ArraySize(Deref(widgetsData)) {
      widget = this.GetFileThumbnailWidget(Deref(widgetsData)[i], gameController);
      if widget == null {
        this.CreateDocumentThumbnailWidgetAsync(gameController, inkWidgetRef.Get(this.m_thumbnailsListWidget), Deref(widgetsData)[i]);
      } else {
        this.InitializeDocumentThumbnailWidget(gameController, widget, Deref(widgetsData)[i]);
      };
      i += 1;
    };
    this.m_isInitialized = true;
  ...
  public final func CreateDocumentWidget(gameController: ref<ComputerInkGameController>, parentWidget: wref<inkWidget>, const widgetData: script_ref<SDocumentWidgetPackage>) -> wref<inkWidget> {
    let screenDef: ScreenDefinitionPackage = gameController.GetScreenDefinition();
    let widget: ref<inkWidget> = gameController.FindWidgetInLibrary(parentWidget, TweakDBInterface.GetWidgetDefinitionRecord(Deref(widgetData).widgetTweakDBID), screenDef.screenDefinition.ComputerScreenType(), screenDef.style, Deref(widgetData).libraryID, Deref(widgetData).libraryPath);
    if widget != null {
  ...
  protected cb func OnDocumentWidgetSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    if spawnData != null {
      ...
      widgetData = FromVariant<SDocumentWidgetPackage>(spawnData.m_widgetData);
  ...
  protected final func UpdateFileWidgetData(const widgetData: script_ref<SDocumentWidgetPackage>, index: Int32) -> Void {
    if index >= 0 && index < ArraySize(this.m_fileWidgetsData) {
      this.m_fileWidgetsData[index] = Deref(widgetData);
    };
  }

  protected final func InitializeDocumentWidget(gameController: ref<ComputerInkGameController>, widget: ref<inkWidget>, const widgetData: script_ref<SDocumentWidgetPackage>) -> Void {
  ...
  protected final func HasFileWidgetData(const widgetData: script_ref<SDocumentWidgetPackage>, gameController: ref<DeviceInkGameControllerBase>) -> Bool {
    return this.GetFileWidgetDataIndex(Deref(widgetData), gameController) >= 0;
  }

  protected final func GetFileWidget(const widgetData: script_ref<SDocumentWidgetPackage>, gameController: ref<ComputerInkGameController>) -> wref<inkWidget> {
    let index: Int32 = this.GetFileWidgetDataIndex(Deref(widgetData), gameController);
    if index >= 0 && index < ArraySize(this.m_fileWidgetsData) {
  ...
  public final func CreateDocumentThumbnailWidget(gameController: ref<ComputerInkGameController>, parentWidget: wref<inkWidget>, const widgetData: script_ref<SDocumentThumbnailWidgetPackage>) -> wref<inkWidget> {
    let screenDef: ScreenDefinitionPackage = gameController.GetScreenDefinition();
    let widget: ref<inkWidget> = gameController.FindWidgetInLibrary(parentWidget, TweakDBInterface.GetWidgetDefinitionRecord(Deref(widgetData).widgetTweakDBID), screenDef.screenDefinition.ComputerScreenType(), screenDef.style, Deref(widgetData).libraryID, Deref(widgetData).libraryPath);
    return widget;
  }
  ...
  protected cb func OnDocumentThumbnailWidgetSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    if spawnData != null {
      deviceGameController = spawnData.m_controller as ComputerInkGameController;
      widgetData = FromVariant<SDocumentThumbnailWidgetPackage>(spawnData.m_widgetData);
      if deviceGameController != null {
        widgetData.widget = widget;
        widgetData.libraryID = spawnData.m_libraryID;
  ...
  protected final func UpdateFileThumbnailWidgetData(const widgetData: script_ref<SDocumentThumbnailWidgetPackage>, index: Int32) -> Void {
    if index >= 0 && index < ArraySize(this.m_fileThumbnailWidgetsData) {
      this.m_fileThumbnailWidgetsData[index] = Deref(widgetData);
    };
  }

  protected final func InitializeDocumentThumbnailWidget(gameController: ref<ComputerInkGameController>, widget: ref<inkWidget>, const widgetData: script_ref<SDocumentThumbnailWidgetPackage>) -> Void {
    ...
    if controller != null {
      controller.Initialize(gameController, widgetData);
      controller.ToggleSelection(Deref(widgetData).isOpened);
      if Deref(widgetData).isOpened {
        if Equals(Deref(widgetData).documentType, EDocumentType.MAIL) {
          gameController.RequestMailWidgetUpdate(Deref(widgetData).documentAdress);
        } else {
          if Equals(Deref(widgetData).documentType, EDocumentType.FILE) {
            gameController.RequestFileWidgetUpdate(Deref(widgetData).documentAdress);
          };
        };
      };
    };
  ...
  protected final func HasFileThumbnailWidgetData(const widgetData: script_ref<SDocumentThumbnailWidgetPackage>, gameController: ref<DeviceInkGameControllerBase>) -> Bool {
    return this.GetFileThumbnailWidgetDataIndex(Deref(widgetData), gameController) >= 0;
  }
  ...
  protected final func GetFileThumbnailWidget(const widgetData: script_ref<SDocumentThumbnailWidgetPackage>, gameController: ref<ComputerInkGameController>) -> wref<inkWidget> {
    let index: Int32 = this.GetFileThumbnailWidgetDataIndex(Deref(widgetData), gameController);
    if index >= 0 && index < ArraySize(this.m_fileThumbnailWidgetsData) {
