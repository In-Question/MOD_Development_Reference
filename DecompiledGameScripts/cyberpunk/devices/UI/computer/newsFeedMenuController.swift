...
public class NewsFeedMenuWidgetController extends inkLogicController {
  ...
  @runtimeProperty("category", "OBSOLETE - Widget Paths")
  ...
  private let m_bannerWidgetsData: [SBannerWidgetPackage];
  ...
  public func InitializeBanners(gameController: ref<ComputerInkGameController>, const widgetsData: script_ref<[SBannerWidgetPackage]>) -> Void {
    let i: Int32;
    let widget: ref<inkWidget>;
    this.HideBannerWidgets();
    i = 0;
    while i < ArraySize(Deref(widgetsData)) {
      widget = this.GetBannerWidget(Deref(widgetsData)[i], gameController);
      if widget == null {
        widget = this.CreateBannerWidget(gameController, inkWidgetRef.Get(this.m_bannersListWidget), Deref(widgetsData)[i]);
        this.AddBannerWidget(widget, Deref(widgetsData)[i], gameController);
      };
      this.InitializeBannerWidget(gameController, widget, Deref(widgetsData)[i]);
      i += 1;
    };
    this.m_isInitialized = true;
  }

  public final func CreateBannerWidget(gameController: ref<ComputerInkGameController>, parentWidget: wref<inkWidget>, const widgetData: script_ref<SBannerWidgetPackage>) -> wref<inkWidget> {
  ...
  protected final func InitializeBannerWidget(gameController: ref<ComputerInkGameController>, widget: ref<inkWidget>, const widgetData: script_ref<SBannerWidgetPackage>) -> Void {
  ...
  public final func ShowFullBanner(gameController: ref<ComputerInkGameController>, const widgetData: script_ref<SBannerWidgetPackage>) -> Void {
    ...
    if this.m_fullBannerWidgetData.widget == null || NotEquals(this.m_fullBannerWidgetData.libraryID, libraryID) || NotEquals(this.m_fullBannerWidgetData.libraryPath, libraryPath) {
      ...
      this.m_fullBannerWidgetData.widget = gameController.FindWidgetInLibrary(gameController.GetMainLayoutController().GetMenuContainer(), widgetRecord, screenDef.screenDefinition.ComputerScreenType(), screenDef.style, Deref(widgetData).libraryID, Deref(widgetData).libraryPath);
