...
public class ComputerDocumentThumbnailWidgetController extends DeviceButtonLogicControllerBase {
  ...
  public func Initialize(gameController: ref<ComputerInkGameController>, const widgetData: script_ref<SDocumentThumbnailWidgetPackage>) -> Void {
    this.RegisterThumbnailCallback(gameController);
    inkTextRef.SetText(this.m_displayNameWidget, Deref(widgetData).displayName);
    this.m_documentAdress = Deref(widgetData).documentAdress;
    this.m_documentType = Deref(widgetData).documentType;
    this.m_questInfo = Deref(widgetData).questInfo;
    if Deref(widgetData).wasRead {
      this.OpenDocument();
    } else {
      this.CloseDocument();
    };
