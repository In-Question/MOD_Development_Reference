...
public class ComputerMainMenuWidgetController extends inkLogicController {
  ...
  public func InitializeMenuButtons(gameController: ref<ComputerInkGameController>, const widgetsData: script_ref<[SComputerMenuButtonWidgetPackage]>) -> Void {
    ...
    while i < ArraySize(Deref(widgetsData)) {
      ...
      if widget == null {
        widget = this.CreateMenuButtonWidget(gameController, inkWidgetRef.Get(this.m_menuButtonsListWidget), Deref(widgetsData)[i]);
        this.AddMenuButtonWidget(widget, Deref(widgetsData)[i], gameController);
      };
