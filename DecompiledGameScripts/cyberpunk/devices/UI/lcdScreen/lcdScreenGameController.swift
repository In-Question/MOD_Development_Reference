...
public class LcdScreenInkGameController extends DeviceInkGameControllerBase {
  ...
  protected func RegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      ...
      this.m_onMessegeChangedListener = blackboard.RegisterListenerVariant(this.GetOwner().GetBlackboardDef().MessegeData, this, n"OnMessegeChanged");
    };
  }
  ...
  protected func UnRegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      ...
      blackboard.UnregisterListenerVariant(this.GetOwner().GetBlackboardDef().MessegeData, this.m_onMessegeChangedListener);
    };
  }

  protected func GetOwner() -> ref<LcdScreen> {
    return this.GetOwnerEntity() as LcdScreen;
  }
  ...
  protected cb func OnMessegeChanged(value: Variant) -> Bool {
    ...
    let messageData: ref<ScreenMessageData> = FromVariant<ref<ScreenMessageData>>(value);
    if messageData == null {
      return false;
    };
  ...
  public final func TurnOn() -> Void {
    ...
    if this.m_messegeRecord == null {
      this.InitializeCustomNumber(this.GetOwner().HasCustomNumber(), this.GetOwner().GetCustomNumber());
      this.InitializeMessageRecord(this.GetOwner().GetMessageRecord());
    };
    this.ResolveMessegeRecord(this.m_messegeRecord);
  }
  ...
  protected final func GetColorFromArray(const calorArray: script_ref<[Int32]>) -> Color {
    let color: Color;
    let i: Int32 = 0;
    while i < ArraySize(Deref(calorArray)) {
      if i == 0 {
        color.Red = Cast<Uint8>(Deref(calorArray)[i]);
      } else {
        if i == 1 {
          color.Green = Cast<Uint8>(Deref(calorArray)[i]);
        } else {
          if i == 2 {
            color.Blue = Cast<Uint8>(Deref(calorArray)[i]);
          } else {
            if i == 3 {
              color.Alpha = Cast<Uint8>(Deref(calorArray)[i]);
            };
          };
        };
      };
