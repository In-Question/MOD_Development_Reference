...
public class TvDeviceWidgetController extends DeviceWidgetControllerBase {
  ...
  private final func ResolveChannelData(data: ref<TvDeviceWidgetCustomData>, const widgetData: script_ref<SDeviceWidgetPackage>, gameController: ref<DeviceInkGameControllerBase>) -> Void {
    ...
    if IsDefined(channelRecord) {
      ...
      if !IsDefined(this.m_globalTVChannel) || IsDefined(this.m_globalTVChannel) && NotEquals(this.m_globalTVChannel.GetName(), channelRecord.ChannelWidget()) {
        ...
        this.SpawnGlobalTVChannelWidget(gameController, channelRecord, Deref(widgetData).libraryPath);
      } else {
      ...
      if NotEquals(this.m_activeVideo, data.videoPath) || NotEquals(Deref(widgetData).deviceState, EDeviceStatus.ON) || !ResRef.IsValid(data.videoPath) {
        this.StopVideo();
      };
