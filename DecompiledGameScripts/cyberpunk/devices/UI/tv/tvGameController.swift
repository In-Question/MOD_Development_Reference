...
public class TvChannelSpawnData extends IScriptable {
  ...
  public final func Initialize(channelName: CName, const localizedName: script_ref<String>, order: Int32) -> Void {
    this.m_channelName = channelName;
    this.m_localizedName = Deref(localizedName);
    this.m_order = order;
  }
}
...
public class TvInkGameController extends DeviceInkGameControllerBase {
  ...
  private let m_activeSequence: [SequenceVideo];

  @default(TvInkGameController, 0)
  private let m_activeSequenceVideo: Int32;

  private let m_globalTVChannels: [wref<inkWidget>];
  ...
  protected func SetupWidgets() -> Void {
    if !this.m_isInitialized {
      ...
      if !this.GetOwner().IsInteractive() {
  ...
  protected func RegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      this.m_onChangeChannelListener = blackboard.RegisterListenerInt(this.GetOwner().GetBlackboardDef().CurrentChannel, this, n"OnChangeChannel");
  ...
  protected func UnRegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      ...
      blackboard.UnregisterListenerInt(this.GetOwner().GetBlackboardDef().CurrentChannel, this.m_onChangeChannelListener);
    };
  }

  protected func GetOwner() -> ref<TV> {
    return this.GetOwnerEntity() as TV;
  }
  ...
  private final func SelectChannel(value: Int32, opt force: Bool) -> Void {
    ...
    channel = this.GetOwner().GetChannelData(value);
  ...
  public final func StopVideo() -> Void {
    ...
    this.m_activeAudio = n"None";
    this.m_mainDisplayWidget.Stop();
  }

  public final func SetChannellText(const channelName: script_ref<String>) -> Void {
    if this.m_channellTextWidget != null {
      this.m_channellTextWidget.SetLocalizedTextScript(channelName);
    };
  }

  public final func SetSecuredText(const text: script_ref<String>) -> Void {
    if this.m_securedTextWidget != null {
      this.m_securedTextWidget.SetText(Deref(text));
    };
  }
  ...
  public func TurnOn() -> Void {
    ...
    if !this.GetOwner().IsDeviceSecured() {
      this.SelectChannel(this.GetOwner().GetDevicePS().GetActiveStationIndex());
    };
  ...
  private final func InitializeGlobalTV() -> Void {
    ...
    channels = this.GetOwner().GetGlobalTVChannels();
    ...
    while i < ArraySize(channels) {
      ...
      if !IsDefined(this.CreateWidgetAsync(this.GetGlobalTVSlot(), channels[i].ChannelWidget(), spawnData)) {
  ...
  protected cb func OnGLobalChannelSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    if IsDefined(spawnData) {
      tvWidgetData = FromVariant<ref<TvChannelSpawnData>>(spawnData.m_widgetData);
    };
    if IsDefined(widget) {
      this.m_globalTVchanellsSpawned += 1;
  ...
  private final func IsGlobalTVChannel(const channel: script_ref<STvChannel>) -> Bool {
    return TDBID.IsValid(Deref(channel).channelTweakID);
  }
  ...
  private final func GetColorFromArray(const colorArray: script_ref<[Int32]>) -> Color {
    let color: Color;
    let i: Int32 = 0;
    while i < ArraySize(Deref(colorArray)) {
      if i == 0 {
        color.Red = Cast<Uint8>(Deref(colorArray)[i]);
      } else {
        if i == 1 {
          color.Green = Cast<Uint8>(Deref(colorArray)[i]);
        } else {
          if i == 2 {
            color.Blue = Cast<Uint8>(Deref(colorArray)[i]);
          } else {
            if i == 3 {
              color.Alpha = Cast<Uint8>(Deref(colorArray)[i]);
            };
          };
        };
      };
