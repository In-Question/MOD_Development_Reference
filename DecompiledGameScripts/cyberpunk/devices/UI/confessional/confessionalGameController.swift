...
public class ConfessionalInkGameController extends DeviceInkGameControllerBase {
  ...
  public func UpdateActionWidgets(const widgetsData: script_ref<[SActionWidgetPackage]>) -> Void {
    ...
    while i < ArraySize(Deref(widgetsData)) {
      if Equals(Deref(widgetsData)[i].wasInitalized, true) {
        ...
        if widget == null {
          this.CreateActionWidgetAsync(this.m_actionsList, Deref(widgetsData)[i]);
        } else {
  ...
  protected func RegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      ...
      this.m_onConfessListener = blackboard.RegisterListenerBool(this.GetOwner().GetBlackboardDef().IsConfessing, this, n"OnConfess");
    };
  }
  ...
  protected func UnRegisterBlackboardCallbacks(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      ...
      blackboard.UnregisterListenerBool(this.GetOwner().GetBlackboardDef().IsConfessing, this.m_onConfessListener);
    };
  }

  protected func GetOwner() -> ref<ConfessionBooth> {
    return this.GetOwnerEntity() as ConfessionBooth;
  }
  ...
  private final func StartConfessing() -> Void {
    ...
    this.PlayVideo(r"base\\movies\\misc\\confessional_booth\\confessional_s.bk2", false, n"None");
  }

  private final func StopConfessing() -> Void {
    if this.m_isConfessing {
      this.m_isConfessing = false;
      this.GetBlackboard().SetBool(this.GetOwner().GetBlackboardDef().IsConfessing, false);
      this.StopVideo();
      this.m_defaultUI.SetVisible(true);
