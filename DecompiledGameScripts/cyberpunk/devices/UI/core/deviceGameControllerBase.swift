...
public struct SWidgetPackageBase {
  ...
  public let orientation: inkEOrientation;
  ...
  public final static func GetLibraryID(widgetDef: ref<WidgetDefinition_Record>, screenTypeDef: ref<DeviceScreenType_Record>, styleDef: ref<WidgetStyle_Record>) -> String {
    ...
    while i < ArraySize(styles) {
      if Equals(styleDef.Type(), styles[i].Type()) {
  ...
  public final static func GetLibraryIDPackage(widgetDef: ref<WidgetDefinition_Record>, screenTypeDef: ref<DeviceScreenType_Record>, styleDef: ref<WidgetStyle_Record>) -> [String] {
    let contentRatio: wref<WidgetRatio_Record>;
    let expectedRatio: String;
    let fullID: String;
    let i: Int32;
...
public class DeviceInkGameControllerBase extends inkGameController {
  ...
  protected let m_actionWidgetsData: [SActionWidgetPackage];

  protected let m_deviceWidgetsData: [SDeviceWidgetPackage];

  protected let m_breadcrumbStack: [SBreadcrumbElementData];
  ...
  public final func IsOwnerFactInvoker() -> Bool {
    return IsDefined(this.GetOwner() as FactInvoker);
  }
  ...
  protected final func CreateActionWidget(parentWidget: wref<inkWidget>, const widgetData: script_ref<SActionWidgetPackage>) -> wref<inkWidget> {
  ...
  protected final func GetActionWidget(const widgetData: script_ref<SActionWidgetPackage>) -> wref<inkWidget> {
    let index: Int32 = this.GetActionWidgetDataIndex(Deref(widgetData));
  ...
  protected final func UpdateActionWidgetData(const widgetData: script_ref<SActionWidgetPackage>, index: Int32) -> Void {
    if index >= 0 && index < ArraySize(this.m_actionWidgetsData) {
      this.m_actionWidgetsData[index] = Deref(widgetData);
    };
  }
  ...
  protected final func HasActionWidgetData(const widgetData: script_ref<SActionWidgetPackage>) -> Bool {
    return this.GetActionWidgetDataIndex(Deref(widgetData)) >= 0;
  }

  protected final func HasActionWidget(const widgetData: script_ref<SActionWidgetPackage>) -> Bool {
    return this.GetActionWidget(widgetData) != null;
  }

  protected final func AddActionWidgetData(const widgetData: script_ref<SActionWidgetPackage>) -> Void {
    ArrayPush(this.m_actionWidgetsData, Deref(widgetData));
  }
  ...
  protected final func InitializeActionWidget(widget: ref<inkWidget>, const widgetData: script_ref<SActionWidgetPackage>) -> Void {
  ...
  protected cb func OnDeviceWidgetSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    if spawnData != null {
      widgetData = FromVariant<SDeviceWidgetPackage>(spawnData.m_widgetData);
      widgetData.widget = widget;
      widgetData.libraryID = spawnData.m_libraryID;
  ...
  protected final func CreateDeviceWidget(parentWidget: wref<inkWidget>, const widgetData: script_ref<SDeviceWidgetPackage>) -> wref<inkWidget> {
    let screenDef: ScreenDefinitionPackage = this.GetScreenDefinition();
    let widget: ref<inkWidget> = this.FindWidgetInLibrary(parentWidget, TweakDBInterface.GetWidgetDefinitionRecord(Deref(widgetData).widgetTweakDBID), screenDef.screenDefinition.TerminalScreenType(), screenDef.style, Deref(widgetData).libraryID, Deref(widgetData).libraryPath);
    return widget;
  }

  protected final func UpdateDeviceWidgetData(const widgetData: script_ref<SDeviceWidgetPackage>, index: Int32) -> Void {
    if index >= 0 && index < ArraySize(this.m_deviceWidgetsData) {
      this.m_deviceWidgetsData[index] = Deref(widgetData);
    };
  }
  ...
  protected final func GetDeviceWidget(const widgetData: script_ref<SDeviceWidgetPackage>) -> wref<inkWidget> {
    let index: Int32 = this.GetDeviceWidgetDataIndex(Deref(widgetData));
    if index >= 0 && index < ArraySize(this.m_deviceWidgetsData) {
  ...
  protected final func HasDeviceWidgetData(const widgetData: script_ref<SDeviceWidgetPackage>) -> Bool {
    return this.GetDeviceWidgetDataIndex(Deref(widgetData)) >= 0;
  }

  protected final func HasDeviceWidget(const widgetData: script_ref<SDeviceWidgetPackage>) -> Bool {
    return this.GetDeviceWidget(widgetData) != null;
  }

  protected final func AddDeviceWidgetData(const widgetData: script_ref<SDeviceWidgetPackage>) -> Void {
    ArrayPush(this.m_deviceWidgetsData, Deref(widgetData));
  }
  ...
  protected final func InitializeDeviceWidget(widget: ref<inkWidget>, const widgetData: script_ref<SDeviceWidgetPackage>) -> Void {
    let controller: ref<DeviceWidgetControllerBase> = widget.GetController() as DeviceWidgetControllerBase;
    if controller != null {
      controller.Initialize(this, Deref(widgetData));
    };
    widget.SetVisible(true);
  }
  ...
  protected cb func OnBreadcrumbBarUpdate(value: Variant) -> Bool {
    let data: SBreadCrumbUpdateData = FromVariant<SBreadCrumbUpdateData>(value);
  ...
  protected cb func OnDeviceWidgetsUpdate(value: Variant) -> Bool {
    let widgetsData: array<SDeviceWidgetPackage> = FromVariant<array<SDeviceWidgetPackage>>(value);
    this.UpdateDeviceWidgets(widgetsData);
  }

  protected cb func OnGlitchingStateChanged(value: Variant) -> Bool {
    let glitchData: GlitchData = FromVariant<GlitchData>(value);
  ...
  protected final func IsInteractivityBlocked() -> Bool {
    ...
    if IsDefined(player) {
      isBlockedByAiming = player.GetPlayerStateMachineBlackboard().GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody) == 6;
  ...
  protected final func GoDown(const element: script_ref<SBreadcrumbElementData>) -> Void {
    ArrayPush(this.m_breadcrumbStack, Deref(element));
  }
  ...
  protected final func SetActiveBreadcrumbElement(const element: script_ref<SBreadcrumbElementData>) -> Void {
    this.m_activeBreadcrumb = Deref(element);
  }
  ...
  protected final func RequestBeadcrumbBarUpdate(const data: script_ref<SBreadCrumbUpdateData>) -> Void {
    let breadcrumbEvent: ref<RequestBreadCrumbBarUpdateEvent> = new RequestBreadCrumbBarUpdateEvent();
    breadcrumbEvent.breadCrumbData = Deref(data);
  ...
  protected cb func OnButtonHoverOver(e: ref<inkPointerEvent>) -> Bool {
    ...
    if IsDefined(controller) {
      this.PlaySound(controller.GetWidgetAudioName(), n"OnHoverOver", controller.GetOnHoverOverKeySfxName());
    };
  }
  ...
  protected cb func OnButtonHoverOut(e: ref<inkPointerEvent>) -> Bool {
    let controller: ref<DeviceButtonLogicControllerBase> = e.GetCurrentTarget().GetController() as DeviceButtonLogicControllerBase;
    if IsDefined(controller) {
      this.PlaySound(controller.GetWidgetAudioName(), n"OnHoverOut", controller.GetOnHoverOutKeySfxName());
    };
  }
  ...
  protected cb func OnButtonPress(e: ref<inkPointerEvent>) -> Bool {
    ...
    if e.IsAction(n"click") {
      ...
      if IsDefined(controller) {
        this.PlaySound(controller.GetWidgetAudioName(), n"OnPress", controller.GetOnPressKeySfxName());
      };
    };
  }
  ...
  protected cb func OnButtonRelease(e: ref<inkPointerEvent>) -> Bool {
    ...
    if e.IsAction(n"click") {
      controller = e.GetCurrentTarget().GetController() as DeviceButtonLogicControllerBase;
      if IsDefined(controller) {
        this.PlaySound(controller.GetWidgetAudioName(), n"OnRelease", controller.GetOnReleaseKeySfxName());
      };
    };
  }
