...
public class NetworkMinigameEndScreenController extends inkLogicController {
  ...
  public final func SetUp(const endData: script_ref<EndScreenData>) -> Void {
    let i: Int32;
    let slot: wref<inkWidget>;
    let slotLogic: wref<NetworkMinigameProgramController>;
    this.m_endData = Deref(endData);
    if Equals(Deref(endData).outcome, OutcomeMessage.Success) {
    ...
    i = 0;
    while i < ArraySize(Deref(endData).unlockedPrograms) {
      ...
      slotLogic.Spawn(Deref(endData).unlockedPrograms[i]);
