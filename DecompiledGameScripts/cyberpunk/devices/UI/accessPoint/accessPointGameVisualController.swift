...
public class NetworkMinigameVisualController extends inkLogicController {
  ...
  protected edit const let m_FluffToHideContainer: [inkWidgetRef];

  protected edit const let m_DottedLinesList: [inkWidgetRef];
  ...
  public final func SetUp(const data: script_ref<NetworkMinigameData>) -> Void {
    ...
    this.m_grid.SetUp(Deref(data).gridData);
    this.m_buffer.Spawn(Deref(data).playerBufferSize);
    this.m_programList.Spawn(Deref(data).playerPrograms);
    this.m_basicAccess.Spawn(Deref(data).basicAccess);
  ...
  public final func SetGridElementPicked(const newData: script_ref<NewTurnMinigameData>) -> Void {
    ...
    if Deref(newData).doConsume {
      selectedCell = this.m_grid.FindCellData(Deref(newData).position);
      selectedCell.assignedCell.Consume();
    };
    if Deref(newData).doRegenerateGrid {
      this.m_grid.SetGridData(Deref(newData).regeneratedGridData);
    };
    this.m_grid.SetCurrentActivePosition(Deref(newData).position, Equals(Deref(newData).nextHighlightMode, HighlightMode.Row));
    this.m_buffer.SetEntries(Deref(newData).newPlayerBufferContent);
    this.m_programList.ProcessListModified(Deref(newData).playerProgramsChange, Deref(newData).playerProgramsAdded, Deref(newData).playerProgramsRemoved);
    basicData = this.m_basicAccess.GetData();
    if Deref(newData).basicAccessCompletionState.isComplete && !basicData.wasCompleted {
      this.m_basicAccess.ShowCompleted(Deref(newData).basicAccessCompletionState.revealLocalizedName);
      this.m_programList.PlaySideBarAnim();
    };
    this.m_programList.UpdatePartialCompletionState(Deref(newData).playerProgramsCompletionState);
    this.m_basicAccess.UpdatePartialCompletionState(Deref(newData).basicAccessCompletionState);
    this.m_bufferFillCount += 1;
    newScale.X = 0.18 * Cast<Float>(this.m_bufferFillCount);
    newScale.Y = 1.00;
  ...
  public final func SetProgramCompleted(const id: script_ref<String>, revealLocalizedName: Bool) -> Void;

  public final func ShowEndScreen(const endData: script_ref<EndScreenData>) -> Void {
  ...
  private final func InitializeFluffLines() -> Void {
    ...
    while i < ArraySize(this.m_DottedLinesList) {
      ...
      positionalIndex = Cast<Float>(FloorF((Cast<Float>(i) + 1.00) / 2.00));
