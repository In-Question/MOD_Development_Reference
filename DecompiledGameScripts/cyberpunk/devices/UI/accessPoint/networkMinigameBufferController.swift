...
public class NetworkMinigameBufferController extends inkLogicController {
  ...
  public let m_slotList: [wref<NetworkMinigameElementController>];
  ...
  public final func Spawn(size: Int32) -> Void {
    ...
    newMargin.left = Cast<Float>(50 * this.m_count);
    newMargin.top = 11.00;
  ...
  public final func SetEntries(const toSet: script_ref<[ElementData]>) -> Void {
    let buffLoc: Float;
    let empity: ElementData;
    let newMargin: inkMargin;
    let i: Int32 = 0;
    while i < ArraySize(this.m_slotList) {
      this.m_slotList[i].SetContent(i < ArraySize(Deref(toSet)) ? Deref(toSet)[i] : empity);
      i += 1;
    };
    this.m_count += 1;
    if this.m_count == 2 {
