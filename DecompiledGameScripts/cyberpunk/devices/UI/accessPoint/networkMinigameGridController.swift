...
public class NetworkMinigameGridController extends inkLogicController {
  ...
  public let m_gridData: [CellData];
  ...
  public final func SetUp(const gridData: script_ref<[CellData]>) -> Void {
  ...
  public final func SetGridData(const gridData: script_ref<[CellData]>) -> Void {
    let i: Int32;
    this.Clear();
    this.m_gridData = Deref(gridData);
    i = 0;
    while i < ArraySize(this.m_gridData) {
      this.m_gridData[i].assignedCell = this.AddCell(Deref(gridData)[i]).GetController() as NetworkMinigameGridCellController;
      i += 1;
    };
  }

  private final func AddCell(const toAdd: script_ref<CellData>) -> wref<inkWidget> {
  ...
  public final func SetCurrentActivePosition(position: Vector2, isHorizontal: Bool) -> Void {
    ...
    this.HighlightCellSet(Cast<Int32>(this.m_isHorizontalHighlight ? position.X : position.Y), false, this.m_isHorizontalHighlight);
  }

  public final func SetLastCellSelected(const cell: script_ref<CellData>) -> Void {
    if !Deref(cell).assignedCell.IsConsumed() {
      this.m_lastSelected = Deref(cell);
  ...
  public final func GetGrid() -> [CellData] {
  ...
  public final func HighlightFromCellHover(position: Vector2) -> Void {
    if this.IsOnCurrentCellSet(position) {
      this.HighlightCellSet(Cast<Int32>(this.m_isHorizontalHighlight ? position.Y : position.X), true, !this.m_isHorizontalHighlight);
    };
  }
  ...
  private final func HighlightCellSet(index: Int32, isHover: Bool, isHorizontal: Bool) -> Void {
    ...
    if isHorizontal {
      ...
      inkWidgetRef.SetTranslation(highlightToMove, 0.00, Cast<Float>(index) * cellSize.Y + this.m_gridVisualOffset.Y);
      cellToHighlightPos = new Vector2(Cast<Float>(index), this.m_lastSelected.position.Y);
    } else {
      highlightToMove = isHover ? this.m_verticalHoverHighlight : this.m_verticalCurrentHighlight;
      inkWidgetRef.SetTranslation(highlightToMove, Cast<Float>(index) * cellSize.X + this.m_gridVisualOffset.Y, 0.00);
      cellToHighlightPos = new Vector2(this.m_lastSelected.position.X, Cast<Float>(index));
    };
    if !isHover {
      if IsDefined(this.m_lastHighlighted.assignedCell) {
        this.m_lastHighlighted.assignedCell.SetHighlightStatus(false);
    ...
    if !isHover && !this.m_firstBoot {
      newHorizontalPivot.X = Cast<Float>((130 * index) / 650);
      newVerticalPivor.Y = Cast<Float>((80 * index) / 400);
    ...
    if isHover && !this.m_firstBoot {
      ...
      if isHorizontal {
        ...
        inkWidgetRef.SetTranslation(this.m_horizontalCurrentHighlight, 0.00, Cast<Float>(index) * cellSize.Y + this.m_gridVisualOffset.Y);
      } else {
        ...
        inkWidgetRef.SetTranslation(this.m_verticalCurrentHighlight, Cast<Float>(index) * cellSize.X + this.m_gridVisualOffset.Y, 0.00);
      };
    };
...
public class NetworkMinigameGridCellController extends inkButtonController {
  ...
  public final func Spawn(const setUp: script_ref<CellData>, grid: wref<NetworkMinigameGridController>) -> Void {
    this.m_cellData = Deref(setUp);
    this.m_grid = grid;
    ...
    this.m_slotsContent.SetContent(Deref(setUp).element);
