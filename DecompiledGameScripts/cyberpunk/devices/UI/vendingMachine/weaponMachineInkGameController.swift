...
public class WeaponMachineInkGameController extends VendingMachineInkGameController {
  ...
  protected cb func OnUpdateStatus(value: Variant) -> Bool {
    this.m_state = FromVariant<PaymentStatus>(value);
    switch this.m_state {
  ...
  public func UpdateActionWidgets(const widgetsData: script_ref<[SActionWidgetPackage]>) -> Void {
    ...
    if !IsDefined(this.m_buttonRef) || IsDefined(this.m_buttonRef) && !this.m_buttonRef.IsProcessing() {
      ...
      if ArraySize(Deref(widgetsData)) > 0 {
        widget = this.GetActionWidget(Deref(widgetsData)[0]);
        ...
        if widget == null {
          this.CreateActionWidgetAsync(inkWidgetRef.Get(this.m_ActionsPanel), Deref(widgetsData)[0]);
        } else {
          this.InitializeActionWidget(widget, Deref(widgetsData)[0]);
        };
      };
    };
  }
  ...
  protected func GetOwner() -> ref<WeaponVendingMachine> {
    return this.GetOwnerEntity() as WeaponVendingMachine;
  }
