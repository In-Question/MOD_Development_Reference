...
public class Speaker extends InteractiveDevice {
  ...
  protected let m_targets: [wref<ScriptedPuppet>];
  ...
  protected func StartGlitching(glitchState: EGlitchState, opt intensity: Float) -> Void {
    ...
    if this.GetDevicePS().UseOnlyGlitchSFX() {
      GameObject.AudioSwitch(this, n"radio_station", n"station_none", n"radio");
    } else {
      GameObject.AudioSwitch(this, n"radio_station", this.GetDevicePS().GetCurrentStation(), n"radio");
    };
  }

  protected func StopGlitching() -> Void {
    if this.GetDevicePS().UseOnlyGlitchSFX() {
      GameObject.StopSound(this, this.GetDevicePS().GetGlitchSFX());
    };
    GameObject.AudioSwitch(this, n"radio_station", this.GetDevicePS().GetCurrentStation(), n"radio");
    this.StopGameEffect();
  }
  ...
  protected cb func OnChangeMusicAction(evt: ref<ChangeMusicAction>) -> Bool {
    ...
    if IsDefined(settings as PlayRadio) {
      ...
      this.GetDevicePS().SetCurrentStation(settings.GetSoundName());
    } else {
  ...
  protected final func CreateGameEffect() -> Void {
    ...
    EffectData.SetFloat(this.m_deafGameEffect.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.radius, this.GetDevicePS().GetRange());
  ...
  public const func GetDevicePS() -> ref<SpeakerControllerPS> {
    return this.GetController().GetPS();
  }

  protected const func GetController() -> ref<SpeakerController> {
    return this.m_controller as SpeakerController;
  }
