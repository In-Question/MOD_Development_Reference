...
public class SensorDevice extends ExplosiveDevice {
  ...
  private let m_targets: [ref<Target>];
  ...
  private let m_HPListenersList: [ref<TargetedObjectDeathListener>];
  ...
  protected let m_lightScanRefs: [ref<gameLightComponent>];

  protected let m_lightAttitudeRefs: [ref<gameLightComponent>];

  protected let m_lightInfoRefs: [ref<gameLightComponent>];
  ...
  @default(SniperNest, Senses.BasicTurret)
  protected let m_defaultSensePreset: TweakDBID;

  private let m_keepHostilityTowardsPlayerHostiles: Bool;

  private let m_hostileUpdateTowardsPlayerHostilesDelayed: Bool;

  @runtimeProperty("category", "Sensor entity specific data")
  protected edit const let m_elementsToHideOnTCS: [CName];

  protected let m_elementsToHideOnTCSRefs: [ref<IPlacedComponent>];

  public let m_previoustagKillList: [wref<GameObject>];
  ...
  protected final func ResolveConnectionWithSecuritySystem() -> Void {
    ...
    if IsDefined(secSys) {
      secSys.RequestLatestOutput(Cast<PersistentID>(this.GetEntityID()));
      if !this.GetDevicePS().IsAttitudeChanged() {
  ...
  protected final func HandleSecuritySystemOutput() -> Void {
    if this.GetDevicePS().IsControlledByPlayer() || this.GetDevicePS().IsSecurityWakeUpBlocked() || this.GetDevicePS().IsAttitudeChanged() {
      return;
    };
  ...
  public func OnMaraudersMapDeviceDebug(sink: ref<MaraudersMapDevicesSink>) -> Void {
    ...
    sink.PushString("Sensor Device State", EnumValueToString("ESensorDeviceStates", Cast<Int64>(EnumInt(this.GetSensorDeviceState()))));
    ...
    sink.PushBool("Is Quest Target Spotted", this.GetDevicePS().IsQuestTargetSpotted());
    sink.PushString("Quest Target to Spot", EntityID.ToDebugString(this.GetDevicePS().GetQuestSpotTargetID()));
    sink.PushString("Quest Forced TargetID", EntityID.ToDebugString(this.GetDevicePS().GetForcedTargetID()));
    sink.PushString("Attitude", NameToString(this.GetAttitudeAgent().GetAttitudeGroup()));
    sink.PushBool("Is Attitude Changed", this.GetDevicePS().IsAttitudeChanged());
    sink.PushBool("Is Temporary Attitude Changed", this.IsTemporaryAttitudeChanged());
    sink.PushBool("Is in follow mode", this.GetDevicePS().IsInFollowMode());
    sink.PushBool("Is in Tag Kill Mode", this.GetDevicePS().IsInTagKillMode());
    sink.PushBool("Is Target Forced Tag Kill", this.IsTargetForcedFromTagKill());
    sink.PushBool("Is Idle Forced", this.GetDevicePS().IsIdleForced());
    sink.PushBool("Is Detecting", this.GetDevicePS().IsDetectingDebug());
    sink.PushBool("Can Rotate", this.GetDevicePS().GetBehaviourCanRotate());
  ...
  protected cb func OnPreUninitialize(evt: ref<entPreUninitializeEvent>) -> Bool {
    ...
    this.GetDevicePS().NotifyAboutSpottingPlayer(false);
  }
  ...
  protected cb func OnDetach() -> Bool {
    ...
    if EntityID.IsDynamic(this.GetEntityID()) {
      if this.GetDevicePS().IsControlledByPlayer() {
        this.GetDevicePS().QuestReleaseCurrentObject();
        tcsEvt = new TCSTakeOverControlDeactivate();
        this.OnTCSTakeOverControlDeactivate(tcsEvt);
      };
    };
    this.KeepHostilityTowardsPlayerHostiles(false);
  }

  protected cb func OnAttitudeChanged(evt: ref<AttitudeChangedEvent>) -> Bool {
    let agentPuppet: ref<ScriptedPuppet>;
    super.OnAttitudeChanged(evt);
    if NotEquals(evt.attitude, EAIAttitude.AIA_Friendly) {
      agentPuppet = evt.otherAgent.GetEntity() as ScriptedPuppet;
  ...
  public const func IsPrevention() -> Bool {
    return this.GetDevicePS().IsPartOfPrevention();
  }
  ...
  private const func GetController() -> ref<SensorDeviceController> {
    return this.m_controller as SensorDeviceController;
  }

  public const func GetDevicePS() -> ref<SensorDeviceControllerPS> {
  ...
  public final const func GetCurrentTargets() -> [ref<Target>] {
  ...
  public const func GetRotationData() -> CameraRotationData {
  ...
  protected func TurnOnDevice() -> Void {
    ...
    if !this.GetDevicePS().IsControlledByPlayer() && !this.GetDevicePS().IsInFollowMode() && !this.HasSupport() {
  ...
  private final func RegisterListenerOnTargetHP(target: ref<GameObject>) -> Void {
    ...
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestRegisteringListener(Cast<StatsObjectID>(targetID), gamedataStatPoolType.Health, HPStatListener);
  ...
  protected cb func OnUnregisterListenerOnTargetHPEvent(evt: ref<UnregisterListenerOnTargetHPEvent>) -> Bool {
    if this.m_targetForcedFormTagKill && evt.listener.m_lsitenTarget == this.m_currentlyFollowedTarget {
      this.RevertTemporaryAttitude();
      this.m_targetForcedFormTagKill = false;
    };
    if !IsFinal() {
    };
    if this.GetDevicePS().IsInTagKillMode() {
      GameInstance.GetBlackboardSystem(this.GetGame()).Get(GetAllBlackboardDefs().TaggedObjectsList).Signal(GetAllBlackboardDefs().TaggedObjectsList.taggedObjectsList);
    };
    ArrayRemove(this.m_HPListenersList, evt.listener);
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestUnregisteringListener(Cast<StatsObjectID>(evt.listener.m_lsitenTarget.GetEntityID()), gamedataStatPoolType.Health, evt.listener);
  ...
  private final func ClearAllHPListeners() -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_HPListenersList) {
      GameInstance.GetStatPoolsSystem(this.GetGame()).RequestUnregisteringListener(Cast<StatsObjectID>(this.m_HPListenersList[i].m_lsitenTarget.GetEntityID()), gamedataStatPoolType.Health, this.m_HPListenersList[i]);
      i += 1;
    };
  ...
  public func SetAsIntrestingTarget(target: wref<GameObject>) -> Bool {
    ...
    if this.GetDevicePS().IsConnectedToSecuritySystem() && !this.GetDevicePS().IsAttitudeChanged() {
  ...
  public func OnCurrentTargetAppears(target: wref<GameObject>) -> Void {
    ...
    if this.m_targetForcedFormTagKill || this.GetDevicePS().IsInFollowMode() {
      if !IsFinal() {
      };
    ...
    if this.GetDevicePS().IsConnectedToSecuritySystem() && !this.GetDevicePS().IsAttitudeChanged() {
      ...
      this.StartBehaviourResolve(ESensorDeviceStates.TARGETLOCK);
      this.StartLockingTarget(this.GetBehaviourTimeToTakeAction());
    };
  }

  private final func GetBehaviourTimeToTakeAction() -> Float {
    if this.IsTemporaryAttitudeChanged() {
      return this.GetDevicePS().GetBehaviourtimeToTakeAction() / 2.00;
    };
    return this.GetDevicePS().GetBehaviourtimeToTakeAction();
  }
  ...
  protected final func ChangeTemporaryAttitude() -> Void {
    ...
    if this.m_targetForcedFormTagKill || this.GetDevicePS().IsControlledByPlayer() {
      ...
      if this.GetDevicePS().IsInFollowMode() {
  ...
  private final func CacheInitialAttitude() -> Void {
    if Equals(this.m_initialAttitude, n"None") {
  ...
  public final const func IsTemporaryAttitudeChanged() -> Bool {
    return Equals(this.m_initialAttitude, n"None");
  }

  private final func RevertTemporaryAttitude() -> Void {
    if NotEquals(this.m_initialAttitude, n"None") {
      if !IsFinal() {
      };
  ...
  private final func SetQuestVisibility(isVisible: Bool) -> Void {
    if EntityID.IsDefined(this.GetDevicePS().GetQuestSpotTargetID()) {
      if this.GetDevicePS().GetQuestSpotTargetID() == this.m_currentlyFollowedTarget.GetEntityID() {
        this.GetDevicePS().SetQuestTargetSpotted(isVisible);
      };
    };
    this.GetDevicePS().SetTargetIsLocked(isVisible);
  }
  ...
  protected cb func OnDetectionRiseEvent(evt: ref<DetectionRiseEvent>) -> Bool {
    ...
    if evt.isVisible && this.GetDevicePS().GetBehaviourCanDetectIntruders() {
      if evt.target.IsPlayer() && !this.m_isPLAYERSAFETargetLock && this.GetDetectionFactor() >= 0.00 {
        if !IsDefined(this.m_currentlyFollowedTarget) && !this.GetDevicePS().IsInFollowMode() {
          ...
          this.ToggleActiveEffect(false);
          this.RunGameEffect(this.m_scanGameEffect, this.GetDevicePS().GetScanGameEffectRef(), this.m_scanFXSlotName, this.GetDeviceFXRecord().ScanGameEffectLength());
  ...
  protected cb func OnOnDetectedEvent(evt: ref<OnDetectedEvent>) -> Bool {
    if evt.isVisible {
      if evt.target.IsPlayer() {
        ...
        this.GetDevicePS().NotifyAboutSpottingPlayer(true);
      };
  ...
  protected cb func OnEnterShapeEvent(evt: ref<EnterShapeEvent>) -> Bool {
    let cameraDeadBodyData: ref<CameraDeadBodyData>;
    let timeStamp: Float;
    if Equals(this.GetAttitudeTowards(GetPlayer(this.GetGame())), EAIAttitude.AIA_Friendly) {
      return false;
    };
    ...
    if !evt.target.IsActive() && !GameSessionDataSystem.CheckDataRequest(this.GetGame(), EGameSessionDataType.CameraDeadBody, ToVariant(cameraDeadBodyData)) {
      ...
      if this.GetSensesComponent().IsAgentVisible(GetPlayer(this.GetGame())) && timeStamp - evt.target.GetReceivedDamageByPlayerLastTimeStamp() < 2.80 {
        this.GetDevicePS().TriggerSecuritySystemNotification(GetPlayer(this.GetGame()), GetPlayer(this.GetGame()).GetWorldPosition(), ESecurityNotificationType.COMBAT);
      } else {
    ...
    if this.GetDevicePS().CanTagEnemies() && evt.target.IsActive() {
  ...
  private final func RecognizeTarget(newObject: wref<GameObject>, opt questForcedIntresting: Bool) -> Void {
    let isIntresting: Bool;
    if !this.GetDevicePS().GetBehaviourCanDetectIntruders() {
      if !IsFinal() {
      };
      return;
    };
    ...
    if GameInstance.GetStatPoolsSystem(this.GetGame()).GetStatPoolValue(Cast<StatsObjectID>(newObject.GetEntityID()), gamedataStatPoolType.Health, true) <= 1.00 {
      if !IsFinal() {
      };
      return;
    };
    ...
    if this.GetDevicePS().IsInFollowMode() && this.m_currentlyFollowedTarget == this.GetForcedTargetObject() {
      if !IsFinal() {
      };
      return;
    };
  ...
  public final func LoseTarget(lostObject: wref<GameObject>, opt forceRemoveTarget: Bool) -> Void {
    ...
    if lostObject == this.GetForcedTargetObject() && this.GetDevicePS().IsInFollowMode() {
      if !IsFinal() {
      };
      return;
    };
  ...
  private final func GetForcedTargetObject() -> wref<GameObject> {
    let entID: EntityID = this.GetDevicePS().GetForcedTargetID();
  ...
  protected cb func OnQuestForceAttitude(evt: ref<QuestForceAttitude>) -> Bool {
    ...
    groupName = FromVariant<CName>(evt.prop.first);
    this.m_attitudeAgent.SetAttitudeGroup(groupName);
    if !IsFinal() {
    };
    this.GetDevicePS().SetIsAttitudeChanged(true);
    this.ReevaluateTargets();
  ...
  private final func CanResolveStateChange(newState: ESensorDeviceStates) -> ESensorDeviceStates {
    if this.GetDevicePS().IsIdleForced() {
      if !IsFinal() {
      };
  ...
  private final func ResolveLogicIDLE() -> Void {
    if this.GetDevicePS().GetBehaviourCanRotate() {
  ...
  private final func ResolveLogicLOSETARGET(iterator: Int32) -> Void {
    ...
    switch iterator {
      case 0:
        ...
        delayTime = this.GetDevicePS().GetBehaviourLastTargetLookAtTime();
        break;
      case 1:
        this.ModeSearch(1.00);
        delayTime = this.GetDevicePS().GetBehaviourLostTargetSearchTime();
        break;
      case 2:
        this.ModeIdleNoTarget();
        endCoroutine = true;
  ...
  private final func ResolveLogicJAMMER() -> Void {
    ...
    this.m_animFeatureData.state = 2;
    this.ApplyAnimFeatureToReplicate(this, this.m_animFeatureDataName, this.m_animFeatureData);
    this.CreateLookAt();
  }
  ...
  private final func CreateLookAt(opt position: Vector4, opt otherTarget: ref<GameObject>) -> Void {
    ...
    this.LookAtStop();
    lookAtEntityEventVert = this.SetupLookAtProperties(this.GetDevicePS().GetLookAtPresetVert(), position, otherTarget);
    this.OverrideLookAtSetupVert(lookAtEntityEventVert);
    lookAtEntityEventHor = this.SetupLookAtProperties(this.GetDevicePS().GetLookAtPresetHor(), position, otherTarget);
  ...
  private final func ModeLookAtCurrentTarget() -> Void {
    this.LookAtStop();
    this.m_animFeatureData.state = 0;
    this.ApplyAnimFeatureToReplicate(this, this.m_animFeatureDataName, this.m_animFeatureData);
    this.CreateLookAt();
  }
  ...
  private final func ModeSearch(opt speedMultipler: Float) -> Void {
    this.LookAtStop();
    this.m_animFeatureData.state = 1;
    this.m_animFeatureData.overrideRootRotation = this.GetDevicePS().GetBehaviourOverrideRootRotation();
    this.m_animFeatureData.pitchAngle = this.GetDevicePS().GetBehaviourPitchAngle();
    this.m_animFeatureData.maxRotationAngle = this.GetDevicePS().GetBehaviourMaxRotationAngle();
    this.m_animFeatureData.rotationSpeed = this.GetDevicePS().GetBehaviourRotationSpeed() * speedMultipler;
    this.ApplyAnimFeatureToReplicate(this, this.m_animFeatureDataName, this.m_animFeatureData);
  ...
  private final func ModeIdleNoTarget() -> Void {
    ...
    this.m_animFeatureData.maxRotationAngle = this.GetDevicePS().GetBehaviourMaxRotationAngle();
    if this.GetDevicePS().GetBehaviourCanRotate() {
      this.m_animFeatureData.rotationSpeed = this.GetDevicePS().GetBehaviourRotationSpeed();
      if !this.m_idleSoundIsPlaying && this.GetDevicePS().IsON() && this.m_playIdleSoundOnIdle {
        GameObject.PlaySoundEvent(this, this.m_idleSound);
        this.m_idleSoundIsPlaying = true;
      };
  ...
  protected cb func OnKillTaggedTarget(value: Variant) -> Bool {
    ...
    listOfObjects = FromVariant<array<wref<GameObject>>>(value);
      ...
      if listOfObjects[i] == this {
        ...
        if listOfObjects[i].IsDead() {
          ...
          if IsDefined(listOfObjects[i].GetAttitudeAgent()) {
            ...
            this.OnAllValidTargetsDisappears();
            this.ForceCancelAllForcedBehaviours();
            this.RecognizeTarget(listOfObjects[i]);
            return false;
          };
        };
      };
  ...
  private final func CheckIfTargetIsTaggedByPlayer(object: wref<GameObject>) -> Bool {
    ...
    if this.GetDevicePS().IsInTagKillMode() {
      ...
      listOfObjects = FromVariant<array<wref<GameObject>>>(BBoard.GetVariant(GetAllBlackboardDefs().TaggedObjectsList.taggedObjectsList));
  ...
  private final func ChangeAttiudetowardsTag(const currentList: script_ref<[wref<GameObject>]>) -> Void {
    let attitudeBetweenGroups: EAIAttitude;
    let i: Int32 = ArraySize(this.m_previoustagKillList) - 1;
    while i >= 0 {
      if ArrayContains(Deref(currentList), this.m_previoustagKillList[i]) {
        ArrayErase(this.m_previoustagKillList, i);
      };
      i -= 1;
    };
    i = 0;
    while i < ArraySize(Deref(currentList)) {
      this.GetAttitudeAgent().SetAttitudeTowards(Deref(currentList)[i].GetAttitudeAgent(), EAIAttitude.AIA_Hostile);
      i += 1;
    };
    i = 0;
    ...
    this.m_previoustagKillList = Deref(currentList);
  }
  ...
  protected cb func OnSetDeviceAttitude(evt: ref<SetDeviceAttitude>) -> Bool {
    ...
    let current_attitude: EAIAttitude = this.GetAttitudeTowards(playerPuppet);
    if NotEquals(current_attitude, evt.Attitude) {
      switch evt.Attitude {
        case EAIAttitude.AIA_Friendly:
          this.SetDeviceFriendly(evt);
          break;
        case EAIAttitude.AIA_Neutral:
          this.SetDeviceNeutral(evt);
          break;
        default:
          this.KeepHostilityTowardsPlayerHostiles(false);
      };
  ...
  private final func SetDeviceNeutral(evt: ref<SetDeviceAttitude>) -> Void {
    let enableAimAssist: ref<EnableAimAssist>;
    let puppetAttitudeAgent: ref<AttitudeAgent>;
    this.SetHasSupport(false);
    ...
    this.QueueEvent(enableAimAssist);
    this.SendReprimandInstructionToSecuritySystem();
    this.RevertTemporaryAttitude();
    puppetAttitudeAgent = GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerMainGameObject().GetAttitudeAgent();
    this.m_attitudeAgent.SetAttitudeTowards(puppetAttitudeAgent, EAIAttitude.AIA_Neutral);
    ...
    this.GetDevicePS().SetBlockSecurityWakeUp(true);
  }

  private final func SetDeviceFriendly(evt: ref<SetDeviceAttitude>) -> Void {
    let disableAimAssist: ref<DisableAimAssist>;
    let puppetAttitudeAgent: ref<AttitudeAgent>;
    this.SetHasSupport(false);
    if !this.IsSurveillanceCamera() {
    ...
    this.CacheInitialAttitude();
    puppetAttitudeAgent = GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerMainGameObject().GetAttitudeAgent();
    this.m_attitudeAgent.SetAttitudeTowards(puppetAttitudeAgent, EAIAttitude.AIA_Friendly);
    ...
    if evt.IsCompleted() {
      this.m_attitudeAgent.SetAttitudeGroup(puppetAttitudeAgent.GetAttitudeGroup());
    } else {
      this.m_attitudeAgent.SetAttitudeGroupUnsavable(puppetAttitudeAgent.GetAttitudeGroup());
    };
    this.m_senseComponent.ReevaluateDetectionOverwrite(evt.GetExecutor());
    this.m_senseComponent.RequestRemovingSenseMappin();
    ...
    this.KeepHostilityTowardsPlayerHostiles(!evt.IgnoreHostiles);
    this.GetDevicePS().SetIsAttitudeChanged(true);
    this.ReevaluateTargets();
    this.DetermineLightAttitudeRefs();
  ...
  private final func KeepHostilityTowardsPlayerHostiles(keep: Bool) -> Void {
    let playerPuppet: ref<PlayerPuppet>;
    if Equals(keep, this.m_keepHostilityTowardsPlayerHostiles) {
      return;
    };
    this.m_keepHostilityTowardsPlayerHostiles = keep;
    playerPuppet = GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerMainGameObject() as PlayerPuppet;
    if keep {
      playerPuppet.RegisterFriendlyDeviceHostileToEnemies(this.GetEntityID());
    } else {
      playerPuppet.UnregisterFriendlyDeviceHostileToEnemies(this.GetEntityID());
    };
  }

  protected final func SetHostileTowardsPlayerHostiles(opt useTargetingSystem: Bool) -> Void {
    let i: Int32;
    let player: ref<PlayerPuppet>;
    let playerTargets: array<TrackedLocation>;
    ...
    if useTargetingSystem {
      ...
      player = GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerMainGameObject() as PlayerPuppet;
      GameInstance.GetTargetingSystem(this.GetGame()).GetTargetParts(player, searchQuery, targets);
      size = ArraySize(targets);
      i = 0;
      while i < size {
        target = TS_TargetPartInfo.GetComponent(targets[i]).GetEntity() as GameObject;
        if IsDefined(target) {
          this.m_attitudeAgent.SetAttitudeTowardsAgentGroup(target.GetAttitudeAgent(), this.m_attitudeAgent, EAIAttitude.AIA_Hostile);
        };
        i += 1;
      };
    };
    ...
    while i < ArraySize(playerTargets) {
      this.m_attitudeAgent.SetAttitudeTowardsAgentGroup((playerTargets[i].entity as GameObject).GetAttitudeAgent(), this.m_attitudeAgent, EAIAttitude.AIA_Hostile);
      i += 1;
    };
  }

  public final func ScheduleHostileUpdateTowardsPlayerHostiles() -> Void {
    let evt: ref<HostileUpdateTowardsPlayerHostiles>;
    if !this.m_keepHostilityTowardsPlayerHostiles {
      return;
    };
    if this.m_hostileUpdateTowardsPlayerHostilesDelayed {
      return;
    };
    evt = new HostileUpdateTowardsPlayerHostiles();
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evt, 0.20);
    this.m_hostileUpdateTowardsPlayerHostilesDelayed = true;
  }

  protected cb func OnHostileUpdateTowardsPlayerHostiles(evt: ref<HostileUpdateTowardsPlayerHostiles>) -> Bool {
    if !this.m_keepHostilityTowardsPlayerHostiles {
      return false;
    };
    this.SetHostileTowardsPlayerHostiles();
    this.ReevaluateTargets();
    this.m_hostileUpdateTowardsPlayerHostilesDelayed = false;
  }
    ...
    if Equals(outline, EFocusOutlineType.QUEST) {
      ...
      if Equals(outline, EFocusOutlineType.HOSTILE) {
        ...
        if Equals(outline, EFocusOutlineType.HACKABLE) {
          ...
          if Equals(outline, EFocusOutlineType.FRIENDLY) {
  ...
  protected final func ToggleActiveEffect(active: Bool) -> Void {
    ...
    if active && !this.GetDevicePS().IsControlledByPlayer() && this.GetDevicePS().IsON() {
      if Equals(attitude, EAIAttitude.AIA_Friendly) {
        this.TerminateGameEffect(this.m_idleGameEffectInstance);
        if !IsDefined(this.m_visionConeEffectInstance) {
          this.RunGameEffect(this.m_idleGameEffectInstance, this.GetDevicePS().GetIdleFriendlyRef(), this.m_scanFXSlotName, this.GetDeviceFXRecord().IdleEffectLength());
        };
        ...
        this.TerminateGameEffect(this.m_idleGameEffectInstance);
        if !IsDefined(this.m_visionConeEffectInstance) {
          this.RunGameEffect(this.m_idleGameEffectInstance, this.GetDevicePS().GetIdleActiveRef(), this.m_scanFXSlotName, this.GetDeviceFXRecord().IdleEffectLength());
        };
  ...
  protected cb func OnNetworkLinkQuickhackEvent(evt: ref<NetworkLinkQuickhackEvent>) -> Bool {
    ...
    ps.DrawBetweenEntities(true, false, this.GetFxResourceByKey(n"pingNetworkLink"), evt.to, evt.from, false, false, false, false);
  ...
  public final func RemoveLink() -> Void {
    ...
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestRemovingStatPool(Cast<StatsObjectID>(netrunnerID), gamedataStatPoolType.QuickHackUpload);
    ...
    if EntityID.IsDefined(proxyID) {
      ps.DrawBetweenEntities(false, true, this.GetFxResourceByKey(n"pingNetworkLink"), netrunnerID, proxyID, false, false);
      ps.DrawBetweenEntities(false, true, this.GetFxResourceByKey(n"pingNetworkLink"), proxyID, targetID, false, false);
      ...
      ps.DrawBetweenEntities(false, true, this.GetFxResourceByKey(n"pingNetworkLink"), netrunnerID, targetID, false, false);
      this.ToggleForcedVisibilityInAnimSystem(n"pingNetworkLink", false, 0.00, netrunnerID);
    };
  ...
  public final const func GetTargets() -> [ref<Target>] {
    return this.m_targets;
  }
  ...
  private final func RunVisionConeGameEffect() -> Void {
    ...
    if Equals(attitude, EAIAttitude.AIA_Friendly) || this.GetDevicePS().IsDeviceSecured() && this.GetDevicePS().IsUserAuthorized(playerPuppet.GetEntityID()) {
      this.TerminateGameEffect(this.m_idleGameEffectInstance);
      this.RunGameEffect(this.m_visionConeEffectInstance, this.GetDevicePS().GetVisionConeFriendlyEffectRef(), this.m_scanFXSlotName, this.GetDeviceFXRecord().VisionConeEffectLength());
    } else {
      this.TerminateGameEffect(this.m_idleGameEffectInstance);
      this.RunGameEffect(this.m_visionConeEffectInstance, this.GetDevicePS().GetVisionConeEffectRef(), this.m_scanFXSlotName, this.GetDeviceFXRecord().VisionConeEffectLength());
    };
  }
}
