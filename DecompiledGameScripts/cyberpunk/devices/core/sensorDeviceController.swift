...
public class SetDeviceTagKillMode extends ActionBool {
  ...
  public final func SetProperties() -> Void {
    this.actionName = n"SetDeviceSupportMode";
...
public class SensorDeviceController extends ExplosiveDeviceController {
  ...
  public const func GetPS() -> ref<SensorDeviceControllerPS> {
    return this.GetBasePS() as SensorDeviceControllerPS;
  }
  ...
  private final const func SensorDeviceRenderDebug(debugDrawer: script_ref<DebugDrawer>) -> Void {
    ...
    matrix = EulerAngles.ToMatrix(new EulerAngles(0.00, this.GetPS().GetBehaviourOverrideRootRotation(), 0.00));
    ...
    maxRotationAngle = Vector4.RotateAxis(new Vector4(0.00, 1.00, 0.00, 0.00), new Vector4(1.00, 0.00, 0.00, 0.00), Deg2Rad(this.GetPS().GetBehaviourPitchAngle()));
    FragmentBuilder.AddArrow(fragmentBuilder, new Vector4(0.00, 0.00, 0.00, 0.00), maxRotationAngle);
    FragmentBuilder.AddWireAngledRange(fragmentBuilder, Matrix.Identity(), 0.05, 1.00, 2.00 * this.GetPS().GetBehaviourMaxRotationAngle(), true);
...
public class SensorDeviceControllerPS extends ExplosiveDeviceControllerPS {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;LookAtPreset")
  @default(SecurityTurretControllerPS, LookatPreset.TurretVertical)
  @default(SniperNestControllerPS, LookatPreset.TurretVertical)
  ...
  @default(SniperNestControllerPS, LookatPreset.TurretHorizontal)
  ...
  protected edit let m_ignoreTargetTrackerComponent: Bool;
  ...
  public final const func IgnoreTargetTrackerComponent() -> Bool {
    return this.m_ignoreTargetTrackerComponent;
  }
  ...
  public final func SetLinkedStatusEffect(const value: script_ref<LinkedStatusEffect>) -> Void {
    this.m_linkedStatusEffect = Deref(value);
  }
  ...
  protected final func OnQuestSpotTargetReference(evt: ref<QuestSpotTargetReference>) -> EntityNotificationType {
    ...
    if EntityID.IsDefined(evt.m_ForcedTarget) {
      ...
      GetFixedEntityIdsFromEntityReference(FromVariant<EntityReference>(evt.prop.first), this.GetGameInstance(), followedTargetIds);
      if Cast<Bool>(ArraySize(followedTargetIds)) {
  ...
  public final func OnForceIgnoreTargets(evt: ref<ForceIgnoreTargets>) -> EntityNotificationType {
    let action: ref<SetDeviceAttitude> = this.ActionSetDeviceAttitude();
  ...
  protected final func OnSetDeviceTagKillMode(evt: ref<SetDeviceTagKillMode>) -> EntityNotificationType {
    let action: ref<SetDeviceAttitude>;
    this.SetIsInTagKillMode(true);
    if this.IsAttitudeFromContextHostile() {
      action = this.ActionSetDeviceAttitude();
