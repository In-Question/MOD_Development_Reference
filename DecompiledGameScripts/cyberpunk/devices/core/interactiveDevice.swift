...
public class InteractiveDevice extends Device {
  ...
  private let m_investigationData: [InvestigationData];
    ...
    if Equals(evt.eventType, gameinteractionsEInteractionEventType.EIET_activate) {
      ...
      if Equals(evt.layerData.tag, n"direct") {
        ...
        if IsDefined(evt.hotspot as Door) {
          ...
          if IsDefined(evt.hotspot as ArcadeMachine) {
            this.OnDirectInteractionActive(evt, true);
          };
        };
      ...
      if Equals(requestType, gamedeviceRequestType.Direct) || Equals(requestType, gamedeviceRequestType.Remote) {
        ...
        this.DetermineInteractionStateByTask(generatedContext);
      };
    } else {
      if IsDefined(this.m_interaction) {
        this.m_interaction.ResetChoices(n"None", true);
      };
      if Equals(evt.layerData.tag, n"LogicArea") {
      ...
      if Equals(evt.layerData.tag, n"direct") {
        ...
        if IsDefined(evt.hotspot as Door) {
          ...
          if IsDefined(evt.hotspot as ArcadeMachine) {
            this.OnDirectInteractionActive(evt, false);
          };
        };
      };
      if Equals(evt.layerData.tag, n"ForceReveal") {
  ...
  protected cb func OnLogicReady(evt: ref<SetLogicReadyEvent>) -> Bool {
    let emptyGlobalNodeRef: GlobalNodeRef;
    super.OnLogicReady(evt);
    if this.IsPotentiallyQuickHackable() {
      this.SetInvestigationPositionsArray(emptyGlobalNodeRef, this.GetNodePosition());
    };
  }

  protected const func GetCachedInvestigationPositionsArray(globalNodeRef: GlobalNodeRef) -> [Vector4] {
    let emptyArray: array<Vector4>;
    let index: Int32 = this.GetInvestigationDataIndexFor(globalNodeRef);
    if index != -1 {
      return this.m_investigationData[index].m_investigationPositionsArray;
    };
    return emptyArray;
  }

  private final const func GetInvestigationDataIndexFor(globalNodeRef: GlobalNodeRef) -> Int32 {
    let i: Int32 = 0;
    while i < ArraySize(this.m_investigationData) {
      if Cast<EntityID>(this.m_investigationData[i].m_globalNodeRef) == Cast<EntityID>(globalNodeRef) {
        return i;
      };
      i += 1;
    };
  ...
  protected func SetInvestigationPositionsArray(globalNodeRef: GlobalNodeRef, const arr: script_ref<[Vector4]>) -> Void {
    ...
    let index: Int32 = this.GetInvestigationDataIndexFor(globalNodeRef);
    if index != -1 {
      this.m_investigationData[index].m_investigationPositionsArray = Deref(arr);
    } else {
      data.m_investigationPositionsArray = Deref(arr);
      data.m_globalNodeRef = globalNodeRef;
      ArrayPush(this.m_investigationData, data);
    };
  }

  protected const func HasInvestigationPositionsArrayCached(globalNodeRef: GlobalNodeRef) -> Bool {
    let index: Int32 = this.GetInvestigationDataIndexFor(globalNodeRef);
    return index != -1 && ArraySize(this.m_investigationData[index].m_investigationPositionsArray) > 0;
  }
  ...
  protected cb func OnPerformedAction(evt: ref<PerformedAction>) -> Bool {
    ...
    if IsDefined(sDeviceAction) && this.GetDevicePS().HasActiveContext(gamedeviceRequestType.Direct) || this.GetDevicePS().HasActiveContext(gamedeviceRequestType.Remote) {
      ...
      this.DetermineInteractionStateByTask(currentContext);
    };
  }
  ...
  protected cb func OnEMPHitEvent(evt: ref<EMPHitEvent>) -> Bool {
    ...
    if this.IsActive() {
      ...
      this.GetDevicePS().SetEMPEffectActiveState(true);
  ...
  protected cb func OnEMPEnded(evt: ref<EMPEnded>) -> Bool {
    this.GetDevicePS().SetEMPEffectActiveState(false);
