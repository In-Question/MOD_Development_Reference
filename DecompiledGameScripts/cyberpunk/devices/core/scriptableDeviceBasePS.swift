...
public class SetCustomPersonalLinkReason extends ScriptableDeviceAction {
  ...
  @runtimeProperty("tooltip", "Default == Disable the feature. Other options enables: Disable Personal Link Auto Disconnect and Personal Link Forced flags")
...
public class SetSkillcheckEvent extends Event {
  ...
  @runtimeProperty("tooltip", "You need to select same container type as target device, or it won't work")
...
public struct SPerformedActions {
  ...
  public persistent let ActionContext: [EActionContext];

  public final static func ContainsActionContext(const self: script_ref<SPerformedActions>, actionContext: EActionContext) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(self).ActionContext) {
      if Equals(Deref(self).ActionContext[i], actionContext) {
        return true;
      };
      i += 1;
    };
...
public class ScriptableDeviceComponentPS extends SharedGameplayPS {
  ...
  @default(DoorProximityDetectorControllerPS, Gameplay-Devices-DisplayNames-DoorDetector)
  ...
  @default(FrameControllerPS, Frame)
  ...
  @default(PerkTrainingControllerPS, LocKey#79367)
  ...
  @default(SniperNestControllerPS, LocKey#91723)
  ...
  @default(WardrobeControllerPS, LocKey#2120)
  @default(WeakFenceControllerPS, LocKey#189)
  @default(WeaponTrainingControllerPS, LocKey#79367)
  ...
  @runtimeProperty("category", "Devices Grid")
  ...
  @runtimeProperty("category", "Devices Grid")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;VirtualNetwork")
  public let m_virtualNetworkShapeID: TweakDBID;

  @runtimeProperty("unsavable", "true")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Device")
  ...
  @default(SniperNestControllerPS, Devices.SniperNest)
  ...
  @default(WardrobeControllerPS, Devices.Wardrobe)
  ...
  @default(SniperNestControllerPS, device_descriptions.SecurityTurret)
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;DeviceContentAssignment")
  protected let m_contentScale: TweakDBID;

  protected persistent let m_skillCheckContainer: ref<BaseSkillCheckContainer>;

  @runtimeProperty("category", "UI Zoom / Fullscreen")
  @default(ComputerControllerPS, true)
  @default(FrameControllerPS, true)
  ...
  @default(PerkTrainingControllerPS, true)
  @default(ScriptableDeviceComponentPS, false)
  protected let m_hasPersonalLinkSlot: Bool;

  @runtimeProperty("category", "Backdoor Properties")
  ...
  @runtimeProperty("unsavable", "true")
  @runtimeProperty("category", "Backdoor Properties")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Minigame_Def")
  @runtimeProperty("tooltip", "Specifies what kind of minigame will be forced on the player")
  ...
  @runtimeProperty("category", "Quest")
  @runtimeProperty("tooltip", "IMPORTANT!: If this is set to true. Player will not be disconnected from the personal link automatically. Make sure if QuestForcePersonalLinkDisconnect action is necessary or not ")
  ...
  @default(SniperNestControllerPS, true)
  ...
  @runtimeProperty("category", "Quest")
  @runtimeProperty("tooltip", "IMPORTANT!: IF TRUE > Connect Personal Link interaction will show up regardless of whether it's viable or sensible from gameplay POV. Use cautiously.")
  protected persistent let m_personalLinkForced: Bool;

  @runtimeProperty("category", "Quest")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Interactions.None")
  @runtimeProperty("tooltip", "IMPORTANT!: IF SET UP > This device is treated as if Personal Link Forced was set to true as well.")
  ...
  protected let m_isTimedTurnOff: Bool;
  ...
  @runtimeProperty("category", "NPC workspot")
  ...
  protected persistent let m_currentlyAuthorizedUsers: [SecuritySystemClearanceEntry];

  protected persistent let m_performedActions: [SPerformedActions];
  ...
  private let m_availableQuickHacks: [CName];

  protected let m_isKeyloggerInstalled: Bool;

  private let m_actionsWithDisabledRPGChecks: [TweakDBID];

  private let m_availableSpiderbotActions: [CName];
  ...
  @runtimeProperty("category", "Device Scanner Options")
  ...
  @default(PerkTrainingControllerPS, false)
  @default(ScriptableDeviceComponentPS, true)
  @default(SensorDeviceControllerPS, true)
  ...
  @default(WeaponTrainingControllerPS, false)
  ...
  @runtimeProperty("category", "Device Debug Properties")
  @runtimeProperty("tooltip", "If this is on and someone used DebugDevices console command, only logs from this device will be shown")
  ...
  private let m_isUnderEMPEffect: Bool;

  @runtimeProperty("category", "Device Operations")
  protected inline persistent let m_deviceOperationsSetup: ref<DeviceOperationsContainer>;

  @runtimeProperty("category", "Connection Highlight Obejcts")
  protected persistent const let m_connectionHighlightObjects: [NodeRef];

  private let m_activeContexts: [gamedeviceRequestType];

  private let m_playstyles: [EPlaystyle];

  private let m_quickHackVulnerabilties: [TweakDBID];

  private let m_quickHackVulnerabiltiesInitialized: Bool;

  private let m_willingInvestigators: [EntityID];

  @default(ScriptableDeviceComponentPS, true)
  private persistent let m_isInteractive: Bool;
  ...
  public const func GetParents(out outDevices: [ref<DeviceComponentPS>]) -> Void {
    let i: Int32;
  ...
  public const func GetImmediateParents() -> [ref<DeviceComponentPS>] {
  ...
  protected func SetDeviceState(state: EDeviceStatus) -> Void {
    ...
    if this.CanBeInDeviceChain() && EnumInt(state) <= -1 {
  ...
  protected final func InitializeContentScale() -> Void {
    ...
    powerLevelMod.value = Cast<Float>(GameInstance.GetLevelAssignmentSystem(this.GetGameInstance()).GetLevelAssignment(this.m_contentScale));
    statSystem.AddModifier(Cast<StatsObjectID>(this.GetMyEntityID()), powerLevelMod);
  }
  ...
  protected final func InitializeStats() -> Void {
    ...
    if IsDefined(record) {
      ...
      statSystem.AddModifiers(Cast<StatsObjectID>(this.GetMyEntityID()), statModifiers);
    };
  }
  ...
  protected final func InitializeStatPools() -> Void {
    ...
    if IsDefined(record) {
      ...
      while i < ArraySize(statPools) {
        statPoolsSystem.RequestAddingStatPool(Cast<StatsObjectID>(this.GetMyEntityID()), statPools[i].GetID());
        i += 1;
      };
    };
  }
  ...
  protected final func TryInitializeSkillChecks() -> Void {
    let container: ref<BaseSkillCheckContainer> = this.GetSkillCheckContainerForSetup();
    if IsDefined(container) {
      this.InitializeSkillChecks(container);
    };
  }

  protected final func InitializeSkillChecks(container: ref<BaseSkillCheckContainer>, opt isOverride: Bool) -> Void {
    if !IsDefined(this.m_skillCheckContainer) {
      this.m_skillCheckContainer = container;
    };
    if isOverride || !this.m_skillCheckContainer.IsInitialized() {
      this.InitializeWrapper(container);
    };
  }

  protected final func InitializeWrapper(container: ref<BaseSkillCheckContainer>) -> Void {
  ...
  public final func SetInteractionState(isActive: Bool) -> Void {
    if NotEquals(this.m_isInteractive, isActive) {
      this.m_isInteractive = isActive;
    };
  }
  ...
  public final const quest func IsPersonalLinkDisconnecting() -> Bool {
    return Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.DISCONNECTING);
  }

  public final const quest func IsPersonalLinkDisconnected() -> Bool {
    return Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) || Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.CONNECTING) || Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.DISCONNECTING);
  }
  ...
  public final const func IsTimedTurnOff() -> Bool {
    return this.m_isTimedTurnOff;
  }

  public final const quest func IsDistracting() -> Bool {
  ...
  public final const func GetPlaystyles() -> [EPlaystyle] {
  ...
  public final func PassDeviceName(const deviceName: script_ref<String>, opt dbgDeviceName: CName) -> Void {
    if !IsStringValid(this.m_deviceName) {
      this.m_deviceName = Deref(deviceName);
    };
  }
  ...
  public final func GetAllQuickHackVulnerabilities() -> [TweakDBID] {
    this.InitializeQuickHackVulnerabilities();
    return this.m_quickHackVulnerabilties;
  }

  public final func GetActiveQuickHackVulnerabilities() -> [TweakDBID] {
    let i: Int32;
  ...
  public final const quest func IsON() -> Bool {
    return Equals(this.m_deviceState, EDeviceStatus.ON);
  }

  protected final func IsOFFTimed() -> Bool {
    return !(!this.IsON() && this.IsTimedTurnOff());
  }

  public final const quest func IsOFF() -> Bool {
    return Equals(this.m_deviceState, EDeviceStatus.OFF);
  }
  ...
  public final const quest func IsDisabled() -> Bool {
    return Equals(this.m_deviceState, EDeviceStatus.DISABLED);
  }

  public final const quest func IsPoweredAndEnabled() -> Bool {
  ...
  public final func GetActionByName(actionName: CName, const context: script_ref<GetActionsContext>) -> ref<DeviceAction> {
    let actions: array<ref<DeviceAction>>;
    let foundAction: ref<DeviceAction>;
    let i: Int32;
    if Equals(Deref(context).requestType, gamedeviceRequestType.Remote) {
      ...
      this.GetActions(actions, Deref(context));
    };
    i = 0;
    while i < ArraySize(actions) {
      if Equals(actions[i].actionName, actionName) {
  ...
  public final func GetMinigameActionByName(actionName: CName, const context: script_ref<GetActionsContext>) -> ref<DeviceAction> {
    let actions: array<ref<DeviceAction>>;
    let foundAction: ref<DeviceAction>;
    let i: Int32;
  ...
  public final const func IsPlayerPerformingTakedown() -> Bool {
    ...
    let playerStateDefault: Bool = blackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Takedown) == 0;
    return !playerStateDefault;
  }

  public final const func IsDisruptivePlayerStatusEffectPresent() -> Bool {
    let player: ref<PlayerPuppet> = GetPlayer(this.GetGameInstance());
    return this.m_isUnderEMPEffect || StatusEffectSystem.ObjectHasStatusEffectOfType(player, gamedataStatusEffectType.Stunned) || StatusEffectSystem.ObjectHasStatusEffectOfType(player, gamedataStatusEffectType.Knockdown) || StatusEffectSystem.ObjectHasStatusEffectOfType(player, gamedataStatusEffectType.Electrocuted);
  }
  ...
  protected func GetMinigameActions(outActions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void;

  protected final func FinalizeGetQuickHackActions(outActions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void {
  ...
  protected final func FinalizeGetActions(out outActions: [ref<DeviceAction>]) -> Void {
  ...
  protected final func GetActionsRestrictionData(allowedNames: script_ref<[String]>, disallowedNames: script_ref<[String]>, inactiveReason: script_ref<String>) -> Bool {
    ...
    if IsDefined(tmpVariant) {
      actionRestrictions = FromVariant<array<TweakDBID>>(tmpVariant);
    };
    i = 0;
    while i < ArraySize(actionRestrictions) {
      if TDBID.IsValid(actionRestrictions[i]) {
        ...
        if arrSize > 0 {
          i1 = 0;
          while i1 < arrSize {
            if !ArrayContains(Deref(allowedNames), record.GetAllowedActionNamesItem(i1)) {
              ArrayPush(Deref(allowedNames), record.GetAllowedActionNamesItem(i1));
            };
            i1 += 1;
          };
          if ArraySize(Deref(allowedNames)) > 0 {
        ...
        if arrSize > 0 {
          i1 = 0;
          while i1 < arrSize {
            if this.IgnoreDisallowedAction(record.GetDisallowedActionNamesItem(i1)) {
            } else {
              if !ArrayContains(Deref(disallowedNames), record.GetDisallowedActionNamesItem(i1)) {
                ArrayPush(Deref(disallowedNames), record.GetDisallowedActionNamesItem(i1));
              };
            };
            i1 += 1;
          };
          if ArraySize(Deref(disallowedNames)) > 0 {
            inactiveReason = record.InactiveReason();
            return true;
          };
        };
  ...
  protected func IgnoreDisallowedAction(actionName: String) -> Bool {
    return false;
  }

  protected final func FindActionInTweakList(actionName: CName, const allowedNames: script_ref<[String]>) -> Bool {
    let i: Int32;
    if ArraySize(Deref(allowedNames)) <= 0 {
      return false;
    };
    i = 0;
    while i < ArraySize(Deref(allowedNames)) {
      if Equals(NameToString(actionName), Deref(allowedNames)[i]) {
        return true;
      };
      i += 1;
    };
  ...
  public final func SetBlockSecurityWakeUp(value: Bool) -> Void {
  ...
  public final const func GetSecurityAreasWithUsersInside(uniqueUsers: script_ref<[AreaEntry]>) -> [ref<SecurityAreaControllerPS>] {
    ...
    while i < ArraySize(secAreas) {
      ...
      while j < ArraySize(areaUsers) {
        if !ArrayContains(Deref(uniqueUsers), areaUsers[j]) {
          ArrayPush(Deref(uniqueUsers), areaUsers[j]);
        };
        j += 1;
      };
      i += 1;
  ...
  public final const func GetSecurityAreasWithUsersInside() -> [ref<SecurityAreaControllerPS>] {
    let trash: array<AreaEntry>;
    return this.GetSecurityAreasWithUsersInside(trash);
  }

  public final const func GetSecurityAreasWithUserInside(whoToCheck: EntityID) -> [ref<SecurityAreaControllerPS>] {
    let i: Int32;
  ...
  public final const func GetSecurityAreasWithUserInside(whoToCheck: ref<GameObject>) -> [ref<SecurityAreaControllerPS>] {
  ...
  public final const func CurrentlyAuthorizedUsers() -> [SecuritySystemClearanceEntry] {
    return this.m_currentlyAuthorizedUsers;
  }

  public final const func UserAuthorizationAttempt(userToAuthorize: EntityID, opt password: CName, opt userToAuthorizeHandle: ref<GameObject>) -> Bool {
    let i: Int32;
    ...
    if !IsDefined(userToAuthorizeHandle) {
      userToAuthorizeHandle = GameInstance.FindEntityByID(this.GetGameInstance(), userToAuthorize) as GameObject;
    };
    i = 0;
    while i < ArraySize(keycards) {
      if GameInstance.GetTransactionSystem(this.GetGameInstance()).HasItem(userToAuthorizeHandle, ItemID.CreateQuery(keycards[i])) {
        this.AddUser(userToAuthorize, this.GetMySecurityAccessLevel());
        return true;
      };
      i += 1;
  ...
  protected func ShouldForceAuthorizeUser(const context: script_ref<GetActionsContext>) -> Bool {
    return Equals(Deref(context).requestType, gamedeviceRequestType.External) && Deref(context).allowsRemoteAuthorization;
  }
  ...
  public final const func GetBackdoorDevices() -> [ref<ScriptableDeviceComponentPS>] {
  ...
  protected func ActionHacking(const context: script_ref<GetActionsContext>) -> ref<ActionHacking> {
  ...
  protected func PushSkillCheckActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Bool {
    ...
    if Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) && IsDefined(this.m_skillCheckContainer.GetHackingSlot()) && this.m_skillCheckContainer.GetHackingSlot().IsActive() && this.IsON() && this.HasPersonalLinkSlot() {
      skillCheckAction = this.ActionHacking(context);
      skillCheckAction.RegisterAsRequester(Deref(context).requestorID);
    ...
    if this.CanAddEngineeringSkillcheck() && IsDefined(this.m_skillCheckContainer.GetEngineeringSlot()) && this.m_skillCheckContainer.GetEngineeringSlot().IsActive() {
  ...
  protected final func HasCyberdeck() -> Bool {
    return GameInstance.GetStatsSystem(this.GetGameInstance()).GetStatBoolValue(Cast<StatsObjectID>(this.GetPlayerEntityID()), gamedataStatType.HasCyberdeck);
  }
  ...
  public final func CreateSkillcheckInfo(const context: script_ref<GetActionsContext>) -> [UIInteractionSkillCheck] {
    ...
    if this.m_skillCheckContainer.GetHackingSlot().IsActive() && this.IsON() {
      skillcheckDescription = this.ActionHacking(context).CreateSkillcheckInfo(Deref(context).processInitiatorObject);
      ArrayPush(blackboardDescription, skillcheckDescription);
    };
    if this.m_skillCheckContainer.GetEngineeringSlot().IsActive() {
      skillcheckDescription = this.ActionEngineering(context).CreateSkillcheckInfo(Deref(context).processInitiatorObject);
      ArrayPush(blackboardDescription, skillcheckDescription);
    };
    if this.m_skillCheckContainer.GetDemolitionSlot().IsActive() {
      skillcheckDescription = this.ActionDemolition(context).CreateSkillcheckInfo(Deref(context).processInitiatorObject);
      ArrayPush(blackboardDescription, skillcheckDescription);
    };
  ...
  protected final func IsSpiderbotActionsConditionsFulfilled() -> Bool {
    if GameInstance.GetStatsSystem(this.GetGameInstance()).GetStatBoolValue(Cast<StatsObjectID>(GameInstance.GetPlayerSystem(this.GetGameInstance()).GetLocalPlayerMainGameObject().GetEntityID()), gamedataStatType.HasSpiderBotControl) {
  ...
  protected func ActionQuickHackHighPitchNoise() -> ref<QuickHackHighPitchNoise> {
    let action: ref<QuickHackHighPitchNoise> = new QuickHackHighPitchNoise();
    action.SetUp(this);
    action.SetProperties();
    action.AddDeviceName(this.m_deviceName);
    action.CreateInteraction();
  ...
  protected func ActionQuickHackAoeDamage() -> ref<QuickHackAoeDamage> {
    let action: ref<QuickHackAoeDamage> = new QuickHackAoeDamage();
    action.SetUp(this);
    action.SetProperties();
    action.AddDeviceName(this.m_deviceName);
    action.CreateInteraction();
  ...
  protected final func OnQuickHackHighPitchNoise(evt: ref<QuickHackHighPitchNoise>) -> EntityNotificationType {
    if evt.IsStarted() {
      this.ExecutePSActionWithDelay(evt, this, evt.GetDurationValue());
    };
    this.UseNotifier(evt);
  ...
  protected final func OnQuickHackAoeDamage(evt: ref<QuickHackAoeDamage>) -> EntityNotificationType {
    this.UseNotifier(evt);
    return EntityNotificationType.SendThisEventToEntity;
  }
  ...
  public final const func GetConnectionHighlightObjects() -> [NodeRef] {
  ...
  private final const func FinalizeNetworkLinkRegistration(registerAsMaster: Bool, const relevantDevices: script_ref<[ref<DeviceComponentPS>]>, breachedResource: FxResource, defaultResource: FxResource, isPing: Bool, lifetime: Float, opt revealSlave: Bool, opt revealMaster: Bool) -> Void {
    let i: Int32;
    ...
    if ArraySize(Deref(relevantDevices)) == 0 {
      return;
    };
    ...
    i = 0;
    while i < ArraySize(Deref(relevantDevices)) {
      if Deref(relevantDevices)[i].IsAttachedToGame() {
        otherDevice = Deref(relevantDevices)[i].GetOwnerEntityWeak() as Device;
      ...
      if registerAsMaster {
        ...
        linkData.slaveID = PersistentID.ExtractEntityID(Deref(relevantDevices)[i].GetID());
        ...
        linkData.masterID = PersistentID.ExtractEntityID(Deref(relevantDevices)[i].GetID());
        linkData.masterPos = otherDevicePosition;
        if (Deref(relevantDevices)[i] as ScriptableDeviceComponentPS).IsBreached() {
          resourceToUse = breachedResource;
        } else {
          resourceToUse = defaultResource;
        };
      };
      linkData.isDynamic = this.IsLinkDynamic() || (Deref(relevantDevices)[i] as ScriptableDeviceComponentPS).IsLinkDynamic();
  ...
  protected func GetInkWidgetTweakDBID(const context: script_ref<GetActionsContext>) -> TweakDBID {
    if !this.IsUserAuthorized(Deref(context).processInitiatorObject.GetEntityID()) && !Deref(context).ignoresAuthorization {
      if this.IsDeviceSecuredWithPassword() {
        return this.GetKeypadWidgetStyle();
      };
      return this.GetKeypadWidgetStyle();
    };
  ...
  protected func GetActionWidgets(const context: script_ref<GetActionsContext>) -> [SActionWidgetPackage] {
    let action: ref<ScriptableDeviceAction>;
    let actions: array<ref<DeviceAction>>;
    let i: Int32;
  ...
  public func RequestDeviceWidgetsUpdate(blackboard: ref<IBlackboard>) -> Void {
    ...
    this.RequestDeviceWidgetsUpdate(blackboard, widgetsData);
  }

  protected final func RequestDeviceWidgetsUpdate(blackboard: ref<IBlackboard>, widgetsData: [SDeviceWidgetPackage]) -> Void {
  ...
  public final func StorePerformedActionID(oryginalAction: ref<ScriptableDeviceAction>) -> Void {
    ...
    if index > -1 {
      ...
      ArrayErase(this.m_performedActions, index);
    } else {
  ...
  public final const func GetPerformedActionsIDs() -> [CName] {
    let IDs: array<CName>;
    let i: Int32 = 0;
  ...
  public final const func GetPerformedActions() -> [SPerformedActions] {
  ...
  private final func SceneInteractionsBlocked() -> Bool {
    let system: ref<InteractionManager> = GameInstance.GetInteractionManager(this.GetGameInstance());
    if IsDefined(system) {
  ...
  public func DetermineInteractionState(interactionComponent: ref<InteractionComponent>, const context: script_ref<GetActionsContext>) -> Void {
    let actions: array<ref<DeviceAction>>;
    ...
    if this.IsMarkedAsQuest() || this.IsAnyMasterFlaggedAsQuest() || IsNameValid(this.GetFactToDisableQuestMarkName()) {
      if this.SceneInteractionsBlocked() {
        return;
      };
    };
    if !(Equals(Deref(context).requestType, gamedeviceRequestType.Direct) || Equals(Deref(context).requestType, gamedeviceRequestType.Remote)) {
      return;
    };
    if this.m_isInteractive && !this.GetHudManager().IsQuickHackPanelOpened() {
      ...
      if ArraySize(activeChoices) == 0 && NotEquals(Deref(context).requestType, gamedeviceRequestType.Remote) {
  ...
  public final static func SetActionsInactiveAll(actions: script_ref<[ref<DeviceAction>]>, opt reason: String, opt exludedAction: CName) -> Void {
    let sAction: ref<ScriptableDeviceAction>;
    let i: Int32 = 0;
  ...
  private final func PushChoicesToInteractionComponent(interactionComponent: ref<InteractionComponent>, const context: script_ref<GetActionsContext>, choices: script_ref<[InteractionChoice]>) -> Void {
    ...
    if shouldPushChoices {
      if this.IsUserAuthorized(Deref(context).processInitiatorObject.GetEntityID()) {
        this.TutorialProcessSkillcheck(choices);
      };
      if IsDefined(interactionComponent) {
  ...
  private final func TutorialProcessSkillcheck(const choices: script_ref<[InteractionChoice]>) -> Void {
    let i: Int32;
    ...
    i = 0;
    while i < ArraySize(Deref(choices)) {
      if ChoiceTypeWrapper.IsType(Deref(choices)[i].choiceMetaData.type, gameinteractionsChoiceType.CheckFailed) {
  ...
  protected func PushReturnActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Bool {
    ...
    if this.IsUnpowered() {
      ...
      if IsDefined(Deref(context).processInitiatorObject) {
        if !this.IsUserAuthorized(Deref(context).processInitiatorObject.GetEntityID()) && !Deref(context).ignoresAuthorization && !this.CanPayToAuthorize() {
          shouldReturn = true;
  ...
  protected final const func SetActionIllegality(outActions: script_ref<[ref<DeviceAction>]>, isIllegal: Bool) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(outActions)) {
      if IsDefined(Deref(outActions)[i] as ScriptableDeviceAction) {
        (Deref(outActions)[i] as ScriptableDeviceAction).SetIllegal(isIllegal);
      };
      i += 1;
    };
  }

  public final func GetRemoteActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Void {
  ...
  protected final const func EvaluateActionsRPGAvailabilty(outActions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void {
    let action: ref<ScriptableDeviceAction>;
    let i: Int32;
    let reason: String;
    if Deref(context).ignoresRPG {
    ...
    i = ArraySize(Deref(outActions)) - 1;
    while i >= 0 {
      action = Deref(outActions)[i] as ScriptableDeviceAction;
      ...
      if !this.IsActionRPGRequirementDisabled(action.GetObjectActionID()) && !action.IsVisible(context) {
        ...
        if IsDefined(action as RemoteBreach) && IsStringValid(reason) {
  ...
  protected final func SetActionsQuickHacksExecutioner(outActions: script_ref<[ref<DeviceAction>]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(outActions)) {
      if IsDefined(Deref(outActions)[i] as ScriptableDeviceAction) {
        (Deref(outActions)[i] as ScriptableDeviceAction).SetExecutor(this.GetLocalPlayerControlledGameObject());
      };
      i += 1;
    };
  }

  protected final func MarkActionsAsQuickHacks(actionsToMark: script_ref<[ref<DeviceAction>]>) -> Void {
    let acion: ref<ScriptableDeviceAction>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(actionsToMark)) {
      acion = Deref(actionsToMark)[i] as ScriptableDeviceAction;
      if IsDefined(acion) {
  ...
  protected final func MarkActionsAsSpiderbotActions(actionsToMark: script_ref<[ref<DeviceAction>]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(actionsToMark)) {
      if IsDefined(Deref(actionsToMark)[i] as ScriptableDeviceAction) {
        (Deref(actionsToMark)[i] as ScriptableDeviceAction).SetAsSpiderbotAction();
      };
      i += 1;
    };
  }

  protected final func ExtractActions(const actionNames: script_ref<[CName]>) -> [ref<DeviceAction>] {
    let extractedActions: array<ref<DeviceAction>>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(actionNames)) {
      ArrayPush(extractedActions, this.GetActionByName(Deref(actionNames)[i]));
  ...
  public func GetQuestActionByName(actionName: CName) -> ref<DeviceAction> {
    ...
    switch actionName {
      ...
      case n"QuestRemoveQuickHacks":
        ...
        break;
      case n"QuestRestoreQuickHacks":
        action = this.ActionQuestRestoreQuickHacks();
        break;
  ...
  public func GetQuestActions(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Void {
    if Clearance.IsInRange(Deref(context).clearance, 99) {
      ...
      ArrayPush(outActions, this.ActionQuestRestoreQuickHacks());
  ...
  public func ActionToggleON() -> ref<ToggleON> {
    ...
    action.AddDeviceName(this.m_deviceName);
    action.CreateInteraction();
    action.SetWidgetStyle(this.m_deviceUIStyle);
    action.CreateActionWidgetPackage();
    return action;
  }
  ...
  protected final func ActionTogglePersonalLink(executor: wref<GameObject>, opt questForcesDisconnection: Bool, opt skipMinigame: Bool) -> ref<TogglePersonalLink> {
    ...
    if questForcesDisconnection && Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) {
      this.SetPersonalLinkStatus(EPersonalLinkConnectionStatus.CONNECTED);
    };
    ...
    if this.IsPersonalLinkActionIllegal() {
      action.SetIllegal(true);
    };
    if !this.CanConnectToPersonalLink() {
      action.SetInactive();
    };
    return action;
  }
  ...
  protected final func CanConnectToPersonalLink() -> Bool {
    return Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) && !this.IsPlayerPerformingTakedown() && !this.IsDisruptivePlayerStatusEffectPresent();
  }

  public final func OnTogglePersonalLink(evt: ref<TogglePersonalLink>) -> EntityNotificationType {
    if (this.IsPlayerPerformingTakedown() || this.IsDisruptivePlayerStatusEffectPresent()) && Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) {
      return EntityNotificationType.DoNotNotifyEntity;
    };
    if Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.NOT_CONNECTED) && evt.GetDurationValue() == 0.00 {
      return EntityNotificationType.DoNotNotifyEntity;
    };
    if Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.DISCONNECTING) && evt.IsCompleted() {
      this.SetPersonalLinkStatus(EPersonalLinkConnectionStatus.NOT_CONNECTED);
      this.UseNotifier(evt);
      return EntityNotificationType.DoNotNotifyEntity;
    };
    if Equals(this.m_personalLinkStatus, EPersonalLinkConnectionStatus.CONNECTED) {
      this.SetPersonalLinkStatus(EPersonalLinkConnectionStatus.DISCONNECTING);
  ...
  private final func SetPersonalLinkStatus(status: EPersonalLinkConnectionStatus) -> Void {
    this.m_personalLinkStatus = status;
    this.GetNetworkBlackboard().SetInt(this.GetNetworkBlackboardDef().PersonalLinkStatus, EnumInt(this.m_personalLinkStatus));
  }
  ...
  public final func ActionQuestForceCameraZoom(enable: Bool, opt instant: Bool) -> ref<QuestForceCameraZoom> {
  ...
  public func OnQuestForceCameraZoom(evt: ref<QuestForceCameraZoom>) -> EntityNotificationType {
    this.UseNotifier(evt);
    this.m_isAdvancedInteractionModeOn = FromVariant<Bool>(evt.prop.first);
    if !IsFinal() {
    };
    return EntityNotificationType.SendThisEventToEntity;
  }
  ...
  protected func ActionScavenge(const context: script_ref<GetActionsContext>) -> ref<ActionScavenge> {
  ...
  public final func OnActionScavenge(evt: ref<ActionScavenge>) -> EntityNotificationType {
    ...
    GameInstance.GetActivityLogSystem(this.GetGameInstance()).AddLog("Scraps looted: " + IntToString(FromVariant<Int32>(evt.prop.first)));
    this.UseNotifier(evt);
    if !IsFinal() {
    };
    return EntityNotificationType.SendThisEventToEntity;
  ...
  public func OnQuickHackToggleOn(evt: ref<QuickHackToggleON>) -> EntityNotificationType {
    ...
    this.SetBlockSecurityWakeUp(FromVariant<Bool>(evt.prop.first));
    evt.SetDurationValue(evt.GetDurationTime());
    action = this.ActionToggleON();
    this.ExecutePSAction(action, n"remote");
    this.UseNotifier(evt);
    if !this.IsON() && evt.Repeat {
      evt.Repeat = false;
    };
    if this.IsON() && evt.IsStarted() && evt.Repeat {
      this.m_isTimedTurnOff = true;
    } else {
      this.m_isTimedTurnOff = false;
    };
    return EntityNotificationType.SendThisEventToEntity;
  }
  ...
  protected final func ActionQuestRestoreQuickHacks() -> ref<QuestRestoreQuickHacks> {
    let action: ref<QuestRestoreQuickHacks> = new QuestRestoreQuickHacks();
    action.SetUp(this);
    action.SetProperties();
    action.AddDeviceName(this.m_deviceName);
    return action;
  ...
  public func OnQuestRestoreQuickHacks(evt: ref<QuestRestoreQuickHacks>) -> EntityNotificationType {
    this.ExposeQuickHacks(true);
  ...
  public func OnQuestResetDeviceToInitialState(evt: ref<QuestResetDeviceToInitialState>) -> EntityNotificationType {
    ...
    GameInstance.GetStatPoolsSystem(this.GetGameInstance()).RequestSettingStatPoolMaxValue(Cast<StatsObjectID>(this.GetMyEntityID()), gamedataStatPoolType.Health, null);
  ...
  protected final func TriggerWakeUpDelayedEvent(duration: Int32) -> Void {
    ...
    GameInstance.GetDelaySystem(this.GetGameInstance()).DelayPSEvent(this.GetID(), this.GetClassName(), wakeUpEvent, Cast<Float>(duration));
  }
  ...
  protected func WakeUpDevice() -> Bool {
    if this.IsOFF() && !this.IsSecurityWakeUpBlocked() && !this.IsBroken() {
  ...
  protected final const func GetFullAuthorizationData() -> [SecurityAccessLevelEntryClient] {
  ...
  private final const func GetFullAuthorizationDataSegregated(passwords: script_ref<[CName]>, keycards: script_ref<[TweakDBID]>) -> Void {
  ...
  public final const func GetKeycards() -> [TweakDBID] {
    let securityData: array<SecurityAccessLevelEntryClient> = this.GetFullAuthorizationData();
    let keycards: array<TweakDBID> = this.ExtractKeycardsFromAuthorizationData(securityData);
    return keycards;
  }

  protected final const func ExtractKeycardsFromAuthorizationData(const data: script_ref<[SecurityAccessLevelEntryClient]>) -> [TweakDBID] {
    let keycards: array<TweakDBID>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(data)) {
      if SecurityAccessLevelEntryClient.IsKeycardValid(Deref(data)[i]) {
        ArrayPush(keycards, Deref(data)[i].m_keycard);
      };
      i += 1;
    };
    return keycards;
  ...
  protected final const func ExtractPasswordsFromAuthorizationData(const data: script_ref<[SecurityAccessLevelEntryClient]>) -> [CName] {
    let passwords: array<CName>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(data)) {
      if SecurityAccessLevelEntryClient.IsPasswordValid(Deref(data)[i]) {
        ArrayPush(passwords, Deref(data)[i].m_password);
      };
      i += 1;
    };
    return passwords;
  ...
  protected final const func ExtractKeycardsFromAuthorizationData(const data: script_ref<[SecurityAccessLevelEntry]>) -> [TweakDBID] {
    let keycards: array<TweakDBID>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(data)) {
      if SecurityAccessLevelEntry.IsKeycardValid(Deref(data)[i]) {
        ArrayPush(keycards, Deref(data)[i].m_keycard);
      };
      i += 1;
    };
  ...
  protected final const func ExtractPasswordsFromAuthorizationData(const data: script_ref<[SecurityAccessLevelEntry]>) -> [CName] {
    let passwords: array<CName>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(data)) {
      if SecurityAccessLevelEntry.IsPasswordValid(Deref(data)[i]) {
        ArrayPush(passwords, Deref(data)[i].m_password);
      };
      i += 1;
    };
  ...
  public final const func ExtractIDs(const persistentStates: script_ref<[ref<PersistentState>]>, persistentIDs: script_ref<[PersistentID]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(persistentStates)) {
      ArrayPush(Deref(persistentIDs), Deref(persistentStates)[i].GetID());
      i += 1;
    };
  }
  ...
  public final const func GetAvailableQuickHacks() -> [CName] {
  ...
  protected final func UpdateAvailAbleQuickHacks(const actions: script_ref<[ref<DeviceAction>]>) -> Void {
    let action: ref<ScriptableDeviceAction>;
    let i: Int32;
    this.ClearAvailableQuickHacks();
    i = 0;
    while i < ArraySize(Deref(actions)) {
      action = Deref(actions)[i] as ScriptableDeviceAction;
      if IsDefined(action) {
  ...
  public final const func GetAvailableSpiderbotActions() -> [CName] {
  ...
  protected final func UpdateAvailableSpiderbotActions(const actions: script_ref<[ref<DeviceAction>]>) -> Void {
    let i: Int32;
    this.ClearAvailableSpiderbotActions();
    i = 0;
    while i < ArraySize(Deref(actions)) {
      this.AddAvailableSpiderbotActions(Deref(actions)[i].actionName);
      i += 1;
    };
  }
  ...
  public final func EnableDebugQuickHacks(val: Int32) -> Void {
    this.m_debugExposeQuickHacks = Cast<Bool>(val);
    if val > 0 {
      this.m_isScanned = true;
    };
  }

  public final const func GetMasterDevicesTweaks() -> [TweakDBID] {
  ...
  public final func SetDurabilityState(newState: EDeviceDurabilityState) -> Void {
    let removeFromChain: ref<RemoveFromChainRequest>;
    if EnumInt(newState) >= 2 && this.CanBeInDeviceChain() {
      removeFromChain = new RemoveFromChainRequest();
      removeFromChain.requestSource = this.GetMyEntityID();
      this.GetTakeOverControlSystem().QueueRequest(removeFromChain);
    };
  ...
  public final const func GetUniqueConnectionTypes() -> [DeviceConnectionScannerData] {
  ...
  public final const func GetDistractionDuration(action: ref<ScriptableDeviceAction>) -> Float {
    ...
    if this.TryGetDevice(device) {
      result = MaxF(device.GetAreaEffectLifetimeByAction(action), action.GetDurationTime());
    };
    return result;
  }

  public final const func GetAreaEffectStimRangeByAction(action: ref<ScriptableDeviceAction>) -> Float {
    let device: wref<Device>;
    return this.TryGetDevice(device) ? device.GetAreaEffectStimRangeByAction(action) : 0.00;
  }

  protected final const func TryGetDevice(out device: wref<Device>) -> Bool {
    device = this.GetOwnerEntityWeak() as Device;
  ...
  public final static func IsConnectedToMaintenanceSystem() -> Bool {
    return false;
  }

  public final func PushPersistentData(const data: script_ref<BaseDeviceData>) -> Void {
    let defaultState: EDeviceStatus;
    if !this.m_wasStateSet {
      this.SetDeviceState(NotEquals(Deref(data).m_deviceState, defaultState) ? Deref(data).m_deviceState : defaultState);
      this.CacheDeviceState(NotEquals(this.m_deviceState, defaultState) ? this.m_deviceState : defaultState);
    };
    this.m_destructionProperties.m_durabilityType = Deref(data).m_durabilityType;
  }
  ...
  protected final func OnQuestResolveSkillchecks(evt: ref<ResolveAllSkillchecksEvent>) -> EntityNotificationType {
    this.QuestResolveSkillchecks();
    return EntityNotificationType.SendThisEventToEntity;
  }

  protected final func OnQuestSetSkillchecks(evt: ref<SetSkillcheckEvent>) -> EntityNotificationType {
    this.QuestSetSkillchecks(evt.skillcheckContainer);
    return EntityNotificationType.SendThisEventToEntity;
  }

  public final func QuestSetSkillchecks(container: ref<BaseSkillCheckContainer>) -> Void {
    this.ErasePassedSkillchecks();
    this.InitializeSkillChecks(container, true);
    this.NotifyParents();
  }
  ...
  protected func ActionVehicleOverrideForceBrakes() -> ref<VehicleOverrideForceBrakes> {
    let action: ref<VehicleOverrideForceBrakes> = new VehicleOverrideForceBrakes();
    action.clearanceLevel = 10;
    action.SetUp(this);
    action.SetProperties(this.m_isControlledByThePlayer);
    action.AddDeviceName(this.m_deviceName);
  ...
  protected func ActionVehicleOverrideExplode() -> ref<VehicleOverrideExplode> {
    let action: ref<VehicleOverrideExplode> = new VehicleOverrideExplode();
    action.clearanceLevel = 10;
    action.SetUp(this);
    action.SetProperties(this.m_isControlledByThePlayer);
    action.AddDeviceName(this.m_deviceName);
  ...
  protected func ActionVehicleOverrideAccelerate() -> ref<VehicleOverrideAccelerate> {
    let action: ref<VehicleOverrideAccelerate> = new VehicleOverrideAccelerate();
    action.clearanceLevel = 10;
    action.SetUp(this);
    action.SetProperties(this.m_isControlledByThePlayer);
    action.AddDeviceName(this.m_deviceName);
  ...
  public func ActionProgramSetDeviceOff() -> ref<ProgramSetDeviceOff> {
    ...
    action.SetProperties();
    action.AddDeviceName(this.m_deviceName);
    multiplier = GameInstance.GetStatsSystem(this.GetGameInstance()).GetStatValue(Cast<StatsObjectID>(GetPlayer(this.GetGameInstance()).GetEntityID()), gamedataStatType.CameraShutdownExtension);
  ...
  public final const func GetWillingInvestigators() -> [EntityID] {
  ...
  public final func SetEMPEffectActiveState(isActive: Bool) -> Void {
    this.m_isUnderEMPEffect = isActive;
  }
}
