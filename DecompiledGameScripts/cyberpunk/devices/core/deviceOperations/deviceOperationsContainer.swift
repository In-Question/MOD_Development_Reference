...
public class DeviceOperationsContainer extends IScriptable {
  ...
  private inline persistent const let m_operations: [ref<DeviceOperationBase>];

  private inline const let m_triggers: [ref<DeviceOperationsTrigger>];
  ...
  private final func ToggleOperations(const operations: script_ref<[SToggleDeviceOperationData]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(operations)) {
      this.ToggleOperationByName(Deref(operations)[i].enable, Deref(operations)[i].operationName);
      i += 1;
    };
  }
