...
public class Device extends DeviceBase {
  ...
  protected let m_controller: ref<ScriptableDeviceComponent>;
  ...
  @default(Device, ScriptableDeviceComponent)
  ...
  @default(FactInvoker, FactInvokerController)
  @default(Fan, FanController)
  @default(Frame, FrameController)
  ...
  @default(HoloTable, HoloTableController)
  ...
  @default(JudysDoor, DoorController)
  ...
  @default(LootContainerAccessPoint, LootContainerAccessPointController)
  ...
  @default(NumericDisplay, NumericDisplayController)
  @default(OdaCementBag, OdaCementBagController)
  @default(PerkTraining, PerkTrainingController)
  @default(PersonnelSystem, PersonnelSystemController)
  @default(PlatformDevice, PlatformController)
  @default(Portal, PortalController)
  ...
  @default(RoboticArms, RoboticArmsController)
  ...
  @default(ServerNode, ServerNodeController)
  @default(SimpleOnTurnOnPlayEffectDevice, SimpleOnTurnOnPlayEffectDeviceController)
  ...
  @default(Wardrobe, WardrobeController)
  @default(WeaponTraining, WeaponTrainingController)
  ...
  private let m_onInputHintManagerInitializedChangedCallback: ref<CallbackHandle>;
  ...
  private let m_cameraZoomActive: Bool;
  ...
  protected let m_advanceInteractionStateResolveDelayID: DelayID;
  ...
  private let m_wasEngineeringSkillcheckTriggered: Bool;

  @runtimeProperty("category", "RPG")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;DeviceContentAssignment")
  protected let m_contentScale: TweakDBID;

  @runtimeProperty("category", "Network Visualisation")
  protected edit let m_networkGridBeamOffset: Vector4;

  @runtimeProperty("category", "Area effects - OBSOLETE USE ONLY TO CORRECT DATA OF EXISTING EFFECTS")
  public let m_areaEffectsData: [SAreaEffectData];

  @runtimeProperty("category", "Area effects - OBSOLETE USE ONLY TO CORRECT DATA OF EXISTING EFFECTS")
  public let m_areaEffectsInFocusMode: [SAreaEffectTargetData];

  protected let m_debugOptions: DebuggerProperties;

  public let m_currentlyUploadingAction: wref<ScriptableDeviceAction>;
  ...
  protected final func DetermineInteractionStateTask(data: ref<ScriptTaskData>) -> Void {
    ...
    let taskData: ref<DetermineInteractionStateTaskData> = data as DetermineInteractionStateTaskData;
  ...
  public func OnMaraudersMapDeviceDebug(sink: ref<MaraudersMapDevicesSink>) -> Void {
    ...
    sink.PushString("Gameplay Role", EnumValueToString("EGameplayRole", Cast<Int64>(EnumInt(this.m_gameplayRoleComponent.GetCurrentGameplayRole()))));
    sink.PushString("State", EnumValueToString("EDeviceStatus", Cast<Int64>(EnumInt(this.GetDevicePS().GetDeviceState()))));
    sink.PushString("Durability Type", EnumValueToString("EDeviceDurabilityType", Cast<Int64>(EnumInt(this.GetDevicePS().GetDurabilityType()))));
    sink.PushString("Durability State", EnumValueToString("EDeviceDurabilityState", Cast<Int64>(EnumInt(this.GetDevicePS().GetDurabilityState()))));
    ...
    while i < ArraySize(playstyle) {
      sink.PushString("Playstyle " + i, EnumValueToString("EPlaystyle", Cast<Int64>(EnumInt(playstyle[i]))));
      i += 1;
    };
    context = this.GetDevicePS().GetActiveContexts();
    i = 0;
    while i < ArraySize(context) {
      sink.PushString("Context " + i, EnumValueToString("gamedeviceRequestType", Cast<Int64>(EnumInt(context[i]))));
      i += 1;
    };
    ...
    if this.GetDevicePS().GetSkillCheckContainer().GetHackingSlot().IsActive() || this.GetDevicePS().GetSkillCheckContainer().GetEngineeringSlot().IsActive() || this.GetDevicePS().GetSkillCheckContainer().GetDemolitionSlot().IsActive() {
      if this.GetDevicePS().GetSkillCheckContainer().GetHackingSlot().IsActive() {
        ...
        sink.PushString("Hacking Skillcheck Diff", EnumValueToString("EGameplayChallengeLevel", Cast<Int64>(EnumInt(this.GetDevicePS().GetSkillCheckContainer().GetHackingSlot().GetDifficulty()))));
      };
      if this.GetDevicePS().GetSkillCheckContainer().GetEngineeringSlot().IsActive() {
        ...
        sink.PushString("Engeneering Skillcheck Diff", EnumValueToString("EGameplayChallengeLevel", Cast<Int64>(EnumInt(this.GetDevicePS().GetSkillCheckContainer().GetEngineeringSlot().GetDifficulty()))));
      };
      if this.GetDevicePS().GetSkillCheckContainer().GetDemolitionSlot().IsActive() {
        ...
        sink.PushString("Demolitions Skillcheck Diff", EnumValueToString("EGameplayChallengeLevel", Cast<Int64>(EnumInt(this.GetDevicePS().GetSkillCheckContainer().GetDemolitionSlot().GetDifficulty()))));
      };
    } else {
    ...
    sink.PushString("Security Access Level", EnumValueToString("ESecurityAccessLevel", Cast<Int64>(EnumInt(this.GetDevicePS().GetSecurityAccessLevel()))));
  ...
  protected cb func OnGameAttached() -> Bool {
    let mod: ref<gameConstantStatModifierData>;
    ...
    if ps.WasQuickHacked() {
      GameInstance.GetStatusEffectSystem(this.GetGame()).ApplyStatusEffect(this.GetEntityID(), t"BaseStatusEffect.WasQuickHacked");
      statsSystem = GameInstance.GetStatsSystem(this.GetGame());
      mod = RPGManager.CreateStatModifier(gamedataStatType.WasQuickHacked, gameStatModifierType.Additive, 1.00) as gameConstantStatModifierData;
      statsSystem.AddModifier(Cast<StatsObjectID>(this.GetEntityID()), mod);
    };
  }
  ...
  protected cb func OnDeath(evt: ref<gameDeathEvent>) -> Bool {
    ...
    this.ClearQuickHacks();
  }

  protected final func Die() -> Void {
  ...
  private final const func IsActionQueueEnabled() -> Bool {
    ...
    let disallowDeviceQhQueue: Bool = TDB.GetBool(t"NewPerks.Intelligence_Left_Milestone_2.disallowDeviceQhQueue");
    return !disallowDeviceQhQueue && QuickHackableQueueHelper.IsActionQueueEnabled(this.m_currentlyUploadingAction, playerPuppet);
  }
  ...
  protected final const func ExecuteAction(const choice: script_ref<InteractionChoice>, executor: wref<GameObject>, layerTag: CName) -> Void {
  ...
  protected final const func ExecuteAction(action: ref<DeviceAction>, opt executor: wref<GameObject>) -> Bool {
    ...
    if sAction != null {
      ...
      sAction.ProcessRPGAction(this.GetGame(), this.m_gameplayRoleComponent);
      return true;
    };
    return false;
  }
  ...
  public const func GetController() -> ref<ScriptableDeviceComponent> {
  ...
  protected cb func OnTogglePersonalLink(evt: ref<TogglePersonalLink>) -> Bool {
    ...
    if Equals(this.GetDevicePS().GetPersonalLinkStatus(), EPersonalLinkConnectionStatus.DISCONNECTING) {
      if !IsDefined(executor) {
        return false;
      };
  ...
  protected func TogglePersonalLink(toggle: Bool, puppet: ref<GameObject>) -> Void {
    ...
    if toggle {
      if puppet.IsPlayer() {
        SaveLocksManager.RequestSaveLockAdd(this.GetGame(), n"PersonalLink");
      ...
      SaveLocksManager.RequestSaveLockRemove(this.GetGame(), n"PersonalLink");
  ...
  protected func InitiatePersonalLinkWorkspot(puppet: ref<GameObject>) -> Void {
    ...
    if !this.m_personalLinkHackSend {
      workspotSystem.SendJumpToTagCommandEnt(puppet, this.GetSlotTag(), true, this.GetEntityID());
  ...
  protected cb func OnQuestForceCameraZoom(evt: ref<QuestForceCameraZoom>) -> Bool {
    this.EvaluateCameraZoomState(GetPlayer(this.GetGame()), evt.IsInstant());
  }

  protected final func EvaluateCameraZoomState(executor: ref<GameObject>, opt instant: Bool) -> Void {
    if executor == null {
      return;
    };
    if !this.GetDevicePS().IsAdvancedInteractionModeOn() {
      this.RegisterPlayerInputListener(false);
      this.SetZoomBlackboardValues(false, 0.50);
      this.ToggleCameraZoom(false, instant);
    } else {
      this.ShowAdvanceInteractionInputHints();
      this.RegisterPlayerInputListener(true);
      this.SetZoomBlackboardValues(true);
      this.ToggleCameraZoom(true, instant);
    };
    this.UpdateDeviceState();
  }
  ...
  private final func ShowAdvanceInteractionInputHints() -> Void {
    this.m_onInputHintManagerInitializedChangedCallback = GameInstance.GetBlackboardSystem(this.GetGame()).Get(GetAllBlackboardDefs().InputSchemes).RegisterListenerVariant(GetAllBlackboardDefs().InputSchemes.InitializedInputHintManagerList, this, n"OnInputHintManagerInitializedChanged", true);
  }

  protected cb func OnInputHintManagerInitializedChanged(value: Variant) -> Bool {
    let evt: ref<UpdateInputHintMultipleEvent>;
    let inputData: InputHintData;
    ...
    while i < ArraySize(currentInitializedInputHintManagerList) {
      if Equals(currentInitializedInputHintManagerList[i], n"GameplayInputHelper") {
        ...
        inputData.source = n"AdvanceInteractionMode";
        inputData.action = n"click";
        inputData.localizedLabel = "LocKey#49383";
        evt.AddInputHint(inputData, true);
        inputData.action = n"right_stick_y";
        inputData.localizedLabel = "LocKey#49382";
        evt.AddInputHint(inputData, true);
        inputData.action = n"UI_Exit";
  ...
  private final func HideAdvanceInteractionInputHints() -> Void {
    let evt: ref<DeleteInputHintBySourceEvent>;
    if IsDefined(this.m_onInputHintManagerInitializedChangedCallback) {
      GameInstance.GetBlackboardSystem(this.GetGame()).Get(GetAllBlackboardDefs().InputSchemes).UnregisterListenerVariant(GetAllBlackboardDefs().InputSchemes.InitializedInputHintManagerList, this.m_onInputHintManagerInitializedChangedCallback);
    };
  ...
  protected cb func OnToggleUIInteractivity(evt: ref<ToggleUIInteractivity>) -> Bool {
    ...
    this.GetDevicePS().SetInteractionState(evt.m_isInteractive);
  }
  ...
  protected final func SetZoomBlackboardValues(newState: Bool, opt lockPlayerFor: Float) -> Void {
    let evt: ref<AdvanceInteractionStateResolveEvent>;
    let invalidID: EntityID;
    let playerObject: ref<GameObject>;
    let playerStateMachineBlackboard: ref<IBlackboard>;
    if this.m_advanceInteractionStateResolveDelayID != GetInvalidDelayID() {
      GameInstance.GetDelaySystem(this.GetGame()).CancelCallback(this.m_advanceInteractionStateResolveDelayID);
      GameInstance.GetDelaySystem(this.GetGame()).CancelDelay(this.m_advanceInteractionStateResolveDelayID);
      this.m_advanceInteractionStateResolveDelayID = GetInvalidDelayID();
    };
    if lockPlayerFor <= 0.00 {
      playerObject = GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerControlledGameObject();
      playerStateMachineBlackboard = GameInstance.GetBlackboardSystem(this.GetGame()).GetLocalInstanced(playerObject.GetEntityID(), GetAllBlackboardDefs().PlayerStateMachine);
      ...
      playerStateMachineBlackboard.SetEntityID(GetAllBlackboardDefs().PlayerStateMachine.UIZoomDeviceID, newState ? this.GetEntityID() : invalidID);
      playerStateMachineBlackboard.FireCallbacks();
    } else {
      evt = new AdvanceInteractionStateResolveEvent();
      evt.m_toggle = newState;
      this.m_advanceInteractionStateResolveDelayID = GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evt, lockPlayerFor);
    };
  }

  protected cb func OnAdvanceInteractionStateResolveEvent(evt: ref<AdvanceInteractionStateResolveEvent>) -> Bool {
    this.m_advanceInteractionStateResolveDelayID = GetInvalidDelayID();
    this.SetZoomBlackboardValues(evt.m_toggle, 0.00);
  }
  ...
  protected func LeaveWorkspot(activator: ref<GameObject>) -> Void {
    ...
    if activator.IsPlayer() {
      ...
      workspotSystem.SendJumpToTagCommandEnt(activator, this.GetSlotTag() + n".end", true, this.GetEntityID());
    } else {
  ...
  public const func ShouldShowScanner() -> Bool {
    ...
    if NotEquals(this.GetCurrentGameplayRole(), EGameplayRole.None) {
      return true;
    };
  ...
  public const func CompileScannerChunks() -> Bool {
    ...
    if devicePS.ShouldScannerShowHealth() {
      ...
      healthChunk.Set(Cast<Int32>(this.GetCurrentHealth()), Cast<Int32>(this.GetTotalHealth()));
  ...
  protected const func GetScannerName() -> String {
    let displayName: String = this.GetDisplayName();
  ...
  public final const func GetActionsDebug(const context: script_ref<GetActionsContext>, debugger: ref<DeviceDebuggerComponent>, out outActions: [ref<DeviceAction>]) -> Bool {
    if !IsDefined(debugger) {
      return false;
    };
    this.GetDevicePS().GetActions(outActions, Deref(context));
    return true;
  }
  ...
  protected func SendQuickhackCommands(shouldOpen: Bool) -> Void {
    ...
    if shouldOpen {
      context = this.GetDevicePS().GenerateContext(gamedeviceRequestType.Remote, Device.GetInteractionClearance(), GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerControlledGameObject(), this.GetEntityID());
      this.GetDevicePS().GetRemoteActions(actions, context);
      if this.m_isQhackUploadInProgerss && !this.IsActionQueueEnabled() || this.IsActionQueueFull() {
        ScriptableDeviceComponentPS.SetActionsInactiveAll(actions, "LocKey#7020");
      };
      QuickHackableHelper.TranslateActionsIntoQuickSlotCommands(actions, commands, this, this.GetDevicePS());
  ...
  private final const func GetPlayerCyberDeck() -> [String] {
  ...
  private final func GetMatchingActionProgramName(const actions: script_ref<[ref<DeviceAction>]>, const searchWord: script_ref<String>) -> Int32 {
    ...
    while i < ArraySize(Deref(actions)) {
      ...
      actionRecord = (Deref(actions)[i] as BaseScriptableAction).GetObjectActionRecord();
      actionRecord.InstigatorPrereqs(instigatorPrereqs);
      i1 = 0;
      while i1 < ArraySize(instigatorPrereqs) {
        statName = TweakDBInterface.GetCName(instigatorPrereqs[i1].GetID() + t".statType", n"None");
    ...
    if Equals(evt.progressBarContext, EProgressBarContext.QuickHack) && Equals(evt.progressBarType, EProgressBarType.UPLOAD) {
      switch evt.state {
        ...
        case EUploadProgramState.COMPLETED:
          ...
          if IsDefined(quickSlotCommandUsed) && !this.IsDead() {
  ...
  private final func ShouldPulseNetwork() -> Bool {
    return !this.GetNetworkSystem().ShouldShowLinksOnMaster() && !this.IsBackdoor() && this.IsConnectedToBackdoorDevice() && !this.IsNetworkKnownToPlayer() && NotEquals(this.GetCurrentGameplayRole(), EGameplayRole.None);
  }

  public const func CanOverrideNetworkContext() -> Bool {
    return NotEquals(this.GetCurrentGameplayRole(), EGameplayRole.None) && this.GetDevicePS().HasAnyDeviceConnection() && (this.ShouldRevealDevicesGrid() || this.IsConnectedToBackdoorDevice());
  }
  ...
  public const func GetDefaultHighlight() -> ref<FocusForcedHighlightData> {
    ...
    if Equals(this.GetCurrentGameplayRole(), EGameplayRole.None) || Equals(this.GetCurrentGameplayRole(), EGameplayRole.Clue) || this.IsAnyClueEnabled() {
      return null;
    };
    if this.m_scanningComponent.IsBraindanceBlocked() || this.m_scanningComponent.IsPhotoModeBlocked() {
      return null;
  ...
  public const func GetCurrentHealth() -> Float {
    let value: Float = GameInstance.GetStatPoolsSystem(this.GetGame()).GetStatPoolValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health, true);
    return value;
  }

  public const func GetTotalHealth() -> Float {
    let value: Float = GameInstance.GetStatPoolsSystem(this.GetGame()).GetStatPoolMaxPointValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health);
    return value;
  }
  ...
  protected final func TriggerAreaEffectDistractionByAction(action: ref<ScriptableDeviceAction>) -> Void {
    ...
    if IsDefined(this.GetFxResourceMapper()) {
      ...
      if quickHackIndex >= 0 {
        ...
        effectData.CopyData(this.GetFxResourceMapper().GetAreaEffectDataByIndex(quickHackIndex));
        this.TriggerArreaEffectDistraction(effectData);
      };
    };
  }
  ...
  public final const func GetAreaEffectLifetimeByAction(action: ref<ScriptableDeviceAction>) -> Float {
    ...
    if IsDefined(this.GetFxResourceMapper()) && this.GetFxResourceMapper().TryGetAreaEffectByAction(action, effectData) {
  ...
  public final const func GetAreaEffectStimRangeByAction(action: ref<ScriptableDeviceAction>) -> Float {
    let effectData: ref<AreaEffectData>;
    return IsDefined(this.GetFxResourceMapper()) && this.GetFxResourceMapper().TryGetAreaEffectByAction(action, effectData) ? effectData.stimRange : 0.00;
  }
  ...
  public final func GetNodePosition(opt nodeRef: NodeRef) -> [Vector4] {
    let i: Int32;
    ...
    let globalRef: GlobalNodeRef = ResolveNodeRefWithEntityID(nodeRef, this.GetEntityID());
    if this.HasInvestigationPositionsArrayCached(globalRef) {
      return this.GetCachedInvestigationPositionsArray(globalRef);
    };
    if GlobalNodeRef.IsDefined(globalRef) {
      ...
      if !Vector4.IsZero(position) {
        pointResults.point = GameInstance.GetNavigationSystem(this.GetGame()).GetNearestNavmeshPointBelowOnlyHumanNavmesh(position, 1.00, 5);
    ...
    if ArraySize(positionsArray) > 0 {
      this.SetInvestigationPositionsArray(globalRef, positionsArray);
    };
    return positionsArray;
  }

  protected const func GetCachedInvestigationPositionsArray(globalNodeRef: GlobalNodeRef) -> [Vector4] {
    let arr: array<Vector4>;
    return arr;
  }

  protected func SetInvestigationPositionsArray(globalNodeRef: GlobalNodeRef, const arr: script_ref<[Vector4]>) -> Void;

  protected const func HasInvestigationPositionsArrayCached(globalNodeRef: GlobalNodeRef) -> Bool {
    return false;
  }
  ...
  private final func ShouldBeHighlightedLongerOnPing() -> Bool {
    return this.IsQuickHackAble() && this.CanPlayerScanThroughWalls();
  }
  ...
  private final func ResolveComponents(const componentsData: script_ref<[SComponentOperationData]>) -> Void {
    ...
    while i < ArraySize(Deref(componentsData)) {
      toggle = Equals(Deref(componentsData)[i].operationType, EComponentOperation.Enable);
      this.ToggleComponentByName(Deref(componentsData)[i].componentName, toggle);
      i += 1;
    };
  }
  ...
  protected final func AddHudButtonHelper(const argText: script_ref<String>, argIcon: CName) -> Void {
    GameInstance.GetBlackboardSystem(this.GetGame()).Get(GetAllBlackboardDefs().UI_HudButtonHelp).SetString(GetAllBlackboardDefs().UI_HudButtonHelp.button1_Text, Deref(argText));
  ...
  protected final func RemoveHudButtonHelper() -> Void {
    ...
    GameInstance.GetBlackboardSystem(this.GetGame()).Get(GetAllBlackboardDefs().UI_HudButtonHelp).SetName(GetAllBlackboardDefs().UI_HudButtonHelp.button1_Icon, n"None");
  }
  ...
  public const func GetContentScale() -> TweakDBID {
    ...
    if TDBID.IsValid(this.GetDevicePS().GetContentAssignmentID()) {
  ...
  public const func DeterminGameplayRoleMappinRange(const data: script_ref<SDeviceMappinData>) -> Float {
    ...
    if NotEquals(Deref(data).gameplayRole, EGameplayRole.None) {
      switch Deref(data).gameplayRole {
  ...
  protected final func ClearQuickHacks() -> Void {
    let quickHackClear: ref<DeactivateQuickHackIndicatorEvent> = new DeactivateQuickHackIndicatorEvent();
    this.QueueEvent(quickHackClear);
  ...
  protected cb func OnQuickHackToggleOn(evt: ref<QuickHackToggleON>) -> Bool {
    if !evt.IsCompleted() && evt.Repeat && evt.GetDurationTime() > 0.00 {
      evt.Repeat = false;
      this.ShowQuickHackDuration(evt);
    };
  }
  ...
  protected cb func OnReturnToDeviceScreenEvent(evt: ref<ReturnToDeviceScreenEvent>) -> Bool {
    this.ExecuteAction(this.GetDevicePS().ActionQuestForceCameraZoom(true, true), GetPlayer(this.GetGame()));
  }
  ...
  protected final func TakeControlOverCamera(isOn: Bool) -> Void {
    let cam: ref<FPPCameraComponent>;
    ...
    if isOn {
      ...
      cam = this.m_cameraComponent as FPPCameraComponent;
      if IsDefined(cam) {
        cam.ResetPitch();
      };
    };
  ...
  protected cb func OnPlayerStateMachineZoom(value: Float) -> Bool {
    this.HandlePlayerStateMachineZoom(value);
  }
  ...
  protected const func ShouldRegisterToHUD() -> Bool {
    ...
    if this.HasAnyClue() {
      ...
      if Equals(this.GetCurrentGameplayRole(), EGameplayRole.None) || Equals(this.GetCurrentGameplayRole(), EGameplayRole.UnAssigned) {
        returnValue = false;
      } else {
  ...
  protected final func KillNPCWorkspotUser(killDelay: Float) -> Void {
    ...
    GameInstance.GetWorkspotSystem(this.GetGame()).SendJumpToTagCommandEnt(npc, n"kill", !this.GetDevicePS().ShouldNPCWorkspotFinishLoop(), this.GetEntityID());
  ...
  protected func ShowMappinOnProximity() -> Void {
    ...
    let statValue: Float = GameInstance.GetStatsSystem(this.GetGame()).GetStatValue(Cast<StatsObjectID>(GetPlayer(this.GetGame()).GetEntityID()), gamedataStatType.HasCyberdeck);
    if statValue > 0.00 {
  ...
  protected func EvaluateProximityRevealInteractionLayerState() -> Void {
    if this.IsActiveBackdoor() {
  ...
  protected func StartRevealingOnProximity() -> Void {
    let statValue: Float = GameInstance.GetStatsSystem(this.GetGame()).GetStatValue(Cast<StatsObjectID>(GetPlayer(this.GetGame()).GetEntityID()), gamedataStatType.HighlightAccessPoint);
    if statValue > 0.00 {
  ...
  protected final func CreateFxInstance(resource: FxResource, transform: WorldTransform) -> ref<FxInstance> {
  ...
  protected cb func OnHandleEngineeringSkillcheckSFX(evt: ref<ActionEngineering>) -> Bool {
    if !this.m_wasEngineeringSkillcheckTriggered {
      GameObject.PlaySoundEvent(this, this.GetEngineeringSkillcheckSFXName());
      this.m_wasEngineeringSkillcheckTriggered = true;
    };
  }

  private final func GetEngineeringSkillcheckSFXName() -> CName {
    return n"ui_jingle_technical_ability_skillcheck";
  }
}
