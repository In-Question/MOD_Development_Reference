...
public class IsDeviceTargetValidFilter extends EffectObjectSingleFilter_Scripted {
  ...
  public final func Process(ctx: EffectScriptContext, filterCtx: EffectSingleFilterScriptContext) -> Bool {
    ...
    let reactionPresetRecord: wref<ReactionPreset_Record>;
    ...
    data = FromVariant<ref<PuppetForceVisionAppearanceData>>(dataVariant);
    if data != null {
...
public class OnlyNearest_AINavPath_Device extends EffectObjectGroupFilter_Scripted {
  ...
  private final func SortTargetsByDistance(source: ref<GameObject>, const targets: script_ref<[ref<ScriptedPuppet>]>) -> [HandleWithValue] {
    ...
    while i < ArraySize(Deref(targets)) {
      targetPos = Deref(targets)[i].GetWorldPosition();
      closestPath = null;
      j = 0;
      ...
      if !IsDefined(closestPath) {
        ...
        singleSortedTarget.handle = Deref(targets)[i];
...
public class IsSourceDeviceActveFilter extends EffectObjectGroupFilter_Scripted {
  ...
  public final func Process(ctx: EffectScriptContext, out filterCtx: EffectGroupFilterScriptContext) -> Bool {
    ...
    if !IsDefined(device) || Equals(device.GetCurrentGameplayRole(), EGameplayRole.None) || !device.IsActive() {
      ArrayClear(filterCtx.resultIndices);
    };
    return true;
  }
...
public class IsNotInstigatorProjectileFilter extends EffectObjectSingleFilter_Scripted {

  public final func Process(ctx: EffectScriptContext, filterCtx: EffectSingleFilterScriptContext) -> Bool {
    let projectileInstigator: wref<GameObject>;
    let instigator: ref<Entity> = EffectScriptContext.GetInstigator(ctx);
    let entity: ref<Entity> = EffectSingleFilterScriptContext.GetEntity(filterCtx);
    let projectileObject: ref<BaseProjectile> = entity as BaseProjectile;
    if IsDefined(projectileObject) {
      projectileInstigator = projectileObject.GetInstigator();
      if IsDefined(projectileInstigator) && projectileInstigator == (instigator as GameObject) {
        return false;
      };
    };
    return true;
...
public class OnlySingleStatusEffectFromInstigator extends EffectObjectSingleFilter_Scripted {
  ...
  public final func Process(ctx: EffectScriptContext, filterCtx: EffectSingleFilterScriptContext) -> Bool {
    ...
    attack = FromVariant<ref<IAttack>>(variant);
...
public class IgnoreAlreadyAffectedEntities extends EffectObjectSingleFilter_Scripted {
  ...
  public final func Process(ctx: EffectScriptContext, filterCtx: EffectSingleFilterScriptContext) -> Bool {
    ...
    affectedEntities = FromVariant<array<EntityID>>(tempVariant);
