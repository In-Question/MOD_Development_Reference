...
public class MasterController extends ScriptableDeviceComponent {

  public const func GetPS() -> ref<MasterControllerPS> {
    return this.GetBasePS() as MasterControllerPS;
  }
}
...
public class MasterControllerPS extends ScriptableDeviceComponentPS {
  ...
  public final func NetrunnerGiveConnectedDevices() -> [ref<DeviceComponentPS>] {
  ...
  public final const func GetAllDescendants(out outDevices: [ref<DeviceComponentPS>]) -> Void {
    GameInstance.GetDeviceSystem(this.GetGameInstance()).GetAllDescendants(PersistentID.ExtractEntityID(this.GetID()), outDevices);
  }

  public final const func GetPuppets() -> [ref<PuppetDeviceLinkPS>] {
  ...
  public const func GetImmediateSlaves() -> [ref<DeviceComponentPS>] {
    let slaves: array<ref<DeviceComponentPS>>;
    GameInstance.GetDeviceSystem(this.GetGameInstance()).GetChildren(this.GetMyEntityID(), slaves);
    return slaves;
  }

  public const func GetLazySlaves() -> [ref<LazyDevice>] {
  ...
  public final const func GetImmediateDescendants() -> [ref<DeviceComponentPS>] {
  ...
  public final const func GetLazyDescendants() -> [ref<LazyDevice>] {
  ...
  protected final const func SendActionsToAllSlaves(const actions: script_ref<[ref<ScriptableDeviceAction>]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(actions)) {
      this.SendActionToAllSlaves(Deref(actions)[i]);
      i += 1;
    };
  }
  ...
  protected final const func GetQuickHacksFromSlave(out outActions: [ref<DeviceAction>], const context: script_ref<GetActionsContext>) -> Void {
    let slaves: array<ref<DeviceComponentPS>> = this.GetImmediateSlaves();
    let i: Int32 = 0;
    while i < ArraySize(slaves) {
  ...
  public func RequestThumbnailWidgetsUpdate(blackboard: ref<IBlackboard>) -> Void {
    ...
    if IsDefined(blackboard) {
      blackboard.SetVariant(this.GetBlackboardDef().ThumbnailWidgetsData, ToVariant(widgetsData));
      blackboard.SignalVariant(this.GetBlackboardDef().ThumbnailWidgetsData);
      blackboard.FireCallbacks();
    };
  }

  public func RequestDeviceWidgetsUpdate(blackboard: ref<IBlackboard>, const devices: script_ref<[PersistentID]>) -> Void {
    let widgetData: SDeviceWidgetPackage;
    let widgetsData: array<SDeviceWidgetPackage>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(devices)) {
      widgetData = this.GetSlaveDeviceWidget(Deref(devices)[i]);
