...
public class ExplosiveTriggerDeviceController extends ExplosiveDeviceController {
  ...
  public const func GetPS() -> ref<ExplosiveTriggerDeviceControllerPS> {
    return this.GetBasePS() as ExplosiveTriggerDeviceControllerPS;
  }
}

public class ExplosiveTriggerDeviceControllerPS extends ExplosiveDeviceControllerPS {

  @runtimeProperty("category", "Trigger conditions")
  ...
  public func GetQuestActionByName(actionName: CName) -> ref<DeviceAction> {
    ...
    if action == null {
      switch actionName {
        case n"ForceSafeForPlayer":
          action = this.ActionQuestSetPlayerSafePass(true);
          break;
        case n"DisableSafeForPlayer":
          action = this.ActionQuestSetPlayerSafePass(false);
      };
    };
    return action;
  }
  ...
  protected func GetSpiderbotActions(actions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void {
    ArrayPush(Deref(actions), this.ActionSpiderbotExplodeExplosiveDevice());
    if this.IsON() {
      ArrayPush(Deref(actions), this.ActionSpiderbotDisarmExplosiveDevice());
    };
  }
  ...
  protected final func OnQuestSetPlayerSafePass(evt: ref<QuestSetPlayerSafePass>) -> EntityNotificationType {
    this.m_playerSafePass = FromVariant<Bool>(evt.prop.first);
    return EntityNotificationType.SendThisEventToEntity;
  }
