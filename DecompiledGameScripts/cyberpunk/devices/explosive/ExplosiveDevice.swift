...
public class ExplosiveDevice extends BasicDistractionDevice {
  ...
  protected let m_spawnedFxInstancesToKill: [ref<FxInstance>];
  ...
  private let m_explodedByQuickhack: Bool;

  @runtimeProperty("category", "Appearance Change Options")
  @runtimeProperty("tooltip", "When true, during explosion it will change the appearance to Exploaded Appearance Name provided below. It should be used when an entity supports proxy mesh.")
  private let m_switchAppearanceOnExplosion: Bool;

  @runtimeProperty("category", "Appearance Change Options")
  private let m_explodedAppearanceName: CName;
  ...
  protected cb func OnPostInitialize(evt: ref<entPostInitializeEvent>) -> Bool {
    ...
    dataArray = this.GetDevicePS().GetExplosionDefinitionArray();
    i = 0;
  ...
  protected func ResolveGameplayState() -> Void {
    ...
    if this.GetDevicePS().DoExplosiveResolveGameplayLogic() {
      if !this.GetDevicePS().IsExploded() {
        ...
        this.m_devicePenetrationHealth = GameInstance.GetStatsSystem(this.GetDevicePS().GetGameInstance()).GetStatValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatType.PenetrationHealth);
  ...
  private const func GetController() -> ref<ExplosiveDeviceController> {
    return this.m_controller as ExplosiveDeviceController;
  }

  public const func GetDevicePS() -> ref<ExplosiveDeviceControllerPS> {
  ...
  protected func HandleDeath(instigator: wref<GameObject>) -> Void {
    ...
    if this.m_killedByExplosion {
      this.StartExplosionPipeline(instigator, 0.05);
    } else {
      this.StartExplosionPipeline(instigator);
    };
  ...
  protected final func StartExplosionPipeline(instigator: wref<GameObject>, opt additionalDelays: Float) -> Void {
    ...
    if largestDelayTime > 0.00 {
      ...
      this.SendSwapMeshDelayedEvent(this.GetDevicePS().GetTimeToMeshSwap());
    };
    this.QueueEvent(lightEvent);
  }
  ...
  private final func Explode(index: Int32, instigator: wref<GameObject>) -> Void {
    ...
    let dataArray: array<ExplosiveDeviceResourceDefinition> = this.GetDevicePS().GetExplosionDefinitionArray();
    ...
    if NotEquals(this.m_loopAudioEvent, n"None") {
    ...
    this.GetDevicePS().SetExplodedState(true);
  ...
  protected func ToggleVisibility(visible: Bool) -> Void {
    ...
    if this.m_switchAppearanceOnExplosion && !visible {
      this.ScheduleAppearanceChange(this.m_explodedAppearanceName);
    };
    i = 0;
    while i < this.m_numberOfComponentsToON {
  ...
  private final func SpawnVFXs(const fxEventName: script_ref<[CName]>) -> Bool {
    let i: Int32;
    if ArraySize(Deref(fxEventName)) == 0 {
      return false;
    };
    i = 0;
    while i < ArraySize(Deref(fxEventName)) {
      GameObjectEffectHelper.StartEffectEvent(this, Deref(fxEventName)[i]);
      i += 1;
    };
    return true;
  }
  ...
  protected cb func OnQuestForceDetonate(evt: ref<QuestForceDetonate>) -> Bool {
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestSettingStatPoolValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health, 0.00, this, false);
  ...
  protected cb func OnQuickHackExplodeExplosive(evt: ref<QuickHackExplodeExplosive>) -> Bool {
    this.m_explodedByQuickhack = true;
  ...
  protected cb func OnHit(evt: ref<gameHitEvent>) -> Bool {
    ...
    let endRadius: Float;
    let oilGrowthRatePerSecond: Float;
    if this.ValidateRelicCWAttack(evt.attackData) {
      return true;
    };
    ...
    if this.m_distractionCollider == evt.hitComponent {
      dataArray = this.GetDevicePS().GetExplosionDefinitionArray();
      if ArraySize(dataArray) > 0 {
        devicePosition = this.GetWorldPosition();
        oilGrowthRatePerSecond = 0.04;
        curTime = GameInstance.GetTimeSystem(this.GetGame()).GetGameTimeStamp();
        endRadius = 1.80;
        curRadius = oilGrowthRatePerSecond * (curTime - this.m_distractionTimeStart);
        if curRadius < endRadius && Vector4.Length2D(evt.hitPosition - devicePosition) < curRadius || curRadius >= endRadius {
          if this.GetDevicePS().GetHealthDecay() > 0.00 {
            this.InitializeHealthDecay(this.GetDevicePS().GetHealthDecay(), 0.00);
            if this.GetDevicePS().GetDistractionHitVFXIgnoreHitPosition() {
  ...
  protected cb func OnDamageReceived(evt: ref<gameDamageReceivedEvent>) -> Bool {
    ...
    if ArraySize(dataArray) > 0 {
      ...
      deviceID = this.GetEntityID();
      if AttackData.IsExplosion(evt.hitEvent.attackData.GetAttackType()) {
        statPoolsSystem.RequestSettingStatPoolValue(Cast<StatsObjectID>(deviceID), gamedataStatPoolType.Health, 0.00, evt.hitEvent.attackData.GetInstigator());
      ...
      if IsDefined(weaponObject) {
        attackPenetration = GameInstance.GetStatsSystem(weaponObject.GetGame()).GetStatValue(Cast<StatsObjectID>(weaponObject.GetEntityID()), gamedataStatType.AttackPenetration) + 1.00;
      };
  ...
  private final func InitializeHealthDecay(health: Float, delay: Float) -> Void {
    ...
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestSettingModifier(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health, gameStatPoolModificationTypes.Decay, modifier);
  }
  ...
  private final func BroadcastEnvironmentalHazardStimuli() -> Void {
    ...
    if !this.m_isBroadcastingEnvironmentalHazardStim {
      if IsDefined(broadcaster) && broadcaster.ResolveStimProcessingCooldown(this.GetEntityID(), gamedataStimType.EnvironmentalHazard, 1.00) {
  ...
  private final func ValidateRelicCWAttack(attackData: ref<AttackData>) -> Bool {
    let isQuickMelee: Bool = Equals(attackData.GetAttackType(), gamedataAttackType.QuickMelee);
    let isSpyTreeMeleeware: Bool = Equals(attackData.GetAttackSubtype(), gamedataAttackSubtype.SpyTreeMeleewareAttack);
    let weaponType: gamedataItemType = attackData.GetWeapon().GetWeaponRecord().ItemType().Type();
    return isQuickMelee || isSpyTreeMeleeware && NotEquals(weaponType, gamedataItemType.Cyb_NanoWires);
  }
  ...
  public const func CanOverrideNetworkContext() -> Bool {
    if this.GetDevicePS().IsExplosiveWithQhacks() {
      return super.CanOverrideNetworkContext();
    };
    return NotEquals(this.GetCurrentGameplayRole(), EGameplayRole.None);
  }
