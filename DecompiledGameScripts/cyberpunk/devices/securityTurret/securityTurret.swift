...
public class SecurityTurret extends SensorDevice {
  ...
  private let m_weaponTDBID: TweakDBID;
  ...
  private const func GetController() -> ref<SecurityTurretController> {
    return this.m_controller as SecurityTurretController;
  }

  public const func GetDevicePS() -> ref<SecurityTurretControllerPS> {
  ...
  protected func TurnOnDevice() -> Void {
    ...
    super.TurnOnDevice();
    this.RunGameEffect(this.m_laserGameEffect, this.GetDevicePS().GetLaserGameEffectRef(), this.m_laserFXSlotName, 10.00);
  ...
  protected cb func OnTCSTakeOverControlActivate(evt: ref<TCSTakeOverControlActivate>) -> Bool {
    ...
    GameInstance.GetAudioSystem(this.GetGame()).AddTriggerEffect(n"te_wea_trigger_trigger", n"TCS_turretTrigger");
  ...
  protected cb func OnTCSTakeOverControlDeactivate(evt: ref<TCSTakeOverControlDeactivate>) -> Bool {
    ...
    this.ShootStop();
    GameInstance.GetAudioSystem(this.GetGame()).RemoveTriggerEffect(n"TCS_turretTrigger");
  ...
  protected final func GiveWeaponToTurret() -> Void {
    ...
    if !IsDefined(this.m_weapon) {
      ...
      if Equals(this.GetDevicePS().GetWeaponItemRecordString(), "") {
        this.itemID = ItemID.FromTDBID(t"Items.w_special_flak");
      } else {
        this.itemID = ItemID.FromTDBID(TDBID.Create("Items." + this.GetDevicePS().GetWeaponItemRecordString()));
      };
  ...
  protected cb func OnTurnONTurretAfterDuration(evt: ref<TurnONTurretAfterDuration>) -> Bool {
    this.TurnOnDevice();
  }
  ...
  protected cb func OnTargetLocked(evt: ref<TargetLockedEvent>) -> Bool {
    if IsHost() {
      ...
      if IsDefined(this.GetCurrentlyFollowedTarget()) && NotEquals(GameObject.GetAttitudeBetween(this, this.GetCurrentlyFollowedTarget()), EAIAttitude.AIA_Friendly) {
        this.GetAttitudeAgent().SetAttitudeTowards(this.GetCurrentlyFollowedTarget().GetAttitudeAgent(), EAIAttitude.AIA_Hostile);
      };
  ...
  public func ControlledDeviceInputAction(isPressed: Bool) -> Void {
    if isPressed {
      this.ShootStart();
      GameInstance.GetAudioSystem(this.GetGame()).AddTriggerEffect(n"te_wea_turret_shoot", n"TCS_Shoot");
      GameInstance.GetAudioSystem(this.GetGame()).Play(n"motion_loop_heavy_2d");
  ...
  private final func ShootStart() -> Void {
    let ps: ref<SecurityTurretControllerPS>;
    if this.m_isBurstDelayOngoing {
      return;
    };
    ps = this.GetDevicePS();
    if !ps.IsON() || NotEquals(this.GetSensorDeviceState(), ESensorDeviceStates.TARGETLOCK) && !ps.IsInFollowMode() && !ps.IsControlledByPlayer() {
      return;
    };
  ...
  private final func MultiplyBaseAIRecoil() -> Float {
    return GameInstance.GetStatsSystem(this.GetGame()).GetStatValue(Cast<StatsObjectID>(this.GetWeapon().GetEntityID()), gamedataStatType.SpreadMaxAI) * 0.10;
  }

  private final func SimplifiedTrackingSetUp(originalPosition: Vector4) -> Vector4 {
    let output: Vector4 = new Vector4(0.00, 0.00, 0.00, 0.00);
    let x: Float = RandRangeF(-1.00, 1.00);
    let y: Float = RandRangeF(-1.00, 1.00);
    let z: Float = RandRangeF(-1.00, 1.00);
    let spread: Vector4 = Vector4.Normalize(new Vector4(x, y, z, 0.00)) * 0.20;
    let verticalOffSet: Vector4 = new Vector4(0.00, 0.00, 1.00, 0.00);
  ...
  private final func ShootAttachedWeapon(opt shootStart: Bool) -> Void {
    ...
    weaponTDBID = ItemID.GetTDBID(this.GetWeapon().GetItemID());
    if weaponTDBID != this.m_weaponTDBID {
      this.m_weaponRecord = TweakDBInterface.GetItemRecord(weaponTDBID) as WeaponItem_Record;
      this.m_weaponTDBID = weaponTDBID;
      this.SelectShootingPattern(this.GetWeapon(), this);
    };
    if this.GetDevicePS().IgnoreTargetTrackerComponent() {
      AIWeapon.Fire(this, this.GetWeapon(), simTime, 1.00, this.m_weaponRecord.PrimaryTriggerMode().Type(), this.SimplifiedTrackingSetUp(this.GetCurrentlyFollowedTarget().GetWorldPosition()), 1.50);
    } else {
      if this.GetDevicePS().IsPartOfPrevention() {
        AIWeapon.Fire(this, this.GetWeapon(), simTime, 1.00, this.m_weaponRecord.PrimaryTriggerMode().Type(), this.GetCurrentlyFollowedTarget().GetWorldPosition(), this.GetCurrentlyFollowedTarget(), 1.50, this.m_currentLookAtEventHor.targetPositionProvider);
      } else {
        if this.GetDevicePS().IsControlledByPlayer() {
          AIWeapon.Fire(GetPlayer(this.GetGame()), this.GetWeapon(), simTime, 1.00, this.m_weaponRecord.PrimaryTriggerMode().Type());
        } else {
          shouldTrackTarget = this.GetCurrentlyFollowedTarget().GetEntityID() == this.GetDevicePS().GetForcedTargetID() && this.GetDevicePS().IsInFollowMode();
          ...
          AIWeapon.Fire(this, this.GetWeapon(), simTime, 1.00, this.m_weaponRecord.PrimaryTriggerMode().Type(), shootObject.GetWorldPosition(), shootObject, shouldTrackTarget, this.m_currentLookAtEventHor.targetPositionProvider);
        };
      };
    };
    AnimationControllerComponent.PushEvent(this, n"Shoot");
    AIWeapon.QueueNextShot(this.GetWeapon(), this.m_weaponRecord.PrimaryTriggerMode().Type(), simTime);
  ...
  protected cb func OnRipOff(evt: ref<RipOff>) -> Bool {
    ...
    StatusEffectHelper.ApplyStatusEffect(GetPlayer(this.GetGame()), t"BaseStatusEffect.PlayerRippingOffTurretWorkspotInv");
  ...
  protected cb func OnAutoKillDelayEvent(evt: ref<AutoKillDelayEvent>) -> Bool {
    GameInstance.GetStatPoolsSystem(this.GetGame()).RequestSettingStatPoolMinValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health, null);
  }
  ...
  protected cb func OnWorkspotFinished(componentName: CName) -> Bool {
    ...
    if Equals(componentName, n"playerDetachWorkspotBack") || Equals(componentName, n"playerDetachWorkspot") {
      StatusEffectHelper.RemoveStatusEffect(GetPlayer(this.GetGame()), t"BaseStatusEffect.PlayerRippingOffTurretWorkspotInv");
    };
    GameInstance.GetTransactionSystem(this.GetGame()).RemoveItemFromSlot(this, t"AttachmentSlots.WeaponRight");
  }
  ...
  protected cb func OnDeath(evt: ref<gameDeathEvent>) -> Bool {
    ...
    let quickHackUploadClear: ref<UploadProgramProgressEvent>;
    ...
    Equals(quickHackUploadClear.progressBarContext, EProgressBarContext.QuickHack);
    ...
    this.QueueEvent(quickHackUploadClear);
  }
  ...
  protected func DamagePipelineFinalized(evt: ref<gameHitEvent>) -> Void {
    ...
    let source: ref<BaseGrenade>;
    let turnONEvt: ref<TurnONTurretAfterDuration>;
    super.DamagePipelineFinalized(evt);
    source = evt.attackData.GetSource() as BaseGrenade;
    if IsDefined(source) {
      if source.IsEMP() && this.GetDevicePS().IsON() {
        this.CutPower();
        duration = source.GetGrenadeAttackStatusEffectDuration();
        turnONEvt = new TurnONTurretAfterDuration();
        GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, turnONEvt, duration);
      };
    };
  ...
  protected func ApplyReplicatedState(const state: ref<DeviceReplicatedState>) -> Void {
    ...
    if this.m_netClientCurrentlyAppliedState.m_health != turretState.m_health {
      GameInstance.GetStatPoolsSystem(this.GetGame()).RequestSettingStatPoolValue(Cast<StatsObjectID>(this.GetEntityID()), gamedataStatPoolType.Health, turretState.m_health, null, true);
