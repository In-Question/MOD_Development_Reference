...
public class VendingTerminal extends InteractiveDevice {
  ...
  protected const let m_vendingBlacklist: [EVendorMode];
  ...
  protected const func GetController() -> ref<VendingTerminalController> {
    return this.m_controller as VendingTerminalController;
  }

  public const func GetDevicePS() -> ref<VendingTerminalControllerPS> {
  ...
  protected cb func OnAreaExit(evt: ref<AreaExitedEvent>) -> Bool {
    this.GetDevicePS().GetVendorDataManager().ClearCart();
    this.RefreshUI();
  }
  ...
  protected cb func OnDispenceItemFromVendor(evt: ref<DispenceItemFromVendor>) -> Bool {
    this.RefreshUI();
    this.DelayVendingMachineEvent(this.GetDevicePS().GetVendorDataManager().GetTimeToCompletePurchase(), evt.GetItemID());
    GameObject.PlaySoundEvent(this, n"dev_vending_machine_processing");
  }
  ...
  protected final func SendDataToUIBlackboard(const TopText: script_ref<String>, const BottomText: script_ref<String>) -> Void {
