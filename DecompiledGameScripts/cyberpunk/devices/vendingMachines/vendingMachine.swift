...
public class VendingMachine extends InteractiveDevice {
  ...
  protected final func DeattachVendor() -> Void {
    GameInstance.GetDelaySystem(this.GetGame()).QueueTask(this, null, n"DeattachVendorTask", gameScriptTaskExecutionStage.Any);
  }

  protected final func DeattachVendorTask(data: ref<ScriptTaskData>) -> Void {
  ...
  protected cb func OnDispenceItemFromVendor(evt: ref<DispenceItemFromVendor>) -> Bool {
    ...
    if evt.IsStarted() {
      ...
      if playerMoney > price {
        this.DelayVendingMachineEvent(this.GetDevicePS().GetTimeToCompletePurchase(), false, true, evt.GetItemID());
  ...
  protected final func DispenseStack(request: ref<DispenseStackRequest>) -> Void {
    MarketSystem.GetInstance(this.GetGame()).QueueRequest(request);
  }
  ...
  protected func CreateQuestDispenseStackRequest(item: ItemID, amount: Int32) -> ref<DispenseStackRequest> {
    let dispenseRequest: ref<DispenseStackRequest> = new DispenseStackRequest();
    dispenseRequest.owner = this;
    dispenseRequest.itemID = item;
  ...
  protected func StartGlitching(glitchState: EGlitchState, opt intensity: Float) -> Void {
    ...
    GameObject.PlaySoundEvent(this, this.GetDevicePS().GetGlitchStartSFX());
    this.RefreshUI();
  }

  protected func StopGlitching() -> Void {
    this.AdvertGlitch(false, this.GetGlitchData(EGlitchState.NONE));
    GameObject.PlaySoundEvent(this, this.GetDevicePS().GetGlitchStopSFX());
    this.GetDevicePS().SetIsReady(true);
    this.RefreshUI();
  }
  ...
  protected cb func OnDispenseStackOfItems(evt: ref<DispenseStackOfItems>) -> Bool {
    this.DispenseStack(this.CreateQuestDispenseStackRequest(ItemID.FromTDBID(evt.item), evt.amount));
  }
  ...
  protected const func GetController() -> ref<VendingMachineController> {
    return this.m_controller as VendingMachineController;
  }

  public const func GetDevicePS() -> ref<VendingMachineControllerPS> {
...
public class DispenseFreeSpecificItem extends Event {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Items.Drink;Items.Food")
...
public class DispenseStackOfItems extends Event {

  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Items.money;Items.Drink;Items.Food;Items.QuestItem")
  public edit let item: TweakDBID;

  public edit let amount: Int32;
