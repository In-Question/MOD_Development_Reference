...
public class IceMachineController extends VendingMachineController {
  ...
  public const func GetPS() -> ref<IceMachineControllerPS> {
    return this.GetBasePS() as IceMachineControllerPS;
  }
}
  ...
  protected func PushShopStockActions(actions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void {
    ...
    if DispenceItemFromVendor.IsDefaultConditionMet(this, context) && this.m_isReady {
      ...
      if ArraySize(shopStock) > 0 {
        ArrayPush(Deref(actions), this.ActionDispenceIceCube(shopStock[0].itemID));
      };
    };
  }
