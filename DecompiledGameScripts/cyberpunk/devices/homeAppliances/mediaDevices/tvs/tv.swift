...
public class SetGlobalTvChannel extends Event {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;ChannelData")
...
public class TV extends InteractiveDevice {
  ...
  protected func ResolveGameplayState() -> Void {
    ...
    if this.GetDevicePS().IsInterfaceMuted() {
  ...
  public const func GetBlackboardDef() -> ref<TVDeviceBlackboardDef> {
    return this.GetDevicePS().GetBlackboardDef();
  }

  protected const func GetController() -> ref<TVController> {
    return this.m_controller as TVController;
  }

  public const func GetDevicePS() -> ref<TVControllerPS> {
  ...
  protected cb func OnNextChannel(evt: ref<NextStation>) -> Bool {
    let currentIndex: Int32 = this.GetDevicePS().GetActiveStationIndex();
    this.SelectChannel(currentIndex);
    this.GetDevicePS().PassChannelName(this.GetChannelName(currentIndex));
    this.TriggerArreaEffectDistraction(this.GetDefaultDistractionAreaEffectData(), evt.GetExecutor());
  }
  ...
  protected cb func OnQuestSetChannel(evt: ref<QuestSetChannel>) -> Bool {
    this.SelectChannel(this.GetDevicePS().GetActiveStationIndex());
  }
  ...
  protected cb func OnQuestSetGlobalChannel(evt: ref<SetGlobalTvChannel>) -> Bool {
    let idx: Int32 = this.GetDevicePS().GetGlobalTVChannelIDX(evt.m_channel);
    if idx >= 0 {
      this.GetDevicePS().SetActiveStationIndex(idx);
      this.SelectChannel(idx);
    };
  }

  protected cb func OnQuestSetGlobalTvOnly(evt: ref<SetGlobalTvOnly>) -> Bool {
    this.GetDevicePS().SetIsGlobalTvOnly(evt.m_isGlobalTvOnly);
    this.SelectChannel(this.GetDevicePS().GetActiveStationIndex());
  }

  public final const func GetDefaultGlitchVideoPath() -> ResRef {
    return this.GetDevicePS().GetDefaultGlitchVideoPath();
  }

  public final const func GetBroadcastGlitchVideoPath() -> ResRef {
    return this.GetDevicePS().GetBroadcastGlitchVideoPath();
  }
  ...
  private final func ToggleSoundEmmiter(mute: Bool) -> Void {
    if mute {
      ...
      GameObject.PlaySound(this, n"unmute_tv_emitter");
    };
  }
  ...
  private final const func GetChannelName(index: Int32) -> String {
    return this.GetDevicePS().GetChannelName(index);
  }

  private final func SelectChannel(const channelName: script_ref<String>) -> Void {
  ...
  private final func GetChannelId(const channelName: script_ref<String>) -> Int32 {
    return this.GetDevicePS().GetChannelID(channelName);
  }

  public final const func GetChannelData(channelIDX: Int32) -> STvChannel {
    return this.GetDevicePS().GetChannelData(channelIDX);
  }
  ...
  protected cb func OnQuestMuteSounds(evt: ref<QuestMuteSounds>) -> Bool {
    this.ToggleSoundEmmiter(this.GetDevicePS().IsInterfaceMuted());
  }

  protected cb func OnQuestToggleInteractivity(evt: ref<QuestToggleInteractivity>) -> Bool;

  public final const func GetGlobalTVChannels() -> [wref<ChannelData_Record>] {
    return this.GetDevicePS().GetGlobalTVChannels();
  }
}
