...
public class DisplayGlass extends InteractiveDevice {
  ...
  protected let m_mesh: ref<IPlacedComponent>;
  ...
  protected cb func OnRequestComponents(ri: EntityRequestComponentsInterface) -> Bool {
    ...
    EntityRequestComponentsInterface.RequestComponent(ri, n"glass_dst", n"IPlacedComponent", false);
  ...
  protected final func UpdateGlassState() -> Void {
    if !this.m_isDestroyed {
      this.ToggleTintGlass(this.GetDevicePS().IsTinted());
    };
  ...
  private final func ToggleTintGlass(on: Bool) -> Void {
    ...
    if !this.m_isDestroyed {
      ...
      if ps.UsesAppearances() {
        if IsDefined(this.m_mesh) {
          componentName = n"glass_dst";
        };
        if on {
          this.SetMeshAppearance(ps.GetTintAppearance(), componentName);
        } else {
          this.SetMeshAppearance(ps.GetClearAppearance(), componentName);
        };
      } else {
  ...
  protected const func GetController() -> ref<DisplayGlassController> {
    return this.m_controller as DisplayGlassController;
  }

  public const func GetDevicePS() -> ref<DisplayGlassControllerPS> {
