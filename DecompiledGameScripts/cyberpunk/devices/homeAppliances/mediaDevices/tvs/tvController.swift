...
public class TVController extends MediaDeviceController {
  ...
  public const func GetPS() -> ref<TVControllerPS> {
    return this.GetBasePS() as TVControllerPS;
  }
}
...
public class TVControllerPS extends MediaDeviceControllerPS {
  ...
  private let m_backupCustomChannels: [STvChannel];
  ...
  private final func InitializeTv() -> Void {
    ...
    this.SetInteractionState(this.m_tvSetup.m_isInteractive);
  }
  ...
  public func GetQuestActionByName(actionName: CName) -> ref<DeviceAction> {
    ...
    if action == null {
      switch actionName {
        case n"QuestMuteSounds":
          action = this.ActionQuestMuteSounds(true);
          break;
        case n"QuestUnMuteSounds":
          action = this.ActionQuestMuteSounds(false);
          break;
        case n"QuestEnableInteractivity":
          action = this.ActionQuestToggleInteractivity(true);
          break;
        case n"QuestDisableInteractivity":
          action = this.ActionQuestToggleInteractivity(false);
      };
    };
    return action;
  }
  ...
  public func OnQuestDefaultStation(evt: ref<QuestDefaultStation>) -> EntityNotificationType {
    ...
    this.m_activeChannelName = this.m_tvSetup.m_channels[this.m_activeStation].channelName;
  ...
  public final const func GetChannelID(const channelName: script_ref<String>) -> Int32 {
    let i: Int32 = 0;
    while i < ArraySize(this.m_tvSetup.m_channels) {
  ...
  public const func IsInteractive() -> Bool {
    return this.m_tvSetup.m_isInteractive && super.IsInteractive();
  }
  ...
  public func OnQuestMuteSounds(evt: ref<QuestMuteSounds>) -> EntityNotificationType {
    this.SetInterfaceMuted(FromVariant<Bool>(evt.prop.first));
    if !IsFinal() {
    };
    return EntityNotificationType.SendThisEventToEntity;
  }
  ...
  public func OnQuestToggleInteractivity(evt: ref<QuestToggleInteractivity>) -> EntityNotificationType {
    this.SetIsInteractive(FromVariant<Bool>(evt.prop.first));
    this.UseNotifier(evt);
    if !IsFinal() {
    };
    return EntityNotificationType.SendThisEventToEntity;
