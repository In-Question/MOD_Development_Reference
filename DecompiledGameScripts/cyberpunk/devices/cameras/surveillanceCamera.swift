...
public class SurveillanceCamera extends SensorDevice {
  ...
  protected final func SetForcedSensesTracingTask(data: ref<ScriptTaskData>) -> Void {
    if this.GetDevicePS().CanTagEnemies() {
  ...
  protected func ResolveGameplayState() -> Void {
    ...
    this.RegisterToGameSessionDataSystem(this.GetDevicePS().CanTagEnemies());
  }
  ...
  private const func GetController() -> ref<SurveillanceCameraController> {
    return this.m_controller as SurveillanceCameraController;
  }

  public const func GetDevicePS() -> ref<SurveillanceCameraControllerPS> {
  ...
  public func OnValidTargetAppears(target: wref<GameObject>) -> Void {
    ...
    this.GetDevicePS().ThreatDetected(true);
    this.RequestAlarm();
    if this.GetDevicePS().CanTagEnemies() && target.IsActive() {
  ...
  public func OnCurrentTargetAppears(target: wref<GameObject>) -> Void {
    ...
    if this.GetDevicePS().ShouldRevealEnemies() {
    ...
    if !this.GetDevicePS().GetSecuritySystem().IsSystemInCombat() && !GameObject.IsFriendlyTowardsPlayer(this) && target == GetPlayer(this.GetGame()) {
  ...
  public func OnAllValidTargetsDisappears() -> Void {
    ...
    this.GetDevicePS().ThreatDetected(false);
    this.RequestAlarm();
  }
  ...
  protected cb func OnToggleStreamFeed(evt: ref<ToggleStreamFeed>) -> Bool {
    ...
    this.ToggleFeed(this.GetDevicePS().ShouldStream());
  }
  ...
  protected cb func OnCameraTagLockEvent(evt: ref<CameraTagLockEvent>) -> Bool {
    this.GetDevicePS().SetTagLockFromSystem(evt.isLocked);
    this.SetForcedSensesTracing();
  }
  ...
  private final func ToggleFeed(shouldBeOn: Bool) -> Void {
    ...
    if shouldBeOn {
      ...
      this.GetDevicePS().ClearFeedReceivers();
    };
  }

  private final func RequestAlarm() -> Void {
    if this.GetDevicePS().IsDetecting() {
      SetFactValue(this.GetGame(), this.GetDevicePS().GetQuestFactOnDetection(), 1);
    } else {
      SetFactValue(this.GetGame(), this.GetDevicePS().GetQuestFactOnDetection(), 0);
    };
  }

  protected func OverrideLookAtSetupHor(out lookAtEntityEvent: ref<LookAtAddEvent>) -> Void {
    lookAtEntityEvent.request.limits.softLimitDegrees = this.GetDevicePS().GetBehaviourMaxRotationAngle() * 2.00;
  }
  ...
  public const func GetRotationData() -> CameraRotationData {
    let res: CameraRotationData = super.GetRotationData();
    if this.GetDevicePS().ShouldOverrideTakeOverAngle() {
      res.m_pitch = this.GetDevicePS().GetOverrideTakeOverPitch();
      res.m_yaw = this.GetDevicePS().GetOverrideTakeOverYaw();
    } else {
