...
public class AOEArea extends InteractiveMasterDevice {
  ...
  private let m_activeStatusEffects: [wref<StatusEffect_Record>];
  ...
  protected cb func OnGameAttached() -> Bool {
    ...
    if this.GetDevicePS().IsAreaActive() {
      this.ActivateEffect();
    };
  }
  ...
  private const func GetController() -> ref<AOEAreaController> {
    return this.m_controller as AOEAreaController;
  }

  public const func GetDevicePS() -> ref<AOEAreaControllerPS> {
  ...
  private final func ActivateArea() -> Void {
    this.ActivateEffect();
    if this.GetDevicePS().IsDangerous() && IsDefined(this.m_obstacleComponent) {
    ...
    if this.GetDevicePS().BlocksVisibility() && IsDefined(this.m_visionBlockerComponent) {
      this.m_visionBlockerComponent.Toggle(true);
    };
  }
  ...
  private final func ActivateEffect() -> Void {
    ...
    if !IsDefined(this.m_gameEffect) {
      attackContext.record = TweakDBInterface.GetAttackRecord(this.GetDevicePS().GetAreaEffect());
      ...
      attack.StartAttack();
      TweakDBInterface.GetAttackRecord(this.GetDevicePS().GetAreaEffect()).StatusEffects(statusEffectRecords);
      i = 0;
  ...
  private final func StopGameEffect() -> Void {
    ...
    if IsDefined(this.m_gameEffect) {
      ...
      if this.GetDevicePS().EffectsOnlyActiveInArea() {
  ...
  public func GetDistractionStimLifetime(defaultValue: Float) -> Float {
    return this.GetDevicePS().GetEffectDuration();
  }
