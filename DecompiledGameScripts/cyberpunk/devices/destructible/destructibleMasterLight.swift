...
public class DestructibleMasterLight extends DestructibleMasterDevice {
  ...
  private let m_lightComponents: [ref<gameLightComponent>];

  private const let m_lightDefinitions: [LightPreset];
  ...
  private const func GetController() -> ref<DestructibleMasterLightController> {
    return this.m_controller as DestructibleMasterLightController;
  }

  public const func GetDevicePS() -> ref<DestructibleMasterLightControllerPS> {
  ...
  private final func CreateColorFromIntArray(const ints: script_ref<[Int32]>, out color: Color) -> Bool {
    if ArraySize(Deref(ints)) != 3 {
      if !IsFinal() {
      };
      color = new Color(255u, 0u, 0u, 255u);
      return false;
    };
    color.Red = Cast<Uint8>(Deref(ints)[0]);
    color.Green = Cast<Uint8>(Deref(ints)[1]);
    color.Blue = Cast<Uint8>(Deref(ints)[2]);
    color.Alpha = 255u;
    return true;
  }
