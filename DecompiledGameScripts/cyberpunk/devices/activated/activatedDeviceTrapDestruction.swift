...
public class ActivatedDeviceTrapDestruction extends ActivatedDeviceTrap {
  ...
  protected const let m_hideMeshNames: [CName];

  protected let m_hideMeshes: [ref<IPlacedComponent>];

  protected const let m_hitColliderNames: [CName];

  protected let m_hitColliders: [ref<IPlacedComponent>];

  protected let m_impulseVector: Vector4;

  protected edit const let physicalMeshImpactVFX: [FxResource];

  protected edit const let m_componentsToEnableNames: [CName];

  protected let m_componentsToEnable: [ref<IPlacedComponent>];

  protected let hitCount: Int32;

  protected let m_wasAttackPerformed: Bool;

  protected let m_alreadyPlayedVFXComponents: [CName];
  ...
  protected cb func OnActivateDevice(evt: ref<ActivateDevice>) -> Bool {
    ...
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, vfxEnableTimer, this.GetDevicePS().GetAnimationTime());
      ...
      while i < ArraySize(this.m_hitColliders) {
        if this.m_hitColliders[i] == evt.hitComponent {
          ...
          if this.hitCount > 0 && !this.GetDevicePS().IsON() {
            this.GetDevicePS().ActivateThisDevice();
          };
        };
        i += 1;
      };
