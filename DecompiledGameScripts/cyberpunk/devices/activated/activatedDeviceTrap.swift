...
public class ActivatedDeviceTrap extends ActivatedDeviceTransfromAnim {
  ...
  protected cb func OnActivateDevice(evt: ref<ActivateDevice>) -> Bool {
    ...
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evnt, this.GetDevicePS().GetAnimationTime());
  }
    ...
    while i < ArraySize(entities) {
      if IsDefined(entities[i] as NPCPuppet) {
        ...
        if StatusEffectSystem.ObjectHasStatusEffect(entities[i] as NPCPuppet, t"BaseStatusEffect.Unconscious") {
          ...
          if StatusEffectSystem.ObjectHasStatusEffect(entities[i] as NPCPuppet, t"BaseStatusEffect.Grappled") {
            StatusEffectHelper.ApplyStatusEffect(entities[i] as NPCPuppet, t"BaseStatusEffect.Unconscious", this.GetEntityID());
          } else {
            if Equals(npcType, gamedataNPCType.Human) || Equals(npcType, gamedataNPCType.Android) {
              if this.GetDevicePS().ShouldRadgollOnAttack() {
  ...
  public final func GetEntitiesInArea() -> [ref<Entity>] {
  ...
  protected func ResolveGameplayState() -> Void {
    ...
    if IsDefined(this.m_fxResourceMapper) {
      this.m_fxResourceMapper.CreateEffectStructDataFromAttack(this.GetDevicePS().GetAttackType(), i, n"trapTargets");
    };
  }
}
