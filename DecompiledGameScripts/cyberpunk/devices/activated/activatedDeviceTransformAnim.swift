...
public class ActivatedDeviceTransfromAnim extends InteractiveDevice {
  ...
  protected const func GetController() -> ref<ActivatedDeviceController> {
    return this.m_controller as ActivatedDeviceController;
  }

  public const func GetDevicePS() -> ref<ActivatedDeviceControllerPS> {
  ...
  protected cb func OnActivateDevice(evt: ref<ActivateDevice>) -> Bool {
    ...
    this.RefreshAnimation();
    if this.GetDevicePS().ShouldGlitchOnActivation() {
  ...
  protected func RefreshAnimation() -> Void {
    ...
    playEvent.timeScale = this.GetDevicePS().GetAnimationTime();
