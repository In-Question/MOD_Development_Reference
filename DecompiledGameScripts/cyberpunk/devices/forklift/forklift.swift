...
public class forklift extends InteractiveDevice {
  ...
  protected const func GetController() -> ref<ForkliftController> {
    return this.m_controller as ForkliftController;
  }

  public const func GetDevicePS() -> ref<ForkliftControllerPS> {
  ...
  protected cb func OnActivateDevice(evt: ref<ActivateDevice>) -> Bool {
    ...
    let isForkliftUp: Bool = this.GetDevicePS().IsForkliftUp();
    if this.m_reversed {
    ...
    this.UpdateAnimState();
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, forkliftCompleteActivateEvent, this.GetDevicePS().GetLiftingAnimationTime());
  }

  private final func UpdateAnimState() -> Void {
    this.m_animFeature.isDown = !this.GetDevicePS().IsForkliftUp();
    this.m_animFeature.isUp = this.GetDevicePS().IsForkliftUp();
    this.m_animFeature.distract = this.GetDevicePS().IsDistracting();
    if this.m_reversed {
      this.m_animFeature.isDown = !this.m_animFeature.isDown;
      this.m_animFeature.isUp = !this.m_animFeature.isUp;
    };
  ...
  protected cb func OnForkliftCompleteActivateEvent(evt: ref<ForkliftCompleteActivateEvent>) -> Bool {
    this.GetDevicePS().ChangeState(EDeviceStatus.ON);
