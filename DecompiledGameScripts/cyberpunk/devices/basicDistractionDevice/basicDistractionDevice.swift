...
public class BasicDistractionDevice extends InteractiveDevice {
  ...
  protected edit const let m_distractionComponentSwapNamesToON: [CName];

  protected edit const let m_distractionComponentSwapNamesToOFF: [CName];

  private let m_distractionComponentON: [ref<IPlacedComponent>];

  private let m_distractionComponentOFF: [ref<IPlacedComponent>];

  @runtimeProperty("category", "Distraction properties")
  protected edit let m_meshAppearanceNameON: CName;

  @runtimeProperty("category", "Distraction properties")
  protected edit let m_meshAppearanceNameOFF: CName;
  ...
  protected cb func OnTakeControl(ri: EntityResolveComponentsInterface) -> Bool {
    ...
    i = 0;
    while i < ArraySize(this.m_distractionComponentSwapNamesToOFF) {
      ArrayPush(this.m_distractionComponentOFF, EntityResolveComponentsInterface.GetComponent(ri, this.m_distractionComponentSwapNamesToOFF[i]) as IPlacedComponent);
      i += 1;
    };
  ...
  protected cb func OnGameAttached() -> Bool {
    ...
    if this.GetDevicePS().GetForceAnimationSystem() {
  ...
  private const func GetController() -> ref<BasicDistractionDeviceController> {
    return this.m_controller as BasicDistractionDeviceController;
  }

  public const func GetDevicePS() -> ref<BasicDistractionDeviceControllerPS> {
  ...
  protected func StartDistraction(opt loopAnimation: Bool) -> Void {
    ...
    this.SetMeshAppearance(this.m_meshAppearanceNameON);
  ...
  protected func StopDistraction() -> Void {
    ...
    this.SetMeshAppearance(this.m_meshAppearanceNameOFF);
    this.MeshSwapOnDistraction(false);
  }

  protected func PlayDistractAnimation(opt loop: Bool) -> Void {
    if Equals(this.GetDevicePS().GetAnimationType(), EAnimationType.REGULAR) {
      this.PlayAnimgraphTransformAnimation();
    } else {
      if Equals(this.GetDevicePS().GetAnimationType(), EAnimationType.TRANSFORM) {
  ...
  protected final func EffectsOnStartPlay() -> Void {
    let effectNames: array<CName> = this.GetDevicePS().GetEffectOnStartNames();
    let i: Int32 = 0;
  ...
  protected final func MeshSwapOnDistraction(start: Bool) -> Void {
    ...
    i = 0;
    while i < ArraySize(this.m_distractionComponentOFF) {
      this.m_distractionComponentOFF[i].Toggle(!start);
      i += 1;
    };
  }
}
