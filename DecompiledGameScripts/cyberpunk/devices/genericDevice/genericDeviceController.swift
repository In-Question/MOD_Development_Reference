...
public class CustomDeviceAction extends ActionBool {
  ...
  public final static func IsDefaultConditionMet(device: ref<ScriptableDeviceComponentPS>, const context: script_ref<GetActionsContext>, const actionData: script_ref<SDeviceActionCustomData>) -> Bool {
    if CustomDeviceAction.IsAvailable(device, actionData) && CustomDeviceAction.IsClearanceValid(Deref(context).clearance) {
      return true;
    };
    return false;
  }

  public final static func IsAvailable(device: ref<ScriptableDeviceComponentPS>, const actionData: script_ref<SDeviceActionCustomData>) -> Bool {
    if device.IsDisabled() {
      return false;
    };
    if Equals(Deref(actionData).RestrictDuringPhonecall, true) && StatusEffectSystem.ObjectHasStatusEffectWithTag(GetPlayer(GetGameInstance()), n"PhoneCall") {
      return false;
    };
    if Equals(Deref(actionData).RestrictInCombat, true) && Equals(GetPlayer(GetGameInstance()).IsInCombat(), true) {
      return false;
    };
    if Equals(Deref(actionData).RestrictIfWanted, true) && GameInstance.GetQuestsSystem(GetGameInstance()).GetFact(n"wanted_level") != 0 {
      return false;
    };
    if device.IsON() && Deref(actionData).On {
      return true;
    };
    if device.IsOFF() && Deref(actionData).Off {
      return true;
    };
    if device.IsUnpowered() && Deref(actionData).Unpowered {
      return true;
    };
    return false;
  }
...
public class GenericDeviceController extends ScriptableDeviceComponent {

  public const func GetPS() -> ref<GenericDeviceControllerPS> {
    return this.GetBasePS() as GenericDeviceControllerPS;
  }
}

public class GenericDeviceControllerPS extends ScriptableDeviceComponentPS {

  @runtimeProperty("category", "Senses")
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;DeviceWidgetDefinition")
  public edit let m_deviceWidgetRecord: TweakDBID;

  @runtimeProperty("category", "UI")
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;ThumbnailWidgetDefinition")
  public edit let m_thumbnailWidgetRecord: TweakDBID;

  private persistent let m_performedCustomActionsIDs: [CName];
  ...
  protected func GetSkillCheckContainerForSetup() -> ref<BaseSkillCheckContainer> {
    return this.m_genericDeviceSkillChecks;
  }
  ...
  protected final func ActionCustom(const actionData: script_ref<SDeviceActionCustomData>) -> ref<CustomDeviceAction> {
    let action: ref<CustomDeviceAction>;
    let displayName: String;
    let tweakDBName: String;
    if TDBID.IsValid(Deref(actionData).displayNameRecord) {
      displayName = LocKeyToString(TweakDBInterface.GetInteractionBaseRecord(Deref(actionData).displayNameRecord).Caption());
    } else {
      tweakDBName = Deref(actionData).displayName;
    };
    action = new CustomDeviceAction();
    action.clearanceLevel = Deref(actionData).customClearance;
    action.SetUp(this);
    action.SetProperties(Deref(actionData).actionID, StringToName(displayName));
    action.AddDeviceName(this.m_deviceName);
    if TDBID.IsValid(Deref(actionData).objectActionRecord) {
      action.SetObjectActionID(Deref(actionData).objectActionRecord);
    } else {
      if Deref(actionData).isQuickHack {
        action.SetObjectActionID(t"DeviceAction.MalfunctionClassHack");
      };
    };
    if Deref(actionData).hasInteraction || Deref(actionData).isQuickHack || Deref(actionData).isSpiderbotAction {
      action.CreateCustomInteraction(tweakDBName, tweakDBName, Deref(actionData).displayNameRecord, Deref(actionData).displayNameRecord);
    };
    if Deref(actionData).hasUI {
      if TDBID.IsValid(Deref(actionData).widgetRecord) {
        action.CreateActionWidgetPackage(Deref(actionData).widgetRecord);
      } else {
  ...
  protected final func ActionTogglePower(const actionData: script_ref<SDeviceActionBoolData>) -> ref<TogglePower> {
    ...
    if TDBID.IsValid(Deref(actionData).nameOnTrueRecord) {
      actionNameOnTrue = Deref(actionData).nameOnTrueRecord;
    } else {
      tweakDBNameOnTrue = Deref(actionData).nameOnTrue;
      actionNameOnTrue = TDBID.Create("Interactions." + tweakDBNameOnTrue);
    };
    if TDBID.IsValid(Deref(actionData).nameOnFalseRecord) {
      actionNameOnFalse = Deref(actionData).nameOnFalseRecord;
    } else {
      tweakDBNameOnFalse = Deref(actionData).nameOnFalse;
    ...
    if Deref(actionData).hasInteraction || Deref(actionData).isQuickHack {
      action.CreateCustomInteraction(tweakDBNameOnTrue, tweakDBNameOnFalse, Deref(actionData).nameOnTrueRecord, Deref(actionData).nameOnFalseRecord);
    };
    if Deref(actionData).hasUI {
      if TDBID.IsValid(Deref(actionData).widgetRecord) {
        action.CreateActionWidgetPackage(Deref(actionData).widgetRecord);
  ...
  public final func ActionToggleON(const actionData: script_ref<SDeviceActionBoolData>) -> ref<ToggleON> {
    let actionNameOnFalse: TweakDBID;
    let actionNameOnTrue: TweakDBID;
    let tweakDBNameOnFalse: String;
    let tweakDBNameOnTrue: String;
  ...
  protected func GetSpiderbotActions(actions: script_ref<[ref<DeviceAction>]>, const context: script_ref<GetActionsContext>) -> Void {
    ...
    if actionON != null {
      ArrayPush(Deref(actions), actionON);
    };
    if actionPower != null {
      ArrayPush(Deref(actions), actionPower);
    };
    i = 0;
    while i < ArraySize(this.m_genericDeviceActionsSetup.m_customActions.actions) {
      if this.m_genericDeviceActionsSetup.m_customActions.actions[i].isEnabled && this.m_genericDeviceActionsSetup.m_customActions.actions[i].isSpiderbotAction {
        ...
        if CustomDeviceAction.IsAvailable(this, this.m_genericDeviceActionsSetup.m_customActions.actions[i]) {
          ...
          ArrayPush(Deref(actions), customAction);
        };
      };
      i += 1;
    };
  ...
  public final const func GetPerformedCustomActionsStorage() -> [CName] {
  ...
  public final func OnQuestToggleCustomAction(evt: ref<QuestToggleCustomAction>) -> EntityNotificationType {
    let enabled: Bool;
    let actionID: CName = FromVariant<CName>(evt.prop.first);
  ...
  protected func ActionHacking(const context: script_ref<GetActionsContext>) -> ref<ActionHacking> {
    let action: ref<ActionHacking> = super.ActionHacking(context);
  ...
  public func GetThumbnailWidget() -> SThumbnailWidgetPackage {
    let widgetData: SThumbnailWidgetPackage = super.GetThumbnailWidget();
