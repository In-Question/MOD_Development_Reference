...
public class AreaEffectVisualizationComponent extends ScriptableComponent {
  ...
  private let m_forceHighlightTargetBuckets: [ref<GameEffectTargetVisualizationData>];
  ...
  protected cb func OnHUDInstruction(evt: ref<HUDInstruction>) -> Bool {
    ...
    if (Equals(instanceState, InstanceState.ON) || Equals(instanceState, InstanceState.HIDDEN)) && evt.highlightInstructions.isLookedAt {
      this.ResolveAreaEffectVisualisations(true, instanceState);
    } else {
      if evt.highlightInstructions.WasProcessed() {
        this.ResolveAreaEffectVisualisations(false, instanceState);
      };
    };
  }
  ...
  public final func ResolveAreaEffectVisualisations(activated: Bool, instanceState: InstanceState) -> Void {
    if this.GetFxMapper().GetAreaEffectDataSize() <= 0 && this.GetFxMapper().GetAreaEffectInFocusSize() <= 0 {
      return;
    };
    this.ResolveAreaEffectsVisibility(activated, instanceState);
  }
  ...
  protected func ResolveAreaEffectsVisibility(show: Bool, instanceState: InstanceState) -> Void {
    this.ResolveAreaSpiderbotVisibility(show);
    if !show || Equals(instanceState, InstanceState.ON) {
  ...
  protected final func StartHighlightingTargets(effectDataIDX: Int32, opt responseData: ref<IScriptable>) -> Void {
    ...
    if this.GetFxMapper().GetAreaEffectDataByIndex(effectDataIDX).effectInstance == null {
      ...
      if IsDefined(device) {
        ...
        stimID = TDBID.Create("stims." + EnumValueToString("gamedataStimType", Cast<Int64>(EnumInt(stimType))) + "Stimuli");
...
public class GameEffectTargetVisualizationData extends IScriptable {
  ...
  private let m_forceHighlightTargets: [EntityID];
