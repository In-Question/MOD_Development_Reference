...
public class MaintenancePanel extends InteractiveMasterDevice {
  ...
  private const func GetController() -> ref<MaintenancePanelController> {
    return this.m_controller as MaintenancePanelController;
  }

  public const func GetDevicePS() -> ref<MaintenancePanelControllerPS> {
  ...
  protected cb func OnWorkspotFinished(componentName: CName) -> Bool {
    ...
    if Equals(componentName, n"disassembleWorkspot") {
      ...
      this.GetDevicePS().RefreshLockOnSlaves();
