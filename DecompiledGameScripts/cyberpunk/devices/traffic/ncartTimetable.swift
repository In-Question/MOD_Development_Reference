...
public class NcartTimetable extends InteractiveDevice {
  ...
  protected const func GetController() -> ref<NcartTimetableController> {
    return this.m_controller as NcartTimetableController;
  }

  public const func GetDevicePS() -> ref<NcartTimetableControllerPS> {
  ...
  private final func InitializeDisplayUpdate() -> Void {
    ...
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evt, this.GetDevicePS().GetUiUpdateFrequency());
    this.UpdateCounterUI();
  }

  private final func UpdateCounterUI() -> Void {
    let counterValue: Int32 = this.GetDevicePS().GetCurrentTimeToDepart();
    this.GetBlackboard().SetInt(GetAllBlackboardDefs().NcartTimetableBlackboard.TimeToDepart, counterValue, true);
    this.GetBlackboard().SetInt(GetAllBlackboardDefs().NcartTimetableBlackboard.NextTrainLine, this.GetDevicePS().GetCurrentLine(), true);
    this.GetBlackboard().FireCallbacks();
  }

  protected cb func OnCounterUpdate(evt: ref<NcartTimeTableCounterUpdateEvent>) -> Bool {
    evt = new NcartTimeTableCounterUpdateEvent();
    this.GetDevicePS().UpdateCurrentTimeToDepart();
    this.UpdateCounterUI();
    GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, evt, this.GetDevicePS().GetUiUpdateFrequency());
  }
}
