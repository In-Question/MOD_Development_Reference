...
public class BillboardDevice extends InteractiveDevice {
  ...
  protected func ResolveGameplayState() -> Void {
    ...
    if this.GetDevicePS().IsUsingDeviceAppearence() {
  ...
  protected final func ToggleLights(on: Bool) -> Void {
    ...
    if !this.GetDevicePS().IsUsingLights() {
      return;
    };
    ...
    this.QueueEvent(evt);
    if on {
      settings = this.GetDevicePS().GetLightsSettings();
      i = 0;
  ...
  private const func GetController() -> ref<BillboardDeviceController> {
    return this.m_controller as BillboardDeviceController;
  }

  public const func GetDevicePS() -> ref<BillboardDeviceControllerPS> {
