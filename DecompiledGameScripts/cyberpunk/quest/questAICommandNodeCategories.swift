...
public native class AICommandNodeFunctionProvider extends IScriptable {
  ...
  private final static func Add(functions: script_ref<[AICommandNodeFunction]>, nodeType: CName, category: CName, const friendlyName: script_ref<String>, paramsType: CName, color: Color) -> Void {
    let fn: AICommandNodeFunction;
    fn.order = Cast<Uint32>(ArraySize(Deref(functions)));
    fn.nodeType = nodeType;
    fn.commandCategory = category;
    fn.friendlyName = Deref(friendlyName);
    fn.paramsType = paramsType;
    fn.nodeColor = color;
    ArrayPush(Deref(functions), fn);
  }

  public final static func CollectFunctions() -> [AICommandNodeFunction] {
    ...
    AICommandNodeFunctionProvider.Add(functions, n"Move", n"move", "Join Crowd", n"questJoinCrowdParams", green);
