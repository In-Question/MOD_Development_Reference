...
public class SimpleCombatConditon extends AIbehaviorconditionScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    AIbehaviorconditionScript.SetUpdateInterval(context, 2.00);
  }
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    ...
    if AIActionHelper.CheckAbility(puppet, this.m_takeCoverAbility) {
      ...
      if AIActionHelper.IsCurrentlyInCoverAttackAction(puppet) {
        if Equals(aiComponent.GetCombatMode(), AIbehaviorCombatModes.Default) {
          return AIbehaviorConditionOutcomes.False;
        };
        return AIbehaviorConditionOutcomes.True;
      };
    ...
    if membersCount - squadMatesInSimpleCombat < 2 && !AIScriptSquad.HasOrder(puppet, n"SimpleCombat") {
      return AIbehaviorConditionOutcomes.False;
    };
    return AIbehaviorConditionOutcomes.True;
  }
