...
public class GlobalDeathCondition extends AIDeathConditions {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    if IsDefined(AIBehaviorScriptBase.GetPuppet(context)) {
      ...
      return Cast<AIbehaviorConditionOutcomes>(GameInstance.GetStatPoolsSystem(AIBehaviorScriptBase.GetGame(context)).HasStatPoolValueReachedMin(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatPoolType.Health));
    };
    return AIbehaviorConditionOutcomes.False;
  }
}
...
public class PassiveGlobalDeathCondition extends AIbehaviorexpressionScript {
  ...
  protected final func CalculateValue(context: ScriptExecutionContext) -> Variant {
    ...
    if Equals(puppet.GetHitReactionComponent().GetDeathStimName(), n"Death") {
      return ToVariant(false);
    };
    return ToVariant(GameInstance.GetStatPoolsSystem(AIBehaviorScriptBase.GetGame(context)).HasStatPoolValueReachedMin(Cast<StatsObjectID>(puppet.GetEntityID()), gamedataStatPoolType.Health));
  }
}
...
public class DeathWithoutRagdollCondition extends AIDeathConditions {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    if Equals(IntEnum<gamedataNPCStanceState>(AIBehaviorScriptBase.GetPuppet(context).GetPuppetStateBlackboard().GetInt(GetAllBlackboardDefs().PuppetState.Stance)), gamedataNPCStanceState.Vehicle) {
