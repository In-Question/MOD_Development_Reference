...
public abstract class AISignalCondition extends AIbehaviorconditionScript {
  ...
  public edit const let m_requiredFlags: [AISignalFlags];
...
public class CheckAbilityCanRetreat extends AIbehaviorconditionScript {

  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    return Cast<AIbehaviorConditionOutcomes>(GameInstance.GetStatsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetStatBoolValue(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatType.CanRetreat));
  }
}
