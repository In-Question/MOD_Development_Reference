...
public class LeaveCoverImmediately extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    this.m_timeStamp = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    if !this.m_triggered && this.m_delay > 0.00 && this.m_delay + this.m_timeStamp >= EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) {
