...
public abstract class AIGenericEntityLookatTask extends AIGenericLookatTask {
  ...
  protected func ShouldLookatBeActive(context: ScriptExecutionContext) -> Bool {
    ...
    if EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) < this.m_activationTimeStamp + this.GetLookActivationDelay() {
      return false;
    };
    return true;
  }
...
public abstract class AISearchingLookat extends AIGenericStaticLookatTask {
  ...
  private func GetAimingLookatTarget(context: ScriptExecutionContext) -> Vector4 {
    if EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) < this.m_targetSwitchTimeStamp + this.m_targetSwitchCooldown && this.GetAbsAngleToTarget(context) < this.GetSoftLookatLimitDegrees() * 0.50 {
      return this.m_currentTarget;
    };
    this.m_targetSwitchCooldown = RandRangeF(1.50, 3.00);
    this.m_targetSwitchTimeStamp = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
  ...
  private final func SelectNewAimingLookatTarget(context: ScriptExecutionContext) -> Vector4 {
    ...
    offsetVector = Vector4.FromHeading(Vector4.Heading(AIBehaviorScriptBase.GetPuppet(context).GetWorldPosition() - this.GetLookatTargetPosition(context)) + 90.00 * Cast<Float>(this.m_sideHorizontal));
...
public class SearchPatternMappingLookat extends AISearchingLookat {
  ...
  private func InitializeMemberVariables(context: ScriptExecutionContext) -> Bool {
    if IsDefined(this.m_targetObjectMapping) && !IsDefined(this.m_lookatTargetObject) {
      this.m_lookatTargetObject = FromVariant<wref<GameObject>>(ScriptExecutionContext.GetMappingValue(context, this.m_targetObjectMapping));
    };
    if !IsDefined(this.m_lookatTargetObject) {
      return false;
    };
    if IsDefined(this.m_minAngleDifferenceMapping) && this.m_minAngleDifference == 0.00 {
      this.m_minAngleDifference = FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_minAngleDifferenceMapping));
    };
    if IsDefined(this.m_maxLookAroundAngleMapping) && this.m_maxLookAroundAngle == 0.00 {
      this.m_maxLookAroundAngle = FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_maxLookAroundAngleMapping));
    };
    return true;
  }
}
...
public class CentaurShieldLookatController extends AILookatTask {
  ...
  private final func GetShieldState(context: ScriptExecutionContext) -> ECentaurShieldState {
    if IsDefined(AIBehaviorScriptBase.GetPuppet(context).GetCustomBlackboard()) {
      return IntEnum<ECentaurShieldState>(AIBehaviorScriptBase.GetPuppet(context).GetCustomBlackboard().GetInt(GetAllBlackboardDefs().CustomCentaurBlackboard.ShieldState));
    };
  ...
  private final func ActivateMainShieldLookat(context: ScriptExecutionContext, lookatTarget: wref<GameObject>) -> Void {
    ...
    this.m_mainShieldLookat.SetLimits(animLookAtLimitDegreesType.None, animLookAtLimitDegreesType.None, animLookAtLimitDistanceType.None, animLookAtLimitDegreesType.None);
    if !IsFinal() {
  ...
  private final func ReevaluateDesiredLookatTarget(context: ScriptExecutionContext) -> Void {
    ...
    if this.m_shieldTarget != null {
      if this.m_shieldTargetTimeStamp < 0.00 {
        this.m_shieldTargetTimeStamp = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
      };
  ...
  private final func IsShieldTargetValid(context: ScriptExecutionContext) -> Bool {
    ...
    if shieldTargetTimeout > 0.00 && EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) >= this.m_shieldTargetTimeStamp + shieldTargetTimeout {
      return false;
    };
    return true;
  }
