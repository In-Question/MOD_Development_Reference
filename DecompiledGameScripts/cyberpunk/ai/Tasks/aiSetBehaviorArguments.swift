...
public class SetTimeStampToArgumentFloat extends SetArguments {

  protected func Activate(context: ScriptExecutionContext) -> Void {
    ScriptExecutionContext.SetArgumentFloat(context, this.m_argumentVar, EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)));
  }

  public func GetEditorSubCaption() -> String {
    return "Set " + ToString(this.m_argumentVar) + " To " + "AI Time";
  }
}
...
public class SetArgumentVector extends SetArguments {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    let newValue: Vector4 = FromVariant<Vector4>(ScriptExecutionContext.GetMappingValue(context, this.m_newValue));
