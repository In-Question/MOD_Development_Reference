...
public class GetRandomPositionAroundPoint extends AIRandomTasks {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    let originObject: wref<GameObject> = FromVariant<ref<GameObject>>(ScriptExecutionContext.GetMappingValue(context, this.m_originPoint));
    if IsDefined(originObject) {
      originOrientation = originObject.GetWorldOrientation();
    } else {
      originPosition = FromVariant<Vector4>(ScriptExecutionContext.GetMappingValue(context, this.m_originPoint));
      Quaternion.SetIdentity(originOrientation);
    };
    if !IsDefined(originObject) && Vector4.IsZero(originPosition) {
      return;
    };
    chosenAngle = RandRangeF(FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_angleMin)), FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_angleMax)));
    chosenDistance = RandRangeF(FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_distanceMin)), FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_distanceMax)));
