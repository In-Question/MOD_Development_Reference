...
public abstract class AISignalSenderTask extends AIbehaviortaskScript {
  ...
  public edit let tags: [CName];

  public edit let flags: [EAIGateSignalFlags];
  ...
  protected final func QueueGateSignal(context: ScriptExecutionContext) -> Void {
    ...
    while i < ArraySize(this.flags) {
      AIGateSignal.AddFlag(signal, Cast<AISignalFlags>(this.flags[i]));
      i += 1;
    };
...
public class CerberusCustomEventSender extends AISignalSenderTask {
