...
public class NPCInitTask extends AIbehaviortaskStackScript {
  ...
  private final func NPCWasDeadOnInit(context: script_ref<ScriptExecutionContext>) -> Bool {
    if GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(Deref(context)).GetGame()).HasStatPoolValueReachedMin(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(Deref(context)).GetEntityID()), gamedataStatPoolType.Health) {
  ...
  private final func HasHLS(context: script_ref<ScriptExecutionContext>, state: gamedataNPCHighLevelState) -> Bool {
    return Equals(state, IntEnum<gamedataNPCHighLevelState>(AIBehaviorScriptBase.GetPuppet(Deref(context)).GetPuppetStateBlackboard().GetInt(GetAllBlackboardDefs().PuppetState.HighLevel)));
  }
  ...
  private final func SendSignal(context: script_ref<ScriptExecutionContext>, tag1: CName, tag2: CName, flag: EAIGateSignalFlags, priority: Float) -> Void {
    ...
    if NotEquals(flag, EAIGateSignalFlags.AIGSF_Undefined) {
      AIGateSignal.AddFlag(signal, Cast<AISignalFlags>(flag));
    };
