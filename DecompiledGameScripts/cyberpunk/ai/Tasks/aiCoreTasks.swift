...
public class SelectorRevalutionBreak extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    AIbehaviortaskScript.SetUpdateInterval(context, this.m_reevaluationDuration);
  }

  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    if EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) < this.m_activationTimeStamp + this.m_reevaluationDuration {
...
public class SetTopThreatToCombatTarget extends AIbehaviortaskScript {
  ...
  protected func Deactivate(context: ScriptExecutionContext) -> Void {
    if NotEquals(AIBehaviorScriptBase.GetPuppet(context).GetHighLevelStateFromBlackboard(), gamedataNPCHighLevelState.Combat) {
  ...
  private final func GetThreatClosestToPlayer(context: ScriptExecutionContext, out target: TrackedLocation) -> Bool {
    let distance: Float;
    let i: Int32;
    let player: ref<PlayerPuppet> = GetPlayer(ScriptExecutionContext.GetOwner(context).GetGame());
    let targetTrackerComp: ref<TargetTrackerComponent> = player.GetTargetTrackerComponent();
    let allThreats: array<TrackedLocation> = targetTrackerComp.GetHostileThreats(true);
    let shortestDistance: Float = 99999.00;
    let closestThreatIndex: Int32 = 0;
    if ArraySize(allThreats) == 0 {
      return false;
    };
    i = 0;
    while i < ArraySize(allThreats) {
      distance = Vector4.Distance(player.GetWorldPosition(), allThreats[i].location.position);
      if distance < shortestDistance || shortestDistance == 99999.00 {
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    if !this.IsCurrentTargetValid(context, currentUpperBodyState) || this.CanSwitchTarget(context, currentUpperBodyState) {
      ...
      if !this.GetCommandCombatTarget(context, newTargetObject) {
        isPlayerGuardian = GameInstance.GetStatsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetStatBoolValue(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatType.IsPlayerGuardian);
        if isPlayerGuardian && this.GetThreatClosestToPlayer(context, topThreat) {
  ...
  private final func CanSwitchTarget(context: ScriptExecutionContext, currentUpperBodyState: gamedataNPCUpperBodyState) -> Bool {
    if EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) < this.m_previousChecktime + this.m_refreshTimer {
      return false;
    };
  ...
  private final func SetCombatTarget(context: ScriptExecutionContext, target: ref<GameObject>) -> Void {
    if target.IsPlayer() {
      if StatusEffectSystem.ObjectHasStatusEffectOfType(target, gamedataStatusEffectType.BlockTargetingPlayer) {
        ...
        if StatusEffectSystem.ObjectHasStatusEffectOfType(ScriptExecutionContext.GetOwner(context), gamedataStatusEffectType.BlockTargetingPlayer) {
          target = null;
        };
      };
    };
    ...
    if IsDefined(target) {
      ...
      this.m_targetChangeTime = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
    };
  }
  ...
  private final func IsTargetLost(context: ScriptExecutionContext, const trackedLocation: script_ref<TrackedLocation>) -> Bool {
    ...
    if Deref(trackedLocation).visible || Deref(trackedLocation).sharedAccuracy >= 0.33 || !Deref(trackedLocation).invalidExpectation {
      return false;
    };
    ...
    vecToTarget = Deref(trackedLocation).sharedLocation.position - ScriptExecutionContext.GetOwner(context).GetWorldPosition();
...
public class TempClearForcedCombatTarget extends AIbehaviortaskScript {
  ...
  protected func Deactivate(context: ScriptExecutionContext) -> Void {
    let threatPersistenceSource: ref<AIThreatPersistenceSource_Record> = TweakDBInterface.GetAIThreatPersistenceSourceRecord(t"AIThreatPersistenceSource.SetNewCombatTarget");
    AICombatTargetHelper.SetNewCombatTarget(context, null);
    AIActionHelper.ClearCommandCombatTarget(context, Cast<Uint32>(threatPersistenceSource.EnumValue()), true);
  }
}
...
public abstract class AICombatTargetHelper extends IScriptable {
  ...
  public final static func SetNewCombatTarget(context: ScriptExecutionContext, target: ref<GameObject>) -> Bool {
    ...
    if GameInstance.GetStatsSystem(owner.GetGame()).GetStatBoolValue(Cast<StatsObjectID>(target.GetEntityID()), gamedataStatType.IsIgnoredByEnemyNPC) {
      return false;
    };
    ...
    if IsDefined(target) && (target.IsPlayer() || ScriptedPuppet.IsPlayerCompanion(target)) {
      ...
      TargetTrackingExtension.SetThreatPersistence(target as ScriptedPuppet, owner, true, Cast<Uint32>(threatPersistenceSource.EnumValue()));
    };
      ...
      if prevTarget.IsPlayer() || ScriptedPuppet.IsPlayerCompanion(prevTarget) {
        ...
        if IsDefined(tte) {
          if !tte.WasThreatPersistent(prevTarget) {
            TargetTrackingExtension.SetThreatPersistence(prevTarget as ScriptedPuppet, owner, false, Cast<Uint32>(threatPersistenceSource.EnumValue()));
          };
        };
      };
      if IsDefined(owner) {
...
public class AddWeapon extends AIbehaviortaskScript {
  ...
  private final static func Execute(puppet: ref<ScriptedPuppet>, weapon: EquipmentPriority) -> Void {
    ...
    let powerLevel: Int32 = Cast<Int32>(statSys.GetStatValue(Cast<StatsObjectID>(puppet.GetEntityID()), gamedataStatType.PowerLevel));
...
public class IncrementArgumentInt extends AIbehaviortaskScript {

  public edit let argumentIntName: CName;

  protected func Activate(context: ScriptExecutionContext) -> Void {
    let argumentIntValue: Int32 = ScriptExecutionContext.GetArgumentInt(context, this.argumentIntName);
    argumentIntValue += 1;
    ScriptExecutionContext.SetArgumentInt(context, this.argumentIntName, argumentIntValue);
  }
}
