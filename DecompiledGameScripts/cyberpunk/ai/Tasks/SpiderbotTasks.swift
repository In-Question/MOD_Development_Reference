...
public class FindClosestScavengeTarget extends AIbehaviortaskScript {
  ...
  private final func GetClosestTarget(context: ScriptExecutionContext, const targets: script_ref<[wref<GameObject>]>) -> wref<GameObject> {
    ...
    while i < ArraySize(Deref(targets)) {
      currentTargetDistance = Vector4.Distance(ScriptExecutionContext.GetOwner(context).GetWorldPosition(), Deref(targets)[i].GetWorldPosition());
      if currentTargetDistance < shortestDistance || shortestDistance == 0.00 {
        shortestDistance = currentTargetDistance;
        closestTarget = Deref(targets)[i];
      };
      i += 1;
    };
...
public class MoveToScavengeTarget extends AIbehaviortaskScript {
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    if this.m_lastTime < 0.00 {
      this.m_lastTime = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
    };
    dt = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) - this.m_lastTime;
    this.m_lastTime = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
    this.m_timeout -= dt;
...
public class HaveScavengeTargets extends AIbehaviorconditionScript {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    ...
    let scavengeComponent: ref<ScavengeComponent> = owner.GetScavengeComponent();
    let scavengeTargets: array<wref<GameObject>> = scavengeComponent.GetScavengeTargets();
    return Cast<AIbehaviorConditionOutcomes>(ArraySize(scavengeTargets) > 0);
  }
}
...
public class IsCombatModuleEquipped extends AIAutonomousConditions {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    let owner: ref<NPCPuppet> = ScriptExecutionContext.GetOwner(context) as NPCPuppet;
    return Cast<AIbehaviorConditionOutcomes>(GameInstance.GetStatsSystem(owner.GetGame()).GetStatBoolValue(Cast<StatsObjectID>(owner.GetEntityID()), gamedataStatType.CanPickUpWeapon));
  }
}
...
public class AIFindPositionAroundSelf extends AIbehaviortaskScript {
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    while i < 5 {
      ...
      Quaternion.SetZRot(quat, angleRad);
      quat = orientation * quat;
      distance = RandRangeF(FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_distanceMin)), FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_distanceMax)));
...
public class AISpiderbotFindBoredMovePosition extends AIFindPositionAroundSelf {
  ...
  protected func AdditionalOutcomeVerification(context: ScriptExecutionContext) -> Bool {
    if Vector4.Distance(ScriptExecutionContext.GetArgumentObject(context, n"FriendlyTarget").GetWorldPosition(), this.m_finalPosition) > FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_maxWanderDistance)) {
      return false;
    };
    return true;
  }
...
public class AISpiderbotCheckIfFriendlyMoved extends AIAutonomousConditions {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    return Cast<AIbehaviorConditionOutcomes>(Vector4.Distance(ScriptExecutionContext.GetArgumentObject(context, n"FriendlyTarget").GetWorldPosition(), ScriptExecutionContext.GetArgumentVector(context, n"FriendlyTargetLastPosition")) > FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.m_maxAllowedDelta)));
  }
}
...
public class AISetHealthRegenerationState extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    if this.healthRegeneration {
      GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).RequestResetingModifier(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatPoolType.Health, gameStatPoolModificationTypes.Regeneration);
    } else {
      GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).RequestSettingModifier(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatPoolType.Health, gameStatPoolModificationTypes.Regeneration, emptyModifier);
    };
  }
}
