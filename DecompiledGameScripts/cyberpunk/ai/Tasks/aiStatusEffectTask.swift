...
public class UnconsciousManagerTask extends StatusEffectTasks {
  ...
  protected func SetUnconsciousBodyVisibleComponent(puppet: ref<NPCPuppet>, state: Bool) -> Void {
    ...
    if IsDefined(reactionComponent) {
      reactionComponent.SetDownedBodyVisibleComponent(state, n"Unconscious");
    };
  }
...
public class SystemCollapseManagerTask extends UnconsciousManagerTask {

  protected func Activate(context: ScriptExecutionContext) -> Void {
    let systemCollapseEffects: array<ref<StatusEffect>>;
    let owner: ref<NPCPuppet> = ScriptExecutionContext.GetOwner(context) as NPCPuppet;
    StatusEffectHelper.GetAppliedEffectsWithTag(owner, n"SystemCollapse", systemCollapseEffects);
    if ArraySize(systemCollapseEffects) != 0 {
      this.SetUnconsciousBodyVisibleComponent(owner, true);
      owner.SetMyKiller(GameInstance.FindEntityByID(owner.GetGame(), systemCollapseEffects[0].GetInstigatorEntityID()) as GameObject);
    };
  }
}
...
public class HeartAttackManagerTask extends StatusEffectTasks {
  ...
  protected func SetHeartAttackBodyVisibleComponent(puppet: ref<NPCPuppet>, state: Bool) -> Void {
    let reactionComponent: ref<ReactionManagerComponent> = puppet.GetStimReactionComponent();
    if IsDefined(reactionComponent) {
      reactionComponent.SetDownedBodyVisibleComponent(state, n"HeartAttack");
    };
  }
...
public class DropWeaponTask extends StatusEffectTasks {

  protected func Activate(context: ScriptExecutionContext) -> Void {
    (ScriptExecutionContext.GetOwner(context) as NPCPuppet).DropWeapons();
  }

  protected func Deactivate(context: ScriptExecutionContext) -> Void;
}

public class SetHitStimSourceTask extends StatusEffectTasks {

  protected func Activate(context: ScriptExecutionContext) -> Void {
    let target: wref<GameObject> = (ScriptExecutionContext.GetOwner(context) as NPCPuppet).GetHitReactionComponent().GetHitSource();
    ScriptExecutionContext.SetArgumentVector(context, n"StimSource", target.GetWorldPosition());
  }
...
public class ToggleVisibleObjectComponent extends StatusEffectTasks {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    let reactionComponent: ref<ReactionManagerComponent> = AIBehaviorScriptBase.GetPuppet(context).GetStimReactionComponent();
    if IsDefined(reactionComponent) {
      reactionComponent.SetDownedBodyVisibleComponent(this.m_componentTargetState, this.m_visibleObjectDescription);
    };
  }
}
...
public class CheckFriendlyNPCAboutToBeHit extends StatusEffectTasks {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    if NotEquals(owner.GetAttitudeTowards(player), EAIAttitude.AIA_Friendly) && (NotEquals(owner.GetRecord().Priority().Type(), gamedataSpawnableObjectPriority.Quest) || owner.IsPrevention()) {
    ...
    if IsDefined(owner) {
      ...
      if GameInstance.GetAINavigationSystem(owner.GetGame()).IsPointOnNavmesh(owner, ownerPosition, navmeshTolerance, pointOnNavmesh) {
        ...
        if Vector4.Distance(owner.GetWorldPosition(), lastValidNavmeshPosition) <= TDB.GetFloat(t"AIGeneralSettings.ragdollRecoveryMinDistanceThreshold") {
          ...
          if (Equals(owner.GetNPCRarity(), gamedataNPCRarity.Boss) || Equals(owner.GetNPCRarity(), gamedataNPCRarity.MaxTac) || isFriendlyToPlayer) && !foundPointOnNavmesh {
...
public class AIRagdollDelegate extends ScriptBehaviorDelegate {
  ...
  private final func HasSpaceToRecover(owner: ref<NPCPuppet>, const queryDimensions: script_ref<[Float]>, originTransform: WorldTransform) -> Bool {
    ...
    let queryExtents: Vector4 = new Vector4(Deref(queryDimensions)[0] * 0.50, Deref(queryDimensions)[1] * 0.50, Deref(queryDimensions)[2] * 0.50, Deref(queryDimensions)[3]);
    ...
    if TraceResult.IsValid(queryPositionTrace) {
      queryPosition = Cast<Vector4>(queryPositionTrace.position);
    } else {
  ...
  public final func DoHandleDownedSignals(context: ScriptExecutionContext) -> Bool {
    ...
    if StatusEffectSystem.ObjectHasStatusEffectOfType(puppet, gamedataStatusEffectType.Defeated) && !signalHandler.IsHighestPriority(EnumValueToName(n"gamedataStatusEffectType", 17l), signalID) {
      this.SendDownedSignal(context, puppet, EnumValueToName(n"gamedataStatusEffectType", 17l));
    } else {
      if StatusEffectSystem.ObjectHasStatusEffectOfType(puppet, gamedataStatusEffectType.DefeatedWithRecover) {
        this.SendStatusEffectSignal(context, puppet, EnumValueToName(n"gamedataStatusEffectType", 18l));
      } else {
  ...
  public final func DoHandleRagdollReaction(context: ScriptExecutionContext) -> Bool {
    ...
    if ScriptedPuppet.IsAlive(puppet) {
      if IsDefined(this.ragdollInstigator) && this.ragdollInstigator.GetEntityID() != ScriptExecutionContext.GetOwner(context).GetEntityID() {
