      ...
      if member == AIBehaviorScriptBase.GetPuppet(context) {
        ...
        if NotEquals(member.GetHighLevelStateFromBlackboard(), gamedataNPCHighLevelState.Alerted) {
          ...
          if !ScriptedPuppet.IsActive(member) {
            ...
            if member.GetStimReactionComponent().GetActiveReactionData().stimTarget == stimTarget && smi.HasOrderBySquadAction(EnumValueToName(n"EAISquadAction", Cast<Int64>(EnumInt(this.m_squadActionName))), members[i]) {
