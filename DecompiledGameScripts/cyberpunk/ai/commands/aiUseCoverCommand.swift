...
public class UseCoverCommandTask extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    AIbehaviortaskScript.SetUpdateInterval(context, AIBehaviorScriptBase.RandomizeOffsetForUpdateInterval(1.00));
  }
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    if tmpID != coverID {
      ...
      AICoverHelper.RegisterNewCover(AIBehaviorScriptBase.GetPuppet(context), coverID);
    };
...
public class UseCoverCommandCleanup extends AIbehaviortaskScript {

  protected inline edit let m_inCommand: ref<AIArgumentMapping>;

  private final func Deactivate(context: ScriptExecutionContext) -> Void {
    let coverID: Uint64 = GameInstance.GetCoverManager(AIBehaviorScriptBase.GetPuppet(context).GetGame()).GetCurrentCover(AIBehaviorScriptBase.GetPuppet(context));
    if coverID == 0u {
      AICoverHelper.UnregisterLastCover(AIBehaviorScriptBase.GetPuppet(context));
    };
    ScriptExecutionContext.SetMappingValue(context, this.m_inCommand, ToVariant(null));
  }
}
