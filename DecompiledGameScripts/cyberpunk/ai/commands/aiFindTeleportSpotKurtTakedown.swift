...
public class FindTeleportPositionKurt extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    let kerenzikovPsmEvent: ref<PSMPostponedParameterBool>;
    let sandevistanPsmEvent: ref<PSMPostponedParameterBool>;
    let player: ref<PlayerPuppet> = GameInstance.GetPlayerSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetLocalPlayerControlledGameObject() as PlayerPuppet;
    if IsDefined(player) {
      sandevistanPsmEvent = new PSMPostponedParameterBool();
      sandevistanPsmEvent.id = n"requestSandevistanDeactivation";
      sandevistanPsmEvent.value = true;
      player.QueueEvent(sandevistanPsmEvent);
      kerenzikovPsmEvent = new PSMPostponedParameterBool();
      kerenzikovPsmEvent.id = n"requestKerenzikovDeactivation";
      kerenzikovPsmEvent.value = true;
      player.QueueEvent(kerenzikovPsmEvent);
    };
  }
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    if !this.CheckForOverlapAdvanced(context, extentsOffset, queryExtents, teleportPosition, targetOrientation) {
  ...
  private final const func GetNavmeshPointWithOffset(const origin: wref<Entity>, out point: Vector4, const rotation: Quaternion, opt offset: Vector4) -> Bool {
    ...
    checkedResults = navigationSystem.FindPointBelowForCharacter(checkedPosition, 0.50, 5, origin, checkedZOffset);
    if NotEquals(checkedResults.status, worldNavigationRequestStatus.OK) {
      return false;
    };
    checkedPosition.Z += checkedZOffset;
  ...
  private final func CheckForOverlapAdvanced(context: ScriptExecutionContext, queryOffset: Vector4, queryExtents: Vector4, queryPosition: Vector4, out queryOrientation: Quaternion) -> Bool {
    let isHit: Bool;
    ...
    if !this.CheckForOverlap(context, queryPosition, queryOffset, queryExtents, queryOrientation, queryFilter) {
      return false;
    };
    ...
    if !this.CheckForOverlap(context, queryPosition, queryOffset, queryExtents, perpOrientation, queryFilter) {
  ...
  private final func CheckForOverlap(context: ScriptExecutionContext, targetPosition: Vector4, targetOffset: Vector4, queryDimensions: Vector4, queryOrientation: Quaternion, queryFilter: QueryFilter) -> Bool {
    let groundPosition: Vector4;
    let queryResult: TraceResult;
    ...
    let queryPosition: Vector4 = targetPosition + queryOrientation * targetOffset + new Vector4(0.00, 0.00, 0.10 + queryExtents.Z, 0.00);
    ...
    groundPosition = targetPosition + queryOrientation * targetOffset;
    if !this.CheckForGround(context, groundPosition) {
      return true;
    };
    return spatialQueriesSystem.OverlapByQueryFilter(queryExtents, queryPosition, queryOrientation, queryFilter, queryResult);
  }

  private final func CheckForGround(context: ScriptExecutionContext, targetPosition: Vector4) -> Bool {
    let queryFilter: QueryFilter;
    let queryResult: TraceResult;
    let spatialQueriesSystem: ref<SpatialQueriesSystem> = GameInstance.GetSpatialQueriesSystem(ScriptExecutionContext.GetOwner(context).GetGame());
    let debugVisualizerSystem: ref<DebugVisualizerSystem> = GameInstance.GetDebugVisualizerSystem(ScriptExecutionContext.GetOwner(context).GetGame());
    ...
    if debugPreviewDuration > 0.00 {
      debugVisualizerSystem.DrawWireSphere(targetPosition, 0.25, new Color(44u, 128u, 44u, 255u), debugPreviewDuration);
    };
    QueryFilter.AddGroup(queryFilter, n"Static");
    QueryFilter.AddGroup(queryFilter, n"Dynamic");
    QueryFilter.AddGroup(queryFilter, n"Terrain");
    QueryFilter.AddGroup(queryFilter, n"Collider");
    QueryFilter.AddGroup(queryFilter, n"Destructible");
    return spatialQueriesSystem.SyncRaycastByQueryFilter(targetPosition, targetPosition - new Vector4(0.00, 0.00, 1.00, 0.00), queryFilter, queryResult);
  }

  private final func CheckForStairs(context: ScriptExecutionContext, targetPosition: Vector4) -> Bool {
    let debugPreviewDuration: Float;
    ...
    if debugPreviewDuration > 0.00 {
      debugVisualizerSystem.DrawText3D(targetPosition, FloatToStringPrec(dotFacingTop, 3), new Color(128u, 44u, 44u, 255u), debugPreviewDuration);
    };
