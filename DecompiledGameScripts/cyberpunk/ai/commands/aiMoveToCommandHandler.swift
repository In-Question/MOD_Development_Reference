...
public class AIMoveToCommandHandler extends AICommandHandlerBase {
  ...
  protected func UpdateCommand(context: ScriptExecutionContext, command: ref<AICommand>) -> AIbehaviorUpdateOutcome {
    ...
    if Equals((ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetNPCType(), gamedataNPCType.Drone) {
      ScriptExecutionContext.SetEnumMappingValue(context, this.m_outMovementType, 0l);
      ...
      if Equals(typedCommand.movementType, moveMovementType.Run) {
        currentHighLevelState = IntEnum<gamedataNPCHighLevelState>((ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetPuppetStateBlackboard().GetInt(GetAllBlackboardDefs().PuppetState.HighLevel));
        if Equals(currentHighLevelState, gamedataNPCHighLevelState.Combat) && !ScriptExecutionContext.SetEnumMappingValue(context, this.m_outMovementType, 2l) {
          return AIbehaviorUpdateOutcome.FAILURE;
        };
...
public class AIMoveOnSplineCommandHandler extends AICommandHandlerBase {
  ...
  protected inline edit let m_allowCrowdOnPath: ref<AIArgumentMapping>;
  ...
  protected func UpdateCommand(context: ScriptExecutionContext, command: ref<AICommand>) -> AIbehaviorUpdateOutcome {
    ...
    ScriptExecutionContext.SetMappingValue(context, this.m_allowCrowdOnPath, ToVariant(typedCommand.allowCrowdOnPath));
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
}
...
public class MoveCommandCleanup extends AIbehaviortaskScript {
...
public class AIMoveCommandsDelegate extends ScriptBehaviorDelegate {
  ...
  private let splineRecalculation: Bool;

  private let disableFootIK: Bool;
  ...
  private let allowCrowdOnPath: Bool;
  ...
  public final func DoStartMoveOnSpline(context: ScriptExecutionContext) -> Bool {
    ...
    if Equals(puppetOwner.GetNPCType(), gamedataNPCType.Drone) {
      ...
      if Equals(cmd.movementType.movementType, moveMovementType.Run) {
        currentHighLevelState = IntEnum<gamedataNPCHighLevelState>(puppetOwner.GetPuppetStateBlackboard().GetInt(GetAllBlackboardDefs().PuppetState.HighLevel));
        if Equals(currentHighLevelState, gamedataNPCHighLevelState.Combat) {
    ...
    this.splineRecalculation = cmd.splineRecalculation;
    this.disableFootIK = cmd.disableFootIK;
    ...
    this.allowCrowdOnPath = cmd.allowCrowdOnPath;
  ...
  public final func GetIsMoveToSplineNeeded(context: ScriptExecutionContext) -> Bool {
    if AIScriptUtils.IsSplineStartRecalculated(context, this.spline, this.splineRecalculation) {
      return false;
    };
  ...
  public final func GetSprintSpeedDistance(context: ScriptExecutionContext) -> Float {
    ...
    let result: Float;
    let tolerance: Float;
    if EnumInt(this.movementType) == 2 {
      return -99999.00;
    };
    if this.shouldBeInFrontOfCompanion {
  ...
  public final func GetRunSpeedDistance(context: ScriptExecutionContext) -> Float {
    ...
    let result: Float;
    let tolerance: Float;
    if EnumInt(this.movementType) == 1 {
      return -99999.00;
    };
    if this.shouldBeInFrontOfCompanion {
...
public class IgnoreMoveCommandInCombatCondition extends AIbehaviorconditionScript {
