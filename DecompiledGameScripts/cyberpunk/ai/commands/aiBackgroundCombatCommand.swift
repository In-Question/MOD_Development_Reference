...
public class AIBackgroundCombatCommandParams extends ScriptedAICommandParams {
  ...
  public inline edit const let m_steps: [AIBackgroundCombatStep];
...
public class AIAnimationTask extends AIbehaviortaskScript {
  ...
  private final func StartPhase(context: ScriptExecutionContext, newPhase: EAIActionPhase) -> Void {
    ...
    this.m_phaseActivationTime = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context));
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    if IsDefined(this.m_animVariation) {
      this.m_animVariationValue = FromVariant<Int32>(ScriptExecutionContext.GetMappingValue(context, this.m_animVariation));
    };
  ...
  protected func Update(context: ScriptExecutionContext) -> AIbehaviorUpdateOutcome {
    ...
    currentPhaseDuration = EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)) - this.m_phaseActivationTime;
...
public class AIBackgroundCombatDelegate extends ScriptBehaviorDelegate {
  ...
  private let m_steps: [AIBackgroundCombatStep];
  ...
  public final func DoCompleteTargetChange(context: ScriptExecutionContext) -> Bool {
    ...
    if this.m_hasDesiredTarget {
      ...
      if !GetGameObjectFromEntityReference(this.m_desiredTarget, game, this.m_currentTarget) {
        globalRef = ResolveNodeRef(this.m_desiredTarget.reference, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot()));
        this.m_currentTarget = GameInstance.FindEntityByID(game, Cast<EntityID>(globalRef)) as GameObject;
      };
      ...
      AIActionHelper.SetCommandCombatTarget(context, this.m_currentTarget, true, Cast<Uint32>(threatPersistenceSource.EnumValue()));
    };
    return true;
  }
}
