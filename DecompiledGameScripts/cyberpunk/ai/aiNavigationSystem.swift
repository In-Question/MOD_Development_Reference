...
public native class AINavigationSystem extends AIINavigationSystem {
  ...
  public final const func FindPointBelowForCharacter(origin: Vector4, querySphereRadius: Float, numberOfSpheres: Int32, const character: wref<Entity>, out resultingZOffset: Float) -> NavigationFindPointResult {
    let pointResults: NavigationFindPointResult;
    let i: Int32 = 0;
    ...
    resultingZOffset = -1.00 * Cast<Float>(i) * querySphereRadius;
    return pointResults;
  }

  public final const func FindPointBelowForCharacter(origin: Vector4, querySphereRadius: Float, numberOfSpheres: Int32, const character: wref<Entity>) -> NavigationFindPointResult {
    let resultingZOffset: Float;
    return this.FindPointBelowForCharacter(origin, querySphereRadius, numberOfSpheres, character, resultingZOffset);
  }
  ...
  public final const func GetNearestNavmeshPointBelow(const character: wref<Entity>, origin: Vector4, querySphereRadius: Float, numberOfSpheres: Int32) -> Vector4 {
    let point: Vector4;
    let pointResults: NavigationFindPointResult = this.FindPointBelowForCharacter(origin, querySphereRadius, numberOfSpheres, character);
    if Equals(pointResults.status, worldNavigationRequestStatus.OK) {
  ...
  public final const func GetNearestNavmeshPointBehind(const origin: wref<Entity>, const querySphereRadius: Float, const numberOfSpheres: Int32, out point: Vector4, opt checkPathToOrigin: Bool) -> Bool {
    ...
    i = 1;
    while i <= numberOfSpheres {
      center = originPosition - originHeading * querySphereRadius * Cast<Float>(i);
      ...
      if NotEquals(pointResults.status, worldNavigationRequestStatus.OK) {
        ...
        if checkPathToOrigin {
          ...
          if ArraySize(navigationPath.path) <= 0 {
            ...
            if NotEquals(ratioCurveName, n"None") && IsDefined(statsDataSystem) {
  ...
  public final const func GetFurthestNavmeshPointInFront(const origin: wref<Entity>, const querySphereRadius: Float, const numberOfSpheres: Int32, out point: Vector4, const opt offsetFromOrigin: Vector4, opt checkPathToOrigin: Bool, opt ratioCurveName: CName) -> Bool {
    let center: Vector4;
    let directLength: Float;
    let i: Int32;
    let navigationPath: ref<NavigationPath>;
  ...
  public final const func TryToFindNavmeshPointAroundPoint(const owner: wref<Entity>, const originPosition: Vector4, const originOrientation: Quaternion, const probeDimensions: Vector4, const numberOfSpheres: Int32, const sphereDistanceFromOrigin: Float, out point: Vector4, opt checkPathToOrigin: Bool) -> Bool {
    ...
    while i > 0 {
      ...
      currentAngle += Cast<Float>(360 / numberOfSpheres);
      if NotEquals(pointResults.status, worldNavigationRequestStatus.OK) {
      } else {
        point = pointResults.point;
        if checkPathToOrigin {
...
public class FindNavmeshPointAroundThePlayer extends AIbehaviortaskScript {
  ...
  protected func Activate(context: ScriptExecutionContext) -> Void {
    ...
    if Equals(owner.GetNPCType(), gamedataNPCType.Cerberus) {
      tolerance = new Vector4(3.00, 3.00, 5.00, 1.00);
    };
