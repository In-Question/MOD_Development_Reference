...
public class AIFollowerRole extends AIRole {
  ...
  @runtimeProperty("customEditor", "scnbPerformerSelector")
  ...
  private persistent let m_followTargetSquads: [CName];
  ...
  public final func OnFriendlyTargetWeaponChange(itemID: ItemID) -> Void {
    ...
    this.m_isFriendMelee = itemRecord.TagsContains(n"MeleeWeapon");
  ...
  public func OnRoleSet(owner: wref<GameObject>) -> Void {
    ...
    currentHighLevelState = IntEnum<gamedataNPCHighLevelState>((owner as ScriptedPuppet).GetPuppetStateBlackboard().GetInt(GetAllBlackboardDefs().PuppetState.HighLevel));
  ...
  public func OnRoleCleared(owner: wref<GameObject>) -> Void {
    ...
    (this.m_owner as NPCPuppet).GetAIControllerComponent().SetCombatSpaceSizeMultiplier(1.00);
    StatusEffectHelper.RemoveStatusEffectsWithTag(this.m_owner, n"FollowerDefeated");
    StatusEffectHelper.RemoveStatusEffect(this.m_owner, t"BaseStatusEffect.FollowerDamage");
