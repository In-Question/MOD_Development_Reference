...
public class DEBUG_ItemRebalancer extends GameObject {
  ...
  private final func RebalanceItem() -> Void {
    ...
    let playerLevel: Float = statSystem.GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.Level);
    evt.reqLevel = playerLevel;
    let entityID: EntityID = Cast<EntityID>(ResolveNodeRefWithEntityID(this.m_nodeRef, this.GetEntityID()));
