...
public class DebugInteractionObject extends GameObject {
  ...
  private const let m_choices: [SDebugChoice];
  ...
  private final func InitializeChoices() -> Void {
    ...
    while i < ArraySize(this.m_choices) {
      if Equals(this.m_choices[i].choiceName, n"None") {
      } else {
  ...
  private final func CreateChoice(const choiceName: script_ref<String>, data: Int32) -> InteractionChoice {
    let choice: InteractionChoice;
    choice.choiceMetaData.tweakDBName = Deref(choiceName);
    choice.caption = choice.choiceMetaData.tweakDBName;
    ArrayPush(choice.data, ToVariant(data));
    return choice;
  }

  private final func ResolveFact(const factName: script_ref<String>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_choices) {
      if Equals(factName, "") {
      } else {
