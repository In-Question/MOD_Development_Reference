...
public class DEBUG_VisualizerComponent extends ScriptableComponent {
  ...
  private let records: [DEBUG_VisualRecord];
  ...
  public final func VisualizePuppets(const pups: script_ref<[ref<ScriptedPuppet>]>, infDuration: Bool, duration: Float) -> Void {
    let alreadyExists: Bool;
    let i: Int32;
    let j: Int32;
    let rec: DEBUG_VisualRecord;
    if ArraySize(Deref(pups)) == 0 || !infDuration && duration <= 0.00 {
      return;
    };
    i = 0;
    while i < ArraySize(Deref(pups)) {
      alreadyExists = false;
      j = 0;
      while j < ArraySize(this.records) {
        if this.records[j].puppet == Deref(pups)[i] {
  ...
  private final func GetNextOffset() -> Vector4 {
    let offset: Vector4 = new Vector4(0.00, 0.00, this.TEXT_TOP - this.TEXT_OFFSET * Cast<Float>(this.offsetCounter), 0.00);
  ...
  private final func ShowText(offset: Vector4, const str: script_ref<String>, color: Color, scale: Float) -> Void {
    ...
    let layerID: Uint32 = dvs.DrawText3D(this.records[this.processedRecordIndex].puppet.GetWorldPosition() + offset, Deref(str), color, MaxF(this.TICK_TIME_DELTA, 0.05));
