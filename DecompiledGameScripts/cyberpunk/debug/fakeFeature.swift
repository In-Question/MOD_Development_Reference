...
public class FakeFeature extends GameObject {
  ...
  private const let m_choices: [SFakeFeatureChoice];

  protected let m_interaction: ref<InteractionComponent>;

  private let m_components: [ref<IPlacedComponent>];
  ...
  protected cb func OnInteractionChoice(choiceEvent: ref<InteractionChoiceEvent>) -> Bool {
    let choiceID: Int32 = FromVariant<Int32>(choiceEvent.choice.data[0]);
  ...
  private final func CreateChoice(const choiceID: script_ref<String>, data: Int32) -> InteractionChoice {
    let choice: InteractionChoice;
    choice.choiceMetaData.tweakDBName = Deref(choiceID);
