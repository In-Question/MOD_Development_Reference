...
public class CommunityProxyPS extends MasterControllerPS {
  ...
  protected final func InitializeConnectionWithCommunity() -> Void {
    ...
    GameInstance.GetEntitySpawnerEventsBroadcaster(this.GetGameInstance()).RegisterSpawnerEventPSListener(this.GetMyEntityID(), n"None", this.GetID(), this.GetClassName());
    i = 0;
  ...
  public final const func ExtractEntityIDs() -> [EntityID] {
  ...
  private final const func ForwardActionToNPCs(action: ref<DeviceAction>) -> Void {
    ...
    while i < ArraySize(communityNPCs) {
      this.GetPersistencySystem().QueuePSEvent(Cast<PersistentID>(communityNPCs[i]), n"ScriptedPuppetPS", action);
      i += 1;
    };
  }
