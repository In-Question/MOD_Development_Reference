...
public class TakedownUtils extends IScriptable {
  ...
  public final static func SetIgnoreLookAtEntity(scriptInterface: ref<StateGameScriptInterface>, targetID: EntityID, b: Bool) -> Void {
    let targetingSystem: ref<TargetingSystem> = scriptInterface.GetTargetingSystem();
    if Equals(b, true) {
  ...
  public final static func SetTakedownAction(stateContext: ref<StateContext>, actionName: ETakedownActionType) -> Void {
    let enumName: CName = EnumValueToName(n"ETakedownActionType", Cast<Int64>(EnumInt(actionName)));
  ...
  public final static func CleanUpGrappleState(caller: ref<DefaultTransition>, stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>, target: wref<GameObject>) -> Void {
    ...
    TakedownUtils.SetTakedownAction(stateContext, TakedownUtils.TakedownActionNameToEnum(n"None"));
  ...
  public final static func CleanUpGrappleState(caller: ref<DefaultTransition>, stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>, targetID: EntityID) -> Void {
    let broadcaster: ref<StimBroadcasterComponent>;
    TakedownUtils.SetInGrappleAnimFeature(scriptInterface, false);
    StatusEffectHelper.RemoveStatusEffect(scriptInterface.executionOwner, t"BaseStatusEffect.HumanShield");
    StatusEffectHelper.RemoveStatusEffect(scriptInterface.executionOwner, t"GameplayRestriction.NoRadialMenus");
    TakedownUtils.SetIgnoreLookAtEntity(scriptInterface, targetID, false);
    caller.SetGameplayCameraParameters(scriptInterface, "cameraDefault");
    TakedownUtils.SetTakedownAction(stateContext, TakedownUtils.TakedownActionNameToEnum(n"None"));
    broadcaster = scriptInterface.executionOwner.GetStimBroadcasterComponent();
    if IsDefined(broadcaster) {
  ...
  public final static func ShouldForceTakedown(scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    let tier: Int32;
    if scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Zones) == 2 {
      return true;
    };
...
public class LocomotionTakedownDecisions extends LocomotionTransition {
  ...
  protected final const func IsPowerLevelDifferentialTooHigh(target: wref<GameObject>) -> Bool {
    ...
    if Equals(powDifference, gameEPowerDifferential.IMPOSSIBLE) {
      return true;
    };
    return false;
  }
...
public class LocomotionTakedownEvents extends LocomotionEventsTransition {
  ...
  protected final func SetGrappleDuration(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>, grappleDuration: Float, target: ref<GameObject>) -> Void {
    ...
    let targetRarity: Float = statSystem.GetStatValue(Cast<StatsObjectID>(target.GetEntityID()), gamedataStatType.NPCRarity);
    let grappleTime: Float = grappleDuration - targetRarity * 1.50;
  ...
  protected final func SelectAerialTakedownWorkspot(scriptInterface: ref<StateGameScriptInterface>, owner: ref<GameObject>, target: ref<GameObject>, opt back: Bool, opt front: Bool, opt left: Bool, opt right: Bool, action: CName) -> CName {
    let takedown: String;
    ...
    if IsNameValid(takedownName) {
      ...
      if Equals(weaponType, gamedataItemType.Cyb_MantisBlades) {
        takedown = NameToString(takedownName);
        takedown += "_MantisBlades";
        takedownName = StringToName(takedown);
        result = takedownName;
      } else {
        if !GameInstance.GetAINavigationSystem(scriptInterface.GetGame()).HasPathForward(this.stateMachineInitData.target, 1.00) || !SpatialQueriesHelper.HasSpaceInFront(scriptInterface.executionOwner, 0.35, 0.90, this.GetStaticFloatParameterDefault("minRequiredSpace", 0.10), 1.25) {
          takedown = NameToString(takedownName);
          takedown += "_Simple";
          takedownName = StringToName(takedown);
          result = takedownName;
...
public class TakedownBeginEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    let vaultInfo: ref<PlayerClimbInfo> = scriptInterface.GetSpatialQueriesSystem().GetPlayerObstacleSystem().GetCurrentClimbInfo(scriptInterface.owner);
    stateContext.SetPermanentBoolParameter(n"PlayerFacingVaultableWall", vaultInfo.vaultValid, true);
    if vaultInfo.vaultValid {
      stateContext.SetPermanentVectorParameter(n"VaultableWallEndPoint", vaultInfo.obstacleEnd, true);
    };
    targetPuppet = this.stateMachineInitData.target as gamePuppet;
    ...
    this.SetBlackboardIntVariable(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.Takedown, 1);
    this.SetBlackboardEntityID(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.TakedownTargetID, targetPuppet.GetEntityID());
    ...
    (targetPuppet as ScriptedPuppet).GetBumpComponent().Toggle(false);
...
public class TakedownLeapToPreyEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetBlackboardIntVariable(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.Takedown, 3);
  }
...
public class TakedownGrapplePreyEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    let distanceToObstacle: Float;
    let distanceToTarget: Float;
    let obstaclePosition: StateResultVector;
    let playerPosition: Vector4;
    let targetPosition: Vector4;
    this.m_isGrappleReactionVOPlayed = false;
    let grappleOverWall: StateResultBool = stateContext.GetPermanentBoolParameter(n"PlayerFacingVaultableWall");
    if grappleOverWall.value {
      obstaclePosition = stateContext.GetPermanentVectorParameter(n"VaultableWallEndPoint");
      playerPosition = scriptInterface.executionOwner.GetWorldPosition();
      targetPosition = this.stateMachineInitData.target.GetWorldPosition();
    ...
    if grappleOverWall.value && distanceToTarget > distanceToObstacle {
      TakedownGameEffectHelper.FillTakedownData(scriptInterface.executionOwner, scriptInterface.executionOwner, this.stateMachineInitData.target, n"takedowns", n"GrappleTargetCover", "");
    } else {
      if Equals(this.stateMachineInitData.actionName, n"GrappleTarget") && (stateContext.GetConditionBool(n"CrouchToggled") || scriptInterface.GetActionValue(n"Crouch") > 0.00) {
        TakedownGameEffectHelper.FillTakedownData(scriptInterface.executionOwner, scriptInterface.executionOwner, this.stateMachineInitData.target, n"takedowns", n"GrappleTargetPlayerCrouch", "");
      } else {
...
public class GrappleStandDecisions extends LocomotionTakedownDecisions {
  ...
  protected final const func ToGrappleBreakFree(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    if this.IsPowerLevelDifferentialTooHigh(this.stateMachineInitData.target) || NPCManager.HasTag((this.stateMachineInitData.target as NPCPuppet).GetRecordID(), n"MiniBoss") || NPCManager.HasTag((this.stateMachineInitData.target as NPCPuppet).GetRecordID(), n"q305_maxtac") || NPCManager.HasTag((this.stateMachineInitData.target as NPCPuppet).GetRecordID(), n"MaxTac_Prevention") {
      return this.IsBreakingFreeAllowed(stateContext, scriptInterface);
    };
    return false;
  }

  protected final const func ToTakedownUnmountPrey(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    if scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Vitals) == 1 {
      return true;
    };
    return false;
  }
...
public class GrappleStandEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetGrappleDuration(stateContext, scriptInterface, this.GetStaticFloatParameterDefault("stateDuration", 11.50), this.stateMachineInitData.target);
    this.SetLocomotionParameters(stateContext, scriptInterface);
    this.SetBlackboardIntVariable(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.Takedown, 2);
...
public class GrappleForceShovePreyDecisions extends GrappleStandDecisions {
  ...
  protected final const func EnterCondition(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    let scriptedPuppet: ref<ScriptedPuppet> = this.stateMachineInitData.target as ScriptedPuppet;
    if scriptedPuppet.IsUnderwater() && !GameInstance.GetAINavigationSystem(scriptedPuppet.GetGame()).IsPointOnNavmesh(scriptedPuppet, scriptedPuppet.GetWorldPosition(), 2.50) {
      return true;
    };
...
public class GrappleForceShovePreyEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetBlackboardIntVariable(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.Takedown, 4);
...
public class GrappleBreakFreeEvents extends GrappleStandEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetLocomotionParameters(stateContext, scriptInterface);
    this.ForceTemporaryWeaponUnequip(stateContext, scriptInterface, true);
    AIActionHelper.ClearWorkspotCommand(this.stateMachineInitData.target as NPCPuppet, true);
    StatusEffectHelper.RemoveStatusEffect(this.stateMachineInitData.target, t"BaseStatusEffect.Grappled");
    TargetTrackingExtension.InjectThreat(this.stateMachineInitData.target as NPCPuppet, scriptInterface.executionOwner);
    StatusEffectHelper.RemoveStatusEffect(this.stateMachineInitData.target, t"BaseStatusEffect.ForceVisibility");
    if Equals(this.GetTakedownAction(stateContext), ETakedownActionType.GrappleFailed) {
...
public class TakedownExecuteTakedownAndDisposeEvents extends LocomotionTakedownEvents {
  ...
  public let m_targetID: EntityID;

  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    this.m_targetID = this.stateMachineInitData.target.GetEntityID();
  ...
  public func OnExit(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    if IsDefined(this.stateMachineInitData.target) {
      ...
      TakedownUtils.CleanUpGrappleState(this, stateContext, scriptInterface, this.m_targetID);
    };
  }
}
...
public class TakedownReleasePreyDecisions extends LocomotionTakedownDecisions {
  ...
  protected final const func EnterCondition(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    if this.GetInStateTime() > 0.50 && !DefaultTransition.IsInWorkspot(scriptInterface) {
      return true;
    };
...
public class TakedownUnmountPreyEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetLocomotionParameters(stateContext, scriptInterface);
    if scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Vitals) == 1 {
      StatusEffectHelper.ApplyStatusEffect(this.stateMachineInitData.target, t"BaseStatusEffect.Unconscious");
      scriptInterface.GetWorkspotSystem().SendFastExitSignal(this.stateMachineInitData.target);
    };
  }
...
public class PickUpBodyAfterTakedownEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    this.SetLocomotionParameters(stateContext, scriptInterface);
    this.SetBlackboardIntVariable(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.Takedown, 0);
    this.SetBlackboardEntityID(scriptInterface, GetAllBlackboardDefs().PlayerStateMachine.TakedownTargetID, emptyEntityID);
  }
...
public class TakedownEndEvents extends LocomotionTakedownEvents {
  ...
  public func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    (this.stateMachineInitData.target as ScriptedPuppet).GetBumpComponent().Toggle(true);
  }
