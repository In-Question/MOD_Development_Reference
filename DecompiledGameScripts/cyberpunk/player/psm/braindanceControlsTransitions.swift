...
public abstract class BraindanceControlsTransition extends DefaultTransition {
  ...
  protected final const func GetCachedPlaySpeedPermVariable(const stateContext: ref<StateContext>) -> scnPlaySpeed {
    let result: StateResultInt = stateContext.GetPermanentIntParameter(n"cachedPlaySpeed");
    return IntEnum<scnPlaySpeed>(result.value);
  }
  ...
  protected final const func GetCurrentBraindanceVisionMode(const scriptInterface: ref<StateGameScriptInterface>) -> braindanceVisionMode {
    ...
    return IntEnum<braindanceVisionMode>(mode);
  }
  ...
  protected final const func CycleBraindanceVisionMode(const scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    let i: Int32 = 0;
    while Cast<Int64>(i) < EnumGetMax(n"braindanceVisionMode") {
      cycleMode -= 1;
      if cycleMode < 0 {
        cycleMode = Cast<Int32>(EnumGetMax(n"braindanceVisionMode"));
      };
      if this.CanBraindanceEnterLayer(scriptInterface, IntEnum<braindanceVisionMode>(cycleMode)) {
        this.SetBraindanceVisionMode(scriptInterface, IntEnum<braindanceVisionMode>(cycleMode));
        break;
      };
      i += 1;
    };
  ...
  protected final const func CyclePlaySpeed(const scriptInterface: ref<StateGameScriptInterface>, direction: scnPlayDirection) -> Void {
    ...
    if Equals(currentPlaySpeed, scnPlaySpeed.Pause) && NotEquals(direction, currentPlayDirection) {
      newPlayDirection = IntEnum<scnPlayDirection>((EnumInt(currentPlayDirection) + 1) % 2);
    ...
    newPlaySpeed = IntEnum<scnPlaySpeed>(Clamp(EnumInt(currentPlaySpeed) + multiplier, 0, Cast<Int32>(EnumGetMax(n"scnPlaySpeed"))));
  ...
  protected final const func CheckTargetThirdPersonPositionCollisions(const scriptInterface: ref<StateGameScriptInterface>, fppPosition: Vector4, fppOrientationEuler: EulerAngles, angle: Float, distance: Float, radius: Float, out outPosition: Vector4) -> Float {
    ...
    foundCollision = scriptInterface.RaycastWithASingleGroup(fppPosition, targetPosition, n"PlayerBlocker");
    if TraceResult.IsValid(foundCollision) {
      collisionDistance = Vector4.Length(fppPosition - Cast<Vector4>(foundCollision.position)) - radius;
      outPosition = Cast<Vector4>(foundCollision.position) - raycastDirection * radius;
    };
  ...
  protected final const func OnBraindancePerspectiveChangedFromFirstPersonToThirdPerson(const scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    let player: ref<PlayerPuppet> = scriptInterface.GetPlayerSystem().GetLocalPlayerControlledGameObject() as PlayerPuppet;
    ...
    targetDistBack = this.CheckTargetThirdPersonPositionCollisions(scriptInterface, fppPosition, fppOrientationEuler, 0.00, distanceOffset, collisionRadius, targetPosBack);
    targetPosition = targetPosBack;
    targetDist = targetDistBack;
    if targetDist < distanceOffset {
      targetDistLeft = this.CheckTargetThirdPersonPositionCollisions(scriptInterface, fppPosition, fppOrientationEuler, -90.00, distanceOffset, collisionRadius, targetPosLeft);
      if targetDistLeft > 0.00 && targetDistLeft > targetDist {
        targetPosition = targetPosLeft;
        targetDist = targetDistLeft;
      };
      targetDistRight = this.CheckTargetThirdPersonPositionCollisions(scriptInterface, fppPosition, fppOrientationEuler, 90.00, distanceOffset, collisionRadius, targetPosRight);
    ...
    foundGround = scriptInterface.RaycastWithASingleGroup(groundRaycastBeginPoint, groundRaycastEndPoint, n"PlayerBlocker");
    if TraceResult.IsValid(foundGround) {
      targetPosition = Cast<Vector4>(foundGround.position);
...
public class ControlsActiveEvents extends BraindanceControlsTransition {
  ...
  protected final func OnUpdate(timeDelta: Float, stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    fadeInDelayFinished = this.GetInStateTime() > 1.00;
    ...
    if (this.forwardInput || this.backwardInput) && fadeInDelayFinished {
      if !this.cacheSet {
    ...
    if this.GetBraindancePauseInput(scriptInterface) && inputMask.pauseAction && fadeInDelayFinished {
    ...
    if this.GetSwitchLayerInput(scriptInterface) && !this.GetSceneSystemInterface(scriptInterface).GetSceneSystemCameraControlEnabled() && inputMask.switchLayerAction && fadeInDelayFinished {
    ...
    if this.GetBdCameraToggleInput(scriptInterface) && fadeInDelayFinished && !this.IsPerspectiveTransitionOn(this.BlockPerspectiveSwitchTimer) && inputMask.cameraToggleAction {
      this.ToggleCameraControlEnabled(scriptInterface, stateContext, this.BlockPerspectiveSwitchTimer);
      this.StopGlitchFx(scriptInterface);
      this.rewindFxActive = false;
    };
    if this.GetRestartInput(scriptInterface) && inputMask.restartAction && fadeInDelayFinished {
  ...
  protected final func OnExit(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    if this.endRecordingMessageSet {
      this.SetEndRecordingNotificationState(false, scriptInterface);
      this.endRecordingMessageSet = false;
    };
  }
