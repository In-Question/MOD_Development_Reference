...
public abstract class LookAtPresetBaseDecisions extends DefaultTransition {
  ...
  public final const func HasItemEquipped(const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    let desiredItemType: gamedataItemType = IntEnum<gamedataItemType>(Cast<Int32>(EnumValueFromString("gamedataItemType", desiredTypeStr)));
...
public abstract class LookAtPresetBaseEvents extends DefaultTransition {
  ...
  public let m_lookAtEvents: [ref<LookAtAddEvent>];

  public let m_attachLeft: Bool;

  public let m_attachRight: Bool;

  public final static func GetLookatPartsRequests(lookatPresetRecord: wref<LookAtPreset_Record>, lookAtParts: script_ref<[LookAtPartRequest]>) -> Void {
    let i: Int32;
    ...
    while i < ArraySize(partRecords) {
      ...
      ArrayPush(Deref(lookAtParts), lookAtPartRequest);
      i += 1;
    };
  }

  public final static func AddLookat(scriptInterface: ref<StateGameScriptInterface>, recordID: TweakDBID, priority: Int32, lookAtEventsArray: script_ref<[ref<LookAtAddEvent>]>, out attachLeft: Bool, out attachRight: Bool) -> Void {
    ...
    ArrayPush(Deref(lookAtEventsArray), lookAtEvent);
  ...
  public final func AddAllLookAtsInList(scriptInterface: ref<StateGameScriptInterface>, const presetNames: script_ref<[String]>, priority: Int32, lookAtEventsArray: script_ref<[ref<LookAtAddEvent>]>) -> Void {
    let attachLeftReturn: Bool;
    let attachRightReturn: Bool;
    this.m_attachLeft = false;
    this.m_attachRight = false;
    let i: Int32 = 0;
    while i < ArraySize(Deref(presetNames)) {
      LookAtPresetBaseEvents.AddLookat(scriptInterface, TDBID.Create("LookatPreset." + Deref(presetNames)[i]), priority, lookAtEventsArray, attachLeftReturn, attachRightReturn);
  ...
  public final static func RemoveAddedLookAts(scriptInterface: ref<StateGameScriptInterface>, lookAtEventsArray: script_ref<[ref<LookAtAddEvent>]>) -> Void {
    let lookAtEvent: ref<LookAtAddEvent>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(lookAtEventsArray)) {
      lookAtEvent = Deref(lookAtEventsArray)[i];
    ...
    ArrayClear(Deref(lookAtEventsArray));
  }
...
public class lookAtPresetGunBaseEvents extends LookAtPresetBaseEvents {
  ...
  public let m_overrideLookAtEvents: [ref<LookAtAddEvent>];
