...
public abstract class EquipmentBaseTransition extends DefaultTransition {
  ...
  protected final const func IsVisualItemInSlot(const scriptInterface: ref<StateGameScriptInterface>, stateMachineInstanceData: StateMachineInstanceData) -> Bool {
    ...
    let itemObject: wref<ItemObject> = scriptInterface.GetTransactionSystem().GetItemInSlot(scriptInterface.executionOwner, TDBID.Create(mappedInstanceData.attachmentSlot));
  ...
  protected final const func GetIsPSMInValidState(const scriptInterface: ref<StateGameScriptInterface>, const stateContext: ref<StateContext>, stateMachineInstanceData: StateMachineInstanceData) -> Bool {
    ...
    let item: ref<ItemObject> = this.GetSlotAttachedItem(scriptInterface, stateMachineInstanceData);
    if !IsDefined(item) || NotEquals(this.GetItemCategoryFromItemID(item.GetItemID()), gamedataItemCategory.Weapon) {
      return true;
    };
    weapon = item as WeaponObject;
    if !weapon.IsRanged() {
      return true;
    };
    weaponPSMState = IntEnum<gamePSMRangedWeaponStates>(scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Weapon));
  ...
  protected final const func GetWeaponEquipDuration(const scriptInterface: ref<StateGameScriptInterface>, const stateContext: ref<StateContext>, stateMachineInstanceData: StateMachineInstanceData) -> Float {
    ...
    if Equals(this.GetProcessedEquipmentManipulationRequest(stateMachineInstanceData, stateContext).equipAnim, gameEquipAnimationType.HACK_ForceInstantEquip) {
      return 0.00;
    };
    weapon = this.GetSlotAttachedItemObject(scriptInterface, stateMachineInstanceData) as WeaponObject;
    return statSystem.GetStatValue(Cast<StatsObjectID>(weapon.GetEntityID()), gamedataStatType.EquipDuration);
  }
  ...
  protected final const func GetWeaponUnEquipDuration(const scriptInterface: ref<StateGameScriptInterface>, stateContext: ref<StateContext>, stateMachineInstanceData: StateMachineInstanceData) -> Float {
    ...
    let instantEquipRequested: Bool = Equals(this.GetProcessedEquipmentManipulationRequest(stateMachineInstanceData, stateContext).equipAnim, gameEquipAnimationType.Instant);
    if instantEquipRequested || this.HasThrowableCooldown(scriptInterface, weapon) {
      duration = 0.00;
    } else {
      duration = statSystem.GetStatValue(Cast<StatsObjectID>(weapon.GetEntityID()), gamedataStatType.UnequipDuration);
    };
  ...
  protected final const func HandleWeaponEquip(scriptInterface: ref<StateGameScriptInterface>, stateContext: ref<StateContext>, stateMachineInstanceData: StateMachineInstanceData, item: ItemID) -> Void {
    ...
    let isInCombat: Bool = scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Combat) == 1;
  ...
  protected final const func HandleWeaponUnequip(scriptInterface: ref<StateGameScriptInterface>, stateContext: ref<StateContext>, stateMachineInstanceData: StateMachineInstanceData, item: ItemID) -> Void {
    ...
    if IsDefined(itemObject) && gunzerkerLevel > 0 && Equals(WeaponObject.GetWeaponType(itemObject.GetItemID()), gamedataItemType.Wea_SubmachineGun) {
  ...
  protected final const func HasThrowableCooldown(scriptInterface: ref<StateGameScriptInterface>, weapon: ref<WeaponObject>) -> Bool {
    if !IsDefined(weapon) {
      return false;
    };
    if !weapon.WeaponHasTag(n"Throwable") {
      return false;
    };
    if scriptInterface.GetStatPoolsSystem().HasStatPoolValueReachedMax(Cast<StatsObjectID>(weapon.GetEntityID()), gamedataStatPoolType.ThrowRecovery) {
      return false;
    };
    return true;
  }

  protected final const func GetSlotAttachedItem(const scriptInterface: ref<StateGameScriptInterface>, stateMachineInstanceData: StateMachineInstanceData) -> ref<ItemObject> {
    let mappedInstanceData: InstanceDataMappedToReferenceName = this.GetMappedInstanceData(stateMachineInstanceData.referenceName);
  ...
  protected final const func CanProcessEquip(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    return !this.IsPlayerInAnyMenu(scriptInterface) && this.CheckGenericEquipItemConditions(stateContext, scriptInterface) && upperBodyState != 4 && (upperBodyState != 5 || this.HasActiveConsumable(scriptInterface));
  }
...
public class UnequippedDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ToUnequippedWaitingForExternalFactors(stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    if !this.IsProperItemEquipped(scriptInterface, stateContext, this.stateMachineInstanceData) && this.IsRightHandLogic(this.stateMachineInstanceData) && upperBodyState != 4 && upperBodyState != 5 && NotEquals(this.GetItemCategoryFromItemID(this.GetSlotAttachedItemID(scriptInterface, this.stateMachineInstanceData)), gamedataItemCategory.Gadget) {
      return true;
    };
    return false;
  }
...
public class DelayedAnimFeatureCall extends DelayCallback {

  public let featureName: CName;

  public let animFeature: ref<AnimFeature_EquipUnequipItem>;

  public let scriptInterface: ref<StateGameScriptInterface>;

  public func Call() -> Void {
    if IsDefined(this.scriptInterface) {
      this.scriptInterface.SetAnimationParameterFeature(this.featureName, this.animFeature);
    };
  }
}
...
public class UnequippedEvents extends EquipmentBaseEvents {
  ...
  protected func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    let delayedAnimFeature: ref<DelayedAnimFeatureCall>;
...
public class UnequippedWaitingForExternalFactorsDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ExitCondition(stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    if this.IsRightHandLogic(this.stateMachineInstanceData) {
      ...
      if this.IsLeftHandLogic(this.stateMachineInstanceData) {
        if !this.CheckGenericEquipItemConditions(stateContext, scriptInterface) || scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody) == 5 {
          return false;
        };
      };
    };
...
public class EquippedDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ToUnequipCycle(stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    upperBodyState = scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody);
    if upperBodyState == 4 && this.IsRightHandLogic(this.stateMachineInstanceData) || upperBodyState == 5 {
      return true;
    };
    if !IsServer() {
...
public class EquipCycleInitDecisions extends EquipmentBaseDecisions {
  ...
  public final const func EnterCondition(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    let record: ref<Item_Record> = TweakDBInterface.GetItemRecord(ItemID.GetTDBID(item));
    let item: ItemID = this.GetSlotActiveItem(scriptInterface, this.stateMachineInstanceData);
    if !ItemID.IsValid(item) {
      return true;
    };
    record = TweakDBInterface.GetItemRecord(ItemID.GetTDBID(item));
    if !IsDefined(record) || NotEquals(record.ItemCategory().Type(), gamedataItemCategory.Weapon) {
      return true;
    };
    equipmentSystemPlayerData = EquipmentSystem.GetData(scriptInterface.executionOwner);
    if !IsDefined(equipmentSystemPlayerData) {
      return true;
    };
    if WeaponObject.IsCyberwareWeapon(item) {
      vehicleState = stateContext.GetStateMachineCurrentState(n"Vehicle");
      if Equals(vehicleState, n"exit") || Equals(vehicleState, n"coolExiting") || Equals(vehicleState, n"slideExiting") {
        return true;
      };
    };
  ...
  protected final const func ToEquipCycle(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    return this.IsVisualItemInSlot(scriptInterface, this.stateMachineInstanceData) || this.IsUsingFluffConsumable(scriptInterface, this.stateMachineInstanceData);
  }
...
public class EquipCycleInitEvents extends EquipmentBaseEvents {
  ...
  protected func OnEnter(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    let item: ItemID;
    let itemHandling: ref<AnimFeature_EquipUnequipItem>;
    let mappedInstanceData: InstanceDataMappedToReferenceName;
    ...
    if ItemID.IsValid(item) {
      if !WeaponObject.IsRanged(item) && !stateContext.IsStateMachineActive(n"Vehicle") {
        mappedInstanceData = this.GetMappedInstanceData(this.stateMachineInstanceData.referenceName);
      ...
      if !this.IsUsingFluffConsumable(scriptInterface, this.stateMachineInstanceData) {
        ...
        if !stateContext.IsStateMachineActive(n"Vehicle") || Equals(GetMountedVehicle(scriptInterface.executionOwner).GetCameraManager().GetActivePerspective(), vehicleCameraPerspective.FPP) {
...
public class EquipCycleDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ToFirstEquip(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    let preventFirstEquip: Bool = scriptInterface.localBlackboard.GetBool(GetAllBlackboardDefs().PlayerStateMachine.ScenePreventFirstEquip) || scriptInterface.localBlackboard.GetBool(GetAllBlackboardDefs().PlayerStateMachine.MountedPreventFirstEquip);
    let itemId: ItemID = this.GetSlotActiveItem(scriptInterface, this.stateMachineInstanceData);
    return firstEquipResult.value && !preventFirstEquip && this.ToEquipped(stateContext, scriptInterface) && !firstEqSystem.HasPlayedFirstEquip(ItemID.GetTDBID(itemId));
  }
}
  ...
  protected func OnExit(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    if this.IsRightHandLogic(this.stateMachineInstanceData) {
      if Equals(this.GetItemCategoryFromItemID(item), gamedataItemCategory.Weapon) {
        ...
        animFeature.stateTransitionDuration = statSystem.GetStatValue(Cast<StatsObjectID>(itemObject.GetEntityID()), gamedataStatType.UnequipDuration);
...
public class FirstEquipDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ToEquipped(const stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    if stateContext.IsStateMachineActive(n"CombatGadget") || stateContext.IsStateMachineActive(n"LocomotionSwimming") || stateContext.IsStateMachineActive(n"Vehicle") || stateContext.IsStateMachineActive(n"CarriedObject") || stateContext.IsStateMachineActive(n"Consumable") || stateContext.IsStateMachineActive(n"LocomotionTakedown") || upperBodyState == 4 || locomotionDetailedState == 8 || locomotionDetailedState == 10 || locomotionDetailedState == 11 || locomotionDetailedState == 12 || locomotionDetailedState == 9 || locomotionDetailedState == 17 || locomotionDetailedState == 5 || locomotionDetailedState == 6 {
      return true;
    };
    leftHandCyberState = IntEnum<gamePSMLeftHandCyberware>(scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.LeftHandCyberware));
    ...
    threshold = statSystem.GetStatValue(Cast<StatsObjectID>(weapon.GetEntityID()), gamedataStatType.EquipDuration_First) - statSystem.GetStatValue(Cast<StatsObjectID>(weapon.GetEntityID()), gamedataStatType.EquipDuration);
    if this.GetInStateTime() >= threshold {
      return true;
    };
    if this.CheckSlotMatchAndCompareRequestType(stateContext, this.stateMachineInstanceData, this.stateMachineInitData, EquipmentManipulationRequestType.Unequip) {
...
public class UnequipCycleDecisions extends EquipmentBaseDecisions {
  ...
  protected final const func ToEquipCycleInit(stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    ...
    let itemId: ItemID = this.GetSlotActiveItem(scriptInterface, this.stateMachineInstanceData);
    let upperBodyState: Int32 = scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody);
    let smConditions: Bool = upperBodyState == 4 || upperBodyState == 5 || Equals(stateContext.GetStateMachineCurrentState(n"Vehicle"), n"entering") || Equals(stateContext.GetStateMachineCurrentState(n"Vehicle"), n"switchSeats");
    return this.GetInStateTime() >= threshold && itemId != ItemID.None() && !smConditions;
  }
  ...
  protected final const func ToUnequipped(stateContext: ref<StateContext>, const scriptInterface: ref<StateGameScriptInterface>) -> Bool {
    let threshold: Float = this.GetUnequipDuration(scriptInterface, stateContext, this.stateMachineInstanceData);
    let upperBodyState: Int32 = scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody);
    if this.GetInStateTime() >= threshold && this.GetSlotActiveItem(scriptInterface, this.stateMachineInstanceData) == ItemID.None() {
      if this.IsLeftHandLogic(this.stateMachineInstanceData) {
      ...
      if (upperBodyState == 4 || upperBodyState == 5 || Equals(stateContext.GetStateMachineCurrentState(n"Vehicle"), n"entering") || Equals(stateContext.GetStateMachineCurrentState(n"Vehicle"), n"switchSeats")) && this.IsRightHandLogic(this.stateMachineInstanceData) {
        return true;
      };
    };
    return false;
...
public class UnequipCycleEvents extends EquipmentBaseEvents {
  ...
  protected func OnExit(stateContext: ref<StateContext>, scriptInterface: ref<StateGameScriptInterface>) -> Void {
    ...
    if this.IsRightHandLogic(this.stateMachineInstanceData) {
      if Equals(this.GetItemCategoryFromItemID(item), gamedataItemCategory.Weapon) {
        animFeature.stateTransitionDuration = statSystem.GetStatValue(Cast<StatsObjectID>(scriptInterface.executionOwnerEntityID), gamedataStatType.UnequipDuration);
        ...
        scriptInterface.SetAnimationParameterFeature(n"equipUnequipItem", animFeature, itemObject);
        upperBodyState = scriptInterface.localBlackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody);
        if (Equals(this.GetEquipAreaFromItemID(item), gamedataEquipmentArea.WeaponHeavy) || (itemObject as ItemObject).GetItemData().HasTag(n"DiscardOnEmpty")) && upperBodyState != 5 {
