...
public class PlayerCombatController extends IScriptable {
  ...
  private final func InitPlayerCombatControllerRefreshPolicy() -> Void {
    ...
    this.m_gameplayActiveFlagsRefreshPolicy.m_isTracked = PlayerCombatControllerRefreshPolicyEnum.Persistent;
  ...
  private final func VerifyActivation() -> Void {
    if this.m_gameplayActiveFlags.m_crouchActive && !this.m_gameplayActiveFlags.m_isTracked && !this.m_gameplayActiveFlags.m_usingJhonnyReplacer {
      if NotEquals(this.m_otherVars.m_state, PlayerCombatState.Stealth) {
        this.InvalidateActivationState(PlayerCombatState.Stealth);
      };
    } else {
      if !this.m_gameplayActiveFlags.m_crouchActive && !this.m_gameplayActiveFlags.m_isTracked {
        ...
        if this.m_gameplayActiveFlags.m_isTracked {
  ...
  private final func ProcessFlagsRefreshPolicy() -> Void {
    ...
    if Equals(this.m_gameplayActiveFlagsRefreshPolicy.m_isTracked, PlayerCombatControllerRefreshPolicyEnum.Eventful) {
  ...
  private final func OnCrouchActiveChanged(value: Int32) -> Void {
    ...
    let factVal: Int32 = GetFact(this.m_owner.GetGame(), n"SniperNestSaveHack");
    if factVal > 0 {
      return;
    };
    if value == 1 {
  ...
  public final func OnCrouchDelayEvent(evt: ref<CrouchDelayEvent>) -> Void {
    if NotEquals(this.m_gameplayActiveFlags.m_crouchTimerPassed, true) && this.m_gameplayActiveFlags.m_crouchActive {
  ...
  private final func ActivateStealth() -> Void {
    if this.m_gameplayActiveFlags.m_crouchActive {
      ...
      this.SetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.Combat, 3);
    } else {
      this.VerifyActivation();
    };
  }
