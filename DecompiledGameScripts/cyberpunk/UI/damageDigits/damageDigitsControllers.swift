...
public class DamageDigitsGameController extends inkProjectedHUDGameController {
  ...
  private let m_individualControllerArray: [wref<DamageDigitLogicController>];

  private let m_accumulatedControllerArray: [AccumulatedDamageDigitsNode];
  ...
  protected cb func OnInitialize() -> Bool {
    let uiDamageInfoBlackboard: wref<IBlackboard> = this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_DamageInfo);
    this.m_damageListBlackboardId = uiDamageInfoBlackboard.RegisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DamageList, this, n"OnDamageAdded");
    this.m_damageDigitsModeBlackboardId = uiDamageInfoBlackboard.RegisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DigitsMode, this, n"OnDamageDigitsModeChanged", true);
    this.m_damageDigitsStickingModeBlackboardId = uiDamageInfoBlackboard.RegisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DigitsStickingMode, this, n"OnDigitsStickingModeChanged", true);
  ...
  protected cb func OnUninitialize() -> Bool {
    let uiDamageInfoBlackboard: wref<IBlackboard> = this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_DamageInfo);
    uiDamageInfoBlackboard.UnregisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DamageList, this.m_damageListBlackboardId);
    uiDamageInfoBlackboard.UnregisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DigitsMode, this.m_damageDigitsModeBlackboardId);
    uiDamageInfoBlackboard.UnregisterListenerVariant(GetAllBlackboardDefs().UI_DamageInfo.DigitsStickingMode, this.m_damageDigitsStickingModeBlackboardId);
  }
  ...
  protected cb func OnVehicleTirePuncturedEvent(evt: ref<VehicleTirePuncturedEvent>) -> Bool {
    let controller: ref<DamageDigitLogicController> = this.m_digitsQueue.Dequeue() as DamageDigitLogicController;
  ...
  protected cb func OnDamageDigitSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    controller.m_stickToTarget = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Individual) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
  ...
  protected cb func OnAccumulatedDamageDigitSpawned(widget: ref<inkWidget>, userData: ref<IScriptable>) -> Bool {
    ...
    this.m_accumulatedControllerArray[index].m_controller.m_stickToTarget = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Accumulated) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
  ...
  private final func ShowDamageFloater(const damageInfo: script_ref<DamageInfo>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(damageInfo).userData.flags) {
      if Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.DealNoDamage) || Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.DontShowDamageFloater) || Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.ImmortalTarget) && !ScriptedPuppet.IsBoss(Deref(damageInfo).entityHit) {
        return false;
      };
      i += 1;
    };
  ...
  private final func IsDamageOverTime(const damageInfo: script_ref<DamageInfo>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(damageInfo).userData.flags) {
      if Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.DamageOverTime) {
        return true;
      };
      i += 1;
    };
  ...
  public final func IsCriticalHit(const damageInfo: script_ref<DamageInfo>) -> Bool {
    return Equals(Deref(damageInfo).hitType, gameuiHitType.CriticalHit) || Equals(Deref(damageInfo).hitType, gameuiHitType.CriticalHit_x2);
  }

  protected cb func OnDamageAdded(value: Variant) -> Bool {
    let accumulatedDigitsSticking: Bool;
    let controller: wref<DamageDigitLogicController>;
    let controllerFound: Bool;
    let damageInfo: DamageInfo;
    let damageList: array<DamageInfo>;
    ...
    let executedNPCPuppetIDList: array<EntityID>;
    let i: Int32;
    let individualDigitsSticking: Bool;
    let isDamageOverTime: Bool;
    let isFinisherTriggered: Bool;
    let k: Int32;
    let listPosition: Int32;
    let npcPuppet: wref<NPCPuppet>;
    let npcPuppetHP: Float;
    let oneInstance: Bool;
    let showingBoth: Bool;
    let showingBothSecondary: Bool;
    if !this.m_showDigitsIndividual && !this.m_showDigitsAccumulated {
      return true;
    };
    damageList = FromVariant<array<DamageInfo>>(value);
    showingBoth = this.m_showDigitsIndividual && this.m_showDigitsAccumulated && (Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.None) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both));
    individualDigitsSticking = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Individual) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
    accumulatedDigitsSticking = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Accumulated) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
    i = 0;
    while i < ArraySize(damageList) {
      damageInfo = damageList[i];
      if this.m_realOwner == damageInfo.instigator {
        npcPuppet = damageInfo.entityHit as NPCPuppet;
        npcPuppetHP = GameInstance.GetStatPoolsSystem(damageInfo.instigator.GetGame()).GetStatPoolValue(Cast<StatsObjectID>(npcPuppet.GetEntityID()), gamedataStatPoolType.Health, false);
        isFinisherTriggered = AttackData.HasFlag(damageInfo.userData.flags, hitFlag.FinisherTriggered);
        if IsDefined(npcPuppet) && (npcPuppetHP <= 0.00 && isFinisherTriggered || NotEquals(damageInfo.hitType, gameuiHitType.Miss) && NotEquals(damageInfo.hitType, gameuiHitType.Glance) && npcPuppet.GetHitReactionComponent().IsExecutedByDismemberment()) {
          if ArrayFindFirst(executedNPCPuppetIDList, npcPuppet.GetEntityID()) == -1 {
            ArrayPush(executedNPCPuppetIDList, npcPuppet.GetEntityID());
          ...
          if this.ShowDamageFloater(damageInfo) {
            if isFinisherTriggered {
              ...
              if AttackData.HasFlag(damageInfo.userData.flags, hitFlag.VehicleImpact) {
                controller = this.m_digitsQueue.Dequeue() as DamageDigitLogicController;
                controller.ShowRammingFloater(damageInfo);
              } else {
                isDamageOverTime = this.IsDamageOverTime(damageInfo);
  ...
  protected cb func OnDamageDigitsModeChanged(value: Variant) -> Bool {
    let damageDigitsMode: gameuiDamageDigitsMode = FromVariant<gameuiDamageDigitsMode>(value);
  ...
  protected cb func OnDigitsStickingModeChanged(value: Variant) -> Bool {
    this.m_damageDigitsStickingMode = FromVariant<gameuiDamageDigitsStickingMode>(value);
  ...
  private final func UpdateDamageDigitsStickingMode() -> Void {
    let i: Int32 = 0;
    while i < this.m_maxVisible {
      this.m_individualControllerArray[i].m_stickToTarget = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Individual) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
      i += 1;
    };
    i = 0;
    while i < this.m_maxAccumulatedVisible {
      this.m_accumulatedControllerArray[i].m_controller.m_stickToTarget = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Accumulated) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Both);
      if this.m_accumulatedControllerArray[i].m_used && this.m_accumulatedControllerArray[i].m_controller.m_currentlySticking {
        this.m_accumulatedControllerArray[i].m_isDamageOverTime = Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.None) || Equals(this.m_damageDigitsStickingMode, gameuiDamageDigitsStickingMode.Individual);
      };
      i += 1;
    };
  }
...
public class DamageDigitLogicController extends inkLogicController {
  ...
  private edit let m_wheelShotWidget: inkTextRef;

  private edit let m_executedWidget: inkTextRef;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    inkWidgetRef.SetVisible(this.m_wheelShotWidget, false);
    inkWidgetRef.SetVisible(this.m_executedWidget, false);
  ...
  public final func Show(const damageInfo: script_ref<DamageInfo>, showingBothDigits: Bool, forceStickToTarget: Bool) -> Void {
    let desiredOpacity: Float;
    let isFinisherTriggered: Bool;
    let state: CName;
    this.m_forceStickToTarget = forceStickToTarget;
    this.CalculateDistanceModifier(Deref(damageInfo).instigator.GetWorldPosition(), Deref(damageInfo).entityHit.GetWorldPosition());
    ...
    if this.m_stickToTarget || this.m_forceStickToTarget {
      if Deref(damageInfo).entityHit.IsDevice() {
        this.m_projection.ResetFixedWorldOffset();
      } else {
        this.m_projection.SetFixedWorldOffset(this.m_animStickTargetOffset);
      };
      this.m_projection.SetEntity(Deref(damageInfo).entityHit);
    } else {
      this.m_projection.ResetFixedWorldOffset();
      this.m_projection.ResetEntity();
      this.m_projection.SetStaticWorldPosition(Deref(damageInfo).hitPosition);
    };
    this.m_projection.RegisterListener(this, n"OnScreenProjectionUpdate");
    this.m_projection.SetEnabled(true);
    isFinisherTriggered = AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.FinisherTriggered);
    if Cast<Int32>(Deref(damageInfo).damageValue) > 0 && (Equals(Deref(damageInfo).userData.hitShapeType, EHitShapeType.Flesh) || isFinisherTriggered) {
    ...
    this.m_successfulCritical = this.m_gameController.IsCriticalHit(damageInfo) || AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.Headshot);
    this.m_textWidget.SetText(" " + Cast<Int32>(Deref(damageInfo).damageValue));
    ...
    state = isFinisherTriggered ? n"Special" : this.BuildStateName(Deref(damageInfo).damageType, Deref(damageInfo).hitType, Deref(damageInfo).userData.flags);
    ...
    this.m_textWidget.SetVisible(true);
    inkWidgetRef.SetVisible(this.m_critWidget, !isFinisherTriggered && this.m_gameController.IsCriticalHit(damageInfo));
    inkWidgetRef.SetVisible(this.m_headshotWidget, !isFinisherTriggered && AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.Headshot));
    inkWidgetRef.SetVisible(this.m_wheelShotWidget, false);
    inkWidgetRef.SetVisible(this.m_executedWidget, false);
  }

  public final func ShowRammingFloater(const damageInfo: script_ref<DamageInfo>) -> Void {
    let state: CName = this.BuildStateName(Deref(damageInfo).damageType, Deref(damageInfo).hitType, Deref(damageInfo).userData.flags);
    this.CalculateDistanceModifier(Deref(damageInfo).instigator.GetWorldPosition(), Deref(damageInfo).entityHit.GetWorldPosition());
    this.UpdatePositionAndScale(false);
    this.UpdateDuration(false);
    if this.m_stickToTarget {
      this.m_projection.SetFixedWorldOffset(Deref(damageInfo).hitPosition - Deref(damageInfo).entityHit.GetWorldPosition() + this.m_animStickTargetOffset);
      this.m_projection.SetEntity(Deref(damageInfo).entityHit);
    } else {
      this.m_projection.ResetFixedWorldOffset();
      this.m_projection.ResetEntity();
    ...
    this.m_textWidget.SetText(" " + Cast<Int32>(Deref(damageInfo).damageValue));
    this.m_textWidget.SetState(state);
    this.m_textWidget.SetOpacity(this.m_animPositiveOpacity);
    this.m_textWidget.SetVisible(true);
    inkWidgetRef.SetVisible(this.m_critWidget, false);
    inkWidgetRef.SetVisible(this.m_headshotWidget, false);
    inkWidgetRef.SetVisible(this.m_executedWidget, false);
  ...
  public final func ShowPuncturedFloater(instigator: wref<GameObject>, vehicle: wref<GameObject>, puncturePosition: Vector4) -> Void {
    this.CalculateDistanceModifier(instigator.GetWorldPosition(), vehicle.GetWorldPosition());
    this.UpdatePositionAndScale(false);
    this.UpdateDuration(false);
    if this.m_stickToTarget {
      this.m_projection.SetFixedWorldOffset(puncturePosition - vehicle.GetWorldPosition() + this.m_animStickTargetOffset);
      this.m_projection.SetEntity(vehicle);
    } else {
      this.m_projection.ResetFixedWorldOffset();
      this.m_projection.ResetEntity();
      this.m_projection.SetStaticWorldPosition(puncturePosition);
    };
    this.m_projection.RegisterListener(this, n"OnScreenProjectionUpdate");
    this.m_projection.SetEnabled(true);
    this.m_successful = true;
    this.m_successfulCritical = true;
    inkWidgetRef.SetOpacity(this.m_wheelShotWidget, this.m_animPositiveOpacity);
    inkWidgetRef.SetVisible(this.m_wheelShotWidget, true);
    this.m_textWidget.SetVisible(false);
    inkWidgetRef.SetVisible(this.m_critWidget, false);
    inkWidgetRef.SetVisible(this.m_headshotWidget, false);
    inkWidgetRef.SetVisible(this.m_executedWidget, false);
  }

  public final func ShowExecutedFloater(const damageInfo: script_ref<DamageInfo>) -> Void {
    this.CalculateDistanceModifier(Deref(damageInfo).instigator.GetWorldPosition(), Deref(damageInfo).entityHit.GetWorldPosition());
    this.UpdatePositionAndScale(false);
    this.UpdateDuration(false);
    if this.m_stickToTarget {
    ...
    this.m_successful = true;
    this.m_successfulCritical = true;
    inkTextRef.SetLocalizedTextScript(this.m_executedWidget, AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.FinisherTriggered) ? "LocKey#93567" : "LocKey#90153");
    inkWidgetRef.SetOpacity(this.m_executedWidget, this.m_animPositiveOpacity);
    inkWidgetRef.SetVisible(this.m_executedWidget, true);
    this.m_textWidget.SetVisible(false);
    inkWidgetRef.SetVisible(this.m_critWidget, false);
    inkWidgetRef.SetVisible(this.m_headshotWidget, false);
    inkWidgetRef.SetVisible(this.m_wheelShotWidget, false);
  ...
  private final func BuildStateName(damageType: gamedataDamageType, hitType: gameuiHitType, flags: [SHitFlag]) -> CName {
...
public class AccumulatedDamageDigitLogicController extends inkLogicController {
  ...
  public final func Show(const damageInfo: script_ref<DamageInfo>, showingBothDigits: Bool, oneInstance: Bool, forceStickToTarget: Bool) -> Void {
    let desiredOpacity: Float;
    let state: CName;
    this.m_damageAccumulated = Deref(damageInfo).damageValue;
    ...
    if Cast<Int32>(Deref(damageInfo).damageValue) > 0 && Equals(Deref(damageInfo).userData.hitShapeType, EHitShapeType.Flesh) {
      this.m_successful = true;
      desiredOpacity = this.m_animPositiveOpacity;
    } else {
      this.m_successful = false;
    ...
    if (!showingBothDigits || oneInstance) && this.m_gameController.IsCriticalHit(damageInfo) {
      this.m_successfulCritical = true;
    ...
    if (!showingBothDigits || oneInstance) && AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.Headshot) {
    ...
    this.m_textWidget.SetText(ToString(Cast<Int32>(this.m_damageAccumulated)));
    this.m_textWidget.SetOpacity(desiredOpacity);
    state = this.BuildStateName(Deref(damageInfo).damageType, Deref(damageInfo).hitType, oneInstance ? false : showingBothDigits, Deref(damageInfo).userData.flags);
    this.m_textWidget.SetState(state);
    inkWidgetRef.SetState(this.m_critWidget, state);
    inkWidgetRef.SetState(this.m_headshotWidget, state);
  }
  ...
  public final func UpdateDamageInfo(const damageInfo: script_ref<DamageInfo>, showingBothDigits: Bool) -> Void {
    ...
    if !showingBothDigits && this.m_gameController.IsCriticalHit(damageInfo) {
      this.m_successfulCritical = true;
      inkWidgetRef.SetVisible(this.m_critWidget, true);
      if IsDefined(this.m_critAnimProxy) && this.m_critAnimProxy.IsPlaying() {
        this.m_critAnimProxy.Stop();
    ...
    if !showingBothDigits && AttackData.HasFlag(Deref(damageInfo).userData.flags, hitFlag.Headshot) {
      inkWidgetRef.SetVisible(this.m_headshotWidget, true);
      if IsDefined(this.m_headshotAnimProxy) && this.m_headshotAnimProxy.IsPlaying() {
        this.m_headshotAnimProxy.Stop();
      };
    ...
    this.m_textWidget.SetText(ToString(Cast<Int32>(this.m_damageAccumulated)));
    this.m_textWidget.SetOpacity(desiredOpacity);
    state = this.BuildStateName(Deref(damageInfo).damageType, Deref(damageInfo).hitType, showingBothDigits, Deref(damageInfo).userData.flags);
    this.m_textWidget.SetState(state);
    inkWidgetRef.SetState(this.m_critWidget, state);
    inkWidgetRef.SetState(this.m_headshotWidget, state);
  ...
  private final func BuildStateName(damageType: gamedataDamageType, hitType: gameuiHitType, showingBothDigits: Bool, flags: [SHitFlag]) -> CName {
    let hitTypeStr: String;
    let statePrefix: String;
    let isCritical: Bool = false;
    if !this.m_successful {
