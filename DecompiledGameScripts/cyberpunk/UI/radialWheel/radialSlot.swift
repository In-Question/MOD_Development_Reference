...
public class RadialStatusEffectController extends inkLogicController {
  ...
  private let effects: [wref<SingleCooldownManager>];
...
public class PointerController extends inkLogicController {
  ...
  private edit const let m_connectors: [inkWidgetRef];
...
public class WeaponRadialSlot extends RadialSlot {
  ...
  public const func GetDebugInfo(out info: [String]) -> Void {
    super.GetDebugInfo(info);
    ArrayPush(info, "Index: " + IntToString(this.index));
    ArrayPush(info, "Area: " + EnumValueToString("gamedataEquipmentArea", Cast<Int64>(EnumInt(this.equipmentArea))));
    ArrayPush(info, "--------------------------------");
  }
}
...
public class RadialSlot extends IScriptable {
  ...
  @runtimeProperty("tooltip", "Achnor at which provided library reference should be spawned into")
  public edit let slotAnchorRef: inkWidgetRef;

  @runtimeProperty("tooltip", "Specify library item that you want to be spawned in this slot")
  ...
  public const func GetDebugInfo(out info: [String]) -> Void {
    ArrayPush(info, EnumValueToString("SlotType", Cast<Int64>(EnumInt(this.slotType))));
...
public class CyclableRadialSlot extends WeaponRadialSlot {
  ...
  public const func GetDebugInfo(out info: [String]) -> Void {
    super.GetDebugInfo(info);
    ArrayPush(info, "HOTKEY: " + EnumValueToString("EHotkey", Cast<Int64>(EnumInt(this.hotkey))));
