...
public class LineSpawnData extends IScriptable {
  ...
  public final func Initialize(const lineData: script_ref<scnDialogLineData>) -> Void {
...
public class BaseSubtitlesGameController extends inkProjectedHUDGameController {
  ...
  protected let m_lineMap: [subtitleLineMapEntry];

  protected let m_pendingShowLines: [CRUID];

  protected let m_pendingHideLines: [CRUID];

  protected let m_settings: ref<UserSettings>;

  protected let m_settingsListener: ref<SubtitlesSettingsListener>;

  @default(BaseSubtitlesGameController, /accessibility/subtitles)
  ...
  private let m_isRadioSubtitleEnabled: Bool;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.UpdateRadioSubsVisibilitySetting();
  ...
  protected final func EnableRadioSubtilesBySettings(value: Bool) -> Void {
    if NotEquals(this.m_isRadioSubtitleEnabled, value) {
      this.m_isRadioSubtitleEnabled = value;
      this.Cleanup();
    };
  }

  protected final func IsKiroshiEnabled(const lineData: script_ref<scnDialogLineData>) -> Bool {
    let lineText: scnDialogDisplayString = scnDialogLineData.GetDisplayText(Deref(lineData));
  ...
  protected func OnHideLine(const lineData: script_ref<subtitleLineMapEntry>) -> Void {
    ArrayRemove(this.m_pendingHideLines, Deref(lineData).id);
  }
  ...
  protected cb func OnShowDialogLine(value: Variant) -> Bool {
    let linesToShow: array<scnDialogLineData> = FromVariant<array<scnDialogLineData>>(value);
    this.ShowDialogLines(linesToShow);
  }

  public final func ShowDialogLines(const linesToShow: script_ref<[scnDialogLineData]>) -> Void {
    let currLine: scnDialogLineData;
    let count: Int32 = ArraySize(Deref(linesToShow));
    let i: Int32 = 0;
    while i < count {
      currLine = Deref(linesToShow)[i];
      if this.m_disabledBySettings && NotEquals(currLine.type, scnDialogLineType.Radio) {
        ...
        if !this.ShouldDisplayLine(currLine) || ArrayContains(this.m_pendingShowLines, currLine.id) {
          ...
          if Equals(currLine.type, scnDialogLineType.Radio) && !this.m_isRadioSubtitleEnabled {
          } else {
  ...
  protected cb func OnHideDialogLineByData(value: Variant) -> Bool {
    let linesToHide: array<scnDialogLineData> = FromVariant<array<scnDialogLineData>>(value);
    this.HideDialogLinesByData(linesToHide);
    this.TryRemovePendingHideLines();
  }

  public final func HideDialogLinesByData(const linesToHide: script_ref<[scnDialogLineData]>) -> Void {
    let currLine: scnDialogLineData;
    let count: Int32 = ArraySize(Deref(linesToHide));
    let i: Int32 = 0;
    while i < count {
      currLine = Deref(linesToHide)[i];
  ...
  protected cb func OnHideDialogLine(value: Variant) -> Bool {
    let linesToHide: array<CRUID> = FromVariant<array<CRUID>>(value);
    this.HideDialogLine(linesToHide);
    this.TryRemovePendingHideLines();
  }

  public final func HideDialogLine(const linesToHide: script_ref<[CRUID]>) -> Void {
    let currLine: CRUID;
    let count: Int32 = ArraySize(Deref(linesToHide));
    let i: Int32 = 0;
    while i < count {
  ...
  private final func UpdateRadioSubsVisibilitySetting() -> Void {
    let configVar: ref<ConfigVarBool> = this.m_settings.GetVar(this.m_groupPath, n"Radio") as ConfigVarBool;
    this.SetRadioSubsVisibilitySetting(configVar.GetValue());
  }
  ...
  private final func SpawnDialogLine(const lineData: script_ref<scnDialogLineData>) -> Void {
    ...
    if NotEquals(Deref(lineData).type, scnDialogLineType.GlobalTV) {
      controller = this.FindLineController(Deref(lineData).id);
    };
    if controller == null {
  ...
  private final func RemoveLineByData(const line: script_ref<scnDialogLineData>) -> Bool {
    let currLine: subtitleLineMapEntry;
    let result: Bool = false;
    let i: Int32 = 0;
    while i < ArraySize(this.m_lineMap) {
      currLine = this.m_lineMap[i];
      if !IsDefined(currLine.owner) || IsDefined(Deref(line).speaker) && Equals(currLine.owner.GetPersistentID(), Deref(line).speaker.GetPersistentID()) {
    ...
    if !result {
      this.OnRemovalFailure(Deref(line).id);
    };
    return result;
  }
  ...
  protected final func IsMainDialogLine(const lineData: script_ref<scnDialogLineData>) -> Bool {
    let player: wref<PlayerPuppet>;
    let playerVehicle: wref<VehicleObject>;
    let speakerVehicle: wref<VehicleObject> = Deref(lineData).speaker as VehicleObject;
    if speakerVehicle != null && Equals(Deref(lineData).type, scnDialogLineType.Radio) {
    ...
    return Deref(lineData).speaker != null && NotEquals(Deref(lineData).type, scnDialogLineType.Radio) && NotEquals(Deref(lineData).type, scnDialogLineType.OverHead) && NotEquals(Deref(lineData).type, scnDialogLineType.OverHeadAlwaysVisible) && NotEquals(Deref(lineData).type, scnDialogLineType.GlobalTV) && NotEquals(Deref(lineData).type, scnDialogLineType.Invisible) || Equals(Deref(lineData).type, scnDialogLineType.OwnerlessRegular);
  }
...
public class BaseSubtitleLineLogicController extends inkLogicController {
  ...
  public func SetLineData(const lineData: script_ref<scnDialogLineData>) -> Void {
    this.GetRootWidget().SetOpacity(Deref(lineData).isPersistent ? 1.00 : 0.50);
  }
