...
public class ChattersGameController extends BaseSubtitlesGameController {
  ...
  private let m_AllControllers: [ChatterKeyValuePair];

  private let m_targetingSystem: ref<TargetingSystem>;

  private let m_broadcastBlockingLines: [CRUID];
  ...
  protected func OnSubCreated(controller: wref<BaseSubtitleLineLogicController>) -> Void {
    ...
    if IsDefined(tmpVariant) {
      nameplateEntityId = FromVariant<EntityID>(tmpVariant);
    };
  ...
  private final func OnNameplateChanged() -> Void {
    let blackboardSystem: ref<BlackboardSystem> = this.GetBlackboardSystem();
    let blackboard: ref<IBlackboard> = blackboardSystem.Get(GetAllBlackboardDefs().UI_NameplateData);
    let nameplateEntityId: EntityID = FromVariant<EntityID>(blackboard.GetVariant(GetAllBlackboardDefs().UI_NameplateData.EntityID));
  ...
  private final func AddBroadcastBlockingLine(const lineData: script_ref<scnDialogLineData>) -> Void {
    if !ArrayContains(this.m_broadcastBlockingLines, Deref(lineData).id) {
      ArrayPush(this.m_broadcastBlockingLines, Deref(lineData).id);
    };
  }
  ...
  private final func IsLineTypeConditionFulfilled(const lineData: script_ref<scnDialogLineData>) -> Bool {
    let player: wref<PlayerPuppet>;
    let speakerVehicle: wref<VehicleObject>;
    if Equals(Deref(lineData).type, scnDialogLineType.OverHead) || Equals(Deref(lineData).type, scnDialogLineType.OverHeadAlwaysVisible) {
      return true;
    };
    if Equals(Deref(lineData).type, scnDialogLineType.Radio) {
      player = this.GetPlayerControlledObject() as PlayerPuppet;
      speakerVehicle = Deref(lineData).speaker as VehicleObject;
      return speakerVehicle == null && Deref(lineData).speaker != player && !this.IsBroadcastBlockedByMainDialogue();
    };
    if Equals(Deref(lineData).type, scnDialogLineType.GlobalTV) {
      return !this.IsBroadcastBlockedByMainDialogue();
    };
    return false;
  }

  private final func IsDistanceConditionFulfilled(const lineData: script_ref<scnDialogLineData>) -> Bool {
    let range: Float = this.m_sceneTier < 2 ? this.c_DisplayRange : this.c_CloseDisplayRange;
    return Vector4.DistanceSquared(this.GetPlayerControlledObject().GetWorldPosition(), Deref(lineData).speaker.GetWorldPosition()) < range * range;
  }
  ...
  protected func ShouldDisplayLine(const lineData: script_ref<scnDialogLineData>) -> Bool {
    ...
    if Equals(Deref(lineData).type, scnDialogLineType.OverHeadAlwaysVisible) {
      return true;
    };
    return Deref(lineData).speaker != null && this.m_sceneTier < 3 && this.IsLineTypeConditionFulfilled(lineData) && this.IsDistanceConditionFulfilled(lineData);
  }
  ...
  protected func OnHideLineByData(const lineData: script_ref<subtitleLineMapEntry>) -> Void {
    ...
    this.StopScreenProjection(Deref(lineData).widget);
    this.m_rootWidget.RemoveChild(Deref(lineData).widget);
    i = 0;
    while i < ArraySize(this.m_AllControllers) {
      if !IsDefined(this.m_AllControllers[i].Owner) || IsDefined(Deref(lineData).owner) && Equals(this.m_AllControllers[i].Owner.GetPersistentID(), Deref(lineData).owner.GetPersistentID()) {
  ...
  protected func OnHideLine(const lineData: script_ref<subtitleLineMapEntry>) -> Void {
    ...
    i = 0;
    while i < ArraySize(this.m_AllControllers) {
      if Equals(this.m_AllControllers[i].Key, Deref(lineData).id) {
        ArrayErase(this.m_AllControllers, i);
      };
      i = i + 1;
    };
  ...
  protected cb func OnScannerModeChanged(val: Variant) -> Bool {
    let evt: ref<ScanningModeEvent> = FromVariant<ref<ScanningModeEvent>>(val);
...
public class ChatterLineLogicController extends BaseSubtitleLineLogicController {
  ...
  private let m_bubbleMinDistance: Float;

  private let m_limitSubtitlesDistance: Bool;

  private let m_isOverHead: Bool;
  ...
  public func SetLineData(const lineData: script_ref<scnDialogLineData>) -> Void {
    ...
    if scnDialogLineData.HasKiroshiTag(Deref(lineData)) {
      ...
      if this.IsKiroshiEnabled() {
        ...
        this.SetupAnimation(Deref(lineData).duration, animCtrl);
  ...
  public final func IsVisible(targetedObject: EntityID) -> Bool {
    let isVisible: Bool;
    if !this.m_limitSubtitlesDistance {
      isVisible = true;
    } else {
      if this.m_isOverHead {
        isVisible = this.m_projection.distanceToCamera <= this.m_subtitlesMaxDistance;
      } else {
        isVisible = this.m_projection.VoIsPerceptible(targetedObject);
      };
    };
    return isVisible;
  }

  public final func IsBubble(targetedObject: EntityID) -> Bool {
    let isBubble: Bool;
    if !this.m_limitSubtitlesDistance {
      isBubble = targetedObject != this.m_ownerId;
    } else {
      isBubble = targetedObject != this.m_ownerId || this.m_projection.distanceToCamera > this.m_bubbleMinDistance && this.m_projection.distanceToCamera <= this.m_subtitlesMaxDistance;
    };
    return isBubble;
  }
  ...
  public final func UpdateProjection(targetedObject: EntityID, owner: wref<ChattersGameController>) -> Void {
    ...
    if IsDefined(this.m_projection) {
      isVisible = this.IsVisible(targetedObject);
      isBubble = this.IsBubble(targetedObject);
  ...
  public final func SetNameplateEntity(blackboardVariant: Variant) -> Void {
    this.m_nameplateEntityId = FromVariant<EntityID>(blackboardVariant);
  }
