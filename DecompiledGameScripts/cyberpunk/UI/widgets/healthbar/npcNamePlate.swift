...
public native class NpcNameplateGameController extends inkProjectedHUDGameController {
  ...
  private let m_cachedMappinControllers: [wref<BaseMappinBaseController>];
  ...
  private final native func SetSlottedWidgets(widgetsToSlot: [ref<inkWidget>], newParentWidger: wref<inkWidget>) -> Void;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    nameplateProjectionData.fixedWorldOffset = new Vector4(0.00, 0.00, 1.18, 0.00);
    ...
    nameplateProjectionData.extraOffsetAtMaxDistance = new Vector3(0.00, 0.00, 0.20);
  ...
  protected final func RegisterPSMListeners(playerPuppet: ref<GameObject>) -> Void {
    ...
    if IsDefined(uiBlackboardPSM) {
      ...
      this.m_visualController.UpdatePlayerZone(IntEnum<gamePSMZones>(uiBlackboardPSM.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Zones)), true);
      this.m_visualController.UpdatePlayerCombat(IntEnum<gamePSMCombat>(uiBlackboardPSM.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Combat)), true);
      this.m_visualController.UpdatePlayerAimStatus(IntEnum<gamePSMUpperBodyStates>(uiBlackboardPSM.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody)), true);
    };
  }
  ...
  protected cb func OnScreenProjectionUpdate(projections: ref<gameuiScreenProjectionsData>) -> Bool {
    let bufferedPuppet: ref<gamePuppet>;
    ...
    if this.m_nameplateProjection.GetEntity() != null && !entityJustSet {
      ...
      if IsDefined(this.m_bufferedGameObject) && this.m_bufferedGameObject.IsAttached() {
      ...
      data = FromVariant<DialogChoiceHubs>(this.m_uiBlackboardInteractions.GetVariant(GetAllBlackboardDefs().UIInteractions.DialogChoiceHubs));
      ...
      if IsDefined(bufferedPuppet = this.m_bufferedGameObject as gamePuppet) {
  ...
  protected cb func OnNameplateDataChanged(value: Variant) -> Bool {
    ...
    if incomingData.npc == null {
      this.m_visualController.SetVisualData(null, incomingData, this.m_isNewNPC);
      this.EnableUpdates(false);
      this.SetMainVisible(false);
      this.SetNameplateProjectionEntity(null);
      this.m_uiBlackboardNameplateBlackboard.SetVariant(this.m_UI_NameplateDataDef.EntityID, ToVariant(invalidID), false);
      ...
      this.GetBlackboardSystem().Get(nameplateBlackboard).SetFloat(this.m_UI_NameplateDataDef.HeightOffset, 0.00, true);
    };
  ...
  protected cb func OnZoneChange(value: Int32) -> Bool {
    this.m_visualController.UpdatePlayerZone(IntEnum<gamePSMZones>(value));
  }

  protected cb func OnAimStatusChange(value: Int32) -> Bool {
    this.m_visualController.UpdatePlayerAimStatus(IntEnum<gamePSMUpperBodyStates>(value));
  }

  protected cb func OnPlayerCombatChange(value: Int32) -> Bool {
    this.m_visualController.UpdatePlayerCombat(IntEnum<gamePSMCombat>(value));
  }
  ...
  protected cb func OnMappinsUpdated(mappinControllers: [wref<BaseMappinBaseController>]) -> Bool {
  ...
  private final func UpdateVisualControllerState(const mappinControllers: script_ref<[wref<BaseMappinBaseController>]>) -> Void {
    ...
    count = ArraySize(Deref(mappinControllers));
    i = 0;
    while i < count {
      controller = Deref(mappinControllers)[i];
  ...
  private final func UpdateSlotAttachment(const mappinControllers: script_ref<[wref<BaseMappinBaseController>]>) -> Void {
    ...
    if this.GetNameplateVisible() {
      ...
      i = 0;
      while i < count {
        attachmentWidget = Deref(mappinControllers)[i].GetWidgetForNameplateSlot();
