...
public class CompanionHealthBarGameController extends inkHUDGameController {
  ...
  private final func RegisterStatsListener() -> Void {
    ...
    this.m_statPoolsSystem.RequestRegisteringListener(Cast<StatsObjectID>(flatheadEntityID), gamedataStatPoolType.Health, this.m_healthStatListener);
    this.m_maxHealth = this.m_statPoolsSystem.GetStatPoolMaxPointValue(Cast<StatsObjectID>(flatheadEntityID), gamedataStatPoolType.Health);
  }

  private final func UnregisterStatsListener() -> Void {
    let flatheadPuppet: wref<ScriptedPuppet> = SubCharacterSystem.GetInstance(this.m_gameInstance).GetFlathead();
    let flatheadEntityID: EntityID = flatheadPuppet.GetEntityID();
    this.m_statPoolsSystem.RequestUnregisteringListener(Cast<StatsObjectID>(flatheadEntityID), gamedataStatPoolType.Health, this.m_healthStatListener);
  }
