...
public class DebugNpcNameplateGameController extends inkProjectedHUDGameController {
  ...
  protected cb func OnNameplateDataChanged(value: Variant) -> Bool {
    ...
    incomingData = FromVariant<NPCNextToTheCrosshair>(value);
  ...
  private final func HelperUpdateText(const argString1: script_ref<String>, const argString2: script_ref<String>) -> Void {
    this.m_debugText1.SetText(Deref(argString1));
    this.m_debugText2.SetText(Deref(argString2));
  }

  private final func GetNPCDebugNameplateStats(obj: ref<GameObject>, str_1: script_ref<String>, str_2: script_ref<String>) -> Void {
    let temp_str: String;
    let system: ref<StatsSystem> = GameInstance.GetStatsSystem(obj.GetGame());
    let temp_float: Float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.Level);
    temp_str += "Level: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.PowerLevel);
    temp_str += "\\nPowerLevel: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.Health);
    temp_str += "\\nMax Health: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.Evasion);
    temp_str += "\\nEvasion: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.PhysicalResistance);
    temp_str += "\\nPhysical Res: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.ThermalResistance);
    temp_str += "\\nThermal Res: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.ChemicalResistance);
    temp_str += "\\nChemical Res: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.ElectricResistance);
    temp_str += "\\nEMP Res: " + IntToString(RoundF(temp_float));
    str_1 = temp_str;
    temp_str = "";
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.DPS);
    temp_str = "\\nDPS: " + IntToString(RoundF(temp_float));
    temp_float = system.GetStatValue(Cast<StatsObjectID>(obj.GetEntityID()), gamedataStatType.Accuracy);
