...
public class BraindanceGameController extends inkHUDGameController {
  ...
  private edit const let m_clueHolder: [inkCompoundRef];

  private edit const let m_clueBarHolder: [inkWidgetRef];

  private edit const let m_speedIndicatorManagers: [inkWidgetRef];

  private let m_clueArray: [wref<BraindanceClueLogicController>];
  ...
  protected cb func OnClueDataUpdated(value: Variant) -> Bool {
    ...
    let clueDescriptor: gameuiBraindanceClueDescriptor = FromVariant<gameuiBraindanceClueDescriptor>(value);
  ...
  protected cb func OnPlaybackSpeedUpdated(value: Variant) -> Bool {
    this.m_currentSpeed = FromVariant<scnPlaySpeed>(value);
    this.UpdateSpeedIndicators();
  }

  protected cb func OnPlaybackDirectionUpdated(value: Variant) -> Bool {
    this.m_currentDirection = FromVariant<scnPlayDirection>(value);
    this.UpdateSpeedIndicators();
  }
  ...
  protected cb func OnVisionModeUpdated(value: Int32) -> Bool {
    this.SetVisionMode(IntEnum<gameuiEBraindanceLayer>(value));
    this.UpdateClues();
  }
  ...
  private final func GetTimeMS(seconds: Float) -> String {
    ...
    let minutesLeft: Int32 = Cast<Int32>(seconds) / 60;
    let minutesString: String = minutesLeft >= 10 ? "" : "0";
    minutesString += ToString(minutesLeft);
    secondsLeft = Cast<Int32>(seconds) % 60;
...
public class SpeedIndicatorIconsManager extends inkLogicController {
  ...
  public final func SetBraindanceSpeed(currentSpeed: scnPlaySpeed, currentDirection: scnPlayDirection) -> Void {
    ...
    inkWidgetRef.SetTranslation(selectedInkWidget, Cast<Float>(translationSign) * translationIntensity, 0.00);
