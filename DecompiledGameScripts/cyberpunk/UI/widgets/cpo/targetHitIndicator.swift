...
public class TargetHitIndicatorGameController extends inkGameController {
  ...
  private let m_weaponChangedListener: ref<AttachmentSlotsScriptListener>;
  ...
  protected cb func OnPlayerAttach(player: ref<GameObject>) -> Bool {
    ...
    if IsDefined(this.m_player) {
      ...
      this.m_isAimingDownSights = playerStateMachineBB.GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody) == 6;
      ...
      this.RegisterWeaponChangedListener();
    };
  ...
  protected cb func OnPlayerDetach(player: ref<GameObject>) -> Bool {
    ...
    if IsDefined(this.m_player) {
      ...
      this.UnregisterWeaponChangedListener();
      this.m_player = null;
    };
  }

  private final func RegisterWeaponChangedListener() -> Void {
    let puppetListener: ref<WeaponChangedListener> = new WeaponChangedListener();
    puppetListener.m_gameController = this;
    this.m_weaponChangedListener = GameInstance.GetTransactionSystem(this.m_player.GetGame()).RegisterAttachmentSlotListener(this.m_player, puppetListener);
  }

  private final func UnregisterWeaponChangedListener() -> Void {
    if IsDefined(this.m_weaponChangedListener) {
      GameInstance.GetTransactionSystem(this.m_player.GetGame()).UnregisterAttachmentSlotListener(this.m_player, this.m_weaponChangedListener);
      this.m_weaponChangedListener = null;
    };
  }
  ...
  protected cb func OnKillAdded(value: Variant) -> Bool {
    ...
    let killList: array<KillInfo> = FromVariant<array<KillInfo>>(value);
    let i: Int32 = 0;
  ...
  private final func ShouldShowDamage(const damageInfo: script_ref<DamageInfo>) -> Bool {
    let i: Int32;
    if Deref(damageInfo).damageValue == 0.00 {
      return false;
    };
    i = 0;
    while i < ArraySize(Deref(damageInfo).userData.flags) {
      if Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.ImmortalTarget) || Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.DealNoDamage) || Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.DontShowDamageFloater) {
        return false;
      };
      i += 1;
    };
  ...
  private final func ShouldShowBonus(const damageInfo: script_ref<DamageInfo>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(damageInfo).userData.flags) {
      if Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.Headshot) || Equals(Deref(damageInfo).userData.flags[i].flag, hitFlag.WeakspotHit) {
        return true;
      };
      i += 1;
    };
  ...
  protected cb func OnAimStatusChange(value: Int32) -> Bool {
    this.m_isAimingDownSights = value == 6;
  ...
  private final func UpdateWidgetPosition() -> Void {
    let evt: ref<NormalizeAndSaveSwayEvent>;
    ...
    if this.m_isAimingDownSights {
      ...
      sway.X = -19.20 * this.m_currentSway.X * multiplier;
      sway.Y = -18.90 * this.m_currentSway.Y * multiplier;
    } else {
      this.m_currentSway = new Vector2(0.00, 0.00);
      sway = new Vector2(0.00, 0.00);
    };
    this.m_rootWidget.SetMargin(new inkMargin(sway.X, sway.Y, 0.00, 0.00));
    evt = new NormalizeAndSaveSwayEvent();
    evt.sway = sway;
    this.QueueEvent(evt);
  }

  public final func OnWeaponChanged() -> Void {
    let stats: ref<StatsSystem>;
    let weapon: ref<WeaponObject>;
    if IsDefined(this.m_player) {
      stats = GameInstance.GetStatsSystem(this.m_player.GetGame());
  ...
  protected cb func OnNormalizeAndSaveSwayEvent(evt: ref<NormalizeAndSaveSwayEvent>) -> Bool {
    let windowSize: Vector2 = this.m_uiSystem.GetCurrentWindowSize();
    let inverseScale: Float = this.m_uiSystem.GetInverseUIScale();
    let uiSize: Vector2 = new Vector2(windowSize.X * inverseScale, windowSize.Y * inverseScale);
    let normSway: Vector2 = new Vector2(evt.sway.X / uiSize.X * 2.00, evt.sway.Y / uiSize.Y * 2.00);
    let UIGameDataBB: ref<IBlackboard> = this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UIGameData);
...
public class WeaponChangedListener extends AttachmentSlotsScriptCallback {

  public let m_gameController: wref<TargetHitIndicatorGameController>;

  public func OnItemEquipped(slot: TweakDBID, item: ItemID) -> Void {
    this.m_gameController.OnWeaponChanged();
  }
}
