...
public class NarrationJournalGameController extends inkHUDGameController {
  ...
  protected cb func OnEventAdded(value: Variant) -> Bool {
    let lastEvent: NarrationEvent = FromVariant<NarrationEvent>(value);
    this.AddEntry(lastEvent);
  }

  private final func AddEntry(const entry: script_ref<NarrationEvent>) -> Void {
...
public class LogEntryLogicController extends inkLogicController {
  ...
  public final func SetValues(const entry: script_ref<NarrationEvent>) -> Void {
    inkTextRef.SetText(this.m_textWidget, Deref(entry).text);
    inkWidgetRef.SetTintColor(this.m_textWidget, Deref(entry).color);
    this.SetTimeout(Deref(entry).durationSec);
  }
