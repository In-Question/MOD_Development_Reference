...
public native class gameuiChatBoxGameController extends inkHUDGameController {
  ...
  protected cb func OnChatAdded(value: Variant) -> Bool {
    let chatList: array<ChatBoxText> = FromVariant<array<ChatBoxText>>(value);
    let i: Int32 = 0;
  ...
  private final func DisplayChat(const chatBoxText: script_ref<ChatBoxText>) -> Void {
  ...
  private final func DisplayHistory(const chatBoxText: script_ref<ChatBoxText>) -> Void {
...
public class TextSectionLogicController extends inkLogicController {
  ...
  public final func Show(const chatBoxText: script_ref<ChatBoxText>) -> Void {
    this.m_textWidget.SetText(Deref(chatBoxText).text);
    this.m_textWidget.SetTintColor(Deref(chatBoxText).color);
    this.SetActive(true);
  }
