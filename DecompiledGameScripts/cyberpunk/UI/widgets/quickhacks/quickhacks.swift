...
public class QuickhacksListGameController extends inkHUDGameController {
  ...
  private edit let m_noQuickhacks: inkCompoundRef;

  private edit let m_openCyberdeckBtn: inkWidgetRef;

  private edit let m_executeBtn: inkWidgetRef;

  private edit let m_executeAndCloseBtn: inkWidgetRef;

  private edit let m_changeTarget: inkWidgetRef;

  private edit let m_tutorialButton: inkWidgetRef;
  ...
  private edit let m_costReductionText: inkTextRef;
  ...
  private edit const let m_vulnerabilityFields: [inkWidgetRef];
  ...
  private let m_quickhackBarArray: [wref<inkCompoundWidget>];
  ...
  @default(QuickhacksListGameController, false)
  private let m_shouldReveal: Bool;

  private let m_memorySpendAnimation: ref<inkAnimProxy>;

  private let m_memorySpendCounter: Int32;

  @default(QuickhacksListGameController, -1)
  private let m_memorySpendIndex: Int32;

  private let m_selectedMemoryLoop: [ref<inkAnimProxy>];
  ...
  private let m_statPoolListenersIndexes: [Int32];

  protected let m_chunkBlackboard: wref<IBlackboard>;

  private let m_nameCallbackID: ref<CallbackHandle>;

  private let m_uiScannerChangeTargetTooltipVisibilityCallback: ref<CallbackHandle>;

  private let m_uiScannerActiveTabCallback: ref<CallbackHandle>;
  ...
  protected cb func OnInitialize() -> Bool {
    let bbSystem: ref<BlackboardSystem> = this.GetBlackboardSystem();
    ...
    this.m_chunkBlackboard = bbSystem.Get(GetAllBlackboardDefs().UI_ScannerModules);
    ...
    this.m_uiScannerChangeTargetTooltipVisibilityCallback = bbSystem.Get(GetAllBlackboardDefs().UI_Scanner).RegisterListenerBool(GetAllBlackboardDefs().UI_Scanner.scannerChangeTargetTooltipVisibility, this, n"OnScannerChangeTargetTooltipVisibilityChanged");
    this.m_uiScannerActiveTabCallback = bbSystem.Get(GetAllBlackboardDefs().UI_Scanner).RegisterListenerVariant(GetAllBlackboardDefs().UI_Scanner.scannerActiveTab, this, n"OnScannerActiveTabChanged");
    this.SetupQuickhacksMemoryBar();
    this.OnMemoryPercentUpdate(1.00);
  ...
  protected cb func OnUninitialize() -> Bool {
    ...
    if IsDefined(this.m_uiScannerChangeTargetTooltipVisibilityCallback) {
      bbSystem.Get(GetAllBlackboardDefs().UI_Scanner).UnregisterListenerBool(GetAllBlackboardDefs().UI_Scanner.scannerChangeTargetTooltipVisibility, this.m_uiScannerChangeTargetTooltipVisibilityCallback);
    };
    if IsDefined(this.m_uiScannerActiveTabCallback) {
      bbSystem.Get(GetAllBlackboardDefs().UI_Scanner).UnregisterListenerVariant(GetAllBlackboardDefs().UI_Scanner.scannerActiveTab, this.m_uiScannerActiveTabCallback);
    };
  ...
  protected cb func OnScannerChangeTargetTooltipVisibilityChanged(value: Bool) -> Bool {
    inkWidgetRef.SetVisible(this.m_changeTarget, value);
  }

  protected cb func OnScannerActiveTabChanged(value: Variant) -> Bool {
    ...
    if this.IsPanelBlocked() {
      if uiQuickSlotsDataBB.GetBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelOpen) {
        if IsDefined(uiQuickSlotsDataBB) {
          uiQuickSlotsDataBB.SetBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelBlocked, true);
        };
        this.SetVisibility(false);
      };
    } else {
      if uiQuickSlotsDataBB.GetBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelBlocked) {
        this.SetVisibility(this.m_shouldReveal);
        if IsDefined(uiQuickSlotsDataBB) {
          uiQuickSlotsDataBB.SetBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelBlocked, false);
        };
      };
    };
  }
  ...
  protected cb func OnQuickhackStarted(value: ref<RevealInteractionWheel>) -> Bool {
    ...
    if value.shouldReveal {
      ...
      this.SetVisibility(!this.IsPanelBlocked());
      uiQuickSlotsDataBB.SetBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelBlocked, this.IsPanelBlocked());
      this.RegisterCooldownStatPoolUpdate();
    } else {
      this.SetVisibility(false);
    };
    this.m_shouldReveal = value.shouldReveal;
  }
    ...
    if isReleased && !isMinigameActive && !this.m_isUILocked {
      ...
      switch actionName {
        case n"UI_MoveDown":
          ...
          this.PlaySound(n"QuickHackMenu", n"OnHover");
  ...
  private final func SelectData(data: ref<QuickhackData>) -> Void {
    ...
    description = GetLocalizedText(this.m_selectedData.m_description);
    ...
    if this.m_selectedData.m_isLocked && this.m_selectedData.m_actionMatchesTarget && this.m_hasActiveUpload {
      ...
      this.SendOverclockPreviewEvent(false);
      ...
      if this.m_selectedData.m_isLocked {
        ...
        if this.m_selectedData.m_noQuickhackData {
          inkWidgetRef.SetVisible(this.m_warningWidget, false);
        } else {
        ...
        this.ApplyQuickhackSelection();
        if this.m_selectedData.m_cost > this.m_lastFillCells {
          this.SendOverclockPreviewEvent(true, this.m_selectedData.m_cost);
        } else {
          this.SendOverclockPreviewEvent(false);
        };
        if IsDefined(this.inkWarningAnimProxy) {
        ...
        if this.m_selectedData.m_showRevealInfo {
          progressBarBB.SetFloat(GetAllBlackboardDefs().UI_HUDProgressBar.ProgressBump, this.m_selectedData.m_awarenessCost / 100.00);
        };
    ...
    this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_QuickSlotsData).SetVariant(GetAllBlackboardDefs().UI_QuickSlotsData.quickHackDataSelected, ToVariant(this.m_selectedData), true);
  }
  ...
  private final func SetupMemoryCostDifferance() -> Void {
    ...
    let increasedCost: Int32 = this.m_selectedData.m_cost - this.m_selectedData.m_costRaw;
    if Equals(this.m_selectedData.m_type, gamedataObjectActionType.VehicleQuickHack) {
      inkWidgetRef.SetVisible(this.m_costReductionPanel, false);
    } else {
      if reducedCost > 0 || this.m_selectedData.m_cost < this.m_selectedData.m_costRaw {
        inkTextRef.SetText(this.m_costReductionValue, IntToString(reducedCost));
        inkTextRef.SetText(this.m_costReductionText, GetLocalizedText("LocKey#48365"));
        inkWidgetRef.SetVisible(this.m_costReductionPanel, true);
        inkWidgetRef.SetState(this.m_costReductionPanel, n"Up");
      } else {
        if increasedCost > 0 || this.m_selectedData.m_cost > this.m_selectedData.m_costRaw {
          inkTextRef.SetText(this.m_costReductionValue, IntToString(increasedCost));
          inkTextRef.SetText(this.m_costReductionText, GetLocalizedText("LocKey#94471"));
          inkWidgetRef.SetVisible(this.m_costReductionPanel, true);
          inkWidgetRef.SetState(this.m_costReductionPanel, n"Low");
        } else {
          inkWidgetRef.SetVisible(this.m_costReductionPanel, false);
        };
      };
  ...
  private final func SetupICE() -> Void {
    if this.m_selectedData.m_ICELevelVisible {
      inkTextRef.SetText(this.m_iceValue, IntToString(Cast<Int32>(this.m_selectedData.m_ICELevel)));
    } else {
  ...
  private final func SetupTargetName() -> Void {
    let displayName: String = ToString(this.m_selectedData.m_actionOwnerName);
    if NotEquals(displayName, "None") {
      inkTextRef.SetText(this.m_targetName, displayName);
    };
  }
  ...
  protected cb func OnSpecialQuickhackAttackTriggered(evt: ref<OnSpecialQuickhackTriggeredEvent>) -> Bool {
    if IsDefined(evt.quickhackData) {
      this.m_selectedData = evt.quickhackData;
      this.ApplyQuickHack(false);
    };
  }

  private final func ShowInventory() -> Void {
    let evt: ref<StartHubMenuEvent> = new StartHubMenuEvent();
    let wrapper: ref<CyberwareDisplayWrapper> = new CyberwareDisplayWrapper();
    wrapper.displayData = this.GetItemDisplayData();
    evt.SetStartMenu(n"inventory_screen", wrapper);
    this.QueueBroadcastEvent(evt);
  }

  private final func GetItemDisplayData() -> InventoryItemDisplayData {
    let data: InventoryItemDisplayData;
    data.m_itemID = EquipmentSystem.GetData(this.m_playerObject).GetActiveItem(gamedataEquipmentArea.SystemReplacementCW);
    data.m_equipmentArea = gamedataEquipmentArea.SystemReplacementCW;
    data.m_slotIndex = 0;
    return data;
  }

  private final func ApplyQuickHack(shouldUseUI: Bool) -> Bool {
    let cmd: ref<QuickSlotCommandUsed>;
    let hackUsed: ref<QhackExecuted>;
    if shouldUseUI {
      this.PlayChoiceAnimation();
      this.SendOverclockPreviewEvent(false, true);
    };
  ...
  private final func LogQuickHack() -> Void {
    ...
    telemetryQuickHack.quality = RPGManager.GetFloatItemQuality(Cast<Float>(this.m_selectedData.m_quality / 2));
  ...
  private final func CanShowQuickHack() -> Bool {
  ...
  private final func SetupQuickhacksMemoryBar() -> Void {
    ...
    this.UpdateQuickhacksMemoryBarSize(FloorF(GameInstance.GetStatsSystem(this.m_gameInstance).GetStatValue(Cast<StatsObjectID>(this.GetPlayerControlledObject().GetEntityID()), gamedataStatType.Memory)));
  ...
  protected cb func OnMemoryPercentUpdate(value: Float) -> Bool {
    ...
    if IsDefined(this.m_selectedData) {
      ...
      usedCells = Min(this.m_lastFillCells, usedCells);
    };
    ...
    fillCells = FloorF(Cast<Float>(maxCells) * value * 0.01);
  ...
  private final func UpdateMemoryBar() -> Void {
    ...
    this.DeplenishMemoryCells();
  }

  private final func DeplenishMemoryCells() -> Void {
    if this.m_memorySpendIndex > this.m_lastFillCells && this.m_memorySpendIndex > 0 {
      this.m_memorySpendCounter = this.m_lastFillCells + this.m_lastUsedCells - 1;
      this.m_memorySpendIndex = this.m_lastFillCells;
      if IsDefined(this.m_memorySpendAnimation) {
  ...
  protected cb func OnDeplenishMemoryCells(opt e: ref<inkAnimProxy>) -> Bool {
    let textureWidget: wref<inkImage>;
    if IsDefined(this.m_memorySpendAnimation) {
      this.m_memorySpendAnimation.Stop();
      this.m_memorySpendAnimation.UnregisterFromCallback(inkanimEventType.OnFinish, this, n"OnDeplenishMemoryCells");
      this.m_memorySpendAnimation = null;
      textureWidget = this.m_quickhackBarArray[this.m_memorySpendCounter].GetWidgetByIndex(0) as inkImage;
      textureWidget.SetTexturePart(n"charge_empty");
      this.m_quickhackBarArray[this.m_memorySpendCounter].SetState(n"Empty");
      this.PlayLibraryAnimationOnTargets(n"memorySelected_out", SelectWidgets(this.m_quickhackBarArray[this.m_memorySpendCounter]));
      this.m_memorySpendCounter -= 1;
    };
    if this.m_memorySpendCounter > this.m_memorySpendIndex - 1 {
      textureWidget = this.m_quickhackBarArray[this.m_memorySpendCounter].GetWidgetByIndex(0) as inkImage;
      textureWidget.SetTexturePart(n"charge_free");
      this.m_quickhackBarArray[this.m_memorySpendCounter].SetState(n"Default");
      this.m_memorySpendAnimation = this.PlayLibraryAnimationOnTargets(n"memorySpend", SelectWidgets(this.m_quickhackBarArray[this.m_memorySpendCounter]));
      this.m_memorySpendAnimation.RegisterToCallback(inkanimEventType.OnFinish, this, n"OnDeplenishMemoryCells");
    };
  }

  private final func PopulateData(data: [ref<QuickhackData>]) -> Void {
    let count: Int32;
    let i: Int32;
    this.m_lastMemoryWarningTransitionAnimName = n"None";
    ...
    if count > 0 && data[0].m_noQuickhackData {
      inkWidgetRef.SetVisible(this.m_listWidget, false);
      inkWidgetRef.SetVisible(this.m_warningWidget, false);
      inkWidgetRef.SetVisible(this.m_costReductionPanel, false);
      inkCompoundRef.RemoveAllChildren(this.m_noQuickhacks);
      this.AsyncSpawnFromLocal(inkWidgetRef.Get(this.m_noQuickhacks), n"noQuickhacks");
      inkWidgetRef.SetVisible(this.m_noQuickhacks, true);
      inkWidgetRef.SetVisible(this.m_openCyberdeckBtn, true);
      inkWidgetRef.SetVisible(this.m_executeAndCloseBtn, false);
      inkWidgetRef.SetVisible(this.m_tutorialButton, false);
      ...
      inkWidgetRef.SetVisible(this.m_listWidget, true);
      inkWidgetRef.SetVisible(this.m_noQuickhacks, false);
      inkWidgetRef.SetVisible(this.m_openCyberdeckBtn, false);
      inkWidgetRef.SetVisible(this.m_executeAndCloseBtn, true);
      inkWidgetRef.SetVisible(this.m_tutorialButton, true);
  ...
  private final func RegisterCooldownStatPoolUpdate() -> Bool {
    let i: Int32;
    let i1: Int32;
    let buffList: array<BuffInfo> = FromVariant<array<BuffInfo>>(this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_PlayerBioMonitor).GetVariant(GetAllBlackboardDefs().UI_PlayerBioMonitor.DebuffsList));
  ...
  protected cb func OnCooldownStatPoolUpdate(value: Variant) -> Bool {
    ...
    let i1: Int32;
    let wasMatched: Bool;
    let buffList: array<BuffInfo> = FromVariant<array<BuffInfo>>(value);
    if ArraySize(buffList) == 0 {
      this.UnregisterCooldownStatPoolUpdate();
      QuickhackModule.RequestRefreshQuickhackMenu(this.GetPlayerControlledObject().GetGame(), this.m_data[0].m_actionOwner);
      return false;
  ...
  private final func IsPanelBlocked() -> Bool {
    let uiBlackboard: ref<IBlackboard> = this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_Scanner);
    return Equals(FromVariant<ScannerDetailTab>(uiBlackboard.GetVariant(GetAllBlackboardDefs().UI_Scanner.scannerActiveTab)), ScannerDetailTab.TwinTone);
  }
  ...
  public final func SendOverclockPreviewEvent(isHovering: Bool, opt cost: Int32, opt justHacked: Bool) -> Void {
    let overclockPreviewEvent: ref<OverclockDamagePreview> = new OverclockDamagePreview();
    ...
    this.QueueEvent(overclockPreviewEvent);
  }
}

public class QuickHackScreenOpen extends Event {

  @runtimeProperty("tooltip", "Open or close Quick Hack menu. SetToOpen == true means open | SetToOpen == false means close")
...
public class QuickHackTimeDilationOverride extends Event {
  ...
  @runtimeProperty("tooltip", "Time that will be set")
...
public class QuickHackLockHacks extends Event {
  ...
  @runtimeProperty("tooltip", "Is lock active")
...
public class ProgramEntry extends IScriptable {

  public let m_itemID: ItemID;

  public let m_equippedItem: wref<Item_Record>;

  public let m_abilities: [InventoryItemAbility];

  public final static func Make(itemID: ItemID, equippedItem: wref<Item_Record>) -> ref<ProgramEntry> {
    let entry: ref<ProgramEntry> = new ProgramEntry();
    entry.m_itemID = itemID;
    entry.m_equippedItem = equippedItem;
    entry.m_abilities = ProgramEntry.GetSpecialAbilities(equippedItem);
    return entry;
  }

  private final static func GetSpecialAbilities(equippedItem: wref<Item_Record>) -> [InventoryItemAbility] {
...
public class EquippedQuickHackData extends IScriptable {

  private let m_programEntries: [ref<ProgramEntry>];

  public final static func Make(player: wref<PlayerPuppet>) -> ref<EquippedQuickHackData> {
    let data: ref<EquippedQuickHackData> = new EquippedQuickHackData();
    let systemReplacementID: ItemID = EquipmentSystem.GetData(player).GetActiveItem(gamedataEquipmentArea.SystemReplacementCW);
    let parts: array<SPartSlots> = ItemModificationSystem.GetAllSlots(player, systemReplacementID);
    if EquipmentSystem.IsCyberdeckEquipped(player) {
      data.m_programEntries = EquippedQuickHackData.GetShardsQuickHacks(parts);
    };
    return data;
  }

  public final func GetAbilitiesByItemID(itemID: ItemID) -> [InventoryItemAbility] {
    let currentItem: ItemID;
    let currentTweakID: TweakDBID;
    let emptyArray: array<InventoryItemAbility>;
    let entry: ref<ProgramEntry>;
    let itemTweakID: TweakDBID;
    let i: Int32 = 0;
    while i < ArraySize(this.m_programEntries) {
      entry = this.m_programEntries[i];
  ...
  private final static func GetShardsQuickHacks(parts: [SPartSlots]) -> [ref<ProgramEntry>] {
    let entries: array<ref<ProgramEntry>>;
    let equippedShard: wref<Item_Record>;
    let installedItem: ItemID;
    let part: SPartSlots;
    let programEntry: ref<ProgramEntry>;
    let i: Int32 = 0;
    while i < ArraySize(parts) {
      part = parts[i];
      installedItem = part.installedPart;
      equippedShard = RPGManager.GetItemRecord(installedItem);
      if IsDefined(equippedShard) {
        programEntry = ProgramEntry.Make(installedItem, equippedShard);
        ArrayPush(entries, programEntry);
      };
      i += 1;
    };
    return entries;
