...
public class DpadWheelGameController extends inkHUDGameController {
  ...
  private let m_dpadItemsList: [wref<DpadWheelItemController>];

  private let m_commandsList: [QuickSlotCommand];
  ...
  private let m_masterListOfAllCyberware: [AbilityData];

  private let m_listOfUnassignedCyberware: [AbilityData];
  ...
  protected cb func OnWheelInteractionStarted(value: Variant) -> Bool {
    let startEvent: QuickWheelStartUIStructure = FromVariant<QuickWheelStartUIStructure>(value);
  ...
  private final func ShowDPadWheel(const eventData: script_ref<QuickWheelStartUIStructure>) -> Void {
    this.m_root.SetVisible(true);
    this.m_data = Deref(eventData);
    this.RefreshRadial();
  }
  ...
  private final func AddCommandsToList(const originalList: script_ref<[QuickSlotCommand]>, newList: [QuickSlotCommand]) -> [QuickSlotCommand] {
    let i: Int32 = 0;
    while i < ArraySize(Deref(originalList)) {
      ArrayPush(newList, Deref(originalList)[i]);
      i += 1;
    };
    return newList;
  }

  private final func SetupCommandList(const data: script_ref<[QuickSlotCommand]>) -> Void {
    this.m_commandsList = Deref(data);
    this.m_angleInterval = 360.00 / Cast<Float>(ArraySize(this.m_commandsList));
  }
  ...
  private final func CreateWheelItems() -> Void {
    ...
    while i < inkCompoundRef.GetNumChildren(this.m_itemContainer) {
      itemAngle = this.m_angleInterval * Cast<Float>(i);
  ...
  private final func CreateHashMarks() -> Void {
    ...
    while i < ArraySize(this.m_commandsList) {
      ...
      itemWidget.SetRotation(this.m_angleInterval * Cast<Float>(i));
      i += 1;
    };
  ...
  private final func UpdateRotation(angleFloat: Float) -> Void {
    ...
    let tempAngle: Float;
    let emptyStruct: QuickWheelStartUIStructure = FromVariant<QuickWheelStartUIStructure>(this.GetBlackboardSystem().Get(GetAllBlackboardDefs().UI_QuickSlotsData).GetVariant(GetAllBlackboardDefs().UI_QuickSlotsData.WheelInteractionStarted));
      ...
      switch commandData.ActionType {
        ...
        case QuickSlotActionType.SelectItem:
          if Equals(commandData.equipType, gamedataEquipmentArea.CyberwareWheel) {
            ...
            if !itemData.Empty {
              if Equals(itemData.EquipmentArea, gamedataEquipmentArea.Weapon) {
                assembledString += "AMMO: " + ToString(this.m_InventoryDataManager.GetAmmoForWeaponType(itemData));
                assembledString += "\\n" + "DAMAGE TYPE: " + ToString(itemData.DamageType);
                inkTextRef.SetText(this.m_itemDesc, assembledString);
              } else {
                inkTextRef.SetText(this.m_itemDesc, itemData.Description);
              };
            };
          };
          break;
  ...
  private final func SendSelectedItemChangeEventToEntity(const data: script_ref<QuickSlotCommand>, opt currentEmpty: Bool) -> Void;
  ...
  private final func CalculatePosition(index: Int32, dist: Float) -> inkMargin {
    ...
    let itemAngle: Float = this.m_angleInterval * Cast<Float>(index);
  ...
  private final func GetNextAbility(const currentAbility: script_ref<AbilityData>, dir: Int32) -> AbilityData {
    let curAbilityIdx: Int32;
    let nextAbility: AbilityData;
    let nextIndex: Int32;
    let selectedData: AbilityData = this.m_selectedWheelItem.GetAbilityData();
    if Deref(currentAbility).Empty {
      ...
      curAbilityIdx = ArrayFindFirst(this.m_masterListOfAllCyberware, Deref(currentAbility));
  ...
  private final func CheckIfAbilityIsAssigned(const ability: script_ref<AbilityData>) -> Bool {
...
public class DpadWheelItemController extends inkLogicController {
  ...
  public final func SetupData(const data: script_ref<QuickSlotCommand>, rotation: Float, numOfWheelItems: Int32, inventoryManager: ref<InventoryDataManagerV2>, isLeft: Bool) -> Void {
  ...
  private final func SetIcon(rotation: Float) -> Void {
    ...
    switch this.m_data.ActionType {
      case QuickSlotActionType.SelectItem:
        ...
        break;
      case QuickSlotActionType.QuickHack:
        if Equals(this.m_quickHackWheelDefIcon, n"None") {
        ...
        if NotEquals(this.m_data.IconName, n"None") {
  ...
  public final func AddAbility(const abilityData: script_ref<AbilityData>) -> Void {
    if !Deref(abilityData).Empty {
      inkWidgetRef.SetVisible(this.m_abilityIcon, true);
      this.m_abilityData = Deref(abilityData);
    };
  }
