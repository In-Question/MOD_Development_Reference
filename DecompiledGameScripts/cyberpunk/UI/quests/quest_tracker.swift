...
public class QuestTrackerGameController extends inkHUDGameController {
  ...
  private edit let m_questTrackerContainer: inkWidgetRef;
  ...
  protected let m_mappinSystem: wref<MappinSystem>;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_journalManager.RegisterScriptCallback(this, n"OnObjectiveIsOptionalChanged", gameJournalListenerType.ObjectiveOptional);
  ...
  protected cb func OnStateChanges(hash: Uint32, className: CName, notifyOption: JournalNotifyOption, changeType: JournalChangeType) -> Bool {
    ...
    if NotEquals(className, n"gameJournalInternetPage") {
  ...
  private final func SortObjectiveListByTimestamp(sortedObjectives: script_ref<[wref<JournalEntry>]>) -> Void {
    let j: Int32;
    let tempVar: wref<JournalEntry>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(sortedObjectives)) {
      j = 0;
      while j < ArraySize(Deref(sortedObjectives)) - 1 {
        if this.m_journalManager.GetEntryTimestamp(Deref(sortedObjectives)[i]) < this.m_journalManager.GetEntryTimestamp(Deref(sortedObjectives)[j + 1]) {
          tempVar = Deref(sortedObjectives)[j + 1];
          Deref(sortedObjectives)[j + 1] = Deref(sortedObjectives)[i];
          Deref(sortedObjectives)[i] = tempVar;
        };
        j += 1;
      };
      i += 1;
  ...
  private final func UpdateTrackerData() -> Void {
    ...
    let z: Int32;
    filter.active = true;
    inkWidgetRef.SetVisible(this.m_questTrackerContainer, false);
    ...
    if m_trackedEntry != null {
      ...
      if m_trackedPhase != null {
        ...
        if m_trackedQuest != null {
          ...
          inkWidgetRef.SetVisible(this.m_questTrackerContainer, NotEquals(m_trackedQuest.GetTitle(this.m_journalManager), ""));
          inkTextRef.SetText(this.m_QuestTitle, m_trackedQuest.GetTitle(this.m_journalManager));
          questType = this.m_journalManager.GetQuestType(m_trackedQuest);
          ...
          inkWidgetRef.SetState(this.m_QuestTitle, isQuestType ? n"Quest" : n"Gigs");
          z = 0;
              ...
              if m_objectiveEntry != null {
                ...
                while j < inkCompoundRef.GetNumChildren(this.m_ObjectiveContainer) {
                  ...
                  if objectiveController.GetObjectiveEntry() == m_objectiveEntry {
                    objectiveController.SetData(m_objectiveEntry.GetDescription(), m_objectiveEntry == m_trackedEntry, this.m_journalManager.GetIsObjectiveOptional(m_objectiveEntry), this.m_journalManager.GetObjectiveCurrentCounter(m_objectiveEntry), this.m_journalManager.GetObjectiveTotalCounter(m_objectiveEntry), m_objectiveEntry, isQuestType);
...
public class QuestTrackerObjectiveLogicController extends inkLogicController {
  ...
  public final static func IsObjectiveEntry() -> Bool {
    return true;
  }
  ...
  public final func SetData(const objectiveTitle: script_ref<String>, isTracked: Bool, isOptional: Bool, currentCounter: Int32, totalCounter: Int32, objectiveEntry: wref<JournalQuestObjective>, isQuestType: Bool) -> Void {
