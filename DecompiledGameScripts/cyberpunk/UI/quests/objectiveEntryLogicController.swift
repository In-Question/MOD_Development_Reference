...
public class ObjectiveEntryLogicController extends inkLogicController {
  ...
  public final func SetEntryData(const data: script_ref<UIObjectiveEntryData>) -> Void {
    let stateIconTexturePart: CName;
    let stateName: CName;
    this.m_root.SetVisible(false);
    this.SetUpdated(true);
    this.StopFadeAnimation();
    this.m_state = Deref(data).m_state;
    this.m_type = Deref(data).m_type;
    this.m_isTracked = Deref(data).m_isTracked;
    this.m_isOptional = Deref(data).m_isOptional;
    if Equals(this.m_state, gameJournalEntryState.Inactive) || Equals(Deref(data).m_name, "") {
      this.NotifyForRemoval();
      return;
    };
    this.m_entryName.SetLetterCase(textLetterCase.UpperCase);
    if this.m_isOptional {
      this.m_entryName.SetText(Equals(this.m_type, UIObjectiveEntryType.Quest) ? Deref(data).m_name : GetLocalizedText(Deref(data).m_name) + Deref(data).m_counter + " [" + GetLocalizedText("UI-ScriptExports-Optional0") + "]");
    } else {
      this.m_entryName.SetText(Equals(this.m_type, UIObjectiveEntryType.Quest) ? Deref(data).m_name : GetLocalizedText(Deref(data).m_name) + Deref(data).m_counter);
    };
    stateIconTexturePart = this.GetStateIconTexturePart(this.m_state, Deref(data).m_isTracked);
    if Equals(stateIconTexturePart, n"None") {
