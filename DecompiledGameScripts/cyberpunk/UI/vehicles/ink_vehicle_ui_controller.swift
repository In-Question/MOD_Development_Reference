...
public class vehicleUIGameController extends inkHUDGameController {
  ...
  private final func CheckIfVehicleShouldTurnOn() -> Void {
    ...
    if this.m_vehicleBlackboard.GetInt(GetAllBlackboardDefs().Vehicle.VehicleState) == 2 {
      this.TurnOn();
    };
  }
...
public class speedometerLogicController extends IVehicleModuleController {
  ...
  @default(speedometerLogicController, -1)
  ...
  public final func OnSpeedValueChanged(speed: Float) -> Void {
    ...
    let multiplier: Float = GameInstance.GetStatsDataSystem(this.m_vehicle.GetGame()).GetValueFromCurve(n"vehicle_ui", velocity, n"speed_to_multiplier");
    let speedValue: Int32 = RoundMath(velocity * multiplier);
    if speedValue != this.m_currentSpeed {
      inkTextRef.SetText(this.m_speedTextWidget, IntToString(speedValue));
      this.m_currentSpeed = speedValue;
    };
  }
}
...
public class tachometerLogicController extends IVehicleModuleController {
  ...
  @default(tachometerLogicController, -1)
  private let m_currentRPM: Int32;
  ...
  public final func OnRpmValueChanged(rpmValue: Float) -> Void {
    let newRPM: Int32 = RoundMath(rpmValue);
    if newRPM == this.m_currentRPM {
      return;
    };
    if this.m_scaleX {
    ...
    inkTextRef.SetText(this.m_rpmValueWidget, IntToString(newRPM));
    this.m_currentRPM = newRPM;
  }
}
...
public class instrumentPanelLogicController extends IVehicleModuleController {
  ...
  public func RegisterCallbacks(vehicle: wref<VehicleObject>, vehBB: wref<IBlackboard>, gameController: wref<vehicleUIGameController>) -> Void {
    if IsDefined(vehBB) {
      if !IsDefined(this.m_lightStateBBConnectionId) {
        this.m_lightStateBBConnectionId = vehBB.RegisterListenerInt(GetAllBlackboardDefs().Vehicle.HeadLightMode, this, n"OnHeadLightModeChanged");
      };
  ...
  public func UnregisterCallbacks() -> Void {
    if IsDefined(this.m_vehBB) {
      if IsDefined(this.m_lightStateBBConnectionId) {
        this.m_vehBB.UnregisterListenerInt(GetAllBlackboardDefs().Vehicle.HeadLightMode, this.m_lightStateBBConnectionId);
      };
      if IsDefined(this.m_cautionStateBBConnectionId) {
  ...
  protected final func ForceUpdate() -> Void {
    this.OnHeadLightModeChanged(this.m_vehBB.GetInt(GetAllBlackboardDefs().Vehicle.HeadLightMode));
    this.OnCautionStateChanged(this.m_vehBB.GetInt(GetAllBlackboardDefs().Vehicle.DamageState));
  }

  public final func OnHeadLightModeChanged(state: Int32) -> Void {
    if state == 0 {
...
public class RadioLogicController extends IVehicleModuleController {
  ...
  public func RegisterCallbacks(vehicle: wref<VehicleObject>, vehBB: wref<IBlackboard>, gameController: wref<vehicleUIGameController>) -> Void {
    if IsDefined(vehBB) {
      if !IsDefined(this.m_radioStateBBConnectionId) {
        this.m_radioStateBBConnectionId = vehBB.RegisterListenerBool(GetAllBlackboardDefs().Vehicle.VehRadioState, this, n"OnRadioStateChanged", true);
      };
      if !IsDefined(this.m_radioNameBBConnectionId) {
        this.m_radioNameBBConnectionId = vehBB.RegisterListenerName(GetAllBlackboardDefs().Vehicle.VehRadioStationName, this, n"OnRadioNameChanged", true);
      };
      this.m_vehBB = vehBB;
    };
  }
  ...
  public func UnregisterCallbacks() -> Void {
    this.InternalUnregisterCallbacks();
  }
  ...
  private final func InternalUnregisterCallbacks() -> Void {
    if IsDefined(this.m_vehBB) {
      if IsDefined(this.m_radioStateBBConnectionId) {
...
public class analogTachLogicController extends IVehicleModuleController {
  ...
  public final func OnRpmValueChanged(rpmValue: Float) -> Void {
    let desiredRotation: Float;
    let normalizedRPM: Float = rpmValue / this.m_rpmMaxValue - 0.13;
    let minimumIdle: Float = 0.05;
    if rpmValue < 500.00 {
      normalizedRPM = 0.00;
    } else {
      if normalizedRPM < minimumIdle {
        normalizedRPM = minimumIdle;
      };
    };
    desiredRotation = normalizedRPM * (this.m_analogTachNeedleMaxRotation + AbsF(this.m_analogTachNeedleMinRotation));
...
public class analogSpeedometerLogicController extends IVehicleModuleController {
  ...
  public final func OnSpeedValueChanged(speed: Float) -> Void {
    ...
    let speedMPH: Float = Cast<Float>(RoundMath(speed * multiplier));
