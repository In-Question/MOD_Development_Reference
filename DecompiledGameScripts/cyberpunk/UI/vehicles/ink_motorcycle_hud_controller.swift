...
public class inkMotorcycleHUDGameController extends BaseVehicleHUDGameController {
  ...
  protected cb func OnRpmMaxChanged(rpmMax: Float) -> Bool {
    let rpm: Int32 = Cast<Int32>(rpmMax);
    let level: Float = Cast<Float>(rpm / this.m_rpmPerChunk);
  ...
  protected final func CheckVehicleType(const desiredType: script_ref<String>) -> Bool {
