...
public native class PanzerHUDGameController extends inkHUDGameController {
  ...
  private let m_targetIndicators: [TargetIndicatorEntry];
  ...
  private final func EvaluateUIState() -> Void {
    ...
    let vehicleState: vehicleEState = IntEnum<vehicleEState>(this.m_vehicleBlackboard.GetInt(GetAllBlackboardDefs().Vehicle.VehicleState));
  ...
  protected cb func OnLeanAngleChanged(leanAngle: Float) -> Bool {
    ...
    rotation = Rad2Deg(AtanF(forward.X, forward.Y)) - 180.00;
  ...
  protected cb func OnStatsChanged(value: Variant) -> Bool {
    ...
    inkWidgetRef.SetSize(this.healthBar, new Vector2(Cast<Float>(newX), 100.00));
  ...
  private final func EnableTargetIndicator(indicatorEntry: TargetIndicatorEntry, const targetData: script_ref<smartGunUITargetParameters>) -> Void {
    let controller: wref<PanzerSmartWeaponTargetController>;
    let indicator: wref<inkWidget> = indicatorEntry.indicator;
    indicator.SetVisible(true);
    indicator.SetMargin(new inkMargin(Deref(targetData).pos.X * 0.50, Deref(targetData).pos.Y * 0.50, 0.00, 0.00));
...
public class PanzerSmartWeaponTargetController extends inkLogicController {
  ...
  public final func SetData(const data: script_ref<smartGunUITargetParameters>) -> Void {
    let playbackOptions: inkAnimOptions;
    if Equals(Deref(data).state, gamesmartGunTargetState.Locking) {
      ...
      this.GetRootWidget().SetVisible(Equals(Deref(data).state, gamesmartGunTargetState.Locked) || Equals(Deref(data).state, gamesmartGunTargetState.Unlocking));
      this.StopAnimation();
    };
    inkTextRef.SetText(this.m_distanceText, "Dis.\\n" + SpaceFill(IntToString(RoundMath(Deref(data).distance)), 2, ESpaceFillMode.JustifyRight, "0") + " m");
  }
