...
public class hudCameraController extends inkHUDGameController {
  ...
  private edit let m_dateAndTimeHolder: inkHorizontalPanelRef;

  private edit let m_recHolder: inkCanvasRef;
  ...
  private let m_canActivateOverclock: Bool;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_canActivateOverclock = QuickHackableHelper.CanActivateOverclockedState(player);
    ...
    GameInstance.GetAudioSystem(player.GetGame()).Play(n"global_menu_hacking_close");
    GameInstance.GetAudioSystem(player.GetGame()).Play(n"ui_hacking_close");
  ...
  protected cb func OnTakeControllOverDevice(value: Variant) -> Bool {
    ...
    if ArraySize(widgets) <= 1 && !this.m_canActivateOverclock {
  ...
  protected cb func OnOverclockHudEvent(overclockEvent: ref<OverclockHudEvent>) -> Bool {
    ...
    inkWidgetRef.SetVisible(this.m_dateAndTimeHolder, !this.m_isOverclockActive);
    inkWidgetRef.SetVisible(this.m_recHolder, !this.m_isOverclockActive);
  }
  ...
  protected cb func OnZoomChange(curZoom: Float) -> Bool {
    ...
    let maxZoomLevel: Float = this.m_maxZoomLevel == 0 ? this.max_zoom_level : Cast<Float>(this.m_maxZoomLevel);
