...
public native class scannerGameController extends inkHUDGameController {
  ...
  private let m_squares: [wref<inkImage>];

  private let m_squaresFilled: [wref<inkImage>];
  ...
  protected cb func OnPlayerAttach(playerPuppet: ref<GameObject>) -> Bool {
    ...
    isFocusModeActive = this.GetPSMBlackboard(playerPuppet).GetInt(GetAllBlackboardDefs().PlayerStateMachine.Vision) == 1;
    if isFocusModeActive {
      this.OnIsEnabledChange(1);
    };
  }
  ...
  private final func ShowScanner(show: Bool) -> Void {
    ...
    if NotEquals(this.m_isShown, show) {
      ...
      if show {
        ...
        GameInstance.GetRazerChromaEffectsSystem(this.m_gameInstance).PlayAnimation(n"Scan", true);
      } else {
        ...
        GameInstance.GetRazerChromaEffectsSystem(this.m_gameInstance).StopAnimation(n"Scan");
      };
    };
  ...
  protected cb func OnObjectData(val: Variant) -> Bool {
    ...
    if !this.scanLock {
      stats = FromVariant<GameObjectScanStats>(val);
      this.curObj = stats;
    };
  }
  ...
  private final func OnScannablesChange(val: Variant) -> Void {
    let i: Int32;
    let scannables: array<ScanningTooltipElementData> = FromVariant<array<ScanningTooltipElementData>>(val);
