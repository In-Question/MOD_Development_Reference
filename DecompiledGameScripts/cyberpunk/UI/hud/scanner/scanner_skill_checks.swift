...
public class ScannerSkillCheckLogicController extends inkLogicController {
  ...
  private let m_SkillCheckObjects: [wref<inkWidget>];
  ...
  public final func Setup(const skillCheckInfo: script_ref<[UIInteractionSkillCheck]>) -> Void {
    let currLogic: ref<ScannerSkillCheckItemLogicController>;
    let currObject: wref<inkWidget>;
    let sizeSkill: Int32 = ArraySize(Deref(skillCheckInfo));
    ...
    while i < limit {
      if i < sizeSkill {
        ...
        if IsDefined(currLogic) {
          currLogic.Setup(Deref(skillCheckInfo)[i]);
        };
      } else {
...
public class ScannerSkillCheckItemLogicController extends inkLogicController {
  ...
  private let m_ConditionDataItems: [wref<inkWidget>];
  ...
  public final func Setup(const skillCheck: script_ref<UIInteractionSkillCheck>) -> Void {
    ...
    this.GetRootWidget().SetState(Deref(skillCheck).isPassed ? this.m_PassedStateName : this.m_FailedStateName);
    sizeSkill = ArraySize(Deref(skillCheck).additionalRequirements);
    ...
    while i < limit {
      if i < sizeSkill {
        ...
        if IsDefined(currLogic) {
          currLogic.Setup(Deref(skillCheck).additionalRequirements[i], Deref(skillCheck).additionalReqOperator);
        };
      } else {
  ...
  private final func ConstructName(const skillCheck: script_ref<UIInteractionSkillCheck>) -> Void {
    let hasActionName: Bool = StrLen(Deref(skillCheck).actionDisplayName) > 0;
    let skillName: String = "";
    if hasActionName {
      skillName += Deref(skillCheck).actionDisplayName + " (";
    };
    skillName += GetLocalizedText(Deref(skillCheck).skillName) + " " + ToString(Deref(skillCheck).requiredSkill);
    if hasActionName {
      skillName += ")";
    };
...
public class ScannerSkillCheckConditionDataItemLogicController extends inkLogicController {
  ...
  private let m_ConditionDescriptions: [wref<inkWidget>];
  ...
  public final func Setup(const conditionData: script_ref<ConditionData>, parentOperator: ELogicOperator) -> Void {
    let currLogic: ref<ScannerSkillCheckConditionDescriptionLogicController>;
    let currObject: wref<inkWidget>;
    let sizeSkill: Int32 = ArraySize(Deref(conditionData).requirementList);
    ...
    while i < limit {
      if i < sizeSkill {
        ...
        if IsDefined(currLogic) {
          currLogic.Setup(Deref(conditionData).requirementList[i]);
          if Deref(conditionData).requirementList[i].passed {
            numPassed += 1;
          };
        };
      } else {
    ...
    this.ConstructTexts(Deref(conditionData).conditionOperator, parentOperator, numPassed, sizeSkill);
  }
...
public class ScannerSkillCheckConditionDescriptionLogicController extends inkLogicController {
  ...
  public final func Setup(const condition: script_ref<Condition>) -> Void {
    this.GetRootWidget().SetState(Deref(condition).passed ? this.m_PassedStateName : this.m_FailedStateName);
    inkTextRef.SetLocalizedTextScript(this.m_NameRef, Deref(condition).description);
  }
}
