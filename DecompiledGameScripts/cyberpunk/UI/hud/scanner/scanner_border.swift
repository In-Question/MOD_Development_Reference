...
public class scannerBorderLogicController extends inkLogicController {
  ...
  private edit const let m_braindanceSetVisible: [inkWidgetRef];

  private edit const let m_braindanceSetHidden: [inkWidgetRef];
...
public class scannerBorderGameController extends inkProjectedHUDGameController {
  ...
  private edit const let m_deviceFluffs: [inkWidgetRef];
  ...
  public final func PlayLockAnimation(showAnim: Bool) -> Void {
    ...
    if !this.m_isExclusiveFocus {
      ...
      if !showAnim {
        this.animLockOff = this.PlayLibraryAnimation(n"LockOffAnim", idleAnimOptions);
      };
    };
  }
  ...
  protected cb func OnPSMVisionStateChanged(value: Int32) -> Bool {
    let newState: gamePSMVision = IntEnum<gamePSMVision>(value);
