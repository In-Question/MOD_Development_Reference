...
public class OnscreenMessageGameController extends inkHUDGameController {
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    if IsDefined(variant) {
      this.m_screenMessage = FromVariant<SimpleScreenMessage>(variant);
    };
  ...
  protected cb func OnScreenMessageUpdate(value: Variant) -> Bool {
    this.m_screenMessage = FromVariant<SimpleScreenMessage>(value);
    this.UpdateWidgets();
  }

  private final func UpdateWidgets() -> Void {
    this.CleanupAnimProxies();
    ...
    if this.m_screenMessage.isShown {
      ...
      this.SetTimeout(this.m_screenMessage.duration);
    } else {
      this.m_root.SetVisible(false);
    };
  }

  private final func CleanupAnimProxies() -> Void {
    if IsDefined(this.m_animProxyShow) {
      this.m_animProxyShow.UnregisterFromAllCallbacks(inkanimEventType.OnFinish);
      this.m_animProxyShow.Stop();
      this.m_animProxyShow = null;
    };
    if IsDefined(this.m_animProxyHide) {
  ...
  private final func SetTimeout(value: Float) -> Void {
    ...
    if IsDefined(this.m_animProxyTimeout) {
      this.m_animProxyTimeout.UnregisterFromAllCallbacks(inkanimEventType.OnFinish);
      this.m_animProxyTimeout.Stop();
      this.m_animProxyTimeout = null;
    };
    if value > 0.00 {
