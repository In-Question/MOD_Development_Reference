...
public class SlotMachineController extends inkLogicController {
  ...
  private edit const let m_winAnimationsID: [CName];

  private edit let m_looseAnimationID: CName;

  private edit const let m_slotWidgets: [inkWidgetRef];

  private edit const let m_imagePresets: [CName];
  ...
  private let m_slots: [wref<SlotMachineSlot>];
...
public class SlotMachineSlot extends inkLogicController {
  ...
  private edit const let m_imagesUpper: [inkImageRef];

  private edit const let m_imagesLower: [inkImageRef];

  private let m_imagePresets: [CName];

  protected cb func OnInitialize() -> Bool {
    this.m_winningRowIndex = Clamp(this.m_winningRowIndex, 0, ArraySize(this.m_imagesUpper));
  }

  public final func SetImagesPresets(const imagePresets: script_ref<[CName]>) -> Void {
    this.m_imagePresets = Deref(imagePresets);
  }
