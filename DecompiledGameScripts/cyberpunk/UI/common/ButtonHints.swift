...
public class ButtonHints extends inkLogicController {
  ...
  public final func AddButtonHint(icon: EInputKey, const label: script_ref<String>) -> Void;
  ...
  public final func AddButtonHint(action: CName, const label: script_ref<String>) -> Void {
  ...
  public final func SetInverted(isInverted: Bool) -> Void {
    if inkWidgetRef.IsValid(this.m_horizontalHolder) {
      if isInverted {
        inkWidgetRef.SetAnchorPoint(this.m_horizontalHolder, new Vector2(0.00, 1.00));
      } else {
        inkWidgetRef.SetAnchorPoint(this.m_horizontalHolder, new Vector2(1.00, 1.00));
      };
    };
  }
...
public class ButtonHintListItem extends inkLogicController {
  ...
  public final func SetData(action: CName, const label: script_ref<String>) -> Void {
    this.m_actionName = action;
    inkTextRef.SetText(this.m_label, Deref(label));
    this.m_buttonHint.SetInputAction(this.m_actionName);
  }

  public final func SetData(icon: EInputKey, const label: script_ref<String>) -> Void;
}
