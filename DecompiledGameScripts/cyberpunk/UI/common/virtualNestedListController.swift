...
public class VirutalNestedListClassifier extends inkVirtualItemTemplateClassifier {
  ...
  public func ClassifyItem(data: Variant) -> Uint32 {
    let listData: ref<VirutalNestedListData> = FromVariant<ref<IScriptable>>(data) as VirutalNestedListData;
...
public class VirtualNestedListDataView extends ScriptableDataView {
  ...
  public let m_toggledLevels: [Int32];

  public final func Setup() -> Void {
    this.m_compareBuilder = CompareBuilder.Make();
  }

  public final func SetToggledLevels(const toggledLevels: script_ref<[Int32]>, defaultCollapsed: Bool) -> Void {
    this.m_toggledLevels = Deref(toggledLevels);
  ...
  private func FilterItem(data: ref<IScriptable>) -> Bool {
    ...
    return (itemData.m_isHeader || Equals(ArrayContains(this.m_toggledLevels, itemData.m_level), this.m_defaultCollapsed)) && this.FilterItems(itemData);
  }
...
public class VirtualNestedListController extends inkVirtualListController {
  ...
  protected let m_toggledLevels: [Int32];
  ...
  public func SetData(const data: script_ref<[ref<VirutalNestedListData>]>, opt keepToggledLevels: Bool, opt sortOnce: Bool) -> Void {
    let castedData: array<ref<IScriptable>>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(data)) {
      ArrayPush(castedData, Deref(data)[i]);
      i += 1;
    };
  ...
  public final func GetToggledLevels() -> [Int32] {
    return this.m_toggledLevels;
  }
