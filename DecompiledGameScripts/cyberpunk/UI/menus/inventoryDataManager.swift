...
public class InventoryDataManager extends IScriptable {
  ...
  public final func GetLastLootedItems(count: Int32, itemsList: script_ref<[wref<gameItemData>]>) -> Void {
    let maxIdx: Int32;
    ...
    while ArraySize(Deref(itemsList)) < maxIdx {
      ArrayPush(Deref(itemsList), ArrayPop(sourceList));
    };
  }

  public final func GetItemsList(out itemsList: [wref<gameItemData>]) -> Void {
    this.m_transactionSystem.GetItemList(this.m_player, itemsList);
  }

  public final func GetItemsListByTag(tag: CName, out itemsList: [wref<gameItemData>]) -> Void {
  ...
  private final const func FillStatsList(itemData: wref<gameItemData>, const statRecords: script_ref<[wref<Stat_Record>]>, statList: script_ref<[StatViewData]>, canCompare: Bool, const compareStatRecords: script_ref<[wref<Stat_Record>]>, opt compareWithData: wref<gameItemData>) -> Void {
    ...
    if ArraySize(Deref(statList)) > 0 {
      Deref(statList)[maxValueIdx].isMaxValue = true;
    };
  }
