...
public native class gameuiMenuItemListGameController extends gameuiSaveHandlingController {
  ...
  protected final func AddMenuItem(const label: script_ref<String>, spawnEvent: CName) -> Void {
  ...
  protected final func AddMenuItem(const label: script_ref<String>, action: PauseMenuAction) -> Void {
    let data: ref<PauseMenuListItemData> = new PauseMenuListItemData();
    data.label = Deref(label);
    data.action = action;
    this.m_menuListController.PushData(data);
