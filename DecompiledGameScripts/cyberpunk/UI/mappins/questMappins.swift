...
public class QuestMappinController extends BaseQuestMappinController {
  ...
  protected final func UpdateAboveBelowVerticalRelation() -> Void {
    ...
    shouldShowVertRelation = this.GetRootWidget().IsVisible() && !this.isCurrentlyClamped && !this.nameplateVisible && distance >= MappinUtils.GetGlobalProfile().VerticalRelationVisibleRangeMin() && distance <= MappinUtils.GetGlobalProfile().VerticalRelationVisibleRangeMax();
  ...
  protected func UpdateIcon() -> Void {
    ...
    if Equals(mappinVariant, gamedataMappinVariant.GrenadeVariant) {
      ...
      if Equals(mappinVariant, gamedataMappinVariant.Zzz10_RemoteControlDrivingVariant) {
        inkImageRef.SetTexturePart(this.iconWidget, MappinUIUtils.MappinToTexturePart(mappinVariant, mappinPhase));
      } else {
    ...
    opacity = this.m_isCompletedPhase ? MappinUtils.GetGlobalProfile().CompletedPOIOpacity() : 1.00;
    scale = this.IsGPSPortal() ? MappinUtils.GetGlobalProfile().GpsPortalIconScale() : 1.00;
  ...
  protected final func IsBroken() -> Bool {
    return this.GetVisualData().m_isBroken;
  }

  protected final func IsShardRead() -> Bool {
    return IsDefined(this.GetVisualData()) && this.GetVisualData().m_isShardRead;
  }
...
public class VehicleMappinComponent extends IScriptable {
  ...
  protected cb func OnVehicleSummonStateChanged(value: Uint32) -> Bool {
    let summonState: vehicleSummonState = IntEnum<vehicleSummonState>(value);
