    ...
    if mappin.IsQuestMappin() {
      ...
      if IsDefined(questMappin) {
        if questMappin.IsUIAnimation() {
          ...
          if ResRef.IsValid(questAnimationRecord.WidgetResource()) && NotEquals(questAnimationRecord.AnimationName(), n"None") && IsDefined(questAnimationRecord.ProfileHandle()) {
