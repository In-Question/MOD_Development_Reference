...
public class InventoryItemDisplay extends BaseButtonView {
  ...
  protected let m_AttachementsDisplay: [wref<InventoryItemAttachmentDisplay>];
  ...
  public func Setup(const itemData: script_ref<InventoryItemData>, opt ownerEntity: ref<Entity>) -> Void {
  ...
  protected func UpdateIcon() -> Void {
    ...
    iconNameStr = InventoryItemData.GetIconPath(this.m_ItemData);
    iconName = StringToName(iconNameStr);
    if !IsStringValid(iconNameStr) || !inkImageRef.IsTexturePartExist(this.m_IconImage, iconName) {
      inkWidgetRef.SetVisible(this.m_IconImage, false);
  ...
  protected func ShowMods(const attachements: script_ref<[ref<InventoryItemAttachments>]>) -> Void {
    let count: Int32;
    ...
    count = ArraySize(Deref(attachements));
    ...
    i = 0;
    while i < count {
      this.m_AttachementsDisplay[i].Setup(Deref(attachements)[i].ItemData);
      i += 1;
    };
  }
...
public class InventoryItemModSlotDisplay extends inkLogicController {
  ...
  public final func Setup(const inventoryItemData: script_ref<InventoryItemData>) -> Void {
    ...
    let quality: CName;
    let isEmpty: Bool = InventoryItemData.IsEmpty(inventoryItemData);
  ...
  public final func Setup(modInfo: ref<UIInventoryItemMod>) -> Void {
    let quality: CName;
    let attachmentInfo: ref<UIInventoryItemModAttachment> = modInfo as UIInventoryItemModAttachment;
    inkWidgetRef.SetVisible(this.m_slotBackground, !attachmentInfo.IsEmpty);
    inkWidgetRef.SetVisible(this.m_slotBorder, attachmentInfo.IsEmpty);
...
public class InventoryItemAttachmentDisplay extends inkLogicController {
  ...
  public final func Setup(const itemData: script_ref<InventoryItemData>) -> Void {
  ...
  public final func Mark(marked: Bool) -> Void {
    let stateName: CName = marked ? this.m_MarkedStateName : n"Default";
