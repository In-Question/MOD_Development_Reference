...
public class InventoryItemsList extends inkLogicController {
  ...
  public final func ShowInventory(const items: script_ref<[wref<gameItemData>]>) -> Void {
    let currObject: wref<inkWidget>;
    let i: Int32 = 0;
    let sizeItems: Int32 = ArraySize(Deref(items));
    ...
    while i < limit {
      if i < sizeItems {
        ...
        this.SetupItemDisplay(currObject, Deref(items)[i]);
      } else {
        this.DeleteItemDisplay(ArrayPop(this.m_InventoryItems));
      };
      i += 1;
  ...
  public final func GetTooltipsData() -> [ref<ATooltipData>] {
  ...
  private final func RefreshTooltips(const tooltipItemData: script_ref<InventoryItemData>, const equippedItemData: script_ref<InventoryItemData>) -> Void {
