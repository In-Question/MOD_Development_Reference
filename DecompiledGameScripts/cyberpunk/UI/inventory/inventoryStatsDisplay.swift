...
public class InventoryStatsDisplay extends inkLogicController {
  ...
  private let m_StatItems: [wref<InventoryStatItemV2>];

  public final func Setup(const stats: script_ref<[StatViewData]>) -> Void {
    let i: Int32;
    let statView: wref<InventoryStatItemV2>;
    let limit: Int32 = ArraySize(Deref(stats));
    ...
    while i < limit {
      ...
      if IsDefined(statView) {
        statView.Setup(Deref(stats)[i], 6 + i);
      };
      i += 1;
    };
  }
...
public class InventoryStatItemV2 extends inkLogicController {
  ...
  public final func Setup(const statViewData: script_ref<StatViewData>, framesDelay: Int32) -> Void {
    this.Setup(Deref(statViewData).statName, Deref(statViewData).value, Deref(statViewData).type);
  ...
  public final func Setup(const statViewData: script_ref<StatViewData>) -> Void {
    this.Setup(Deref(statViewData).statName, Deref(statViewData).value, Deref(statViewData).type);
  }

  public final func Setup(scannerStatDetails: ScannerStatDetails) -> Void {
    this.Setup("", Cast<Int32>(scannerStatDetails.value), scannerStatDetails.statType);
  }

  public final func Setup(const statName: script_ref<String>, statValue: Int32, statType: gamedataStatType) -> Void {
    inkTextRef.SetLetterCase(this.m_LabelRef, textLetterCase.UpperCase);
    inkTextRef.SetText(this.m_LabelRef, Deref(statName));
  ...
  private final func PlayIntroAnimation(framesDelay: Int32) -> Void {
    ...
    scaleInterp.SetStartDelay(0.03 * Cast<Float>(framesDelay));
    ...
    alphaInterp.SetStartDelay(0.03 * Cast<Float>(framesDelay));
