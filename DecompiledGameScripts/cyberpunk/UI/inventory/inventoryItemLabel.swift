...
public class ItemLabelContainerController extends inkLogicController {
  ...
  protected let m_items: [wref<ItemLabelController>];
  ...
  public final func Add(type: ItemLabelType, opt params: String) -> Void {
    ...
    if Equals(type, ItemLabelType.New) {
      widget = this.SpawnFromLocal(root, n"itemLabelNew");
    } else {
      if Equals(type, ItemLabelType.Owned) {
        widget = this.SpawnFromLocal(root, n"itemLabelOwned");
      } else {
