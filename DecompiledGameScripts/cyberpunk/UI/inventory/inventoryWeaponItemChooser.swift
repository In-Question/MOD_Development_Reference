...
public class InventoryWeaponItemChooser extends InventoryGenericItemChooser {
  ...
  protected func ForceDisplayLabel() -> Bool {
    let itemParts: array<ref<InventoryItemAttachments>> = InventoryUtils.GetParts(this.itemDisplay.GetItemData());
    return ArraySize(itemParts) > 0;
  }

  private final func UpdateModsLabel(const parts: script_ref<[ref<InventoryItemAttachments>]>) -> Void {
    let attachmentsCounter: Int32;
    let hasMods: Bool;
    let hasParts: Bool;
    let attachments: array<ref<InventoryItemAttachments>> = InventoryItemData.GetAttachments(this.itemDisplay.GetItemData());
    let i: Int32 = 0;
    let limit: Int32 = ArraySize(attachments);
    while i < limit {
    ...
    hasMods = attachmentsCounter - ArraySize(Deref(parts)) > 0;
    hasParts = ArraySize(Deref(parts)) > 0;
  ...
  private final func GetAllPartsTypes() -> [WeaponPartType] {
  ...
  private final func GetPartDataByType(const parts: script_ref<[ref<InventoryItemAttachments>]>, type: WeaponPartType) -> ref<InventoryItemAttachments> {
    let emptyResult: ref<InventoryItemAttachments>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(parts)) {
      if Equals(InventoryUtils.GetPartType(Deref(parts)[i]), type) {
        return Deref(parts)[i];
      };
      i += 1;
    };
    return emptyResult;
  ...
  protected func RebuildParts() -> Void {
    let i: Int32;
    let j: Int32;
    let part: ref<InventoryItemAttachments>;
    ...
    let itemParts: array<ref<InventoryItemAttachments>> = InventoryUtils.GetParts(inventoryItemData);
