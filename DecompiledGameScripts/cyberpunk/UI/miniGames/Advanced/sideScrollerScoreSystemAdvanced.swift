...
public class SideScrollerMiniGameScoreSystemAdvanced extends ScriptableSystem {
  ...
  private let scoreData: [Int32; 3];

  private let gameNames: [String; 3];
  ...
  private final const func GetGameId(const gameName: script_ref<String>) -> Int32 {
    let i: Int32 = 0;
  ...
  private final func OnSendScore(request: ref<SendScoreRequestAdvanced>) -> Void {
    ...
    if id >= 0 {
      score = Cast<Int32>(request.m_gameState.GetScore());
      if this.scoreData[id] < score {
        this.scoreData[id] = score;
      };
      request.m_gameState.SetMaxScore(Cast<Uint32>(this.scoreData[id]));
    } else {
      request.m_gameState.SetMaxScore(0u);
    };
  }

  public final const func GetMaxScore(const gameName: script_ref<String>) -> Int32 {
