...
public native class HackingMinigameGameController extends inkGameController {
  ...
  private edit let m_isAlternativeVariant: Bool;

  private edit let m_helpInputHintRef: inkWidgetRef;
  ...
  private let m_isOfficerBreach: Bool;

  private let m_isRemoteBreach: Bool;

  private let m_isItemBreach: Bool;

  private let m_numberAttempts: Int32;
  ...
  public native let minigameDefaultsTDBID: TweakDBID;

  public final native func GetProgramsChains() -> [CharactersChain];

  public final native func GetUnlockablePrograms() -> [UnlockableProgram];
  ...
  public final native func GetPlayerPrograms() -> [MinigameProgramData];

  public final native func SetTrapIconAtCell(cellCoordinates: Vector2, trap: CName) -> Void;

  public final native func AddUnlockableProgram(program: UnlockableProgram, instruction: [Uint32]) -> Void;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    networkID = FromVariant<TweakDBID>(bbNetwork.GetVariant(GetAllBlackboardDefs().NetworkBlackboard.NetworkTDBID));
    ...
    powerLevel = GameInstance.GetStatsSystem((entity as GameObject).GetGame()).GetStatValue(Cast<StatsObjectID>(entity.GetEntityID()), gamedataStatType.PowerLevel);
    ...
    if bbNetwork.GetBool(GetAllBlackboardDefs().NetworkBlackboard.ItemBreach) {
      ...
      if bbNetwork.GetBool(GetAllBlackboardDefs().NetworkBlackboard.SuicideBreach) {
        ...
        if this.m_isOfficerBreach {
          ...
          minigameDef = FromVariant<TweakDBID>(bbNetwork.GetVariant(GetAllBlackboardDefs().NetworkBlackboard.MinigameDef));
          if TDBID.IsValid(minigameDef) {
            this.m_miniGameRecord = TweakDBInterface.GetMinigame_DefRecord(minigameDef);
          } else {
            this.m_miniGameRecord = TweakDBInterface.GetMinigame_DefRecord(this.minigameDefaultsTDBID);
          };
        };
      };
    };
    ...
    if this.m_miniGameRecord.GridSize() != 0 {
      ...
      minigameData.gridSize = Cast<Uint32>(this.m_dimension);
    } else {
    ...
    if this.m_miniGameRecord.BufferSize() != 0 {
      minigameData.bufferSize = Cast<Uint32>(this.m_miniGameRecord.BufferSize());
    } else {
      this.ScaleBuffer(entity, bufferSize, powerLevel + extraDifficulty, player);
      minigameData.bufferSize = Cast<Uint32>(bufferSize);
    };
    ...
    if ArraySize(overrideProgramList) > 0 {
      ...
      scalingProgramsRule.SetBufferSize(Cast<Int32>(minigameData.bufferSize));
    ...
    if inkWidgetRef.IsValid(this.m_helpInputHintRef) {
      inkWidgetRef.SetVisible(this.m_helpInputHintRef, !this.m_isTutorialActive);
    };
  ...
  private final func ToggleTutorialOverlay(value: Bool) -> Void {
    ...
    if value && !this.m_isAlternativeVariant && !this.m_isTutorialActive {
  ...
  public final func ScaleTraps(entity: ref<Entity>, player: ref<PlayerPuppet>, out prob: Float, powerLevel: Float) -> Void {
    let difficulty: EGameplayChallengeLevel = (entity as Device).GetDevicePS().GetBackdoorAccessPoint().GetSkillCheckContainer().GetHackingSlot().GetDifficulty();
    powerLevel -= GameInstance.GetStatsSystem(player.GetGame()).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.PowerLevel);
    switch difficulty {
      case EGameplayChallengeLevel.NONE:
        powerLevel -= 10.00;
        break;
  ...
  public final func ScaleTimer(player: ref<PlayerPuppet>, entity: ref<Entity>, out time: Float, powerLevel: Float, out waitInteraction: Bool) -> Void {
    ...
    statValue = GameInstance.GetStatsSystem(player.GetGame()).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.MinigameTimeLimitExtension);
    if statValue != 0.00 {
      time *= statValue;
    };
    ...
    if this.m_numberAttempts > 1 {
      tempMultiplier = 1.00 / (Cast<Float>(this.m_numberAttempts - 1) * 1.33);
  ...
  public final func FilterTraps(trapList: script_ref<[ref<MiniGame_Trap_Record>]>) -> Void {
    let i: Int32 = ArraySize(Deref(trapList)) - 1;
    while i >= 0 {
      if this.m_isOfficerBreach && Equals(Deref(trapList)[i].TrapType().Type(), gamedataMinigameTrapType.Device) {
        ArrayErase(Deref(trapList), i);
      } else {
        if !this.m_isOfficerBreach && Equals(Deref(trapList)[i].TrapType().Type(), gamedataMinigameTrapType.NPC) {
          ArrayErase(Deref(trapList), i);
        };
      };
      i -= 1;
  ...
  public final func ScaleBuffer(entity: ref<Entity>, out size: Float, powerLevel: Float, player: ref<PlayerPuppet>) -> Void {
    ...
    let difficulty: EGameplayChallengeLevel = (entity as Device).GetDevicePS().GetBackdoorAccessPoint().GetSkillCheckContainer().GetHackingSlot().GetDifficulty();
    let baseStatValue: Float = GameInstance.GetStatsSystem(player.GetGame()).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.BufferSize);
    size = baseStatValue;
    statValue += Cast<Float>(player.GetBufferModifier());
    statValue2 = GameInstance.GetStatsSystem(player.GetGame()).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.MinigameBufferExtension);
...
public class MinigameGenerationRuleOverridePrograms extends MinigameGenerationRule {
  ...
  public final func ConvertToUint(const arr: script_ref<[Int32]>) -> [Uint32] {
    let arr2: array<Uint32>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(arr)) {
      ArrayPush(arr2, Cast<Uint32>(Deref(arr)[i]));
      i += 1;
    };
    return arr2;
  }
...
public class MinigameGenerationRuleScalingPrograms extends MinigameGenerationRule {
  ...
  public final func FilterPlayerPrograms(programs: script_ref<[MinigameProgramData]>) -> Void {
    ...
    while i >= 0 {
      ...
      if !IsNameValid(Deref(programs)[i].programName) || Equals(Deref(programs)[i].programName, n"None") {
        ...
        if Equals(this.m_entity.GetClassName(), n"VendingMachine") && Deref(programs)[i].actionID == t"MinigameAction.NetworkDataMineLootAllMaster" {
          ArrayErase(Deref(programs), i);
        };
        if !this.m_isRemoteBreach && NotEquals(miniGameActionRecord.Type().Type(), gamedataMinigameActionType.AccessPoint) {
          ArrayErase(Deref(programs), i);
        } else {
          if this.m_isRemoteBreach && Equals(miniGameActionRecord.Type().Type(), gamedataMinigameActionType.AccessPoint) {
            ArrayErase(Deref(programs), i);
          } else {
            if Equals(miniGameActionRecord.Category().Type(), gamedataMinigameCategory.CameraAccess) && !data.surveillanceCamera {
              ArrayErase(Deref(programs), i);
            } else {
              if Equals(miniGameActionRecord.Category().Type(), gamedataMinigameCategory.TurretAccess) && !data.securityTurret {
                ArrayErase(Deref(programs), i);
              } else {
                if Equals(miniGameActionRecord.Type().Type(), gamedataMinigameActionType.NPC) && !data.puppet {
                  ArrayErase(Deref(programs), i);
                };
              };
            };
  ...
  public final func GenerateRarities(length: Int32, const overlap: script_ref<[Overlap]>, id: Int32) -> [Uint32] {
    let rand: Float;
    let rarities: array<Uint32>;
    let i: Int32 = 0;
    while i < length {
      rand = RandRangeF(0.00, 1.00);
      ArrayPush(rarities, Cast<Uint32>(this.minigameController.GetRarity(rand)));
      i += 1;
    };
    i = 0;
    while i < ArraySize(Deref(overlap)) {
      if Deref(overlap)[i].instructionNumber == id {
        if Deref(overlap)[i].atStart {
          rarities[0] = Cast<Uint32>(Deref(overlap)[i].rarity);
        } else {
          rarities[ArraySize(rarities) - 1] = Cast<Uint32>(Deref(overlap)[i].rarity);
        };
      };
      if Deref(overlap)[i].otherInstruction == id {
        if Deref(overlap)[i].atStart {
          rarities[ArraySize(rarities) - 1] = Cast<Uint32>(Deref(overlap)[i].rarity);
        } else {
          rarities[0] = Cast<Uint32>(Deref(overlap)[i].rarity);
  ...
  public final func DefineLength(combinedPowerLevel: Float, bufferSize: Int32, numPrograms: Int32) -> Int32 {
    ...
    normalizedLevel = 2.00 + ((combinedPowerLevel - min) * 3.00) / (max - min);
    length = Cast<Int32>(normalizedLevel);
    perkModifierLevel = GameInstance.GetStatsSystem(this.m_player.GetGame()).GetStatValue(Cast<StatsObjectID>(this.m_player.GetEntityID()), gamedataStatType.ShorterChains);
