...
public native class QuadRacerLogicController extends MinigameLogicController {
  ...
  protected func OnGameStateUpdateLogic(gameStateUpdateEvent: ref<MiniGameStateUpdateEvent>) -> Void {
    ...
    let secondsLeft: Int32 = Cast<Int32>(gameState.timeLeft);
    ...
    if inkWidgetRef.IsValid(this.m_speedText) {
      inkTextRef.SetText(this.m_speedText, IntToString(Cast<Int32>(gameState.speed / this.m_speedCoeficient)) + " MpH");
    };
    if secondsLeft <= 0 {
  ...
  private final func PlayNotificationAnimation(const text: script_ref<String>) -> Void {
    ...
    inkTextRef.SetText(this.m_notificationText, Deref(text));
  }
