...
public class JournalWrapper extends ABaseWrapper {
  ...
  private let m_listOfJournalEntries: [wref<JournalEntry>];
  ...
  public final func GetQuests(out entries: [wref<JournalEntry>]) -> Void {
    this.m_journalManager.GetQuests(this.m_journalContext, entries);
  }

  private final func BuildQuestData_Recursive(currEntity: wref<JournalEntry>, out description: String, out questObjectives: [ref<QuestObjectiveWrapper>], out links: [wref<JournalEntry>], foundTracked: Bool) -> Void {
  ...
  public final func BuildQuestData(currQuest: wref<JournalQuest>) -> ref<QuestDataWrapper> {
    ...
    questData.Init(this.m_journalManager, currQuest, title, descriptionString, links, questObjectives, this.m_journalManager.GetEntryState(currQuest), foundTracked, this.m_journalManager.GetEntryHash(currQuest), recommendedLevel, !this.IsVisited(currQuest), this.m_journalManager.GetDistrict(currQuest));
    return questData;
  }
  ...
  public final func GetDescriptionForCodexEntry(entry: wref<JournalCodexEntry>, out result: [wref<JournalEntry>]) -> Void {
...
public class QuestDataWrapper extends AJournalEntryWrapper {
  ...
  private let m_questObjectives: [ref<QuestObjectiveWrapper>];

  private let m_links: [wref<JournalEntry>];
  ...
  public final func Init(journalManager: wref<JournalManager>, currQuest: wref<JournalQuest>, const title: script_ref<String>, const description: script_ref<String>, const links: script_ref<[wref<JournalEntry>]>, const questObjectives: script_ref<[ref<QuestObjectiveWrapper>]>, questStatus: gameJournalEntryState, isTracked: Bool, uniqueId: Int32, recommendedLevel: Int32, isNew: Bool, district: ref<District_Record>) -> Void {
    this.SetUniqueId(uniqueId);
    this.m_quest = currQuest;
    this.m_title = Deref(title);
    this.m_description = Deref(description);
    this.m_links = Deref(links);
    this.m_questObjectives = Deref(questObjectives);
  ...
  public final func GetLinks() -> [wref<JournalEntry>] {
    return this.m_links;
  }

  public final func GetObjectives() -> [ref<QuestObjectiveWrapper>] {
  ...
  public final func IsOptional() -> Bool {
    ...
    while i < limit {
      currObjective = this.m_questObjectives[i];
      if this.m_journalManager.GetIsObjectiveOptional(currObjective.GetQuestObjective()) {
        return true;
      };
      i += 1;
    };
...
public class QuestObjectiveWrapper extends ABaseQuestObjectiveWrapper {
  ...
  private let m_questSubObjectives: [ref<QuestSubObjectiveWrapper>];
  ...
  public final func GetSubObjectives() -> [ref<QuestSubObjectiveWrapper>] {
