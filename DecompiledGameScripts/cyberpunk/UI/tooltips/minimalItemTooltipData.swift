...
public class MinimalItemTooltipData extends ATooltipData {
  ...
  @default(MinimalItemTooltipData, -1.0f)
  ...
  public let attributeAllocationStats: [ref<MinimalItemTooltipStatData>];

  public let stats: [ref<MinimalItemTooltipStatData>];

  public let mods: [ref<MinimalItemTooltipModData>];

  public let dedicatedMods: [ref<MinimalItemTooltipModAttachmentData>];
  ...
  public let itemPerkGroup: gamedataPerkWeaponGroupType;
  ...
  public let hasRarity: Bool;

  public let displayContextData: ref<ItemDisplayContextData>;
  ...
  public final static func FromInventoryTooltipData(tooltipData: ref<InventoryTooltipData>) -> ref<MinimalItemTooltipData> {
    ...
    let manager: wref<UIInventoryItemsManager>;
    let result: ref<MinimalItemTooltipData>;
    let scopeAttachmentSlot: ref<InventoryItemAttachments>;
    let silencerAttachmentSlot: ref<InventoryItemAttachments>;
    ...
    if Equals(result.equipmentArea, gamedataEquipmentArea.Weapon) || RPGManager.IsItemTypeCyberwareWeapon(result.itemType) {
      i = 0;
      ...
      if Equals(result.itemCategory, gamedataItemCategory.Clothing) || Equals(result.itemCategory, gamedataItemCategory.Cyberware) {
        i = 0;
        while i < ArraySize(tooltipData.primaryStats) {
    ...
    result.attributeAllocationStats = MinimalItemTooltipData.GetAttributeAllocationStatsFromTooltipData(tooltipData, true);
  ...
  public final static func GetSecondaryStatsFromTooltipData(data: ref<InventoryTooltipData>, itemCategory: gamedataItemCategory, opt filterZero: Bool) -> [ref<MinimalItemTooltipStatData>] {
    ...
    while i < ArraySize(data.additionalStats) {
      if !ArrayContains(pushedStats, data.additionalStats[i].statType) {
        statsTweakID = TDBID.Create("BaseStats." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(data.additionalStats[i].statType))));
        roundValue = TweakDBInterface.GetBool(statsTweakID + t".roundValue", false);
        currentValue = data.additionalStats[i].currentValueF;
        if filterZero && roundValue ? RoundF(currentValue) <= 0 : AbsF(currentValue) <= 0.00 {
        } else {
          if NotEquals(itemCategory, gamedataItemCategory.Part) && NotEquals(itemCategory, gamedataItemCategory.WeaponMod) && Equals(data.additionalStats[i].statType, gamedataStatType.Armor) {
          } else {
            if Equals(data.itemType, gamedataItemType.Gad_Grenade) {
              if MinimalItemTooltipData.ShouldFilterOutGrenadeStat(data.additionalStats[i].statType) {
                ...
                if MinimalItemTooltipData.IsAttributeAllocationStat(data.additionalStats[i].statType) {
                } else {
    ...
    while i < ArraySize(data.comparedStats) {
      if !ArrayContains(pushedStats, data.comparedStats[i].statType) {
        statsTweakID = TDBID.Create("BaseStats." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(data.comparedStats[i].statType))));
        roundValue = TweakDBInterface.GetBool(statsTweakID + t".roundValue", false);
        currentValue = data.comparedStats[i].currentValueF;
        if NotEquals(itemCategory, gamedataItemCategory.Part) && NotEquals(itemCategory, gamedataItemCategory.WeaponMod) && Equals(data.comparedStats[i].statType, gamedataStatType.Armor) {
        } else {
          if Equals(data.itemType, gamedataItemType.Gad_Grenade) {
  ...
  public final static func GetAttributeAllocationStatsFromTooltipData(data: ref<InventoryTooltipData>, opt filterZero: Bool) -> [ref<MinimalItemTooltipStatData>] {
    let currentValue: Float;
    let pushedStats: array<gamedataStatType>;
    let result: array<ref<MinimalItemTooltipStatData>>;
    let roundValue: Bool;
    ...
    i = 0;
    while i < ArraySize(data.comparedStats) {
      if !MinimalItemTooltipData.IsAttributeAllocationStat(data.comparedStats[i].statType) {
      } else {
        if !ArrayContains(pushedStats, data.comparedStats[i].statType) {
          statsTweakID = TDBID.Create("BaseStats." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(data.comparedStats[i].statType))));
          roundValue = TweakDBInterface.GetBool(statsTweakID + t".roundValue", false);
  ...
  public final static func FillModsFromTooltipData(data: ref<InventoryTooltipData>, mods: script_ref<[ref<MinimalItemTooltipModData>]>, dedicatedMods: script_ref<[ref<MinimalItemTooltipModAttachmentData>]>) -> Void {
    ...
    while i < ArraySize(packages) {
      ArrayPush(Deref(mods), packages[i]);
      i += 1;
    };
  ...
  public final static func GetModsDataPackages(itemData: wref<gameItemData>, itemRecord: wref<Item_Record>, displayContext: InventoryTooltipDisplayContext, opt parentItemData: wref<gameItemData>, opt slotID: TweakDBID, mods: script_ref<[ref<MinimalItemTooltipModData>]>) -> Void {
    ...
    let attunementStatRecord: ref<Stat_Record> = TweakDBInterface.GetStatRecord(t"BaseStats.AttunementHelper");
    itemRecord.OnEquip(dataPackages);
    i = 0;
    while i < ArraySize(dataPackages) {
      uiDataPackage = dataPackages[i].UIData();
      if IsDefined(uiDataPackage) && IsStringValid(uiDataPackage.LocalizedDescription()) {
    ...
    while i < ArraySize(dataPackagesToDisplay) {
      ...
      ArrayPush(Deref(mods), recordData);
      i += 1;
    };
  }

  private final static func GetDefaultModAttachmentData(isEmpty: Bool, qualityName: CName, slotID: TweakDBID, const itemName: script_ref<String>, abilities: script_ref<[InventoryItemAbility]>, type: InventoryItemAttachmentType) -> ref<MinimalItemTooltipModAttachmentData> {
    let attachmentData: ref<MinimalItemTooltipModAttachmentData>;
    ...
    if attachmentData.isEmpty {
      ...
      if attachmentData.abilitiesSize == 0 {
        attachmentData.slotName = Deref(itemName);
      } else {
  ...
  public final static func GetMinimalTooltipDataRequirements(itemData: wref<gameItemData>, owner: wref<GameObject>, hasSmartlink: Bool, playerStrength: Int32, playerReflex: Int32, playerLevel: Int32) -> ref<MinimalItemTooltipDataRequirements> {
    ...
    let requiredStrength: Int32 = Cast<Int32>(itemData.GetStatValueByType(gamedataStatType.Strength));
    let requiredReflex: Int32 = Cast<Int32>(itemData.GetStatValueByType(gamedataStatType.Reflexes));
    let requiredLevel: Int32 = Cast<Int32>(itemData.GetStatValueByType(gamedataStatType.Level));
  ...
  public final static func GetMinimalTooltipDataRequirements(itemData: wref<gameItemData>, tooltipData: ref<InventoryTooltipData>) -> ref<MinimalItemTooltipDataRequirements> {
    ...
    let resultStat: ref<MinimalItemTooltipDataStatRequirement>;
    ...
    if !InventoryItemData.IsEmpty(tooltipData.inventoryItemData) {
      ...
      if NotEquals(requirement.statType, gamedataStatType.Invalid) && !InventoryItemData.IsRequirementMet(tooltipData.inventoryItemData) {
        ...
        resultStat.statColor = "StatTypeColor." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(requirement.statType)));
        resultStat.statLocKey = "LocKey#49215";
        ArrayPush(result.anyStatRequirements, resultStat);
      };
      player = tooltipData.GetManager().GetAttachedPlayer();
      if IsDefined(player) && tooltipData.isPerkRequired && RPGManager.CheckPerkPrereqs(itemData, player, perkRequiredName) {
        result.isPerkRequirementNotMet = true;
        result.perkLocKey = tooltipData.perkRequiredName;
      };
      if Equals(InventoryItemData.GetItemType(tooltipData.inventoryItemData), gamedataItemType.Cyb_HealingAbility) && Equals(tooltipData.category, "Cyberware") {
        requirements = InventoryItemData.GetEquipRequirements(tooltipData.inventoryItemData);
        i = 0;
        while i < ArraySize(requirements) {
          if Equals(requirements[i].statType, gamedataStatType.HumanityAvailable) {
            resultStat = new MinimalItemTooltipDataStatRequirement();
    ...
    if !InventoryItemData.IsEquippable(tooltipData.inventoryItemData) {
      ...
      if NotEquals(requirement.statType, gamedataStatType.Invalid) {
        ArrayPush(requirements, requirement);
      };
      i = 0;
      while i < ArraySize(requirements) {
        resultStat = new MinimalItemTooltipDataStatRequirement();
        ...
        resultStat.statName = GetLocalizedText(UILocalizationHelper.GetStatNameLockey(statRecord));
        resultStat.statColor = "StatTypeColor." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(requirements[i].statType)));
        if MinimalItemTooltipData.IsAttributeAllocationStat(requirements[i].statType) {
  ...
  public final static func GetSecondaryStatsForStatMap(itemData: wref<gameItemData>, itemType: gamedataItemType, itemCategory: gamedataItemCategory) -> [ref<MinimalItemTooltipStatData>] {
    let i: Int32;
    let limit: Int32;
    let result: array<ref<MinimalItemTooltipStatData>>;
    ...
    let statValue: Float;
    let statsMap: wref<UIStatsMap_Record> = TweakDBInterface.GetUIStatsMapRecord(TDBID.Create("UIMaps." + EnumValueToString("gamedataItemType", Cast<Int64>(EnumInt(itemType)))));
    if IsDefined(statsMap) {
      ...
      while i < limit {
        ...
        if Equals(itemCategory, gamedataItemCategory.Clothing) && Equals(statType, gamedataStatType.Armor) {
          ...
          if MinimalItemTooltipData.IsAttributeAllocationStat(statType) {
          } else {
            if Equals(itemType, gamedataItemType.Gad_Grenade) {
              if MinimalItemTooltipData.ShouldFilterOutGrenadeStat(statType) {
                ...
                if statData.roundValue ? RoundF(statValue) > 0 : AbsF(statValue) > 0.00 {
  ...
  public final func GetStatsManager(opt refetch: Bool) -> wref<UIInventoryItemStatsManager> {
    ...
    this.statsManager = UIInventoryItemStatsManager.Make(this.itemData, record, this.managerRef);
    this.statsManagerFetched = true;
    return this.statsManager;
  }

  public final func GetStatsManagerPure(opt refetch: Bool) -> wref<UIInventoryItemStatsManager> {
    return this.statsManager;
  }
}
