...
public class ItemTooltipController extends AGenericTooltipControllerWithDebug {
  ...
  protected func UpdateIcon() -> Void {
    ...
    if IsDefined(this.m_data) && IsStringValid(InventoryItemData.GetIconPath(this.m_data.inventoryItemData)) {
    ...
    if NotEquals(iconName, "None") && NotEquals(iconName, "") {
      ...
      emptyIcon = UIItemsHelper.GetSlotShadowIcon(TDBID.None(), this.GetItemType(), InventoryItemData.GetEquipmentArea(this.m_data.inventoryItemData));
  ...
  protected final func GetDamageStatsFromSecondayStats() -> [InventoryTooltipData_StatData] {
  ...
  public final func ProcessDoTEffects(effects: wref<StatusEffect_Record>) -> [ref<DamageEffectUIEntry>] {
  ...
  protected final func GetDoTEffects(attackRecord: wref<Attack_Record>) -> [ref<DamageEffectUIEntry>] {
    let i: Int32;
    let j: Int32;
  ...
  protected final func UpdateGrenadeRange(tweakRecord: ref<Grenade_Record>) -> Void {
    ...
    finalRangeDecimalPart = RoundF((finalRange - Cast<Float>(RoundF(finalRange))) * 10.00) % 10;
  ...
  protected final func UpdateAmmo() -> Void {
    if Equals(this.m_data.equipArea, gamedataEquipmentArea.Weapon) || Equals(this.m_data.equipArea, gamedataEquipmentArea.WeaponHeavy) {
      if InventoryItemData.GetGameItemData(this.m_data.inventoryItemData).HasTag(n"MeleeWeapon") {
  ...
  protected final func UpdateRarity() -> Void {
    ...
    inkTextRef.SetText(this.m_itemRarityText, isIconic ? rarityLabel + " UpdateRarity2 " + iconicLabel : rarityLabel);
  ...
  protected final func UpdateArmor() -> Void {
    ...
    while i < ArraySize(this.m_data.primaryStats) {
      if Equals(this.m_data.primaryStats[i].statType, gamedataStatType.Armor) {
        armor = this.m_data.primaryStats[i].currentValueF;
        armorDiffValue = this.m_data.primaryStats[i].diffValueF;
      };
      i += 1;
    };
    ...
    armorParams.AddNumber("valueDecimalPart", RoundF((armor - Cast<Float>(RoundF(armor))) * 10.00) % 10);
  ...
  protected final func UpdateDPS() -> Void {
    ...
    dpsParams.AddNumber("valueDecimalPart", RoundF((dps - Cast<Float>(RoundF(dps))) * 10.00) % 10);
  ...
  protected final func GetSecondaryStatsData(data: ref<InventoryTooltipData>) -> [InventoryTooltipData_StatData] {
  ...
  private final func FilterStatsWithValue(const stats: script_ref<[InventoryTooltipData_StatData]>) -> [InventoryTooltipData_StatData] {
    let result: array<InventoryTooltipData_StatData>;
    let statTweakID: TweakDBID;
    let i: Int32 = 0;
    while i < ArraySize(Deref(stats)) {
      statTweakID = TDBID.Create("BaseStats." + EnumValueToString("gamedataStatType", Cast<Int64>(EnumInt(Deref(stats)[i].statType))));
      if TweakDBInterface.GetBool(statTweakID + t".roundValue", false) {
        if Deref(stats)[i].currentValue != 0 {
          ArrayPush(result, Deref(stats)[i]);
        };
      } else {
        if AbsF(Deref(stats)[i].currentValueF) > 0.00 {
          ArrayPush(result, Deref(stats)[i]);
        };
      };
      i += 1;
  ...
  private final func ShouldDisplayGrenadeStat(const stat: script_ref<InventoryTooltipData_StatData>) -> Bool {
    return NotEquals(Deref(stat).statType, gamedataStatType.Range) && !this.IsDamageStat(Deref(stat).statType);
  }

  private final func FilterGrenadeStats(const stats: script_ref<[InventoryTooltipData_StatData]>) -> [InventoryTooltipData_StatData] {
    let i: Int32;
    let result: array<InventoryTooltipData_StatData>;
    ...
    i = 0;
    while i < ArraySize(Deref(stats)) {
      if this.ShouldDisplayGrenadeStat(Deref(stats)[i]) {
        ArrayPush(result, Deref(stats)[i]);
      };
      i += 1;
    };
  ...
  private final func FilterArmorStat(const stats: script_ref<[InventoryTooltipData_StatData]>) -> [InventoryTooltipData_StatData] {
    let result: array<InventoryTooltipData_StatData>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(stats)) {
      if NotEquals(Deref(stats)[i].statType, gamedataStatType.Armor) {
        ArrayPush(result, Deref(stats)[i]);
      };
      i += 1;
    };
  ...
  protected final func UpdateSecondaryStats() -> Void {
    ...
    if NotEquals(itemRecord.ItemCategory().Type(), gamedataItemCategory.Part) && NotEquals(itemRecord.ItemCategory().Type(), gamedataItemCategory.WeaponMod) {
  ...
  protected final func UpdateAttachments() -> Void {
    let dedicatedMods: array<ref<InventoryItemAttachments>>;
    let genericMods: array<ref<InventoryItemAttachments>>;
    let i: Int32 = 0;
  ...
  protected final func UpdateMods(const mods: script_ref<[ref<InventoryItemAttachments>]>) -> Void {
    ...
    attachmentsSize = ArraySize(Deref(mods));
    ...
    if totalModsSize > 0 {
      ...
      if attachmentsSize > 0 || dataPackagesSize > 0 {
        ...
        while i < attachmentsSize {
          ...
          controller.SetData(Deref(mods)[i]);
          i += 1;
        };
      };
  ...
  protected final func UpdateDedicatedMods(const mods: script_ref<[ref<InventoryItemAttachments>]>) -> Void {
    let controller: ref<ItemTooltipModController>;
    let modsToShow: array<ref<InventoryItemAttachments>>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(mods)) {
      if !InventoryItemData.IsEmpty(Deref(mods)[i].ItemData) {
        ArrayPush(modsToShow, Deref(mods)[i]);
      };
      i += 1;
    };
  ...
  protected final func UpdateRequirements() -> Void {
    ...
    if this.m_data.isPerkRequired {
      ...
      inkTextRef.SetLocalizedTextScript(this.m_itemAttributeRequirementsText, "LocKey#42796", textParams);
    };
    if InventoryItemData.IsPart(this.m_data.inventoryItemData) && !InventoryItemData.IsRequirementMet(this.m_data.inventoryItemData) {
      inkWidgetRef.SetVisible(this.m_itemRequirements, true);
      inkTextRef.SetLocalizedTextScript(this.m_itemAttributeRequirementsText, "LocKey#79881", textParams);
    };
  }

  protected final func UpdateDescription() -> Void {
    if IsDefined(this.m_data) && IsStringValid(this.m_data.description) {
  ...
  protected final func UpdateCraftedIcon() -> Void {
    let isCrafted: Bool;
    let showIcon: Bool = isCrafted && NotEquals(this.m_data.displayContext, InventoryTooltipDisplayContext.Crafting);
    inkWidgetRef.SetVisible(this.m_craftedItemIcon, showIcon);
    inkWidgetRef.SetVisible(this.m_craftedItemContainer, showIcon);
  }
