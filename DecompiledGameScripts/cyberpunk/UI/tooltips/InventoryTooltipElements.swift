...
public class TooltipSpecialAbilityList extends inkLogicController {
  ...
  private let m_itemsList: [wref<inkWidget>];

  private let m_data: [InventoryItemAbility];

  private let m_qualityName: CName;

  public final func SetData(qualityState: CName, const data: script_ref<[InventoryItemAbility]>) -> Void {
...
public class TooltipSpecialAbilityDisplay extends inkLogicController {
  ...
  public final func Setup(qualityName: CName, const data: script_ref<InventoryItemAbility>) -> Void {
    inkImageRef.SetTexturePart(this.m_AbilityIcon, Deref(data).IconPath);
    if inkWidgetRef.IsValid(this.m_AbilityDescription) {
      inkTextRef.SetText(this.m_AbilityDescription, Deref(data).Description);
    };
...
public class InventoryItemAttachmentsList extends inkLogicController {
  ...
  private let m_data: [CName];

  public final func SetData(const data: script_ref<[CName]>) -> Void {
    this.m_data = Deref(data);
    this.UpdateLayout();
  }
...
public class InventoryItemStatList extends inkLogicController {
  ...
  private let m_data: [InventoryTooltipData_StatData];
...
public class InventoryItemStatItem extends inkLogicController {
  ...
  public final func SetData(const data: script_ref<InventoryTooltipData_StatData>) -> Void {
    let stateName: CName = n"Default";
    let diffValue: Int32 = Deref(data).diffValue;
    if diffValue != 0 {
      stateName = diffValue > 0 ? this.m_positiveState : this.m_negativeState;
    };
    if NotEquals(Deref(data).state, EInventoryDataStatDisplayType.Value) {
      inkTextRef.SetText(this.m_valueText, IntToString(Deref(data).currentValue));
    ...
    inkTextRef.SetText(this.m_labelText, Deref(data).statName);
...
public class StatisticDifferenceBarController extends inkLogicController {
  ...
  public final func Setup(const dataObject: script_ref<InventoryTooltipData_StatData>) -> Void {
    let rMax: Float = Cast<Float>(Deref(dataObject).maxStatValue);
    let rMin: Float = Cast<Float>(Deref(dataObject).minStatValue);
    let rDiff: Float = Cast<Float>(Deref(dataObject).diffValue);
    let rValue: Float = Cast<Float>(Deref(dataObject).currentValue);
