...
public class InventorySlotTooltip extends AGenericTooltipController {
  ...
  private edit const let m_elementsToSetRarityState: [inkWidgetRef];

  private edit const let m_rarityElementsRefs: [inkImageRef];
  ...
  public final func Show(const data: script_ref<ItemViewData>) -> Void {
  ...
  private final func SetStats(statList: wref<InventoryItemStatList>, data: script_ref<[InventoryTooltipData_StatData]>) -> Void {
  ...
  private final func UpdateDescription() -> Void {
    ...
    if inkWidgetRef.IsValid(this.m_descriptionText) {
      ...
      if IsDefined(this.m_data) && IsStringValid(this.m_data.additionalDescription) {
