...
public class TransmogMessageDescTooltip extends MessageTooltip {
  ...
  public func SetData(tooltipData: ref<ATooltipData>) -> Void {
    let messageData: ref<TransmogMessageTooltipData>;
    let useNoIcon: Bool;
    super.SetData(tooltipData);
    messageData = tooltipData as TransmogMessageTooltipData;
    useNoIcon = IsDefined(messageData) && messageData.NoIcon;
    InkImageUtils.RequestSetImage(this, this.m_icon, useNoIcon ? "UIIcon.WardrobeNoAppearanceTransmog" : ToString(messageData.IconPath));
    inkWidgetRef.SetState(this.m_icon, useNoIcon ? n"NoIcon" : n"Default");
    if IsDefined(messageData) {
      inkWidgetRef.SetVisible(this.m_descriptionWrapper, NotEquals(messageData.Description, ""));
      inkWidgetRef.SetVisible(this.m_descriptionLine, NotEquals(messageData.Description, "") && NotEquals(messageData.Title, ""));
      inkWidgetRef.SetVisible(this.m_titleWrapper, NotEquals(messageData.Title, ""));
