...
public class CyberEquipGameController extends ArmorEquipGameController {
  ...
  private let eyesTags: [CName];

  private let brainTags: [CName];

  private let musculoskeletalTags: [CName];

  private let nervousTags: [CName];

  private let cardiovascularTags: [CName];

  private let immuneTags: [CName];

  private let integumentaryTags: [CName];

  private let handsTags: [CName];

  private let armsTags: [CName];

  private let legsTags: [CName];

  private let quickSlotTags: [CName];

  private let weaponsQuickSlotTags: [CName];

  private let fragmentTags: [CName];
  ...
  private final func CreateButton(const title: script_ref<String>, const btnPath: script_ref<String>, area: gamedataEquipmentArea, numSlots: Int32) -> Void {
  ...
  protected final func RemovedEverythingButCyberware(const items: script_ref<[wref<gameItemData>]>) -> [wref<gameItemData>] {
    let validItems: array<wref<gameItemData>>;
    let i: Int32 = 0;
