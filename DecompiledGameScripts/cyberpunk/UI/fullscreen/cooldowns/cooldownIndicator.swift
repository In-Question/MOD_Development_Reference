...
public native class inkCooldownGameController extends inkGameController {
  ...
  private edit let m_cooldownTitle: inkWidgetRef;
  ...
  private let m_effectTypes: [gamedataStatusEffectType];

  private let m_cooldownPool: [wref<SingleCooldownManager>];

  private let m_matchBuffer: [wref<SingleCooldownManager>];
  ...
  private final func ParseBuffList(const buffList: script_ref<[UIBuffInfo]>) -> Void {
    ...
    j = 0;
    while j < ArraySize(Deref(buffList)) {
      foundIndex = -1;
      i = 0;
      while i < ArraySize(this.m_matchBuffer) {
        if IsDefined(this.m_matchBuffer[i]) && this.m_matchBuffer[i].IsIDMatch(Deref(buffList)[j].buffID) {
          foundIndex = i;
          this.m_matchBuffer[foundIndex].Update(Deref(buffList)[j].timeRemaining, Deref(buffList)[j].stackCount);
      ...
      if foundIndex < 0 {
        this.RequestCooldownVisualization(Deref(buffList)[j]);
      };
      j += 1;
    };
    i = 0;
  ...
  private final const func GetBuffs(buffs: script_ref<[BuffInfo]>) -> Void {
    buffs = FromVariant<array<BuffInfo>>(this.m_blackboard.GetVariant(this.m_blackboardDef.BuffsList));
  }

  private final const func GetDebuffs(debuffs: script_ref<[BuffInfo]>) -> Void {
    debuffs = FromVariant<array<BuffInfo>>(this.m_blackboard.GetVariant(this.m_blackboardDef.DebuffsList));
  }

  protected cb func OnCooldownUpdate(buffList: [BuffInfo]) -> Bool;
