...
public class PaperDollSlotController extends inkButtonDpadSupportedController {
  ...
  protected let m_areaTags: [CName];
  ...
  public final func SetButtonDetails(const argText: script_ref<String>, equipArea: gamedataEquipmentArea, slotIndex: Int32, const areaTags: script_ref<[CName]>) -> Void {
    let currListIcon: wref<inkImage>;
    this.m_rootWidget = this.GetRootWidget() as inkCanvas;
    this.m_areaTags = Deref(areaTags);
  ...
  public final func GetAreaTags() -> [CName] {
...
public class ArmorEquipGameController extends gameuiMenuGameController {
  ...
  protected let m_inventory: [wref<gameItemData>];
  ...
  protected let m_recipeItemList: [TweakDBID];
  ...
  protected let m_paperDollList: [CName];

  protected let scrollOffset: Int32;

  protected let faceTags: [CName];

  protected let headTags: [CName];

  protected let chestTags: [CName];

  protected let legTags: [CName];

  protected let weaponTags: [CName];

  protected let consumableTags: [CName];

  protected let modulesTags: [CName];

  protected let framesTags: [CName];
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    ArrayPush(this.weaponTags, n"MeleeWeapon");
  ...
  protected final func HelperAddPaperdollButton(const argTitle: script_ref<String>, containerSlot: ref<inkCanvas>, equipArea: gamedataEquipmentArea, slotIndex: Int32, const areaTags: script_ref<[CName]>) -> Void {
  ...
  protected final func OnInventoryChange(value: Variant) -> Void {
    this.m_inventory = FromVariant<array<wref<gameItemData>>>(value);
  }
  ...
  protected final func RemovedCyberware(const items: script_ref<[wref<gameItemData>]>) -> [wref<gameItemData>] {
    let validItems: array<wref<gameItemData>>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(items)) {
      if !this.m_transactionSystem.HasTag(this.player, n"Cyberware", Deref(items)[i].GetID()) {
        ArrayPush(validItems, Deref(items)[i]);
      };
      i += 1;
    };
    return validItems;
  ...
  protected final func RefreshTooltipsPaperdoll(const tooltipItemData: script_ref<ItemViewData>) -> Void {
    let tooltipData: ref<InventoryTooltipData>;
    this.HideTooltips();
