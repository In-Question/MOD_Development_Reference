...
public class BackpackEquipSlotChooserPopup extends inkGameController {
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_comparisonResolver = InventoryItemPreferredComparisonResolver.Make(this.m_data.inventoryScriptableSystem);
    this.SpawnWeaponSlots(this.m_data.inventoryScriptableSystem);
    this.PlayLibraryAnimation(n"backpack_equip_chooser_intro");
  }

  private final func SpawnWeaponSlots(inventoryScriptableSystem: ref<UIInventoryScriptableSystem>) -> Void {
  ...
  protected cb func OnSlotHoverOver(e: ref<inkPointerEvent>) -> Bool {
    let controller: ref<InventoryItemDisplayController>;
    let inspectedItem: wref<UIInventoryItem>;
    ...
    if IsDefined(inspectedItem) && IsDefined(controller) {
      itemTooltipData = new UIInventoryItemTooltipWrapper();
      itemTooltipData.m_data = inspectedItem;
      this.m_tooltipsManager.ShowTooltipAtWidget(n"itemTooltip", e.GetCurrentTarget(), itemTooltipData, gameuiETooltipPlacement.RightTop);
    };
  }
