...
public class SocialPanelContactsList extends inkLogicController {
  ...
  private let m_ItemsList: [wref<SocialPanelContactsListItem>];
  ...
  public final func RefreshContactsList(const contacts: script_ref<[SocialPanelContactInfo]>) -> Void {
    let currentController: wref<SocialPanelContactsListItem>;
    let i: Int32 = 0;
    let limit: Int32 = ArraySize(Deref(contacts));
    let numItems: Int32 = 0;
    while i < limit {
      if IsDefined(Deref(contacts)[i].Contact) {
        this.AddContactItem(Deref(contacts)[i], numItems);
        numItems += 1;
      };
      i += 1;
    };
