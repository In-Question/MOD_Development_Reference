...
public class MenuHubGameController extends gameuiMenuGameController {
  ...
  private final func InitMenusData() -> Void {
    ...
    this.m_menusData = HubMenuUtility.CreateMenuData(this.m_player);
  ...
  protected cb func OnOpenMenuRequest(evt: ref<OpenMenuRequest>) -> Bool {
    ...
    let isCraftingAvailable: Bool = HubMenuUtility.IsCraftingAvailable(this.m_player);
    let isCratingMenu: Bool = Equals(evt.m_menuName, n"crafting_main");
    ...
    if NotEquals(evt.m_eventData.fullscreenName, n"world_map") {
      this.ForceResetCursorType();
    };
  }
