...
public class MetaQuestLogicController extends inkLogicController {
  ...
  public final func SetMetaQuests(const status: script_ref<MetaQuestStatus>) -> Void {
    if NotEquals(Deref(status).MetaQuest1Description, "") {
      this.m_metaQuest1Description = Deref(status).MetaQuest1Description;
    } else {
      this.m_metaQuest1Description = "[Error: No description defined by the quest block. Contact quest team.]";
    };
    if NotEquals(Deref(status).MetaQuest2Description, "") {
      this.m_metaQuest2Description = Deref(status).MetaQuest2Description;
    } else {
      this.m_metaQuest2Description = "[Error: No description defined by the quest block. Contact quest team.]";
    };
    if NotEquals(Deref(status).MetaQuest3Description, "") {
      this.m_metaQuest3Description = Deref(status).MetaQuest3Description;
    } else {
      this.m_metaQuest3Description = "[Error: No description defined by the quest block. Contact quest team.]";
    };
    inkTextRef.SetText(this.m_MetaQuest1Value, Deref(status).MetaQuest1Value + "%");
    inkTextRef.SetText(this.m_MetaQuest2Value, Deref(status).MetaQuest2Value + "%");
    inkTextRef.SetText(this.m_MetaQuest3Value, Deref(status).MetaQuest3Value + "%");
    inkWidgetRef.SetVisible(this.m_MetaQuest1, !Deref(status).MetaQuest1Hidden);
    inkWidgetRef.SetVisible(this.m_MetaQuest2, !Deref(status).MetaQuest2Hidden);
    inkWidgetRef.SetVisible(this.m_MetaQuest3, !Deref(status).MetaQuest3Hidden);
  }
  ...
  protected cb func OnItem1HoverOver(evt: ref<inkPointerEvent>) -> Bool {
    ...
    inkWidgetRef.SetMargin(this.m_MetaQuestHint, new inkMargin(-1070.00, -60.00, 0.00, 0.00));
  ...
  protected cb func OnItem2HoverOver(evt: ref<inkPointerEvent>) -> Bool {
    ...
    inkWidgetRef.SetMargin(this.m_MetaQuestHint, new inkMargin(50.00, 570.00, 0.00, 0.00));
    this.animTooltip.Stop();
