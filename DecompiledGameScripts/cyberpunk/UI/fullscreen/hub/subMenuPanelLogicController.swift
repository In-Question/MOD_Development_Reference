...
public class SubMenuPanelLogicController extends PlayerStatsUIHolder {
  ...
  private let m_menuSelectorCtrl: wref<hubStaticSelectorController>;
  ...
  protected cb func OnInitialize() -> Bool {
    this.m_menuSelectorCtrl = inkWidgetRef.GetController(this.m_menuselectorWidget) as hubStaticSelectorController;
    this.HideName(false);
  ...
  public final func SetActive(isActive: Bool, opt hideSubmenu: Bool) -> Void {
    ...
    if isActive {
      this.RegisterToGlobalInputCallback(n"OnPostOnPress", this, n"OnButtonPressed");
      this.RegisterToGlobalInputCallback(n"OnPostOnAxis", this, n"OnButtonPressed");
      this.m_menuSelectorCtrl.RegisterToCallback(n"OnSelectionChanged", this, n"OnMenuChanged");
    } else {
      this.UnregisterFromGlobalInputCallback(n"OnPostOnPress", this, n"OnButtonPressed");
      this.UnregisterFromGlobalInputCallback(n"OnPostOnAxis", this, n"OnButtonPressed");
      this.m_menuSelectorCtrl.UnregisterFromCallback(n"OnSelectionChanged", this, n"OnMenuChanged");
    };
  }

  protected cb func OnButtonPressed(evt: ref<inkPointerEvent>) -> Bool {
    if evt.IsHandled() {
      return false;
    };
