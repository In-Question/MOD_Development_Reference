...
public class RadialMenuHubGameController extends gameuiMenuGameController {

  private let m_menusData: ref<MenuDataBuilder>;

  private let m_menuEventDispatcher: wref<inkMenuEventDispatcher>;

  private let m_menuCtrl: wref<RadialMenuHubLogicController>;

  private let m_metaCtrl: wref<MetaQuestLogicController>;

  private let m_subMenuCtrl: wref<RadialSubMenuPanelLogicController>;

  private let m_timeCtrl: wref<RadialHubTimeSkipController>;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_menuCtrl = this.GetControllerByType(n"RadialMenuHubLogicController") as RadialMenuHubLogicController;
    this.m_subMenuCtrl = this.GetControllerByType(n"RadialSubMenuPanelLogicController") as RadialSubMenuPanelLogicController;
    this.m_metaCtrl = this.GetControllerByType(n"MetaQuestLogicController") as MetaQuestLogicController;
    this.m_timeCtrl = this.GetControllerByType(n"RadialHubTimeSkipController") as RadialHubTimeSkipController;
    this.InitMenusData();
    ...
    status.MetaQuest1Value = Cast<Int32>(data.percent);
    ...
    status.MetaQuest2Value = Cast<Int32>(data.percent);
    ...
    status.MetaQuest3Value = Cast<Int32>(data.percent);
    ...
    this.RegisterToGlobalInputCallback(n"OnPostOnRelative", this, n"OnGlobalPostRelative");
    this.RegisterToGlobalInputCallback(n"OnPostOnAxis", this, n"OnGlobalPostAxis");
    this.SetupBlackboards();
    TimeDilationHelper.SetTimeDilationWithProfile(this.m_player, "hubMenu", true, true);
    ...
    this.m_menuCtrl.SetWindowSize(GameInstance.GetUISystem(this.m_player.GetGame()).GetCurrentWindowSize());
  }
  ...
  protected cb func OnTimeSkipOpenedEvent(e: ref<TimeSkipOpenedEvent>) -> Bool {
    this.m_menuCtrl.SetTimeSkipOpened(true);
  }

  protected cb func OnTimeSkipClosedEvent(e: ref<TimeSkipClosedEvent>) -> Bool {
    this.m_menuCtrl.SetTimeSkipOpened(false);
  }

  protected cb func OnTimeSkipHoverOverEvent(e: ref<TimeSkipHoverOverEvent>) -> Bool {
    this.m_menuCtrl.SetButtonHoverOver(-2);
  }

  protected cb func OnTimeSkipHoverOutEvent(e: ref<TimeSkipHoverOutEvent>) -> Bool {
    this.m_menuCtrl.SetButtonHoverOut(-2);
  }

  protected cb func OnMenuButtonHoverOverEvent(e: ref<MenuButtonHoverOverEvent>) -> Bool {
    this.m_menuCtrl.SetButtonHoverOver(e.buttonId);
  }

  protected cb func OnMenuButtonHoverOutEvent(e: ref<MenuButtonHoverOutEvent>) -> Bool {
    this.m_menuCtrl.SetButtonHoverOut(e.buttonId);
  }

  protected cb func OnMetaQuestStatusUpdated(value: Variant) -> Bool {
    let status: MetaQuestStatus = FromVariant<MetaQuestStatus>(value);
    this.m_metaCtrl.SetMetaQuests(status);
  }
  ...
  protected cb func OnDropQueueUpdatedEvent(evt: ref<DropQueueUpdatedEvent>) -> Bool {
    ...
    while i < ArraySize(dropQueue) {
      ...
      result += item.GetStatValueByType(gamedataStatType.Weight) * Cast<Float>(dropQueue[i].quantity);
      i += 1;
    };
  ...
  private final func InitMenusData() -> Void {
    ...
    this.m_menusData = MenuDataBuilder.Make().Add(HubMenuItems.Inventory, HubMenuItems.Inventory, n"inventory_screen", n"ico_inventory", n"UI-PanelNames-INVENTORY").Add(HubMenuItems.Map, HubMenuItems.Map, n"world_map", n"ico_map", n"UI-PanelNames-MAP", null, Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"map_blocked"))).Add(HubMenuItems.Character, HubMenuItems.Character, n"new_perks", n"ico_character", n"UI-PanelNames-CHARACTER").Add(HubMenuItems.Journal, HubMenuItems.Journal, n"quest_log", n"ico_journal", n"UI-PanelNames-JOURNAL").Add(HubMenuItems.Codex, HubMenuItems.Codex, n"codex", n"ico_data", n"UI-PanelNames-CODEX", CodexUserData.Make(CodexDataSource.Codex)).Add(HubMenuItems.Crafting, HubMenuItems.Inventory, n"crafting_main", n"ico_cafting", n"Gameplay-RPG-Skills-CraftingName", !isCarftingAvailable).Add(HubMenuItems.Stats, HubMenuItems.Character, n"temp_stats", n"ico_stats_hub", n"UI-PanelNames-STATS").Add(HubMenuItems.Phone, HubMenuItems.Journal, n"phone", n"ico_phone", n"UI-PanelNames-PHONE").Add(HubMenuItems.Tarot, HubMenuItems.Codex, n"tarot_main", n"ico_tarot_hub", n"UI-PanelNames-TAROT", null, !Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"q101_done"))).Add(HubMenuItems.Shards, HubMenuItems.Codex, n"shards", n"ico_shards_hub", n"UI-PanelNames-SHARDS", CodexUserData.Make(CodexDataSource.Onscreen)).Add(HubMenuItems.Backpack, HubMenuItems.Inventory, n"backpack", n"ico_backpack", n"UI-PanelNames-BACKPACK").Add(HubMenuItems.Cyberware, HubMenuItems.Inventory, n"cyberware_equip", n"ico_cyberware", n"UI-PanelNames-CYBERWARE");
    this.m_menuCtrl.SetMenusData(this.m_menusData, !Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"q101_done")), Cast<Bool>(GameInstance.GetQuestsSystem(this.m_player.GetGame()).GetFact(n"map_blocked")), this.m_dataManager.GetAttributePoints(), this.m_dataManager.GetPerkPoints());
    emptyData = this.m_menusData.GetData(3);
    this.m_subMenuCtrl.AddMenus(emptyData, this.m_menusData.Get());
  ...
  protected cb func OnOpenMenuRequest(evt: ref<OpenMenuRequest>) -> Bool {
    ...
    if GetFact(this.m_player.GetGame(), n"PendingRespec") == 1 {
      EquipmentSystem.UnequipPrereqItems(this.m_player);
      SetFactValue(this.m_player.GetGame(), n"PendingRespec", 0);
    };
  ...
  protected cb func OnUninitialize() -> Bool {
    ...
    this.QueueBroadcastEvent(setMenuModeEvent);
    this.UnregisterFromGlobalInputCallback(n"OnPostOnRelease", this, n"OnButtonRelease");
    TimeDilationHelper.SetTimeDilationWithProfile(this.m_player, "hubMenu", false, false);
