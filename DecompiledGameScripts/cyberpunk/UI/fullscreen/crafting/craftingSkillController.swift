...
public class CraftingSkillWidget extends inkGameController {
  ...
  protected cb func OnCharacterProficiencyUpdated(evt: ref<ProficiencyProgressEvent>) -> Bool {
    if Equals(evt.type, gamedataProficiencyType.TechnicalAbilitySkill) && !this.m_isLevelUp {
      this.SetProgress();
  ...
  private final func SetLevel() -> Void {
    ...
    inkTextRef.SetText(this.m_nextLevelText, IntToString(playerDevelopmentSystem.GetProficiencyLevel(puppet, gamedataProficiencyType.TechnicalAbilitySkill)));
    inkTextRef.SetText(this.m_amountText, IntToString(playerDevelopmentSystem.GetProficiencyLevel(puppet, gamedataProficiencyType.TechnicalAbilitySkill)));
  }
  ...
  private final func SetProgress() -> Void {
    ...
    let puppet: wref<PlayerPuppet> = this.GetPlayerControlledObject() as PlayerPuppet;
    let playerDevelopmentSystem: wref<PlayerDevelopmentSystem> = GameInstance.GetScriptableSystemsContainer((this.GetOwnerEntity() as GameObject).GetGame()).Get(n"PlayerDevelopmentSystem") as PlayerDevelopmentSystem;
    let currExp: Int32 = playerDevelopmentSystem.GetCurrentLevelProficiencyExp(puppet, gamedataProficiencyType.TechnicalAbilitySkill);
    let addExp: Int32 = currExp - this.m_currentExp;
    this.m_currentExp = currExp;
    let remainingExp: Int32 = playerDevelopmentSystem.GetRemainingExpForLevelUp(puppet, gamedataProficiencyType.TechnicalAbilitySkill);
    ...
    currentFill = Cast<Float>(currExp) / Cast<Float>(totalExp);
