...
public class CraftingItemTemplateClassifier extends inkVirtualItemTemplateClassifier {
  ...
  public final static func IsSmall(data: Variant) -> Bool {
    ...
    let recipe: ref<RecipeData> = FromVariant<ref<IScriptable>>(data) as RecipeData;
    if IsDefined(recipe) {
      return NotEquals(recipe.id.EquipArea().Type(), gamedataEquipmentArea.Weapon);
    };
    inventoryItem = FromVariant<ref<IScriptable>>(data) as ItemCraftingData;
  ...
  public func SortItem(left: ref<IScriptable>, right: ref<IScriptable>) -> Bool {
    ...
    if IsDefined(itemDataLeft) && IsDefined(itemDataRight) {
      ...
      if Equals(leftItem.Name, "") {
        leftItem = ItemCompareBuilder.BuildInventoryItemSortData(itemDataLeft.inventoryItem, this.m_uiScriptableSystem);
      };
      if Equals(rightItem.Name, "") {
        rightItem = ItemCompareBuilder.BuildInventoryItemSortData(itemDataRight.inventoryItem, this.m_uiScriptableSystem);
      };
    } else {
    ...
    return this.PreSortingInjection(ItemCompareBuilder.Make(leftItem, rightItem)).ChimeraMod().QualityDesc().ItemType().NameAsc().GetBool();
  }
  ...
  public func FilterItem(item: ref<IScriptable>) -> Bool {
    ...
    switch this.m_itemFilterType {
      case ItemFilterCategory.RangedWeapons:
        return itemRecord.TagsContains(n"RangedWeapon");
      case ItemFilterCategory.MeleeWeapons:
        return itemRecord.TagsContains(n"MeleeWeapon");
      ...
      case ItemFilterCategory.Programs:
        return itemRecord.TagsContains(n"SoftwareShard") || itemRecord.TagsContains(n"QuickhackCraftingPart");
