...
public class ShardsMenuGameController extends gameuiMenuGameController {
  ...
  private edit let m_rightViewPlaceholderRef: inkWidgetRef;
  ...
  protected cb func OnSetUserData(userData: ref<IScriptable>) -> Bool {
    this.m_userDataEntry = userData as ShardAttachmentData.m_hash;
  }
  ...
  private final func SelectEntry() -> Void {
    ...
    if this.m_userDataEntry != 0 && this.m_listController.GetDataSize() != 0 {
      dataIndex = this.m_listController.FindDataIndex(this.m_userDataEntry);
      if dataIndex >= 0 {
        this.m_listController.ShowLevelForDataIndex(dataIndex);
        itemIndex = this.m_listController.GetIndexByJournalHash(this.m_userDataEntry);
        this.ForceSelectIndex(Cast<Uint32>(itemIndex));
      };
    };
  }
  ...
  public func FindItem(hash: Int32) -> Int32 {
    let curHash: Int32;
    let entryData: wref<ShardEntryData>;
    let i: Int32 = 0;
    while i < this.m_listController.GetDataSize() {
      entryData = FromVariant<ref<ShardEntryData>>(this.m_listController.GetItem(Cast<Uint32>(i)));
  ...
  protected cb func OnShardSelectedEvent(evt: ref<ShardSelectedEvent>) -> Bool {
    ...
    let inputDevice: InputDevice = this.GetPlayerControlledObject().PlayerLastUsedKBM() ? InputDevice.KBM : InputDevice.PAD;
    ...
    if evt.m_group {
      ...
      this.m_entryViewController.ShowEntry(evt.m_data, inputDevice, inputScheme);
      inkWidgetRef.SetVisible(this.m_rightViewPlaceholderRef, false);
    ...
    this.m_activeData.m_toggledLevels = this.m_listController.GetToggledLevels();
    selectedEntry = this.m_journalManager.GetEntry(Cast<Uint32>(this.m_activeData.m_entryHash));
  ...
  protected cb func OnShardForceSelectionEvent(evt: ref<ShardForceSelectionEvent>) -> Bool {
    if evt.m_selectionIndex != -1 {
      this.m_listController.SelectItem(Cast<Uint32>(evt.m_selectionIndex));
      this.m_listController.ToggleItem(Cast<Uint32>(evt.m_selectionIndex));
    };
  }

  protected cb func OnShardsMenuGameControllerDelayInit(evt: ref<ShardForceSelectionEvent>) -> Bool {
    let entry: wref<JournalEntry>;
    let journalEntry: wref<JournalOnscreen>;
    let locatedIndex: Int32;
    let threadEntry: wref<JournalOnscreenGroup>;
    let shardHash: Int32 = -1;
    ...
    if evt.m_selectionIndex != -1 {
      ...
      threadEntry = entry as JournalOnscreenGroup;
      if threadEntry != null {
        journalEntry = this.m_journalManager.GetParentEntry(threadEntry) as JournalOnscreen;
        shardHash = this.m_journalManager.GetEntryHash(journalEntry);
        if !this.m_listController.IsLevelToggled(shardHash) {
          this.m_listController.ToggleLevel(shardHash);
        };
      };
      locatedIndex = this.FindItem(evt.m_hash);
      if locatedIndex == -1 && shardHash != -1 {
        locatedIndex = this.FindItem(shardHash);
      };
      if locatedIndex != -1 {
  ...
  private final func ForceSelectIndex(i: Uint32) -> Void {
    let initEvent: ref<ShardForceSelectionEvent> = new ShardForceSelectionEvent();
    initEvent.m_selectionIndex = Cast<Int32>(i);
  ...
  private final func ProcessItem(const item: script_ref<InventoryItemData>, data: script_ref<[ref<VirutalNestedListData>]>, level: Int32, opt newEntries: script_ref<[Int32]>) -> Bool {
    ...
    ArrayPush(Deref(data), this.GetVirtualDataForCrypted(item, journalEntry, level, newEntries));
    return true;
  }

  private final func GetVirtualDataForCrypted(const item: script_ref<InventoryItemData>, curShard: wref<JournalOnscreen>, level: Int32, opt newEntries: script_ref<[Int32]>) -> ref<VirutalNestedListData> {
  ...
  private final func HideNodataWarning() -> Void {
    ...
    inkWidgetRef.SetVisible(this.m_rightViewPlaceholderRef, true);
    inkWidgetRef.SetVisible(this.m_entryViewRef, true);
