...
public class BriefingScreen extends inkHUDGameController {
  ...
  protected cb func OnBriefingSizeCalled(value: Variant) -> Bool {
    ...
    logicController.SetSize(FromVariant<questJournalSizeEventType>(value));
  }

  protected cb func OnBriefingAlignmentCalled(value: Variant) -> Bool {
    let logicController: ref<BriefingScreenLogic> = inkWidgetRef.GetController(this.m_logicControllerRef) as BriefingScreenLogic;
    logicController.SetAlignment(FromVariant<questJournalAlignmentEventType>(value));
  }

  private final func FindEntry(const toFind: script_ref<String>, const entries: script_ref<[wref<JournalEntry>]>) -> ref<JournalEntry> {
    let i: Int32 = 0;
    while i < ArraySize(Deref(entries)) {
      if Equals(Deref(entries)[i].GetId(), toFind) {
        return Deref(entries)[i];
      };
      i += 1;
    };
    return null;
