...
public class CharacterCreationStatsMenu extends BaseCharacterCreationController {
  ...
  private let m_attributesControllers: [wref<characterCreationStatsAttributeBtn>];
  ...
  private let m_menuVisited: Bool;

  protected cb func OnSetUserData(userData: ref<IScriptable>) -> Bool {
    let statsMenuUserData: ref<StatsMenuUserData> = userData as StatsMenuUserData;
    this.m_menuVisited = IsDefined(statsMenuUserData) && statsMenuUserData.m_menuVisited;
  }
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    attribute01Controller.SetData(attributeType, Cast<Int32>(this.m_characterCustomizationState.GetAttribute(attributeType)));
    ...
    attribute02Controller.SetData(attributeType, Cast<Int32>(this.m_characterCustomizationState.GetAttribute(attributeType)));
    ...
    attribute03Controller.SetData(attributeType, Cast<Int32>(this.m_characterCustomizationState.GetAttribute(attributeType)));
    ...
    attribute04Controller.SetData(attributeType, Cast<Int32>(this.m_characterCustomizationState.GetAttribute(attributeType)));
    ...
    attribute05Controller.SetData(attributeType, Cast<Int32>(this.m_characterCustomizationState.GetAttribute(attributeType)));
  ...
  private final func SaveChanges() -> Void {
    let i: Int32;
    this.m_characterCustomizationState.SetAttributePointsAvailable(Cast<Uint32>(this.m_attributePointsAvailable));
    i = 0;
    while i < ArraySize(this.m_attributesControllers) {
      this.m_characterCustomizationState.SetAttribute(this.m_attributesControllers[i].data.attribute, Cast<Uint32>(this.m_attributesControllers[i].data.value));
      i += 1;
    };
  }
  ...
  public final func DisableInputBelowConfirmationPopup(disabled: Bool) -> Void {
    ...
    let i: Int32 = 0;
    while i < ArraySize(this.m_attributesControllers) {
      this.m_attributesControllers[i].SetInputDisabled(disabled);
      i += 1;
    };
        ...
        if evt.IsAction(n"back") && inkWidgetRef.IsVisible(this.m_nextMenuConfirmation) {
          ...
          if evt.IsAction(n"one_click_confirm") && !inkWidgetRef.IsVisible(this.m_nextMenuConfirmation) && StringToInt(inkTextRef.GetText(this.m_skillPointLabel)) > 0 {
            ...
            if evt.IsAction(n"one_click_confirm") && !inkWidgetRef.IsVisible(this.m_nextMenuConfirmation) && StringToInt(inkTextRef.GetText(this.m_skillPointLabel)) == 0 {
              ...
              if evt.IsAction(n"system_notification_confirm") && inkWidgetRef.IsVisible(this.m_nextMenuConfirmation) {
                this.PlaySound(n"Button", n"OnPress");
                evt.Handle();
                this.DisableInputBelowConfirmationPopup(false);
                this.NextMenu();
  ...
  private final func SkipStatsMenu() -> Void {
    if this.m_menuVisited {
      this.PriorMenu();
    } else {
      this.NextMenu();
    };
  ...
  private final func FillAttributeData(const label: script_ref<String>, value: Int32, const desc: script_ref<String>) -> ref<CharacterCreationAttributeData> {
    let newItem: ref<CharacterCreationAttributeData> = new CharacterCreationAttributeData();
    newItem.label = Deref(label);
    newItem.value = value;
    newItem.desc = Deref(desc);
    return newItem;
  }
