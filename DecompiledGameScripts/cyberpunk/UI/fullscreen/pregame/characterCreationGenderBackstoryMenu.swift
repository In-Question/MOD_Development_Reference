...
public class CharacterCreationGenderSelectionMenu extends BaseCharacterCreationController {
  ...
  private final func SetAttributePreset(lifePath: TweakDBID) -> Void {
    ...
    while i < ArraySize(UIAttRecords) {
      this.m_characterCustomizationState.SetAttribute(UIAttRecords[i].Attribute().StatType(), Cast<Uint32>(UIAttRecords[i].Value()));
      i += 1;
    };
  }

  private final func SetEP1AttributePreset() -> Void {
    ...
    i = 0;
    while i < ArraySize(buildAttributes) {
      this.m_characterCustomizationState.SetAttribute(buildAttributes[i].Attribute().StatType(), Cast<Uint32>(buildAttributes[i].Level()));
      i += 1;
    };
  }
...
public class CharacterCreationGenderBackstoryPathHeader extends inkLogicController {
  ...
  public final func SetData(const title: script_ref<String>, const desc: script_ref<String>) -> Void {
    inkTextRef.SetText(this.m_label, Deref(title));
    inkTextRef.SetText(this.m_desc, Deref(desc));
  }
}
