...
public class PhotoModeGridList extends inkRadioGroupController {
  ...
  private let m_buttons: [wref<PhotoModeGridButton>];

  private let m_rows: [wref<inkWidget>];
  ...
  public final func Setup(listItem: ref<PhotoModeMenuListItem>, rows: Int32, itemsInRow: Int32) -> Float {
    ...
    this.m_visibleSize = Cast<Float>(this.m_firstOffset + this.m_rowOffset * this.m_visibleRows);
    this.m_scrollRow = 0;
    size = this.GetRootWidget().GetSize();
    inkWidgetRef.SetSize(this.m_ContentRoot, size.X, Cast<Float>(this.m_firstOffset + this.m_rowOffset * this.m_RowsCount));
    if this.m_visibleRows < this.m_RowsCount {
      this.m_sliderController.Setup(0.00, Cast<Float>(this.m_RowsCount - this.m_visibleRows), 0.00);
      this.m_sliderController.SetPercentageHandleSize(Cast<Float>(this.m_visibleRows) / Cast<Float>(this.m_RowsCount));
    } else {
      this.m_sliderController.Setup(0.00, 0.00, 0.00, 1.00);
    };
    this.Toggle(0);
  ...
  protected cb func OnSliderValueChanged(sliderController: wref<inkSliderController>, progress: Float, value: Float) -> Bool {
    this.ScrollToRow(Cast<Int32>(value));
  }
  ...
  public final func HandleReleasedInput(e: ref<inkPointerEvent>, opt gameCtrl: wref<inkGameController>) -> Void {
    if e.IsAction(n"PhotoMode_SaveSettings") {
      ...
      if e.IsAction(n"PhotoMode_ScrollUp") {
        this.m_sliderController.ChangeValue(Cast<Float>(this.m_scrollRow - 1));
      } else {
  ...
  public final func SetGridData(const gridData: script_ref<[PhotoModeOptionGridButtonData]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(gridData)) {
      if i >= ArraySize(this.m_buttons) {
        break;
      };
      this.SetGridButtonImage(Cast<Uint32>(i), Deref(gridData)[i].atlasResource, Deref(gridData)[i].imagePart, Deref(gridData)[i].optionData);
      i += 1;
    };
  }

  public final func SetGridButtonImage(buttonIndex: Uint32, atlasPath: ResRef, imagePart: CName, buttonData: Int32) -> Void {
    let i: Int32 = Cast<Int32>(buttonIndex);
    if i < ArraySize(this.m_buttons) {
  ...
  public final func SetGridButtonImageForceVisible(buttonIndex: Uint32) -> Void {
    let i: Int32 = Cast<Int32>(buttonIndex);
    if i < ArraySize(this.m_buttons) {
      this.m_buttons[i].SetForceVisible();
    };
  }
  ...
  protected final func AddRow() -> wref<inkWidget> {
    ...
    newRow.SetMargin(0.00, Cast<Float>(this.m_firstOffset + ArraySize(this.m_rows) * this.m_rowOffset), 0.00, 0.00);
  ...
  public final func UpdateScroll() -> Void {
    ...
    if oldScroll != this.m_scrollRow {
      this.m_sliderController.ChangeValue(Cast<Float>(this.m_scrollRow));
    };
  }
  ...
  private final func ScrollToRow(row: Int32) -> Void {
    ...
    this.UpdateButtonsVisibility();
    scrollValue = Cast<Float>(this.m_scrollRow) / Cast<Float>(this.m_RowsCount - this.m_visibleRows);
