...
public class PhotoModeGridButton extends inkToggleController {
  ...
  private let m_alwaysVisible: Bool;
  ...
  public final func SetImage(atlasPath: ResRef, imagePart: CName) -> Void {
    if Equals(imagePart, n"None") || !ResRef.IsValid(atlasPath) {
  ...
  public final func SetForceVisible() -> Void {
    this.m_alwaysVisible = true;
    inkWidgetRef.SetScale(this.m_DynamicImg, new Vector2(1.00, 1.00));
    inkWidgetRef.SetOpacity(this.m_DynamicImg, 1.00);
  }

  public final func SetImageScalingSpeed(speed: Float) -> Void {
    this.m_imageScalingSpeed = speed;
  }

  public final func SetOpacityScalingSpeed(speed: Float) -> Void {
    this.m_opacityScalingSpeed = speed;
  }
  ...
  public final func UpdateSize(timeDelta: Float) -> Void {
    ...
    let scaleY: Float;
    let t: Float;
    if this.m_visibleOnGrid && NotEquals(this.m_currentImagePart, n"None") && inkImageRef.IsTexturePartExist(this.m_DynamicImg, this.m_currentImagePart) && !this.m_alwaysVisible {
