...
public class PhotoModeFrame extends inkLogicController {
  ...
  private edit const let m_images: [inkImageRef];
  ...
  public final func SetImages(const imageParts: script_ref<[CName]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(imageParts)) && i < ArraySize(this.m_images) {
      inkImageRef.SetTexturePart(this.m_images[i], Deref(imageParts)[i]);
      i += 1;
    };
    this.m_currentImagePart = Deref(imageParts)[0];
    this.m_opacity = 0.00;
  }
  ...
  public final func Update(timeDelta: Float) -> Void {
    if NotEquals(this.m_currentImagePart, n"None") && inkImageRef.IsTexturePartExist(this.m_images[0], this.m_currentImagePart) {
