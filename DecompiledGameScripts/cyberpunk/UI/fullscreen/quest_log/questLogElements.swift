...
public class QuestListController extends inkLogicController {
  ...
  private let m_QuestItems: [wref<QuestItemController>];

  private let m_LastQuestData: wref<QuestDataWrapper>;

  public final func Setup(questType: gameJournalQuestType, const questTypeLocTag: script_ref<String>) -> Void {
    this.m_QuestType = questType;
    inkTextRef.SetText(this.m_CategoryName, Deref(questTypeLocTag));
  }

  public final func CanAddQuest(questData: script_ref<ref<QuestDataWrapper>>) -> Bool {
    return (Equals(Deref(questData).GetType(), this.m_QuestType) || Equals(Deref(questData).GetType(), gameJournalQuestType.MinorQuest) && (Equals(this.m_QuestType, gameJournalQuestType.SideQuest) || Equals(this.m_QuestType, gameJournalQuestType.CourierSideQuest))) && Equals(Deref(questData).GetStatus(), gameJournalEntryState.Active);
  }
