...
public class questLogV2GameController extends gameuiMenuGameController {
  ...
  private let m_ObjectiveItems: [wref<ObjectiveController>];

  private let m_QuestLists: [wref<QuestListController>];
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.CreateQuestGroup(gameJournalQuestType.ApartmentQuest, "APARTMENTS");
    this.CreateQuestGroup(gameJournalQuestType.CourierQuest, "COURIERS");
    this.RefreshUI();
  }
  ...
  private final func CreateQuestGroup(questType: gameJournalQuestType, const questLOCKey: script_ref<String>) -> Void {
  ...
  private final func CreateQuestObjectives(currQuestData: ref<QuestDataWrapper>) -> Void {
    ...
    while i < limit {
      ...
      if i >= ArraySize(questObjectives) || !questObjectives[i].IsActive() {
        ...
        if questObjectives[i].IsActive() {
          this.m_ObjectiveItems[i].Setup(questObjectives[i], journalManager.GetIsObjectiveOptional(questObjectives[i].GetQuestObjective()));
          if !questObjectives[i].IsActive() {
            targetParent = inkWidgetRef.Get(this.m_ObjectivesListRef) as inkCompoundWidget;
          } else {
            if journalManager.GetIsObjectiveOptional(questObjectives[i].GetQuestObjective()) {
              targetParent = inkWidgetRef.Get(this.m_ObjectivesListRef) as inkCompoundWidget;
            } else {
              targetParent = inkWidgetRef.Get(this.m_ObjectivesListRef) as inkCompoundWidget;
            };
