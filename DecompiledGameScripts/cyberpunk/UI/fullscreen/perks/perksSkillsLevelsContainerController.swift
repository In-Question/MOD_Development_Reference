...
public class PerksSkillsLevelsContainerController extends inkLogicController {
  ...
  public final func UpdateLevelBar() -> Void {
    ...
    if this.m_proficiencyDisplayData.m_level == this.m_proficiencyDisplayData.m_maxLevel {
      ...
      percentage = Cast<Float>(this.m_proficiencyDisplayData.m_expPoints) / Cast<Float>(this.m_proficiencyDisplayData.m_maxExpPoints) * 100.00;
    };
...
public class PerksSkillsLevelDisplayController extends inkLogicController {
  ...
  public final func Setup(data: ref<AreaDisplayData>) -> Void {
    ...
    if maxedSum > 0 {
      state = n"Maxed";
      inkWidgetRef.SetOpacity(this.m_tint, Cast<Float>(maxedSum) / Cast<Float>(ArraySize(data.m_perks)));
    } else {
      if boughtSum > 0 {
        state = n"Bought";
        inkWidgetRef.SetOpacity(this.m_tint, Cast<Float>(boughtSum) / Cast<Float>(boughtMax));
      } else {
