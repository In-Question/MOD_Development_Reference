...
public class PerkScreenController extends inkLogicController {
  ...
  private let m_widgetMap: [wref<PerkDisplayContainerController>];

  private let m_traitController: wref<PerkDisplayContainerController>;

  private let m_currentIndex: Int32;

  private let m_connectionLines: [Int32; 45];
...
public class PerksSkillLabelContentContainer extends HubMenuLabelContentContainer {
  ...
  public func SetData(const data: script_ref<MenuData>) -> Void {
    this.m_data = Deref(data);
    this.RefreshSkillData(Deref(data).userData as ProficiencyDisplayData);
  }
...
public class PerksScreenStaticData extends IScriptable {
  ...
  public final static func GetPerksConnectionLines() -> [Int32; 45] {
    let result: [Int32; 45];
    result[0] = 29;
    result[1] = 17;
    result[2] = 27;
...
public class ProficiencyTabButtonController extends TabButtonController {
  ...
  private edit let m_bottom_bar: inkWidgetRef;
  ...
  protected cb func OnToggleChanged(controller: wref<inkToggleController>, isToggled: Bool) -> Bool {
    ...
    let buttonColor: CName = isToggledState ? n"MainColors.ActiveBlue" : n"MainColors.Red";
    inkWidgetRef.Get(this.m_bottom_bar).BindProperty(n"tintColor", buttonColor);
...
public class TabRadioGroup extends inkRadioGroupController {
  ...
  public let toggles: [wref<TabButtonController>];

  private let m_TooltipsManager: wref<gameuiTooltipsManager>;

  public final func SetData(enumCount: Int32, opt tooltipsManager: wref<gameuiTooltipsManager>, opt labelList: [String], opt iconList: [String]) -> Void {
