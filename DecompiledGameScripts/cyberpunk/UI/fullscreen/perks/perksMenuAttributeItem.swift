...
public class PerksMenuAttributeItemController extends inkLogicController {
  ...
  protected edit const let m_proficiencyButtonRefs: [inkWidgetRef];
  ...
  protected cb func OnAttributeItemClicked(evt: ref<inkPointerEvent>) -> Bool {
    ...
    let perksMenuAttributeItemReleasedEvent: ref<PerksMenuAttributeItemReleased>;
    if evt.IsAction(n"click") {
      ...
      perksMenuAttributeItemClickedEvent.isHeld = this.m_holdStarted;
      ...
      if evt.IsAction(n"upgrade_attribute") {
        ...
        this.QueueEvent(perksMenuAttributeItemReleasedEvent);
        this.m_holdStarted = false;
      };
    };
  }
  ...
  protected cb func OnAttributeItemHold(evt: ref<inkPointerEvent>) -> Bool {
    ...
    let isEspionage: Bool = Equals(this.m_attributeType, PerkMenuAttribute.Espionage);
    if isEspionage {
      return false;
    };
    if evt.IsAction(n"upgrade_attribute") {
      if progress > 0.00 && !this.m_holdStarted {
        ...
        if !this.m_data.availableToUpgrade || !this.m_dataManager.IsAttributeUpgradeable(this.m_attributeType) {
...
public class ProficiencyButtonController extends inkButtonController {
  ...
  public final func SetLabel(const value: script_ref<String>) -> Void {
    inkTextRef.SetText(this.m_labelText, Deref(value));
  }
