...
public class PerksMainGameController extends gameuiMenuGameController {
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_inCombat = this.m_dataManager.GetPlayer().GetPlayerStateMachineBlackboard().GetInt(GetAllBlackboardDefs().PlayerStateMachine.Combat) == 1;
  ...
  protected cb func OnPlayerAttach(playerPuppet: ref<GameObject>) -> Bool {
    ...
    this.m_equipmentSystem = GameInstance.GetScriptableSystemsContainer(playerPuppet.GetGame()).Get(n"EquipmentSystem") as EquipmentSystem;
    ...
    switch this.m_activeScreen {
      ...
      case CharacterScreenType.Attributes:
        if !StatusEffectSystem.ObjectHasStatusEffectWithTag(this.GetPlayerControlledObject(), n"LockInHubMenu") {
          ...
          if GetFact(this.m_dataManager.GetPlayer().GetGame(), n"PendingRespec") == 1 {
            EquipmentSystem.UnequipPrereqItems(this.m_dataManager.GetPlayer());
            SetFactValue(this.m_dataManager.GetPlayer().GetGame(), n"PendingRespec", 0);
          };
        };
    };
  }
  ...
  protected cb func OnResetConfirmed(data: ref<inkGameNotificationData>) -> Bool {
    ...
    if IsDefined(resultData) && Equals(resultData.result, GenericMessageNotificationResult.Yes) {
      ...
      removeAllPerks.Set(this.m_dataManager.GetPlayer(), false, false);
      PlayerDevelopmentSystem.GetInstance(this.m_dataManager.GetPlayer()).QueueRequest(removeAllPerks);
      SetFactValue(this.m_dataManager.GetPlayer().GetGame(), n"PendingRespec", 1);
    };
  }
  ...
  private final func SetPerksButtonHintHoverOver(data: ref<BasePerkDisplayData>) -> Void {
    ...
    if cursorData.GetActionsListSize() >= 0 {
      this.SetCursorContext(n"hoverOnHoldToComplete", cursorData);
    } else {
