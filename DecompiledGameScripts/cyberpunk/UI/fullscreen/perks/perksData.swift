...
public class NewPerkDisplayData extends BasePerkDisplayData {

  public let m_area: gamedataNewPerkSlotType;

  public let m_type: gamedataNewPerkType;

  public let m_isRipperdoc: Bool;

  public func CreateTooltipData(manager: ref<PlayerDevelopmentDataManager>) -> ref<BasePerksMenuTooltipData> {
    let data: ref<NewPerkTooltipData> = new NewPerkTooltipData();
    data.manager = manager;
    data.perkType = this.m_type;
    data.perkArea = this.m_area;
    data.proficiency = this.m_proficiency;
    data.attributeId = this.m_attributeId;
    data.isRipperdoc = this.m_isRipperdoc;
    data.perkData = this;
    return data;
  }
}
...
public class ProficiencyDisplayData extends IDisplayData {
  ...
  public let m_areas: [ref<AreaDisplayData>];

  public let m_passiveBonusesData: [ref<LevelRewardDisplayData>];
...
public class AttributeDisplayData extends IDisplayData {
  ...
  public let m_proficiencies: [ref<ProficiencyDisplayData>];
...
public class PerkLevelData_Records extends BasePerkLevelData_Records {

  private let m_arr: [wref<PerkLevelData_Record>];

  public final func Initialize(perkRecord: script_ref<wref<Perk_Record>>) -> Void {
    Deref(perkRecord).Levels(this.m_arr);
  }

  public const func GetItemAt(index: Int32) -> wref<PerkLevelData_Record> {
    if ArraySize(this.m_arr) <= index {
      return null;
    };
...
public class NewPerkLevelData_Records extends BasePerkLevelData_Records {

  private let m_perkRecord: wref<NewPerk_Record>;

  private let m_arr: [wref<NewPerkLevelData_Record>];

  public final func Initialize(perkRecord: script_ref<wref<NewPerk_Record>>) -> Void {
    this.m_perkRecord = Deref(perkRecord);
    Deref(perkRecord).Levels(this.m_arr);
  }

  public final func GetNewPerkRecord() -> wref<NewPerk_Record> {
    return this.m_perkRecord;
  }
  ...
  public const func GetItemAt(index: Int32) -> wref<NewPerkLevelData_Record> {
    if ArraySize(this.m_arr) <= index {
      return null;
    };
    return this.m_arr[index];
...
public abstract class BasePerkLevelData_Records extends IScriptable {

  public const func GetItemAt(index: Int32) -> wref<TweakDBRecord> {
    return null;
  }
}
...
public class PlayerDevelopmentDataManager extends IScriptable {
  ...
  public final func GetNewPerkDisplayData(perkType: gamedataNewPerkType, perkArea: gamedataNewPerkSlotType, proficiency: gamedataProficiencyType, attributeId: TweakDBID, isRipperdoc: Bool, opt playerDevelopmentData: wref<PlayerDevelopmentData>) -> ref<NewPerkDisplayData> {
    return this.GetNewPerkDisplayData(perkType, perkArea, proficiency, TweakDBInterface.GetAttributeRecord(attributeId), isRipperdoc, playerDevelopmentData);
  }

  public final func GetNewPerkDisplayData(perkType: gamedataNewPerkType, perkArea: gamedataNewPerkSlotType, proficiency: gamedataProficiencyType, attributeRecord: ref<Attribute_Record>, isRipperdoc: Bool, opt playerDevelopmentData: wref<PlayerDevelopmentData>) -> ref<NewPerkDisplayData> {
    let curPerkDisplayData: ref<NewPerkDisplayData>;
    ...
    if Equals(curPerkDisplayData.m_localizedName, "") {
    ...
    if Equals(curPerkDisplayData.m_localizedDescription, "") {
    ...
    curPerkDisplayData.m_locked = !this.m_playerDevSystem.IsNewPerkUnlocked(this.m_player, perkType);
  ...
  public final func GetProficiencyDisplayData(proficiency: gamedataProficiencyType) -> ref<ProficiencyDisplayData> {
    let proficiencyRecord: wref<Proficiency_Record>;
    let attributeRecord: wref<Attribute_Record> = this.GetAttributeRecordFromProficiency(proficiency);
    if PlayerDevelopmentData.IsProfficiencyObsolete(proficiency) {
      proficiencyRecord = this.GetProficiencyRecord(attributeRecord, proficiency);
      return this.GetProficiencyDisplayData(proficiencyRecord.Type(), attributeRecord);
    };
    return this.GetProficiencyDisplayData(RPGManager.GetProficiencyRecord(proficiency), attributeRecord);
  }
  ...
  public final func GetProficiencyDisplayData(proficiency: gamedataProficiencyType, attributeRecord: ref<Attribute_Record>) -> ref<ProficiencyDisplayData> {
    ...
    return this.GetProficiencyDisplayData(proficiencyRecord, attributeRecord);
  }

  public final func GetProficiencyDisplayData(proficiencyRecord: ref<Proficiency_Record>, attributeRecord: ref<Attribute_Record>) -> ref<ProficiencyDisplayData> {
    let proficiency: gamedataProficiencyType = proficiencyRecord.Type();
  ...
  public final func GetPassiveBonusDisplayData(proficiencyRecord: ref<Proficiency_Record>) -> [ref<LevelRewardDisplayData>] {
    ...
    while profIdx < attributeRecord.GetProficienciesCount() {
      ...
      while areaIdx < curProficiency.GetPerkAreasCount() {
        ...
        while perkIdx < curPerkArea.GetPerksCount() {
          ...
          curPerkDisplayData = this.GetPerkDisplayData(perkrecord.Type(), curPerkArea.Type(), curProficiency.Type(), attributeRecord, playerDevelopmentData);
          ArrayPush(curPerkAreaDisplayData.m_perks, curPerkDisplayData);
          perkIdx += 1;
        };
        ArrayPush(curProfDisplayData.m_areas, curPerkAreaDisplayData);
  ...
  public final func GetAttributes() -> [ref<AttributeData>] {
  ...
  public final func GetAttributeEnumFromRecordID(recordID: TweakDBID) -> PerkMenuAttribute {
    ...
    if recordID == t"BaseStats.Espionage" {
  ...
  public final func GetSpyPerkPoints() -> Int32 {
    return this.m_playerDevSystem.GetDevPoints(this.m_player, gamedataDevelopmentPointType.Espionage);
  }
  ...
  public final func IsPerkUpgradeable(data: ref<BasePerkDisplayData>, opt showNotification: Bool) -> Bool {
    let isEspionage: Bool;
    let isEspionageMilestone: Bool;
    let isPerkUpgradeable: Bool;
    let newPerkData: ref<NewPerkDisplayData>;
    let notificationEvent: ref<UIMenuNotificationEvent>;
    let notificationType: UIMenuNotificationType;
    if data.IsA(n"NewPerkDisplayData") {
      newPerkData = data as NewPerkDisplayData;
      isEspionage = this.IsEspionagePerk(newPerkData.m_type);
      isEspionageMilestone = this.IsEspionageMilestonePerk(newPerkData.m_type);
    } else {
      isEspionage = false;
      isEspionageMilestone = false;
    };
    isPerkUpgradeable = true;
    if isEspionage && this.GetSpyPerkPoints() <= isEspionageMilestone ? 2 : 0 {
      notificationType = UIMenuNotificationType.NoPerksPoints;
      isPerkUpgradeable = false;
    } else {
      if !isEspionage && this.GetPerkPoints() <= 0 {
        notificationType = UIMenuNotificationType.NoPerksPoints;
        isPerkUpgradeable = false;
      };
    };
  ...
  public final func IsNewPerkRefundable(data: ref<NewPerkDisplayData>) -> Bool {
    let allParentsSold: Bool = PlayerDevelopmentSystem.GetData(this.m_player).CheckIfAllnewPerkParentSold(data.m_type);
  ...
  public final func IsAttributeUpgradeable(attribute: PerkMenuAttribute) -> Bool {
    return PlayerDevelopmentSystem.GetData(this.m_player).CanAttributeBeBought(PerkAttributeHelper.PerkMenuAttributeToStat(attribute));
  }

  public final func IsEspionagePerk(perkType: gamedataNewPerkType) -> Bool {
    return PlayerDevelopmentSystem.GetData(this.m_player).IsNewPerkEspionage(perkType);
  }

  public final func IsEspionageMilestonePerk(perkType: gamedataNewPerkType) -> Bool {
    return PlayerDevelopmentSystem.GetData(this.m_player).IsEspionageMilestonePerk(perkType);
  }

  public final func GetAttributeLevel(attribute: PerkMenuAttribute) -> Int32 {
    let attributeDataType: gamedataAttributeDataType = PerkAttributeHelper.PerkMenuAttributeToAttributeDataType(attribute);
    return PlayerDevelopmentSystem.GetData(this.m_player).GetAttributePoints(attributeDataType);
  }
}
