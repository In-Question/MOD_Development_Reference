...
public class PerkDisplayController extends inkButtonController {
  ...
  private final func UpdateLayout(data: ref<BasePerkDisplayData>) -> Void {
    ...
    InkImageUtils.RequestSetImage(this, this.m_icon, data.m_iconID);
    inkWidgetRef.SetVisible(this.m_icon, TDBID.IsValid(data.m_iconID));
    if !this.m_isTrait {
  ...
  private final func UpdateLayout(data: ref<PerkDisplayData>) -> Void {
    ...
    let statType: gamedataStatType = IntEnum<gamedataStatType>(Cast<Int32>(EnumValueFromName(n"gamedataStatType", statPrereqRecord.StatType())));
    let statValue: Float = GameInstance.GetStatsSystem(this.m_dataManager.GetPlayer().GetGame()).GetStatValue(Cast<StatsObjectID>(this.m_dataManager.GetPlayer().GetEntityID()), statType);
    numPointsRequired = Cast<Int32>(statPrereqRecord.ValueToCheck()) - Cast<Int32>(statValue);
  ...
  private final func UpdateLayout(data: ref<TraitDisplayData>) -> Void {
    ...
    let proficiencyType: gamedataProficiencyType = IntEnum<gamedataProficiencyType>(Cast<Int32>(EnumValueFromName(n"gamedataProficiencyType", statPrereqRecord.StatType())));
    numPointsRequired = Cast<Int32>(prereq.ValueToCheck()) - this.m_playerDevelopmentData.GetProficiencyLevel(proficiencyType);
  ...
  protected final func GetFluffRand(perkData: ref<PerkDisplayData>, opt offset: Int32) -> Int32 {
    return Cast<Int32>(RandNoiseF(EnumInt(perkData.m_proficiency) * 1000 + EnumInt(perkData.m_area) * 100 + EnumInt(perkData.m_type) + offset, 1000.00, 9999.00));
  }

  protected final func GetFluffRand(traitData: ref<TraitDisplayData>, opt offset: Int32) -> Int32 {
    return Cast<Int32>(RandNoiseF(traitData.m_level * 100 + EnumInt(traitData.m_type) + offset, 1000.00, 9999.00));
  }
