...
public class ItemQuantityPickerController extends inkGameController {
  ...
  protected let m_inventoryItem: wref<UIInventoryItem>;
  ...
  protected cb func OnAction(action: ListenerAction, consumer: ListenerActionConsumer) -> Bool {
    ...
    if isPressed || isAxis || isRelative {
      delta = ListenerAction.GetValue(action) * MaxF(0.03, 1.30 / MaxF(Cast<Float>(this.m_maxValue), 1.00));
  ...
  protected cb func OnHandlePressInput(evt: ref<inkPointerEvent>) -> Bool {
    if (evt.IsAction(n"popup_moveRight") || evt.IsAction(n"option_switch_next_settings")) && this.m_choosenQuantity < this.m_maxValue {
      this.m_choosenQuantity += 1;
      this.UpdateProgress();
    } else {
      if (evt.IsAction(n"popup_moveLeft") || evt.IsAction(n"option_switch_prev_settings")) && this.m_choosenQuantity > 1 {
        this.m_choosenQuantity -= 1;
        this.UpdateProgress();
      } else {
  ...
  protected cb func OnAxisInput(evt: ref<inkPointerEvent>) -> Bool {
    let delta: Float = evt.GetAxisData() * MaxF(0.03, 1.30 / MaxF(Cast<Float>(this.m_maxValue), 1.00));
  ...
  protected final func UpdateProgress() -> Void {
    this.m_sliderController.ChangeProgress(Cast<Float>(this.m_choosenQuantity - 1) / Cast<Float>(this.m_maxValue - 1));
    inkTextRef.SetText(this.m_quantityTextChoosen, IntToString(this.m_choosenQuantity));
    this.UpdatePriceText();
    this.UpdateWeight();
    if this.m_sendQuantityChangedEvent {
  ...
  protected cb func OnSliderValueChanged(controller: wref<inkSliderController>, progress: Float, value: Float) -> Bool {
    this.m_choosenQuantity = RoundF(Cast<Float>(this.m_maxValue - 1) * progress) + 1;
    inkTextRef.SetText(this.m_quantityTextChoosen, IntToString(this.m_choosenQuantity));
    this.UpdatePriceText();
    this.UpdateWeight();
    if this.m_sendQuantityChangedEvent {
  ...
  private final func Close(success: Bool) -> Void {
    ...
    this.m_closeData.inventoryItem = this.m_inventoryItem;
