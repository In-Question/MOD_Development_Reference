...
public class ItemsNotificationQueue extends gameuiGenericNotificationGameController {
  ...
  protected cb func OnPlayerAttach(playerPuppet: ref<GameObject>) -> Bool {
    ...
    this.m_delaySystem = GameInstance.GetDelaySystem(this.m_playerPuppet.GetGame());
  ...
  private final func NeedsNotification(const newItem: script_ref<InventoryItemData>) -> Bool {
    let isIconic: Bool = RPGManager.IsItemIconic(InventoryItemData.GetGameItemData(newItem));
    if isIconic {
      return true;
    };
  ...
  private final func ShouldRarityForceNotification(const newItem: script_ref<InventoryItemData>) -> Bool {
    return Equals(InventoryItemData.GetQuality(newItem), n"Legendary");
  }

  private final func IsBestInBackpack(const newItem: script_ref<InventoryItemData>) -> Bool {
    ...
    while i < limit {
      ...
      if ItemID.GetTDBID(InventoryItemData.GetID(newItem)) == ItemID.GetTDBID(InventoryItemData.GetID(itemToCompare)) {
        if Equals(this.m_comparisonResolver.CompareItemsByStats(itemToCompare, newItem), ItemComparisonState.Better) {
          return false;
        };
      };
      i += 1;
  ...
  protected cb func OnCharacterProficiencyUpdated(evt: ref<ProficiencyProgressEvent>) -> Bool {
    switch evt.type {
      ...
      case gamedataProficiencyType.StrengthSkill:
        this.PushXPNotification(evt.expValue, evt.remainingXP, evt.delta, n"Skills", "LocKey#92105", evt.type, evt.currentLevel, evt.isLevelMaxed);
        break;
      case gamedataProficiencyType.ReflexesSkill:
        this.PushXPNotification(evt.expValue, evt.remainingXP, evt.delta, n"Skills", "LocKey#92106", evt.type, evt.currentLevel, evt.isLevelMaxed);
        break;
      case gamedataProficiencyType.TechnicalAbilitySkill:
        this.PushXPNotification(evt.expValue, evt.remainingXP, evt.delta, n"Skills", "LocKey#92108", evt.type, evt.currentLevel, evt.isLevelMaxed);
        break;
      case gamedataProficiencyType.IntelligenceSkill:
        this.PushXPNotification(evt.expValue, evt.remainingXP, evt.delta, n"Skills", "LocKey#92107", evt.type, evt.currentLevel, evt.isLevelMaxed);
        break;
      case gamedataProficiencyType.CoolSkill:
        this.PushXPNotification(evt.expValue, evt.remainingXP, evt.delta, n"Skills", "LocKey#92109", evt.type, evt.currentLevel, evt.isLevelMaxed);
    };
  }
  ...
  public final func PushXPNotification(value: Int32, remainingPointsToLevelUp: Int32, delta: Int32, notificationColorTheme: CName, const notificationName: script_ref<String>, type: gamedataProficiencyType, currentLevel: Int32, isLevelMaxed: Bool) -> Void {
    let notificationData: gameuiGenericNotificationData;
  ...
  public final func PushCurrencyNotification(diff: Int32, total: Uint32) -> Void {
    ...
    notificationData.notificationData = userData;
    RewardNotificationCurrencyDelayed.CreateAndDispatch(this, notificationData, this.m_delaySystem);
  }
  ...
  public final func PushItemNotification(itemID: ItemID, itemRarity: CName) -> Void {
    ...
    if itemID != MarketSystem.Money() && !isShard && !ArrayContains(currentItemRecordTags, n"DontShowLooted") {
  ...
  private final func GetComparisonState(const item: script_ref<InventoryItemData>) -> ItemComparisonState {
...
public class ItemAddedNotification extends GenericNotificationController {
  ...
  public func SetNotificationData(notificationData: ref<GenericNotificationViewData>) -> Void {
    ...
    this.m_itemIconGender = UIGenderHelper.GetIconGender(this.GetPlayerControlledObject() as PlayerPuppet);
  ...
  private final func SetIcon(itemID: TweakDBID, rarity: CName) -> Void {
    ...
    if IsStringValid(iconPath) {
      ...
      iconName = NameToString(iconsNameResolver.TranslateItemToIconName(IsDefined(recipeRecord) ? itemRecord.GetID() : itemID, Equals(this.m_itemIconGender, ItemIconGender.Male)));
    };
  ...
  protected func UpdateRarity(rarity: CName) -> Void {
    let visible: Bool = NotEquals(rarity, n"None");
    visible = false;
...
public class RewardNotificationCurrencyDelayed extends DelayCallback {

  private let m_notificationQueue: wref<ItemsNotificationQueue>;

  private let m_notificationData: gameuiGenericNotificationData;

  public final static func CreateAndDispatch(queue: wref<ItemsNotificationQueue>, notificationData: gameuiGenericNotificationData, delaySystem: wref<DelaySystem>) -> Void {
    let DELAY_UNTIL_NOTIFY: Float = 0.10;
    let notification: ref<RewardNotificationCurrencyDelayed> = new RewardNotificationCurrencyDelayed();
  ...
  public func Call() -> Void {
    if IsDefined(this.m_notificationQueue) {
      this.m_notificationQueue.AddNewNotificationData(this.m_notificationData);
    };
  }
}
