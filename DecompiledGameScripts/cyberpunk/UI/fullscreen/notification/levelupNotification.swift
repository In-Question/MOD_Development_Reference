...
public class LevelUpNotificationQueue extends gameuiGenericNotificationGameController {
  ...
  private let m_lastEspionageLevel: Int32;

  private let m_warningBlackboard: wref<IBlackboard>;

  private let m_warningBlackboardDef: ref<UI_NotificationsDef>;
  ...
  protected cb func OnInitialize() -> Bool {
    this.m_warningBlackboardDef = GetAllBlackboardDefs().UI_Notifications;
    this.m_warningBlackboard = this.GetBlackboardSystem().Get(this.m_warningBlackboardDef);
    this.m_warningMessageCallbackId = this.m_warningBlackboard.RegisterDelayedListenerVariant(this.m_warningBlackboardDef.WarningMessage, this, n"OnWarningMessageAppeared");
  ...
  protected cb func OnUninitialize() -> Bool {
    ...
    if IsDefined(this.m_warningBlackboard) {
      this.m_warningBlackboard.UnregisterDelayedListener(this.m_warningBlackboardDef.WarningMessage, this.m_warningMessageCallbackId);
    };
  }

  protected cb func OnWarningMessageAppeared(value: Variant) -> Bool {
  ...
  private final func SetLastRelicLevel() -> Void {
    let gameInstance: GameInstance = (this.GetOwnerEntity() as GameObject).GetGame();
    let playerDevelopmentSystem: ref<PlayerDevelopmentSystem> = GameInstance.GetScriptableSystemsContainer(gameInstance).Get(n"PlayerDevelopmentSystem") as PlayerDevelopmentSystem;
    this.m_lastEspionageLevel = playerDevelopmentSystem.GetProficiencyLevel(this.GetPlayerControlledObject(), gamedataProficiencyType.Espionage);
  }
...
public class LevelUpNotification extends GenericNotificationController {
  ...
  private let m_levelupData: ref<LevelUpNotificationViewData>;
  ...
  public func SetNotificationData(notificationData: ref<GenericNotificationViewData>) -> Void {
    ...
    this.m_levelupData = notificationData as LevelUpNotificationViewData;
    inkTextRef.SetLetterCase(this.m_LevelUpLabelText, textLetterCase.UpperCase);
    inkWidgetRef.SetVisible(this.m_actionRef, this.m_levelupData.action != null);
    if Equals(this.m_levelupData.levelupdata.type, gamedataProficiencyType.Level) || Equals(this.m_levelupData.levelupdata.type, gamedataProficiencyType.StreetCred) {
      inkTextRef.SetText(this.m_LevelUpValue, IntToString(this.m_levelupData.levelupdata.lvl));
      inkTextRef.SetText(this.m_LevelUpPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.lvl - 1)));
      inkTextRef.SetText(this.m_PerkPointsValue, IntToString(this.m_levelupData.levelupdata.perkPoints));
      inkTextRef.SetText(this.m_PerkPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.perkPoints - 1)));
      if Equals(this.m_levelupData.levelupdata.type, gamedataProficiencyType.Level) {
        inkTextRef.SetText(this.m_AttributePointsValue, IntToString(this.m_levelupData.levelupdata.attributePoints));
        inkTextRef.SetText(this.m_AttributePointsPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.attributePoints - 1)));
      ...
      if Equals(this.m_levelupData.levelupdata.type, gamedataProficiencyType.StreetCred) {
        this.unlockedActivites = TweakDBInterface.GetInt(TDBID.Create("Constants.StreetCredActivityUnlocks.level" + this.m_levelupData.levelupdata.lvl), 0);
      ...
      this.PlaySound(this.m_levelupData.soundEvent, this.m_levelupData.soundAction);
    } else {
      if Equals(this.m_levelupData.levelupdata.type, gamedataProficiencyType.Espionage) {
        inkTextRef.SetText(this.m_LevelUpValue, IntToString(this.m_levelupData.levelupdata.lvl));
        inkTextRef.SetText(this.m_LevelUpPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.lvl - 1)));
        inkTextRef.SetText(this.m_AttributePointsValue, IntToString(this.m_levelupData.levelupdata.attributePoints));
        inkTextRef.SetText(this.m_AttributePointsPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.attributePoints - 1)));
        inkTextRef.SetText(this.m_PerkPointsValue, IntToString(this.m_levelupData.levelupdata.espionagePoints));
        if this.m_animation.IsPlaying() {
          this.m_animation.Stop();
        };
        this.m_animation = this.PlayLibraryAnimation(n"Relic_Up");
        this.PlaySound(this.m_levelupData.soundEvent, this.m_levelupData.soundAction);
      } else {
        inkTextRef.SetText(this.m_LevelUpLabelText, this.m_levelupData.proficiencyRecord.Loc_name_key());
        inkTextRef.SetText(this.m_LevelUpValue, IntToString(this.m_levelupData.levelupdata.lvl));
        inkTextRef.SetText(this.m_LevelUpPreviousValue, IntToString(Max(0, this.m_levelupData.levelupdata.lvl - 1)));
        this.bonusRecord = this.m_levelupData.proficiencyRecord.GetPassiveBonusesItem(this.m_levelupData.levelupdata.lvl - 1);
        this.bonusDisplay = new LevelRewardDisplayData();
        this.bonusDisplay.level = this.m_levelupData.levelupdata.lvl - 1;
        ...
        if Cast<Bool>(this.bonusDisplay.locPackage.GetParamsCount()) {
        ...
        iconRecord = TweakDBInterface.GetUIIconRecord(TDBID.Create("UIIcon." + ToString(this.m_levelupData.levelupdata.type)));
