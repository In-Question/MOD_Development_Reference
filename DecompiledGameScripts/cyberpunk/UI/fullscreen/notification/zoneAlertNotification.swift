...
public class ZoneAlertNotificationQueue extends gameuiGenericNotificationGameController {
  ...
  protected cb func OnPlayerAttach(playerPuppet: ref<GameObject>) -> Bool {
    ...
    this.m_playerInCombat = Equals(IntEnum<gamePSMCombat>(this.m_blackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Combat)), gamePSMCombat.InCombat);
  ...
  protected cb func OnCombatChange(value: Int32) -> Bool {
    this.m_playerInCombat = Equals(IntEnum<gamePSMCombat>(value), gamePSMCombat.InCombat);
  }
  ...
  protected cb func OnSecurityDataChange(arg: Variant) -> Bool {
    ...
    let securityZoneData: SecurityAreaData = FromVariant<SecurityAreaData>(arg);
    let securityAreaType: ESecurityAreaType = securityZoneData.shouldHideOnMinimap ? ESecurityAreaType.DISABLED : securityZoneData.securityAreaType;
    notificationData.time = this.m_duration;
    if (securityZoneData.entered || Equals(securityAreaType, ESecurityAreaType.DISABLED)) && !this.m_playerInCombat {
      switch securityAreaType {
        case ESecurityAreaType.DANGEROUS:
          ...
          userData.m_canBeMerged = false;
          if NotEquals(this.m_currentSecurityZoneType, securityAreaType) {
    ...
    this.m_currentSecurityZoneType = securityAreaType;
  }
}
