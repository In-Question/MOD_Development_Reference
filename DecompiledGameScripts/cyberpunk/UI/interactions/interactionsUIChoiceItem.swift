...
public class interactionItemLogicController extends inkLogicController {
  ...
  public final func SetZoneChange(value: Int32) -> Void {
    let zone: gamePSMZones = IntEnum<gamePSMZones>(value);
    switch zone {
  ...
  private final func SetLabel(data: script_ref<InteractionChoiceData>, isItemBroken: Bool) -> Void {
    ...
    let locText: String = GetLocalizedText(isItemBroken ? "LocKey#6887" : Deref(data).localizedName);
  ...
  public final func SetCaptionParts(const argList: script_ref<[ref<InteractionChoiceCaptionPart>]>) -> Void {
    ...
    while i < ArraySize(Deref(argList)) {
      ...
      if Equals(currType, gamedataChoiceCaptionPartType.Icon) {
        iconRecord = Deref(argList)[i] as InteractionChoiceCaptionIconPart.iconRecord;
        ...
        if TDBID.IsValid(iconID) {
          ...
          if NotEquals(iconName, n"invalid") && NotEquals(iconName, n"None") {
            showQHHolder = true;
          };
        };
        if showQHHolder {
        ...
        if Equals(currType, gamedataChoiceCaptionPartType.QuickhackCost) {
          ...
          inkTextRef.SetText(this.m_QuickHackCost, IntToString(Deref(argList)[i] as InteractionChoiceCaptionQuickhackCostPart.cost));
        };
      };
      i = i + 1;
    };
