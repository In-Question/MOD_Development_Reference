...
public class dialogWidgetGameController extends InteractionUIBase {
  ...
  private let m_hubControllers: [wref<DialogHubLogicController>];
...
public class DialogHubLogicController extends inkLogicController {
  ...
  private let m_itemControllers: [wref<DialogChoiceLogicController>];
  ...
  public final func SetData(const value: script_ref<ListChoiceHubData>, isSelected: Bool, selectedInd: Int32, hasAboveElements: Bool, hasBelowElements: Bool, currentNum: Int32, argTotalCountAcrossHubs: Int32) -> Void {
    let curSpawnRequest: wref<inkAsyncSpawnRequest>;
    let currentItem: wref<DialogChoiceLogicController>;
    this.m_data = Deref(value);
  ...
  private final func SetupTitle(const title: script_ref<String>, isActive: Bool, isPossessed: Bool) -> Void {
    ...
    this.m_titleWidget.SetText(Deref(title));
...
public class DialogChoiceLogicController extends inkLogicController {
  ...
  private edit let m_tagWrapper: inkWidgetRef;

  private edit let m_tagSeparator: inkWidgetRef;

  private edit let m_tagTextRef: inkTextRef;
  ...
  private let m_CaptionControllers: [wref<CaptionImageIconsLogicController>];

  private let m_SecondaryCaptionControllers: [wref<CaptionImageIconsLogicController>];
  ...
  public final func SetChoiceText(const text: script_ref<String>, const tag: script_ref<String>) -> Void {
    inkTextRef.SetText(this.m_ActiveTextRef, Deref(text));
    inkTextRef.SetText(this.m_tagTextRef, Deref(tag));
    if NotEquals(tag, "") && NotEquals(text, "") {
      inkWidgetRef.SetVisible(this.m_tagTextRef, true);
      inkWidgetRef.SetVisible(this.m_tagSeparator, true);
      inkWidgetRef.SetVisible(this.m_ActiveTextRef, true);
    } else {
      if Equals(tag, "") && NotEquals(text, "") {
        inkWidgetRef.SetVisible(this.m_tagTextRef, false);
        inkWidgetRef.SetVisible(this.m_tagSeparator, false);
        inkWidgetRef.SetVisible(this.m_ActiveTextRef, true);
      } else {
        if NotEquals(tag, "") && Equals(text, "") {
          inkWidgetRef.SetVisible(this.m_tagTextRef, true);
          inkWidgetRef.SetVisible(this.m_tagSeparator, false);
          inkWidgetRef.SetVisible(this.m_ActiveTextRef, false);
        };
      };
    };
  }
  ...
  public final func SetDedicatedInput(value: CName) -> Void {
    this.m_hasDedicatedInput = NotEquals(value, n"None") && NotEquals(value, n"None");
  ...
  public final func SetVisible(visible: Bool) -> Void {
    this.m_RootWidget.SetVisible(visible);
  }
    ...
    if currentSize > newSize {
      if newSize == 0 {
        ...
        while ArraySize(this.m_CaptionControllers) > newSize {
          ...
          inkCompoundRef.RemoveChildByIndex(this.m_CaptionHolder, inkCompoundRef.GetNumChildren(this.m_CaptionHolder) - 1);
        };
        while ArraySize(this.m_SecondaryCaptionControllers) > newSize {
          ArrayPop(this.m_SecondaryCaptionControllers);
          inkCompoundRef.RemoveChildByIndex(this.m_SecondaryCaptionHolder, inkCompoundRef.GetNumChildren(this.m_SecondaryCaptionHolder) - 1);
        };
      };
    };
  }
  ...
  private final func HideAllCaptionParts() -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_CaptionControllers) {
      this.m_CaptionControllers[i].HideAllHolders();
      i = i + 1;
    };
    i = 0;
    while i < ArraySize(this.m_SecondaryCaptionControllers) {
      this.m_SecondaryCaptionControllers[i].HideAllHolders();
      i = i + 1;
    };
  }
  ...
  public final func SetCaptionParts(const argList: script_ref<[ref<InteractionChoiceCaptionPart>]>) -> Void {
    ...
    let i: Int32;
    this.ResizeCaptionParts(ArraySize(Deref(argList)));
    this.HideAllCaptionParts();
    i = 0;
    while i < ArraySize(Deref(argList)) {
      ...
      if Equals(currType, gamedataChoiceCaptionPartType.Icon) {
        currController.SetGenericIcon(Deref(argList)[i] as InteractionChoiceCaptionIconPart.iconRecord);
        currentSecondaryController.HideAllHolders();
      } else {
        if Equals(currType, gamedataChoiceCaptionPartType.Blueline) {
          currBluelineHolder = Deref(argList)[i] as InteractionChoiceCaptionBluelinePart;
  ...
  private final func UpdateColors() -> Void {
    ...
    if this.m_isPhoneLockActive {
      ...
      inkWidgetRef.SetState(this.m_tagWrapper, this.m_Inactive);
    } else {
      if this.m_isSelected {
        inkWidgetRef.SetState(this.m_ActiveTextRef, this.m_Black);
        inkWidgetRef.SetState(this.m_tagWrapper, this.m_Black);
      } else {
        inkWidgetRef.SetState(this.m_ActiveTextRef, iconColor);
        inkWidgetRef.SetState(this.m_tagWrapper, iconColor);
      };
    };
  }

  public final func SetGlowline() -> Void;
  ...
  public final func SetDimmed(value: Bool) -> Void {
    ...
    inkWidgetRef.SetOpacity(this.m_tagWrapper, opacity);
...
public class CaptionImageIconsLogicController extends inkLogicController {
  ...
  @runtimeProperty("category", "Life Path")
  @runtimeProperty("tooltip", "Wrapper for Lifepath icon + text + separator")
  private edit let m_LifeWrapper: inkCompoundRef;

  @runtimeProperty("category", "Life Path")
  @runtimeProperty("tooltip", "Container for whole Life Path container")
  private edit let m_LifeHolder: inkCompoundRef;

  @runtimeProperty("category", "Skill Check")
  ...
  @runtimeProperty("category", "Pay")
  @runtimeProperty("tooltip", "Wrapper for  Pay icon + amount + separator")
  private edit let m_PayWrapper: inkCompoundRef;

  @runtimeProperty("category", "Pay")
  @runtimeProperty("tooltip", "Container for whole Pay container")
  ...
  public final func SetSelectedColor(backgroundColor: CName, iconColor: CName) -> Void {
    ...
    inkWidgetRef.SetState(this.m_LifeWrapper, iconColor);
    inkWidgetRef.SetState(this.m_PayWrapper, iconColor);
  }
