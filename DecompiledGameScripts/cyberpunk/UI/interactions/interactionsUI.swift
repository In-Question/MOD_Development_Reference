...
public class interactionWidgetGameController extends inkHUDGameController {
  ...
  private let m_widgetsPool: [wref<inkWidget>];

  private let m_widgetsCallbacks: [ref<CallbackHandle>];
  ...
  @default(interactionWidgetGameController, false)
  private let m_dataActive: Bool;
  ...
  private let m_currentOptions: [InteractionChoiceData];
  ...
  protected cb func OnUpdateInteraction(argValue: Variant) -> Bool {
    ...
    let data: InteractionChoiceHubData = FromVariant<InteractionChoiceHubData>(argValue);
    this.m_id = data.id;
    if data.active {
    ...
    this.m_dataActive = data.active;
    this.ResolveRootVisibility();
  }
  ...
  private final func ResolveRootVisibility() -> Void {
    this.m_root.SetVisible(this.m_dataActive && this.m_pendingRequests <= 0);
  }

  private final func GetSkillchecks(const choiceHubData: script_ref<InteractionChoiceHubData>) -> [UIInteractionSkillCheck] {
    let action: ref<DeviceAction>;
    let k: Int32;
    let skillCheckAction: ref<ActionSkillCheck>;
    let skillChecks: array<UIInteractionSkillCheck>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(choiceHubData).choices) {
      k = 0;
      while k < ArraySize(Deref(choiceHubData).choices[i].data) {
        action = FromVariant<ref<DeviceAction>>(Deref(choiceHubData).choices[i].data[k]);
  ...
  private final func GetSkillcheck(const choice: script_ref<InteractionChoiceData>, skillcheck: script_ref<UIInteractionSkillCheck>) -> Bool {
    let action: ref<DeviceAction>;
    let skillCheckAction: ref<ActionSkillCheck>;
    let i: Int32 = 0;
  ...
  protected cb func OnChangeActiveVisualizer(value: Variant) -> Bool {
    let info: VisualizersInfo = FromVariant<VisualizersInfo>(value);
