...
public class weaponIndicatorController extends inkHUDGameController {
  ...
  public edit const let m_triggerModeControllers: [CName];

  public edit const let m_ammoLogicControllers: [CName];

  public edit const let m_chargeLogicControllers: [CName];

  private let m_triggerModeInstances: [wref<TriggerModeLogicController>];

  private let m_ammoLogicInstances: [wref<AmmoLogicController>];

  private let m_chargeLogicInstances: [wref<ChargeLogicController>];
  ...
  protected cb func OnPlayerAttach(playerPuppet: ref<GameObject>) -> Bool {
    ...
    if Cast<Bool>(ArraySize(this.m_triggerModeInstances)) {
    ...
    if Cast<Bool>(ArraySize(this.m_ammoLogicInstances)) {
    ...
    if Cast<Bool>(ArraySize(this.m_chargeLogicInstances)) {
  ...
  protected cb func OnPlayerDetach(playerPuppet: ref<GameObject>) -> Bool {
    if Cast<Bool>(ArraySize(this.m_triggerModeInstances)) && IsDefined(this.m_onTriggerMode) {
      this.m_blackboard.UnregisterListenerVariant(GetAllBlackboardDefs().Weapon.TriggerMode, this.m_onTriggerMode);
    };
    if Cast<Bool>(ArraySize(this.m_ammoLogicInstances)) && IsDefined(this.m_onMagazineAmmoCount) {
      this.bb.UnregisterListenerVariant(GetAllBlackboardDefs().UI_EquipmentData.EquipmentData, this.m_onMagazineAmmoCount);
    };
    if Cast<Bool>(ArraySize(this.m_chargeLogicInstances)) && IsDefined(this.m_onCharge) {
  ...
  protected cb func OnTriggerMode(value: Variant) -> Bool {
    ...
    let i: Int32 = 0;
    while i < count {
      this.m_triggerModeInstances[i].OnTriggerModeChanged(FromVariant<ref<TriggerMode_Record>>(value));
      i += 1;
    };
  }
  ...
  protected cb func OnMagazineAmmoCapacity(value: Variant) -> Bool {
    let count: Int32;
    let i: Int32;
    this.m_BufferedRosterData = FromVariant<ref<SlotDataHolder>>(value);
    ...
    if ItemID.IsValid(currentData.weaponID) {
      ...
      i = 0;
      while i < count {
        this.m_ammoLogicInstances[i].OnMagazineAmmoCapacityChanged(Cast<Uint32>(this.m_ActiveWeapon.ammoCurrent));
        i += 1;
      };
    };
  }
