...
public class KillMarkerGameController extends inkGameController {
  ...
  protected cb func OnNPCNeutralized(value: Variant) -> Bool {
    let incomingData: ENeutralizeType = FromVariant<ENeutralizeType>(value);
    if NotEquals(incomingData, ENeutralizeType.None) && !this.m_animProxy.IsPlaying() {
  ...
  protected cb func OnKillMarkerComplete(proxy: ref<inkAnimProxy>) -> Bool {
    let data: ENeutralizeType = ENeutralizeType.None;
