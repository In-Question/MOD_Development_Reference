...
public class CrosshairGameController_Launcher extends gameuiCrosshairBaseGameController {
  ...
  protected cb func OnPSMLeftHandCyberwareStateChanged(value: Int32) -> Bool {
    this.UpdateCrosshairState(IntEnum<gamePSMLeftHandCyberware>(value));
  }
  ...
  protected func OnState_Aim() -> Void {
    let hasReilcPerk: Bool = RPGManager.HasStatFlag(this.GetPlayerControlledObject(), gamedataStatType.CanUseNewMeleewareAttackSpyTree);
    if hasReilcPerk {
      this.PlayLibraryAnimation(n"go_to_charge_state100");
    } else {
      this.PlayLibraryAnimation(n"go_to_charge_state75");
    };
  }
