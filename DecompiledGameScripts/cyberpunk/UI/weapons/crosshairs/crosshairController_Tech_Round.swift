...
public class CrosshairGameController_Tech_Round extends BaseTechCrosshairController {
  ...
  private let m_crosshair: wref<inkWidget>;
  ...
  private let m_chargeAnimationProxy: ref<inkAnimProxy>;
  ...
  @default(CrosshairGameController_Tech_Round, false)
  private let m_stateADS: Bool;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    this.m_crosshair = this.GetWidget(n"reticle") as inkCanvas;
  ...
  protected final func OnTriggerModeChanged(value: Variant) -> Void {
    ...
    if Equals(this.m_currentFireMode, gamedataTriggerMode.Charge) && !this.m_stateADS {
  ...
  protected final func OnAmmoCountChanged(value: Uint32) -> Void {
    this.m_currentAmmo = Cast<Int32>(value);
  }

  protected final func OnAmmoCapacityChanged(value: Uint32) -> Void {
    this.m_currentMaxAmmo = Cast<Int32>(value);
  }
  ...
  protected final func OnChargeChanged(charge: Float) -> Void {
    ...
    this.m_chargeBarBG.SetSize(new Vector2(WeaponObject.GetBaseMaxChargeThreshold(weaponObject) * 3.00, 6.00));
    this.m_chargeBarMG.SetSize(new Vector2(WeaponObject.GetFullyChargedThreshold(weaponObject) * 3.00, 6.00));
    if charge >= actualMaxValue {
      ...
      if IsDefined(this.m_chargeAnimationProxy) {
        ...
        this.m_chargeBar.SetOpacity(1.00);
        this.m_crosshair.SetOpacity(1.00);
      };
    };
  }
  ...
  protected final func ColapseCrosshair(full: Bool, duration: Float) -> Void {
    ...
    this.m_root.PlayAnimation(anim);
    anim = new inkAnimDef();
    ...
    marginInterpolator.SetMode(inkanimInterpolationMode.EasyIn);
    anim.AddInterpolator(marginInterpolator);
    inkWidgetRef.StopAllAnimations(this.m_bottomPart);
  ...
  protected final func ExpandCrosshair(full: Bool, duration: Float) -> Void {
    ...
    marginInterpolator.SetEndMargin(new inkMargin(-this.m_bufferedSpread.X * this.offsetLeftRight, 0.00, 0.00, 0.00));
    marginInterpolator.SetDuration(duration);
    marginInterpolator.SetType(inkanimInterpolationType.Linear);
    marginInterpolator.SetMode(inkanimInterpolationMode.EasyIn);
    anim.AddInterpolator(marginInterpolator);
    ...
    marginInterpolator = new inkAnimMargin();
    marginInterpolator.SetStartMargin(new inkMargin(0.00, 0.00, 0.00, 0.00));
    marginInterpolator.SetEndMargin(new inkMargin(0.00, this.m_bufferedSpread.Y, 0.00, 0.00));
    marginInterpolator.SetDuration(duration);
    marginInterpolator.SetType(inkanimInterpolationType.Linear);
    marginInterpolator.SetMode(inkanimInterpolationMode.EasyIn);
    anim.AddInterpolator(marginInterpolator);
