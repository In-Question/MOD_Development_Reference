...
public class blunderbussWeaponController extends inkGameController {
  ...
  private let m_shots: [wref<inkWidget>];
  ...
  protected cb func OnTriggerMode(value: Variant) -> Bool {
    let isChargeMode: Bool = Equals(FromVariant<ref<TriggerMode_Record>>(value).Type(), gamedataTriggerMode.Charge);
  ...
  protected cb func OnMagazineAmmoCount(value: Uint32) -> Bool {
    ...
    let i: Int32 = 0;
    while i < count {
      this.m_shots[i].SetVisible(i < Cast<Int32>(value));
      i += 1;
    };
  }
}
