...
public class CooldownRequest extends IScriptable {
  ...
  private let m_contactBook: [PSOwnerData];
  ...
  public final func SetUpAdvanced(action: ref<BaseScriptableAction>, const addressees: script_ref<[PSOwnerData]>, shouldTriggerCooldownImmediately: Bool) -> Void {
    this.m_action = action;
    this.m_contactBook = Deref(addressees);
    this.SetUpTriggerType(shouldTriggerCooldownImmediately);
  }
  ...
  public final func GetContactBook() -> [PSOwnerData] {
...
public class CooldownPackage extends IScriptable {
  ...
  private let m_addressees: [PSOwnerData];
  ...
  public final const func GetAddressees() -> [PSOwnerData] {
...
public class CooldownStorage extends IScriptable {
  ...
  private let m_packages: [ref<CooldownPackage>];

  private let m_currentID: Uint32;

  private let m_map: [CooldownPackageDelayIDs];
  ...
  private final func UpdateMap(label: CooldownStorageID, const ids: script_ref<[DelayID]>) -> Void {
    let mapEntry: CooldownPackageDelayIDs;
    mapEntry.packageID = label;
    mapEntry.delayIDs = Deref(ids);
