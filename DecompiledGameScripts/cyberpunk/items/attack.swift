...
public native class gameAttackComputed extends IScriptable {
  ...
  public final native func GetAttackValues() -> [Float];

  public final native func SetAttackValues(attackValues: [Float]) -> Void;

  public final native func GetOriginalAttackValues() -> [Float];
  ...
  public final func GetDominatingDamageType() -> gamedataDamageType {
    ...
    let i: Int32 = 0;
    while i < 4 {
      currentValue = this.GetAttackValue(IntEnum<gamedataDamageType>(i));
  ...
  public func OnTick(weapon: ref<WeaponObject>) -> Void {
    ...
    if IsDefined(weapon) {
      ...
      if IsDefined(beamEffect) {
        ...
        if IsDefined(targetComponentVariant) {
          targetComponent = FromVariant<wref<IPlacedComponent>>(targetComponentVariant);
        };
...
public class RoyceLaserSight extends Attack_Beam {
  ...
  public func OnTick(weapon: ref<WeaponObject>) -> Void {
    ...
    if IsDefined(weapon) {
      ...
      if IsDefined(beamEffect) {
        EffectData.GetVariant(beamEffect.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.targetComponent, targetComponentVariant);
        targetComponent = FromVariant<wref<IComponent>>(targetComponentVariant);
        if IsDefined(targetComponent) {
          target = targetComponent.GetEntity();
        };
        this.HandleTargetEvents(weapon, target);
