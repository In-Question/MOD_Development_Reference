...
public native class ItemObject extends TimeDilatable {
  ...
  public final native func GetAnimationParameters() -> [CName];
  ...
  public final native const func IsVisualSpawnAttached() -> Bool;
  ...
  protected cb func OnVisualSpawnAttached() -> Bool {
    ...
    this.m_isIconic = RPGManager.IsItemBroken(this.GetItemData());
    if this.IsContainer() {
      this.ToggleLootHighlight(this.IsQuest());
      this.EvaluateLootQualityByTask();
    };
    if this.GetItemData().HasTag(n"WorkspotSyncAnimated") {
      owner = this.GetOwner();
      if IsDefined(owner) {
  ...
  private final func EvaluateLootQuality() -> Void {
    ...
    this.m_isIconic = this.m_isIconic || RPGManager.IsItemIconic(this.GetItemData());
    this.m_isBroken = this.m_isBroken || RPGManager.IsItemBroken(this.GetItemData());
    this.ToggleLootHighlight(this.IsQuest());
  }
  ...
  public const func IsQuest() -> Bool {
    return super.IsQuest() || this.GetItemData().HasTag(n"Quest");
  }
