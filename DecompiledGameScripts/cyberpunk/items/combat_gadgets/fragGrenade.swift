...
public native class WeaponGrenade extends ItemObject {
  ...
  public final native func ActivateFocusedShootingAim(aimRequest: AimRequest) -> Void;

  public final native func DeactivateFocusedShootingAim() -> Void;

  public final native func IsGrenadeTargetedWithFocusedShootingPerk() -> Bool;

  public final func EnableFocusedShootingHighlight() -> Void {
    ...
    highlightData.inTransitionTime = 0.00;
    highlightData.outTransitionTime = 0.50;
  ...
  public final func DisableFocusedShootingHighlight() -> Void {
    let highlightData: ref<FocusForcedHighlightData> = new FocusForcedHighlightData();
    highlightData.sourceID = this.GetOwner().GetEntityID();
    highlightData.sourceName = this.GetOwner().GetClassName();
    highlightData.outlineType = EFocusOutlineType.HOSTILE;
  ...
  public final func TriggerLookAtThisGrenade() -> Void {
    ...
    aimRequest.cameraInputMagToBreak = 0.40;
    aimRequest.cameraMouseInputMagToBreak = 10.00;
    ...
    this.ActivateFocusedShootingAim(aimRequest);
  }
}
...
public class BaseGrenade extends WeaponGrenade {
  ...
  protected let m_timeSinceExplosion: Float;
  ...
  protected let m_spawnBlinkEffectDelayID: DelayID;
  ...
  @default(BaseGrenade, false)
  protected let m_hasExploded: Bool;

  protected let m_targetTracker: ref<EffectInstance>;

  protected let m_potentialHomingTargets: [GrenadePotentialHomingTarget];

  protected let m_homingGrenadeTarget: GrenadePotentialHomingTarget;

  protected let m_cuttingGrenadePotentialTargets: [CuttingGrenadePotentialTarget];

  protected let m_drillTargetPosition: Vector4;

  protected let m_attacksSpawned: [ref<EffectInstance>];
  ...
  protected let m_smokeEffectRadius: Float;

  protected let m_smokeEffectDuration: Float;

  protected let m_smokeVisionBlockerId: Uint32;

  @default(BaseGrenade, false)
  private let m_isSmokeEffectActive: Bool;

  @default(BaseGrenade, 3.5f)
  private let m_smokeVFXDeescalationOffset: Float;
  ...
  protected final func PreloadAttackResources() -> Void {
    ...
    PreloadGameEffectAttackResources(this.m_tweakRecord.NpcHitReactionAttack() as Attack_GameEffect_Record, effectSystem);
    PreloadGameEffectAttackResources(this.m_tweakRecord.ShallowWaterAttack() as Attack_GameEffect_Record, effectSystem);
    PreloadGameEffectAttackResources(this.m_tweakRecord.DeepWaterAttack() as Attack_GameEffect_Record, effectSystem);
    PreloadGameEffectAttackResources(this.m_tweakRecord.EnhancedPerkAttack() as Attack_GameEffect_Record, effectSystem);
  ...
  protected final func ReleaseAttackResources() -> Void {
    ...
    ReleaseGameEffectAttackResources(this.m_tweakRecord.NpcHitReactionAttack() as Attack_GameEffect_Record, effectSystem);
    ReleaseGameEffectAttackResources(this.m_tweakRecord.ShallowWaterAttack() as Attack_GameEffect_Record, effectSystem);
    ReleaseGameEffectAttackResources(this.m_tweakRecord.DeepWaterAttack() as Attack_GameEffect_Record, effectSystem);
    ReleaseGameEffectAttackResources(this.m_tweakRecord.EnhancedPerkAttack() as Attack_GameEffect_Record, effectSystem);
  ...
  protected cb func OnProjectileInitialize(eventData: ref<gameprojectileSetUpEvent>) -> Bool {
    ...
    this.m_projectileComponent.SetCollisionEvaluator(new GrenadeCollisionEvaluator());
    this.m_projectileComponent.SetDeactivationDepth(this.m_tweakRecord.SinkingDepth());
    this.PreloadAttackResources();
  }

  protected func Reset() -> Void {
    let invalidHomingTarget: GrenadePotentialHomingTarget;
    let delaySystem: ref<DelaySystem> = GameInstance.GetDelaySystem(this.GetGame());
    let invalidDelayID: DelayID = GetInvalidDelayID();
    this.m_isAlive = true;
    this.m_isSinking = false;
    this.m_landedOnGround = false;
    this.m_isStuck = false;
    this.m_isTracking = false;
    this.m_timeSinceLaunch = 0.00;
    this.m_timeSinceExplosion = 0.00;
    this.m_timeOfFreezing = 999999.00;
    this.m_hasClearedIgnoredObject = false;
    this.m_isLockingOn = false;
    this.m_isLockedOn = false;
    this.m_readyToTrack = false;
    this.m_lockOnFailed = false;
    this.m_shotDownByThePlayer = false;
    this.m_forceExplosion = false;
    this.m_detonateOnImpact = false;
    this.m_targetAcquired = false;
    this.m_collidedWithNPC = false;
    this.m_isBroadcastingStim = false;
    this.m_playingFastBeep = false;
    this.m_hasExploded = false;
    this.m_homingGrenadeTarget = invalidHomingTarget;
    ArrayClear(this.m_potentialHomingTargets);
    ArrayClear(this.m_cuttingGrenadePotentialTargets);
    ArrayClear(this.m_attacksSpawned);
    this.m_drillTargetPosition = Vector4.EmptyVector();
    this.lastHitNormal = Vector4.EmptyVector();
    ...
    this.m_smokeEffectRadius = TweakDBInterface.GetFloat(ItemID.GetTDBID(this.GetItemID()) + t".smokeEffectRadius", 0.00);
    this.m_smokeEffectDuration = TweakDBInterface.GetFloat(ItemID.GetTDBID(this.GetItemID()) + t".smokeEffectDuration", 0.00);
    this.m_smokeVisionBlockerId = 0u;
    if this.m_spawnBlinkEffectDelayID != invalidDelayID {
      delaySystem.CancelDelay(this.m_spawnBlinkEffectDelayID);
      this.m_spawnBlinkEffectDelayID = invalidDelayID;
    };
    if this.m_detonateRequestDelayID != invalidDelayID {
      delaySystem.CancelDelay(this.m_detonateRequestDelayID);
      this.m_detonateRequestDelayID = invalidDelayID;
    };
    if this.m_releaseRequestDelayID != invalidDelayID {
      delaySystem.CancelDelay(this.m_releaseRequestDelayID);
  ...
  public final func GetAttackRadius() -> Float {
    let data: wref<gameItemData>;
    let value: Float;
    if this.IsGrenadeTargetedWithFocusedShootingPerk() && this.m_tweakRecord.EnhancedAttackRadiusFromPerk() > 0.00 {
      return this.m_tweakRecord.EnhancedAttackRadiusFromPerk();
    };
  ...
  public final func GetAttackDuration() -> Float {
    if this.IsGrenadeTargetedWithFocusedShootingPerk() && this.m_tweakRecord.EnhancedAttackDurationFromPerk() > 0.00 {
      return this.m_tweakRecord.EnhancedAttackDurationFromPerk();
    };
    return this.m_tweakRecord.AttackDuration();
  }

  public final func GetDefaultAttack() -> wref<Attack_Record> {
    if this.IsGrenadeTargetedWithFocusedShootingPerk() && IsDefined(this.m_tweakRecord.EnhancedPerkAttack()) {
      return this.m_tweakRecord.EnhancedPerkAttack();
    };
    return this.m_tweakRecord.Attack();
  }
  ...
  protected cb func OnShoot(eventData: ref<gameprojectileShootEvent>) -> Bool {
    ...
    GameInstance.GetQuestsSystem(this.GetGame()).SetFact(n"player_threw_a_grenade", 1);
    this.Reset();
    if this.m_user != null && this.m_mappinID.value == 0u {
      grenadeMappinScriptData = new GrenadeMappinData();
      grenadeMappinScriptData.m_grenadeType = this.GetGrenadeType();
      grenadeMappinScriptData.m_iconID = this.GetMappinIconIDForGrenadeType(grenadeMappinScriptData.m_grenadeType);
    ...
    if this.m_isContinuousEffect {
      ...
      this.m_spawnBlinkEffectDelayID = GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, spawnBlinkEffectRequest, this.m_delayToDetonate, true);
      this.RequestGrenadeDetonation(this.m_delayToDetonate);
    };
    if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Sticky) {
  ...
  protected cb func OnCollision(eventData: ref<gameprojectileHitEvent>) -> Bool {
    ...
    if !this.m_isAlive || this.m_isSinking {
      return false;
    };
    i = 0;
    while i < ArraySize(eventData.hitInstances) {
      ...
      if hitInstance.isWaterSurfaceImpact {
        if !hitInstance.hitThroughWaterSurface {
          ...
          if !GameObject.IsCooldownActive(this, n"grenade_water_impact_effects") {
            this.SpawnEffectFromLibrary(n"splash_effect");
            GameObject.PlaySound(this, n"w_bul_hit_water_heavy");
            RenderingSystem.AddWaterImpulse(hitInstance.position, this.m_tweakRecord.WaterSurfaceImpactImpulseRadius(), this.m_tweakRecord.WaterSurfaceImpactImpulseStrength());
            GameObject.StartCooldown(this, n"grenade_water_impact_effects", 0.20);
          };
        };
      } else {
        if this.m_hasHitWater && hitInstance.position.Z - this.m_waterHeight <= this.m_tweakRecord.SinkingDepth() {
          break;
        };
        puppetTarget = hitInstance.hitObject as ScriptedPuppet;
        this.lastHitNormal = Cast<Vector4>(hitInstance.traceResult.normal);
        ...
        if this.IsGrenadeOfType(EGrenadeType.Piercing) && (NotEquals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Homing) || this.m_targetAcquired) {
          ...
          if this.m_additionalAttackOnCollision && this.m_targetAcquired {
            ...
            if this.m_detonateOnImpact {
              ...
              if this.m_landedOnGround && !this.m_isBroadcastingStim {
              ...
              if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Homing) && !this.m_isTracking {
                ...
                if this.IsGrenadeOfType(EGrenadeType.Cutting) && !this.m_isTracking && this.m_delayToDetonate + this.GetAttackDuration() - this.m_timeSinceLaunch > 0.50 {
  ...
  protected cb func OnHit(evt: ref<gameHitEvent>) -> Bool {
    ...
    if this.m_isAlive {
      ...
      if instigatorIsPlayer && this.m_canBeShot {
        ...
        this.Detonate();
        RPGManager.AwardExperienceInstantly(GetPlayer(this.GetGame()), 100, gamedataProficiencyType.CoolSkill);
      } else {
        if this.m_isStuck && evt.attackData.HasFlag(hitFlag.DetonateGrenades) {
          this.RequestGrenadeDetonation(Vector4.Distance(this.GetWorldPosition(), evt.attackData.GetAttackPosition()) / 5.00);
        };
      };
    };
  }
  ...
  protected cb func OnTick(eventData: ref<gameprojectileTickEvent>) -> Bool {
    ...
    if this.m_hasExploded {
      this.m_timeSinceExplosion += eventData.deltaTime;
    };
    ...
    if isUnderwater && !this.m_isSinking {
      this.TryToSinkAndRequestDetonation();
    };
    if this.m_isSinking {
      if this.IsGrenadeOfType(EGrenadeType.Cutting) && this.m_timeSinceLaunch > 2.50 {
        this.Release(true);
      };
      return false;
    };
    if this.m_isAlive && !this.m_isContinuousEffect {
      if this.m_forceExplosion {
        ...
        if this.m_detonationTimerActive && this.m_detonationTimer >= this.m_landedOnGround ? this.m_tweakRecord.DeliveryMethod().DetonationTimer() * 0.50 : this.m_tweakRecord.DeliveryMethod().DetonationTimer() {
          this.Detonate(this.lastHitNormal);
        } else {
      ...
      if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Sticky) {
        if this.m_tweakRecord.DeliveryMethod().DetonationTimer() - this.m_detonationTimer <= 0.80 && !this.m_playingFastBeep {
          this.m_playingFastBeep = true;
          this.PlayStickyGrenadeShortBeepSound();
        };
      } else {
        if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Homing) {
          if !isUnderwater && this.m_timeSinceLaunch >= this.m_timeOfFreezing && !this.m_isLockingOn && !this.m_lockOnFailed {
            ...
            if this.m_isLockingOn && this.m_timeSinceLaunch - this.m_timeOfFreezing >= (this.m_tweakRecord.DeliveryMethod() as HomingGDM_Record).LockOnDelay() && !this.m_readyToTrack {
              ...
              if this.m_isLockingOn && this.m_timeSinceLaunch - this.m_timeOfFreezing >= (this.m_tweakRecord.DeliveryMethod() as HomingGDM_Record).FreezeDuration() && !this.m_targetAcquired {
                ...
                this.RequestGrenadeDetonation((this.m_tweakRecord.DeliveryMethod() as HomingGDM_Record).LockOnFailDetonationDelay());
    ...
    if !isUnderwater {
      if this.m_isAlive && this.IsGrenadeOfType(EGrenadeType.Cutting) && this.m_timeOfFreezing > 0.00 && this.m_timeSinceLaunch > this.m_timeOfFreezing && !this.m_isLockingOn {
      ...
      if this.m_isContinuousEffect && this.m_timeSinceLaunch >= this.GetAttackDuration() {
      ...
      if this.m_isAlive && this.IsGrenadeOfType(EGrenadeType.Smoke) {
        if this.m_isSmokeEffectActive && this.m_timeSinceExplosion >= this.m_smokeEffectDuration - this.m_smokeVFXDeescalationOffset {
          GameObject.BreakReplicatedEffectLoopEvent(this, n"fx_explosion");
          this.m_isSmokeEffectActive = false;
        };
        if this.m_timeSinceExplosion >= this.m_smokeEffectDuration {
          this.UnregisterSmokeExplosion();
          this.Release(true);
        };
      };
    };
  ...
  protected func Detonate(opt hitNormal: Vector4) -> Void {
    ...
    let attack: wref<Attack_Record>;
    let attackDuration: Float;
    let attackRadius: Float;
    let currentPosition: Vector4;
    let detonationLocation: Vector4;
    let detonationRumbleName: String;
    let detonationSoundName: CName;
    let detonationStimRadius: Float;
    let effect: ref<EffectInstance>;
    let minDetonationDelay: Float;
    let vfxOffset: Vector4;
    let isUnderwater: Bool = false;
    ...
    isUnderwater = this.IsUnderwater();
    if isUnderwater {
      isDeepUnderwater = currentPosition.Z - this.m_waterHeight <= this.m_tweakRecord.DeepWaterDepth();
    };
    if this.m_hasExploded {
      return;
    };
    if this.m_isContinuousEffect && isUnderwater {
      minDetonationDelay = this.m_delayToDetonate + this.m_tweakRecord.SinkingDetonationDelay();
      if this.m_timeSinceLaunch < minDetonationDelay {
        this.RequestGrenadeDetonation(minDetonationDelay - this.m_timeSinceLaunch);
        return;
      };
    };
    if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Homing) {
    ...
    attackDuration = this.GetAttackDuration();
    if isUnderwater {
      attack = isDeepUnderwater ? this.m_tweakRecord.DeepWaterAttack() : this.m_tweakRecord.ShallowWaterAttack();
      attackRadius = attack.Range();
      if this.m_tweakRecord.SpawnFxAtWaterSurface() {
        vfxOffset = new Vector4(0.00, 0.00, this.m_waterHeight - currentPosition.Z, 0.00);
      };
      effect = this.SpawnAttack(attack, attackRadius, attackDuration, hitNormal, currentPosition, vfxOffset);
      effect.AttachToEntity(this, GetAllBlackboardDefs().EffectSharedData.position);
      this.AddWaterImpulsesOnDetonation(currentPosition, attackRadius, 4);
      detonationRumbleName = this.m_tweakRecord.UnderwaterDetonationRumbleName();
      detonationStimRadius = this.m_tweakRecord.UnderwaterDetonationStimRadius();
    } else {
      attack = this.GetDefaultAttack();
      attackRadius = this.GetAttackRadius();
      if this.IsGrenadeOfType(EGrenadeType.Cutting) {
        this.SpawnLaserAttack(attack, 48, attackRadius, attackDuration, false, 0.01);
      } else {
        effect = this.SpawnAttack(attack, attackRadius, attackDuration, hitNormal);
        effect.AttachToEntity(this, GetAllBlackboardDefs().EffectSharedData.position);
        if this.IsGrenadeOfType(EGrenadeType.Recon) {
          this.SpawnLaserAttack(TweakDBInterface.GetAttackRecord(t"Attacks.ReconGrenadeBeams"), 36, attackRadius, attackDuration, true);
          additionalEffect = this.SpawnAttack(this.m_tweakRecord.AdditionalAttack(), attackRadius, attackDuration, hitNormal);
          additionalEffect.AttachToEntity(this, GetAllBlackboardDefs().EffectSharedData.position);
        };
      };
      if this.m_additionalAttackOnDetonate && !this.IsGrenadeOfType(EGrenadeType.Recon) {
        attack = this.m_tweakRecord.AdditionalAttack();
        additionalEffect = this.SpawnAttack(attack, attack.Range(), 0.00, hitNormal);
        additionalEffect.AttachToEntity(this, GetAllBlackboardDefs().EffectSharedData.position);
      };
      this.SpawnVisualEffectsOnDetonation();
      detonationSoundName = StringToName(this.m_tweakRecord.DetonationSound());
      GameObject.PlayMetadataEvent(this, detonationSoundName);
      GameInstance.GetAudioSystem(this.GetGame()).PlayShockwave(detonationSoundName, detonationLocation);
      detonationRumbleName = this.m_tweakRecord.DetonationRumbleName();
      detonationStimRadius = this.m_tweakRecord.DetonationStimRadius();
    };
    GameObject.PlaySound(this, TDB.GetCName(TDBID.Create("rumble.world." + detonationRumbleName)));
    this.TriggerStimuli(detonationStimRadius);
    if !this.IsGrenadeOfType(EGrenadeType.Recon) && !this.IsGrenadeOfType(EGrenadeType.Cutting) && !this.IsGrenadeOfType(EGrenadeType.Smoke) {
      this.SendCombatGadgetIsAliveFeature();
    };
    if this.m_tweakRecord.RemoveMeshOnDetonation() || isUnderwater {
      if IsDefined(this.m_visualComponent) {
        this.m_visualComponent.Toggle(false);
      };
      if IsDefined(this.m_stickyMeshComponent) {
        this.m_stickyMeshComponent.Toggle(false);
        this.m_decalsStickyComponent.Toggle(false);
      };
      if IsDefined(this.m_homingMeshComponent) {
        this.m_homingMeshComponent.Toggle(false);
      };
    };
    this.OnExplosion();
    this.m_hasExploded = true;
    if this.m_tweakRecord.ReleaseOnDetonation() || isUnderwater {
      this.Release(isUnderwater);
    } else {
      if this.IsGrenadeOfType(EGrenadeType.Smoke) {
        this.ReleaseMappin();
        this.RegisterSmokeExplosion(currentPosition);
      };
    };
  }
  ...
  protected func SpawnVisualEffectsOnDetonation() -> Void {
    if this.IsGrenadeOfType(EGrenadeType.Incendiary) {
      ...
      if this.IsGrenadeOfType(EGrenadeType.Piercing) {
        ...
        if this.IsGrenadeOfType(EGrenadeType.Smoke) {
          GameObject.StartReplicatedEffectEvent(this, n"fx_explosion", true, true);
          this.m_isSmokeEffectActive = true;
        };
      };
    };
  }

  protected final func AddWaterImpulsesOnDetonation(position: Vector4, attackRadius: Float, numImpulses: Int32) -> Void {
    let i: Int32;
    let impulseRadius: Float = attackRadius * this.m_tweakRecord.WaterDetonationImpulseRadiusCoef() * RandRangeF(0.80, 1.20);
    let impulseStrength: Float = this.m_tweakRecord.WaterDetonationImpulseStrength() * RandRangeF(0.10, 1.20);
  ...
  private final func RegisterSmokeExplosion(position: Vector4) -> Void {
    let blockerShape: ref<VisionBlockerShape_BasicHemisphere> = VisionBlockerShape_BasicHemisphere.Create(position, this.m_smokeEffectRadius);
    let visionBlockersRegistrar: ref<VisionBlockersRegistrar> = GameInstance.GetSenseManager(this.GetGame()).GetVisionBlockersRegistrar();
    this.m_smokeVisionBlockerId = visionBlockersRegistrar.RegisterVisionBlocker(blockerShape, EVisionBlockerType.Smoke, 0.00, 5.00);
    visionBlockersRegistrar.AttachToParent(this.m_smokeVisionBlockerId, this);
  }

  private final func UnregisterSmokeExplosion() -> Void {
    GameInstance.GetSenseManager(this.GetGame()).GetVisionBlockersRegistrar().UnregisterVisionBlocker(this.m_smokeVisionBlockerId);
  }
  ...
  protected func IsGrenadeOfType(compareType: EGrenadeType) -> Bool {
    ...
    switch compareType {
      ...
      case EGrenadeType.Smoke:
        return ArrayContains(tags, n"SmokeGrenade");
  ...
  private final func GetGrenadeType() -> EGrenadeType {
    let tags: array<CName> = this.m_tweakRecord.Tags();
  ...
  private final func GetMappinIconIDForGrenadeType(grenadeType: EGrenadeType) -> TweakDBID {
    switch grenadeType {
      case EGrenadeType.Frag:
  ...
  protected final func TriggerStimuli(radius: Float) -> Void {
    ...
    if radius > 0.00 {
      ...
      if IsDefined(broadcaster) {
        ...
        broadcaster.TriggerSingleBroadcast(this, this.m_tweakRecord.DetonationStimType().Type(), radius, investigateData);
      };
    };
  }
  ...
  protected final func TriggerGrenadeLandedStimuli(hasLifeTime: Bool) -> Void {
    ...
    if IsDefined(broadcaster) {
      if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Sticky) || this.IsGrenadeOfType(EGrenadeType.Recon) {
        if !this.m_landedCooldownActive {
          investigateData.attackInstigator = this.GetUser();
          broadcaster.TriggerSingleBroadcast(this, gamedataStimType.Recon, this.GetAttackRadius(), investigateData);
  ...
  protected final func SpawnLaserAttack(attackRecord: ref<Attack_Record>, numberOfLasers: Int32, opt range: Float, opt duration: Float, playSlotAnimation: Bool, opt delayPerLaser: Float) -> Void {
    ...
    while i < numberOfLasers {
      if delayPerLaser <= 0.00 {
        ...
        GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, spawnLaserEvent, delayPerLaser * Cast<Float>(i), true);
      };
      i += 1;
    };
  }
  ...
  protected final func RequestGrenadeRelease(delay: Float) -> Void {
    let releaseRequestEvent: ref<GrenadeReleaseRequestEvent>;
    if this.m_releaseRequestDelayID == GetInvalidDelayID() {
      releaseRequestEvent = new GrenadeReleaseRequestEvent();
      this.m_releaseRequestDelayID = GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, releaseRequestEvent, delay, true);
    };
  }
  ...
  protected final func Release(opt isInstant: Bool) -> Void {
    ...
    if !GameInstance.GetRuntimeInfo(this.GetGame()).IsMultiplayer() || this.MultiplayerCanRelease() {
      ...
      delay = this.GetAttackDuration() > 0.00 ? this.GetAttackDuration() : 1.50;
      if isInstant {
        delay = 0.01;
      };
  ...
  private final func InitializeRotation() -> Void {
    ...
    if !useSeed {
      seed = Cast<Int32>(timeSystem.GetGameTimeStamp());
    };
    i = 0;
  ...
  protected final func DrillThrough(const collisionEventData: script_ref<gameprojectileHitInstance>) -> Void {
    ...
    this.m_drillTargetPosition = this.GetDrillTargetPosition(Deref(collisionEventData).position, Cast<Vector4>(Deref(collisionEventData).traceResult.normal));
    if IsDefined(Deref(collisionEventData).hitObject as ScriptedPuppet) {
    ...
    this.RequestGrenadeDetonation(drillDuration + 2.50);
  ...
  protected final func GetDrillTargetPosition(currentPosition: Vector4, hitNormal: Vector4) -> Vector4 {
    ...
    if Equals(geometryDescriptionResult.queryStatus, worldgeometryDescriptionQueryStatus.OK) && Equals(geometryDescriptionResult.obstacleDepthStatus, worldgeometryProbingStatus.None) {
  ...
  protected final func RequestGrenadeDetonation(delay: Float) -> Void {
    let detonateRequestEvent: ref<GrenadeDetonateRequestEvent>;
    if this.m_detonateRequestDelayID == GetInvalidDelayID() {
      detonateRequestEvent = new GrenadeDetonateRequestEvent();
      this.m_detonateRequestDelayID = GameInstance.GetDelaySystem(this.GetGame()).DelayEvent(this, detonateRequestEvent, delay, true);
    };
  }
  ...
  public final func IsEMP() -> Bool {
    let tags: array<CName> = this.m_tweakRecord.Tags();
    return ArrayContains(tags, n"EMPGrenade");
  }

  public final func GetGrenadeAttackStatusEffectDuration() -> Float {
    let attackStatusEffects: array<wref<StatusEffectAttackData_Record>>;
    let duration: Float;
    let durationStatusEffects: array<wref<StatModifier_Record>>;
    let gameInstance: GameInstance;
    this.m_tweakRecord.Attack().StatusEffects(attackStatusEffects);
    attackStatusEffects[0].StatusEffect().Duration().StatModifiers(durationStatusEffects);
    gameInstance = this.GetGame();
    duration = RPGManager.CalculateStatModifiers(durationStatusEffects, gameInstance, GetPlayer(gameInstance), Cast<StatsObjectID>(GetPlayer(gameInstance).GetEntityID()));
    return duration;
  }

  protected final func IsUnderwater() -> Bool {
    let currentPosition: Vector4;
    let isUnderwater: Bool = false;
    if this.m_hasHitWater {
      currentPosition = this.GetWorldPosition();
      isUnderwater = currentPosition.Z - this.m_waterHeight <= this.m_tweakRecord.ShallowWaterDepth();
    };
    return isUnderwater;
  }

  protected final func TryToSinkAndRequestDetonation() -> Void {
    let currentPosition: Vector4;
    let delaySystem: ref<DelaySystem>;
    let invalidDelayID: DelayID;
    let parabolicTrajectoryParams: ref<ParabolicTrajectoryParams>;
    if !this.m_isAlive || this.m_isSinking || !this.m_hasHitWater {
      return;
    };
    currentPosition = this.GetWorldPosition();
    if this.IsGrenadeOfType(EGrenadeType.Smoke) {
    ...
    if currentPosition.Z - this.m_waterHeight > this.m_tweakRecord.SinkingDepth() {
      return;
    };
    this.m_isSinking = true;
    this.RequestGrenadeDetonation(this.m_tweakRecord.SinkingDetonationDelay());
    if Equals(this.deliveryMethod, gamedataGrenadeDeliveryMethodType.Homing) {
      GameObject.StopReplicatedEffectEvent(this, n"fx_homing_freeze");
      GameObject.StopReplicatedEffectEvent(this, n"homing_thrust");
    } else {
    ...
    this.StopNPCGrenadeBeepSound();
    this.RemoveGrenadeLandedStimuli();
    parabolicTrajectoryParams = ParabolicTrajectoryParams.GetAccelVelParabolicParams(new Vector4(0.00, 0.00, -0.30, 0.00), 0.40, 0.40);
    this.m_projectileComponent.ClearTrajectories();
  ...
  protected final func ActivateSmartTrajectory() -> Void {
    ...
    if NotEquals(this.m_homingGrenadeTarget.targetSlot, n"None") && slotComponent.GetSlotTransform(this.m_homingGrenadeTarget.targetSlot, slotTransform) {
  ...
  protected cb func OnTargetAcquired(evt: ref<GrenadeTrackerTargetAcquiredEvent>) -> Bool {
    ...
    if this.IsGrenadeOfType(EGrenadeType.Cutting) && ScriptedPuppet.IsAlive(evt.target) {
      if ScriptedPuppet.IsBoss(evt.target) || ScriptedPuppet.IsMaxTac(evt.target) {
        ...
        this.SpawnAttack(attack, 2.00, evt.target.GetWorldPosition());
      } else {
        this.SpawnAttack(this.m_tweakRecord.AdditionalAttack(), 2.00, evt.target.GetWorldPosition());
      };
    } else {
      i = 0;
      while i < ArraySize(this.m_potentialHomingTargets) {
...
public class ConsumablesChargesHelper extends IScriptable {

  public final static func RefreshAllCharges(gameInstance: GameInstance, statPool: gamedataStatPoolType) -> Void {
    GameInstance.GetStatPoolsSystem(gameInstance).RequestSettingStatPoolMaxValue(Cast<StatsObjectID>(GetPlayer(gameInstance).GetEntityID()), statPool, null);
  }

  public final static func RefreshToSpecificAmount(gameInstance: GameInstance, statPool: gamedataStatPoolType, amount: Int32) -> Void {
    ...
    if difference > 0 {
      ConsumablesChargesHelper.RefreshAmountOfCharges(gameInstance, statPool, difference);
    };
  }

  public final static func RefreshAmountOfCharges(gameInstance: GameInstance, statPool: gamedataStatPoolType, amount: Int32) -> Void {
    let chargeToRestore: Int32;
    switch statPool {
      case gamedataStatPoolType.HealingItemsCharges:
    ...
    GameInstance.GetStatPoolsSystem(gameInstance).RequestChangingStatPoolValue(Cast<StatsObjectID>(GetPlayer(gameInstance).GetEntityID()), statPool, Cast<Float>(chargeToRestore), null, false, false);
  }

  public final static func HotkeyRefresh(gameInstance: GameInstance) -> Void {
    let hotkeyRefresh: ref<HotkeyRefreshRequest>;
    let eqs: ref<EquipmentSystem> = GameInstance.GetScriptableSystemsContainer(gameInstance).Get(n"EquipmentSystem") as EquipmentSystem;
    if IsDefined(eqs) {
  ...
  public final static func LeaveTheBestQualityConsumable(gameInstance: GameInstance, tag: CName) -> Bool {
    let bestGrenadeQuality: Int32;
    let currentGrenadeQuality: Int32;
    let i: Int32;
    let itemid: ItemID;
    let playerItems: array<wref<gameItemData>>;
    let itemRemoved: Bool = false;
  ...
  public final static func HideConsumableRecipe(gameInstance: GameInstance, recipeToHide: TweakDBID) -> Void {
    let craftingSystem: ref<CraftingSystem> = CraftingSystem.GetInstance(gameInstance);
    let playerCraftBook: ref<CraftBook> = craftingSystem.GetPlayerCraftBook();
    playerCraftBook.HideRecipe(recipeToHide, true);
  }

  public final static func GetConsumableTag(tags: [CName]) -> CName {
    let tag: CName = n"None";
    if ArrayContains(tags, n"FragGrenade") {
      tag = n"FragGrenade";
    } else {
      if ArrayContains(tags, n"FlashGrenade") {
        ...
        if ArrayContains(tags, n"SmokeGrenade") {
          ...
          if ArrayContains(tags, n"EMPGrenade") {
            ...
            if ArrayContains(tags, n"BiohazardGrenade") {
              tag = n"BiohazardGrenade";
            } else {
              if ArrayContains(tags, n"IncendiaryGrenade") {
                tag = n"IncendiaryGrenade";
              } else {
                if ArrayContains(tags, n"ReconGrenade") {
                  tag = n"ReconGrenade";
                } else {
                  if ArrayContains(tags, n"CuttingGrenade") {
                    tag = n"CuttingGrenade";
                  } else {
...
public class GrenadeCollisionEvaluator extends gameprojectileScriptCollisionEvaluator {
