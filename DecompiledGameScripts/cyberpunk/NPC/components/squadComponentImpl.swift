...
public class SquadMemberBaseComponent extends SquadMemberComponent {
  ...
  public final func PerformSquadVerb(squadActionName: CName, squadVerb: EAISquadVerb) -> Void {
    ...
    switch squadVerb {
      case EAISquadVerb.OpenSquadAction:
        ticketTimeout = -1.00;
        if this.GetTicketType(squadActionName, this.GetEntity(), ticketRecord) {
  ...
  private final func GetTicketType(actionName: CName, entity: ref<Entity>, out ticketRecord: ref<AITicket_Record>) -> Bool {
...
public class PuppetSquadInterface extends CombatSquadScriptInterface {
  ...
  private let m_ticketHistory: [SquadTicketReceipt];
  ...
  private final func GetAcknowledgeDelay(entity: wref<Entity>, ticketRecord: wref<AITicket_Record>, ticketHistoryID: Int32, out acknowledgeDelay: Float) -> Bool {
    ...
    if ticketRecord.MinTicketDesyncTime() > 0.00 {
      ...
      acknowledgeDelay *= Cast<Float>(ticketHistory.acknowledgesInQueue) + 1.00;
      return false;
    };
    return true;
  }
  ...
  private final func AllocateTacticsSectors() -> Void {
    ...
    while i < ticketEnumSize {
      AIScriptSquad.GetTicketRecord(EnumValueToName(n"gamedataAITicketType", Cast<Int64>(i)), this.m_baseSquadRecord, ticketRecord);
      ...
      if IsDefined(tacticRecord) {
        ...
        if ArraySize(sectorsRecord) == 0 {
          ...
          while j < ArraySize(sectorsRecord) {
            sectorEnumValue = Cast<Int32>(EnumValueFromName(n"AICombatSectorType", sectorsRecord[j].EnumName()));
            ArrayPush(sectors, IntEnum<AICombatSectorType>(sectorEnumValue));
            j += 1;
          };
          this.RegisterTactic(EnumValueToName(n"gamedataAITicketType", Cast<Int64>(i)), sectors, alley, tacticRecord.TacticTimeout());
        };
      };
      i += 1;
    };
