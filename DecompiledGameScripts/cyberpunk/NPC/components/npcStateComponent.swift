...
public class NPCStatesComponent extends AINetStateComponent {
  ...
  private let m_keepRecentThreatAfterRelaxedDuration: Float;
  ...
  public final func OnGameAttach() -> Void {
    ...
    this.m_keepRecentThreatAfterRelaxedDuration = TweakDBInterface.GetFloat(t"AIGeneralSettings.keepRecentThreatAfterRelaxedDuration", 0.00);
  ...
  public final func GetCurrentHighLevelState() -> gamedataNPCHighLevelState {
    return IntEnum<gamedataNPCHighLevelState>(this.GetReplicatedHighLevelState());
  }
  ...
  public final func GetCurrentUpperBodyState() -> gamedataNPCUpperBodyState {
    return IntEnum<gamedataNPCUpperBodyState>(this.GetReplicatedUpperBodyState());
  }
  ...
  public final func GetCurrentDefenseMode() -> gamedataDefenseMode {
    return IntEnum<gamedataDefenseMode>(this.GetReplicatedDefenseMode());
  }
  ...
  public final func GetCurrentLocomotionMode() -> gamedataLocomotionMode {
    return IntEnum<gamedataLocomotionMode>(this.GetReplicatedLocomotionMode());
  }
  ...
  public final func GetCurrentStanceState() -> gamedataNPCStanceState {
    return IntEnum<gamedataNPCStanceState>(this.GetReplicatedStanceState());
  }
  ...
  public final func GetCurrentHitReactionModeState() -> EHitReactionMode {
    return IntEnum<EHitReactionMode>(this.GetReplicatedHitReactionModeState());
  }
  ...
  public final func GetCurrentBehaviorState() -> gamedataNPCBehaviorState {
    return IntEnum<gamedataNPCBehaviorState>(this.GetReplicatedBehaviorState());
  }
  ...
  public final func GetCurrentPhaseState() -> ENPCPhaseState {
    return IntEnum<ENPCPhaseState>(this.GetReplicatedPhaseState());
  }
  ...
  public final static func AlertPuppet(ownerPuppet: wref<ScriptedPuppet>) -> Bool {
    ...
    if Equals(currentHLS, gamedataNPCHighLevelState.Relaxed) {
      ...
      if !ownerPuppet.IsBoss() && NotEquals(ownerPuppet.GetNPCRarity(), gamedataNPCRarity.MaxTac) {
        return true;
      };
    };
  ...
  private final func MaxtacSetQuestCammo(newState: gamedataNPCHighLevelState) -> Void {
    let puppet: ref<ScriptedPuppet> = this.GetPuppet();
    ...
    if !VehicleComponent.IsMountedToVehicle(gameInstance, puppet) {
      if Equals(newState, gamedataNPCHighLevelState.Combat) || Equals(newState, gamedataNPCHighLevelState.Alerted) {
        ...
        if Equals(newState, gamedataNPCHighLevelState.Relaxed) {
          GameObjectEffectHelper.StartEffectEvent(puppet, n"camo_intro_vfx");
  ...
  private final func UpdateHighLevelState(newState: gamedataNPCHighLevelState, previousState: gamedataNPCHighLevelState) -> Void {
    ...
    let puppet: ref<ScriptedPuppet> = this.GetPuppet();
    let aiRole: ref<AIRole> = puppet.GetAIControllerComponent().GetAIRole();
    this.MaxtacSetQuestCammo(newState);
    ...
    this.GetPuppetStateBlackboard().FireCallbacks();
  ...
  private final func OnRelaxed() -> Void {
    ...
    if TargetTrackingExtension.Get(this.GetPuppet(), targetTracker) {
      if this.GetPuppet().IsPrevention() {
        targetTracker.ResetRecentlyDroppedThreat(1.00);
      } else {
        targetTracker.ResetRecentlyDroppedThreat(this.m_keepRecentThreatAfterRelaxedDuration);
      };
    };
  }

  private final func OnFear() -> Void {
    (this.GetOwner() as NPCPuppet).GetInfluenceComponent().SetReservationRadius(0.20);
  }
  ...
  private final func OnAlerted() -> Void {
    ...
    this.GetPuppet().RefreshCachedReactionPresetData();
    this.GetPuppet().TryRegisterToPrevention();
  }

  private final static func GetCombatTarget(npcPuppet: ref<NPCPuppet>) -> wref<GameObject> {
    let combatTarget: wref<GameObject> = FromVariant<wref<GameObject>>(npcPuppet.GetAIControllerComponent().GetBehaviorArgument(n"CombatTarget"));
    return combatTarget;
  }

  private final func OnCombat() -> Void {
    let aiHoldPositionCommand: ref<AIHoldPositionCommand>;
    let broadcaster: ref<StimBroadcasterComponent>;
    let cooldownIsActive: Bool;
    let delayID: DelayID;
    let evt: ref<DodgeToAvoidCombatEvent>;
    let failureCooldownIsActive: Bool;
    let game: GameInstance;
    let player: ref<PlayerPuppet>;
    let presetName: String;
    let targetTracker: wref<TargetTrackingExtension>;
    let npcPuppet: ref<NPCPuppet> = this.GetPuppet() as NPCPuppet;
    npcPuppet.GetInfluenceComponent().SetReservationRadius(1.00);
    ...
    GameObject.StartCooldown(npcPuppet, n"chatter_flank_order", 7.00);
    if GameInstance.GetStatsSystem(game).GetStatValue(Cast<StatsObjectID>(npcPuppet.GetEntityID()), gamedataStatType.IsHeavyRangedArchetype) > 0.00 {
      GameObject.PlayVoiceOver(npcPuppet, n"hmg_charge", n"Scripts:OnCombat");
    };
    if this.GetPuppet().IsCrowd() {
      GameObject.PlayVoiceOver(npcPuppet, n"crowd_combat", n"Scripts:OnCombat");
    } else {
      GameObject.PlayVoiceOver(npcPuppet, n"start_combat", n"Scripts:OnCombat", ScriptedPuppet.IsPlayerCompanion(npcPuppet) ? 1.50 : 0.00);
    };
    delayID = AIActionHelper.QueueNearbyCombatNotification(npcPuppet);
    if IsDefined(NPCStatesComponent.GetCombatTarget(npcPuppet)) {
      game = npcPuppet.GetGame();
      player = GetPlayer(game);
      cooldownIsActive = StatusEffectHelper.HasStatusEffectWithTagConst(player, n"DodgeOutOfSightCooldown");
      failureCooldownIsActive = StatusEffectHelper.HasStatusEffectWithTagConst(player, n"SecondChancePerkCooldown");
      if player.CanAvoidCombatWithDodge() && !npcPuppet.PuppetIsNotVisible() && !cooldownIsActive && !failureCooldownIsActive {
        ...
        GameInstance.GetDelaySystem(game).DelayEvent(player, evt, 1.00, false);
        GameInstance.GetDelaySystem(game).DelayEvent(player, evt, 2.00, false);
        GameInstance.GetDelaySystem(game).DelayEvent(player, evt, 3.00, false);
      };
      ...
      if IsDefined(broadcaster) {
        broadcaster.TriggerSingleBroadcast(npcPuppet, gamedataStimType.SilentAlarm, 10.00);
      };
    };
    ...
    AICoverHelper.GetCoverBlackboard(npcPuppet).SetFloat(GetAllBlackboardDefs().AICover.startCoverEvaluationTimeStamp, -1.00);
    npcPuppet.GetBumpComponent().Toggle(false);
    npcPuppet.ReevaluatEAIThreatCalculationType();
    if PreventionSystem.CheckNPCSpawnedType(this.GetPuppet().GetGame(), this.GetPuppet().GetEntityID(), DynamicVehicleType.RoadBlockade) {
      aiHoldPositionCommand = new AIHoldPositionCommand();
      aiHoldPositionCommand.duration = 20.00;
      AIComponent.SendCommand(this.GetPuppet(), aiHoldPositionCommand);
    };
    this.GetPuppet().RefreshCachedReactionPresetData();
    this.GetPuppet().TryRegisterToPrevention();
  }

  protected cb func OnNotifyNearbyAboutCombatEvent(evt: ref<NotifyNearbyAboutCombatEvent>) -> Bool {
    let broadcaster: ref<StimBroadcasterComponent>;
    let npcObject: ref<GameObject>;
    let targetTracker: wref<TargetTrackingExtension>;
    let npcPuppet: ref<NPCPuppet> = this.GetPuppet() as NPCPuppet;
    if !npcPuppet.IsActive() {
      return true;
    };
    npcObject = this.GetEntity() as GameObject;
    if IsDefined(NPCStatesComponent.GetCombatTarget(npcPuppet)) {
      if !StatusEffectSystem.ObjectHasStatusEffectWithTag(npcObject, n"CommsNoiseJam") {
      ...
      broadcaster = npcPuppet.GetStimBroadcasterComponent();
      if IsDefined(broadcaster) {
        broadcaster.TriggerSingleBroadcast(this.GetOwner(), gamedataStimType.Alarm, 10.00);
      };
    };
    if TargetTrackingExtension.Get(npcPuppet, targetTracker) && !StatusEffectSystem.ObjectHasStatusEffectWithTag(npcObject, n"ResetSquadSync") {
  ...
  private final func PlayDeadVO() -> Void {
    let broadcaster: ref<StimBroadcasterComponent>;
    if NotEquals(this.m_previousHighLevelState, gamedataNPCHighLevelState.Combat) || StatusEffectSystem.ObjectHasStatusEffectOfType(this.GetEntity() as GameObject, gamedataStatusEffectType.MuteAudioStims) {
  ...
  private final func UpdateDefensiveState(enable: Bool) -> Void {
    let puppetID: StatsObjectID = Cast<StatsObjectID>(this.GetPuppet().GetEntityID());
    let stats: ref<StatsSystem> = GameInstance.GetStatsSystem(this.GetPuppet().GetGame());
    if enable {
      if stats.GetStatValue(puppetID, gamedataStatType.IsBlocking) == 0.00 {
        ...
        stats.AddModifier(puppetID, this.m_statFlagDefensiveState);
      };
    } else {
      if stats.GetStatValue(puppetID, gamedataStatType.IsBlocking) == 1.00 {
        stats.RemoveModifier(puppetID, this.m_statFlagDefensiveState);
      };
    };
  }
  ...
  private final func TurnOnParryState() -> Void {
    ...
    let puppetID: StatsObjectID = Cast<StatsObjectID>(this.GetPuppet().GetEntityID());
    let stats: ref<StatsSystem> = GameInstance.GetStatsSystem(this.GetPuppet().GetGame());
    if stats.GetStatValue(puppetID, gamedataStatType.IsDeflecting) == 0.00 {
  ...
  private final func TurnOffParryState() -> Void {
    ...
    let puppetID: StatsObjectID = Cast<StatsObjectID>(this.GetPuppet().GetEntityID());
    let stats: ref<StatsSystem> = GameInstance.GetStatsSystem(this.GetPuppet().GetGame());
    if stats.GetStatValue(puppetID, gamedataStatType.IsDeflecting) == 1.00 {
      statFlag = RPGManager.CreateStatModifier(gamedataStatType.IsDeflecting, gameStatModifierType.Additive, -1.00);
      stats.AddModifier(puppetID, statFlag);
    };
  }
  ...
  private final func UpdateHitReactionsExceptionState() -> Void {
    ...
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_bulkyStaggerMinRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_staggerMinRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_unstoppableRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_unstoppableTwitchMinRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_unstoppableTwitchNoneRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_forceImpactRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_forceStaggerRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_forceKnockdownRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_fragileRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_weakRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_toughRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_bulkyRecordID));
    statSystem.RemoveModifierGroup(Cast<StatsObjectID>(entityID), TDBID.ToNumber(this.m_regularRecordID));
  ...
  private final func OnRegular(statSystem: ref<StatsSystem>, entityID: EntityID, recordID: TweakDBID) -> Void {
    ...
    statSystem.ApplyModifierGroup(Cast<StatsObjectID>(entityID), tdbId);
