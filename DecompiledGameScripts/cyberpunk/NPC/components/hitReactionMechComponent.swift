...
public class HitReactionMechComponent extends HitReactionComponent {
  ...
  public func OnGameAttached() -> Void {
    if this.MechIsDeadOnInit() {
      this.SetHitReactionType(animHitReactionType.None);
      this.SendMechDataToAIBehavior(animHitReactionType.Death);
    };
  }

  private final func MechIsDeadOnInit() -> Bool {
    let puppet: ref<NPCPuppet> = this.GetOwner() as NPCPuppet;
    if puppet.IsDead() {
      if StatusEffectSystem.ObjectHasStatusEffect(this.GetOwner(), t"WorkspotStatus.Death") {
        return false;
      };
      return true;
    };
  ...
  public func EvaluateHit(newHitEvent: ref<gameHitEvent>) -> Void {
    ...
    if !IsDefined(newHitEvent) {
      if !this.m_isAlive {
        if npc.ShouldSkipDeathAnimation() {
          this.m_animHitReaction.hitType = 0;
        } else {
    ...
    if this.m_animHitReaction.hitType == 1 {
