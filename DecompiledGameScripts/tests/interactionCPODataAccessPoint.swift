...
public class CPOMissionDevice extends GameObject {
  ...
  protected func RegisterFactsListener() -> Void {
    if NotEquals(this.m_factToUnblock, n"None") {
  ...
  protected final func SetFacts(const facts: script_ref<[SFactToChange]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(facts)) {
      this.SetFact(Deref(facts)[i].factName, Deref(facts)[i].factValue, Deref(facts)[i].operationType);
      i += 1;
    };
  }
...
public class CPOMissionDataAccessPoint extends CPOMissionDevice {
  ...
  protected const let m_factsOnDownload: [SFactToChange];

  protected const let m_factsOnUpload: [SFactToChange];
  ...
  protected final func IsDamagePresetValid(presetName: CName) -> Bool {
    ...
    if NotEquals(presetName, n"None") {
...
public native class MultiplayerGiveChoiceTokenEvent extends Event {
  ...
  public final func GiveChoiceToken(player: ref<PlayerPuppet>) -> Void {
    ...
    if IsDefined(playerToGiveData) {
      ...
      transferDataEvent.dataDamagesPresetName = n"None";
  ...
  public final static func CreateDelayedEvent(player: ref<GameObject>, compatibleDeviceName: CName, timeout: Uint32) -> DelayID {
    ...
    return GameInstance.GetDelaySystem(player.GetGame()).DelayEvent(player, evt, Cast<Float>(timeout));
  }
}
