...
public class ApplyShaderOnObjectEffector extends Effector {
  ...
  private let m_applicationTargetName: CName;

  private let m_applicationTarget: wref<GameObject>;

  private let m_effects: [ref<EffectInstance>];
  ...
  protected func Initialize(record: TweakDBID, game: GameInstance, parentRecord: TweakDBID) -> Void {
    this.m_applicationTargetName = TweakDBInterface.GetCName(record + t".applicationTarget", n"None");
  ...
  protected func ActionOn(owner: ref<GameObject>) -> Void {
    this.m_owner = owner;
    this.GetApplicationTarget(this.m_owner, this.m_applicationTargetName, this.m_applicationTarget);
