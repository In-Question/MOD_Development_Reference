...
public class SetTargetHealthEffector extends Effector {
  ...
  private final func Set(target: ref<NPCPuppet>) -> Void {
    ...
    GameInstance.GetStatPoolsSystem(target.GetGame()).RequestChangingStatPoolValue(Cast<StatsObjectID>(target.GetEntityID()), gamedataStatPoolType.Health, this.m_healthValueToSet, null, true, true);
  }
}
