...
public class TriggerHackingMinigameEffector extends Effector {
  ...
  protected func Initialize(record: TweakDBID, game: GameInstance, parentRecord: TweakDBID) -> Void {
    ...
    this.m_fact = TweakDBInterface.GetCName(record + t".factName", n"None");
  ...
  protected cb func OnItemCracked(value: Int32) -> Bool {
    ...
    if NotEquals(HackingMinigameState.InProgress, IntEnum<HackingMinigameState>(value)) {
      if Equals(HackingMinigameState.Succeeded, IntEnum<HackingMinigameState>(value)) {
        ...
        if Equals(HackingMinigameState.Failed, IntEnum<HackingMinigameState>(value)) {
