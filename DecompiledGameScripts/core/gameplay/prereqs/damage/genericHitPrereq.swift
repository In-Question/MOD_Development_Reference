...
public class GenericHitPrereqState extends PrereqState {
  ...
  public let m_missEvent: ref<gameMissEvent>;
  ...
  public final func SetMissEvent(missEvent: ref<gameMissEvent>) -> Void {
    this.m_missEvent = missEvent;
  }

  public final const func GetMissEvent() -> ref<gameMissEvent> {
    return this.m_missEvent;
  }
  ...
  protected final func GetObjectToCheck(const obj: script_ref<String>, hitEvent: ref<gameHitEvent>) -> wref<GameObject> {
    switch Deref(obj) {
      case "Instigator":
...
public class GenericHitPrereq extends IScriptablePrereq {
  ...
  public let m_pipelineType: DMGPipelineType;

  public let m_attackType: gamedataAttackType;

  public let m_conditions: [ref<BaseHitPrereqCondition>];

  public let m_ignoreSelfInflictedPressureWave: Bool;
  ...
  protected const func OnRegister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    if this.m_isSync {
      GameInstance.GetDamageSystem(game).RegisterSyncListener(castedState.m_listener, owner.GetEntityID(), this.m_callbackType, this.m_pipelineStage, this.m_pipelineType);
      if this.m_processMiss {
        GameInstance.GetDamageSystem(game).RegisterSyncListener(castedState.m_listener, owner.GetEntityID(), gameDamageCallbackType.MissTriggered, this.m_pipelineStage, this.m_pipelineType);
      };
    } else {
      GameInstance.GetDamageSystem(game).RegisterListener(castedState.m_listener, owner.GetEntityID(), this.m_callbackType, this.m_pipelineType);
      if this.m_processMiss {
        GameInstance.GetDamageSystem(game).RegisterListener(castedState.m_listener, owner.GetEntityID(), gameDamageCallbackType.MissTriggered, this.m_pipelineType);
      };
    };
    return false;
  }
  ...
  protected const func OnUnregister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Void {
    ...
    if this.m_isSync {
      ...
      if this.m_processMiss {
        GameInstance.GetDamageSystem(game).UnregisterSyncListener(castedState.m_listener, owner.GetEntityID(), gameDamageCallbackType.MissTriggered, this.m_pipelineStage);
      };
    } else {
  ...
  protected func Initialize(recordID: TweakDBID) -> Void {
    ...
    this.m_processMiss = TweakDBInterface.GetBool(recordID + t".processMiss", false);
    ...
    this.m_pipelineType = IntEnum<DMGPipelineType>(Cast<Int32>(EnumValueFromString("DMGPipelineType", str)));
    ...
    this.m_ignoreSelfInflictedPressureWave = TweakDBInterface.GetBool(recordID + t".ignoreSelfInflictedPressureWave", true);
  }
...
public class HitCallback extends ScriptedDamageSystemListener {
  ...
  protected func OnMissTriggered(missEvent: ref<gameMissEvent>) -> Void {
    this.m_state.SetMissEvent(missEvent);
    this.m_state.OnMissTriggered(missEvent);
  }
}
