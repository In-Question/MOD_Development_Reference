...
public class StatPoolSpentPrereq extends IScriptablePrereq {
  ...
  protected const func OnRegister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    GameInstance.GetStatPoolsSystem(game).RequestRegisteringListener(Cast<StatsObjectID>(owner.GetEntityID()), this.m_statPoolType, castedState.m_listener);
    return false;
  }
  ...
  protected const func OnUnregister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Void {
    let owner: ref<GameObject> = context as GameObject;
    let castedState: ref<StatPoolSpentPrereqState> = state as StatPoolSpentPrereqState;
    GameInstance.GetStatPoolsSystem(game).RequestUnregisteringListener(Cast<StatsObjectID>(owner.GetEntityID()), this.m_statPoolType, castedState.m_listener);
    castedState.m_listener = null;
  }

  protected func Initialize(recordID: TweakDBID) -> Void {
    this.m_statPoolType = IntEnum<gamedataStatPoolType>(Cast<Int32>(EnumValueFromName(n"gamedataStatPoolType", TweakDBInterface.GetCName(recordID + t".statPoolType", n"None"))));
