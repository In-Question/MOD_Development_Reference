...
public class ItemInSlotPrereq extends IScriptablePrereq {
  ...
  public let m_waitForVisuals: Bool;
  ...
  protected const func OnRegister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    castedState.m_listener.m_waitForVisuals = this.m_waitForVisuals;
  ...
  protected func Initialize(recordID: TweakDBID) -> Void {
    ...
    this.m_slotID = TweakDBInterface.GetAttachmentSlotRecord(TweakDBInterface.GetForeignKeyDefault(recordID + t".attachmentSlot")).GetID();
    this.m_itemType = TweakDBInterface.GetItemTypeRecord(TweakDBInterface.GetForeignKeyDefault(recordID + t".itemType")).Type();
    this.m_itemCategory = TweakDBInterface.GetItemCategoryRecord(TweakDBInterface.GetForeignKeyDefault(recordID + t".itemCategory")).Type();
    this.m_weaponEvolution = TweakDBInterface.GetWeaponEvolutionRecord(TweakDBInterface.GetForeignKeyDefault(recordID + t".weaponEvolution")).Type();
    ...
    this.m_waitForVisuals = TweakDBInterface.GetBool(recordID + t".waitForVisuals", false);
  }
  ...
  public final const func CheckGenericWeaponModSlots(itemID: ItemID, owner: wref<GameObject>) -> Bool {
    ...
    while index < ArraySize(attachmentSlotList) && index < 4 {
      if this.IsGenericWeaponMod(attachmentSlotList[index], Cast<Int32>(quality)) {
        return false;
      };
      index += 1;
    };
