...
public class StanceNPCStatePrereq extends NPCStatePrereq {
  ...
  protected func Initialize(recordID: TweakDBID) -> Void {
    ...
    this.m_valueToListen = IntEnum<gamedataNPCStanceState>(Cast<Int32>(EnumValueFromString("gamedataNPCStanceState", this.GetStateName(recordID))));
  }
...
public class CurrentStanceNPCStatePrereq extends IScriptablePrereq {
  ...
  protected func Initialize(record: TweakDBID) -> Void {
    ...
    this.m_valueToCheck = IntEnum<gamedataNPCStanceState>(Cast<Int32>(EnumValueFromString("gamedataNPCStanceState", stateName)));
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    currentPuppetState = IntEnum<gamedataNPCStanceState>(bb.GetInt(GetAllBlackboardDefs().PuppetState.Stance));
