...
public class GameplaySkillCondition extends GameplayConditionBase {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;BaseStats.SkillStat")
  protected persistent let m_skillToCheck: TweakDBID;

  private persistent let m_difficulty: EGameplayChallengeLevel;

  private persistent let m_skillBonus: TweakDBID;

  private persistent let m_requiredLevel: Int32;
  ...
  public final func TrySetRequiredLevel(requiredLevel: Int32) -> Void {
    if !this.IsRequiredLevelSetup() {
      this.m_requiredLevel = requiredLevel;
    };
  }

  private final func IsRequiredLevelSetup() -> Bool {
    return this.m_requiredLevel >= 0;
  }
  ...
  public final func GetPlayerSkill(requester: ref<GameObject>) -> Int32 {
    ...
    let playerEntity: EntityID = GameInstance.GetPlayerSystem(requester.GetGame()).GetLocalPlayerControlledGameObject().GetEntityID();
    let skillValue: Int32 = Cast<Int32>(statsSystem.GetStatValue(Cast<StatsObjectID>(playerEntity), this.GetStatType()));
    skillValue += Cast<Int32>(statsSystem.GetStatValue(Cast<StatsObjectID>(playerEntity), this.GetStatBonusType()));
    return skillValue;
  }
  ...
  private final func GetStatBonusType() -> gamedataStatType {
    return TweakDBInterface.GetStatRecord(this.m_skillBonus).StatType();
  }
  ...
  public final func SetProperties(sel_skill: EDeviceChallengeSkill, sel_difficulty: EGameplayChallengeLevel) -> Void {
    ...
    this.m_skillBonus = TDBID.Create("BaseStats." + skillString + "SkillcheckBonus");
    this.m_requiredLevel = -1;
  }
...
public class GameplayPerkCondition extends GameplayConditionBase {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;Perk")
...
public class GameplayStatCondition extends GameplayConditionBase {
  ...
  public final func GetPlayerStat(requester: ref<GameObject>) -> Int32 {
    let statsSystem: ref<StatsSystem> = GameInstance.GetStatsSystem(requester.GetGame());
    return Cast<Int32>(statsSystem.GetStatValue(Cast<StatsObjectID>(this.GetPlayer(requester).GetEntityID()), this.GetStatType()));
  }
