...
public class GameplayConditionContainer extends IScriptable {
  ...
  protected inline const let m_conditionGroups: [ConditionGroupData];
  ...
  private final func Evaluate(requester: ref<GameObject>, const group: script_ref<ConditionGroupData>) -> Bool {
    let i: Int32;
    let passed: Bool;
    if Equals(Deref(group).logicOperator, ELogicOperator.AND) {
      passed = true;
    };
    i = 0;
    while i < ArraySize(Deref(group).conditions) {
      if Deref(group).conditions[i].Evaluate(requester) {
        if Equals(Deref(group).logicOperator, ELogicOperator.OR) {
          passed = true;
          break;
        };
      } else {
  ...
  public final func CreateDescription(obj: ref<GameObject>, entID: EntityID) -> [ConditionData] {
