...
public class SkillCheckPrereq extends DevelopmentCheckPrereq {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    return Cast<Float>(skillLevel) >= this.requiredLevel;
  }
  ...
  protected func Initialize(recordID: TweakDBID) -> Void {
    ...
    this.skillToCheck = IntEnum<gamedataProficiencyType>(Cast<Int32>(EnumValueFromName(n"gamedataProficiencyType", type)));
  }
}
...
public class StatCheckPrereq extends DevelopmentCheckPrereq {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    let statValue: Float = GameInstance.GetStatsSystem(game).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), this.statToCheck);
  ...
  protected func Initialize(recordID: TweakDBID) -> Void {
    ...
    type = TDB.GetCName(tweakID);
    this.statToCheck = IntEnum<gamedataStatType>(Cast<Int32>(EnumValueFromName(n"gamedataStatType", type)));
  }
}
...
public class NPCDeadPrereq extends IScriptablePrereq {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    if IsDefined(context as NPCPuppet) {
      npcOwner = context as NPCPuppet;
      npcOwnerID = Cast<StatsObjectID>(npcOwner.GetEntityID());
...
public class PlayerNotGrapplingPrereq extends IScriptablePrereq {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    if IsDefined(playerControlledObject) {
      blackboard = GameInstance.GetBlackboardSystem(game).GetLocalInstanced(playerControlledObject.GetEntityID(), GetAllBlackboardDefs().PlayerStateMachine);
      playerGrappling = blackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Takedown) == 1 || blackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Takedown) == 2 || blackboard.GetInt(GetAllBlackboardDefs().PlayerStateMachine.Takedown) == 4;
    } else {
...
public class IsMountedByPreventionNPCPrereqState extends PrereqState {

  public let mountingListener: ref<gameScriptedPrereqMountingListenerWrapper>;

  protected final func OnMountingStateChanged() -> Void {
    let prereq: ref<IsMountedByPreventionNPCPrereq> = this.GetPrereq() as IsMountedByPreventionNPCPrereq;
    this.OnChanged(prereq.IsFulfilled(GetGameInstance(), this.GetContext()));
  }
}

public class IsMountedByPreventionNPCPrereq extends IScriptablePrereq {
  ...
  protected const func OnRegister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Bool {
    let castedState: ref<IsMountedByPreventionNPCPrereqState> = state as IsMountedByPreventionNPCPrereqState;
    let vehicle: ref<VehicleObject> = context as VehicleObject;
    if IsDefined(vehicle) {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    let player: ref<PlayerPuppet> = GameInstance.GetPlayerSystem(game).GetLocalPlayerMainGameObject() as PlayerPuppet;
    let vehicle: ref<VehicleObject> = context as VehicleObject;
    if IsDefined(player) && IsDefined(vehicle) && vehicle.IsPrevention() {
      return !this.isCheckInverted;
    };
    return this.isCheckInverted;
  }
...
public class SelectedForMultiplayerChoiceDialog extends IScriptablePrereq {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    return GameInstance.GetQuestsSystem(game).GetFactStr(factName) == Cast<Int32>(entity.GetControllingPeerID());
  }
}
...
public class AccessPointCompatibleWithUser extends InteractionScriptedCondition {
  ...
  public final const func Test(const activatorObject: wref<GameObject>, const hotSpotObject: wref<GameObject>, const hotSpotLayer: wref<HotSpotLayerDefinition>) -> Bool {
    ...
    if IsDefined(device) && IsDefined(playerPuppet) {
      if NotEquals(device.GetCompatibleDeviceName(), n"None") || NotEquals(playerPuppet.GetCompatibleCPOMissionDeviceName(), n"None") {
...
public class CPOMissionPlayerVoted extends InteractionScriptedCondition {
  ...
  public const func Test(const activatorObject: wref<GameObject>, const hotSpotObject: wref<GameObject>, const hotSpotLayer: wref<HotSpotLayerDefinition>) -> Bool {
    ...
    let playerPuppet: ref<PlayerPuppet> = activatorObject as PlayerPuppet;
    if IsDefined(device) && IsDefined(playerPuppet) {
      if NotEquals(device.GetCompatibleDeviceName(), n"None") {
...
public class PuppetMortalityListener extends ScriptStatsListener {

  public let state: ref<PuppetMortalPrereqState>;

  public func OnGodModeChanged(ownerID: EntityID, newType: gameGodModeType) -> Void {
    this.state.ProcessGodModeChanged(newType);
  }
}

public class PuppetMortalPrereqState extends PrereqState {

  public let m_owner: wref<GameObject>;

  public let m_listener: wref<PuppetMortalityListener>;

  public final func ProcessGodModeChanged(newType: gameGodModeType) -> Void {
    let prereq: ref<PuppetMortalPrereq> = this.GetPrereq() as PuppetMortalPrereq;
    let checkPassed: Bool = prereq.Evaluate(this.m_owner, newType);
    this.OnChanged(checkPassed);
  }
}
...
public class PuppetMortalPrereq extends IScriptablePrereq {
  ...
  protected const func OnRegister(state: ref<PrereqState>, game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    let castedState: ref<PuppetMortalPrereqState> = state as PuppetMortalPrereqState;
    castedState.m_owner = owner;
    let listener: ref<PuppetMortalityListener> = new PuppetMortalityListener();
    listener.state = castedState;
    castedState.m_listener = listener;
    GameInstance.GetStatsSystem(owner.GetGame()).RegisterListener(Cast<StatsObjectID>(owner.GetEntityID()), castedState.m_listener);
    return false;
  }
  ...
  public final const func Evaluate(owner: ref<GameObject>, godModeType: gameGodModeType) -> Bool {
    if Equals(godModeType, gameGodModeType.Immortal) || Equals(godModeType, gameGodModeType.Invulnerable) {
  ...
  public const func IsFulfilled(game: GameInstance, context: ref<IScriptable>) -> Bool {
    ...
    if !isGodMode {
      type = IntEnum<gameGodModeType>(-1);
    };
    return this.Evaluate(puppet, type);
  }
}
