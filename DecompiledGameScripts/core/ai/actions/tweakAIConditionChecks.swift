...
public abstract class AICondition extends IScriptable {
  ...
  public final static func ActivationCheck(const context: ScriptExecutionContext, actionRecord: ref<AIAction_Record>) -> Bool {
  ...
  public final static func NextPhaseCheck(const context: ScriptExecutionContext, phase: Uint32, actionRecord: ref<AIAction_Record>) -> Bool {
    if !IsDefined(actionRecord) {
      return false;
    };
  ...
  public final static func RepeatPhaseCheck(const context: ScriptExecutionContext, phase: Uint32, actionRecord: ref<AIAction_Record>) -> Bool {
    if !IsDefined(actionRecord) {
      return false;
    };
  ...
  public final static func CheckActionConditions(const context: ScriptExecutionContext, conditions: script_ref<[wref<AIActionCondition_Record>]>) -> Bool {
    let i: Int32;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AISlotCond_Record>) -> Bool {
    ...
    if NotEquals(condition.ItemTag(), n"None") && !item.GetItemData().HasTag(condition.ItemTag()) {
      return condition.Invert() ? true : false;
    };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIStatusEffectCond_Record>) -> Bool {
    ...
    if NotEquals(statusEffectGameplayTag, n"None") {
  ...
  private final static func CheckAngle2ObjInternal(owner: wref<gamePuppet>, source: wref<GameObject>, sourcePosition: Vector4, sourceCoverID: Uint64, target: wref<GameObject>, targetPosition: Vector4, targetCoverID: Uint64, coneAngleRange: Vector2, condition: wref<AISpatialCond_Record>, out returnVal: Bool) -> Bool {
    ...
    if coneAngleRange.X > 0.00 || coneAngleRange.Y > 0.00 || condition.AngleDirection() != 0 {
      ...
      if angleToTarget * Cast<Float>(condition.AngleDirection()) < 0.00 {
        returnVal = condition.Invert() ? true : false;
        return true;
      };
    };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIMovementCond_Record>) -> Bool {
    ...
    if NotEquals(condition.OffMeshLinkType(), n"None") {
      offMeshExplorationType = movePoliciesComponent.GetExplorationOffMeshLinkType();
      if NotEquals(condition.OffMeshLinkType(), EnumValueToName(n"moveExplorationType", Cast<Int64>(EnumInt(offMeshExplorationType)))) {
        movePoliciesComponent.GetOffMeshLinkType(offMeshConnectionType);
        if NotEquals(condition.OffMeshLinkType(), EnumValueToName(n"worldOffMeshConnectionType", Cast<Int64>(EnumInt(offMeshConnectionType)))) {
          return condition.Invert() ? true : false;
        };
      };
    };
    if NotEquals(condition.MovementType(), n"None") {
      movementTypeCond = IntEnum<moveMovementType>(Cast<Int32>(EnumValueFromName(n"moveMovementType", condition.MovementType())));
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIStatPoolCond_Record>) -> Bool {
    ...
    if percRange.X >= 0.00 || percRange.Y >= 0.00 {
      statPercValue = GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetStatPoolValue(Cast<StatsObjectID>(obj.GetEntityID()), condition.StatPool().StatPoolType(), true);
    ...
    if isIncreasing >= 0 {
      if isIncreasing == 1 {
        GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetModifier(Cast<StatsObjectID>(obj.GetEntityID()), condition.StatPool().StatPoolType(), gameStatPoolModificationTypes.Regeneration, statPoolMod);
      } else {
        if isIncreasing == 0 {
          GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetModifier(Cast<StatsObjectID>(obj.GetEntityID()), condition.StatPool().StatPoolType(), gameStatPoolModificationTypes.Decay, statPoolMod);
        };
      };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIAmmoCountCond_Record>) -> Bool {
    ...
    if condition.Min() >= 0 && ammoCount < Cast<Uint32>(condition.Min()) {
      return condition.Invert() ? true : false;
    };
    if condition.Max() >= 0 && ammoCount > Cast<Uint32>(condition.Max()) {
      return condition.Invert() ? true : false;
    };
    percRange = condition.Percentage();
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AICalculateLineOfSightVector_Record>) -> Bool {
    ...
    startOffset = Cast<Vector4>(condition.StartPositionOffset());
    endOffset = Cast<Vector4>(condition.EndPositionOffset());
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIReactionCond_Record>) -> Bool {
    ...
    if NotEquals(condition.ReactionBehaviorName(), n"None") {
      behavior = EnumValueToString("gamedataOutput", Cast<Int64>(EnumInt(activeReactionData.reactionBehaviorName)));
    ...
    if IsDefined(condition.ThresholdValue()) {
      switch condition.ThresholdValue().StatPoolType() {
        case gamedataStatPoolType.Fear:
          currentStat = GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetStatPoolValue(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), gamedataStatPoolType.Fear, false);
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIPreviousAttackCond_Record>) -> Bool {
    let attacksToCheck: array<CName>;
    let i: Int32;
    let previousAttackDelay: Float;
    let previousAttackName: CName;
    ...
    if ArraySize(attacksToCheck) > 0 {
      ...
      while i < ArraySize(attacksToCheck) {
        if Equals(attacksToCheck[i], n"SubActionInAir") && ownerPuppet.GetPuppetStateBlackboard().GetBool(GetAllBlackboardDefs().PuppetState.InAirAnimation) {
          return condition.Invert() ? false : true;
        };
        if Equals(attacksToCheck[i], previousAttackName) {
          return condition.Invert() ? false : true;
        };
        i += 1;
      };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIFriendlyFireCond_Record>) -> Bool {
    ...
    if IsDefined(target) && target.IsAttached() {
      if ScriptedPuppet.IsBeingGrappled(target) {
        ...
        if GameInstance.GetStatsSystem(ownerPuppet.GetGame()).GetStatValue(Cast<StatsObjectID>(ownerPuppet.GetEntityID()), gamedataStatType.IsAggressive) <= 0.00 && GameInstance.GetStatsSystem(ownerPuppet.GetGame()).GetStatValue(Cast<StatsObjectID>(ownerPuppet.GetEntityID()), gamedataStatType.IsReckless) <= 0.00 {
  ...
  private final static func IsAwareOfBeingFlankedByThreat(ownerPuppet: wref<ScriptedPuppet>, const threat: script_ref<TrackedLocation>) -> Bool {
    let threatAsPuppet: ref<ScriptedPuppet>;
    let trackedLocation: TrackedLocation;
    if Deref(threat).accuracy > 0.00 {
      return true;
    };
    threatAsPuppet = Deref(threat).entity as ScriptedPuppet;
  ...
  private final static func IsAwareOfBeingFlankedByAnyThreat(ownerPuppet: wref<ScriptedPuppet>, const threats: script_ref<[TrackedLocation]>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(Deref(threats)) {
      if AICondition.IsAwareOfBeingFlankedByThreat(ownerPuppet, Deref(threats)[i]) {
        return true;
      };
      i += 1;
    };
    ...
    if condition.IsEnteringOrLeavingCover() >= 0 {
      if condition.IsEnteringOrLeavingCover() == 0 {
        if coverManager.IsEnteringOrLeavingCover(target) {
          AIActionTarget.GetCoverID(context, condition.Cover(), currentCoverID);
          if NotEquals(GameInstance.GetCoverManager(target.GetGame()).GetCoverHeight(currentCoverID), gameCoverHeight.High) {
            result = true;
          };
        };
        if condition.Invert() {
      ...
      if condition.IsEnteringOrLeavingCover() == 2 {
        if Equals(coverManager.GetCoverActionType(target), AIUninterruptibleActionType.EnteringCover) {
          AIActionTarget.GetCoverID(context, condition.Cover(), currentCoverID);
          if NotEquals(GameInstance.GetCoverManager(target.GetGame()).GetCoverHeight(currentCoverID), gameCoverHeight.High) {
            result = true;
          };
      ...
      if Equals(coverManager.GetCoverActionType(target), AIUninterruptibleActionType.LeavingCover) {
        AIActionTarget.GetCoverID(context, condition.Cover(), currentCoverID);
        if NotEquals(GameInstance.GetCoverManager(target.GetGame()).GetCoverHeight(currentCoverID), gameCoverHeight.High) {
          result = true;
        };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIHitCond_Record>) -> Bool {
    ...
    if NotEquals(condition.AttackTag(), n"None") && NotEquals(condition.AttackTag(), attackTag) {
      return condition.Invert() ? true : false;
    };
    if condition.CumulatedDamageThreshold() > 0 {
      cumulatedDamge = targetPuppet.GetHitReactionComponent().GetCumulatedDamage();
      if cumulatedDamge <= Cast<Float>(condition.CumulatedDamageThreshold()) {
        return condition.Invert() ? true : false;
      };
    };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIVehicleCond_Record>) -> Bool {
    ...
    if activePassangersRange.X > 0.00 || activePassangersRange.Y > 0.00 {
      ...
      if activePassangersRange.X > 0.00 && Cast<Float>(activePassangers) < activePassangersRange.X {
        return condition.Invert() ? true : false;
      };
      if activePassangersRange.Y > 0.00 && Cast<Float>(activePassangers) > activePassangersRange.Y {
        return condition.Invert() ? true : false;
      };
    };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AITargetInUnmountingRangeCond_Record>) -> Bool {
    let target: wref<GameObject>;
    let result: Bool = false;
    let nonPlayerDistanceCheck: Float = 15.00;
    AIActionTarget.GetObject(context, condition.Target(), target);
    if target.IsPlayer() {
      if ScriptExecutionContext.GetOwner(context).IsPrevention() {
        result = GameInstance.GetPreventionSpawnSystem(target.GetGame()).IsInUnmountingRange(Cast<Vector3>(ScriptExecutionContext.GetOwner(context).GetWorldPosition()));
      } else {
        result = GameInstance.GetDynamicSpawnSystem(target.GetGame()).IsInUnmountingRange(Cast<Vector3>(ScriptExecutionContext.GetOwner(context).GetWorldPosition()));
      };
    } else {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AITargetStandingOnTopOfMovingVehicleCond_Record>) -> Bool {
    let target: wref<GameObject>;
    let vehicleBelow: wref<VehicleObject>;
    let vehicleBelowID: EntityID;
    let result: Bool = false;
    let minSpeedThreshold: Float = 2.00;
    AIActionTarget.GetObject(context, condition.Target(), target);
    if VehicleComponent.IsMountedToVehicle(ScriptExecutionContext.GetOwner(context).GetGame(), target) {
      return condition.Invert() ? !result : result;
    };
    vehicleBelowID = (target as ScriptedPuppet).TryGetVehicleBelow();
    ...
    if IsDefined(vehicleBelow) && vehicleBelow.GetCurrentSpeed() > minSpeedThreshold {
      result = true;
    };
    return condition.Invert() ? !result : result;
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIMovingInCirclesCond_Record>) -> Bool {
    let blackboard: ref<IBlackboard>;
    let i: Int32;
    let object: wref<GameObject>;
    let positionHistory: array<Vector4>;
    let transformHistoryComponent: ref<TransformHistoryComponent>;
    let vehicleObject: wref<VehicleObject>;
    let result: Bool = false;
    let currentTime: Float = 0.00;
    let leaveVehicleDelay: Float = 0.00;
    let timeMargin: Float = 1.00;
    let timeMarginForDelay: Float = 0.25;
    let maxDelayTime: Float = 8.00;
    if !IsDefined(condition.Target()) {
      return condition.Invert() ? !result : result;
    };
    if condition.Time() <= 0.00 || condition.Range() <= 0.00 {
      return condition.Invert() ? !result : result;
    };
    if !AIActionTarget.GetObject(context, condition.Target(), object) {
      return condition.Invert() ? !result : result;
    };
    transformHistoryComponent = (object as ScriptedPuppet).GetTransformHistoryComponent();
    if !IsDefined(transformHistoryComponent) {
      return condition.Invert() ? !result : result;
    };
    ...
    while currentTime <= condition.Time() {
      ArrayPush(positionHistory, transformHistoryComponent.GetInterpolatedPositionFromHistory(currentTime));
    ...
    if ArraySize(positionHistory) < 2 {
      return condition.Invert() ? !result : result;
    };
    i = 1;
    while i < ArraySize(positionHistory) {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIEverPerformedChase_Record>) -> Bool {
    let vehicle: wref<VehicleObject>;
    let result: Bool = false;
    if !IsDefined(condition) || !IsDefined(condition.Vehicle()) {
      return false;
    };
    if !AIActionTarget.GetVehicleObject(context, condition.Vehicle(), vehicle) {
      return false;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIAssignedVehicleCanReachTargetCond_Record>) -> Bool {
    let object: wref<GameObject>;
    let vehicle: wref<VehicleObject>;
    let wheeledVehicle: wref<WheeledObject>;
    let result: Bool = false;
    if !IsDefined(condition) || !IsDefined(condition.Vehicle()) || !IsDefined(condition.Target()) {
      return false;
    };
    if !AIActionTarget.GetObject(context, condition.Target(), object) {
      return false;
    ...
    wheeledVehicle = vehicle as WheeledObject;
    if !IsDefined(wheeledVehicle) {
      return !condition.Invert();
    };
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIVehicleInsideInnerAreaOfAreaSpeedLimiter_Record>) -> Bool {
    let vehicle: wref<VehicleObject>;
    let result: Bool = false;
    if AIActionTarget.GetVehicleObject(context, condition.Vehicle(), vehicle) {
      result = vehicle.IsVehicleInsideInnerAreaOfAreaSpeedLimiter();
    };
    return NotEquals(result, condition.Invert());
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIAssignedVehicleInPanicDriving_Record>) -> Bool {
    let vehicle: wref<VehicleObject>;
    let wheeledVehicle: wref<WheeledObject>;
    let result: Bool = false;
    if !IsDefined(condition) || !IsDefined(condition.Vehicle()) {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIAssignedVehicleInRace_Record>) -> Bool {
    let racingSystem: ref<RacingSystem>;
    let vehicle: wref<VehicleObject>;
    let wheeledVehicle: wref<WheeledObject>;
    let result: Bool = false;
    if !IsDefined(condition) || !IsDefined(condition.Vehicle()) {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIWorkspotCond_Record>) -> Bool {
    ...
    if IsDefined(condition.WorkspotObj()) {
      ...
      globalRef = ResolveNodeRef(workspotData.workspotRef, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot()));
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AISecurityCond_Record>) -> Bool {
    ...
    if IsDefined(condition.AreaType()) {
      ownerPuppet = ScriptExecutionContext.GetOwner(context) as ScriptedPuppet;
      securityAreaType = IntEnum<ESecurityAreaType>(Cast<Int32>(EnumValueFromName(n"ESecurityAreaType", condition.AreaType().EnumName())));
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AICommandCond_Record>) -> Bool {
    ...
    while i < count {
      ...
      if AIActionHelper.IsMoveCommand(condition.GetHasCommandsItem(i)) {
        if aiComponent.GetActionBlackboard().GetBool(GetAllBlackboardDefs().AIAction.ignoreInCombatMoveCommand) && NPCPuppet.IsInCombat(object as ScriptedPuppet) {
          return condition.Invert() ? true : false;
        };
      };
      i += 1;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AINPCTypeCond_Record>) -> Bool {
    ...
    if condition.DeviceState() >= 0 && IsDefined(device) {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AILoSPositionCond_Record>) -> Bool {
    let aiComponent: ref<AIHumanComponent>;
    let object: wref<GameObject>;
    let result: Bool;
    if !IsDefined(condition.Target()) {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIIsFromDynamicSpawnSystem_Record>) -> Bool {
    ...
    let result: Bool = dynamicSpawnSystem.IsEntityRegistered(ScriptExecutionContext.GetOwner(context).GetEntityID());
    return condition.Invert() ? !result : result;
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIIsFromPreventionSystem_Record>) -> Bool {
    let preventionSpawnSystem: ref<PreventionSpawnSystem> = GameInstance.GetPreventionSpawnSystem(ScriptExecutionContext.GetOwner(context).GetGame());
    let result: Bool = preventionSpawnSystem.IsEntityRegistered(ScriptExecutionContext.GetOwner(context).GetEntityID());
    if condition.Invert() {
      return !result;
    };
    return result;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIIsHeatStage5Active_Record>) -> Bool {
    ...
    let result: Bool = Equals(preventionSystem.GetHeatStage(), EPreventionHeatStage.Heat_5);
    if condition.Invert() {
      return !result;
    };
    return result;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AITargetInPreventionFreeArea_Record>) -> Bool {
    let preventionSpawnSystem: ref<PreventionSpawnSystem>;
    let result: Bool;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIIsShootingBlockedFromPrevention_Record>) -> Bool {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AICanShootInCarChaseCond_Record>) -> Bool {
    let target: ref<ScriptedPuppet>;
    let targetID: EntityID;
    let targetMountingInfo: MountingInfo;
    let targetVehicle: wref<VehicleObject>;
    let vehicle: wref<VehicleObject>;
    let result: Bool = false;
    let ownerID: EntityID = ScriptExecutionContext.GetOwner(context).GetEntityID();
    let ownerMountingInfo: MountingInfo = GameInstance.GetMountingFacility(ScriptExecutionContext.GetOwner(context).GetGame()).GetMountingInfoSingleWithIds(ownerID);
    ...
    target = ScriptExecutionContext.GetArgumentObject(context, n"CombatTarget") as ScriptedPuppet;
    if IsDefined(target) {
    ...
    if IsDefined(vehicle) && !vehicle.IsChasingTarget() {
      result = true;
    } else {
      if IsDefined(targetVehicle) && !targetVehicle.IsPlayerDriver() {
        result = true;
      } else {
        if !IsDefined(targetVehicle) && IsDefined(target) && EntityID.IsDefined(target.TryGetVehicleBelow()) {
          result = true;
        } else {
          if IsDefined(target) && !target.IsPlayer() {
            result = true;
          } else {
            if Equals(ownerMountingInfo.slotId.id, n"seat_front_left") {
  ...
  private final static func GetDifficultyValue(difficulty: gameDifficulty) -> Int32 {
    switch difficulty {
  ...
  private final static func GetPowerDifferentialValue(difficulty: gameEPowerDifferential) -> Int32 {
    switch difficulty {
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIGameDifficultyCond_Record>) -> Bool {
    let currentDifficulty: Int32 = AICondition.GetDifficultyValue(GameInstance.GetStatsDataSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetDifficulty());
    let comparedDifficulty: Int32 = condition.ComparedDifficulty();
    let comparisonType: EComparisonType = IntEnum<EComparisonType>(Cast<Int32>(EnumValueFromName(n"EComparisonType", condition.ComparisonType())));
    let result: Bool = ProcessCompareInt(comparisonType, comparedDifficulty, currentDifficulty);
    return condition.Invert() ? !result : result;
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AINPCDifficultyCond_Record>) -> Bool {
    let comparedDifficulty: Int32;
    let comparisonType: EComparisonType;
    let currentDifficulty: Int32;
    let result: Bool = false;
    let npc: ref<NPCPuppet> = ScriptExecutionContext.GetOwner(context) as NPCPuppet;
    if IsDefined(npc) {
      currentDifficulty = AICondition.GetPowerDifferentialValue(RPGManager.CalculatePowerDifferential(npc));
      comparedDifficulty = condition.ComparedDifficulty();
      comparisonType = IntEnum<EComparisonType>(Cast<Int32>(EnumValueFromName(n"EComparisonType", condition.ComparisonType())));
      result = ProcessCompareInt(comparisonType, comparedDifficulty, currentDifficulty);
    };
    return condition.Invert() ? !result : result;
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AINPCRarityCond_Record>) -> Bool {
    ...
    rarity = IntEnum<gamedataNPCRarity>(Cast<Int32>(EnumValueFromString("gamedataNPCRarity", condition.Rarity())));
    result = Equals(rarity, targetPuppet.GetNPCRarity());
    return condition.Invert() ? !result : result;
  }

  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIInArmedVehicleCond_Record>) -> Bool {
    let vehicle: wref<VehicleObject>;
  ...
  public final static func Check(const context: ScriptExecutionContext, condition: wref<AIPercentageChanceCond_Record>) -> Bool {
    let result: Bool;
    let successProbabilityPercent: Int32 = condition.SuccessProbability();
