...
public abstract class AISubActionPlayVoiceOver_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionPlayVoiceOver_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    ...
    if record.Delay() == 0.00 && !record.Repeat() {
      return AIbehaviorUpdateOutcome.SUCCESS;
    };
...
public abstract class AISubActionSetEquipWeaponsUtils extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionCharacterRecordEquip_Record>, const itemsToEquip: script_ref<[NPCItemToEquip]>, const duration: Float) -> AIbehaviorUpdateOutcome {
    let puppet: ref<ScriptedPuppet>;
    if ArraySize(Deref(itemsToEquip)) == 0 {
      return AIbehaviorUpdateOutcome.FAILURE;
    };
    ...
    AIActionHelper.SetItemsEquipData(puppet, Deref(itemsToEquip));
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
}
...
public abstract class AISubActionSetEquipPrimaryWeapons_Record_Implementation extends IScriptable {
  ...
  public final static func GetItemsToEquip(context: ScriptExecutionContext, record: wref<AISubActionCharacterRecordEquip_Record>, itemsToEquip: script_ref<[NPCItemToEquip]>) -> Bool {
...
public abstract class AISubActionEquipOnSlot_Record_Implementation extends IScriptable {
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionEquipOnSlot_Record>) -> Void {
    ...
    if IsDefined(BBoard) {
      itemsToEquip = FromVariant<array<NPCItemToEquip>>(BBoard.GetVariant(GetAllBlackboardDefs().AIAction.ownerItemsToEquip));
      if ArraySize(itemsToEquip) != 0 {
        ...
        if IsDefined(transactionSystem) {
          ...
          while i < ArraySize(itemsToEquip) {
            if !(ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).IsMechanical() {
              if cleanupDone {
              } else {
                if !transactionSystem.IsSlotEmpty(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponLeft") && !transactionSystem.IsSlotEmptySpawningItem(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponLeft") {
                  ...
                  if !transactionSystem.HasItemInSlot(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponLeft", itemsToEquip[i].itemID) && transactionSystem.RemoveItemFromSlot(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponLeft", true) {
                    NPCPuppet.SetAnimWrapperBasedOnEquippedItem(ScriptExecutionContext.GetOwner(context) as NPCPuppet, t"AttachmentSlots.WeaponLeft", itemToRemove.GetItemID(), 0.00);
                  };
                };
                if !transactionSystem.IsSlotEmpty(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponRight") && !transactionSystem.IsSlotEmptySpawningItem(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponRight") {
                  ...
                  if !transactionSystem.HasItemInSlot(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponLeft", itemsToEquip[i].itemID) && transactionSystem.RemoveItemFromSlot(ScriptExecutionContext.GetOwner(context), t"AttachmentSlots.WeaponRight", true) {
                    NPCPuppet.SetAnimWrapperBasedOnEquippedItem(ScriptExecutionContext.GetOwner(context) as NPCPuppet, t"AttachmentSlots.WeaponRight", itemToRemove.GetItemID(), 0.00);
                  };
                };
                cleanupDone = true;
              };
        ...
        if !record.UseItemSpawnDelayFromWeapon() || BBoard.GetFloat(GetAllBlackboardDefs().AIAction.ownerEquipItemTime) == 0.00 {
          AISubActionEquipOnSlot_Record_Implementation.Equip(context, itemsToEquip, record.CacheEquippedItem());
        };
      };
    };
  }
  ...
  public final static func ApplyAnimFeature(context: ScriptExecutionContext, const itemsToEquip: script_ref<[NPCItemToEquip]>, sendAnimFeature: Bool) -> Void {
    let i: Int32;
    let animFeature: ref<AnimFeature_AIAction> = new AnimFeature_AIAction();
    animFeature.state = 1;
    animFeature.stateDuration = -1.00;
    if ArraySize(Deref(itemsToEquip)) > 1 {
      animFeature.animVariation = 2;
    } else {
      if GameInstance.GetTransactionSystem(ScriptExecutionContext.GetOwner(context).GetGame()).HasTag(ScriptExecutionContext.GetOwner(context), n"MeleeWeapon", Deref(itemsToEquip)[0].itemID) {
    ...
    if sendAnimFeature {
      i = 0;
      while i < ArraySize(Deref(itemsToEquip)) {
        NPCPuppet.SetAnimWrapperBasedOnEquippedItem(ScriptExecutionContext.GetOwner(context) as NPCPuppet, Deref(itemsToEquip)[i].slotID, Deref(itemsToEquip)[i].itemID, 1.00);
        i += 1;
      };
    };
  ...
  public final static func Equip(context: ScriptExecutionContext, const itemsToEquip: script_ref<[NPCItemToEquip]>, cacheEquipped: Bool) -> Bool {
    let i: Int32;
    ...
    i = 0;
    while i < ArraySize(Deref(itemsToEquip)) {
      if ArrayContains(reservedEquipSlotIDs, Deref(itemsToEquip)[i].slotID) {
      } else {
        if equipOnBody && transactionSystem.HasItemInSlot(owner, Deref(itemsToEquip)[i].bodySlotID, Deref(itemsToEquip)[i].itemID) {
          result = transactionSystem.ChangeItemToSlot(owner, Deref(itemsToEquip)[i].slotID, Deref(itemsToEquip)[i].itemID);
          if result {
            ArrayPush(reservedEquipSlotIDs, Deref(itemsToEquip)[i].slotID);
          };
        } else {
          highPriority = NotEquals(WeaponObject.GetWeaponType(Deref(itemsToEquip)[i].itemID), gamedataItemType.Invalid);
          result = transactionSystem.AddItemToSlot(owner, Deref(itemsToEquip)[i].slotID, Deref(itemsToEquip)[i].itemID, highPriority, !cacheEquipped);
          if result {
            ArrayPush(reservedEquipSlotIDs, Deref(itemsToEquip)[i].slotID);
          };
        };
...
public abstract class AISubActionEquipOnBody_Record_Implementation extends IScriptable {
  ...
  public final static func EquipOnBody(context: ScriptExecutionContext, const itemsToEquip: script_ref<[NPCItemToEquip]>) -> Bool {
    ...
    if equipOnBody {
      i = 0;
      while i < ArraySize(Deref(itemsToEquip)) {
        if !transactionSystem.HasItem(owner, Deref(itemsToEquip)[i].itemID) {
          transactionSystem.GiveItem(owner, Deref(itemsToEquip)[i].itemID, 1);
        };
        if !transactionSystem.HasItemInSlot(owner, Deref(itemsToEquip)[i].bodySlotID, Deref(itemsToEquip)[i].itemID) {
          highPriority = NotEquals(WeaponObject.GetWeaponType(Deref(itemsToEquip)[i].itemID), gamedataItemType.Invalid);
          if transactionSystem.AddItemToSlot(owner, Deref(itemsToEquip)[i].bodySlotID, Deref(itemsToEquip)[i].itemID, highPriority) {
            result = true;
          };
        };
        i += 1;
...
public abstract class AISubActionForceEquip_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionForceEquip_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    if record.Delay() == 0.00 || record.Delay() > 0.00 && duration >= record.Delay() {
      if IsDefined(record.AttachmentSlot()) && AISubActionForceEquip_Record_Implementation.Equip(context, record) {
        return AIbehaviorUpdateOutcome.SUCCESS;
      };
      return AIbehaviorUpdateOutcome.FAILURE;
    };
  ...
  public final static func ApplyAnimFeature(context: ScriptExecutionContext, const itemsToEquip: script_ref<[NPCItemToEquip]>, record: wref<AISubActionForceEquip_Record>) -> Void {
    let i: Int32;
    let animFeature: ref<AnimFeature_AIAction> = new AnimFeature_AIAction();
    animFeature.state = 1;
    if record.AnimationTime() >= 0.00 {
...
public abstract class AISubActionSetUnequipWeaponsUtils extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionCharacterRecordUnequip_Record>, const itemsToUnequip: script_ref<[NPCItemToEquip]>, const duration: Float) -> AIbehaviorUpdateOutcome {
    let puppet: ref<ScriptedPuppet>;
    if ArraySize(Deref(itemsToUnequip)) == 0 {
      return AIbehaviorUpdateOutcome.FAILURE;
    };
    puppet = ScriptExecutionContext.GetOwner(context) as ScriptedPuppet;
    if !IsDefined(puppet) {
    ...
    AIActionHelper.SetItemsUnequipData(puppet, Deref(itemsToUnequip), record.DropItem());
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
}
...
public abstract class AISubActionSetUnequipPrimaryWeapons_Record_Implementation extends IScriptable {
  ...
  public final static func GetItemsToUnequip(context: ScriptExecutionContext, record: wref<AISubActionCharacterRecordUnequip_Record>, itemsToUnequip: script_ref<[NPCItemToEquip]>) -> Bool {
...
public abstract class AISubActionUnequipOnSlot_Record_Implementation extends IScriptable {
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionUnequipOnSlot_Record>) -> Void {
    ...
    let BBoard: ref<IBlackboard> = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetActionBlackboard();
    if IsDefined(BBoard) {
      itemsToUnequip = FromVariant<array<NPCItemToEquip>>(BBoard.GetVariant(GetAllBlackboardDefs().AIAction.ownerItemsToEquip));
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionUnequipOnSlot_Record>, const duration: Float, interrupted: Bool) -> Void {
    ...
    if IsDefined(BBoard) {
      ...
      itemsToUnequip = FromVariant<array<NPCItemToEquip>>(BBoard.GetVariant(GetAllBlackboardDefs().AIAction.ownerItemsUnequipped));
      i = 0;
  ...
  public final static func Unequip(context: ScriptExecutionContext, dropItem: Bool, itemsToUnequip: [NPCItemToEquip]) -> Bool {
    let BBoard: ref<IBlackboard>;
    let i: Int32;
  ...
  public final static func ApplyAnimFeature(context: ScriptExecutionContext, const itemsToUnequip: script_ref<[NPCItemToEquip]>) -> Void {
    let itemID: ItemID;
    let animFeature: ref<AnimFeature_AIAction> = new AnimFeature_AIAction();
    animFeature.state = 1;
    animFeature.stateDuration = -1.00;
    let i: Int32 = 0;
    while i < ArraySize(Deref(itemsToUnequip)) {
      itemID = Deref(itemsToUnequip)[i].itemID;
      animFeature.animVariation = 0;
      if GameInstance.GetTransactionSystem(ScriptExecutionContext.GetOwner(context).GetGame()).HasTag(ScriptExecutionContext.GetOwner(context), n"MeleeWeapon", itemID) {
        animFeature.animVariation = 1;
      };
...
public abstract class AISubActionForceUnequip_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionForceUnequip_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    if record.Delay() == 0.00 || record.Delay() > 0.00 && duration >= record.Delay() {
      if AISubActionForceUnequip_Record_Implementation.Unequip(context, record) {
        return AIbehaviorUpdateOutcome.SUCCESS;
      };
      return AIbehaviorUpdateOutcome.FAILURE;
    };
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionForceUnequip_Record>, const duration: Float, interrupted: Bool) -> Void {
    ...
    BBoard = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetActionBlackboard();
    if IsDefined(BBoard) {
      itemsToUnequip = FromVariant<array<NPCItemToEquip>>(BBoard.GetVariant(GetAllBlackboardDefs().AIAction.ownerItemsForceUnequipped));
      if ArraySize(itemsToUnequip) > 0 {
        i = 0;
        while i < ArraySize(itemsToUnequip) {
          NPCPuppet.SetAnimWrapperBasedOnEquippedItem(ScriptExecutionContext.GetOwner(context) as NPCPuppet, itemsToUnequip[i].slotID, itemsToUnequip[i].itemID, 0.00);
  ...
  public final static func Unequip(context: ScriptExecutionContext, record: wref<AISubActionForceUnequip_Record>) -> Bool {
    ...
    if IsDefined(record.AttachmentSlot()) {
      itemObj = GameInstance.GetTransactionSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetItemInSlot(ScriptExecutionContext.GetOwner(context), record.AttachmentSlot().GetID());
      if record.DropItem() {
        result = AISubActionForceUnequip_Record_Implementation.Drop(context, record) || !IsDefined(itemObj);
      } else {
        if IsDefined(itemObj) {
          itemID = itemObj.GetItemID();
  ...
  public final static func Drop(context: ScriptExecutionContext, record: wref<AISubActionForceUnequip_Record>) -> Bool {
    ...
    if IsDefined(weapon) {
      if GameInstance.GetTransactionSystem(ScriptExecutionContext.GetOwner(context).GetGame()).RemoveItemFromAnySlot(ScriptExecutionContext.GetOwner(context), weapon.GetItemID(), true) {
        return true;
      };
    };
    return false;
...
public abstract class AISubActionDisableAimAssist_Record_Implementation extends IScriptable {
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionDisableAimAssist_Record>) -> Void {
    if record.Delay() <= 0.00 {
      AISubActionDisableAimAssist_Record_Implementation.DisableAimAssist(context, record.Reason());
    };
  }
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionDisableAimAssist_Record>, const duration: Float, interrupted: Bool) -> Void {
    AISubActionDisableAimAssist_Record_Implementation.EnableAimAssist(context);
  }

  public final static func EnableAimAssist(context: ScriptExecutionContext) -> Void {
  ...
  public final static func DisableAimAssist(context: ScriptExecutionContext, opt reason: CName) -> Void {
    let player: ref<PlayerPuppet>;
    let puppet: ref<NPCPuppet>;
    if Equals(reason, n"timeDilation") {
    ...
    puppet = ScriptExecutionContext.GetOwner(context) as NPCPuppet;
    if !puppet.IsAimAssistEnabled() {
      return;
    };
...
public abstract class AISubActionApplyTimeDilation_Record_Implementation extends IScriptable {
  ...
  private final static func IsConditionFulfilled(context: ScriptExecutionContext, record: wref<AISubActionApplyTimeDilation_Record>) -> Bool {
    return !IsDefined(record.Condition()) || AICondition.CheckActionCondition(context, record.Condition());
  }
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionApplyTimeDilation_Record>) -> Void {
    ...
    player = GameInstance.GetPlayerSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetLocalPlayerMainGameObject() as PlayerPuppet;
    if IsDefined(player) {
      playerTimeDilation = 1.00 / GameInstance.GetTimeSystem(player.GetGame()).GetActiveTimeDilation(n"sandevistan", true);
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionApplyTimeDilation_Record>, const duration: Float, interrupted: Bool) -> Void {
    ...
    if blackboard.GetFloat(GetAllBlackboardDefs().AIAction.ownerTimeDilation) != -1.00 && !ScriptExecutionContext.GetOwner(context).HasIndividualTimeDilation() {
...
public abstract class AISubActionModifyStatPool_Record_Implementation extends IScriptable {
  ...
  public final static func ModifyStatPool(context: ScriptExecutionContext, record: wref<AISubActionModifyStatPool_Record>) -> Void {
    if IsDefined(record.StatPool()) && record.Amount() != 0.00 {
      GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).RequestChangingStatPoolValue(Cast<StatsObjectID>(ScriptExecutionContext.GetOwner(context).GetEntityID()), record.StatPool().StatPoolType(), record.Amount(), null, false, record.Perc());
    };
  }
}
...
public abstract class AISubActionStatusEffect_Record_Implementation extends IScriptable {
  ...
  private final static func IsConditionFulfilled(context: ScriptExecutionContext, record: wref<AISubActionStatusEffect_Record>) -> Bool {
    return !IsDefined(record.Condition()) || AICondition.CheckActionCondition(context, record.Condition());
  }

  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionStatusEffect_Record>) -> Void {
    if record.Delay() == 0.00 && AISubActionStatusEffect_Record_Implementation.IsConditionFulfilled(context, record) {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionStatusEffect_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    if !AISubActionStatusEffect_Record_Implementation.IsConditionFulfilled(context, record) {
      return AIbehaviorUpdateOutcome.SUCCESS;
    };
    if record.Delay() <= 0.00 {
      return AIbehaviorUpdateOutcome.SUCCESS;
...
public abstract class AISubActionChangeAttitude_Record_Implementation extends IScriptable {
  ...
  public final static func ChangeAttitude(context: ScriptExecutionContext, record: wref<AISubActionChangeAttitude_Record>) -> Void {
    ...
    desiredAttitude = IntEnum<EAIAttitude>(Cast<Int32>(EnumValueFromName(n"EAIAttitude", record.Attitude())));
...
public abstract class AISubActionThrowItem_Record_Implementation extends IScriptable {
  ...
  public final static func GetCachedGrenadeQuery(context: ScriptExecutionContext, out targetPosition: Vector4, out throwAngle: Float, outStartType: gameGrenadeThrowStartType) -> Bool {
    ...
    outStartType = FromVariant<gameGrenadeThrowStartType>((ScriptExecutionContext.GetOwner(context) as NPCPuppet).GetAIControllerComponent().GetCombatGadgetBlackboard().GetVariant(GetAllBlackboardDefs().CombatGadget.lastThrowStartType));
  ...
  public final static func ThrowItem(context: ScriptExecutionContext, record: wref<AISubActionThrowItem_Record>) -> Bool {
    ...
    let throwableKnife: ref<ThrowableKnifeNPC>;
    ...
    isNPCThrowableKnife = (item as ThrowableKnifeNPC) != null;
    if isGrenade {
      (ownerPuppet as ScriptedPuppet).GetAIControllerComponent().GetActionBlackboard().SetFloat(GetAllBlackboardDefs().AIAction.lastGrenadeThrowTimestamp, EngineTime.ToFloat(ScriptExecutionContext.GetAITime(context)));
    };
    if isGrenade || isNPCThrowableKnife {
      ...
      if !IsDefined(target) {
        ...
        if IsNameValid(record.ThrowType()) {
          if Equals(record.ThrowType(), n"smoke_defensive") {
            ownerPosition = ownerPuppet.GetWorldPosition();
            targetPosition = target.GetWorldPosition();
            if ownerPosition.Z - targetPosition.Z > 1.00 {
              distanceToTarget = Vector4.Distance(targetPosition, ownerPosition);
              targetPosition = ownerPosition + Vector4.ClampLength(targetPosition - ownerPosition, 2.50, distanceToTarget - 4.00);
            } else {
              targetPosition = ownerPosition + Vector4.ClampLength(targetPosition - ownerPosition, 0.00, 2.50);
            };
          } else {
            if Equals(record.ThrowType(), n"smoke_offensive") {
              ownerPosition = ownerPuppet.GetWorldPosition();
              targetPosition = target.GetWorldPosition();
              distanceToTarget = Vector4.Distance(targetPosition, ownerPosition);
              targetPosition = ownerPosition + Vector4.ClampLength(targetPosition - ownerPosition, 2.50, distanceToTarget - 4.00);
    ...
    launchEvent.launchParams.logicalOrientationProvider = IOrientationProvider.CreateEntityOrientationProvider(null, n"None", ownerPuppet);
    launchEvent.launchParams.visualPositionProvider = IPositionProvider.CreateEntityPositionProvider(item);
    launchEvent.launchParams.visualOrientationProvider = IOrientationProvider.CreateEntityOrientationProvider(null, n"None", item);
    ...
    if isNPCThrowableKnife {
      throwableKnife = item as ThrowableKnifeNPC;
      if throwableKnife.isFollowingKnife() {
        launchEvent.trajectoryParams = AISubActionThrowItem_Record_Implementation.CreateCurvedTrajectoryFollowTargetSlot(target, n"Chest", t"projectile.npc_throwable_knife_params");
      } else {
        launchEvent.trajectoryParams = AISubActionThrowItem_Record_Implementation.CreateCurvedTrajectory(target, t"projectile.npc_throwable_knife_params", record.PositionPredictionTime());
      };
    } else {
      launchEvent.trajectoryParams = AISubActionThrowItem_Record_Implementation.CreateParabolicTrajectory(record, targetPosition, throwAngle);
    };
  ...
  private final static func CreateParabolicTrajectory(record: wref<AISubActionThrowItem_Record>, targetPosition: Vector4, throwAngle: Float) -> ref<gameprojectileTrajectoryParams> {
    return ParabolicTrajectoryParams.GetAccelTargetAngleParabolicParams(new Vector4(0.00, 0.00, record.TrajectoryGravity(), 0.00), targetPosition, throwAngle);
  }

  private final static func CreateCurvedTrajectory(target: wref<GameObject>, paramsTDBRecord: TweakDBID, predictionTime: Float) -> ref<gameprojectileTrajectoryParams> {
    let targetPosition: Vector4;
    let targetVelocity: Vector4;
    let trajectoryParams: ref<FollowCurveTrajectoryParams>;
    AIActionHelper.GetTargetSlotPosition(target, n"Chest", targetPosition);
    trajectoryParams = new FollowCurveTrajectoryParams();
    AISubActionThrowItem_Record_Implementation.ReadCurvedTrajectoryTDBParams(trajectoryParams, paramsTDBRecord);
    if predictionTime > 0.00 && IsDefined(target as gamePuppet) {
      targetVelocity = (target as gamePuppet).GetVelocity();
      targetVelocity.Z = 0.00;
      targetPosition += Vector4.ClampLength(targetVelocity, 0.00, 4.50) * predictionTime;
    };
    trajectoryParams.targetPosition = targetPosition;
    return trajectoryParams;
  }

  private final static func CreateCurvedTrajectoryFollowTargetSlot(target: wref<GameObject>, slotName: CName, paramsTDBRecord: TweakDBID) -> ref<gameprojectileTrajectoryParams> {
    let trajectoryParams: ref<FollowCurveTrajectoryParams> = new FollowCurveTrajectoryParams();
    AISubActionThrowItem_Record_Implementation.ReadCurvedTrajectoryTDBParams(trajectoryParams, paramsTDBRecord);
  ...
  private final static func ReadCurvedTrajectoryTDBParams(out trajectoryParams: ref<FollowCurveTrajectoryParams>, paramsTDBRecord: TweakDBID) -> Void {
    trajectoryParams.startVelocity = TweakDBInterface.GetFloat(paramsTDBRecord + t".startVelocity", -1.00);
    trajectoryParams.linearTimeRatio = TweakDBInterface.GetFloat(paramsTDBRecord + t".linearTimeRatio", -1.00);
    trajectoryParams.interpolationTimeRatio = TweakDBInterface.GetFloat(paramsTDBRecord + t".interpolationTimeRatio", -1.00);
    trajectoryParams.returnTimeMargin = TweakDBInterface.GetFloat(paramsTDBRecord + t".returnTimeMargin", -1.00);
    trajectoryParams.bendTimeRatio = TweakDBInterface.GetFloat(paramsTDBRecord + t".bendTimeRatio", -1.00);
    trajectoryParams.bendFactor = TweakDBInterface.GetFloat(paramsTDBRecord + t".bendFactor", -1.00);
    trajectoryParams.halfLeanAngle = TweakDBInterface.GetFloat(paramsTDBRecord + t".halfLeanAngle", -1.00);
    trajectoryParams.endLeanAngle = TweakDBInterface.GetFloat(paramsTDBRecord + t".endLeanAngle", -1.00);
    trajectoryParams.angleInterpolationDuration = TweakDBInterface.GetFloat(paramsTDBRecord + t".angleInterpolationDuration", -1.00);
    trajectoryParams.angleInHitPlane = TweakDBInterface.GetFloat(paramsTDBRecord + t".angleInHitPlane", 30.00);
    trajectoryParams.angleInVerticalPlane = TweakDBInterface.GetFloat(paramsTDBRecord + t".angleInVerticalPlane", 0.00);
    trajectoryParams.accuracy = TweakDBInterface.GetFloat(paramsTDBRecord + t".accuracy", -1.00);
  }
...
public abstract class AISubActionAttackWithWeapon_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionAttackWithWeapon_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    let weapon: wref<WeaponObject>;
    if record.StopContinuousAttackOnDurationEnd() && AISubActionAttackWithWeapon_Record_Implementation.GetWeapon(context, record, weapon) && weapon.IsContinuousAttackStarted() {
  ...
  public final static func AttackWithWeapon(context: ScriptExecutionContext, record: wref<AISubActionAttackWithWeapon_Record>) -> AIbehaviorUpdateOutcome {
    ...
    let continuousAttack: wref<Attack_Continuous>;
    ...
    continuousAttack = attack as Attack_Continuous;
    if IsDefined(continuousAttack) {
      AISubActionAttackWithWeapon_Record_Implementation.PrepareContinuousAttack(weapon);
      effect = continuousAttack.GetRunningContinuousEffect();
      if !IsDefined(effect) {
        return AIbehaviorUpdateOutcome.FAILURE;
      };
      AISubActionAttackWithWeapon_Record_Implementation.FillEffectData(context, record, attack, effect);
      AISubActionAttackWithWeapon_Record_Implementation.StartPreparedContinuousAttack(context, weapon);
  ...
  public final static func PrepareContinuousAttack(weapon: wref<WeaponObject>) -> Void {
    weapon.PrepareContinuousAttack(weapon.GetWorldPosition(), weapon.GetWorldForward());
  }

  public final static func StartPreparedContinuousAttack(context: ScriptExecutionContext, weapon: wref<WeaponObject>) -> Void {
  ...
  public final static func FillEffectData(context: ScriptExecutionContext, record: wref<AISubActionAttackWithWeapon_Record>, attack: wref<Attack_GameEffect>, effect: wref<EffectInstance>) -> Void {
    ...
    let yOffset: Float;
    let meleeAttackRecord: wref<Attack_Melee_Record> = record.Attack() as Attack_Melee_Record;
  ...
  public final static func SetAttackNameInBlackBoard(context: ScriptExecutionContext, const attackName: CName) -> Void {
    let blackBoard: ref<IBlackboard>;
    if NotEquals(attackName, n"None") {
      blackBoard = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetActionBlackboard();
...
public abstract class AISubActionMeleeAttackManager_Record_Implementation extends IScriptable {
  ...
  public final static func StartWeaponTrailEffect(context: ScriptExecutionContext, const weapons: script_ref<[wref<ItemObject>]>) -> Void {
    let weaponRecord: ref<WeaponItem_Record>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(weapons)) {
      weaponRecord = TweakDBInterface.GetItemRecord(ItemID.GetTDBID(Deref(weapons)[i].GetItemID())) as WeaponItem_Record;
      if Equals(weaponRecord.ItemType().Type(), gamedataItemType.Cyb_StrongArms) {
        AISubActionMeleeAttackManager_Record_Implementation.StartStrongArmsTrailEffect(ScriptExecutionContext.GetOwner(context), Deref(weapons)[i]);
      } else {
        (Deref(weapons)[i] as WeaponObject).StartCurrentMeleeTrailEffect();
      };
      i += 1;
    };
  }
  ...
  public final static func StopWeaponTrailEffect(context: ScriptExecutionContext, const weapons: script_ref<[wref<ItemObject>]>) -> Void {
    let weaponRecord: ref<WeaponItem_Record>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(weapons)) {
      weaponRecord = TweakDBInterface.GetItemRecord(ItemID.GetTDBID(Deref(weapons)[i].GetItemID())) as WeaponItem_Record;
      if Equals(weaponRecord.ItemType().Type(), gamedataItemType.Cyb_StrongArms) {
        AISubActionMeleeAttackManager_Record_Implementation.stopStrongArmsTrailEffect(ScriptExecutionContext.GetOwner(context), Deref(weapons)[i]);
      } else {
        (Deref(weapons)[i] as WeaponObject).StopCurrentMeleeTrailEffect();
      };
      i += 1;
    };
  }
...
public abstract class AISubActionChimeraMetalstorm_Record_Implementation extends IScriptable {

  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionChimeraMetalstorm_Record>) -> Void {
    AISubActionShootWithWeapon_Record_Implementation.Activate(context, record);
  }

  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionChimeraMetalstorm_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    return AISubActionShootWithWeapon_Record_Implementation.Update(context, record, duration);
  }

  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionChimeraMetalstorm_Record>, const duration: Float, interrupted: Bool) -> Void {
    return AISubActionShootWithWeapon_Record_Implementation.Deactivate(context, record, duration, interrupted);
  }
}

public abstract class AISubActionShootFromCar_Record_Implementation extends IScriptable {

  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionShootFromCar_Record>) -> Void {
    let bboard: ref<IBlackboard>;
    ...
    if VehicleComponent.GetVehicle(ScriptExecutionContext.GetOwner(context).GetGame(), ScriptExecutionContext.GetOwner(context).GetEntityID(), vehicle) {
      if vehicle.IsArmedVehicle() && !vehicle.IsNPCShooting() {
        ...
        bboard = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetShootingBlackboard();
        bboard.SetBool(GetAllBlackboardDefs().AIShooting.shootingFromCar, false);
      };
    };
  }

  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionShootFromCar_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    let bboard: ref<IBlackboard>;
    let maxBurstProjecitles: Int32;
    let minBurstProjecitles: Int32;
    let target: wref<GameObject>;
    let targetPosition: Vector4;
    let vehicle: wref<VehicleObject>;
    if record.Delay() > 0.00 && duration < record.Delay() {
    ...
    if VehicleComponent.GetVehicle(ScriptExecutionContext.GetOwner(context).GetGame(), ScriptExecutionContext.GetOwner(context).GetEntityID(), vehicle) && vehicle.IsArmedVehicle() {
      ...
      if !bboard.GetBool(GetAllBlackboardDefs().AIShooting.shootingFromCar) {
        ...
        vehicle.NPCShoot(targetPosition, Cast<Uint32>(RandRange(minBurstProjecitles, maxBurstProjecitles)));
        bboard.SetBool(GetAllBlackboardDefs().AIShooting.shootingFromCar, true);
        return AIbehaviorUpdateOutcome.IN_PROGRESS;
      };
      if vehicle.IsNPCShooting() {
        return AIbehaviorUpdateOutcome.IN_PROGRESS;
      };
      return AIbehaviorUpdateOutcome.SUCCESS;
    };
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionShootFromCar_Record>, const duration: Float, interrupted: Bool) -> Void {
    let bboard: ref<IBlackboard> = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetShootingBlackboard();
    bboard.SetBool(GetAllBlackboardDefs().AIShooting.shootingFromCar, false);
  }
}
...
public abstract class AISubActionShootWithWeapon_Record_Implementation extends IScriptable {
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionShootWithWeapon_Record>) -> Void {
    ...
    (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetShootingBlackboard().SetFloat(GetAllBlackboardDefs().AIShooting.pauseConditionCheckTimeStamp, 0.00);
    i = 0;
    while i < ArraySize(weapons) {
      if IsDefined(record.TriggerMode()) {
        ...
        if AIActionHelper.WeaponHasTriggerMode(weapons[i], weaponRecord, gamedataTriggerMode.Burst) {
          weapons[i].GetAIBlackboard().SetInt(GetAllBlackboardDefs().AIShooting.requestedTriggerMode, 0);
        } else {
          if Equals(weaponRecord.PrimaryTriggerMode().Type(), gamedataTriggerMode.Charge) {
            weapons[i].GetAIBlackboard().SetInt(GetAllBlackboardDefs().AIShooting.requestedTriggerMode, 4);
          } else {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionShootWithWeapon_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    ...
    let pauseConditionCheckInterval: Float;
    let pauseConditionCheckTimeStamp: Float;
    ...
    if count > 0 {
      ...
      if pauseConditionCheckInterval <= 0.00 || pauseConditionCheckInterval > 0.00 && duration >= pauseConditionCheckTimeStamp + pauseConditionCheckInterval || pauseConditionCheckTimeStamp == 0.00 {
        i = 0;
        while i < count {
    ...
    while i < ArraySize(weapons) {
      ...
      GameInstance.GetStatPoolsSystem(ScriptExecutionContext.GetOwner(context).GetGame()).RequestSettingStatPoolValue(Cast<StatsObjectID>(weapons[i].GetEntityID()), gamedataStatPoolType.WeaponCharge, chargeLevel, ScriptExecutionContext.GetOwner(context));
      ...
      if record.IsA(n"gamedataAISubActionShootToPoint_Record") {
        ...
        if record.IsA(n"gamedataAISubActionMissileRainGrid_Record") {
          ...
          if record.IsA(n"gamedataAISubActionMissileRainCircular_Record") {
            ...
            if record.IsA(n"gamedataAISubActionChimeraMetalstorm_Record") {
              didShoot = AISubActionShootWithWeapon_Record_Implementation.ShootChimeraMetalstorm(context, record as AISubActionChimeraMetalstorm_Record, weapons[i], requestedTriggerMode, target, targetPosition, duration);
            };
          };
        };
      };
      if !didShoot {
        ...
        AISubActionShootWithWeapon_Record_Implementation.Shoot(context, record, duration, weapons[i], requestedTriggerMode, targetPosition, target, record.PredictionTime(), record.WeaponCustomEvent());
      };
  ...
  public final static func Shoot(context: ScriptExecutionContext, record: wref<AISubActionShootWithWeapon_Record>, const duration: Float, weapon: wref<WeaponObject>, requestedTriggerMode: gamedataTriggerMode, targetPosition: Vector4, target: wref<GameObject>, opt offset: Vector4, opt predictionTime: Float, opt muzzleOffset: Vector4, opt weaponCustomEvent: CName) -> Void {
    let rangedAttack: TweakDBID;
    if IsDefined(record.RangedAttack()) {
      rangedAttack = record.RangedAttack().GetID();
    };
    AIWeapon.Fire(ScriptExecutionContext.GetOwner(context), weapon, duration, record.TbhCoefficient(), requestedTriggerMode, targetPosition, target, rangedAttack, 0.00, record.AimingDelay(), Vector4.Vector3To4(record.TargetOffset()) + offset, AISubActionShootWithWeapon_Record_Implementation.ShouldTrackTarget(ScriptExecutionContext.GetOwner(context), record, weapon), predictionTime, muzzleOffset, weaponCustomEvent);
  }
  ...
  private final static func ShouldTrackTarget(owner: wref<gamePuppet>, record: wref<AISubActionShootWithWeapon_Record>, weapon: wref<WeaponObject>) -> Bool {
    if IsDefined(record as AISubActionMissileRainGrid_Record) || IsDefined(record as AISubActionMissileRainCircular_Record) || IsDefined(record as AISubActionChimeraMetalstorm_Record) {
      return true;
    };
  ...
  public final static func ShootToPoints(context: ScriptExecutionContext, record: wref<AISubActionShootToPoint_Record>, weapon: wref<WeaponObject>, requestedTriggerMode: gamedataTriggerMode, duration: Float) -> Bool {
    ...
    while i < ArraySize(shootPointPosition) {
      AISubActionShootWithWeapon_Record_Implementation.Shoot(context, record, duration, weapon, requestedTriggerMode, shootPointPosition[i], target, record.WeaponCustomEvent());
      i += 1;
    };
  ...
  private final static func ShootMissileRainGrid(context: ScriptExecutionContext, record: wref<AISubActionMissileRainGrid_Record>, weapon: wref<WeaponObject>, requestedTriggerMode: gamedataTriggerMode, target: wref<GameObject>, targetPosition: Vector4, duration: Float) -> Bool {
    ...
    i = 0;
    while i < numMissiles {
      AISubActionShootWithWeapon_Record_Implementation.Shoot(context, record, duration, weapon, requestedTriggerMode, targetPosition, target, Vector4.Vector3To4(missileOffsets[i]), record.WeaponCustomEvent());
      i += 1;
    };
    scriptedPuppet = ScriptExecutionContext.GetOwner(context) as ScriptedPuppet;
    if scriptedPuppet.IsBoss() {
  ...
  private final static func ShootChimeraMetalstorm(context: ScriptExecutionContext, record: wref<AISubActionChimeraMetalstorm_Record>, weapon: wref<WeaponObject>, requestedTriggerMode: gamedataTriggerMode, target: wref<GameObject>, targetPosition: Vector4, duration: Float) -> Bool {
    let i: Int32;
    let isOdd: Bool;
    let jh: Int32;
    let jv: Int32;
    let muzzleOffset: Vector4;
    let muzzleRotation: Quaternion;
    let muzzleTransform: Transform;
    ...
    jh = RandRange(-9, 9);
    isOdd = jv % 2 != 0;
    muzzleOffset.X = Cast<Float>(jv) * 0.10;
    muzzleOffset.Y = 0.00;
    muzzleOffset.Z = Cast<Float>(jh) * 0.10 + isOdd ? 0.05 : 0.00;
    ...
    AISubActionShootWithWeapon_Record_Implementation.Shoot(context, record, duration, weapon, requestedTriggerMode, targetPosition, target, targetOffset, muzzleOffset, record.WeaponCustomEvent());
    i = 0;
    while i < 2 {
      jv = RandRange(-2, 2);
      jh = RandRange(-9, 9);
      ...
      targetOffset.X = muzzleOffset.Z;
      targetOffset.Y = muzzleOffset.Y;
      targetOffset.Z = -muzzleOffset.X + 1.50;
      targetOffset = Quaternion.Transform(muzzleRotation, targetOffset);
      AISubActionShootWithWeapon_Record_Implementation.Shoot(context, record, duration, weapon, requestedTriggerMode, targetPosition, null, targetOffset, muzzleOffset, record.WeaponCustomEvent());
      i += 1;
    };
    return true;
  }

  public final static func SetShootPointsBytag(context: ScriptExecutionContext, const tags: script_ref<[CName]>, shootPointPosition: script_ref<[Vector4]>) -> Bool {
    let targetsPosition: array<Vector4>;
    if ArraySize(Deref(tags)) <= 0 {
      return false;
    };
    GameInstance.FindWaypointsByTag(ScriptExecutionContext.GetOwner(context).GetGame(), Deref(tags)[0], targetsPosition);
    shootPointPosition = targetsPosition;
    return true;
  }

  public final static func ConvertVector3ArrayToVector4Array(const v3: script_ref<[Vector3]>) -> [Vector4] {
    let tempVector4: Vector4;
    let tempVector4Array: array<Vector4>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(v3)) {
      tempVector4.X = Deref(v3)[i].X;
      tempVector4.Y = Deref(v3)[i].Y;
      tempVector4.Z = Deref(v3)[i].Z;
  ...
  public final static func SetShootPointsByCoordinate(context: ScriptExecutionContext, target: wref<GameObject>, const coordinateArray: script_ref<[Vector4]>, shootPointPosition: script_ref<[Vector4]>) -> Bool {
    let tempVector: Vector4;
    let targetPosition: Vector4 = target.GetWorldPosition();
    let i: Int32 = 0;
    while i < ArraySize(Deref(coordinateArray)) {
      tempVector = targetPosition + Deref(coordinateArray)[i];
      ArrayPush(Deref(shootPointPosition), tempVector);
      i += 1;
    };
    return true;
  }

  public final static func GetWeapon(context: ScriptExecutionContext, record: wref<AISubActionShootWithWeapon_Record>, weaponsList: script_ref<[wref<WeaponObject>]>) -> Bool {
    let weapon: wref<WeaponObject>;
    let count: Int32 = record.GetWeaponSlotsCount();
    let i: Int32 = 0;
    while i < count {
      weapon = GameInstance.GetTransactionSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetItemInSlot(ScriptExecutionContext.GetOwner(context), record.GetWeaponSlotsItem(i).GetID()) as WeaponObject;
      if IsDefined(weapon) {
        ArrayPush(Deref(weaponsList), weapon);
      };
      i += 1;
    };
    return ArraySize(Deref(weaponsList)) > 0;
  }
}
...
public abstract class AISubActionInAir_Record_Implementation extends IScriptable {

  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionInAir_Record>) -> Void {
    if record.Delay() == 0.00 {
      (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetPuppetStateBlackboard().SetBool(GetAllBlackboardDefs().PuppetState.InAirAnimation, true);
    };
  }

  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionInAir_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    if record.Delay() >= 0.00 && duration >= record.Delay() {
      (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetPuppetStateBlackboard().SetBool(GetAllBlackboardDefs().PuppetState.InAirAnimation, true);
      return AIbehaviorUpdateOutcome.SUCCESS;
    };
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionInAir_Record>, const duration: Float, interrupted: Bool) -> Void {
    (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetPuppetStateBlackboard().SetBool(GetAllBlackboardDefs().PuppetState.InAirAnimation, false);
  }
}
...
public abstract class AISubActionSetTargetByTag_Record_Implementation extends IScriptable {
  ...
  public final static func SetTargetByTag(context: ScriptExecutionContext, record: wref<AISubActionSetTargetByTag_Record>) -> Bool {
    ...
    itemObject = targetsObject[index] as ItemObject;
    if IsDefined(itemObject) && itemObject.IsConnectedWithDrop() {
      return AIActionTarget.Set(context, record.Target(), itemObject.GetConnectedItemDrop(), targetPosition);
    };
    i = 0;
    while i < ArraySize(targetsObject) {
      itemObject = targetsObject[i] as ItemObject;
      if IsDefined(itemObject) && itemObject.IsConnectedWithDrop() {
        return AIActionTarget.Set(context, record.Target(), itemObject.GetConnectedItemDrop(), targetPosition);
      };
      i += 1;
  ...
  public final static func GetDistancesFromTargetToPositions(context: ScriptExecutionContext, const target: Vector4, const positions: script_ref<[Vector4]>, outDistances: script_ref<[Float]>) -> Bool {
    let i: Int32;
    if ArraySize(Deref(positions)) <= 0 {
      return false;
    };
    ArrayClear(Deref(outDistances));
    ArrayResize(Deref(outDistances), ArraySize(Deref(positions)));
    i = 0;
    while i < ArraySize(Deref(positions)) {
      Deref(outDistances)[i] = Vector4.Distance(target, Deref(positions)[i]);
      i += 1;
    };
    return ArraySize(Deref(outDistances)) > 0;
  }

  public final static func GetDistancesFromEntity(context: ScriptExecutionContext, const targets: script_ref<[ref<Entity>]>, distances: script_ref<[Float]>) -> Bool {
    let i: Int32;
    let ownerRefVector: Vector4;
    if ArraySize(Deref(targets)) <= 0 {
      return false;
    };
    ownerRefVector = ScriptExecutionContext.GetOwner(context).GetWorldPosition();
    ArrayClear(Deref(distances));
    ArrayResize(Deref(distances), ArraySize(Deref(targets)));
    i = 0;
    while i < ArraySize(Deref(targets)) {
      Deref(distances)[i] = Vector4.Distance(ownerRefVector, Deref(targets)[i].GetWorldPosition());
      i += 1;
    };
    return ArraySize(Deref(distances)) > 0;
  }

  public final static func GetPositionsFromEntity(context: ScriptExecutionContext, const targets: script_ref<[ref<Entity>]>, positions: script_ref<[Vector4]>) -> Bool {
    let i: Int32;
    if ArraySize(Deref(targets)) <= 0 {
      return false;
    };
    ArrayClear(Deref(positions));
    ArrayResize(Deref(positions), ArraySize(Deref(targets)));
    i = 0;
    while i < ArraySize(Deref(targets)) {
      Deref(positions)[i] = Deref(targets)[i].GetWorldPosition();
      i += 1;
    };
    return ArraySize(Deref(positions)) > 0;
  }

  public final static func SelectIndex(context: ScriptExecutionContext, selectionPreference: CName, minDistance: Float, maxDistance: Float, const distances: script_ref<[Float]>, minDistanceObj: Float, maxDistanceObj: Float, const distancesObj: script_ref<[Float]>, opt targetsPosition: [Vector4], const allowedOffMeshTags: script_ref<[CName]>, lineOfSightTarget: Vector4, blockLoSByNonPenetrableObj: Bool) -> Int32 {
    if ArraySize(Deref(distances)) == 0 {
      return -1;
    };
    if Equals(selectionPreference, n"Farthest") {
      return AISubActionSetTargetByTag_Record_Implementation.GetFarthestIndexInRange(context, minDistance, maxDistance, distances, minDistanceObj, maxDistanceObj, distancesObj, targetsPosition, allowedOffMeshTags, lineOfSightTarget, blockLoSByNonPenetrableObj);
    };
    if Equals(selectionPreference, n"Closest") {
      return AISubActionSetTargetByTag_Record_Implementation.GetClosestIndexInRange(context, minDistance, maxDistance, distances, minDistanceObj, maxDistanceObj, distancesObj, targetsPosition, allowedOffMeshTags, lineOfSightTarget, blockLoSByNonPenetrableObj);
    };
    return AISubActionSetTargetByTag_Record_Implementation.GetRandomIndexInRange(context, minDistance, maxDistance, distances, minDistanceObj, maxDistanceObj, distancesObj, allowedOffMeshTags, targetsPosition, lineOfSightTarget, blockLoSByNonPenetrableObj);
  }

  public final static func CheckPath(context: ScriptExecutionContext, startPosition: Vector4, endPosition: Vector4, opt offMeshTags: [CName]) -> Bool {
  ...
  public final static func GetClosestIndexInRange(context: ScriptExecutionContext, minOwnerDistance: Float, maxOwnerDistance: Float, const ownerDistances: script_ref<[Float]>, minDistanceObj: Float, maxDistanceObj: Float, const distancesObj: script_ref<[Float]>, const targetsPosition: [Vector4], const allowedOffMeshTags: script_ref<[CName]>, lineOfSightTarget: Vector4, blockLoSByNonPenetrableObj: Bool) -> Int32 {
    let currentMinDistanceObj: Float;
    let hasLoSTarget: Bool;
    let i: Int32;
    let isMaxDistanceSpecified: Bool;
    let isMinDistanceSpecified: Bool;
    let isWithinMaxDistance: Bool;
    let isWithinMinDistance: Bool;
    let extremeDistance: Float = 9999.00;
    let targetPositionOffset: Vector4 = new Vector4(0.00, 0.00, 1.80, 0.00);
    let closestPointIndex: Int32 = -1;
    if ArraySize(targetsPosition) != ArraySize(Deref(ownerDistances)) || ArraySize(targetsPosition) != ArraySize(Deref(distancesObj)) {
      return closestPointIndex;
    };
    if maxDistanceObj < 0.00 {
      maxDistanceObj = extremeDistance;
    };
    if maxOwnerDistance < 0.00 {
      maxOwnerDistance = extremeDistance;
    };
    currentMinDistanceObj = extremeDistance;
    isMinDistanceSpecified = minOwnerDistance >= 0.00 || minDistanceObj >= 0.00;
    isMaxDistanceSpecified = maxOwnerDistance != extremeDistance || maxDistanceObj != extremeDistance;
    if !AISubActionSetTargetByTag_Record_Implementation.CheckPath(context, ScriptExecutionContext.GetOwner(context).GetWorldPosition(), lineOfSightTarget, Deref(allowedOffMeshTags)) {
      lineOfSightTarget = ScriptExecutionContext.GetOwner(context).GetWorldPosition();
    };
    i = 0;
    while i < ArraySize(Deref(ownerDistances)) {
      isWithinMinDistance = Deref(ownerDistances)[i] >= minOwnerDistance && Deref(distancesObj)[i] >= minDistanceObj;
      if isMinDistanceSpecified && !isWithinMinDistance {
      } else {
        isWithinMaxDistance = Deref(ownerDistances)[i] <= maxOwnerDistance && Deref(distancesObj)[i] <= maxDistanceObj;
        if isMaxDistanceSpecified && !isWithinMaxDistance {
          ...
          if hasLoSTarget && !GameInstance.GetSenseManager(ScriptExecutionContext.GetOwner(context).GetGame()).IsPositionVisible(lineOfSightTarget + targetPositionOffset, targetsPosition[i] + targetPositionOffset, blockLoSByNonPenetrableObj) {
            ...
            if !AISubActionSetTargetByTag_Record_Implementation.CheckPath(context, ScriptExecutionContext.GetOwner(context).GetWorldPosition(), targetsPosition[i], Deref(allowedOffMeshTags)) {
            } else {
              if Deref(distancesObj)[i] >= currentMinDistanceObj {
              } else {
                currentMinDistanceObj = Deref(distancesObj)[i];
                closestPointIndex = i;
              };
            };
          };
  ...
  public final static func GetFarthestIndexInRange(context: ScriptExecutionContext, minOwnerDistance: Float, maxOwnerDistance: Float, const ownerDistances: script_ref<[Float]>, minDistanceObj: Float, maxDistanceObj: Float, const distancesObj: script_ref<[Float]>, const targetsPosition: [Vector4], const allowedOffMeshTags: script_ref<[CName]>, lineOfSightTarget: Vector4, blockLoSByNonPenetrableObj: Bool) -> Int32 {
    let currentMaxDistanceObj: Float;
    let hasLoSTarget: Bool;
    let i: Int32;
    let isMaxDistanceSpecified: Bool;
    let isMinDistanceSpecified: Bool;
    ...
    let extremeDistance: Float = 9999.00;
    let targetPositionOffset: Vector4 = new Vector4(0.00, 0.00, 1.80, 0.00);
    let farthestPointIndex: Int32 = -1;
    if ArraySize(targetsPosition) != ArraySize(Deref(ownerDistances)) || ArraySize(targetsPosition) != ArraySize(Deref(distancesObj)) {
        ...
        if isMaxDistanceSpecified && !isWithinMaxDistance {
          ...
          if hasLoSTarget && !GameInstance.GetSenseManager(ScriptExecutionContext.GetOwner(context).GetGame()).IsPositionVisible(lineOfSightTarget + targetPositionOffset, targetsPosition[i] + targetPositionOffset, blockLoSByNonPenetrableObj) {
            ...
            if !AISubActionSetTargetByTag_Record_Implementation.CheckPath(context, ScriptExecutionContext.GetOwner(context).GetWorldPosition(), targetsPosition[i], Deref(allowedOffMeshTags)) {
              ...
              if Deref(distancesObj)[i] <= currentMaxDistanceObj {
              } else {
                currentMaxDistanceObj = Deref(distancesObj)[i];
  ...
  public final static func GetRandomIndexInRange(context: ScriptExecutionContext, minOwnerDistance: Float, maxOwnerDistance: Float, const ownerDistances: script_ref<[Float]>, minDistanceObj: Float, maxDistanceObj: Float, const distancesObj: script_ref<[Float]>, const allowedOffMeshTags: script_ref<[CName]>, opt targetsPosition: [Vector4], lineOfSightTarget: Vector4, blockLoSByNonPenetrableObj: Bool) -> Int32 {
    ...
    if ArraySize(filteredIndexes) > 0 {
      selectedIndex = filteredIndexes[RandRange(0, ArraySize(filteredIndexes))];
    };
    return selectedIndex;
  }
}
...
public abstract class AISubActionSetWaypointByTag_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionSetWaypointByTag_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    if record.Delay() >= 0.00 && duration >= record.Delay() {
      if AISubActionSetWaypointByTag_Record_Implementation.SetWaypointByTag(context, record) {
        return AIbehaviorUpdateOutcome.SUCCESS;
      };
      return AIbehaviorUpdateOutcome.FAILURE;
    };
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionSetWaypointByTag_Record>, const duration: Float, interrupted: Bool) -> Void {
    if record.Delay() < 0.00 {
      AISubActionSetWaypointByTag_Record_Implementation.SetWaypointByTag(context, record);
    };
  }

  public final static func SetWaypointByTag(context: ScriptExecutionContext, record: wref<AISubActionSetWaypointByTag_Record>) -> Bool {
    let allowedOffMeshTags: array<CName>;
    let blackboard: ref<IBlackboard>;
    let desiredDistance: Vector2;
    let desiredDistanceFromObj: Vector2;
    ...
    if searchingForPosition && record.AvoidSelectingSameTargetMethod() == 1 {
      excludedWaypoints = FromVariant<array<Vector4>>(blackboard.GetVariant(GetAllBlackboardDefs().AIActionBossData.excludedWaypointPosition));
      if ArraySize(excludedWaypoints) > 0 {
        index = ArrayFindFirst(positions, excludedWaypoints[0]);
        if index >= 0 {
      ...
      if searchingForPosition && record.AvoidSelectingSameTargetMethod() == 2 {
        excludedWaypoints = FromVariant<array<Vector4>>(blackboard.GetVariant(GetAllBlackboardDefs().AIActionBossData.excludedWaypointPosition));
        if ArraySize(excludedWaypoints) > 0 {
          i = 0;
    ...
    if record.AvoidSelectingSameTargetMethod() == 1 {
      ArrayClear(excludedWaypoints);
      ArrayPush(excludedWaypoints, targetPosition);
      blackboard.SetVariant(GetAllBlackboardDefs().AIActionBossData.excludedWaypointPosition, ToVariant(excludedWaypoints));
    } else {
  ...
  public final static func SetStimSource(context: ScriptExecutionContext, record: wref<AISubActionSetStimSource_Record>) -> Void {
    ...
    if record.UseInvestigateData() {
      ...
      if !Vector4.IsZero(position) {
        AISubActionSetStimSource_Record_Implementation.SetStimSourcePosition(context, record, position);
      } else {
        AISubActionSetStimSource_Record_Implementation.SetStimSourcePosition(context, record, investigateData.controllerEntity.GetWorldPosition());
      };
      ...
      if record.StimTarget().IsObject() {
        if AIActionTarget.GetObject(context, record.StimTarget(), object) {
          ...
          AISubActionSetStimSource_Record_Implementation.SetStimSourcePosition(context, record, object.GetWorldPosition());
        };
      } else {
        if record.StimTarget().IsPosition() {
          ...
          AISubActionSetStimSource_Record_Implementation.SetStimSourcePosition(context, record, reactionData.stimSource);
        };
      };
    };
  }

  private final static func SetStimSourcePosition(context: ScriptExecutionContext, record: wref<AISubActionSetStimSource_Record>, position: Vector4) -> Void {
    if record.RevertStimSourceDirection() {
      position = 2.00 * ScriptExecutionContext.GetOwner(context).GetWorldPosition() - position;
    };
...
public abstract class AISubActionChangeCoverSelectionPreset_Record_Implementation extends IScriptable {
  ...
  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionChangeCoverSelectionPreset_Record>) -> Void {
    ...
    let blackboard: ref<IBlackboard> = AICoverHelper.GetCoverBlackboard(ScriptExecutionContext.GetOwner(context) as ScriptedPuppet);
...
public abstract class AISubActionStartCooldown_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionStartCooldown_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    ...
    if record.MinActionDuration() > 0.00 && record.MinActionDuration() > duration {
      return AIbehaviorUpdateOutcome.IN_PROGRESS;
    };
    if record.Delay() > 0.00 && duration >= record.Delay() {
      AISubActionStartCooldown_Record_Implementation.StartCooldowns(context, record);
...
public abstract class AISubActionSecuritySystemNotification_Record_Implementation extends IScriptable {
  ...
  public final static func NotifySecuritySystem(context: ScriptExecutionContext, record: wref<AISubActionSecuritySystemNotification_Record>) -> Void {
    ...
    notificationType = IntEnum<ESecurityNotificationType>(Cast<Int32>(EnumValueFromName(n"ESecurityNotificationType", record.NotificationType().EnumName())));
    if IsDefined(threat) {
...
public abstract class AISubActionCallSquadSearchBackUp_Record_Implementation extends IScriptable {
  ...
  public final static func CallBackup(context: ScriptExecutionContext, record: wref<AISubActionCallSquadSearchBackUp_Record>) -> Void {
    let broadcaster: ref<StimBroadcasterComponent>;
    let calledCount: Int32;
    let combatVersion: Bool;
    let commsCallEffects: array<ref<StatusEffect>>;
    let diff: Vector4;
    let distance: Float;
    let i: Int32;
    let j: Int32;
    let member: wref<Entity>;
    let memberObject: ref<GameObject>;
    let members: array<wref<Entity>>;
    let membersDistances: array<Float>;
    let minDistance: Float;
    let minSkipThreshold: Int32;
    let ownerPosition: Vector4;
    let ownerReactionComp: ref<ReactionManagerComponent>;
    let player: ref<GameObject>;
    let psi: ref<PuppetSquadInterface>;
    let remainingDuration: Float;
    let skippedCount: Int32;
    let statsSystem: ref<StatsSystem>;
    let whistleEffects: array<ref<StatusEffect>>;
    let owner: ref<GameObject> = ScriptExecutionContext.GetOwner(context);
    let numberOfTargets: Int32 = 1;
    let maxDistance: Float = 0.00;
    AISquadHelper.GetSquadBaseInterface(owner, psi);
    members = psi.ListMembersWeak();
    ArrayRemove(members, owner);
    if ArraySize(members) == 0 {
      return;
    };
    if record.UseCommsCallLogic() || record.NumberOfTargets() > 0.00 {
      ...
      while i < ArraySize(members) {
        ...
        ArrayPush(membersDistances, distance);
        i += 1;
      };
      i = 0;
      while i < ArraySize(members) - 1 {
        j = 0;
        while j < ArraySize(members) - i - 1 {
    ...
    if !record.UseCommsCallLogic() {
      ...
      i = 0;
      while i < ArraySize(members) && (record.NumberOfTargets() <= 0.00 || i < Cast<Int32>(record.NumberOfTargets())) {
        memberObject = members[i] as GameObject;
        broadcaster.SendDrirectStimuliToTarget(owner, gamedataStimType.Call, memberObject);
        i += 1;
      };
      return;
    };
    StatusEffectHelper.GetAppliedEffectsWithTag(owner, n"CommsCallIn", commsCallEffects);
    if ArraySize(commsCallEffects) == 0 {
      return;
    };
    StatusEffectHelper.GetAppliedEffectsWithTag(owner, n"Whistle", whistleEffects);
    if ArraySize(whistleEffects) > 0 {
      if StatusEffectSystem.ObjectHasStatusEffectWithTag(owner, n"CommsCallInMultipleOnWhistle") {
        numberOfTargets = 4;
      } else {
        if StatusEffectSystem.ObjectHasStatusEffectWithTag(owner, n"CommsCallInMultipleOnWhistle2") {
          numberOfTargets = 5;
        };
      };
      if whistleEffects[0].GetLastApplicationSimTimestamp() < commsCallEffects[0].GetLastApplicationSimTimestamp() {
        ...
        ownerReactionComp = (owner as ScriptedPuppet).GetStimReactionComponent();
        if IsDefined(ownerReactionComp) && Equals(ownerReactionComp.GetPendingReactionName(), gamedataOutput.PlayerCall) {
    ...
    maxDistance = statsSystem.GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.CommsCallDistance);
    combatVersion = StatusEffectSystem.ObjectHasStatusEffectWithTag(owner, n"CombatCommsCallIn");
    remainingDuration = commsCallEffects[0].GetRemainingDuration();
    if record.Invert() {
      memberObject = members[0] as GameObject;
      i = 1;
      while i < ArraySize(members) {
      ...
      AISubActionCallSquadSearchBackUp_Record_Implementation.CallBackupOneTarget(memberObject, owner, combatVersion, remainingDuration, true);
      return;
    };
    calledCount = 0;
    skippedCount = 0;
    ...
    minSkipThreshold = 2;
    i = 0;
    while i < ArraySize(members) && calledCount < numberOfTargets {
      ...
      if i < minSkipThreshold && membersDistances[i] < minDistance {
        ...
        memberObject = members[i] as GameObject;
        AISubActionCallSquadSearchBackUp_Record_Implementation.CallBackupOneTarget(owner, memberObject, combatVersion, remainingDuration, false);
        calledCount += 1;
      };
      i += 1;
    };
    i = 0;
    while i < skippedCount && calledCount < numberOfTargets {
      memberObject = members[i] as GameObject;
      AISubActionCallSquadSearchBackUp_Record_Implementation.CallBackupOneTarget(owner, memberObject, combatVersion, remainingDuration, false);
      calledCount += 1;
      i += 1;
  ...
  private final static func CallBackupOneTarget(source: ref<GameObject>, target: ref<GameObject>, combatVersion: Bool, remainingDuration: Float, purelyDirect: Bool) -> Void {
    let broadcaster: ref<StimBroadcasterComponent> = source.GetStimBroadcasterComponent();
    if !IsDefined(broadcaster) {
      return;
    };
    ...
    if combatVersion {
      StatusEffectHelper.ApplyStatusEffectForTimeWindow(target, t"BaseStatusEffect.CommsCallCombatTargetHelper", source.GetEntityID(), 0.00, remainingDuration);
    };
  }
}
...
public abstract class AISubActionQuickHack_Record_Implementation extends IScriptable {
  ...
  public final static func CancelConnection(context: ScriptExecutionContext, record: wref<AISubActionQuickHack_Record>) -> Void {
    ...
    if IsDefined(signalTable) {
      signalId = signalTable.GetOrCreateSignal(n"HackingCompleted");
      if !signalTable.GetCurrentValue(signalId) {
        if StatusEffectSystem.ObjectHasStatusEffect(GameInstance.GetPlayerSystem(ScriptExecutionContext.GetOwner(context).GetGame()).GetLocalPlayerMainGameObject() as PlayerPuppet, t"StatusEffect.HackReveal") {
...
public abstract class AISubActionActivateStrongArmsFX_Record_Implementation extends IScriptable {
  ...
  public final static func SpawnStrongArmsFX(context: ScriptExecutionContext) -> Void {
    ...
    let weapon: wref<WeaponObject> = ScriptedPuppet.GetWeaponRight(ScriptExecutionContext.GetOwner(context));
    if !IsDefined(weapon) {
      return;
    };
...
public abstract class AISubActionMountVehicle_Record_Implementation extends IScriptable {
  ...
  private final static func GetAIComponent(context: ScriptExecutionContext) -> ref<AIHumanComponent> {
    let comp: ref<AIHumanComponent>;
    AIHumanComponent.Get(ScriptExecutionContext.GetOwner(context) as ScriptedPuppet, comp);
    return comp;
  }
  ...
  public final static func MountVehicle(context: ScriptExecutionContext, record: wref<AISubActionMountVehicle_Record>) -> Bool {
    ...
    let aiComponent: ref<AIHumanComponent> = AISubActionMountVehicle_Record_Implementation.GetAIComponent(context);
    ...
    if Equals(slotName, n"first_available") && IsDefined(vehicle.GetAIComponent()) {
      ...
      if IsNameValid(slotName) && IsDefined(aiComponent) {
...
public abstract class AISubActionSetWorldPosition_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionSetWorldPosition_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
...
public abstract class AISubActionCover_Record_Implementation extends IScriptable {
  ...
  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionCover_Record>, const duration: Float, interrupted: Bool) -> Void {
    let pullSquadSyncRequest: ref<DelaySetCoverNPCCurrentlyExposed>;
    let puppet: wref<ScriptedPuppet> = ScriptExecutionContext.GetOwner(context) as ScriptedPuppet;
    if !IsDefined(puppet) {
      return;
    };
    if record.ExposedInCover() != -1 && AICoverHelper.GetCoverNPCCurrentlyExposed(puppet) {
      ...
      if interrupted {
        ...
        GameInstance.GetDelaySystem(puppet.GetGame()).DelayEvent(puppet, pullSquadSyncRequest, 0.40);
      } else {
        AICoverHelper.SetCoverNPCCurrentlyExposed(puppet, false);
      };
    };
  ...
  public final static func SetDesiredCover(context: ScriptExecutionContext, record: wref<AISubActionCover_Record>) -> Void {
    ...
    if checkedCoverID > 0u {
      if cm.IsShootingSpot(checkedCoverID) && !GameObject.IsCooldownActive(puppet, n"ShootingSpotSelection") {
        AIActionHelper.StartCooldown(puppet, n"ShootingSpotSelection", 600.00);
      };
        ...
        if ArraySize(exposureMethods) == 0 {
          ...
          while i < selectedCoversSize {
            if !msc.coversUseLOS[i] {
              ...
              if cm.IsShootingSpot(msc.selectedCovers[i]) && !AISubActionCover_Record_Implementation.EvaluateShootingSpot(puppet, target, cm, msc.selectedCovers[i]) {
              } else {
  ...
  public final static func EvaluateShootingSpot(puppet: wref<ScriptedPuppet>, target: wref<GameObject>, cm: wref<CoverManager>, consideredCoverID: Uint64) -> Bool {
    let shootingSpotPosition: Vector4 = cm.GetCoverPosition(consideredCoverID);
    let targetPosition: Vector4 = target.GetWorldPosition();
    if shootingSpotPosition.Z - targetPosition.Z < 1.50 && GameObject.IsCooldownActive(puppet, n"ShootingSpotSelection") {
      return false;
    };
    return true;
  }
  ...
  public final static func CalculateCoverExposureMethod(puppet: wref<ScriptedPuppet>, const coverID: Uint64, record: wref<AISubActionCover_Record>, const target: wref<GameObject>, lineOfSightTolerance: Float, const trackedLocation: script_ref<TrackedLocation>, exposureMethods: script_ref<[AICoverExposureMethod]>) -> AICoverExposureMethod {
...
public abstract class AISubActionHitData_Record_Implementation extends IScriptable {
  ...
  public final static func GetAnimVariation(context: ScriptExecutionContext, record: wref<AISubActionHitData_Record>) -> Int32 {
    let backDirection: Int32;
    let newDirection: Int32;
    let object: wref<GameObject>;
    let animVariation: Int32 = -1;
    let hitData: ref<AnimFeature_HitReactionsData> = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetHitReactionComponent().GetLastHitReactionData();
...
public abstract class AISubActionBlockData_Record_Implementation extends IScriptable {

  public final static func GetAnimVariation(context: ScriptExecutionContext, record: wref<AISubActionBlockData_Record>) -> Int32 {
    let animVariation: Int32 = -1;
    let ownerPuppet: wref<ScriptedPuppet> = ScriptExecutionContext.GetOwner(context) as ScriptedPuppet;
    let attackDirection: Int32 = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetHitReactionComponent().GetAttackDirection();
    switch attackDirection {
      case 0:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Center);
        break;
      case 1:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Center);
        break;
      case 2:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Left);
        break;
      case 3:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Left);
        break;
      case 4:
        ...
        break;
      case 5:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Right);
        break;
      case 6:
        animVariation = AISubActionBlockData_Record_Implementation.SetAnimVariation(ownerPuppet, EAIBlockDirection.Right);
        break;
  ...
  public final static func SetAnimVariation(owner: wref<ScriptedPuppet>, direction: EAIBlockDirection) -> Int32 {
    ...
    if !blackBoard.GetBool(GetAllBlackboardDefs().AIAction.ownerCurrentAnimVariationSet) {
      ...
      if ArraySize(animVariationList) > 1 {
        previousAnimVariation = AISubActionBlockData_Record_Implementation.GetAnimVariationInBlackBoard(blackBoard, owner);
        if previousAnimVariation == 1 && ArrayContains(animVariationList, 2) {
          animVariation = 2;
        } else {
          if previousAnimVariation == 2 && ArrayContains(animVariationList, 1) {
            animVariation = 1;
          } else {
            if ArrayContains(animVariationList, previousAnimVariation) {
              ArrayRemove(animVariationList, previousAnimVariation);
            };
            animVariation = RandRange(0, ArraySize(animVariationList));
      ...
      AISubActionBlockData_Record_Implementation.SetAnimVariationInBlackBoard(blackBoard, owner, animVariation);
    } else {
      return AISubActionBlockData_Record_Implementation.GetAnimVariationInBlackBoard(blackBoard, owner);
    };
    return animVariation;
  }

  public final static func SetAnimVariationInBlackBoard(blackBoard: ref<IBlackboard>, owner: wref<ScriptedPuppet>, animVariation: Int32) -> Void {
  ...
  public final static func GetAnimVariationInBlackBoard(blackBoard: ref<IBlackboard>, owner: wref<ScriptedPuppet>) -> Int32 {
    return blackBoard.GetInt(GetAllBlackboardDefs().AIAction.ownerLastBlockAnimVariation);
  }

  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionBlockData_Record>, const duration: Float, interrupted: Bool) -> Void {
    let blackBoard: ref<IBlackboard> = (ScriptExecutionContext.GetOwner(context) as ScriptedPuppet).GetAIControllerComponent().GetActionBlackboard();
...
public abstract class AISubActionInitialReactionParams_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionInitialReaction_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
...
public abstract class AISubActionRandomize_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionRandomize_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    return AIbehaviorUpdateOutcome.SUCCESS;
  }
  ...
  public final static func GetAnimVariation(context: ScriptExecutionContext, record: wref<AISubActionRandomize_Record>) -> Int32 {
    ...
    if !blackBoard.GetBool(GetAllBlackboardDefs().AIAction.ownerCurrentAnimVariationSet) {
      ...
      if ArraySize(animVariationList) > 1 {
        if FromVariant<TweakDBID>(blackBoard.GetVariant(GetAllBlackboardDefs().AIAction.ownerLastAnimVariationAction)) != TDBID.None() {
...
public abstract class AISubActionCallReinforcements_Record_Implementation extends IScriptable {
  ...
  public final static func StartCallReinforcement(context: ScriptExecutionContext, record: wref<AISubActionCallReinforcements_Record>) -> Void {
    ...
    if !m_statPoolsSystem.IsStatPoolAdded(Cast<StatsObjectID>(m_puppet.GetEntityID()), m_statPoolType) {
...
public abstract class AISubActionGeneratePointOfInterestTarget_Record_Implementation extends IScriptable {
  ...
  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionGeneratePointOfInterestTarget_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    ...
    seed = Cast<Int32>(100.00 * (currentPointOfInterest.Y + currentPointOfInterest.Z));
...
public abstract class AISubActionSetTopThreatPersistance_Record_Implementation extends IScriptable {

  public final static func Activate(context: ScriptExecutionContext, record: wref<AISubActionSetTopThreatPersistance_Record>) -> Void {
    AISubActionSetTopThreatPersistance_Record_Implementation.SetTopThreatPersistance(context, record, true);
  }

  public final static func Update(context: ScriptExecutionContext, record: wref<AISubActionSetTopThreatPersistance_Record>, const duration: Float) -> AIbehaviorUpdateOutcome {
    return AIbehaviorUpdateOutcome.IN_PROGRESS;
  }

  public final static func Deactivate(context: ScriptExecutionContext, record: wref<AISubActionSetTopThreatPersistance_Record>, const duration: Float, interrupted: Bool) -> Void {
    AISubActionSetTopThreatPersistance_Record_Implementation.ResetAllThreatsPersistenceForSource(context, record);
  }

  public final static func SetTopThreatPersistance(context: ScriptExecutionContext, record: wref<AISubActionSetTopThreatPersistance_Record>, set: Bool) -> Void {
    let combatTarget: wref<GameObject> = ScriptExecutionContext.GetArgumentObject(context, n"CombatTarget");
    if IsDefined(combatTarget) {
      TargetTrackingExtension.SetThreatPersistence(ScriptExecutionContext.GetOwner(context) as ScriptedPuppet, combatTarget, set, Cast<Uint32>(record.Source().EnumValue()));
    };
  }

  public final static func ResetAllThreatsPersistenceForSource(context: ScriptExecutionContext, record: wref<AISubActionSetTopThreatPersistance_Record>) -> Void {
    TargetTrackingExtension.SetCurrentThreatsPersistence(ScriptExecutionContext.GetOwner(context) as ScriptedPuppet, false, Cast<Uint32>(record.Source().EnumValue()));
  }
}
