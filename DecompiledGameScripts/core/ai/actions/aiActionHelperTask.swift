...
public abstract class AIActionHelperTask extends AIbehaviortaskScript {
  ...
  private final func GetActionStringName(context: ScriptExecutionContext) -> String {
    ...
    if IsDefined(this.m_actionTweakIDMapping) {
      actionTweakID = NameToString(FromVariant<CName>(ScriptExecutionContext.GetMappingValue(context, this.m_actionTweakIDMapping)));
    };
...
public class DestroyWeakspot extends AIActionHelperTask {
  ...
  public let m_weakspotArray: [wref<WeakspotObject>];
  ...
  protected final func DestroyWeakspot(context: ScriptExecutionContext, const weakspots: script_ref<[wref<WeakspotObject>]>, index: Int32) -> Void {
    GameInstance.GetStatPoolsSystem(AIBehaviorScriptBase.GetGame(context)).RequestSettingStatPoolValue(Cast<StatsObjectID>(Deref(weakspots)[index].GetEntityID()), gamedataStatPoolType.WeakspotHealth, 0.00, null, true);
  }
}
...
public class SetDestinationWaypoint extends AIActionHelperTask {
  ...
  private let m_destinations: [Vector4];

  private let m_finalDestinations: [Vector4];
  ...
  protected final func GetLowestDistanceIndex(const distances: script_ref<[Float]>) -> Int32 {
    let k: Int32;
    let lowestValue: Float;
    let j: Int32 = 0;
    while j < ArraySize(Deref(distances)) {
      if Deref(distances)[j] < lowestValue || lowestValue == 0.00 {
        lowestValue = Deref(distances)[j];
        k = j;
      };
      j += 1;
    };
  ...
  protected final func GetDistances(refVector: Vector4) -> [Float] {
...
public class CheckFloatIsValid extends AIbehaviorconditionScript {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    if IsDefined(this.actionTweakIDMapping) {
      this.value = FromVariant<Float>(ScriptExecutionContext.GetMappingValue(context, this.actionTweakIDMapping));
      if this.value <= 0.00 {
        return AIbehaviorConditionOutcomes.False;
      };
    };
...
public class CheckBoolisValid extends AIbehaviorconditionScript {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    let value: Bool;
    if IsDefined(this.actionTweakIDMapping) {
      value = FromVariant<Bool>(ScriptExecutionContext.GetMappingValue(context, this.actionTweakIDMapping));
...
public class CheckVectorIsValid extends AIbehaviorconditionScript {
  ...
  protected func Check(context: ScriptExecutionContext) -> AIbehaviorConditionOutcomes {
    if IsDefined(this.actionTweakIDMapping) {
      this.value = FromVariant<Vector4>(ScriptExecutionContext.GetMappingValue(context, this.actionTweakIDMapping));
