...
public abstract class AITicketCondition extends IScriptable {
  ...
  public final static func PerformChecking(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, conditions: script_ref<[wref<AITicketCondition_Record>]>, ticketRecord: wref<AITicket_Record>, squadRecord: wref<AISquadParams_Record>) -> Bool {
  ...
  public final static func PerformChecking(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, conditions: script_ref<[wref<AITicketCondition_Record>]>, ticketRecord: wref<AITicket_Record>) -> Bool {
    ...
    while i >= 0 {
      if willingSquadMembers[i] != entity && (!interface.IsSquadActionOpen(ticketName, willingSquadMembers[i]) || interface.HasOrderBySquadAction(ticketName, willingSquadMembers[i])) {
        ArrayErase(willingSquadMembers, i);
      };
      i -= 1;
    };
  ...
  public final static func Check(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, check: wref<AITicketCheck_Record>, ticketRecord: wref<AITicket_Record>, out squadMembers: [wref<Entity>]) -> Bool {
  ...
  public final static func CheckOR(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, check: wref<AISquadORCondition_Record>, ticketRecord: wref<AITicket_Record>, out squadMembers: [wref<Entity>]) -> Bool {
  ...
  public final static func CheckContainsSelf(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, check: wref<AISquadContainsSelfCheck_Record>, const squadMembers: script_ref<[wref<Entity>]>) -> Bool {
    return ArrayContains(Deref(squadMembers), entity);
  }

  public final static func CheckMembersAmount(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, check: wref<AISquadMembersAmountCheck_Record>, const squadMembers: script_ref<[wref<Entity>]>) -> Bool {
    let amount: Int32 = ArraySize(Deref(squadMembers));
    if !check.CountSelf() {
      amount -= 1;
    };
  ...
  public final static func CheckAND(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, check: wref<AISquadANDCondition_Record>, ticketRecord: wref<AITicket_Record>, out squadMembers: [wref<Entity>]) -> Bool {
  ...
  public final static func FilterOut(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, filter: wref<AITicketFilter_Record>, ticketRecord: wref<AITicket_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
  ...
  public final static func CheckNumberOfTickets(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, ticketRecord: wref<AITicket_Record>) -> Bool {
    ...
    if ticketRecord.ScaleNumberOfTicketsFromWorkspots() {
      workspotObject = FromVariant<wref<GameObject>>((entity as ScriptedPuppet).GetAIControllerComponent().GetBehaviorArgument(n"StimTarget"));
    ...
    if ticketRecord.PercentageNumberOfTickets() >= 0.00 {
      ticketCap = FloorF(Cast<Float>(ArraySize(squadMembers)) * ticketRecord.PercentageNumberOfTickets());
  ...
  public final static func FilterAgainstChoosingPreviouslySelected(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, condition: wref<AISquadAvoidLastFilter_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    let canditate: wref<Entity>;
    let gameObject: wref<GameObject>;
    let ticketName: CName;
    if ArraySize(Deref(squadMembers)) <= 1 {
      return;
    };
    ...
    if condition.Invert() {
      ArrayClear(Deref(squadMembers));
      ArrayPush(Deref(squadMembers), canditate);
    } else {
      ArrayRemove(Deref(squadMembers), canditate);
    };
  }

  public final static func CheckDistanceRelationToTarget(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, condition: wref<AISquadDistanceRelationToTargetCheck_Record>, closest: Bool, squadMembers: script_ref<[wref<Entity>]>) -> Bool {
    ...
    while i < ArraySize(Deref(squadMembers)) {
      if !AIHumanComponent.GetScriptContext(Deref(squadMembers)[i] as ScriptedPuppet, context) {
      } else {
        if !AIActionTarget.Get(context, condition.Target(), false, target, targetPosition) {
        } else {
          distance = AbsF(Vector4.Distance(Deref(squadMembers)[i].GetWorldPosition(), targetPosition) - condition.RingRadius());
          if compareDistance == 0.00 || Equals(closest, compareDistance > distance) {
            compareDistance = distance;
            candidate = Deref(squadMembers)[i];
          };
        };
      };
      i += 1;
  ...
  public final static func FilterSpatialForOwnTarget(entity: wref<Entity>, condition: wref<AISquadSpatialForOwnTarget_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    ...
    i = ArraySize(Deref(squadMembers)) - 1;
    while i >= 0 {
      if !AIHumanComponent.GetScriptContext(Deref(squadMembers)[i] as ScriptedPuppet, squadMemberContext) {
        return;
      };
      if !AICondition.CheckSpatial(ownerContext, squadMemberContext, condition.Spatial()) {
        if condition.Invert() {
          ArrayPush(candidates, Deref(squadMembers)[i]);
        } else {
          ArrayErase(Deref(squadMembers), i);
        };
      };
      i -= 1;
    };
  ...
  public final static func FilterAICondition(condition: wref<AISquadFilterByAICondition_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    let candidates: array<wref<Entity>>;
    let context: ScriptExecutionContext;
    let i: Int32;
  ...
  public final static func CheckDistanceRelationToSector(entity: wref<Entity>, interface: wref<PuppetSquadInterface>, condition: wref<AISquadDistanceRelationToSectorCheck_Record>, closest: Bool, ticketRecord: wref<AITicket_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Bool {
    let candidate: wref<Entity>;
    let compareDistance: Float;
    let distance: Float;
    let i: Int32;
    ...
    while i < ArraySize(sectors) {
      sectorInt = Cast<Int32>(EnumValueFromName(n"AICombatSectorType", sectors[i].EnumName()));
      j = 0;
      while j < ArraySize(Deref(squadMembers)) {
        distance = combatAlley.GetDistanceFromSector(IntEnum<AICombatSectorType>(sectorInt), Deref(squadMembers)[j].GetWorldPosition());
        if compareDistance == 0.00 || Equals(closest, compareDistance > distance) {
          compareDistance = distance;
          candidate = Deref(squadMembers)[j];
        };
        j += 1;
      };
      i += 1;
  ...
  public final static func FilterJustSelf(entity: wref<Entity>, condition: wref<AISquadJustSelfFilter_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    if condition.Invert() {
      ArrayRemove(Deref(squadMembers), entity);
    } else {
      ArrayClear(Deref(squadMembers));
      ArrayPush(Deref(squadMembers), entity);
    };
  }

  public final static func FilterInSector(interface: wref<PuppetSquadInterface>, condition: wref<AISquadInSectorFilter_Record>, ticketRecord: wref<AITicket_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    let candidates: array<wref<Entity>>;
    ...
    j = ArraySize(Deref(squadMembers)) - 1;
    while j >= 0 {
      isInAnySector = false;
      i = 0;
      while i < ArraySize(sectors) {
        sectorInt = Cast<Int32>(EnumValueFromName(n"AICombatSectorType", sectors[i].EnumName()));
        isInAnySector = NotEquals(combatAlley.GetSector(Deref(squadMembers)[j].GetWorldPosition()), IntEnum<AICombatSectorType>(sectorInt));
        if isInAnySector {
          break;
        };
        i += 1;
      ...
      if !isInAnySector {
        if condition.Invert() {
          ArrayPush(candidates, Deref(squadMembers)[j]);
        } else {
          ArrayErase(Deref(squadMembers), j);
        };
      };
      j -= 1;
    };
  ...
  public final static func FilterTargetSpotted(entity: wref<Entity>, condition: wref<AISquadFilterOwnTargetSpotted_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    let candidates: array<wref<Entity>>;
    let i: Int32;
    let ownerContext: ScriptExecutionContext;
    let squadMemberContext: ScriptExecutionContext;
  ...
  public final static func FilterItemPriority(condition: wref<AISquadItemPriorityFilter_Record>, squadMembers: script_ref<[wref<Entity>]>) -> Void {
    ...
    backupMembers = Deref(squadMembers);
    ...
    while j < prioritiesAmount && !foundMatch {
      i = 0;
      while i < ArraySize(Deref(squadMembers)) {
        itemRecord = TweakDBInterface.GetItemRecord(ItemID.GetTDBID(AITicketCondition.GetItem(Deref(squadMembers)[i]).GetItemID()));
