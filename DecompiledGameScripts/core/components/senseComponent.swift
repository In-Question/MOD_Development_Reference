...
public native class SenseComponent extends IPlacedComponent {
  ...
  public native let forceDisableUI: Bool;

  private let m_highLevelCb: ref<CallbackHandle>;

  private let m_reactionCb: ref<CallbackHandle>;

  private let m_wantedLevelChangedCb: ref<CallbackHandle>;
  ...
  private let m_enabledSenses: Bool;

  private let m_shouldStartDetectingPlayerCached: Bool;

  private let m_wasPlayerLastReactionTarget: Bool;

  @default(SenseComponent, ESenseLogSource.Undefined)
  private let m_logSource: ESenseLogSource;
  ...
  public final native func SetCrowdsAggressiveState(isAggresive: Bool) -> Void;
  ...
  public final native func ResetDetection(entityID: EntityID) -> Bool;
  ...
  public final native func GetSenseShapes() -> [ref<ISenseShape>];
  ...
  public final native func UpdateCrowdMappin() -> Void;
  ...
  public final native func SetVisionBlockersIgnoredBySensor(ignoredVisionBlockers: VisionBlockerTypeFlags) -> Bool;

  private final native func SetIsPlayerInterestingFromSecuritySystemPOV(isPlayerInteresting: Bool) -> Void;

  public final native func IgnoreLODChange(ignoreLOD: Bool) -> Void;

  private final const func LogEnabled() -> Bool {
    return AIComponent.DebugLogEnabled(this.GetConstOwnerPuppet());
  }

  private final const func Log(category: CName, message: String) -> Void {
    let aiComponent: ref<AIHumanComponent> = this.GetConstOwnerPuppet().GetAIControllerComponent();
    if IsDefined(aiComponent) {
      aiComponent.DebugLog(category, message);
    };
  }

  private final const func LogBySource(category: CName, message: String) -> Void {
    switch this.m_logSource {
      case ESenseLogSource.AddToBlacklistEvent:
        this.Log(category, "OnAddToBlacklist: " + message);
        break;
      case ESenseLogSource.RemoveFromBlacklistEvent:
        this.Log(category, "OnRemoveFromBlacklist: " + message);
        break;
      case ESenseLogSource.SecurityAreaCrossingPerimeter:
        this.Log(category, "OnSecurityAreaCrossing: " + message);
        break;
      default:
  ...
  public final func LogStart(source: ESenseLogSource, message: String) -> Void {
    this.m_logSource = source;
    if !this.LogEnabled() {
      return;
    };
    this.LogBySource(this.LogCategory("start"), message);
  }
  ...
  public final const func LogInfo(message: String) -> Void {
    ...
    this.LogBySource(this.LogCategory("info"), message);
  }

  public final const func LogTrueFalse(message: String, value: Bool) -> Void {
    if !this.LogEnabled() {
      return;
    };
    this.LogBySource(this.LogCategory("info"), message + " = " + value ? "TRUE" : "FALSE");
  }
  ...
  public final const func GetConstOwnerPuppet() -> ref<ScriptedPuppet> {
    return this.GetEntity() as ScriptedPuppet;
  }

  public final func TryCreateSenseMappin() -> Bool {
    if !this.m_hideUIElements && this.m_enabledSenses {
  ...
  public final static func RequestMainPresetChange(obj: wref<GameObject>, const presetName: script_ref<String>) -> Void {
    let presetID: TweakDBID = TDBID.Create("Senses." + presetName);
    SenseComponent.RequestPresetChange(obj, presetID, true);
  }

  public final static func RequestSecondaryPresetChange(obj: wref<GameObject>, const presetName: script_ref<String>) -> Void {
    let presetID: TweakDBID = TDBID.Create("Senses." + presetName);
  ...
  protected cb func OnSensePresetChangeEvent(evt: ref<SensePresetChangeEvent>) -> Bool {
    if evt.reset {
      ...
      this.m_secondaryPreset = TDBID.None();
    } else {
      if evt.mainPreset {
  ...
  protected cb func OnSenseInitialize(evt: ref<SenseInitializeEvent>) -> Bool {
    ...
    let puppet: wref<ScriptedPuppet> = this.GetOwnerPuppet();
    if IsDefined(puppet) {
      this.m_hideUIElements = puppet.GetBoolFromCharacterTweak("hideUIElements", false);
    };
    if this.forceDisableUI {
      this.m_hideUIElements = true;
    };
    if IsDefined(sensorDevice) && sensorDevice.GetDevicePS().IsPartOfPrevention() {
      if sensorDevice.GetPreventionSystem().AreTurretsActive() {
        this.m_enabledSenses = true;
        if !this.m_hideUIElements {
          this.CreateSenseMappin();
        };
      };
    } else {
    ...
    if IsDefined(puppet) {
      ...
      if IsDefined(this.m_puppetBlackboard) {
        ...
        this.m_wasPlayerLastReactionTarget = false;
      };
      this.m_uiWantedBarBB = GameInstance.GetBlackboardSystem(puppet.GetGame()).Get(GetAllBlackboardDefs().UI_WantedBar);
      if IsDefined(this.m_uiWantedBarBB) {
        this.m_wantedLevelChangedCb = this.m_uiWantedBarBB.RegisterListenerInt(GetAllBlackboardDefs().UI_WantedBar.CurrentWantedLevel, this, n"OnCurrentWantedLevelChanged");
      };
    };
  }
  ...
  private final func OnDetach() -> Void {
    ...
    if IsDefined(puppet) {
      ...
      this.m_uiWantedBarBB = GameInstance.GetBlackboardSystem(puppet.GetGame()).Get(GetAllBlackboardDefs().UI_WantedBar);
      if IsDefined(this.m_uiWantedBarBB) {
        this.m_uiWantedBarBB.UnregisterListenerInt(GetAllBlackboardDefs().UI_WantedBar.CurrentWantedLevel, this.m_wantedLevelChangedCb);
      };
    };
  ...
  protected cb func OnCurrentWantedLevelChanged(value: Int32) -> Bool {
    let hasDeescalated: Bool;
    let reevaluatedInterest: Bool;
    let wantedLevel: EPreventionHeatStage;
    if !this.GetOwner().IsPrevention() {
      return true;
    };
    wantedLevel = IntEnum<EPreventionHeatStage>(value);
    hasDeescalated = Equals(wantedLevel, EPreventionHeatStage.Heat_0);
    if !hasDeescalated {
      return true;
    };
    this.ReevaluateDetectionOverwrite(GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerControlledGameObject());
    reevaluatedInterest = this.ReevaluatePlayerInterestFromSecuritySystemPOV();
    if reevaluatedInterest {
      this.LogInfo("set player INTERESTING from Security System POV");
    } else {
      this.LogInfo("set player NOT INTERESTING from Security System POV");
    };
    this.SetIsPlayerInterestingFromSecuritySystemPOV(reevaluatedInterest);
  }
  ...
  protected cb func OnReactionChanged(value: Int32) -> Bool {
    let targetToReevaluate: wref<GameObject>;
    ...
    if !IsDefined(reactionData) && !this.m_wasPlayerLastReactionTarget {
      return true;
    };
    targetToReevaluate = IsDefined(reactionData.stimTarget) ? reactionData.stimTarget : GameInstance.GetPlayerSystem(this.GetGame()).GetLocalPlayerControlledGameObject();
    this.ReevaluateDetectionOverwrite(targetToReevaluate);
    this.m_wasPlayerLastReactionTarget = targetToReevaluate.IsPlayer();
  }
  ...
  protected cb func OnAddToBlacklistEvent(evt: ref<AddToBlacklistEvent>) -> Bool {
    let reevaluatedInterest: Bool;
    this.LogStart(ESenseLogSource.AddToBlacklistEvent, "processing event");
    if !evt.isPlayerEntity {
      this.LogInfo("not player in blacklist");
      return false;
    };
    if NotEquals(evt.reason, BlacklistReason.COMBAT) {
      this.LogInfo("blacklist for non-combat reason");
      return false;
    };
    reevaluatedInterest = this.ReevaluatePlayerInterestFromSecuritySystemPOV();
    if reevaluatedInterest {
  ...
  protected cb func OnRemoveFromBlacklistEvent(evt: ref<RemoveFromBlacklistEvent>) -> Bool {
    let reevaluatedInterest: Bool;
    this.LogStart(ESenseLogSource.RemoveFromBlacklistEvent, "processing event");
    if !evt.isPlayerEntity {
      this.LogInfo("not player in blacklist");
      return false;
    };
  ...
  private final func PlayerEnteredPerception(player: wref<PlayerPuppet>) -> Void {
    ...
    if !this.HasSenseMappin() && !this.m_enabledSenses {
      return;
    };
    if IsDefined(this.m_playerInPerception) {
      this.PlayerExitedPercpetion(this.m_playerInPerception);
  ...
  protected cb func OnSecurityAreaCrossingPerimeter(evt: ref<SecurityAreaCrossingPerimeter>) -> Bool {
    let isBlacklistedForCombat: Bool;
    let isEnteringOrExitingWhenBlacklisted: Bool;
    let reevaluatedInterest: Bool;
    let securitySystem: ref<SecuritySystemControllerPS>;
    let shouldStartDetectingBreacher: Bool;
    let owner: wref<GameObject> = this.GetEntity() as GameObject;
    let breacher: wref<GameObject> = evt.GetWhoBreached();
    this.LogStart(ESenseLogSource.SecurityAreaCrossingPerimeter, "processing event");
    if !IsDefined(owner) || !IsDefined(breacher) {
      this.LogInfo("no owner or breacher");
      return false;
    };
    shouldStartDetectingBreacher = this.ReevaluateDetectionOverwrite(breacher);
    if !breacher.IsPlayerControlled() {
      this.LogInfo("breacher is not player");
      return false;
    };
    securitySystem = owner.GetSecuritySystem();
    isBlacklistedForCombat = IsDefined(securitySystem) && securitySystem.IsEntityBlacklistedForAtLeast(breacher, BlacklistReason.COMBAT);
    isEnteringOrExitingWhenBlacklisted = evt.GetEnteredState() || isBlacklistedForCombat;
    this.LogTrueFalse("enteredState", evt.GetEnteredState());
    this.LogTrueFalse("isBlacklistedForCombat", isBlacklistedForCombat);
    this.LogTrueFalse("shouldStartDetectingBreacher", shouldStartDetectingBreacher);
  ...
  private final func ReevaluatePlayerInterestFromSecuritySystemPOV() -> Bool {
    return this.IsTargetInterestingForSecuritySystem(GetPlayer(this.GetGame()));
  }
  ...
  protected cb func OnSensorOwnerChangedEvent(evt: ref<SensorOwnerChangedEvent>) -> Bool {
    this.SetVisionBlockersIgnoredBySensor(GetIgnoredVisionBlockerTypes(this.GetGame(), Cast<StatsObjectID>(evt.newOwnerId)));
  }

  public final func UpdateVisionBlockersIgnoredBySensor() -> Void {
    this.SetVisionBlockersIgnoredBySensor(GetIgnoredVisionBlockerTypes(this.GetGame(), Cast<StatsObjectID>(this.GetEntity().GetEntityID())));
  }
  ...
  public final static func ShouldIgnoreIfPlayerCompanion(owner: wref<Entity>, target: wref<Entity>) -> Bool {
    ...
    if IsDefined(ownerPuppet) {
      ...
      if IsDefined(aiControllerComponent) {
        commandCombatTarget = FromVariant<wref<GameObject>>(aiControllerComponent.GetBehaviorArgument(n"CommandCombatTarget"));
    ...
    i = 0;
    while i < ArraySize(trackers) {
      if TargetTrackingExtension.GetTrackedLocation(trackers[i] as ScriptedPuppet, playerPuppet, trackedLocation) {
        if trackedLocation.accuracy > 0.10 {
          return false;
        };
      };
      i += 1;
  ...
  private final func IsPlayerRecentlyDroppedThreat(target: wref<GameObject>) -> Bool {
    ...
    if !TargetTrackingExtension.Get(this.GetOwnerPuppet(), tte) {
      return false;
    };
  ...
  public final func ReevaluateDetectionOverwrite(target: wref<GameObject>, opt isVisible: Bool) -> Bool {
    if !this.IsEnabled() {
      ...
      if target.IsPlayer() {
        this.SetDetectionAttitudeOverwrite(n"player");
        this.m_shouldStartDetectingPlayerCached = false;
      };
      return false;
    };
    if !IsDefined(target) {
    ...
    if this.ShouldStartDetecting(target) {
      ...
      if target.IsPlayer() {
        ...
        this.m_shouldStartDetectingPlayerCached = true;
      };
  ...
  public final func ShouldStartDetectingPlayer(player: ref<PlayerPuppet>) -> Bool {
    ...
    if owner.IsPuppet() {
      ...
      if !TargetTrackingExtension.IsThreatInThreatList(ownerPuppet, player, false, true) {
        if !ownerPuppet.IsAggressive() && NotEquals(ownerPuppet.GetStimReactionComponent().GetReactionPreset().Type(), gamedataReactionPresetType.Civilian_Guard) && NotEquals(ownerPuppet.GetStimReactionComponent().GetReactionPreset().Type(), gamedataReactionPresetType.Civilian_Grabbable) {
          return false;
        };
      };
  ...
  public final func GetShouldStartDetectingPlayerCached() -> Bool {
    return this.m_shouldStartDetectingPlayerCached;
  }
...
public class SetNPCSensesMainPreset extends AIbehaviortaskScript {

  public edit let newSensesPresetName: String;
...
public class ResetNPCSensesMainPreset extends AIbehaviortaskScript {
