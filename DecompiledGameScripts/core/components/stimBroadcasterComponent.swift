...
public struct StimTargetData {
  ...
  public final static func IsValid(self: StimTargetData) -> Bool {
    return GlobalNodeRef.IsDefined(ResolveNodeRef(self.spawnerRef, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot()))) && IsNameValid(self.entryID);
  }
}

public class StimTargetsEvent extends Event {

  public edit let targets: [StimTargetData];
...
public class StimBroadcasterComponent extends ScriptableComponent {
  ...
  public let m_activeRequests: [ref<StimRequest>];
  ...
  public let m_targets: [NPCstubData];

  private let m_blockedStims: [StimIdentificationData];

  @default(StimBroadcasterComponent, 1.0f)
  public let m_fallbackInterval: Float;

  public final const func IsStimProcessingBlocked(sourceID: EntityID, stimType: gamedataStimType, opt stimName: CName) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_blockedStims) {
      if this.m_blockedStims[i].sourceID == sourceID && Equals(this.m_blockedStims[i].stimType, stimType) && Equals(this.m_blockedStims[i].stimName, stimName) {
        return true;
      };
      i += 1;
    };
  ...
  public final func ResolveStimProcessingCooldown(sourceID: EntityID, stimType: gamedataStimType, opt stimName: CName, cooldown: Float) -> Bool {
    let cooldownEvent: ref<BlockStimProcessingCooldownEvent>;
    let stimData: StimIdentificationData;
    stimData.sourceID = sourceID;
    stimData.stimType = stimType;
    stimData.stimName = stimName;
    if this.IsStimBlocked(stimData) {
      return false;
    };
    if cooldown > 0.00 {
      cooldownEvent = new BlockStimProcessingCooldownEvent();
      stimData.delayID = GameInstance.GetDelaySystem(this.GetOwner().GetGame()).DelayEvent(this.GetOwner(), cooldownEvent, cooldown, false);
      cooldownEvent.stimData = stimData;
    };
    this.AddBlockedStim(stimData);
    return true;
  }

  protected cb func OnBlockStimProcessingCooldownEvent(evt: ref<BlockStimProcessingCooldownEvent>) -> Bool {
    this.RemoveBlockedStim(evt.stimData.delayID);
  }

  private final func IsStimBlocked(data: script_ref<StimIdentificationData>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_blockedStims) {
      if this.m_blockedStims[i].sourceID == Deref(data).sourceID && Equals(this.m_blockedStims[i].stimType, Deref(data).stimType) && Equals(this.m_blockedStims[i].stimName, Deref(data).stimName) {
        return true;
      };
      i += 1;
    };
  ...
  private final func AddBlockedStim(data: script_ref<StimIdentificationData>) -> Void {
    ArrayPush(this.m_blockedStims, Deref(data));
  }

  private final func RemoveBlockedStim(data: script_ref<StimIdentificationData>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_blockedStims) {
      if this.m_blockedStims[i].sourceID == Deref(data).sourceID && Equals(this.m_blockedStims[i].stimType, Deref(data).stimType) && Equals(this.m_blockedStims[i].stimName, Deref(data).stimName) {
        ArrayErase(this.m_blockedStims, i);
        return;
      };
      i += 1;
    };
  ...
  private final func RemoveBlockedStim(delayID: DelayID) -> Void {
    let i: Int32;
    ...
    i = 0;
    while i < ArraySize(this.m_blockedStims) {
      if this.m_blockedStims[i].delayID == delayID {
        ArrayErase(this.m_blockedStims, i);
        return;
      };
      i += 1;
  ...
  public final func SendDrirectStimuliToTarget(contextOwner: wref<GameObject>, gdStimType: gamedataStimType, target: ref<GameObject>, opt investigateData: stimInvestigateData, opt delay: Float, opt purelyDirect: Bool, opt propagationChange: Bool) -> Void {
    let broadcastEvent: ref<BroadcastEvent>;
    let newStim: ref<StimuliEvent>;
    if contextOwner == this.GetOwner() {
      newStim = StimBroadcasterComponentHelper.CreateStimEvent(this.GetOwner(), gdStimType, investigateData, propagationChange);
  ...
  private final func HasStimTarget(data: StimTargetData) -> Bool {
    let spawnerID: EntityID = Cast<EntityID>(ResolveNodeRef(data.spawnerRef, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot())));
    let i: Int32 = 0;
  ...
  private final func AddStimmTarget(data: StimTargetData) -> Void {
    ...
    stubData.spawnerID = Cast<EntityID>(ResolveNodeRef(data.spawnerRef, Cast<GlobalNodeRef>(GlobalNodeID.GetRoot())));
  ...
  private final func IsEqual(stim: ref<StimuliEvent>, gdStimType: gamedataStimType) -> Bool {
    if Equals(stim.name, EnumValueToName(n"gamedataStimType", Cast<Int64>(EnumInt(gdStimType)))) {
      return true;
    };
    return false;
  }
  ...
  private final func DetermineHowManyRepeats(request: ref<StimRequest>) -> Int32 {
    ...
    let repeats: Int32 = Cast<Int32>(duration / interval);
    return repeats;
  }
