...
public class DiodeControlComponent extends ScriptableComponent {
  ...
  private edit const let m_affectedLights: [CName];
  ...
  private final func ApplyPrimaryPreset(const preset: script_ref<DiodeLightPreset>, delay: Float, force: Bool) -> Void {
    ...
    if !this.m_secondaryPresetActive || force {
      this.ApplyPreset(Deref(preset), delay);
    };
    this.m_primaryLightPreset = Deref(preset);
  }

  private final func ApplySecondaryPreset(const preset: script_ref<DiodeLightPreset>, delay: Float, duration: Float) -> Void {
    let removeSecondaryPresetEvent: ref<RemoveSecondaryDiodeLightPresetEvent>;
    if this.m_secondaryPresetActive {
      this.GetDelaySystem().CancelDelay(this.m_secondaryPresetRemovalID);
    };
    this.ApplyPreset(Deref(preset), delay);
    this.m_secondaryLightPreset = Deref(preset);
  ...
  private final func ApplyPreset(preset: DiodeLightPreset, opt delay: Float) -> Void {
    ...
    if preset.overrideColorMin {
      this.QueueLightSettings(preset.colorMin, 1.00, 0.00, n"None", false, delay);
  ...
  private final func QueueLightSettings(const colorValues: script_ref<[Int32]>, strength: Float, time: Float, curve: CName, loop: Bool, delay: Float) -> Void {
    ...
    if delay <= 0.00 {
      ...
      changeSettingsEvent.colorValues = Deref(colorValues);
  ...
  private final func ChangeLightSettings(const colorValues: script_ref<[Int32]>, strength: Float, time: Float, curve: CName, loop: Bool) -> Void {
    ...
    while i < ArraySize(this.m_affectedLights) {
      ...
      changeSettingsEvent.settings = new ScriptLightSettings(strength, new Color(Cast<Uint8>(Deref(colorValues)[0]), Cast<Uint8>(Deref(colorValues)[1]), Cast<Uint8>(Deref(colorValues)[2]), Cast<Uint8>(Deref(colorValues)[3])));
      changeSettingsEvent.time = time;
      changeSettingsEvent.curve = curve;
      changeSettingsEvent.loop = loop;
