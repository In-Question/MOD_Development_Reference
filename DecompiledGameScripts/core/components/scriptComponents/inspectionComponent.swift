...
public class InspectionComponent extends ScriptableComponent {
  ...
  private final func PlaceItemInInspectSlot(const itemTDBIDString: script_ref<String>, offset: Float) -> Void {
    let owner: ref<PlayerPuppet> = this.GetOwner() as PlayerPuppet;
    let itemTDBID: TweakDBID = TDBID.Create(Deref(itemTDBIDString));
