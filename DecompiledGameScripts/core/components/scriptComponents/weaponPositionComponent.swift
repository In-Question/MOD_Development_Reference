...
public class WeaponPositionComponent extends ScriptableComponent {
  ...
  protected cb func OnAction(action: ListenerAction, consumer: ListenerActionConsumer) -> Bool {
    ...
    if Equals(ListenerAction.GetName(action), n"Debug_ToggleFocusMode") {
      this.m_visionSwitch = this.GetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.VisionDebug) == 1;
      if ListenerAction.IsButtonJustPressed(action) {
        if this.m_visionSwitch {
          this.SetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.VisionDebug, 0);
        } else {
          if !this.m_visionSwitch {
            this.SetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.VisionDebug, 1);
          };
        };
      };
    };
  ...
  private final func IsOwnerAiming() -> Bool {
    return this.GetBlackboardIntVariable(GetAllBlackboardDefs().PlayerStateMachine.UpperBody) == 6;
  }
