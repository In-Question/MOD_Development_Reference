...
public class DetectorModuleComponent extends ScriptableComponent {
  ...
  protected cb func OnSenseVisibilityEvent(evt: ref<SenseVisibilityEvent>) -> Bool {
    ...
    if GameInstance.GetStatsSystem(this.GetOwner().GetGame()).GetStatBoolValue(Cast<StatsObjectID>(this.GetOwner().GetEntityID()), gamedataStatType.CanShareThreatsWithPlayer) {
