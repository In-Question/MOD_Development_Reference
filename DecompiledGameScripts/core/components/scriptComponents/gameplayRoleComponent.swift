...
public class GameplayRoleComponent extends ScriptableComponent {
  ...
  @runtimeProperty("category", "Gameplay Role")
  ...
  @default(GameplayRoleComponent, EMappinVisualState.None)
  private let m_forcedMappinVisualState: EMappinVisualState;

  private let m_mappins: [SDeviceMappinData];
  ...
  private let m_enabledMinimapMappins: [gamedataMappinVariant];
    ...
    switch evt.state {
      case EUploadProgramState.STARTED:
        ...
        if Equals(evt.progressBarContext, EProgressBarContext.QuickHack) {
          ...
          if Equals(visualData.m_progressBarType, EProgressBarType.DURATION) {
            this.ActivateQuickHackDurationIndicator(visualData);
          } else {
      ...
      case EUploadProgramState.COMPLETED:
        if Equals(evt.progressBarContext, EProgressBarContext.QuickHack) {
          if Equals(visualData.statPoolType, gamedataStatPoolType.QuickHackDuration) {
            this.DeactivateQuickHackDurationIndicator();
          } else {
            if this.GetQuickHackQueueSize() == 0 {
              this.DeactivateQuickHackIndicator();
            } else {
              this.RemoveQuickhackMappinFromQueue();
            };
          };
        } else {
          if Equals(evt.progressBarContext, EProgressBarContext.PhoneCall) {
  ...
  private final func ActivateQuickHackDurationIndicator(visualData: ref<GameplayRoleMappinData>) -> Void {
    this.HideRoleMappins();
    this.ToggleMappin(gamedataMappinVariant.Zzz15_QuickHackDurationVariant, true, true, visualData);
  }

  private final func DeactivateQuickHackDurationIndicator() -> Void {
    this.ToggleMappin(gamedataMappinVariant.Zzz15_QuickHackDurationVariant, false);
    this.RequestHUDRefresh();
  }
  ...
  private final func GetQuickHackQueueSize() -> Int32 {
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if Equals(this.m_mappins[i].mappinVariant, gamedataMappinVariant.QuickHackVariant) {
        return this.m_mappins[i].visualStateData.m_action.GetDeviceActionQueueSize();
      };
      i += 1;
    };
    return 0;
  ...
  private final func DeterminGamplayRole() -> Void {
    ...
    if NotEquals(this.m_currentGameplayRole, EGameplayRole.None) && NotEquals(this.m_currentGameplayRole, EGameplayRole.UnAssigned) {
  ...
  private final func InitializeGamepleyRoleMappin() -> Void {
    if this.GetOwner().IsAttached() {
      if Equals(this.m_currentGameplayRole, EGameplayRole.UnAssigned) || Equals(this.m_currentGameplayRole, EGameplayRole.None) {
        this.m_currentGameplayRole = this.GetOwner().DeterminGameplayRole();
      };
  ...
  private final const func GetMappinDataForGamepleyRole(role: EGameplayRole) -> SDeviceMappinData {
    let mappin: SDeviceMappinData;
    if NotEquals(role, EGameplayRole.None) && NotEquals(role, EGameplayRole.UnAssigned) || !this.HasMappin(role) {
      mappin.enabled = false;
      mappin.active = false;
      mappin.range = 35.00;
  ...
  private final func GetUISystem() -> ref<UISystem> {
    return GameInstance.GetUISystem(this.GetOwner().GetGame());
  }
  ...
  private func EvaluateMappins() -> Void {
    ...
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if NotEquals(this.m_mappins[i].gameplayRole, EGameplayRole.None) && NotEquals(this.m_mappins[i].gameplayRole, EGameplayRole.UnAssigned) {
  ...
  private final func EvaluatePositions() -> Void {
    ...
    while i < ArraySize(this.m_mappins) {
      if !this.m_mappins[i].enabled {
        ...
        if direction != 0 {
          direction *= -1;
          offsetValue += this.m_offsetValue * Cast<Float>(direction);
        } else {
          if direction == 0 {
            offsetValue = 0.00;
            direction = 1;
  ...
  private final func GetNextAxis(currentAxis: EAxisType) -> EAxisType {
    ...
    if EnumInt(currentAxis) < 3 {
      axisValue += 1;
      nextAxis = IntEnum<EAxisType>(axisValue);
    } else {
      nextAxis = EAxisType.X;
    };
    return nextAxis;
  }
  ...
  public final func ShowRoleMappins() -> Void {
    ...
    if this.HasActiveMappin(gamedataMappinVariant.QuickHackVariant) || this.HasActiveMappin(gamedataMappinVariant.Zzz15_QuickHackDurationVariant) {
      return;
    };
    ...
    while i < ArraySize(this.m_mappins) {
      if !this.m_mappins[i].enabled {
        ...
        if Equals(this.m_mappins[i].gameplayRole, EGameplayRole.None) || Equals(this.m_mappins[i].gameplayRole, EGameplayRole.UnAssigned) {
        } else {
  ...
  private final func CreateRoleMappinData(const data: script_ref<SDeviceMappinData>) -> ref<GameplayRoleMappinData> {
    ...
    if NotEquals(this.m_forcedMappinVisualState, EMappinVisualState.None) {
      roleMappinData.m_mappinVisualState = this.m_forcedMappinVisualState;
    } else {
    ...
    roleMappinData.m_isBroken = this.GetOwner().GetIsBroken();
    ...
    roleMappinData.m_textureID = this.GetIconIdForMappinVariant(Deref(data).mappinVariant);
    if Equals(Deref(data).mappinVariant, gamedataMappinVariant.Zzz07_PlayerStashVariant) || Equals(Deref(data).mappinVariant, gamedataMappinVariant.Zzz08_WardrobeVariant) || Equals(Deref(data).mappinVariant, gamedataMappinVariant.Zzz19_DelamainTaxiVariant) || Equals(Deref(data).mappinVariant, gamedataMappinVariant.Zzz20_DelamainTaxiDestinationVariant) {
      showOnMiniMap = true;
    } else {
      if roleMappinData.m_isQuest && roleMappinData.m_textureID != t"MappinIcons.ShardMappin" || roleMappinData.m_isTagged {
        showOnMiniMap = true;
      } else {
        if NotEquals(Deref(data).mappinVariant, gamedataMappinVariant.LootVariant) && (roleMappinData.m_isCurrentTarget || roleMappinData.m_visibleThroughWalls) {
          showOnMiniMap = true;
        } else {
          showOnMiniMap = ArrayContains(this.m_enabledMinimapMappins, Deref(data).mappinVariant);
        };
      };
    };
    if roleMappinData.m_textureID == t"MappinIcons.ShardMappin" {
      roleMappinData.m_isShardRead = this.IsShardRead();
    };
    roleMappinData.m_showOnMiniMap = showOnMiniMap;
    return roleMappinData;
  }

  private final func IsShardRead() -> Bool {
    let isRead: Bool = false;
    let shard: ref<ShardCaseContainer> = this.GetOwner() as ShardCaseContainer;
    if IsDefined(shard) {
      isRead = shard.IsShardRead();
    };
    return isRead;
  }
  ...
  private final func CompareRoleMappinsData(data1: ref<GameplayRoleMappinData>, data2: ref<GameplayRoleMappinData>) -> Bool {
    ...
    if NotEquals(data1.m_isBroken, data2.m_isBroken) {
      return false;
    };
          ...
          if Equals(mappinVariant, gamedataMappinVariant.LootVariant) {
            if this.GetOwner().IsShardContainer() {
              ...
              if this.GetOwner().IsQuest() {
                ...
                if this.GetOwner().IsAmmoLoot() {
                  id = t"MappinIcons.LootAmmoGenericMappin";
                } else {
                  if this.GetOwner().GetIsIconic() {
                          ...
                          if Equals(mappinVariant, gamedataMappinVariant.EffectHideBodyVariant) {
                            ...
                            if Equals(mappinVariant, gamedataMappinVariant.ImportantInteractionVariant) {
                              ...
                              if Equals(mappinVariant, gamedataMappinVariant.GenericRoleVariant) {
                                ...
                                if Equals(mappinVariant, gamedataMappinVariant.Zzz07_PlayerStashVariant) {
                                  id = t"MappinIcons.PlayerStashMappin";
                                } else {
                                  if Equals(mappinVariant, gamedataMappinVariant.Zzz08_WardrobeVariant) {
  ...
  private final func ShowSingleMappin(index: Int32, visualData: ref<GameplayRoleMappinData>, opt bindPositionToSlotName: CName) -> Void {
    let currentPos: Vector4;
    ...
    if this.IsMappinDynamic() {
      if Equals(this.m_mappins[index].mappinVariant, gamedataMappinVariant.PhoneCallVariant) {
        ...
        if Equals(this.m_mappins[index].mappinVariant, gamedataMappinVariant.QuickHackVariant) {
          ...
          if Equals(this.m_mappins[index].mappinVariant, gamedataMappinVariant.Zzz15_QuickHackDurationVariant) {
            slotname = n"durationBar";
          } else {
      ...
      if IsNameValid(bindPositionToSlotName) {
        this.GetOwner().GetUISlotComponent().GetSlotTransform(bindPositionToSlotName, slotTransform);
        currentPos = WorldPosition.ToVector4(WorldTransform.TransformPoint(slotTransform, this.m_mappins[index].offset));
        this.m_mappins[index].id = this.GetMappinSystem().RegisterMappin(mappinData, currentPos);
      } else {
  ...
  private final const func HasMappin(const data: script_ref<SDeviceMappinData>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if Equals(this.m_mappins[i].mappinVariant, Deref(data).mappinVariant) && this.m_mappins[i].mappinType == Deref(data).mappinType && Equals(this.m_mappins[i].caption, Deref(data).caption) {
        return true;
      };
      i += 1;
    };
  ...
  private final func IsMappinDataValid(const mappinData: script_ref<SDeviceMappinData>) -> Bool {
    return NotEquals(Deref(mappinData).mappinVariant, gamedataMappinVariant.Invalid) && TDBID.IsValid(Deref(mappinData).mappinType);
  }
  ...
  public final func AddQuickhackMappinToQueue(visualData: ref<GameplayRoleMappinData>) -> Void {
    let evt: ref<QueueQuickHackEvent>;
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if Equals(this.m_mappins[i].mappinVariant, gamedataMappinVariant.QuickHackVariant) && this.m_mappins[i].active {
        evt = new QueueQuickHackEvent();
        evt.mappinID = this.m_mappins[i].id;
        evt.data = visualData;
        this.GetUISystem().QueueEvent(evt);
        return;
      };
      i += 1;
    };
  ...
  public final func RemoveQuickhackMappinFromQueue() -> Void {
    let evt: ref<DequeueQuickHackEvent>;
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if Equals(this.m_mappins[i].mappinVariant, gamedataMappinVariant.QuickHackVariant) && this.m_mappins[i].active {
  ...
  public final func ToggleMappin(mappinVariant: gamedataMappinVariant, enable: Bool, show: Bool, visualData: ref<GameplayRoleMappinData>, opt bindPositionToSlotName: CName) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_mappins) {
      if Equals(this.m_mappins[i].mappinVariant, mappinVariant) {
        ...
        if !enable {
          ...
          if show && !this.m_mappins[i].active {
            this.EvaluatePositions();
            this.ShowSingleMappin(i, visualData, bindPositionToSlotName);
          };
        };
      };
      i += 1;
  ...
  public final func ToggleMappin(mappinIndex: Int32, enable: Bool) -> Void {
    ...
    if !enable && this.m_mappins[mappinIndex].active {
      this.HideSingleMappin(mappinIndex);
    };
  }

  public final func AddMappin(const data: script_ref<SDeviceMappinData>) -> Bool {
    if this.IsMappinDataValid(data) && !this.HasMappin(data) {
      ArrayPush(this.m_mappins, Deref(data));
      return true;
    };
    return false;
  }
  ...
  public final const func GetForcedMappinVisualState() -> EMappinVisualState {
    return this.m_forcedMappinVisualState;
  }
  ...
  public final func EnableMappinVariantOnMinimap(mappinVariant: gamedataMappinVariant) -> Void {
    if !ArrayContains(this.m_enabledMinimapMappins, mappinVariant) {
      ArrayPush(this.m_enabledMinimapMappins, mappinVariant);
    };
  }
}
