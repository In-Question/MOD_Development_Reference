...
public struct DeviceLink {
  ...
  public final static func Construct(const persistentStates: script_ref<[ref<PersistentState>]>) -> [DeviceLink] {
    let links: array<DeviceLink>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(persistentStates)) {
      ArrayPush(links, DeviceLink.Construct(Deref(persistentStates)[i]));
      i += 1;
    };
    return links;
  }
...
public class DeviceLinkComponentPS extends SharedGameplayPS {
  ...
  private final const func Disconnect(const links: script_ref<[ref<DeviceComponentPS>]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(links)) {
      this.Disconnect(Deref(links)[i]);
      i += 1;
    };
  }
  ...
  private final const func Connect(const links: script_ref<[ref<DeviceComponentPS>]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(links)) {
      this.Connect(Deref(links)[i]);
      i += 1;
    };
  }
  ...
  protected final func OnAddToBlacklistEvent(evt: ref<AddToBlacklistEvent>) -> EntityNotificationType {
    return EntityNotificationType.SendThisEventToEntity;
  }

  protected final func OnRemoveFromBlacklistEvent(evt: ref<RemoveFromBlacklistEvent>) -> EntityNotificationType {
    return EntityNotificationType.SendThisEventToEntity;
  }
