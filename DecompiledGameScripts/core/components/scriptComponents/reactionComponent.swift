...
public class StimFilters extends IScriptable {
  ...
  public final static func AllowFriendlyInCombat(stimType: gamedataStimType) -> Bool {
    return Equals(stimType, gamedataStimType.Terror) || Equals(stimType, gamedataStimType.CombatCall) || Equals(stimType, gamedataStimType.CombatWhistle) || Equals(stimType, gamedataStimType.EnvironmentalHazard);
  }
...
public class ReactionManagerComponent extends ScriptableComponent {
  ...
  private let m_stimuliCache: [ref<StimEventTaskData>];

  private let m_reactionCache: [ref<AIReactionData>];
  ...
  private let m_ignoreList: [EntityID];

  private let m_investigationList: [StimEventData];
  ...
  private let m_environmentalHazards: [ref<StimuliEvent>];

  private let m_environmentalHazardsDelayIDs: [DelayID];
  ...
  private let m_personalities: [gamedataStatType];
  ...
  @default(ReactionManagerComponent, 20.f)
  ...
  private let m_deadBodyInvestigators: [EntityID];
  ...
  private final const func LogEnabled() -> Bool {
    return AIComponent.DebugLogEnabled(this.GetOwnerPuppet());
  }

  private final const func Log(category: CName, message: String) -> Void {
    let aiComponent: ref<AIHumanComponent> = this.GetOwnerPuppet().GetAIControllerComponent();
  ...
  private final const func LogStim(category: CName, stimType: gamedataStimType, stimPropagation: gamedataStimPropagation, message: String) -> Void {
    let logMessage: String = "(" + EnumValueToString("gamedataStimPropagation", Cast<Int64>(EnumInt(stimPropagation))) + ") ";
  ...
  public final func GetIgnoreList() -> [EntityID] {
  ...
  private final func GetRealStimSource(stimEvent: ref<StimuliEvent>) -> wref<GameObject> {
    let investigateData: stimInvestigateData;
    let source: wref<GameObject>;
    if this.IsSourceGrenade(stimEvent) {
      ...
      if StatusEffectSystem.ObjectHasStatusEffect(stimEvent.sourceObject, t"BaseStatusEffect.ImpactedThrownNPC") {
        ...
        source = investigateData.attackInstigator as PlayerPuppet;
      };
    };
    if !IsDefined(source) {
  ...
  private final func ShouldStimBeProcessed(stimEvent: ref<StimuliEvent>, delayed: Bool) -> Bool {
    ...
    if Equals(this.m_highLevelState, gamedataNPCHighLevelState.Combat) && !StimFilters.AllowFriendlyInCombat(stimType) {
      source = this.GetRealStimSource(stimEvent);
    ...
    reactionData = this.m_desiredReaction;
    if !IsDefined(reactionData) {
  ...
  private final const func GetRules() -> [wref<Rule_Record>] {
    let rules: array<wref<Rule_Record>>;
    this.m_reactionPreset.Rules(rules);
    return rules;
  }

  private final const func GetReactionOutput(stimType: gamedataStimType, const rules: script_ref<[wref<Rule_Record>]>) -> ReactionOutput {
  ...
  private final func ProcessEnvironmentalHazard(stimEvent: ref<StimuliEvent>) -> Void {
    ...
    if Equals(stimEvent.stimPropagation, gamedataStimPropagation.Visual) && !this.IsTargetInFront(stimEvent.sourceObject) {
      return;
    };
  ...
  private final func HandleCrowdReaction(stimEvent: ref<StimuliEvent>) -> Void {
    ...
    let reactionSystem: ref<ReactionSystem> = GameInstance.GetReactionSystem(game);
    let mounted: Bool = VehicleComponent.IsMountedToVehicle(game, owner);
    if this.m_inCrowd && GameInstance.GetGameFeatureManager(game).AggressiveCrowdsEnabled() && (!mounted || VehicleComponent.IsDriver(game, owner.GetEntityID())) && this.ShouldTriggerAggressiveCrowdNPCCombat(stimEvent) && (owner.IsPrevention() || reactionSystem.TryRegisteringAggressiveNPC(owner, !mounted)) {
      if mounted {
      ...
      if Equals(this.m_reactionPreset.Type(), gamedataReactionPresetType.Child) && stimEvent.IsTagInStimuli(n"ChildDanger") {
        ...
        if VehicleComponent.IsMountedToVehicle(game, owner) && NotEquals(stimEvent.GetStimType(), gamedataStimType.HijackVehicle) && NotEquals(stimEvent.GetStimType(), gamedataStimType.Dying) {
          ...
          if this.m_driverIsAggressive && Equals(stimEvent.GetStimType(), gamedataStimType.EnvironmentalHazard) {
            this.TriggerReactionBehaviorForCrowd(stimEvent, gamedataOutput.Panic, true);
          } else {
            this.m_previousFearPhase = this.ConvertFearStageToFearPhase(this.m_crowdFearStage);
  ...
  private final func ShouldFearWhileAggressiveCrowdNPCCombat(stimEvent: ref<StimuliEvent>) -> Bool {
    ...
    if Equals(stimEvent.GetStimType(), gamedataStimType.EnvironmentalHazard) {
      return true;
    };
  ...
  private final func ShouldTriggerAggressiveCrowdNPCCombat(stimEvent: ref<StimuliEvent>) -> Bool {
    ...
    let owner: ref<GameObject> = this.GetOwner();
    let isMountedToVehicle: Bool = VehicleComponent.IsMountedToVehicle(owner.GetGame(), owner);
    ...
    if !isMountedToVehicle && !this.IsTargetInFront(playerPuppet, 120.00) {
      return false;
    };
    if !isMountedToVehicle && !this.IsTargetInFront(playerPuppet, 100.00, true) {
      return false;
    };
    ...
    if !this.m_driverAllowedToGetAggressive && Equals(stimEvent.GetStimType(), gamedataStimType.HijackVehicle) && isMountedToVehicle {
      return false;
    };
  ...
  public final const func GetReactionCache() -> [ref<AIReactionData>] {
    return this.m_reactionCache;
  }

  public final const func GetStimuliCache() -> [ref<StimEventTaskData>] {
  ...
  public final func GetEnvironmentalHazards() -> [ref<StimuliEvent>] {
  ...
  private final func IsInList(const list: script_ref<[StimEventData]>, stimData: StimEventData) -> Bool {
    let i: Int32 = 0;
  ...
  private final func ReevaluateReactionPreset(opt ignoreSavedPreset: Bool) -> Void {
    if StatusEffectSystem.ObjectHasStatusEffectWithTag(this.GetOwner(), n"Braindance") || StatusEffectSystem.ObjectHasStatusEffectWithTag(this.GetOwner(), n"Drunk") {
      if (this.GetOwner() as ScriptedPuppet).IsCivilian() {
        this.MapReactionPreset("Civilian_NoReaction");
      } else {
