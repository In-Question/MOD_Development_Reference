...
public native class gameLightComponent extends LightComponent {
  ...
  private final func SetupLightSettings(const inputData: script_ref<EditableGameLightSettings>, out outputData: gameLightSettings) -> Void {
    if Deref(inputData).modifyStrength {
      outputData.strength = Deref(inputData).strength;
    };
    if Deref(inputData).modifyIntensity {
      outputData.intensity = Deref(inputData).intensity;
    };
    if Deref(inputData).modifyRadius {
      outputData.radius = Deref(inputData).radius;
    };
    if Deref(inputData).modifyColor {
      outputData.color = Deref(inputData).color;
    };
    if Deref(inputData).modifyInnerAngle {
      outputData.innerAngle = Deref(inputData).innerAngle;
    };
    if Deref(inputData).modifyOuterAngle {
      outputData.outerAngle = Deref(inputData).outerAngle;
    };
  }
  ...
  public final static func ChangeLightSettingByRefs(const lightRefs: script_ref<[ref<gameLightComponent>]>, setting: ScriptLightSettings, opt inTime: Float, opt interpolationCurve: CName, opt loop: Bool) -> Void {
    let lightSettings: gameLightSettings;
    let i: Int32 = 0;
    while i < ArraySize(Deref(lightRefs)) {
      if !IsDefined(Deref(lightRefs)[i]) {
      } else {
        lightSettings = Deref(lightRefs)[i].GetCurrentSettings();
        lightSettings.strength = setting.strength;
        lightSettings.color = setting.color;
        Deref(lightRefs)[i].SetParameters(lightSettings, inTime, interpolationCurve, loop);
      };
      i += 1;
    };
  }
