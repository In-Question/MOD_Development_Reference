...
public class gameLootContainerBasePS extends GameObjectPS {
  ...
  @runtimeProperty("category", "Quest")
  protected persistent let m_shouldHideLockedUI: Bool;

  @runtimeProperty("category", "Quest")
  protected persistent let m_started: Bool;

  public final func SetLocked(locked: Bool) -> Void {
  ...
  public final func SetStarted(started: Bool) -> Void {
  ...
  public final const func ShouldHideLockedUI() -> Bool {
    return this.m_shouldHideLockedUI;
  }
...
public native class gameLootBag extends GameObject {
  ...
  private let m_isInIconForcedVisibilityRange: Bool;
  ...
  private final const func IsEmpty() -> Bool {
...
public native class gameLootContainerBase extends GameObject {
  ...
  protected const func GetPS() -> ref<gameLootContainerBasePS> {
    return this.GetBasePS() as gameLootContainerBasePS;
  }

  public final const func ShouldHideLockedUI() -> Bool {
    return this.GetPS().ShouldHideLockedUI();
  }
  ...
  public const func IsQuest() -> Bool {
    let ps: ref<gameLootContainerBasePS> = this.GetPS();
    if IsDefined(ps) {
  ...
  protected final func ToggleLootHighlight(enable: Bool) -> Void {
    let effectInstance: ref<EffectInstance> = GameInstance.GetGameEffectSystem(this.GetGame()).CreateEffectStatic(n"loot_highlight", n"container_highlight", this);
    EffectData.SetEntity(effectInstance.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.entity, this);
    EffectData.SetBool(effectInstance.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.renderMaterialOverride, false);
    EffectData.SetBool(effectInstance.GetSharedData(), GetAllBlackboardDefs().EffectSharedData.enable, enable);
...
public native class gameContainerObjectBase extends gameLootContainerBase {
  ...
  protected cb func OnToggleContainerLockEvent(evt: ref<ToggleContainerLockEvent>) -> Bool {
    this.GetPS().SetLocked(evt.isLocked);
  ...
  protected final func RefereshInteraction(activator: ref<GameObject>, opt force: Bool) -> Void {
...
public class LootContainerObjectAnimatedByTransform extends gameContainerObjectBase {
  ...
  protected cb func OnResetContainerEvent(evt: ref<ResetContainerEvent>) -> Bool {
    let evtRegenerateLoot: ref<RegenerateLootEvent> = new RegenerateLootEvent();
    let evtTransformAnimationReset: ref<gameTransformAnimationResetEvent> = new gameTransformAnimationResetEvent();
    let setContainerStateEvent: ref<SetContainerStateEvent> = new SetContainerStateEvent();
    let ps: ref<gameLootContainerBasePS> = this.GetPS();
    this.QueueEvent(evtRegenerateLoot);
    setContainerStateEvent.isDisabled = false;
    GameInstance.GetPersistencySystem(this.GetGame()).QueuePSEvent(ps.GetID(), ps.GetClassName(), setContainerStateEvent);
    this.RegisterToHUDManagerByTask(true);
    if this.IsQuest() {
      this.ToggleLootHighlight(true);
      this.MarkAsQuest(true);
      this.ResolveQualityRangeInteractionLayer();
    };
    evtTransformAnimationReset.animationName = n"Open";
    this.QueueEventForEntityID(this.GetEntityID(), evtTransformAnimationReset);
  ...
  private final const func HasTransactionFromTweakID(itemId: TweakDBID) -> Bool {
    let transactionSystem: wref<TransactionSystem> = GameInstance.GetTransactionSystem(this.GetGame());
    let currentItemID: ItemID = ItemID.FromTDBID(itemId);
    let currentGameData: wref<gameItemData> = transactionSystem.GetItemData(this, currentItemID);
    let tweakID: TweakDBID = ItemID.GetTDBID(currentGameData.GetID());
    return tweakID != TDBID.None();
  }

  public const func IsRifleAmmoLoot() -> Bool {
    return this.HasTransactionFromTweakID(t"Ammo.RifleAmmo");
  }

  public const func IsShotgunAmmoLoot() -> Bool {
    return this.HasTransactionFromTweakID(t"Ammo.ShotgunAmmo");
  }

  public const func IsHandgunAmmoLoot() -> Bool {
    return this.HasTransactionFromTweakID(t"Ammo.HandgunAmmo");
  }

  public const func IsSniperAmmoLoot() -> Bool {
    return this.HasTransactionFromTweakID(t"Ammo.SniperRifleAmmo");
  }
}

public class LootContainerObjectAnimatedByTransformWithFlare extends LootContainerObjectAnimatedByTransform {

  public let m_colliderWithInteraction: ref<IComponent>;

  public let m_colliderWithoutInteraction: ref<IComponent>;

  public let m_lightComponent1: ref<IComponent>;

  public let m_lightComponent2: ref<IComponent>;

  protected cb func OnRequestComponents(ri: EntityRequestComponentsInterface) -> Bool {
    super.OnRequestComponents(ri);
    EntityRequestComponentsInterface.RequestComponent(ri, n"ColliderWithoutInteraction", n"entColliderComponent", false);
    EntityRequestComponentsInterface.RequestComponent(ri, n"ColliderWithInteraction", n"entColliderComponent", false);
    EntityRequestComponentsInterface.RequestComponent(ri, n"L_Flare_FX", n"entLightComponent", false);
    EntityRequestComponentsInterface.RequestComponent(ri, n"L_Flare_FX_01", n"entLightComponent", false);
  }

  protected cb func OnTakeControl(ri: EntityResolveComponentsInterface) -> Bool {
    super.OnTakeControl(ri);
    this.m_colliderWithInteraction = EntityResolveComponentsInterface.GetComponent(ri, n"ColliderWithInteraction");
    this.m_colliderWithoutInteraction = EntityResolveComponentsInterface.GetComponent(ri, n"ColliderWithoutInteraction");
    this.m_lightComponent1 = EntityResolveComponentsInterface.GetComponent(ri, n"L_Flare_FX");
    this.m_lightComponent2 = EntityResolveComponentsInterface.GetComponent(ri, n"L_Flare_FX_01");
  }

  protected cb func OnToggleContainerLockEvent(evt: ref<ToggleContainerLockEvent>) -> Bool {
    super.OnToggleContainerLockEvent(evt);
    if this.wasOpened {
      this.m_colliderWithInteraction.Toggle(true);
      this.m_colliderWithoutInteraction.Toggle(false);
      this.RequestHUDRefresh();
      this.m_lightComponent1.Toggle(false);
      this.m_lightComponent2.Toggle(false);
      GameObjectEffectHelper.BreakEffectLoopEvent(this, n"e_vfx_flare_smoke_red_1");
      GameObjectEffectHelper.BreakEffectLoopEvent(this, n"e_vfx_flare_smoke_red_2");
    };
  }
  ...
  protected cb func OnGameAttached() -> Bool {
    ...
    if !isEmpty && !isDisabled {
      isLocked = this.IsLocked(GetPlayer(this.GetGame()));
      this.m_colliderWithInteraction.Toggle(!isLocked);
      this.m_colliderWithoutInteraction.Toggle(isLocked);
      this.m_lightComponent1.Toggle(isLocked);
      this.m_lightComponent2.Toggle(isLocked);
      if isLocked {
        GameObjectEffectHelper.StartEffectEvent(this, n"e_vfx_flare_smoke_red_1");
        GameObjectEffectHelper.StartEffectEvent(this, n"e_vfx_flare_smoke_red_2");
      };
    };
  }
