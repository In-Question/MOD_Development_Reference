...
public abstract class AIActionMovePolicy extends IScriptable {
  ...
  public final static func Add(const context: ScriptExecutionContext, record: wref<MovementPolicy_Record>, out policy: ref<MovePolicies>) -> Void {
    ...
    let directionOrientationVector: Vector4;
    let distance: Float;
    let i: Int32;
      ...
      if AISquadHelper.GetSquadmates(target, squadMembers, false) {
        ...
        while i < ArraySize(squadMembers) {
          if ScriptExecutionContext.GetOwner(context) == squadMembers[i] {
            policy.SetSquadInfo(Cast<Uint32>(i), Cast<Uint32>(ArraySize(squadMembers)));
          };
          i += 1;
        };
      };
    ...
    if Equals(puppet.GetNPCType(), gamedataNPCType.Drone) {
      ...
      policy.SetMovementType(IntEnum<moveMovementType>(Cast<Int32>(EnumValueFromName(n"moveMovementType", record.MovementType()))));
    };
    if IsDefined(record.DestinationOrientationPosition()) {
      if AIActionTarget.Get(context, record.DestinationOrientationPosition(), false, destinationOrientationTarget, destinationOrientationPosition) {
        directionOrientationVector = new Vector4(destinationOrientationPosition.X, destinationOrientationPosition.Y, 0.00, 0.00) - new Vector4(targetPosition.X, targetPosition.Y, 0.00, 0.00);
    ...
    policy.SetCirclingPolicy(IntEnum<moveCirclingDirection>(Cast<Int32>(EnumValueFromName(n"moveCirclingDirection", record.CirclingDirection()))));
    policy.SetKeepLineOfSight(IntEnum<moveLineOfSight>(Cast<Int32>(EnumValueFromName(n"moveLineOfSight", record.KeepLineOfSight()))));
    policy.SetLineOfSightPointPreference(IntEnum<moveLineOfSightPointPreference>(Cast<Int32>(EnumValueFromName(n"moveLineOfSightPointPreference", record.LineOfSightPointPreference()))));
