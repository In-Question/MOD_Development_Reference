...
public class ToggleWeakspotHighlightEvent extends Event {
...
public class FocusForcedHighlightData extends IScriptable {
  ...
  public final func GetBlackwallVisionApperance() -> VisionAppearance {
    let apperance: VisionAppearance;
    apperance.patternType = this.patternType;
    if this.hudData == null {
      apperance.outline = this.GetOutlineColorIndex();
...
public final native class VisionModeComponent extends GameComponent {
  ...
  private let m_forcedHighlights: [ref<FocusForcedHighlightData>];
  ...
  private let m_activeRevealRequests: [gameVisionModeSystemRevealIdentifier];

  private let m_isFocusModeActive: Bool;

  private let m_wasCleanedUp: Bool;

  private let m_slaveObjectsToHighlight: [EntityID];
  ...
  private final func ForceVisionAppearance(data: ref<FocusForcedHighlightData>) -> Void {
    ...
    if StatusEffectSystem.ObjectHasStatusEffectWithTag(this.GetOwner(), n"BlackwallMark") {
  ...
  private final func SendRevealStateChangedEvent(state: ERevealState, reason: gameVisionModeSystemRevealIdentifier, opt onlyRevealWeakspots: Bool) -> Void {
    let evt: ref<RevealStateChangedEvent>;
    let hudManager: ref<HUDManager>;
    let request: ref<RevealStatusNotification>;
    if NotEquals(state, ERevealState.CONTINUE) && !onlyRevealWeakspots {
  ...
  protected final func ForwardHighlightToSlaveEntity(data: ref<FocusForcedHighlightData>, apply: Bool) -> Void {
    ...
    this.UpdateSlaveObjectsToHighlight(objectsToHighlight);
    if ArraySize(this.m_slaveObjectsToHighlight) <= 0 {
      return;
    };
    ...
    i = 0;
    while i < ArraySize(this.m_slaveObjectsToHighlight) {
      if EntityID.IsDefined(this.m_slaveObjectsToHighlight[i]) {
        this.GetOwner().QueueEventForEntityID(this.m_slaveObjectsToHighlight[i], evt);
      };
      i += 1;
    };
    if !apply {
      ArrayClear(this.m_slaveObjectsToHighlight);
    };
  }

  private final func UpdateSlaveObjectsToHighlight(objectsToHighlight: script_ref<[wref<GameObject>]>) -> Void {
    let currentID: EntityID;
    let i: Int32 = 0;
    while i < ArraySize(Deref(objectsToHighlight)) {
      currentID = Deref(objectsToHighlight)[i].GetEntityID();
      if !ArrayContains(this.m_slaveObjectsToHighlight, currentID) {
        ArrayPush(this.m_slaveObjectsToHighlight, currentID);
      };
      i += 1;
    };
  }
  ...
  private final func RevealObject(reveal: Bool, reason: gameVisionModeSystemRevealIdentifier, lifetime: Float, opt onlyRevealWeakspots: Bool) -> Void {
    let index: Int32;
    ...
    if reveal {
      ...
      if !this.IsRevealed() {
        this.SendRevealStateChangedEvent(ERevealState.STARTED, reason, onlyRevealWeakspots);
      } else {
        this.SendRevealStateChangedEvent(ERevealState.CONTINUE, reason, onlyRevealWeakspots);
      };
  ...
  protected final cb func OnToggleWeakspotHighlightEvent(evt: ref<ToggleWeakspotHighlightEvent>) -> Bool {
    ...
    this.RevealObject(Equals(evt.operation, EToggleOperationType.ADD), reason, 0.00, true);
  }
