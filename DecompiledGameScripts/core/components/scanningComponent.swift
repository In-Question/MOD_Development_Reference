...
public abstract class ObjectScanningDescription extends IScriptable {
  ...
  public const func GetCustomDesriptions() -> [TweakDBID] {
...
public class DeviceScanningDescription extends ObjectScanningDescription {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;device_scanning_data")
  protected persistent let DeviceGameplayDescription: TweakDBID;

  @runtimeProperty("customInnerTypeEditor", "TweakDBGroupInheritance;device_scanning_data")
  protected persistent const let DeviceCustomDescriptions: [TweakDBID];

  @runtimeProperty("customEditor", "TweakDBGroupInheritance;device_gameplay_role")
  public let DeviceGameplayRole: TweakDBID;

  @runtimeProperty("customInnerTypeEditor", "TweakDBGroupInheritance;device_role_action_desctiption")
  public let DeviceRoleActionsDescriptions: [TweakDBID];
  ...
  public final const func GetDeviceRoleActionsDescriptions() -> [TweakDBID] {
...
public class NPCScanningDescription extends ObjectScanningDescription {
  ...
  @runtimeProperty("customEditor", "TweakDBGroupInheritance;npc_scanning_data")
  protected edit persistent let NPCGameplayDescription: TweakDBID;

  @runtimeProperty("customInnerTypeEditor", "TweakDBGroupInheritance;npc_scanning_data")
  protected persistent const let NPCCustomDescriptions: [TweakDBID];
...
public native class gameScanningComponentPS extends GameComponentPS {
  ...
  private persistent let m_storedClues: [ref<CluePSData>];
  ...
  public final const func UpdateFocusClueData(id: Int32, clueData: script_ref<FocusClueDefinition>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_storedClues) {
      if id == this.m_storedClues[i].GetID() {
        Deref(clueData).isEnabled = this.m_storedClues[i].IsEnabled();
        Deref(clueData).wasInspected = this.m_storedClues[i].WasInspected();
        Deref(clueData).conclusionQuestState = this.m_storedClues[i].GetConclusionState();
        return true;
      };
      i += 1;
    };
  ...
  public final func StoreClueData(id: Int32, const clueData: script_ref<FocusClueDefinition>, isScanned: Bool) -> Void {
    let data: ref<CluePSData>;
    if !this.GetStoredClueData(id, data) {
      data = new CluePSData();
      ArrayPush(this.m_storedClues, data);
    };
    data.SetupData(id, Deref(clueData).isEnabled, Deref(clueData).wasInspected, isScanned, Deref(clueData).conclusionQuestState);
  }
...
public native class ScanningComponent extends GameComponent {
  ...
  private const let m_clues: [FocusClueDefinition];

  private inline let m_objectDescription: ref<ObjectScanningDescription>;

  @runtimeProperty("customEditor", "TweakDBGroupInheritance;device_descriptions.ScanningBarText")
  ...
  public final const func GetLinkedClueData(clueIndex: Int32, linkedClueData: script_ref<LinkedFocusClueData>) -> Bool {
    ...
    if IsNameValid(clue.clueGroupID) {
      Deref(linkedClueData).clueGroupID = clue.clueGroupID;
      Deref(linkedClueData).ownerID = this.GetOwner().GetEntityID();
      Deref(linkedClueData).clueIndex = clueIndex;
      Deref(linkedClueData).isEnabled = clue.isEnabled;
      Deref(linkedClueData).wasInspected = clue.wasInspected;
      Deref(linkedClueData).isScanned = this.IsScanned();
      Deref(linkedClueData).psData.id = this.GetPersistentID();
      Deref(linkedClueData).psData.className = n"gameScanningComponentPS";
      return true;
    };
    return false;
  }
  ...
  public final const func GetAllClues() -> [FocusClueDefinition] {
    return this.m_clues;
  }
  ...
  public final func GetScannableObjects(out arr: [ScanningTooltipElementDef]) -> Void {
    let i: Int32;
    let k: Int32;
  ...
  public final func GetScannableDataForSingleClueByIndex(index: Int32, out conclusionData: ScanningTooltipElementDef) -> [ScanningTooltipElementDef] {
  ...
  public final func GetExtendedClueRecords(clueIndex: Int32) -> [ClueRecordData] {
  ...
  public final const func HasAnyStoredClues() -> Bool {
    return this.GetMyPS().HasAnyStoredClues();
  }
  ...
  private final func UpdateLinkedClues(const linkedCluekData: script_ref<LinkedFocusClueData>, updatePS: Bool) -> Void {
    let setAsScanned: Bool;
    let i: Int32 = 0;
    while i < ArraySize(this.m_clues) {
      if !IsNameValid(this.m_clues[i].clueGroupID) {
      } else {
        if Equals(this.m_clues[i].clueGroupID, Deref(linkedCluekData).clueGroupID) {
          this.SetClueState(i, Deref(linkedCluekData).isEnabled, Deref(linkedCluekData).wasInspected, false, !updatePS);
          if Deref(linkedCluekData).isEnabled && !this.IsScanned() && Deref(linkedCluekData).isScanned {
            setAsScanned = true;
          };
        };
      };
  ...
  protected cb func OnRevealStateChanged(evt: ref<RevealStateChangedEvent>) -> Bool {
    if Equals(evt.state, ERevealState.STARTED) || Equals(evt.state, ERevealState.CONTINUE) {
      this.SetScannableThroughWallsIfPossible();
    } else {
  ...
  public final func SetScannableThroughWallsIfPossible() -> Void {
    ...
    if this.GetOwner().CanPlayerScanThroughWalls() {
      if this.GetOwner().IsDevice() || isNetRunnerNPC || !isNetRunnerNPC && StatusEffectSystem.ObjectHasStatusEffectWithTag(this.GetOwner(), n"Int60_Ping") {
