...
public native class AIComponent extends GameComponent {
  ...
  public final native func StopExecutingCommandById(cmdId: Uint32, success: Bool) -> Bool;

  public final native func CancelOrInterruptCommand(commandName: CName, useInheritance: Bool, success: Bool) -> Bool;

  public final native func GetEnqueuedOrExecutingCommand(commandName: CName, useInheritance: Bool) -> ref<AICommand>;
  ...
  public final native const func DebugLogEnabled() -> Bool;

  public final static func DebugLogEnabled(puppet: wref<ScriptedPuppet>) -> Bool {
  ...
  public final native const func GetCombatMode() -> AIbehaviorCombatModes;
...
public native class DriveToPointAutonomousUpdate extends DriveCommandUpdate {

  public native let targetPosition: Vector4;

  public native let minimumDistanceToTarget: Float;

  public native let driveDownTheRoadIndefinitely: Bool;

  public final func CreateCmd() -> ref<AIVehicleDriveToPointAutonomousCommand> {
    let cmd: ref<AIVehicleDriveToPointAutonomousCommand> = new AIVehicleDriveToPointAutonomousCommand();
    cmd.maxSpeed = this.maxSpeed;
    cmd.minSpeed = this.minSpeed;
    cmd.clearTrafficOnPath = this.clearTrafficOnPath;
    cmd.minimumDistanceToTarget = this.minimumDistanceToTarget;
    cmd.targetPosition = Vector4.Vector4To3(this.targetPosition);
    cmd.driveDownTheRoadIndefinitely = this.driveDownTheRoadIndefinitely;
    return cmd;
  }
}

public native class DrivePatrolUpdate extends DriveCommandUpdate {

  public native let numPatrolLoops: Uint32;

  public native let emergencyPatrol: Bool;

  public final func CreateCmd() -> ref<AIVehicleDrivePatrolCommand> {
    let cmd: ref<AIVehicleDrivePatrolCommand> = new AIVehicleDrivePatrolCommand();
    cmd.maxSpeed = this.maxSpeed;
    cmd.minSpeed = this.minSpeed;
    cmd.clearTrafficOnPath = this.clearTrafficOnPath;
    cmd.numPatrolLoops = this.numPatrolLoops;
    cmd.emergencyPatrol = this.emergencyPatrol;
    return cmd;
  }
}
...
public native class AIVehicleAgent extends AIComponent {
  ...
  private let m_keepStrategyOnSearch: Bool;

  private let m_initCmd: ref<AIVehicleCommand>;

  public final native func SetDriveToPointAutonomousUpdate(update: ref<DriveToPointAutonomousUpdate>) -> Void;

  public final native func SetDrivePatrolUpdate(update: ref<DrivePatrolUpdate>) -> Void;

  public final native func TryReserveSeatOrFirstAvailable(reserver: EntityID, preferredSeatName: CName) -> CName;

  public final native func ReleaseSeat(seatName: CName) -> Void;

  public final native func ReleaseSeatReservedBy(reserver: EntityID) -> Void;

  public final native const func IsSeatReserved(seatName: CName) -> Bool;

  public final native const func GetReservedSeatsCount() -> Uint32;

  public final const func KeepStrategyOnSearch() -> Bool {
    return this.m_keepStrategyOnSearch;
  }

  public final func SetKeepStrategyOnSearch(keep: Bool) -> Void {
    this.m_keepStrategyOnSearch = keep;
  }

  public final func GetInitCmd() -> ref<AIVehicleCommand> {
    return this.m_initCmd;
  }

  public final func SetInitCmd(cmd: ref<AIVehicleCommand>) -> Void {
    this.m_initCmd = cmd;
  }

  public final const func InitCommandIsA(commandClassName: CName) -> Bool {
    return IsDefined(this.m_initCmd) && this.m_initCmd.IsA(commandClassName);
  }
}
...
public native class AIHumanComponent extends AIComponent {
  ...
  private let m_prereqsBlackboard: wref<IBlackboard>;
  ...
  private let m_lastReservedSeatVehicle: EntityID;

  private let m_assignedVehicleStuck: Bool;
  ...
  public final native func WasForcedToEnterCrowd() -> Bool;
  ...
  private final func OnAttach() -> Void {
    ...
    if IsDefined(spawnAIRole) && NotEquals(spawnAIRole.GetRoleEnum(), EAIRole.None) {
  ...
  public final func GetAIPrereqsBlackboard() -> ref<IBlackboard> {
    return this.GetOrCreateBlackboard(GetAllBlackboardDefs().AIPrereqs, this.m_prereqsBlackboard);
  }
  ...
  public final native func OverrideAssignedVehicleInfoForPreventionBlockade() -> Void;

  protected cb func OnVehicleAssign(evt: ref<MountAIEvent>) -> Bool {
    if Equals(evt.name, n"Mount") {
      this.m_assignedVehicleStuck = false;
  ...
  protected cb func OnStuckEvent(evt: ref<VehicleStuckEvent>) -> Bool {
    this.m_assignedVehicleStuck = true;
  }

  public final const func IsAssignedVehicleStuck() -> Bool {
    return this.m_assignedVehicleStuck;
  }
  ...
  public final func GetVehicleHandle(entityID: EntityID, out vehicle: ref<VehicleObject>) -> Bool {
    if !EntityID.IsDefined(entityID) {
      return false;
    };
    vehicle = GameInstance.FindEntityByID(this.GetGame(), entityID) as VehicleObject;
    return vehicle != null;
  }

  protected cb func OnReserveAssignedSeat(evt: ref<ReserveAssignedSeat>) -> Bool {
    let assignedVehicleID: EntityID;
    let slotName: CName;
    let vehicle: ref<VehicleObject>;
    let vehicleSlot: MountingSlotId;
    let ownerEntityID: EntityID = this.GetEntity().GetEntityID();
    this.OverrideAssignedVehicleInfoForPreventionBlockade();
    vehicleSlot = this.GetAssignedVehicleSlot();
    assignedVehicleID = this.GetAssignedVehicleId();
    slotName = vehicleSlot.id;
    if !GameInstance.GetMountingFacility(this.GetGame()).IsMountedToAnything(ownerEntityID) {
      if EntityID.IsDefined(assignedVehicleID) && IsNameValid(slotName) && this.GetVehicleHandle(assignedVehicleID, vehicle) {
        slotName = vehicle.GetAIComponent().TryReserveSeatOrFirstAvailable(ownerEntityID, slotName);
        if IsNameValid(slotName) {
          this.OnSeatReserved(assignedVehicleID);
        };
      };
    };
  }

  public final func ReleaseReservedSeat() -> Void {
    let vehicle: ref<VehicleObject>;
    if this.GetVehicleHandle(this.m_lastReservedSeatVehicle, vehicle) {
      vehicle.GetAIComponent().ReleaseSeatReservedBy(this.GetEntity().GetEntityID());
    };
  }

  public final func OnSeatReserved(vehicleID: EntityID) -> Void {
    if this.m_lastReservedSeatVehicle == vehicleID {
      return;
    };
    this.ReleaseReservedSeat();
    this.m_lastReservedSeatVehicle = vehicleID;
  }
  ...
  private final func OnAIRoleChanged(newRole: ref<AIRole>, oldRole: ref<AIRole>) -> Void {
    ...
    if IsDefined(oldRole) && NotEquals(oldRole.GetRoleEnum(), EAIRole.None) {
    ...
    if IsDefined(newRole) && NotEquals(newRole.GetRoleEnum(), EAIRole.None) {
  ...
  public final const func IsPlayerCompanion() -> Bool {
    ...
    friendlyTarget = FromVariant<wref<GameObject>>(this.GetBehaviorArgument(n"FriendlyTarget"));
    if !IsDefined(friendlyTarget) || !friendlyTarget.IsAttached() {
      return false;
    };
  ...
  private final func FriendlyFireTargetUpdateInit() -> Void {
    ...
    let target: wref<GameObject> = FromVariant<wref<GameObject>>(this.GetBehaviorArgument(n"CombatTarget"));
  ...
  private final func LoSFinderCheckInit() -> Bool {
    ...
    puppet = this.GetEntity() as ScriptedPuppet;
    targetGO = FromVariant<wref<GameObject>>(this.GetBehaviorArgument(n"CombatTarget"));
    ...
    if this.m_LoSFinderCheckID == 0u || this.m_LoSFinderVisibleObject != visibleObject {
      ...
      params.SetSeeker(puppet, GetIgnoredVisionBlockerTypes(this.GetGame(), Cast<StatsObjectID>(puppet.GetEntityID())));
      params.SetTarget(visibleObject, GetIgnoredVisionBlockerTypes(this.GetGame(), Cast<StatsObjectID>(target.GetEntityID())));
  ...
  public final func GetReachedLoSPosition(ownerId: EntityID, entityPos: Vector4, losMode: Int32, radiusXY: Float, radiusZ: Float, maxNotFoundTime: Float) -> Bool {
    let currentSearchTime: Float;
    let losPosition: Vector3;
    if !IsDefined(this.m_loSFinderSystem) || Equals(this.m_loSFinderSystem.GetLoSMode(ownerId), gameLoSMode.Invalid) {
      return true;
    };
    if radiusXY <= 0.00 {
      return true;
    };
    if !this.m_loSFinderSystem.GetLoSPosition(ownerId, losPosition, currentSearchTime) {
      if currentSearchTime <= 0.00 && -currentSearchTime > maxNotFoundTime {
        return true;
      };
      return false;
    };
  ...
  public final const func CheckTweakCondition(const ActionConditionName: script_ref<String>) -> Bool {
    ...
    TDBRecord = ScriptExecutionContext.CreateActionID(this.m_scriptContext, Deref(ActionConditionName), AIScriptActionDelegate.GetActionPackageType(this.m_scriptContext));
  ...
  public final func TryBulletDodgeOpportunity() -> Bool {
    ...
    let owner: ref<ScriptedPuppet> = this.GetEntity() as ScriptedPuppet;
    if IsDefined(owner) && AIActionHelper.CheckAbility(owner, this.m_kerenzikovAbilityRecord) && this.CheckTweakCondition("DodgeBulletSelectorCondition") {
  ...
  public final func OnCommandStateChanged(command: ref<AICommand>, oldState: AICommandState, newState: AICommandState) -> Void {
    if Equals(newState, AICommandState.Executing) || Equals(newState, AICommandState.Enqueued) || Equals(newState, AICommandState.NotExecuting) {
      ...
      if command as AIMoveCommand.ignoreInCombat {
        this.GetActionBlackboard().SetBool(GetAllBlackboardDefs().AIAction.ignoreInCombatMoveCommand, true);
      };
    } else {
      AIActiveCommandList.Remove(this.m_activeCommands, command.GetClassName());
      if command as AIMoveCommand.ignoreInCombat {
        this.GetActionBlackboard().SetBool(GetAllBlackboardDefs().AIAction.ignoreInCombatMoveCommand, false);
      };
    };
  ...
  private final func TrackActionCommandID(commandClassNames: script_ref<[CName]>) -> Void {
    let i: Int32 = 0;
  ...
  private final func ClearActionCommandID(commandClassNames: script_ref<[CName]>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(Deref(commandClassNames)) {
