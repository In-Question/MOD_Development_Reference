...
public class DropPointRequest extends ScriptableSystemRequest {
  ...
  @runtimeProperty("Tooltip", "NOT_ACTIVE = DropPoints will not accept them | ACTIVE = drop points will allow to deposit them | COLLECTED = treated as if it was already deposited")
...
public class ToggleDropPointSystemRequest extends ScriptableSystemRequest {
  ...
  @default(ToggleDropPointSystemRequest, quest)
...
public class DropPointPackage extends IScriptable {
  ...
  @runtimeProperty("Tooltip", "NOT_ACTIVE = DropPoints will not accept them | ACTIVE = drop points will allow to deposit them | EXPOSED = treated as if it was already deposited")
  private persistent let status: DropPointPackageStatus;

  private persistent let predefinedDrop: PersistentID;

  private let statusHistory: [DropPointPackageStatus];
...
public class DropPointSystem extends ScriptableSystem {
  ...
  private persistent let m_packages: [ref<DropPointPackage>];

  private persistent let m_mappins: [ref<DropPointMappinRegistrationData>];

  @default(DropPointSystem, true)
  private persistent let m_isEnabled: Bool;

  private persistent let m_dropPointSystemLocks: [DropPointSystemLock];
  ...
  protected final const func HasMeaningfulItems(const items: script_ref<[wref<gameItemData>]>, dropPoint: PersistentID) -> Bool {
    let k: Int32;
    let i: Int32 = 0;
    while i < ArraySize(this.m_packages) {
      k = 0;
      while k < ArraySize(Deref(items)) {
        if this.m_packages[i].Record() == ItemID.GetTDBID(Deref(items)[k].GetID()) && Equals(this.m_packages[i].Status(), DropPointPackageStatus.ACTIVE) {
          if !PersistentID.IsDefined(this.m_packages[i].Holder()) || Equals(this.m_packages[i].Holder(), dropPoint) {
            return true;
          };
        };
  ...
  protected final func OnToggleDropPointSystemRequest(request: ref<ToggleDropPointSystemRequest>) -> Void {
    ...
    if !request.isEnabled {
      this.AddDropPointSystemLock(request.reason);
    } else {
      this.RemoveDropPointSystemLock(request.reason);
    };
    newState = ArraySize(this.m_dropPointSystemLocks) <= 0;
    shouldUpdate = NotEquals(this.m_isEnabled, newState);
    this.m_isEnabled = newState;
    if shouldUpdate {
      if this.m_isEnabled {
  ...
  private final func AddDropPointSystemLock(reason: CName) -> Void {
    let i: Int32;
    let newLock: DropPointSystemLock;
    if IsNameValid(reason) {
      i = 0;
      while i < ArraySize(this.m_dropPointSystemLocks) {
        if Equals(this.m_dropPointSystemLocks[i].lockReason, reason) {
          return;
        };
        i += 1;
      };
      newLock.lockReason = reason;
      ArrayPush(this.m_dropPointSystemLocks, newLock);
    };
  }

  private final func RemoveDropPointSystemLock(reason: CName) -> Void {
    let i: Int32;
    if IsNameValid(reason) {
      i = 0;
      while i < ArraySize(this.m_dropPointSystemLocks) {
        if Equals(this.m_dropPointSystemLocks[i].lockReason, reason) {
          ArrayErase(this.m_dropPointSystemLocks, i);
          return;
        };
        i += 1;
      };
  ...
  protected final func OnRegisterDropPointMappinRequest(request: ref<RegisterDropPointMappinRequest>) -> Void {
    ...
    if this.m_isEnabled && Equals(registrationData.GetMappinID(), invalidMappinID) {
      this.RegisterDropPointMappin(registrationData);
    };
    if NotEquals(request.trackingAlternativeMappinID, invalidMappinID) && Equals(registrationData.GetTrackingAlternativeMappinID(), invalidMappinID) {
  ...
  protected final func OnUnregisterDropPointMappinRequest(request: ref<UnregisterDropPointMappinRequest>) -> Void {
    ...
    if request.removeFromSystem {
      this.RemoveMappinData(request.ownerID);
    } else {
  ...
  private final func RemoveMappinData(ownerID: EntityID) -> Void {
    let i: Int32 = ArraySize(this.m_mappins) - 1;
    while i >= 0 {
      if this.m_mappins[i].GetOwnerID() == ownerID {
        this.UnregisterDropPointMappin(this.m_mappins[i]);
        this.m_mappins[i] = null;
