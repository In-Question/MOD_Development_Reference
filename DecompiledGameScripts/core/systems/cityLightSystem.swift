...
public class TimetableCallbackData extends IScriptable {
  ...
  private persistent let m_recipients: [RecipientData];
  ...
  public final const func GetRecipients() -> [RecipientData] {
...
public class CityLightSystem extends ScriptableSystem {
  ...
  private persistent let m_timeSystemCallbacks: [ref<TimetableCallbackData>];

  private persistent let m_fuses: [FuseData];
  ...
  private persistent let m_forcedStatesStack: [ForcedStateData];
  ...
  private final func SendNotificationToRecipients(const recipients: script_ref<[RecipientData]>, time: GameTime) -> Void {
    let excessRecipients: array<RecipientData>;
    let totalNotifications: Int32;
    let i: Int32 = 0;
    while i < ArraySize(Deref(recipients)) {
      if totalNotifications >= 50 {
        ArrayPush(excessRecipients, Deref(recipients)[i]);
      } else {
        if this.ShouldNotifyRecipient(Deref(recipients)[i], time) {
          this.SendDeviceTimetableEvent(Deref(recipients)[i]);
  ...
  private final func SendNotificationByRequest(const recipients: script_ref<[RecipientData]>, time: GameTime) -> Void {
    let request: ref<NotifyRecipientsRequest> = new NotifyRecipientsRequest();
    request.recipients = Deref(recipients);
    request.time = time;
  ...
  private final static func GetMaxNotificationsPerFrame() -> Int32 {
    return 50;
  }
  ...
  private final func SendForceStateDeviceTimetableEvent(const fuse: script_ref<FuseData>, state: ECLSForcedState) -> Void {
    ...
    evt.requesterID = PersistentID.ExtractEntityID(Deref(fuse).psOwnerData.id);
    if EntityID.IsDefined(evt.requesterID) {
      GameInstance.GetPersistencySystem(this.GetGameInstance()).QueuePSEvent(Deref(fuse).psOwnerData.id, Deref(fuse).psOwnerData.className, evt);
    };
  }

  private final func AddTimeTableCallbacks(requesterData: PSOwnerData, const timeTable: script_ref<[SDeviceTimetableEntry]>, opt lights: Int32) -> Void {
    ...
    let i: Int32 = 0;
    while i < ArraySize(Deref(timeTable)) {
      callbackData = this.GetTimeTableCallback(Deref(timeTable)[i].time);
      recipientData.fuseID = fuseID;
      recipientData.entryID = i;
      if callbackData == null {
        callbackData = new TimetableCallbackData();
        callbackData.Initialize(Deref(timeTable)[i].time, recipientData);
  ...
  private final func AddFuse(requesterData: PSOwnerData, const timeTable: script_ref<[SDeviceTimetableEntry]>, opt lights: Int32) -> Int32 {
    let fuse: FuseData;
    let id: Int32;
    if !this.HasFuse(requesterData, id) {
      fuse.psOwnerData = requesterData;
      fuse.timeTable = Deref(timeTable);
      fuse.lights = lights;
  ...
  private final const func GetFuse(fuseID: Int32, fuseData: script_ref<FuseData>) -> Bool {
  ...
  private final const func GetActiveTimeTableEntry(const fuse: script_ref<FuseData>, out entry: SDeviceTimetableEntry) -> Bool {
    let entryTime: GameTime;
    ...
    let id: Int32 = -1;
    let i: Int32 = 0;
    while i < ArraySize(Deref(fuse).timeTable) {
      entryTime = GameTime.MakeGameTime(0, Deref(fuse).timeTable[i].time.hours, Deref(fuse).timeTable[i].time.minutes, Deref(fuse).timeTable[i].time.seconds);
  ...
  private final const func GetActiveTimeTableEntryID(const fuse: script_ref<FuseData>) -> Int32 {
    let entryTime: GameTime;
    let storedtime: GameTime;
    let time: GameTime;
    let wasTimeStored: Bool;
  ...
  public final const func GetCallbacks() -> [ref<TimetableCallbackData>] {
  ...
  private final func ShowDebug_fuses(const fuses: script_ref<[FuseData]>) -> Void {
    ...
    i = 0;
    while i < ArraySize(Deref(fuses)) {
      globalIDString = PersistentID.ToDebugString(Deref(fuses)[i].psOwnerData.id);
      if !PersistentID.IsDefined(Deref(fuses)[i].psOwnerData.id) {
      ...
      SDOSink.PushString(sink, "connectedLights", ToString(Deref(fuses)[i].lights));
      k = 0;
      while k < ArraySize(Deref(fuses)[i].timeTable) {
        activeEntry = Deref(fuses)[i].timeTable[k];
        ...
        if this.GetActiveTimeTableEntryID(Deref(fuses)[i]) == k {
          entryString += " [ACTIVE]";
        };
