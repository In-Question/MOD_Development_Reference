...
public class IconsModule extends HUDModule {
  ...
  private final const func IsEnemyGrappled() -> Bool {
    return this.m_hud.GetPlayerSMBlackboard().GetInt(GetAllBlackboardDefs().PlayerStateMachine.Takedown) == 2;
  }
  ...
  public func Suppress(jobs: script_ref<[HUDJob]>) -> Void {
    ...
    while i < ArraySize(Deref(jobs)) {
      instruction = Deref(jobs)[i].instruction.iconsInstruction;
  ...
  protected func DuplicateLastInstance(actor: ref<HUDActor>) -> ref<IconsInstance> {
    return super.DuplicateLastInstance(actor) as IconsInstance;
  }
}
