...
public class QuickhackModule extends HUDModule {
  ...
  protected func Process(out task: HUDJob, mode: ActiveMode) -> Void {
    ...
    if IsDefined(this.m_hud.GetCurrentTarget()) && (Equals(this.m_hud.GetCurrentTarget().GetType(), HUDActorType.VEHICLE) || Equals(this.m_hud.GetCurrentTarget().GetType(), HUDActorType.DEVICE) || Equals(this.m_hud.GetCurrentTarget().GetType(), HUDActorType.BODY_DISPOSAL_DEVICE) || Equals(this.m_hud.GetCurrentTarget().GetType(), HUDActorType.PUPPET)) {
  ...
  public func Suppress(jobs: script_ref<[HUDJob]>) -> Void {
    ...
    while i < ArraySize(Deref(jobs)) {
      instruction = Deref(jobs)[i].instruction.quickhackInstruction;
      instruction.SetState(InstanceState.DISABLED, this.DuplicateLastInstance(Deref(jobs)[i].actor));
      i += 1;
    };
  }

  protected func DuplicateLastInstance(actor: ref<HUDActor>) -> ref<QuickhackInstance> {
    return super.DuplicateLastInstance(actor) as QuickhackInstance;
  }
  ...
  public final static func IsQuickhackBlockedByScene(player: ref<GameObject>) -> Bool {
    ...
    if StatusEffectSystem.ObjectHasStatusEffectWithTag(player, n"NoQuickHacks") || tier >= 3 && tier <= 5 {
      return true;
    };
    return false;
  }
  ...
  public final static func TranslateEmptyQuickSlotCommands(context: GameInstance) -> [ref<QuickhackData>] {
  ...
  private final func SendFakeCommands(const commands: script_ref<[ref<QuickhackData>]>, shouldReveal: Bool) -> Void {
    ...
    quickSlotsManagerNotification.commands = Deref(commands);
    this.m_hud.QueueEntityEvent(playerRef.GetEntityID(), quickSlotsManagerNotification);
  }

  public final static func CheckCommandDuplicates(commands: script_ref<[ref<QuickhackData>]>, opt characterRecord: wref<Character_Record>) -> Void {
    ...
    while i >= 0 {
      remove = false;
      i1 = 0;
      while i1 < ArraySize(Deref(commands)) {
        if i <= i1 {
          break;
        };
        if Equals(Deref(commands)[i].m_title, Deref(commands)[i1].m_title) {
          remove = true;
          break;
        };
        i1 += 1;
      ...
      if remove {
        if IsDefined(characterRecord) {
          if IsDefined(Deref(commands)[i].m_action) {
            if characterRecord.ObjectActionsContains(Deref(commands)[i].m_action.GetObjectActionRecord()) {
              ArrayPush(removeIndexes, i1);
            } else {
              if IsDefined(Deref(commands)[i1].m_action) {
                actionRectord = Deref(commands)[i1].m_action.GetObjectActionRecord();
              ...
              if ArrayContains(removeIndexes, i) {
                ...
                if ArrayContains(removeIndexes, i1) {
                  ...
                  if Deref(commands)[i].m_actionMatchesTarget {
                    ArrayPush(removeIndexes, i1);
                  } else {
                    ArrayPush(removeIndexes, i);
                  };
    ...
    while i < ArraySize(removeIndexes) {
      Deref(commands)[removeIndexes[i]] = null;
      i += 1;
    };
    i = ArraySize(Deref(commands)) - 1;
    while i >= 0 {
      if !IsDefined(Deref(commands)[i]) {
        ArrayErase(Deref(commands), i);
      };
      i -= 1;
    };
  }

  public final static func SortCommandPriority(commands: script_ref<[ref<QuickhackData>]>, context: GameInstance) -> Void {
    let actionTDB: TweakDBID;
    let sortedArray: array<ref<QuickhackData>>;
    let activeIndex: Int32 = -1;
    let i: Int32 = ArraySize(Deref(commands)) - 1;
    while i >= 0 {
      if !Deref(commands)[i].m_isLocked || Deref(commands)[i].m_actionMatchesTarget {
        actionTDB = Deref(commands)[i].m_action.GetObjectActionID();
        if actionTDB == t"QuickHack.RemoteBreach" || actionTDB == t"DeviceAction.RemoteBreach" || actionTDB == t"QuickHack.ICEBreakerHack" {
          ArrayInsert(sortedArray, 0, Deref(commands)[i]);
        } else {
          ArrayPush(sortedArray, Deref(commands)[i]);
        };
        ArrayErase(Deref(commands), i);
        activeIndex += 1;
      };
    ...
    if !NetworkSystem.ShouldShowOnlyTargetQuickHacks(context) {
      ...
      while i < ArraySize(Deref(commands)) {
        if Deref(commands)[i].m_isLocked {
          ArrayPush(sortedArray, Deref(commands)[i]);
        };
        i += 1;
      };
