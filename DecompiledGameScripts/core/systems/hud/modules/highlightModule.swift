...
public class HighlightModule extends HUDModule {
  ...
  public func Suppress(jobs: script_ref<[HUDJob]>) -> Void {
    ...
    while i < ArraySize(Deref(jobs)) {
      instruction = Deref(jobs)[i].instruction.highlightInstructions;
  ...
  protected func DuplicateLastInstance(actor: ref<HUDActor>) -> ref<HighlightInstance> {
    return super.DuplicateLastInstance(actor) as HighlightInstance;
  }
}
