...
public class HUDManager extends NativeHudManager {
  ...
  private let m_modulesArray: [ref<HUDModule>];
  ...
  @default(HUDManager, false)
  private let m_uiQuickHackKeepContext: Bool;
  ...
  private let m_uiQuickhackKeepContextCallbackID: ref<CallbackHandle>;
  ...
  protected final func OnRegisterAssociation(request: ref<HUDManagerAssociationRequest>) -> Void {
    if this.IsRequestLegal(request) {
      this.ProcessAssociationRegistration(request);
    };
  }
  ...
  protected final func OnVisionModeResetRequest(request: ref<VisionModeResetRequest>) -> Void {
    GameInstance.GetBlackboardSystem(this.GetPlayer().GetGame()).Get(GetAllBlackboardDefs().PlayerStateMachine).SetInt(GetAllBlackboardDefs().PlayerStateMachine.Vision, 0);
    this.OnVisionModeChanged(0);
  }
  ...
  protected cb func OnVisionModeChanged(value: Int32) -> Bool {
    ...
    visionType = IntEnum<gameVisionModeType>(value);
  ...
  protected cb func OnAction(action: ListenerAction, consumer: ListenerActionConsumer) -> Bool {
    ...
    inputSchemesBB = GameInstance.GetBlackboardSystem(this.GetGameInstance()).Get(GetAllBlackboardDefs().InputSchemes);
    ...
    if ListenerAction.IsButtonJustPressed(action) {
      switch actionName {
        case n"Ping":
          if inputSchemeNum != 2u {
            this.StartPulse();
          };
          break;
        case n"DescriptionChange":
          this.SendScannerTabChangeEvent();
          break;
        default:
      };
    };
  ...
  private final func SendScannerTabChangeEvent() -> Void {
    let evt: ref<ScannerTabChangeEvent> = new ScannerTabChangeEvent();
    this.GetPlayer().QueueEvent(evt);
  }
  ...
  protected cb func OnNameplateChanged(value: Variant) -> Bool {
    ...
    request.playerTarget = FromVariant<EntityID>(value);
    this.QueueRequest(request);
  }
  ...
  private final func ProcessAssociationRegistration(request: ref<HUDManagerAssociationRequest>) -> Void {
    if request.isRegistering {
      this.RegisterAssociatedActor_Script(request);
    } else {
      this.UnregisterAssociatedActor_Script(request);
    };
  }

  private final func RegisterAssociatedActor_Script(request: ref<HUDManagerAssociationRequest>) -> Void {
    this.RegisterAssociatedActor(request.ownerID, request.associatedID);
  }

  private final func UnregisterAssociatedActor_Script(request: ref<HUDManagerAssociationRequest>) -> Void {
    this.UnregisterAssociatedActor(request.ownerID);
  }
  ...
  private final func RefreshHudForSingleActor(actor: ref<HUDActor>, opt targetModules: [wref<HUDModule>]) -> Void {
    let actors: array<ref<HUDActor>>;
    let i: Int32;
    let jobs: array<HUDJob>;
  ...
  private final func CreateJobsForClueActors(const actors: script_ref<[ref<HUDActor>]>) -> [HUDJob] {
    let jobs: array<HUDJob>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(actors)) {
      if IsDefined(Deref(actors)[i]) && Deref(actors)[i].IsClue() {
        ArrayPush(jobs, this.CreateJob(Deref(actors)[i]));
      };
      i += 1;
    };
    return jobs;
  ...
  private final func CreateJobsByActorType(const actors: script_ref<[ref<HUDActor>]>, type: HUDActorType) -> [HUDJob] {
    let jobs: array<HUDJob>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(actors)) {
      if IsDefined(Deref(actors)[i]) && Equals(Deref(actors)[i].GetType(), type) {
        ArrayPush(jobs, this.CreateJob(Deref(actors)[i]));
      };
      i += 1;
    };
  ...
  private final func CreateJobs(const actors: script_ref<[ref<HUDActor>]>) -> [HUDJob] {
    let jobs: array<HUDJob>;
    let i: Int32 = 0;
    while i < ArraySize(Deref(actors)) {
      if IsDefined(Deref(actors)[i]) {
        ArrayPush(jobs, this.CreateJob(Deref(actors)[i]));
      };
      i += 1;
    };
  ...
  private final func IterateModules(jobs: [HUDJob]) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_modulesArray) {
      this.m_modulesArray[i].Iterate(jobs);
      i += 1;
  ...
  private final func SendInstructions(const jobs: script_ref<[HUDJob]>) -> Void {
    let excessJobsPackage: array<HUDJob>;
    let totalInstructions: Int32;
    let i: Int32 = 0;
    while i < ArraySize(Deref(jobs)) {
      if totalInstructions >= 50 {
        ArrayPush(excessJobsPackage, Deref(jobs)[i]);
      } else {
        this.SendSingleInstruction(Deref(jobs)[i].actor.GetEntityID(), Deref(jobs)[i].instruction);
      };
      totalInstructions += 1;
      i += 1;
    };
  ...
  private final func PostponeModuleIteration(const remainingJobs: script_ref<[HUDJob]>) -> Void {
    let request: ref<IterateModulesRequest> = new IterateModulesRequest();
    request.remainingJobs = Deref(remainingJobs);
  ...
  private final func SendInstructionsByRequest(const jobs: script_ref<[HUDJob]>) -> Void {
    let request: ref<SendInstructionRequest> = new SendInstructionRequest();
    request.jobs = Deref(jobs);
  ...
  private final static func GetMaxInstructionsPerFrame() -> Int32 {
    return 50;
  }
  ...
  protected final func RegisterUICallbacks() -> Void {
    ...
    blackboard = GameInstance.GetBlackboardSystem(this.GetGameInstance()).Get(GetAllBlackboardDefs().UI_QuickSlotsData);
    if IsDefined(blackboard) && !IsDefined(this.m_uiQuickhackKeepContextCallbackID) {
      this.m_uiQuickhackKeepContextCallbackID = blackboard.RegisterListenerBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelKeepContext, this, n"OnQuickHackUIKeepContextChanged");
    };
  ...
  protected final func UnregisterUICallbacks() -> Void {
    ...
    blackboard = GameInstance.GetBlackboardSystem(this.GetGameInstance()).Get(GetAllBlackboardDefs().UI_QuickSlotsData);
    if blackboard != null && IsDefined(this.m_uiQuickhackKeepContextCallbackID) {
      blackboard.UnregisterListenerBool(GetAllBlackboardDefs().UI_QuickSlotsData.quickhackPanelKeepContext, this.m_uiQuickhackKeepContextCallbackID);
    };
    blackboard = GameInstance.GetBlackboardSystem(this.GetGameInstance()).Get(GetAllBlackboardDefs().UIInteractions);
  ...
  protected cb func OnScannerUIVisibleChanged(visible: Bool) -> Bool {
    ...
    if !this.m_uiQuickHackKeepContext && this.m_uiQuickHackVisible {
      return true;
    };
    if visible {
  ...
  protected cb func OnQuickHackUIVisibleChanged(visible: Bool) -> Bool {
    ...
    if this.m_uiQuickHackKeepContext {
      return true;
    };
    if visible {
  ...
  protected cb func OnQuickHackUIKeepContextChanged(visible: Bool) -> Bool {
    let uiSystem: ref<UISystem> = GameInstance.GetUISystem(this.GetGameInstance());
    if uiSystem == null || Equals(this.m_uiQuickHackKeepContext, visible) {
      return true;
    };
    this.m_uiQuickHackKeepContext = visible;
    if visible {
      ...
      if this.m_uiScannerVisible && this.m_uiQuickHackVisible {
        uiSystem.SwapGameContext(UIGameContext.Scanning, UIGameContext.QuickHack);
      };
    };
  }
  ...
  private final func HUDLog(const message: script_ref<String>) -> Void;
  ...
  public final static func SetScannerHintMessege(game: GameInstance, const text: script_ref<String>) -> Void {
    let blackboard: ref<IBlackboard>;
    if GameInstance.IsValid(game) {
      blackboard = GameInstance.GetBlackboardSystem(game).Get(GetAllBlackboardDefs().HUD_Manager);
      blackboard.SetString(GetAllBlackboardDefs().HUD_Manager.HudHintMessegeContent, Deref(text));
    };
  }
  ...
  public final const func CanPulse() -> Bool {
    let player: ref<GameObject> = this.GetPlayer();
    let statValue: Float = GameInstance.GetStatsSystem(this.GetGameInstance()).GetStatValue(Cast<StatsObjectID>(player.GetEntityID()), gamedataStatType.HasCybereye);
