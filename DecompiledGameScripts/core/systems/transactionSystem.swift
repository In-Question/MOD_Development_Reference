...
public abstract class AIActionTransactionSystem extends IScriptable {
  ...
  public final static func ChooseSingleItemsSetFromPool(powerLevel: Int32, seed: Uint32, itemPool: wref<NPCEquipmentItemPool_Record>, out outItems: [wref<NPCEquipmentItem_Record>]) -> Void {
    let randomVal: Float;
    ...
    randomVal = RandNoiseF(Cast<Int32>(seed), 0.00, weightSum);
    i = 0;
    while i < poolSize {
      tempPoolEntry = itemPool.GetPoolItem(i);
      if powerLevel >= tempPoolEntry.MinLevel() {
  ...
  public final static func CalculateEquipmentItems(const puppet: wref<ScriptedPuppet>, const equipmentGroupName: CName, out items: [wref<NPCEquipmentItem_Record>], opt powerLevel: Int32) -> Void {
  ...
  public final static func CalculateEquipmentItems(const puppet: wref<ScriptedPuppet>, equipmentGroupRecord: wref<NPCEquipmentGroup_Record>, out items: [wref<NPCEquipmentItem_Record>], powerLevel: Int32) -> Void {
    ...
    let bitsMask: Uint64 = Cast<Uint64>(PowF(2.00, 32.00)) - 1u;
    if !IsDefined(equipmentGroupRecord) {
      return;
    };
    seed = EntityID.GetHash(id);
    groupID = Cast<Uint32>(TDBID.ToNumber(equipmentGroupRecord.GetID()) & bitsMask);
    seed = seed ^ groupID;
    if powerLevel < 0 {
    ...
    while i < itemsCount {
      ...
      if IsDefined(entry as NPCEquipmentItem_Record) {
        ...
        seed += 1u;
        AIActionTransactionSystem.ChooseSingleItemsSetFromPool(powerLevel, seed, entry as NPCEquipmentItemPool_Record, items);
      };
      i += 1;
    };
  }
  ...
  public final static func CheckEquipmentGroupForEquipment(const context: ScriptExecutionContext, condition: wref<AIItemCond_Record>) -> Bool {
    ...
    while i < itemsCount {
      ...
      if !IsDefined(item) {
        ...
        if IsDefined(condition.ItemType()) && NotEquals(condition.ItemType().Name(), n"None") {
          if NotEquals(item.ItemType().Type(), condition.ItemType().Type()) {
          } else {
            if NotEquals(condition.ItemCategory().Name(), n"None") {
              if item.ItemCategory() != condition.ItemCategory() {
              } else {
                if NotEquals(condition.ItemTag(), n"None") {
  ...
  public final static func GetEquipment(const context: ScriptExecutionContext, checkPrimaryEquipment: Bool, itemsList: script_ref<[NPCItemToEquip]>) -> Bool {
  ...
  public final static func GetEquipmentWithCondition(const context: ScriptExecutionContext, checkPrimaryEquipment: Bool, checkForUnequip: Bool, itemsList: script_ref<[NPCItemToEquip]>) -> Bool {
    ...
    let secondaryEquipmentDuplicatesPrimary: Bool;
    ...
    if checkForUnequip {
      ...
      if !checkPrimaryEquipment {
        ...
        while i < ArraySize(items) {
          if ArrayContains(primaryItems, items[i]) {
        ...
        if !IsDefined(currentItem.EquipSlot()) {
          ...
          if AIActionTransactionSystem.GetItemID(puppet, currentItem.Item(), bodySlotId, itemID) {
            ...
            if !checkForUnequip && !transactionSystem.HasItemInSlot(puppet, currentItem.EquipSlot().GetID(), itemID) && transactionSystem.HasItem(puppet, itemID) {
              ...
              if checkForUnequip && (transactionSystem.HasItemInSlot(puppet, currentItem.EquipSlot().GetID(), itemID) || hasFistsWoundedEquipped) {
                currentItem.UnequipCondition(conditions);
              } else {
              };
            };
            if checkForUnequip && ArraySize(conditions) == 0 && checkPrimaryEquipment && !hasFistsWoundedEquipped {
            } else {
              if ArraySize(conditions) > 0 && !AICondition.CheckActionConditions(context, conditions) {
              } else {
                if !checkForUnequip && ArraySize(conditions) == 0 && ArraySize(Deref(itemsList)) > 0 {
                  k = 0;
                  while k < ArraySize(Deref(itemsList)) {
                    if Deref(itemsList)[k].slotID == currentItem.EquipSlot().GetID() {
                    };
                    k += 1;
                  };
                };
                if !checkPrimaryEquipment && checkForUnequip {
                  AIActionTransactionSystem.GetEquipmentWithCondition(context, true, false, itemsToEquip);
                  if ArraySize(itemsToEquip) == 0 {
                    AIActionTransactionSystem.GetEquipmentWithCondition(context, false, false, itemsToEquip);
                  };
                  if ArraySize(itemsToEquip) == 0 {
                  } else {
  ...
  public final static func GetDefaultEquipment(const context: ScriptExecutionContext, characterRecord: wref<Character_Record>, checkForUnequip: Bool, itemsList: script_ref<[NPCItemToEquip]>) -> Bool {
  ...
  public final static func GetOnBodyEquipment(obj: wref<ScriptedPuppet>, itemsToEquip: script_ref<[NPCItemToEquip]>) -> Bool {
    let characterRecord: wref<Character_Record>;
    let equipmentGroup: wref<NPCEquipmentGroup_Record>;
    let i: Int32;
    let itemID: ItemID;
    ...
    while i < ArraySize(items) {
      if IsDefined(items[i].Item()) && AIActionTransactionSystem.GetItemIDFromRecord(items[i].Item(), itemID) {
        ...
        ArrayPush(Deref(itemsToEquip), itemToEquip);
      };
      i += 1;
    };
    equipmentGroup = characterRecord.SecondaryEquipment();
    ...
    return ArraySize(Deref(itemsToEquip)) > 0;
  }

  public final static func GetOnBodyEquipmentRecords(obj: wref<ScriptedPuppet>, outEquipmentRecords: script_ref<[wref<NPCEquipmentItem_Record>]>) -> Bool {
    let characterRecord: wref<Character_Record>;
    let equipmentGroup: wref<NPCEquipmentGroup_Record>;
    let i: Int32;
    let items: array<wref<NPCEquipmentItem_Record>>;
    ...
    while i < ArraySize(items) {
      if IsDefined(items[i].Item()) && IsDefined(items[i].OnBodySlot()) {
        ArrayPush(Deref(outEquipmentRecords), items[i]);
      };
      i += 1;
    };
    equipmentGroup = characterRecord.SecondaryEquipment();
    ...
    return ArraySize(Deref(outEquipmentRecords)) > 0;
  }
