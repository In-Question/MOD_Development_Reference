...
public class FocusCluesSystem extends ScriptableSystem {
  ...
  private persistent let m_linkedClues: [LinkedFocusClueData];

  private persistent let m_disabledGroupes: [CName];
  ...
  private final func AddLinkedClue(const clue: script_ref<LinkedFocusClueData>) -> Void {
    ArrayPush(this.m_linkedClues, Deref(clue));
  }

  private final func RemoveLinkedClue(const clue: script_ref<LinkedFocusClueData>) -> Void {
    let i: Int32 = ArraySize(this.m_linkedClues) - 1;
    while i >= 0 {
      if Deref(clue).ownerID == this.m_linkedClues[i].ownerID && Equals(Deref(clue).clueGroupID, this.m_linkedClues[i].clueGroupID) {
  ...
  private final func HasLinkedClue(const clue: script_ref<LinkedFocusClueData>) -> Bool {
    let i: Int32 = 0;
  ...
  private final func GetLinkedClueGroupData(groupID: CName, clue: script_ref<LinkedFocusClueData>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_linkedClues) {
  ...
  public final const func GetClueGroupData(groupID: CName, clue: script_ref<FocusClueDefinition>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_linkedClues) {
      if Equals(groupID, this.m_linkedClues[i].clueGroupID) {
        Deref(clue).isEnabled = this.m_linkedClues[i].isEnabled;
        Deref(clue).wasInspected = this.m_linkedClues[i].wasInspected;
        return true;
      };
      i += 1;
    };
  ...
  private final func IsGroupTagged(const clue: script_ref<LinkedFocusClueData>) -> Bool {
    let clueObject: ref<GameObject>;
    let i: Int32 = 0;
    while i < ArraySize(this.m_linkedClues) {
      if Equals(Deref(clue).clueGroupID, this.m_linkedClues[i].clueGroupID) {
  ...
  private final func UpdateLinkedClues(const clue: script_ref<LinkedFocusClueData>) -> Void {
    let currentClue: LinkedFocusClueData;
    let disableGroup: Bool;
    this.m_activeLinkedClue = Deref(clue);
    let i: Int32 = ArraySize(this.m_linkedClues) - 1;
    while i >= 0 {
      currentClue = this.m_linkedClues[i];
      if Equals(Deref(clue).clueGroupID, currentClue.clueGroupID) {
        currentClue.wasInspected = Deref(clue).wasInspected;
        currentClue.isScanned = Deref(clue).isScanned;
        currentClue.isEnabled = Deref(clue).isEnabled;
        if this.m_linkedClues[i].ownerID != Deref(clue).ownerID {
          if NotEquals(this.m_linkedClues[i].wasInspected, currentClue.wasInspected) || NotEquals(this.m_linkedClues[i].isScanned, currentClue.isScanned) || NotEquals(this.m_linkedClues[i].isEnabled, currentClue.isEnabled) {
            this.SendlinkedClueUpdateEvent(currentClue, Deref(clue).ownerID);
          };
        };
    ...
    if disableGroup {
      this.DisableGroup(Deref(clue).clueGroupID);
    } else {
      this.EnableGroup(Deref(clue).clueGroupID);
    };
  }
  ...
  private final func SendlinkedClueUpdateEvent(const linkedClue: script_ref<LinkedFocusClueData>, requester: EntityID) -> Void {
    let clueEvent: ref<linkedClueUpdateEvent>;
    if !EntityID.IsDefined(Deref(linkedClue).ownerID) {
      return;
    };
    clueEvent = new linkedClueUpdateEvent();
    clueEvent.linkedCluekData = Deref(linkedClue);
    clueEvent.requesterID = requester;
    GameInstance.GetPersistencySystem(this.GetGameInstance()).QueuePSEvent(Deref(linkedClue).psData.id, Deref(linkedClue).psData.className, clueEvent);
  }

  private final func ResolveLinkedCluesTagging(const clue: script_ref<LinkedFocusClueData>, tag: Bool) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_linkedClues) {
      if Equals(Deref(clue).clueGroupID, this.m_linkedClues[i].clueGroupID) {
        if this.m_linkedClues[i].ownerID != Deref(clue).ownerID {
          this.SendlinkedClueTagEvent(this.m_linkedClues[i], Deref(clue).ownerID, tag);
        };
      };
      i += 1;
    };
  ...
  private final func SendlinkedClueTagEvent(const linkedClue: script_ref<LinkedFocusClueData>, requester: EntityID, tag: Bool) -> Void {
    let clueEvent: ref<linkedClueTagEvent> = new linkedClueTagEvent();
    clueEvent.requesterID = requester;
    clueEvent.tag = tag;
    GameInstance.GetPersistencySystem(this.GetGameInstance()).QueueEntityEvent(Deref(linkedClue).ownerID, clueEvent);
  }
  ...
  public final const func GetActiveLinkedClueScannableData() -> [ScanningTooltipElementDef] {
