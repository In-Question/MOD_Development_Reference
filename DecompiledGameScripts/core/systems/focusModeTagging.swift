...
public class FocusModeTaggingSystem extends ScriptableSystem {
  ...
  private final func IsPlayerAiming(playerPuppet: wref<GameObject>) -> Bool {
    return this.GetPlayerStateMachineBlackboard(playerPuppet).GetInt(GetAllBlackboardDefs().PlayerStateMachine.UpperBody) == 6;
  }

  private final func IsPlayerInFocusMode(playerPuppet: wref<GameObject>) -> Bool {
    return this.GetPlayerStateMachineBlackboard(playerPuppet).GetInt(GetAllBlackboardDefs().PlayerStateMachine.Vision) == 1;
  }
  ...
  private final func RegisterObjectToBlackboard(target: ref<GameObject>) -> Void {
    ...
    let listOfObjects: array<wref<GameObject>> = FromVariant<array<wref<GameObject>>>(BBoard.GetVariant(GetAllBlackboardDefs().TaggedObjectsList.taggedObjectsList));
  ...
  private final func GetTaggedObjectsList() -> [wref<GameObject>] {
    let listOfObjects: array<wref<GameObject>>;
    let BBoard: ref<IBlackboard> = GameInstance.GetBlackboardSystem(this.GetGameInstance()).Get(GetAllBlackboardDefs().TaggedObjectsList);
    let listOfObjectsVariant: Variant = BBoard.GetVariant(GetAllBlackboardDefs().TaggedObjectsList.taggedObjectsList);
    if IsDefined(listOfObjectsVariant) {
      listOfObjects = FromVariant<array<wref<GameObject>>>(listOfObjectsVariant);
    };
    this.CleanupTaggedObjects(listOfObjects);
    return listOfObjects;
  }

  private final const func CleanupTaggedObjects(listToClean: script_ref<[wref<GameObject>]>) -> Void {
    let i: Int32 = ArraySize(Deref(listToClean)) - 1;
    i;
    while i >= 0 {
      if Deref(listToClean)[i] == null {
        ArrayErase(Deref(listToClean), i);
      };
      i -= 1;
    };
  }
