...
public class PingCachedData extends IScriptable {
  ...
  private final func GetCurrentMaxValue() -> Float {
    return this.m_timeout / Cast<Float>(this.m_ammountOfIntervals) * Cast<Float>(this.m_currentInterval);
  }

  private final func GetCurrentMinValue() -> Float {
    let value: Float = this.GetCurrentMaxValue() - this.m_timeout / Cast<Float>(this.m_ammountOfIntervals);
    if this.m_currentInterval == 1 {
      value = value + (this.m_timeout / Cast<Float>(this.m_ammountOfIntervals)) / Cast<Float>(this.m_ammountOfIntervals);
    };
    return value;
  }
}
...
public class NetworkSystem extends ScriptableSystem {
  ...
  private let m_networkLinks: [SNetworkLinkData];

  private let m_networkRevealTargets: [EntityID];

  private let m_networkRevealTargetsLastSession: [EntityID];
  ...
  private let m_activePings: [ref<PingCachedData>];
  ...
  private let m_backdoors: [PersistentID];
  ...
  protected final func UnregisterPlayerSpawnedCallback() -> Void {
    if Cast<Bool>(this.m_playerSpawnCallback) {
  ...
  private final func DeterminNetworkLinkPriority(const linkData: script_ref<SNetworkLinkData>) -> EPriority {
    switch Deref(linkData).linkType {
      case ELinkType.NETWORK:
        if Deref(linkData).isPing {
  ...
  private final func RemoveNetworkLinkByData(const linkData: script_ref<SNetworkLinkData>) -> Void {
    let instant: Bool;
    let targets: array<EntityID>;
    let i: Int32 = ArraySize(this.m_networkLinks) - 1;
    while i >= 0 {
      if Equals(this.m_networkLinks[i].linkType, Deref(linkData).linkType) && this.m_networkLinks[i].slaveID == Deref(linkData).slaveID && this.m_networkLinks[i].masterID == Deref(linkData).masterID && Equals(this.m_networkLinks[i].slavePos, Deref(linkData).slavePos) && Equals(this.m_networkLinks[i].masterPos, Deref(linkData).masterPos) {
        if Deref(linkData).lifetime > 0.00 {
          if Equals(Deref(linkData).linkType, ELinkType.NETWORK) && Deref(linkData).isPing {
            ArrayPush(targets, Deref(linkData).masterID);
            ArrayPush(targets, Deref(linkData).slaveID);
          };
          instant = false;
        };
  ...
  private final func UnregisterNetworkLinkWithDelay(const linkData: script_ref<SNetworkLinkData>) -> DelayID {
    let unregisterReq: ref<UnregisterNetworkLinkRequest> = new UnregisterNetworkLinkRequest();
    ArrayPush(unregisterReq.linksData, Deref(linkData));
    unregisterReq.linksData[0].delayID = GameInstance.GetDelaySystem(this.GetGameInstance()).DelayScriptableSystemRequest(this.GetClassName(), unregisterReq, Deref(linkData).lifetime, false);
    return unregisterReq.linksData[0].delayID;
  }

  private final func RegisterNetworkLinkWithDelay(const linkData: script_ref<SNetworkLinkData>, delay: Float) -> Void {
    let registerReq: ref<RegisterNetworkLinkRequest> = new RegisterNetworkLinkRequest();
    ArrayPush(registerReq.linksData, Deref(linkData));
    GameInstance.GetDelaySystem(this.GetGameInstance()).DelayScriptableSystemRequest(this.GetClassName(), registerReq, delay, false);
  }

  public final const func HasNetworkLink(const linkData: script_ref<SNetworkLinkData>, out index: Int32) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_networkLinks) {
      if Equals(Deref(linkData).slavePos, this.m_networkLinks[i].slavePos) && Equals(Deref(linkData).masterPos, this.m_networkLinks[i].masterPos) && Deref(linkData).slaveID == this.m_networkLinks[i].slaveID && Deref(linkData).masterID == this.m_networkLinks[i].masterID && Equals(Deref(linkData).linkType, this.m_networkLinks[i].linkType) && Equals(Deref(linkData).isPing, this.m_networkLinks[i].isPing) {
        index = i;
        return true;
      };
      i += 1;
  ...
  public final const func HasNetworkLink(const linkData: script_ref<SNetworkLinkData>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_networkLinks) {
      if Equals(Deref(linkData).slavePos, this.m_networkLinks[i].slavePos) && Equals(Deref(linkData).masterPos, this.m_networkLinks[i].masterPos) && Deref(linkData).slaveID == this.m_networkLinks[i].slaveID && Deref(linkData).masterID == this.m_networkLinks[i].masterID && Equals(Deref(linkData).linkType, this.m_networkLinks[i].linkType) {
        return true;
      };
      i += 1;
    };
  ...
  private final func CancelNetworkLinkDelay(const linkData: script_ref<SNetworkLinkData>) -> Void {
    let invalidDealyID: DelayID;
    if Deref(linkData).lifetime > 0.00 && Deref(linkData).delayID != invalidDealyID {
      GameInstance.GetDelaySystem(this.GetGameInstance()).CancelDelay(Deref(linkData).delayID);
    };
  }

  private final const func CompareLinks(const linkData1: script_ref<SNetworkLinkData>, const linkData2: script_ref<SNetworkLinkData>) -> Bool {
    return Deref(linkData1).slaveID == Deref(linkData2).slaveID && Deref(linkData1).masterID == Deref(linkData2).masterID && Equals(Deref(linkData1).linkType, Deref(linkData2).linkType) && Equals(Deref(linkData1).fxResource, Deref(linkData2).fxResource) && Equals(Deref(linkData1).isDynamic, Deref(linkData2).isDynamic) && Equals(Deref(linkData1).drawLink, Deref(linkData2).drawLink) && Equals(Deref(linkData1).isPing, Deref(linkData2).isPing) && Equals(Deref(linkData1).revealSlave, Deref(linkData2).revealSlave) && Equals(Deref(linkData1).revealMaster, Deref(linkData2).revealMaster);
  }

  private final const func CompareLinksData(const linkData1: script_ref<SNetworkLinkData>, const linkData2: script_ref<SNetworkLinkData>) -> Bool {
    return Equals(Deref(linkData1).fxResource, Deref(linkData2).fxResource) && Equals(Deref(linkData1).isDynamic, Deref(linkData2).isDynamic) && Equals(Deref(linkData1).drawLink, Deref(linkData2).drawLink) && Equals(Deref(linkData1).isPing, Deref(linkData2).isPing) && Equals(Deref(linkData1).revealSlave, Deref(linkData2).revealSlave) && Equals(Deref(linkData1).revealMaster, Deref(linkData2).revealMaster);
  }

  public final const func HasNetworkLinkWithHigherPriority(const linkData: script_ref<SNetworkLinkData>) -> Bool {
    let i: Int32 = 0;
    while i < ArraySize(this.m_networkLinks) {
      if Equals(Deref(linkData).slavePos, this.m_networkLinks[i].slavePos) && Equals(Deref(linkData).masterPos, this.m_networkLinks[i].masterPos) && Deref(linkData).slaveID == this.m_networkLinks[i].slaveID && Deref(linkData).masterID == this.m_networkLinks[i].masterID {
        if EnumInt(this.m_networkLinks[i].priority) > EnumInt(Deref(linkData).priority) {
          return true;
        };
      };
      i += 1;
  ...
  private final func GetNetworkBeam(const linkData: script_ref<SNetworkLinkData>) -> ref<FxInstance> {
    let i: Int32 = 0;
    while i < ArraySize(this.m_networkLinks) {
      if Equals(Deref(linkData).slavePos, this.m_networkLinks[i].slavePos) && Equals(Deref(linkData).masterPos, this.m_networkLinks[i].masterPos) && Deref(linkData).slaveID == this.m_networkLinks[i].slaveID && Deref(linkData).masterID == this.m_networkLinks[i].masterID {
  ...
  private final func KillNetworkBeamByData(const linkData: script_ref<SNetworkLinkData>) -> Void {
    let i: Int32 = 0;
    while i < ArraySize(this.m_networkLinks) {
      if Equals(this.m_networkLinks[i].linkType, Deref(linkData).linkType) && this.m_networkLinks[i].slaveID == Deref(linkData).slaveID && this.m_networkLinks[i].masterID == Deref(linkData).masterID {
        this.KillNetworkBeam(i, false);
        return;
      };
      i += 1;
  ...
  private final func ActivateNetworkLinks(const toActivate: script_ref<[Int32]>) -> Void {
    let excessLnks: array<Int32>;
    let request: ref<ActivateLinksRequest>;
    let totalIinks: Int32;
    let i: Int32 = 0;
    while i < ArraySize(Deref(toActivate)) {
      if totalIinks > 10 {
        ArrayPush(excessLnks, Deref(toActivate)[i]);
      } else {
        this.ActivateNetworkLinkByTask(Deref(toActivate)[i]);
      };
      totalIinks += 1;
      i += 1;
    };
  ...
  private final func DeactivateNetworkLinks(const toDeactivate: script_ref<[Int32]>, hasContext: Bool) -> Void {
    let excessLnks: array<Int32>;
    let request: ref<DeactivateLinksRequest>;
    let totalIinks: Int32;
    let i: Int32 = 0;
    while i < ArraySize(Deref(toDeactivate)) {
      if totalIinks > 10 {
        ArrayPush(excessLnks, Deref(toDeactivate)[i]);
      } else {
        this.DeactivateNetworkLinkByTask(Deref(toDeactivate)[i], hasContext);
      };
      totalIinks += 1;
      i += 1;
    };
  ...
  private final func ResolveConnectionHighlight(enable: Bool, const linkData: script_ref<SNetworkLinkData>) -> Void {
    let slaveID: EntityID = Deref(linkData).slaveID;
    let masterID: EntityID = Deref(linkData).masterID;
    if enable && !Deref(linkData).isActive || !enable && Deref(linkData).isActive {
      if Deref(linkData).revealSlave && EntityID.IsDefined(slaveID) {
        this.SendConnectionHighlightEvent(enable, slaveID, masterID, linkData);
      };
      if Deref(linkData).revealMaster && EntityID.IsDefined(masterID) {
        this.SendConnectionHighlightEvent(enable, masterID, slaveID, linkData);
      };
    };
  }

  private final func SendConnectionHighlightEvent(enable: Bool, target: EntityID, source: EntityID, const linkData: script_ref<SNetworkLinkData>) -> Void {
    let evt: ref<RevealDeviceRequest> = new RevealDeviceRequest();
    evt.shouldReveal = enable;
    evt.sourceID = source;
    evt.linkData = Deref(linkData);
  ...
  protected cb func OnVisionModeChanged(value: Int32) -> Bool {
    this.EvaluateVisionMode(IntEnum<gameVisionModeType>(value));
  }
  ...
  private final func GetAllSlavesOfMaster(masterID: EntityID) -> [EntityID] {
  ...
  private final func GetAllMastersOfSlave(slaveID: EntityID) -> [EntityID] {
  ...
  private final const func IsFreeLinkLimitReached(const linkData: script_ref<SNetworkLinkData>) -> Bool {
    let i: Int32;
    let sourceLinkCount: Int32;
    let targetLinkCount: Int32;
    if !Deref(linkData).isActive || !EntityID.IsDefined(Deref(linkData).slaveID) || !EntityID.IsDefined(Deref(linkData).masterID) {
      return false;
    };
    i = 0;
    while i < ArraySize(this.m_networkLinks) {
      if this.m_networkLinks[i].slaveID == Deref(linkData).slaveID {
        if Equals(this.m_networkLinks[i].linkType, ELinkType.FREE) {
          targetLinkCount += 1;
        };
      };
      if this.m_networkLinks[i].masterID == Deref(linkData).masterID {
        if Equals(this.m_networkLinks[i].linkType, ELinkType.FREE) {
          sourceLinkCount += 1;
        };
      };
      if sourceLinkCount > 20 || targetLinkCount > 20 {
        return true;
      };
      i += 1;
    };
  ...
  private final func AddActivePing(source: ref<GameObject>, pingType: EPingType, duration: Float, linkType: ELinkType, revealNetworkAtEnd: Bool, fxResource: FxResource, virtualNetworkShapeID: TweakDBID) -> Void {
    ...
    if !this.HasActivePing(source.GetEntityID()) {
      ...
      ArrayClear(this.m_networkRevealTargetsLastSession);
  ...
  private final func CreateForksForVirtualLink1(linkData: SNetworkLinkData, ping: ref<PingCachedData>, sphereCentre: Vector4, radius: Float, direction: Vector4, const vertices: script_ref<[Vector4]>) -> Void {
    let foundVertice: Vector4;
    let numberOfForks: Int32 = RandRange(0, 2);
    let baseRadius: Float = 4.17;
    ...
    while i < numberOfForks {
      ...
      foundVertice = this.FindBestMatchingVertice(linkData.slavePos, linkDirection, Rad2Deg(this.GetVirtualLinkForkAngleTollerance()), 25.00 * RandRangeF(1.00, 1.20), foundVertice, vertices);
  ...
  private final const func IsActivePingsLimitReached() -> Bool {
    return ArraySize(this.m_activePings) > 10;
  }
  ...
  private final func HasDiffrentParentsThanTargets(sourceID: EntityID, const targets: script_ref<[EntityID]>) -> Bool {
    let i: Int32;
    let k: Int32;
    let l: Int32;
    ...
    while i < ArraySize(Deref(targets)) {
      GameInstance.GetDeviceSystem(this.GetGameInstance()).GetLazyParents(Deref(targets)[i], targetParents);
      k = 0;
      while k < ArraySize(targetParents) {
        l = 0;
        while l < ArraySize(sourceParents) {
          if Equals(sourceParents[l].GetPersistentID(), targetParents[k].GetPersistentID()) {
            return false;
          };
          l += 1;
        };
  ...
  private final func HasDiffrentChildrenThanTargets(sourceID: EntityID, const targets: script_ref<[EntityID]>) -> Bool {
    let i: Int32;
    let k: Int32;
    let l: Int32;
    ...
    i = 0;
    while i < ArraySize(Deref(targets)) {
      GameInstance.GetDeviceSystem(this.GetGameInstance()).GetLazyChildren(Deref(targets)[i], targetChildren);
      k = 0;
      while k < ArraySize(targetChildren) {
        l = 0;
        while l < ArraySize(sourceChildren) {
          if Equals(sourceChildren[l].GetPersistentID(), targetChildren[k].GetPersistentID()) {
            return false;
          };
          l += 1;
        };
  ...
  private final func ResolveNetworkRevealTarget(const linkData: script_ref<SNetworkLinkData>) -> Void {
    ...
    if this.HasActivePingWithRevealNetwork(Deref(linkData).slaveID) {
      if hasTargets {
        ignoreNetworkReveal = ArrayContains(this.m_networkRevealTargetsLastSession, Deref(linkData).masterID) || !this.HasDiffrentChildrenThanTargets(Deref(linkData).masterID, currentTargets) && !this.HasDiffrentParentsThanTargets(Deref(linkData).masterID, currentTargets);
      };
      if !ignoreNetworkReveal {
        this.AddNetworkRevealTarget(Deref(linkData).masterID);
        ignoreNetworkReveal = false;
      };
    };
    if this.HasActivePingWithRevealNetwork(Deref(linkData).masterID) {
      if hasTargets {
        ignoreNetworkReveal = ArrayContains(this.m_networkRevealTargetsLastSession, Deref(linkData).slaveID) || !this.HasDiffrentChildrenThanTargets(Deref(linkData).slaveID, currentTargets) && !this.HasDiffrentParentsThanTargets(Deref(linkData).slaveID, currentTargets);
      };
      if !ignoreNetworkReveal {
        this.AddNetworkRevealTarget(Deref(linkData).slaveID);
        ignoreNetworkReveal = false;
      };
    };
  }
  ...
  private final func RevealNetworkOnCachedTarget(const targets: script_ref<[EntityID]>) -> Void {
    let i: Int32;
    let k: Int32;
    ...
    while i >= 0 {
      k = 0;
      while k < ArraySize(Deref(targets)) {
        if this.m_networkRevealTargets[i] == Deref(targets)[k] {
          if EntityID.IsDefined(this.m_networkRevealTargets[i]) && !this.HasActivePing(Deref(targets)[k]) {
            this.SendRevealNetworkEvent(this.m_networkRevealTargets[i]);
          };
          ArrayPush(this.m_networkRevealTargetsLastSession, this.m_networkRevealTargets[i]);
  ...
  private final func OnRevealNetworkRequestRequest(request: ref<RevealNetworkRequestRequest>) -> Void {
    this.SendRevealNetworkEvent(request.target, request.delay);
  }

  private final func SendRevealNetworkEvent(target: EntityID, opt delay: Float) -> Void {
  ...
  private final static func GetMaximumNumberOfActivePings() -> Int32 {
    return 10;
  }

  private final static func GetNumberOfVirtualLinksPerObject() -> Int32 {
    return 10;
  }

  private final static func GetVirtualLinkDepth() -> Int32 {
    return 6;
  }
  ...
  private final static func GetMaxAmountOfVirtualLinkForks() -> Int32 {
    return 2;
  }

  private final static func GetVirtualLinksSphereRadius() -> Float {
    return 25.00;
  }
  ...
  private final func FindBestMatchingVertice(point: Vector4, direction: Vector4, angle: Float, radius: Float, excludeVertice: Vector4, const vertices: script_ref<[Vector4]>) -> Vector4 {
    ...
    let i: Int32 = 0;
    while i < ArraySize(Deref(vertices)) {
      if Equals(Deref(vertices)[i], excludeVertice) {
      } else {
        distance = Vector4.Distance(point, Deref(vertices)[i]);
        if distance > radius {
        } else {
          foundVerticeDirection = Vector4.Normalize(Deref(vertices)[i] - point);
          currentAngle = Vector4.GetAngleBetween(direction, foundVerticeDirection);
          if currentAngle > angle {
          } else {
            if currentAngle < bestAngle {
              bestAngle = currentAngle;
              foundVertice = Deref(vertices)[i];
            };
          };
        };
      };
  ...
  private final func GetRandomPointOnSphere(sphereCentre: Vector4, radius: Float, slice: Int32) -> Vector4 {
    let point: Vector4;
    let angleIncrementPhi: Float = 0.63;
    let angleIncrementTheta: Float = 0.16;
    let minAngleTheta: Float = Cast<Float>(slice) * angleIncrementTheta;
    let maxAngleTheta: Float = Cast<Float>(slice + 1) * angleIncrementTheta;
    let minAnglePhi: Float = Cast<Float>(slice) * angleIncrementPhi;
    let maxAnglePhi: Float = Cast<Float>(slice + 1) * angleIncrementPhi;
    let theta: Float = RandRangeF(minAngleTheta, maxAngleTheta);
    let phi: Float = RandRangeF(minAnglePhi, maxAnglePhi);
