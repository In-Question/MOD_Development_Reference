...
public native class SettingsSelectorController extends inkLogicController {
  ...
  protected edit let m_indentMarker: inkWidgetRef;

  protected let m_SettingsEntry: wref<ConfigVar>;

  protected let m_hoveredChildren: [wref<inkWidget>];
  ...
  public final native func GetWarning() -> CName;
  ...
  public final func ResetAdditionalText() -> Void {
    this.m_additionalText = n"None";
  }

  public func SetInteractive(interactive: Bool) -> Void;
  ...
  protected cb func OnInitialize() -> Bool {
    ...
    inkWidgetRef.SetVisible(this.m_indentMarker, false);
  }
  ...
  public final func AddLabelIndent(opt isRightAligned: Bool) -> Void {
    inkWidgetRef.SetVisible(this.m_indentMarker, true);
    if isRightAligned {
      inkWidgetRef.UpdateMargin(this.m_LabelText, 0.00, 0.00, 100.00, 0.00);
    } else {
      inkWidgetRef.UpdateMargin(this.m_LabelText, 100.00, 0.00, 0.00, 0.00);
    };
  }
...
public class SettingsSelectorControllerBool extends SettingsSelectorController {
  ...
  public func SetInteractive(interactive: Bool) -> Void {
    inkWidgetRef.SetInteractive(this.m_onStateBody, interactive);
    inkWidgetRef.SetInteractive(this.m_offStateBody, interactive);
  }
...
public class SettingsSelectorControllerFloat extends SettingsSelectorControllerRange {
  ...
  public final func HACK_DisableIfNeededForHDRCalibrationScreen() -> Void {
    let buttonController: wref<inkButtonController>;
    if this.m_SettingsEntry.IsDisabled() {
      this.m_sliderController.HACK_SetDisabledForHDRCalibrationScreen();
      this.m_sliderController.SetInputDisabled(true);
      buttonController = inkWidgetRef.GetControllerByType(this.m_sliderWidget, n"inkButtonController") as inkButtonController;
      buttonController.SetEnabled(false);
    };
  }

  protected cb func OnSliderValueChanged(sliderController: wref<inkSliderController>, progress: Float, value: Float) -> Bool {
    this.m_newValue = ClampF(value, this.m_sliderController.GetMinValue(), this.m_sliderController.GetMaxValue());
    this.Refresh();
  }
...
public class SettingsSelectorControllerList extends SettingsSelectorControllerRange {
  ...
  protected final func PopulateDots(size: Int32) -> Void {
    ...
    dotWidth = parentWidth / Cast<Float>(size) - 8.00;
...
public class SettingsSelectorControllerListName extends SettingsSelectorControllerList {
  ...
  public func Refresh() -> Void {
    ...
    if !this.m_realValue.ListHasDisplayValues() {
      ...
      varValue = this.m_realValue.GetDisplayValue(this.m_currentIndex);
      if Equals(varValue, n"UI-Settings-Language-Russian") && Equals(this.m_realValue.GetName(), n"VoiceOver") {
        varValue = n"UI-Settings-Language-Russian2";
      };
      if Equals(this.m_additionalText, n"None") {
        inkTextRef.SetText(this.m_ValueText, GetLocalizedTextByKey(varValue));
      } else {
