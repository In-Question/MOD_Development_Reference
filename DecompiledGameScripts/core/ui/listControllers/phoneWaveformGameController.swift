...
public native class PhoneWaveformGameController extends inkGameController {
  ...
  private let m_bars: [wref<inkWidget>];

  private let m_traces: [wref<inkWidget>];
  ...
  private final func InitWaveform() -> Void {
    ...
    this.m_barSize = new Vector2((this.m_cachedRootSize.X - this.m_barsPadding * Cast<Float>(count)) / Cast<Float>(count), this.m_cachedRootSize.Y);
    while ArraySize(this.m_bars) > 0 {
      this.m_root.RemoveChild(ArrayPop(this.m_bars));
    };
    i = 0;
    while i < count {
      margin = new inkMargin(Cast<Float>(i) * this.m_barSize.X + this.m_barsPadding * Cast<Float>(i), this.m_cachedRootSize.Y, 0.00, 0.00);
  ...
  private final func UpdateWaveform(const audioData: script_ref<[Vector4]>) -> Void {
    let bar: wref<inkWidget>;
    let rndValue: Float;
    let size: Vector2;
    ...
    let i: Int32 = 0;
    while i < count {
      value = (this.m_maxValue - AbsF(Deref(audioData)[i].Z)) / this.m_maxValue;
      if value > 0.00 {
  ...
  private final func FixSize(bar: wref<inkWidget>, value: Float) -> Vector2 {
    ...
    if !FloatIsEqual(Cast<Float>(intX) / 2.00, Cast<Float>(RoundF(Cast<Float>(intX) / 2.00))) {
      intX += 1;
    };
    if !FloatIsEqual(Cast<Float>(intY) / 2.00, Cast<Float>(RoundF(Cast<Float>(intY) / 2.00))) {
      intY += 1;
    };
    res = new Vector2(Cast<Float>(intX), Cast<Float>(intY));
    bar.SetSize(res);
    return res;
  }
}
