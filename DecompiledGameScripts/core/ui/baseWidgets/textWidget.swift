...
public native class inkText extends inkLeafWidget {
  ...
  public final func SetLocalizedTextScript(const locKey: script_ref<String>, opt textParams: ref<inkTextParams>) -> Void {
    if IsStringNumber(locKey) {
      this.SetText(Deref(locKey), textParams);
    } else {
      if !IsStringValid(locKey) {
        this.SetText("", textParams);
      } else {
        this.SetLocalizedTextString(Deref(locKey), textParams);
      };
    };
  }
  ...
  public final native func SetWrappingAtPosition(position: Float) -> Void;
}
