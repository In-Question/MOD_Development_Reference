...
public native class inkHUDGameController extends inkGameController {
  ...
  protected final native func ToggleVisibility(value: Bool, isSkippingInOutAnimation: Bool) -> Void;
  ...
  protected final func ShowRequest() -> Void {
    if !this.m_moduleShown {
      ...
      if NotEquals(this.m_showAnimationName, n"None") {
  ...
  protected final func HideRequest() -> Void {
    if this.m_moduleShown {
      ...
      if NotEquals(this.m_hideAnimationName, n"None") {
  ...
  protected final func IsPlayingMultiplayer() -> Bool {
