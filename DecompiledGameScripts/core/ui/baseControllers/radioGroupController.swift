...
public native class inkRadioGroupController extends inkLogicController {
  ...
  public final func AddToggles(const toAdd: script_ref<[wref<inkToggleController>]>) -> Void {
    let i: Int32 = 0;
    let limit: Int32 = ArraySize(Deref(toAdd));
    while i < limit {
      this.AddToggle(Deref(toAdd)[i]);
      i += 1;
    };
  }
  ...
  public final native func GetControllers(out controllers: [wref<inkToggleController>]) -> Void;
}
