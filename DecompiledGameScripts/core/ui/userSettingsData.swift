...
public class GameplaySettingsSystem extends ScriptableSystem {
  ...
  public final static func GetVehicleCombatHoldToShootEnabled(owner: ref<GameObject>) -> Bool {
    return GameplaySettingsSystem.GetGameplaySettingsSystemInstance(owner).m_gameplaySettingsListener.m_vehicleCombatHoldToShootEnabled;
  }
...
public class GameplaySettingsListener extends ConfigVarListener {
  ...
  private let m_vehicleControlsGroup: ref<ConfigGroup>;
  ...
  public let m_vehicleCombatHoldToShootEnabled: Bool;

  @default(GameplaySettingsListener, /accessibility/difficulty)
  ...
  @default(GameplaySettingsListener, /controls/vehicle)
  private let m_vehicleControlsPath: CName;
  ...
  private let m_additiveCameraOptionName: CName;

  @default(GameplaySettingsListener, FastForward)
  private let m_fastForwardOptionName: CName;

  @default(GameplaySettingsListener, input_hints)
  private let m_inputHintsOptionName: CName;

  @default(GameplaySettingsListener, MovementDodge)
  private let m_movementDodgeOptionName: CName;

  @default(GameplaySettingsListener, VehicleCombatHoldToShoot)
  private let m_vehicleCombatHoldToShootOptionName: CName;
  ...
  public final func Initialize(player: wref<PlayerPuppet>) -> Void {
    ...
    this.m_vehicleControlsGroup = this.m_userSettings.GetGroup(this.m_vehicleControlsPath);
    ...
    this.Register(this.m_vehicleControlsPath);
    ...
    this.UpdateVehicleCombatHoldToShootSettings();
  ...
  public func OnVarModified(groupPath: CName, varName: CName, varType: ConfigVarType, reason: ConfigChangeReason) -> Void {
    ...
    switch varName {
      case this.m_inputHintsOptionName:
        this.UpdateInputHintsEnabled();
        break;
      case this.m_additiveCameraOptionName:
        this.RefreshAdditiveCameraMovementsSetting();
        this.m_player.OnAdditiveCameraMovementsSettingChanged();
        break;
      case this.m_fastForwardOptionName:
        this.UpdateFFSetting();
        break;
      case this.m_movementDodgeOptionName:
        this.UpdateMovementDodgeSettings();
        break;
      case this.m_vehicleCombatHoldToShootOptionName:
        this.UpdateVehicleCombatHoldToShootSettings();
        break;
      default:
    };
  ...
  private final func UpdateInputHintsEnabled() -> Void {
    ...
    if IsDefined(this.m_hudGroup) {
      settingsVar = this.m_hudGroup.GetVar(this.m_inputHintsOptionName) as ConfigVarBool;
  ...
  private final func RefreshAdditiveCameraMovementsSetting() -> Void {
    ...
    if IsDefined(this.m_diffSettingsGroup) {
      additiveCameraMovementsVar = this.m_diffSettingsGroup.GetVar(this.m_additiveCameraOptionName) as ConfigVarListFloat;
  ...
  private final func UpdateFFSetting() -> Void {
    ...
    if IsDefined(this.m_miscSettingsGroup) {
      FFSettingsVar = this.m_miscSettingsGroup.GetVar(this.m_fastForwardOptionName) as ConfigVarListString;
  ...
  private final func UpdateMovementDodgeSettings() -> Void {
    ...
    if IsDefined(this.m_controlsGroup) && this.m_controlsGroup.HasVar(this.m_movementDodgeOptionName) {
      controlsSettingsVar = this.m_controlsGroup.GetVar(this.m_movementDodgeOptionName) as ConfigVarBool;
  ...
  private final func UpdateVehicleCombatHoldToShootSettings() -> Void {
    let configVar: ref<ConfigVarBool>;
    if IsDefined(this.m_vehicleControlsGroup) && this.m_vehicleControlsGroup.HasVar(this.m_vehicleCombatHoldToShootOptionName) {
      configVar = this.m_vehicleControlsGroup.GetVar(this.m_vehicleCombatHoldToShootOptionName) as ConfigVarBool;
      this.m_vehicleCombatHoldToShootEnabled = configVar.GetValue();
    } else {
      this.m_vehicleCombatHoldToShootEnabled = false;
    };
  }
}
