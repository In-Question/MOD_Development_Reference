...
public class SampleDeviceClassPS extends GameObjectPS {
  ...
  public final func GetActions() -> [ref<DeviceAction>] {
...
public class PSD_Trigger extends GameObject {
  ...
  protected cb func OnInteraction(interaction: ref<InteractionChoiceEvent>) -> Bool {
    ...
    let i: Int32 = 0;
    let j: Int32 = 0;
    let targetEntityID: EntityID = Cast<EntityID>(ResolveNodeRefWithEntityID(this.m_ref, this.GetEntityID()));
    let objPS: ref<GameComponentPS> = GameInstance.GetPersistencySystem(this.GetGame()).GetConstAccessToPSObject(Cast<PersistentID>(targetEntityID), this.m_className) as GameComponentPS;
    ...
    while i < ArraySize(actions) {
      ...
      while j < ArraySize(propertyArr) {
        if Equals(propertyArr[j].typeName, n"Bool") {
          propertyArr[j].first = ToVariant(!FromVariant<Bool>(propertyArr[j].first));
        };
        j += 1;
      };
