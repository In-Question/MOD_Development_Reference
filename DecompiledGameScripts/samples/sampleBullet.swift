...
public class sampleBullet extends BaseProjectile {
  ...
  protected cb func OnProjectileInitialize(eventData: ref<gameprojectileSetUpEvent>) -> Bool {
    ...
    this.m_BulletCollisionEvaluator.SetUser(this.m_user);
  }
...
public class BulletCollisionEvaluator extends gameprojectileScriptCollisionEvaluator {
  ...
  @default(BulletCollisionEvaluator, false)
  private let m_isSmartBullet: Bool;

  public final func SetIsExplodingBullet(isExplodingBullet: Bool) -> Void {
    this.m_isExplodingBullet = isExplodingBullet;
  }

  public final func SetIsSmartBullet(isSmartBullet: Bool) -> Void {
    this.m_isSmartBullet = isSmartBullet;
  }

  public final func SetWeaponParams(const params: script_ref<gameprojectileWeaponParams>) -> Void {
    this.m_weaponParams = Deref(params);
  }

  public final func SetUser(user: wref<GameObject>) -> Void {
    this.m_user = user;
  }
  ...
  protected func EvaluateCollision(defaultOnCollisionAction: gameprojectileOnCollisionAction, params: ref<CollisionEvaluatorParams>) -> gameprojectileOnCollisionAction {
    ...
    if params.target.IsVehicle() && VehicleComponent.GetDriverMounted(this.m_user.GetGame(), params.target.GetEntityID()) == this.m_user {
      return gameprojectileOnCollisionAction.Pierce;
    };
    if !this.m_isExplodingBullet && !this.m_isSmartBullet && params.isAutoBounceSurface && params.numBounces < 5u {
    ...
    validBounces = params.numBounces < Cast<Uint32>(this.m_weaponParams.ricochetData.count);
