...
public class sampleSmartBullet extends BaseProjectile {
  ...
  private let m_trackedTargetComponent: wref<TargetingComponent>;
  ...
  protected cb func OnProjectileInitialize(eventData: ref<gameprojectileSetUpEvent>) -> Bool {
    ...
    this.m_BulletCollisionEvaluator.SetIsSmartBullet(true);
    this.m_BulletCollisionEvaluator.SetUser(this.m_owner);
    if IsDefined(this.m_weapon) {
      ...
      this.m_useSpiralParams = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunAddSpiralTrajectory) > 0.00;
      this.m_spiralParams.radius = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunSpiralRadius);
      this.m_spiralParams.cycleTimeMin = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunSpiralCycleTimeMin);
      this.m_spiralParams.cycleTimeMax = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunSpiralCycleTimeMax);
      this.m_spiralParams.randomizeDirection = Cast<Bool>(this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunSpiralRandomizeDirection));
      if eventData.owner.IsNPC() {
        this.m_startVelocity = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunNPCProjectileVelocity);
      } else {
        if eventData.owner.IsPlayer() {
          this.m_startVelocity = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunPlayerProjectileVelocity);
        };
      };
      velVariance = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunProjectileVelocityVariance);
      ...
      this.m_smartGunHitProbability = ClampF(this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunHitProbability), 0.00, 1.00);
      if IsDefined(this.m_owner) && IsDefined(this.m_statsSystem) {
        this.m_smartGunHitProbability *= 1.00 + this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_owner.GetEntityID()), gamedataStatType.SmartGunHitProbabilityMultiplier);
        this.m_smartGunHitProbability = ClampF(this.m_smartGunHitProbability, 0.00, 1.00);
      };
      this.m_smartGunMissDelay = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunMissDelay);
      this.m_smartGunMissRadius = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.SmartGunMissRadius);
  ...
  private final func StartTrailEffect() -> Void {
    ...
    if !this.m_trailStarted {
      ...
      GameObjectEffectHelper.StartEffectEvent(this, this.m_trailName, true, effectBlackboard);
  ...
  private final func SetCurrentDamageTrailName() -> Void {
    ...
    let cachedThreshold: Float = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.PhysicalDamage);
    this.m_trailName = n"trail";
    if this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ThermalDamage) > cachedThreshold {
      cachedThreshold = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ThermalDamage);
      this.m_trailName = n"trail_thermal";
    };
    if this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ElectricDamage) > cachedThreshold {
      cachedThreshold = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ElectricDamage);
      this.m_trailName = n"trail_electric";
    };
    if this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ChemicalDamage) > cachedThreshold {
      cachedThreshold = this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_weaponID), gamedataStatType.ChemicalDamage);
  ...
  protected cb func OnShootTarget(eventData: ref<gameprojectileShootTargetEvent>) -> Bool {
    ...
    if IsDefined(eventData.params.trackedTargetComponent) {
      this.m_trackedTargetComponent = eventData.params.trackedTargetComponent as TargetingComponent;
    ...
    if IsDefined(this.m_followPhaseParams.targetComponent) {
      ...
      if IsDefined(targetEntity) {
        ...
        if this.GetInitialDistanceToTarget() > 4.00 && this.m_randomTargetMissChance < ClampF(this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_targetID), gamedataStatType.SmartTargetingDisruptionProbability), 0.00, 1.00) {
          this.DisableTargetCollisions(this.m_targetID);
        };
        if IsDefined(targetEntity as GameObject) {
          isTargetPlayer = (targetEntity as GameObject).IsPlayer();
        };
      };
    };
    ...
    this.m_followPhaseParams.accuracy = isTargetPlayer ? 0.08 : 0.01;
          ...
          if Equals(this.m_phase, ESmartBulletPhase.Linear) {
            ...
            if Equals(this.m_phase, ESmartBulletPhase.Miss) {
              ...
              if this.m_shouldStopAndDrop {
                ...
                targetAngle = Vector4.GetAngleDegAroundAxis(Matrix.GetDirectionVector(localToWorld), targetPosition - Matrix.GetTranslation(localToWorld), new Vector4(0.00, 0.00, 1.00, 0.00));
                if targetAngle > missParamsRecord.AreaToIgnoreHalfYaw() {
                  leftSideMiss = false;
                  randomYaw = RandRangeF(missParamsRecord.MinMissAngleYaw(), 0.00);
                } else {
                  if targetAngle < -missParamsRecord.AreaToIgnoreHalfYaw() {
                    leftSideMiss = true;
                    randomYaw = RandRangeF(0.00, missParamsRecord.MaxMissAngleYaw());
                  } else {
  ...
  protected cb func OnCollision(projectileHitEvent: ref<gameprojectileHitEvent>) -> Bool {
    ...
    while i < ArraySize(projectileHitEvent.hitInstances) {
      ...
      if this.m_alive {
        ...
        if IsDefined(this.m_trackedTargetComponent) {
          this.m_trackedTargetComponent.OnTargetHit();
        };
        ...
        if !gameObj.HasTag(n"bullet_no_destroy") && this.m_BulletCollisionEvaluator.HasReportedStopped() && Equals(hitInstance.position, this.m_BulletCollisionEvaluator.GetStoppedPosition()) {
          ...
          if !this.m_hasExploded && IsDefined(this.m_attack) {
            ...
            explosionAttackRecord = IAttack.GetExplosiveHitAttack(this.m_attack.GetRecord());
            if IsDefined(explosionAttackRecord) {
              Attack_GameEffect.SpawnExplosionAttack(explosionAttackRecord, this.m_weapon as WeaponObject, this.m_owner, this, hitInstance.position, 0.05);
            };
          };
  ...
  private final func UpdateReadyToMiss() -> Void {
    ...
    if this.GetInitialDistanceToTarget() > 4.00 {
      ...
      if !this.m_readyToMiss && EntityID.IsDefined(this.m_targetID) {
        targetMissProbability = ClampF(this.m_statsSystem.GetStatValue(Cast<StatsObjectID>(this.m_targetID), gamedataStatType.SmartTargetingDisruptionProbability), 0.00, 1.00);
  ...
  private final func DisableTargetCollisions(targetID: EntityID) -> Void {
    if EntityID.IsDefined(targetID) && targetID != this.m_ignoredTargetID && !this.m_statsSystem.GetStatBoolValue(Cast<StatsObjectID>(targetID), gamedataStatType.SmartTargetingShouldNotDisableCollision) {
